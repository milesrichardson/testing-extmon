...
function make_popup(uid, e, index, bSecureSearch) {
  ...
  var config;
  if (document.all) {
    config = eval("(" + e.srcElement.parentNode.parentNode.getAttribute("mcBalloonConfig") + ")");
  } else {
    config = eval("( " + e.target.parentNode.parentNode.getAttribute("mcBalloonConfig") + ")");
  }
  var annoItem;
  if (document.all) {
    annoItem = eval("(" + e.srcElement.parentNode.parentNode.getAttribute("mcLinkinfo") + ")");
  } else {
    annoItem = eval("(" + e.target.parentNode.parentNode.getAttribute("mcLinkinfo") + ")");
  }
...
function OffsetLeft(b) {
  ...
  while (b.offsetParent) {
    ...
    if (b.className && b.className == "prel") {
      ...
      if (document.location.href.match(/uol\.com/) && b.tagName && b.tagName == "DT") {
...
function MoveBubble(i) {
  if (g_OkToMove) {
    ...
    if (d + (borderheight / 2 - hedge) > k + a - scrollbarheight) {
      g.style.top = Math.max(d - (borderheight - hedge), k + a - borderheight) + "px";
    } else {
...
function shut_ff(a) {
  try {
    if (
      !contains_ff(document.getElementById("BubbleLayer" + g_UID), a.relatedTarget) &&
      !contains_ff(document.getElementById("ArrowLayer" + g_UID), a.relatedTarget) &&
