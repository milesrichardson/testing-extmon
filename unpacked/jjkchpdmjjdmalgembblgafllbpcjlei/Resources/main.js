...
if (document.all) document.onmousewheel = BubbleHide;
else window.addEventListener("DOMMouseScroll", shut_ff, false);
...
var DefaultBalloonImages = {
  green: {
    backgroundImages: {
      ...
      BOTTOM_R: "url(" + g_path + "g_bottom_r.gif)", //"BOTTOM_SEP": "url("+g_path+"g_bottom_sep.gif)",
      FOOTER_L: "url(" + g_path + "g_footer_l.gif)",
      FOOTER_C: "url(" + g_path + "g_footer_c.gif)",
      FOOTER_R: "url(" + g_path + "g_footer_r.gif)"
    },
    srcImages: {
      ICON: g_path + "g_icon.gif",
      BANNER_SEP: "" + g_path + "g_banner_sep.gif" //, "UPSELL_SEP": "+g_path+g_upsell_border.gif"
    }
  },
  yellow: {
    backgroundImages: {
      ...
      BOTTOM_R: "url(" + g_path + "y_bottom_r.gif)", //"BOTTOM_SEP": "url("+g_path+"y_bottom_sep.gif)",
      FOOTER_L: "url(" + g_path + "y_footer_l.gif)",
      FOOTER_C: "url(" + g_path + "y_footer_c.gif)",
      FOOTER_R: "url(" + g_path + "y_footer_r.gif)"
    },
    srcImages: {
      ICON: "" + g_path + "y_icon.gif",
      BANNER_SEP: "" + g_path + "y_banner_sep.gif" //, "UPSELL_SEP": ""+g_path+"y_upsell_border.gif"
    }
  },
  red: {
    backgroundImages: {
      ...
      BOTTOM_R: "url(" + g_path + "r_bottom_r.gif)", //"BOTTOM_SEP": "url("+g_path+"r_bottom_sep.gif)",
      FOOTER_L: "url(" + g_path + "r_footer_l.gif)",
      FOOTER_C: "url(" + g_path + "r_footer_c.gif)",
      FOOTER_R: "url(" + g_path + "r_footer_r.gif)"
    },
    srcImages: {
      ICON: "" + g_path + "r_icon.gif",
      BANNER_SEP: "" + g_path + "r_banner_sep.gif" //, "UPSELL_SEP": ""+g_path+"r_upsell_border.gif"
    }
  },
  white: {
    backgroundImages: {
      ...
      BOTTOM_R: "url(" + g_path + "w_bottom_r.gif)", //"BOTTOM_SEP": "url("+g_path+"w_bottom_sep.gif)",
      FOOTER_L: "url(" + g_path + "w_footer_l.gif)",
      FOOTER_C: "url(" + g_path + "w_footer_c.gif)",
      FOOTER_R: "url(" + g_path + "w_footer_r.gif)"
    },
    srcImages: {
      ICON: "" + g_path + "w_icon.gif",
      BANNER_SEP: "" + g_path + "w_banner_sep.gif" //, "UPSELL_SEP": ""+g_path+"w_upsell_border.gif"
    }
  }
};
...
function popup(e, annoItem, facetList, config, bSecureSearch) {
  ...
  if (typeof CCList != "undefined" && CCList.length > 0 && colour != "white") {
    ...
    document.getElementById("CCDesc" + g_UID).href =
      typeof config.CCLinkDesc_URL != "undefined" && config.CCLinkDesc_URL != ""
        ? config.CCLinkDesc_URL
    ...
    for (var nIndex = 0; nIndex < CCList.length && nIndex < 3; nIndex++) {
      ...
      for (; nDup < nIndex; nDup++) {
        if (CCList[nIndex]["desc"] == document.getElementById("CC_DESC_" + nDup + g_UID).innerHTML) break;
      }
      if (nDup < nIndex) break;
    ...
    document.getElementById("CCHeader" + g_UID).innerHTML =
      "<span id='submitsitetxt' class='sastyle_link_upsell'>" + annoItem.balloonConfig.CCUpsell + "</span>";
    document.getElementById("BOTTOM_SEP" + g_UID).style.borderLeft = "solid 1px Gray";
...
function MoveBubble(e) {
  if (g_OkToMove) {
    ...
    if (baseX + scrollbarwidth + borderwidth > pageXOffset + innerWidth) {
      // try left side
      if (document.all) {
...
function shut_ie() {
  ...
  try {
    if (
      !document.getElementById("BubbleLayer" + g_UID).contains(e.toElement) &&
      !document.getElementById("ArrowLayer" + g_UID).contains(e.toElement) &&
      !g_icon.contains(e.toElement)
    )
      BubbleHide();
  } catch (e) {}
}
...
function shut_ff(e) {
  try {
    if (
      !contains_ff(document.getElementById("BubbleLayer" + g_UID), e.relatedTarget) &&
      !contains_ff(document.getElementById("ArrowLayer" + g_UID), e.relatedTarget) &&
      typeof g_icon != "undefined" &&
      !contains_ff(g_icon, e.relatedTarget)
    )
      BubbleHide();
  } catch (e) {}
}
...
function make_popup(uid, e, index, bSecureSearch) {
  ...
  if (!document.all && contains_ff(document.getElementById("BALLOON" + g_UID), e.relatedTarget)) return;

  if (typeof annoItem.linkTitleEncoded == "undefined") annoItem.linkTitleEncoded = annoItem.base_domain;
...
PopUtil.prototype.getSiteReportLink = function getSiteReportLink(config, annoItem) {
  ...
  return (
    ...
    hrefurl +
    "' target=_blank>" +
    config.siteReport.linkText +
    "</a>"
  );
};
PopUtil.prototype.getSubmitForTestingLink = function getSubmitForTestingLink(config) {
  return (
    "<a style='text-decoration:none' href='#' onclick='return false;'><img id='submitsiteimg' border='0' style='display:inline;margin-bottom:12px;width:16px;font-size:medium;' src='" +
...
PopUtil.prototype.getUpsellLink = function getUpsellLink(config) {
  ...
  if (this.displayUpsell(config)) {
    upsellLink =
      ...
      "<td style='width:2px;height:24px;background:url(" +
      g_path +
      "upsell-l.gif) 0 0 no-repeat;'></td>" +
      "<td class='sastyle_link_upsell' style='cursor:default;height:24px;background:url(" +
      g_path +
      "upsell-m.gif) 0 0 repeat-x;text-decoration:none;white-space:nowrap;color:black;line-height:normal;'>" +
      "<a class='sastyle_link_upsell' style='padding:0 10px;text-decoration:none;font-weight:normal' target='_blank' href='" +
      config.upsellInfo.url +
      "'>" +
      config.upsellInfo.message +
...
PopUtil.prototype.populateFacets = function populateFacets(config, annoItem) {
  ...
  for (var fi in facetList) {
    ...
    url =
      dssUrlInfo.siteReportBaseUrl +
      annoItem.domain_specifier +
      "/" +
      FacetToPageName[facetInfo.name] +
      "?" +
      dssUrlInfo.requestArgs +
      "&ref=safesearch";
