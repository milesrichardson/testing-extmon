...
function handleSASite() {
  ...
  if (wlAnchor && wlAnchor.href.indexOf("about:blank") != -1) {
    ...
    wlAnchor.addEventListener("click", function () {
      handleSpecialURL(strOrigUrl);
    });
  }
}
...
function handleChromeDssRequest(doc, url, requestListData, callback) {
  ...
  chrome.extension.sendRequest({ command: "dss query", dssUrl: url, dssData: dsspostdata }, function (response) {
    callback(response.dssResponse, null);
  });
}
function onResourcesReady(response) {
  ...
  try {
    response.htmlRes["balloon.html"] = response.htmlRes["balloon.html"].replace(
      new RegExp("sacore:empty.gif", "g"),
      response.imageDatamap["empty.gif"]
    );
    ...
    if (typeof annotate != "undefined") fnAnnotate = annotate;
    if (typeof showWarningBanner != "undefined") fnShowWarning = showWarningBanner;
