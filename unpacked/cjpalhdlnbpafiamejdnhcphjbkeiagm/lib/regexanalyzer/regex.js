...
export default (function (root, name, factory) {
  ...
  var rnd = function (a, b) {
    ...
    character_range = function (first, last) {
      ...
      var ch,
        chars,
        start = first[CODEPOINT](0),
        end = last[CODEPOINT](0);
    ...
    walk = function walk(ret, node, state) {
      ...
      var i,
        l,
        r,
        type = node instanceof Node ? node.type : null;
    ...
    map_src = function map_src(ret, node, state) {
      ...
      if (T_ALTERNATION === type) {
        ...
        return []
          .concat("[" + (node.flags.NegativeMatch ? "^" : ""))
          .concat(array(node.val))
          .concat("]");
        ...
        if (node.flags.MatchZeroOrOne) q = "?";
          ...
          q =
            node.flags.min === node.flags.max
              ? "{" + node.flags.min + "}"
              : "{" + node.flags.min + "," + (-1 === node.flags.max ? "" : node.flags.max) + "}";
    ...
    map_max = function map_max(ret, node, state) {
      ...
      if (T_ALTERNATION === type) {
        var i,
          l = node.val.length,
          cur,
          max = l ? walk(0, node.val[0], state) : 0;
        if (-1 !== max) {
          for (i = 1; i < l; ++i) {
            cur = walk(0, node.val[i], state);
            if (-1 === cur) {
    ...
    map_1st = function map_1st(ret, node, state) {
      ...
      if (T_SEQUENCE === type) {
        ...
        for (i = 0; i < l; ++i) {
          n = node.val[i];
          seq.push(n);
          if (T_QUANTIFIER === n.type && 0 === n.flags.min) continue;
          else if (T_SPECIAL === n.type && (n.flags.MatchStart || n.flags.MatchEnd)) continue;
          break;
        }
    ...
    reduce_str = function reduce_str(ret, node, state) {
      ...
      if (sample) {
        ret +=
          T_STRING === type
            ? state.isCaseInsensitive
              ? case_insensitive(sample)
              : sample
    ...
    reduce_src = function reduce_src(ret, node, state) {
      ...
      if (T_CHARS === type) {
        ...
        var range = [node.val[0], node.val[1]];
        if (state.escaped) {
          if (T_UNICODECHAR === range[0].type)
            range[0] = range[0].flags.UnicodePoint ? ESC + "u{" + range[0].flags.Code + "}" : ESC + "u" + pad(range[0].flags.Code, 4);
          else if (T_HEXCHAR === range[0].type) range[0] = ESC + "x" + pad(range[0].flags.Code, 2);
          else range[0] = esc_re(range[0], ESC, 1);
          if (T_UNICODECHAR === range[1].type)
            range[1] = range[1].flags.UnicodePoint ? ESC + "u{" + range[1].flags.Code + "}" : ESC + "u" + pad(range[1].flags.Code, 4);
        ...
        ret.src += node.flags.UnicodePoint
          ? ESC + "u{" + node.flags.Code + "}"
          : state.escaped
          ? ESC + "u" + pad(node.flags.Code, 4)
          : node.flags.Char;
    ...
    reduce_peek = function reduce_peek(ret, node, state) {
      ...
      var type = node.type,
        inCharGroup = state.node && T_CHARGROUP === state.node.type,
    ...
    chargroup = function chargroup(re_obj) {
      ...
      while (re_obj.pos < lre) {
        ...
        if (escaped) {
          ...
          if ("u" === ch) {
            ...
            if (m) {
              ...
              ch = Node(T_UNICODECHAR, m[0], {
                Char: m[2] ? fromCodePoint(parseInt(m[1], 16)) : fromCharCode(parseInt(m[1], 16)),
                Code: m[1],
                UnicodePoint: !!m[2]
              });
              isUnicode = true;
        ...
        if (isRange) {
          if (ch instanceof Node && ch.type === T_SPECIAL && -1 !== ["s", "S", "d", "D", "w", "W"].indexOf(ch.val)) {
          ...
          if (escaped) {
            ...
            if ("]" === ch) {
              ...
              if (
                ...
                (prevch instanceof Node && prevch.type === T_SPECIAL && -1 !== ["s", "S", "d", "D", "w", "W"].indexOf(prevch.val))
              ) {
                // take it as literal
        ...
        if (escaped) {
          ...
          // parse num repeats
            ...
            flag = {
              val: m[0],
              MatchMinimum: m[1],
              MatchMaximum: m[2] || "unlimited",
              min: parseInt(m[1], 10),
              max: m[2] ? parseInt(m[2], 10) : -1
            };
      ...
      for (n in peek) {
        ...
        for (c in p) {
          if ("\\d" === c) {
            ...
            cases = concat(
              cases,
              ["_"].concat(character_range("0", "9")).concat(character_range("a", "z")).concat(character_range("A", "Z"))
            );
          } else if ("\\b" === c) {
            delete p[c];
...
*/
  ...
  Composer[PROTO] = {
    ...
    token: function (token, escaped) {
      var self = this;
      if (null != token) self.ast[self.level].node.push(escaped ? esc_re(String(token), ESC) : String(token));
      return self;
    },
    ...
    repeat: function (min, max, greedy) {
      ...
      var repeat =
        (null == max || min === max ? "{" + String(min) + "}" : "{" + String(min) + "," + String(max) + "}") + (!greedy ? "?" : "");
    ...
    lookBehindGroup: function (positive) {
      ...
      while (n--) {
        ...
        if (0 < self.level) {
          --self.level;
          if (T_ALTERNATION === type) self.ast[self.level].node.push(part.join("|"));
          else if (T_GROUP === type) self.ast[self.level].node.push("(" + flag + part.join("") + ")");
          else if (T_CHARGROUP === type) self.ast[self.level].node.push("[" + flag + part.join("") + "]");
          else self.ast[self.level].node.push(part.join(""));
        }
      }
      return self;
    }
