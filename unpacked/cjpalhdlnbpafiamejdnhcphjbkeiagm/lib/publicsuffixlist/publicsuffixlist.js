...
*/
  ...
  const toSelfie = function (encoder) {
    ...
    return {
      magic: SELFIE_MAGIC,
      buf32: Array.from(new Uint32Array(pslBuffer8.buffer, 0, pslByteLength >>> 2))
    };
  };
  ...
  const fromSelfie = function (selfie, decoder) {
    ...
    } else if (selfie instanceof Object && selfie.magic === SELFIE_MAGIC && Array.isArray(selfie.buf32)) {
  ...
  const enableWASM = (() => {
    ...
    const getWasmInstance = async function (wasmModuleFetcher, path) {
      ...
      try {
        ...
        const pageCount = pslBuffer8 !== undefined ? (pslBuffer8.byteLength + 0xffff) >>> 16 : 1;
        ...
        const newPageCount = pslBuffer8 !== undefined ? (pslBuffer8.byteLength + 0xffff) >>> 16 : 0;
