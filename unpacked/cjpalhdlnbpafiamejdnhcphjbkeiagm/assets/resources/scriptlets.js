...
function safeSelf() {
  ...
  const safe = {
    ...
    uboLog(...args) {
      if (scriptletGlobals.has("canDebug") === false) {
        return;
      }
      if (args.length === 0) {
        return;
    ...
    initPattern(pattern, options = {}) {
      ...
      return {
        pattern,
        re: new this.RegExp(pattern.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), options.flags),
        expect
      };
    },
...
function getExceptionToken() {
  const safe = safeSelf();
  const token = String.fromCharCode((Date.now() % 26) + 97) + safe.Math_floor(safe.Math_random() * 982451653 + 982451653).toString(36);
...
builtinScriptlets.push({
  ...
  dependencies: ["get-exception-token.fn", "safe-self.fn", "should-debug.fn", "should-log.fn"]
});
...
function abortCurrentScriptCore(target = "", needle = "", context = "") {
  ...
  const validate = () => {
    ...
    if (log) {
      safe.uboLog(`matched script text: ${scriptText}`);
    }
...
function setConstantCore(trusted = false, chain = "", cValue = "") {
  if (chain === "") {
    return;
  }
  const safe = safeSelf();
  ...
  function setConstant(chain, cValue) {
    ...
    if (cValue === "undefined") {
      ...
      if (cValue.startsWith("{") && cValue.endsWith("}")) {
        try {
          cValue = safe.JSON_parse(cValue).value;
        } catch (ex) {
          return;
        }
      }
    ...
    const trapProp = function (owner, prop, configurable, handler) {
      if (handler.init(configurable ? owner[prop] : cValue) === false) {
        return;
      }
...
builtinScriptlets.push({
  ...
  fn: objectPruneFn,
  dependencies: ["matches-stack-trace.fn", "object-find-owner.fn", "safe-self.fn", "should-log.fn"]
});
...
function objectPruneFn(obj, rawPrunePaths, rawNeedlePaths, stackNeedleDetails = { matchAll: true }, extraArgs = {}) {
...
function objectFindOwnerFn(root, path, prune = false) {
  ...
  for (;;) {
    ...
    const prop = chain.slice(0, pos);
    if ((prop === "[]" && Array.isArray(owner)) || (prop === "*" && owner instanceof Object)) {
...
function setCookieFn(trusted = false, name = "", value = "", expires = "", path = "", options = {}) {
  ...
  if (cookieBefore !== undefined && options.dontOverwrite) {
    return;
  }
  if (cookieBefore === value && options.reload) {
    return;
  }
...
function setLocalStorageItemFn(which = "local", trusted = false, key = "", value = "") {
  if (key === "") {
    return;
  }
...
function matchesStackTrace(needleDetails, logLevel = 0) {
  const safe = safeSelf();
  const exceptionToken = getExceptionToken();
  ...
  if (logLevel === 1 || (logLevel === 2 && r) || (logLevel === 3 && !r)) {
...
builtinScriptlets.push({
  ...
  dependencies: ["match-object-properties.fn", "object-prune.fn", "parse-properties-to-match.fn", "safe-self.fn", "should-log.fn"]
});
...
builtinScriptlets.push({
  ...
  dependencies: ["match-object-properties.fn", "parse-properties-to-match.fn", "safe-self.fn", "should-log.fn"]
});
        ...
        .then((responseBefore) => {
          ...
          return response
            ...
            .then((textBefore) => {
              ...
              if (outcome === logLevel || logLevel === "all") {
                log(`replace-fetch-response (${outcome})`, `\n\tpattern: ${pattern}`, `\n\treplacement: ${replacement}`);
              }
              if (outcome === "nomatch") {
                return responseBefore;
              }
...
builtinScriptlets.push({
  ...
  dependencies: ["get-exception-token.fn"]
});
...
builtinScriptlets.push({
  ...
  dependencies: ["run-at.fn", "safe-self.fn", "should-debug.fn", "should-log.fn"]
});
...
function addEventListenerDefuser(type = "", pattern = "") {
  ...
  const trapEddEventListeners = () => {
    ...
    self.EventTarget.prototype.addEventListener = new Proxy(self.EventTarget.prototype.addEventListener, eventListenerHandler);
  };
  runAt(() => {
...
builtinScriptlets.push({
  name: "no-fetch-if.js",
  aliases: ["prevent-fetch.js"],
  fn: noFetchIf,
  dependencies: ["safe-self.fn"]
});
function noFetchIf(arg1 = "") {
    ...
    apply: function (target, thisArg, args) {
      ...
      try {
        ...
        for (const { key, re } of needles) {
          if (props.has(key) === false || re.test(props.get(key)) === false) {
            proceed = true;
            break;
          }
        }
      } catch (ex) {}
      return proceed ? Reflect.apply(target, thisArg, args) : Promise.resolve(new Response());
    }
  });
}
...
function preventRefresh(arg1 = "") {
  ...
  const defuse = () => {
    ...
    const s = arg1 === "" ? meta.getAttribute("content") : arg1;
...
function removeAttr(token = "", selector = "", behavior = "") {
...
function removeClass(token = "", selector = "", behavior = "") {
  if (typeof token !== "string") {
    return;
  }
  if (token === "") {
...
builtinScriptlets.push({
  ...
  aliases: ["set.js"],
  fn: setConstant,
  dependencies: ["set-constant-core.fn"]
});
...
function noSetIntervalIf(needle = "", delay = "") {
  ...
  const log = needleNot === false && needle === "" && delay === undefined ? console.log : undefined;
  const reNeedle = safe.patternToRegex(needle);
  self.setInterval = new Proxy(self.setInterval, {
    apply: function (target, thisArg, args) {
      const a = String(args[0]);
...
function webrtcIf(good = "") {
  ...
  const rtcName = window.RTCPeerConnection ? "RTCPeerConnection" : window.webkitRTCPeerConnection ? "webkitRTCPeerConnection" : "";
  if (rtcName === "") {
    return;
  }
  ...
  const isGoodConfig = function (instance, config) {
    if (neuteredPeerConnections.has(instance)) {
      return false;
    }
    if (config instanceof Object === false) {
      return true;
    }
    if (Array.isArray(config.iceServers) === false) {
      return true;
    }
    for (const server of config.iceServers) {
      ...
      if (typeof server.username === "string") {
        if (reGood.test(server.username)) {
          return true;
        }
      }
      if (typeof server.credential === "string") {
        if (reGood.test(server.credential)) {
          return true;
        }
      }
    }
  ...
  peerConnectionProto.createDataChannel = new Proxy(peerConnectionProto.createDataChannel, {
    apply: function (target, thisArg, args) {
  ...
  window[rtcName] = new Proxy(peerConnectionCtor, {
...
builtinScriptlets.push({
  ...
  fn: noXhrIf,
  dependencies: ["match-object-properties.fn", "parse-properties-to-match.fn", "safe-self.fn"]
});
function noXhrIf(propsToMatch = "", directive = "") {
  ...
  const generateRandomString = (len) => {
    let s = "";
    do {
      s += safe.Math_random().toString(36).slice(2);
    } while (s.length < 10);
...
builtinScriptlets.push({
  ...
  fn: noWindowOpenIf,
  dependencies: ["safe-self.fn", "should-log.fn"]
});
...
function golemDe() {
  ...
  window.addEventListener = function (a, b) {
    ...
    if (typeof haystack === "string" && /^\s*function\s*\(\)\s*\{\s*window\.clearTimeout\(r\)\s*\}\s*$/.test(haystack)) {
      b();
    }
  }.bind(window);
}
...
function multiup() {
  const handler = (ev) => {
    ...
    if (target.matches("button[link]") === false) {
      return;
    }
    ...
    if (ancestor !== target.parentElement) {
      return;
    }
...
function trustedSetCookie(name = "", value = "", offsetExpiresSec = "", path = "") {
  ...
  if (offsetExpiresSec !== "") {
    if (offsetExpiresSec === "1day") {
      ...
      if (/^\d+$/.test(offsetExpiresSec) === false) {
        return;
      }
  ...
  setCookieFn(true, name, value, expires, path, safeSelf().getExtraArgs(Array.from(arguments), 4));
}
...
function trustedReplaceXhrResponse(pattern = "", replacement = "", propsToMatch = "") {
  ...
  self.XMLHttpRequest = class extends self.XMLHttpRequest {
    ...
    get response() {
      ...
      const outcome = textAfter !== textBefore ? "match" : "nomatch";
      if (outcome === logLevel || logLevel === "all") {
        log(`trusted-replace-xhr-response (${outcome})`, `\n\tpattern: ${pattern}`, `\n\treplacement: ${replacement}`);
      }
...
builtinScriptlets.push({
  ...
  world: "ISOLATED",
  dependencies: ["run-at-html-element.fn", "safe-self.fn"]
});
function trustedClickElement(
  ...
  const extraArgs = safe.getExtraArgs(Array.from(arguments), 3);
  const uboLog =
    extraArgs.log !== undefined
      ? (...args) => {
          safe.uboLog(...args);
        }
      : () => {};

  const selectorList = selectors.split(/\s*,\s*/).filter((s) => {
    try {
...
builtinScriptlets.push({
  ...
  dependencies: ["object-find-owner.fn", "object-prune.fn", "safe-self.fn"]
});
function trustedPruneInboundObject(entryPoint = "", argPos = "", rawPrunePaths = "", rawNeedlePaths = "") {
  ...
  context[prop] = new Proxy(context[prop], {
    apply: function (target, thisArg, args) {
      const targetArg = argIndex <= args.length ? args[argIndex - 1] : undefined;
...
function trustedPruneOutboundObject(entryPoint = "", rawPrunePaths = "", rawNeedlePaths = "") {
  if (entryPoint === "") {
    return;
  }
  let context = globalThis;
  ...
  context[prop] = new Proxy(context[prop], {
    apply: function (target, thisArg, args) {
      const objBefore = Reflect.apply(target, thisArg, args);
      if (objBefore instanceof Object === false) {
        return objBefore;
      }
      const objAfter = objectPruneFn(objBefore, rawPrunePaths, rawNeedlePaths, { matchAll: true }, extraArgs);
      return objAfter || objBefore;
