...
const encodeValue = function (lz4CodecInstance, dataIn) {
  ...
  if (outputArray instanceof Uint8Array === false) {
    return;
  }
  outputArray[0] = 0x18;
  outputArray[1] = 0x4d;
...
const decodeValue = function (lz4CodecInstance, inputArray) {
  ...
  //let t0 = window.performance.now();
  if (inputArray[0] !== 0x18 || inputArray[1] !== 0x4d || inputArray[2] !== 0x22 || inputArray[3] !== 0x04) {
    console.error("decodeValue: invalid input array");
    return;
  }
  const outputSize = (inputArray[4] << 0) | (inputArray[5] << 8) | (inputArray[6] << 16) | (inputArray[7] << 24);
