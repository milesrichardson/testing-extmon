...
(() => {
  ...
  if (vAPI.commands instanceof Object === false) {
    return;
  }
  ...
  const relaxBlockingMode = (() => {
    ...
    return function (tab) {
      if (tab instanceof Object === false || tab.id <= 0) {
        return;
      }

      const normalURL = µb.normalizeTabURL(tab.id, tab.url);

      if (µb.getNetFilteringSwitch(normalURL) === false) {
        return;
      }
      ...
      if (newProfileBits === undefined) {
        return;
      }

      const noReload = (newProfileBits & 0b00000001) === 0;

      if ((curProfileBits & 0b00000010) !== 0 && (newProfileBits & 0b00000010) === 0) {
      ...
      if (µb.userSettings.advancedUserEnabled) {
        if ((curProfileBits & 0b00000100) !== 0 && (newProfileBits & 0b00000100) === 0) {
        ...
        if ((curProfileBits & 0b00001000) !== 0 && (newProfileBits & 0b00001000) === 0) {
          µb.toggleFirewallRule({
            srcHostname: hn,
            desHostname: "*",
        ...
        if ((curProfileBits & 0b00010000) !== 0 && (newProfileBits & 0b00010000) === 0) {
          µb.toggleFirewallRule({
            srcHostname: hn,
            desHostname: "*",
  ...
  vAPI.commands.onCommand.addListener(async (command) => {
    ...
    switch (command) {
      ...
      case "launch-element-zapper": {
        ...
        µb.elementPickerExec(tab.id, 0, undefined, command === "launch-element-zapper");
        break;
      }
      case "launch-logger": {
        const hash = tab.url.startsWith(vAPI.getURL("")) ? "" : `#_+${tab.id}`;
        µb.openNewTab({
