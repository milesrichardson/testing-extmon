...
const MODIFY_REALMS = REDIRECT_REALM | CSP_REALM | REMOVEPARAM_REALM | PERMISSIONS_REALM | URLTRANSFORM_REALM | REPLACE_REALM;
...
const bitFromType = (type) => 1 << ((typeNameToTypeValue[type] >>> TypeBitsOffset) - 1);
...
const allNetworkTypesBits = (1 << (otherTypeBitValue >>> TypeBitsOffset)) - 1;
...
const unsupportedTypeBit = 1 << ((typeNameToTypeValue["unsupported"] >>> TypeBitsOffset) - 1);
...
const restrFromGenericPattern = function (s, anchor = 0) {
  let reStr = s
    .replace(restrFromGenericPattern.rePlainChars, "\\$&")
...
class LogData {
  constructor(categoryBits, tokenHash, iunit) {
    ...
    if (logData.isRegex === false && raw.charCodeAt(0) === 0x2f /* '/' */ && raw.charCodeAt(raw.length - 1) === 0x2f /* '/' */) {
      raw += "*";
    }
...
const filterDataShrink = () => {
  const newLen = Math.max(roundToFilterDataPageSize(filterDataWritePtr), FILTER_DATA_PAGE_SIZE);
...
const filterDataFromSelfie = (selfie) => {
  if (typeof selfie !== "string" || selfie === "") {
    return false;
  }
...
const filterRefsToSelfie = () => {
  ...
  for (let i = 0; i < filterRefsWritePtr; i++) {
    ...
    for (const prop of Object.keys(v)) {
      const value = v[prop];
      out[prop] = prop.startsWith("$") ? (typeof value === "string" ? "" : null) : value;
    }
    refs.push({ t: 3, v: out });
  }
...
const filterGetRegexPattern = (idata, out) => {
  const fc = filterClasses[filterData[idata + 0]];
  if (fc.hasRegexPattern === undefined) {
    return;
  }
...
const filterIsBidiTrieable = (idata) => {
  const fc = filterClasses[filterData[idata + 0]];
  if (fc.isBidiTrieable === undefined) {
    return false;
  }
...
const filterMatchAndFetchModifiers = (idata, env) => {
  const fc = filterClasses[filterData[idata + 0]];
  if (fc.matchAndFetchModifiers === undefined) {
    return;
  }
...
const filterGetModifierType = (idata) => {
  const fc = filterClasses[filterData[idata + 0]];
  if (fc.getModifierType === undefined) {
    return;
  }
...
const dnrRuleFromCompiled = (args, rule) => {
  const fc = filterClasses[args[0]];
  if (fc.dnrFromCompiled === undefined) {
    return false;
  }
...
class FilterPatternPlain {
  ...
  static match(idata) {
    ...
    if (bidiTrie.startsWith(left, bidiTrie.haystackLen, filterData[idata + 1], n) === 0) {
      return false;
    }
  ...
  static dumpInfo(idata) {
    const pattern = bidiTrie.extractString(filterData[idata + 1], filterData[idata + 2]);
...
class FilterPatternGeneric {
  ...
  static getRegexPattern(idata) {
    return restrFromGenericPattern(bidiTrie.extractString(filterData[idata + 1], filterData[idata + 2]), filterData[idata + 3]);
  }
  ...
  static compile(details) {
    const out = [FilterPatternGeneric.fid, details.pattern, details.anchor];
    details.anchor = 0;
    return out;
  }
  ...
  static dumpInfo(idata) {
    return bidiTrie.extractString(filterData[idata + 1], filterData[idata + 2]);
  }
}
...
class FilterAnchorHnLeft {
  static match(idata) {
    ...
    if (
      ...
      if (lastBeg !== -1) {
        if (lastBeg >= bidiTrie.haystackLen || haystackCodes[lastBeg + 1] !== 0x2f || haystackCodes[lastBeg + 2] !== 0x2f) {
          lastBeg = -1;
        }
      }
      if (lastBeg !== -1) {
    ...
    return left < lastEnd && (left === lastBeg || (left > lastBeg && haystackCodes[left - 1] === 0x2e)) /* '.' */;
  }
...
class FilterRegex {
  ...
  static match(idata) {
    const refs = filterRefs[filterData[idata + 4]];
    if (refs.$re === null) {
      refs.$re = new RegExp(this.getRegexPattern(idata), filterData[idata + 3] === 0 ? "i" : "");
    }
  ...
  static compile(details) {
    return [FilterRegex.fid, details.pattern, details.patternMatchCase ? 1 : 0];
  }
...
class FilterMethod {
  static match(idata) {
    ...
    return (
      (methodBits !== 0 && ($requestMethodBit & methodBits) !== 0) || (notMethodBits !== 0 && ($requestMethodBit & notMethodBits) === 0)
    );
  }
...
class FilterNotType {
  ...
  static dnrFromCompiled(args, rule) {
    ...
    for (let i = 1; bits !== 0 && i < typeValueToDNRTypeName.length; i++) {
      ...
      if (rc.excludedResourceTypes.includes(type)) {
        continue;
      }
...
const compileDomainOpt = (ctors, iterable, prepend, units) => {
  ...
  const compiledHit = [];
  if (toTrie.length !== 0) {
    compiledHit.push(ctors[2].compile(toTrie.sort(), trieWhich));
  }
  ...
  const compiledMiss = [];
  if (toTrie.length !== 0) {
    compiledMiss.push(ctors[6].compile(toTrie.sort(), trieWhich));
  }
...
class FilterDomainHit {
  static getDomainOpt(idata) {
    return this.hntrieContainer.extractHostname(filterData[idata + 1], filterData[idata + 2]);
  }

  static match(idata) {
    return this.hntrieContainer.matchesHostname(this.getMatchTarget(), filterData[idata + 1], filterData[idata + 2]);
  }
...
class FilterDomainHitSet {
  static getDomainOpt(idata) {
    return this.hntrieContainer.extractDomainOpt(filterData[idata + 1], filterData[idata + 2]);
  }
  ...
  static getTrieCount(idata) {
    ...
    return Array.from(this.hntrieContainer.trieIterator(filterData[idata + 4])).length;
  }
  ...
  static match(idata) {
    ...
    if ((which & 0b10) !== 0) {
      const pos = this.hntrieContainer.setNeedle(this.getMatchTarget(0b10)).matches(itrie);
      if (pos !== -1) {
  ...
  static add(idata, hn) {
    ...
    filterData[idata + 3] |= hn.charCodeAt(hn.length - 1) !== 0x2a /* '*' */ ? 0b01 : 0b10;
    filterData[idata + 5] = -1;
...
class FilterFromDomainHitSet extends FilterDomainHitSet {
  ...
  static getMatchTarget(which) {
    return (which & 0b01) !== 0 ? $docHostname : $docEntity.compute();
  }
...
class FilterToDomainHitSet extends FilterDomainHitSet {
  static getMatchTarget(which) {
    return (which & 0b01) !== 0 ? $requestHostname : $requestEntity.compute();
  }
...
class FilterModifier {
  ...
  static compile(details) {
    return [FilterModifier.fid, details.action, details.modifyType, details.modifyValue || ""];
  }
...
class FilterCollection {
  ...
  static create(fid = -1) {
    return filterDataAlloc(fid !== -1 ? fid : FilterCollection.fid, 0);
  }
...
class FilterCompositeAll extends FilterCollection {
  ...
  static hasRegexPattern(idata) {
    return (
      this.forEach(idata, (iunit) => {
        ...
        if (fc.hasRegexPattern(iunit) === true) {
          return true;
        }
      }) || false
    );
  ...
  static getRegexPattern(idata) {
    return this.forEach(idata, (iunit) => {
      const fc = filterGetClass(iunit);
      if (fc.getRegexPattern === undefined) {
        return;
      }
...
class FilterPlainTrie {
  ...
  static addUnitToTrie(idata, iunit) {
    ...
    const itrie = filterData[idata + 1];
    const id = bidiTrie.add(itrie, trieDetails.i, trieDetails.n, trieDetails.itok);
  ...
  static optimize(idata, optimizeBits = 0b11) {
    ...
    if ((optimizeBits & 0b10) !== 0) {
      if (filterData[idata + 2] >= 5) {
        const ioptimized = this.optimizeMatch(idata, FilterBucketIfOriginHits, 5);
        if (ioptimized !== 0) {
          const icollection = filterData[idata + 1];
          const i = filterData[icollection + 1];
          if (i === 0) {
      ...
      if (filterData[idata + 2] >= 5) {
        const ioptimized = this.optimizeMatch(idata, FilterBucketIfRegexHits, 5);
        if (ioptimized !== 0) {
          const icollection = filterData[idata + 1];
          const i = filterData[icollection + 1];
          if (i === 0) {
...
class FilterBucketIfOriginHits extends FilterBucketIf {
  ...
  static create(coalesced, ibucket) {
    ...
    const ipretest = super.create(FilterBucketIfOriginHits.fid, ibucket, ihittest);
    return ipretest;
  }
}
...
class FilterBucketIfRegexHits extends FilterBucketIf {
  ...
  static create(coalesced, ibucket) {
    ...
    return super.create(FilterBucketIfRegexHits.fid, ibucket, filterRefAdd(new RegExp(reString, "i")));
  }
...
class FilterStrictParty {
  ...
  static compile(details) {
    return [FilterStrictParty.fid, details.strictParty > 0 ? 0 : 1];
  }
  ...
  static logData(idata, details) {
    details.options.push(filterData[idata + 1] === 0 ? "strict1p" : "strict3p");
  }
}
...
class FilterOnHeaders {
  static match(idata) {
    ...
    return re === undefined ? (headerValue === value) !== not : re.test(headerValue) !== not;
  }
...
const urlTokenizer = new (class {
  ...
  toSelfie() {
    return sparseBase64.encode(this.knownTokens.buffer, this.knownTokens.byteLength);
  }
...
class FilterCompiler {
  ...
  processOptionWithValue(parser, id) {
    switch (id) {
      ...
      case sfp.NODE_TYPE_NET_OPTION_NAME_DENYALLOW:
        this.denyallowOpt = this.processHostnameList(parser.getNetFilterDenyallowOptionIterator());
      ...
      case sfp.NODE_TYPE_NET_OPTION_NAME_FROM:
        this.fromDomainOpt = this.processHostnameList(parser.getNetFilterFromOptionIterator(), this.fromDomainOptList);
      ...
      case sfp.NODE_TYPE_NET_OPTION_NAME_REDIRECT: {
        const actualId = this.action === ALLOW_REALM ? sfp.NODE_TYPE_NET_OPTION_NAME_REDIRECTRULE : id;
      ...
      case sfp.NODE_TYPE_NET_OPTION_NAME_TO:
        this.toDomainOpt = this.processHostnameList(parser.getNetFilterToOptionIterator(), this.toDomainOptList);
        if (this.toDomainOpt === "") {
          return false;
        }
    ...
    for (const type of parser.getNodeTypes()) {
      switch (type) {
        ...
        case sfp.NODE_TYPE_NET_OPTION_NAME_EMPTY: {
          const id = this.action === ALLOW_REALM ? sfp.NODE_TYPE_NET_OPTION_NAME_REDIRECTRULE : sfp.NODE_TYPE_NET_OPTION_NAME_REDIRECT;
  ...
  extractTokenFromRegex(pattern) {
    ...
    for (;;) {
      ...
      if (token.length < MAX_TOKEN_LENGTH && (lastIndex === pattern.length || pattern.charAt(lastIndex) === "\x01")) {
        continue;
      }
      const badness = token.length > 1 ? this.badTokens.get(token) || 0 : 1;
      if (badness < bestBadness) {
  ...
  extractTokenFromQuerypruneValue() {
    ...
    if (match !== null) {
      return this.extractTokenFromRegex(match[1].replace(/(\{\d*)\\,/, "$1,"));
    }
    if (pattern.startsWith("|")) {
  ...
  domainIsEntity(s) {
    const l = s.length;
    return l > 2 && s.charCodeAt(l - 1) === 0x2a /* '*' */ && s.charCodeAt(l - 2) === 0x2e /* '.' */;
  }
  ...
  compile(parser, writer) {
    ...
    writer.select(this.badFilter ? "NETWORK_FILTERS:BAD" : "NETWORK_FILTERS:GOOD");
  ...
  compileToFilter(writer) {
    ...
    // Origin
    if (this.fromDomainOpt !== "") {
      compileFromDomainOpt(this.fromDomainOptList, units.length !== 0 && patternClass.isSlow === true, units);
    }

    // Destination
    if (this.toDomainOpt !== "") {
      compileToDomainOpt(this.toDomainOptList, units.length !== 0 && patternClass.isSlow === true, units);
    }
    ...
    if (this.modifyType !== undefined) {
      ...
      this.action = (this.action & ~ActionBitsMask) | modifierBitsFromType.get(this.modifyType);
    }

    this.compileToAtomicFilter(units.length === 1 ? units[0] : FilterCompositeAll.compile(units), writer);
  }
  ...
  compileToAtomicFilter(fdata, writer) {
    ...
    do {
      if (typeBits & 1) {
        writer.push([catBits | (bitOffset << TypeBitsOffset), this.tokenHash, fdata]);
      }
      bitOffset += 1;
      typeBits >>>= 1;
    } while (typeBits !== 0);
...
FilterContainer.prototype.prime = function () {
  origHNTrieContainer.reset(keyvalStore.getItem("SNFE.origHNTrieContainer.trieDetails"));
  destHNTrieContainer.reset(keyvalStore.getItem("SNFE.destHNTrieContainer.trieDetails"));
  bidiTriePrime();
};
...
FilterContainer.prototype.freeze = function () {
  ...
  for (const line of this.goodFilters) {
    ...
    if ((bits & ActionBitsMask) === BLOCKIMPORTANT_REALM) {
      this.addFilterUnit(bits & ~IMPORTANT_REALM, tokenHash, filterFromCompiled(fdata));
    }
  }
...
FilterContainer.prototype.dnrFromCompiled = function (op, context, ...args) {
  ...
  for (const rule of ruleset) {
    ...
    if (Array.isArray(rule.condition.initiatorDomains) && rule.condition.initiatorDomains.some((hn) => hn.endsWith(".*"))) {
      const domains = rule.condition.initiatorDomains.filter((hn) => hn.endsWith(".*") === false);
      if (domains.length === 0) {
        ...
        dnrAddRuleWarning(
          ...
          `Salvaged rule by ignoring ${
            rule.condition.initiatorDomains.length - domains.length
          } entity-based domain= option: ${rule.condition.initiatorDomains.join("|")}`
        );
        rule.condition.initiatorDomains = domains;
      }
    }
    if (Array.isArray(rule.condition.excludedInitiatorDomains) && rule.condition.excludedInitiatorDomains.some((hn) => hn.endsWith(".*"))) {
      const domains = rule.condition.excludedInitiatorDomains.filter((hn) => hn.endsWith(".*") === false);
      rule.condition.excludedInitiatorDomains = domains.length !== 0 ? domains : undefined;
    }
  }

  // Patch modifier filters
  for (const rule of ruleset) {
    if (rule.__modifierType === undefined) {
      continue;
    }
...
FilterContainer.prototype.addFilterUnit = function (bits, tokenHash, inewunit) {
  const bucket = this.bitsToBucket.get(bits) || new Map();
  if (bucket.size === 0) {
    this.bitsToBucket.set(bits, bucket);
  }
...
FilterContainer.prototype.optimize = function (throttle = 0) {
  ...
  while (this.unitsToOptimize.length !== 0) {
    ...
    switch (fc) {
      case FilterHostnameDict:
        ...
        break;
      case FilterBucket: {
        const optimizeBits = tokenHash === NO_TOKEN_HASH || (bits & MODIFY_REALMS) !== 0 ? 0b10 : 0b01;
  ...
  keyvalStore.setItem("SNFE.destHNTrieContainer.trieDetails", destHNTrieContainer.optimize());
  bidiTrieOptimize();
  filterDataShrink();
};
...
FilterContainer.prototype.toSelfie = async function (storage, path) {
  ...
  keyvalStore.setItem("SNFE.origHNTrieContainer.trieDetails", origHNTrieContainer.optimize());

  return Promise.all([
    storage.put(`${path}/destHNTrieContainer`, destHNTrieContainer.serialize(sparseBase64)),
    storage.put(`${path}/origHNTrieContainer`, origHNTrieContainer.serialize(sparseBase64)),
    storage.put(`${path}/bidiTrie`, bidiTrie.serialize(sparseBase64)),
    storage.put(`${path}/filterData`, filterDataToSelfie()),
    storage.put(`${path}/filterRefs`, filterRefsToSelfie()),
    storage.put(
      `${path}/main`,
      JSON.stringify({
...
FilterContainer.prototype.fromSelfie = async function (storage, path) {
  ...
  const results = await Promise.all([
    ...
    storage.get(`${path}/destHNTrieContainer`).then((details) => destHNTrieContainer.unserialize(details.content, sparseBase64)),
    storage.get(`${path}/origHNTrieContainer`).then((details) => origHNTrieContainer.unserialize(details.content, sparseBase64)),
    storage.get(`${path}/bidiTrie`).then((details) => bidiTrie.unserialize(details.content, sparseBase64)),
    storage.get(`${path}/filterData`).then((details) => filterDataFromSelfie(details.content)),
    storage.get(`${path}/filterRefs`).then((details) => filterRefsFromSelfie(details.content))
  ]);

  if (results.slice(1).every((v) => v === true) === false) {
    return false;
  }
...
FilterContainer.prototype.matchAndFetchModifiers = function (fctxt, modifierName) {
  if (this.notReady) {
    return;
  }
  ...
  const bucket01 = typeBits !== 0 ? this.bitsToBucket.get(catBits01) : undefined;
  const bucket10 = partyBits !== 0 ? this.bitsToBucket.get(catBits10) : undefined;
  const bucket11 = typeBits !== 0 && partyBits !== 0 ? this.bitsToBucket.get(catBits11) : undefined;

  if (bucket00 === undefined && bucket01 === undefined && bucket10 === undefined && bucket11 === undefined) {
    return;
  }
  ...
  for (;;) {
    th = tokenHashes[i];
    if (th === INVALID_TOKEN_HASH) {
      break;
    }
    env.th = th;
    $tokenBeg = tokenHashes[i + 1];
    if (bucket00 !== undefined && (iunit = bucket00.get(th) || 0) !== 0) {
      env.bits = catBits00;
      env.iunit = iunit;
      filterMatchAndFetchModifiers(iunit, env);
    }
    if (bucket01 !== undefined && (iunit = bucket01.get(th) || 0) !== 0) {
      env.bits = catBits01;
      env.iunit = iunit;
      filterMatchAndFetchModifiers(iunit, env);
    }
    if (bucket10 !== undefined && (iunit = bucket10.get(th) || 0) !== 0) {
      env.bits = catBits10;
      env.iunit = iunit;
      filterMatchAndFetchModifiers(iunit, env);
    }
    if (bucket11 !== undefined && (iunit = bucket11.get(th) || 0) !== 0) {
      env.bits = catBits11;
      env.iunit = iunit;
      filterMatchAndFetchModifiers(iunit, env);
    }
...
FilterContainer.prototype.realmMatchString = function (realmBits, typeBits, partyBits) {
  if (this.notReady) {
    return false;
  }
  ...
  const bucket00 = exactType === 0 ? this.bitsToBucket.get(catBits00) : undefined;
  const bucket01 = exactType !== 0 || typeBits !== 0 ? this.bitsToBucket.get(catBits01) : undefined;
  const bucket10 = exactType === 0 && partyBits !== 0 ? this.bitsToBucket.get(catBits10) : undefined;
  const bucket11 = (exactType !== 0 || typeBits !== 0) && partyBits !== 0 ? this.bitsToBucket.get(catBits11) : undefined;
  ...
  if (bucket00 !== undefined && (iunit = bucket00.get(DOT_TOKEN_HASH) || 0) !== 0 && filterMatch(iunit) === true) {
    catBits = catBits00;
  } else if (bucket01 !== undefined && (iunit = bucket01.get(DOT_TOKEN_HASH) || 0) !== 0 && filterMatch(iunit) === true) {
    catBits = catBits01;
  } else if (bucket10 !== undefined && (iunit = bucket10.get(DOT_TOKEN_HASH) || 0) !== 0 && filterMatch(iunit) === true) {
    catBits = catBits10;
  } else if (bucket11 !== undefined && (iunit = bucket11.get(DOT_TOKEN_HASH) || 0) !== 0 && filterMatch(iunit) === true) {
    catBits = catBits11;
  }
  // Pattern-based filters
    ...
    for (;;) {
      ...
      $tokenBeg = tokenHashes[i + 1];
      if (bucket00 !== undefined && (iunit = bucket00.get(tokenHash) || 0) !== 0 && filterMatch(iunit) === true) {
        catBits = catBits00;
        break;
      }
      if (bucket01 !== undefined && (iunit = bucket01.get(tokenHash) || 0) !== 0 && filterMatch(iunit) === true) {
        catBits = catBits01;
        break;
      }
      if (bucket10 !== undefined && (iunit = bucket10.get(tokenHash) || 0) !== 0 && filterMatch(iunit) === true) {
        catBits = catBits10;
        break;
      }
      if (bucket11 !== undefined && (iunit = bucket11.get(tokenHash) || 0) !== 0 && filterMatch(iunit) === true) {
        catBits = catBits11;
        break;
      }
      i += 2;
...
FilterContainer.prototype.transformRequest = function (fctxt) {
  ...
  if (directive.refs instanceof Object === false) {
    return;
  }
...
function compareRedirectRequests(redirectEngine, a, b) {
  ...
  const { token: btok, priority: bint, bits: bbits } = parseRedirectRequestValue(b);
...
function parseQueryPruneValue(directive) {
  if (directive.cache === null) {
    directive.cache = sfp.parseQueryPruneValue(directive.value);
  }
  return directive.cache;
}
...
FilterContainer.prototype.dump = function () {
  ...
  for (const [realmBits, realmName] of realms) {
    ...
    for (const [partyBits, partyName] of partyness) {
      ...
      for (const typeName in typeNameToTypeValue) {
        ...
        for (const [th, iunit] of bucket) {
          thCounts.add(th);
          const ths = thConstants.has(th) ? thConstants.get(th) : `0x${th.toString(16)}`;
          toOutput(4, `+ th: ${ths}`);
          dumpUnit(iunit, 5);
        }
      }
  ...
  const knownTokens = urlTokenizer.knownTokens.reduce((a, b) => (b !== 0 ? a + 1 : a), 0);
