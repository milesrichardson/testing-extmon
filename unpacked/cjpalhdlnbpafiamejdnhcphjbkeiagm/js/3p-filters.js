...
const renderFilterLists = () => {
  ...
  const initializeListEntry = (listDetails, listEntry) => {
    ...
    const listEntryPrevious = qs$(`[data-key="${listDetails.group}"] [data-key="${listkey}"]`);
    ...
    dom.cl.toggle(listEntry, "isDefault", listDetails.isDefault === true || listkey === "user-filters");
    ...
    dom.cl.toggle(listEntry, "unsecure", typeof remoteURL === "string" && remoteURL.lastIndexOf("http:", 0) === 0);
    dom.cl.toggle(listEntry, "failed", asset.error !== undefined);
    dom.cl.toggle(listEntry, "obsolete", asset.obsolete === true);
    const lastUpdateString = lastUpdateTemplateString.replace("{{ago}}", i18n.renderElapsedTimeToString(asset.writeTime || 0));
  ...
  const createListEntries = (parentkey, listTree, depth = 0) => {
    ...
    for (const [listkey, listDetails] of treeEntries) {
      ...
      qs$(listEntry, ":scope > .detailbar .listname").append(i18n.patchUnicodeFlags(listDetails.title));
      if (listDetails.lists !== undefined) {
  ...
  const onListsReceived = (response) => {
    ...
    // Build list tree
    const listTree = {};
    const groupKeys = ["user", "default", "ads", "privacy", "malware", "multipurpose", "annoyances", "regions", "custom"];
    ...
    qs$("#parseCosmeticFilters").checked = listsetDetails.parseCosmeticFilters === true;
...
const renderWidgets = () => {
  dom.cl.toggle("#buttonApply", "disabled", filteringSettingsHash === hashFromCurrentFromSettings());
  const updating = dom.cl.has(dom.body, "updating");
  dom.cl.toggle("#buttonUpdate", "active", updating);
  dom.cl.toggle("#buttonUpdate", "disabled", updating === false && qs$("#lists .listEntry.checked.obsolete:not(.toRemove)") === null);
  dom.cl.toggle("#buttonPurgeAll", "disabled", updating || qs$("#lists .listEntry.cached:not(.obsolete)") === null);
};
...
const hashFromListsetDetails = () => {
  const hashParts = [listsetDetails.parseCosmeticFilters === true, listsetDetails.ignoreGenericCosmeticFilters === true];
  const listHashes = [];
...
const hashFromCurrentFromSettings = () => {
  const hashParts = [qs$("#parseCosmeticFilters").checked, qs$("#ignoreGenericCosmeticFilters").checked];
  const listHashes = [];
  ...
  hashParts.push(listHashes.sort().join(), (textarea !== null && textarea.value.trim()) || "", qs$("#lists .listEntry.toRemove") !== null);
...
const updateListNode = (listNode) => {
  ...
  dom.text(qs$(listNode, ".nodestats"), renderNodeStats(checkedListLeaves.length, allListLeaves.length));
  dom.cl.toggle(listNode, "searchMatch", qs$(listNode, ":scope > .listEntries > .listEntry.searchMatch") !== null);
  ...
  dom.cl.toggle(qs$(listNode, ":scope > .detailbar .checkbox"), "partial", checkedListLeaves.length !== allListLeaves.length);
  dom.prop(qs$(listNode, ":scope > .detailbar input"), "checked", checkedListLeaves.length !== 0);
  dom.text(qs$(listNode, ".leafstats"), renderLeafStats(usedFilterCount, totalFilterCount));
...
dom.on("#buttonPurgeAll", "click", (ev) => {
...
const searchFilterLists = () => {
  ...
  const reflectSearchMatches = (listEntry) => {
    ...
    dom.cl.toggle(listEntry, "searchMatch", qs$(listEntry, ":scope > .listEntries > .listEntry.searchMatch") !== null);
  };
  const toI18n = (tags) => {
    if (tags === "") {
      return "";
...
const expandedListSet = new Set(["uBlock filters", "AdGuard – Annoyances", "EasyList – Annoyances"]);
