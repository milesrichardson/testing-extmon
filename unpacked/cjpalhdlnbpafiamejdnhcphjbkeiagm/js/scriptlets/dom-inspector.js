...
(() => {
  ...
  const domLayout = (function () {
    const skipTagNames = new Set(["br", "head", "link", "meta", "script", "style", "title"]);
  ...
  const elementsFromSpecialSelector = function (selector) {
    ...
    const xpr = document.evaluate(matches[1], document, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
  ...
  const highlightElements = function () {
    ...
    for (elem of rwRedNodes.keys()) {
      ...
      h = rect.height;
      ws = w.toFixed(1);
      poly = "M" + xl.toFixed(1) + " " + yt.toFixed(1) + "h" + ws + "v" + h.toFixed(1) + "h-" + ws + "z";
      islands.push(poly);
    }
  ...
  const bootstrap = function (ev) {
    ...
    pickerDoc.documentElement.style.setProperty("color-scheme", "dark light", "important");
  ...
  pickerRoot.addEventListener("load", (ev) => {
    bootstrap(ev);
  });
