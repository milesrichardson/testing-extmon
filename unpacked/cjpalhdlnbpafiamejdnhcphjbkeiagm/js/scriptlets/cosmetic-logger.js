...
(() => {
  ...
  if (typeof vAPI !== "object" || vAPI.domWatcher instanceof Object === false) {
    return;
  }
  ...
  const safeGroupSelectors = function (selectors) {
    const arr = Array.isArray(selectors) ? selectors : Array.from(selectors);
    return arr
      .map((s) => {
        return rePseudoElements.test(s) ? s.replace(rePseudoElements, "") : s;
      })
      .join(",\n");
  };
  ...
  const processDeclarativeSimple = function (node, out) {
    ...
    if ((node === document || node.matches(simpleDeclarativeStr) === false) && hasSelector(simpleDeclarativeStr, node) === false) {
      return;
    }
    for (const selector of simpleDeclarativeSet) {
      if ((node === document || safeMatchSelector(selector, node) === false) && safeQuerySelector(selector, node) === null) {
        continue;
      }
      out.push(`##${selector}`);
  ...
  const processDeclarativeComplex = function (out) {
    ...
    if (hasSelector(complexDeclarativeStr) === false) {
      return;
    }
    for (const selector of complexDeclarativeSet) {
      if (safeQuerySelector(selector) === null) {
        continue;
      }
      out.push(`##${selector}`);
  ...
  function processProcedural(out) {
    ...
    for (const [raw, pselector] of proceduralDict) {
      if (pselector.converted) {
        if (safeQuerySelector(pselector.selector) === null) {
          continue;
        }
  ...
  const processExceptions = function (out) {
    ...
    if (hasSelector(exceptionStr) === false) {
      return;
    }
  ...
  const processProceduralExceptions = function (out) {
    if (proceduralExceptionDict.size === 0) {
      return;
    }
    for (const exception of proceduralExceptionDict.values()) {
      if (exception.test() === false) {
        continue;
      }
  ...
  const handlers = {
    onFiltersetChanged: function (changes) {
      ...
      if (Array.isArray(changes.procedural) && changes.procedural.length !== 0) {
      ...
      if (Array.isArray(changes.exceptions)) {
        for (const selector of changes.exceptions) {
          ...
          if (details.action !== undefined && details.tasks === undefined && details.action[0] === "style") {
            exceptionDict.set(details.selector, details.raw);
            continue;
          }
          proceduralExceptionDict.set(details.raw, vAPI.domFilterer.createProceduralFilter(details));
        }
        exceptionStr = undefined;
      }
      nodesToProcess.clear();
