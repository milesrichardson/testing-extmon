...
(() => {
  ...
  if (typeof vAPI.domFilterer !== "object") {
    return;
  }
  if (vAPI.domFilterer === null) {
    return;
  }
  ...
  if (Array.isArray(allSelectors.procedural) && allSelectors.procedural.length !== 0) {
  ...
  if (Array.isArray(allSelectors.exceptions)) {
    ...
    for (const selector of allSelectors.exceptions) {
      ...
      if (pselector.test() === false) {
        continue;
      }
      matchedSelectors.push(`#@#${pselector.raw}`);
    }
    if (exceptionDict.size !== 0 && hasSelector(safeGroupSelectors(exceptionDict.keys()))) {
