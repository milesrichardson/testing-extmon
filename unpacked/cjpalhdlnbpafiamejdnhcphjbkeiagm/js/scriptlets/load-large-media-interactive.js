...
(() => {
  ...
  const mediaNotLoaded = function (elem) {
    switch (elem.localName) {
      ...
      case "img": {
        ...
        return style !== null ? style.getPropertyValue("display") !== "none" : elem.offsetHeight !== 0 && elem.offsetWidth !== 0;
      }
      default:
        break;
    }
  ...
  const onMouseClick = function (ev) {
    ...
    const toLoad = [];
    const elems = document.elementsFromPoint instanceof Function ? document.elementsFromPoint(ev.clientX, ev.clientY) : [ev.target];
    for (const elem of elems) {
