...
function addToDNR(context, list) {
  ...
  const lineIter = new LineIterator(sfp.utils.preparser.prune(list.text, env));
  ...
  staticNetFilteringEngine.dnrFromCompiled("add", context, new CompiledListReader(writer.toString()));
}
...
function finalizeRuleset(context, network) {
  ...
  const mergeRules = (rulesetMap, mergeTarget) => {
    ...
    for (const ids of mergeMap.values()) {
      ...
      const leftHandSet = new Set(extractTargetValue(leftHand, mergeTarget) || []);
