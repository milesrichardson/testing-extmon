...
class HNTrieContainer {
  ...
  growBuf(trieGrow, charGrow) {
    ...
    const bufLen = Math.max(roundToPageSize(char1 + charGrow), this.buf.length);
  ...
  reallocateBuf(newSize) {
    ...
    if (this.wasmMemory === null) {
      ...
      newBuf.set(newBuf.length < this.buf.length ? this.buf.subarray(0, newBuf.length) : this.buf);
      this.buf = newBuf;
    } else {
      const growBy = ((newSize + 0xffff) >>> 16) - (this.buf.length >>> 16);
      if (growBy <= 0) {
        return;
      }
