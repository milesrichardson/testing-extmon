...
import { domainFromHostname, hostnameFromURI } from "./uri-utils.js";
...
const initWorker = function () {
  ...
  for (const listKey in µb.availableFilterLists) {
    ...
    entries.set(listKey, {
      title: listKey !== µb.userFiltersPath ? entry.title : i18n$("1pPageName"),
      supportURL: entry.supportURL || ""
    });
  }
  if (entries.size === 0) {
...
const fromNetFilter = async function (rawFilter) {
  if (typeof rawFilter !== "string" || rawFilter === "") {
    return;
  }
  ...
  if (compiler.compile(parser, writer) === false) {
    return;
  }
...
const fromExtendedFilter = async function (details) {
  ...
  worker.postMessage({
    ...
    hostname,
    ignoreGeneric: staticNetFilteringEngine.matchRequestReverse("generichide", details.url) === 2,
    ignoreSpecific: staticNetFilteringEngine.matchRequestReverse("specifichide", details.url) === 2,
