...
function removeURL(url) {
  ...
  const afterURL = foundAt !== -1 ? list.children[foundAt].textContent : "";
  setURL(afterURL);
...
async function start() {
  ...
  dom.on("#reloadURL", "click", () => {
    ...
    fetchResource(url).then((r) => {
      if (urlToDocMap.has(url) === false) {
        return;
      }
      const afterDoc = r !== undefined ? new CodeMirror.Doc(r.text, r.mime || "") : beforeDoc;
  ...
  dom.on("#content", "click", ".cm-href", (ev) => {
    ...
    for (;;) {
      ...
      if (previous.classList.contains("cm-href") === false) {
        break;
      }
    ...
    for (;;) {
      ...
      if (next.classList.contains("cm-href") === false) {
        break;
      }
