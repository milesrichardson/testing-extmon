...
class DynamicSwitchRuleFiltering {
  ...
  toArray() {
    const out = [];
    for (const hostname of this.switches.keys()) {
      const prettyHn = hostname.includes("xn--") && punycode ? punycode.toUnicode(hostname) : hostname;
      for (const switchName in switchBitOffsets) {
        if (switchBitOffsets[switchName] === undefined) {
          continue;
        }
  ...
  validateRuleParts(parts) {
    ...
    if (nameToSwitchStateMap[parts[2]] === undefined) {
      return;
    }
