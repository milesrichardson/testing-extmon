...
const onStartMovingWidget = (() => {
  ...
  const stop = (ev) => {
    ...
    if (widget.classList.contains("moving") === false) {
      return;
    }
...
const nodeFromURL = function (parent, url, re, type) {
  ...
  if (re === undefined) {
    ...
    if (matches === null || matches[0].length === 0) {
      ...
      if (matches.index !== 0) {
        fragment.appendChild(document.createTextNode(url.slice(0, matches.index)));
      }
      const b = document.createElement("b");
      b.textContent = url.slice(matches.index, re.lastIndex);
      fragment.appendChild(b);
      if (re.lastIndex !== url.length) {
        fragment.appendChild(document.createTextNode(url.slice(re.lastIndex)));
      }
    }
  }
  if (/^https?:\/\//.test(url)) {
...
const LogEntry = function (details) {
  ...
  for (const prop in receiver) {
    if (details.hasOwnProperty(prop) && details[prop] !== receiver[prop]) {
...
const processLoggerEntries = function (response) {
  ...
  for (const entry of entries) {
    ...
    if (parsed.tabId !== undefined && allTabIds.has(parsed.tabId) === false) {
      if (autoDeleteVoidedRows) {
        continue;
      }
      parsed.voided = true;
    }
    if (parsed.type === "main_frame" && parsed.aliased === false && (parsed.filter === undefined || parsed.filter.modifier !== true)) {
...
const parseLogEntry = function (details) {
  ...
  textContent.push(padTo2(logDate.getUTCHours()) + ":" + padTo2(logDate.getUTCMinutes()) + ":" + padTo2(logDate.getSeconds()));
...
const viewPort = (() => {
  ...
  const onLayoutChanged = function () {
    ...
    const cellWidths = Array.from(qsa$(vwLineSizer, ".oneLine span")).map((el, i) => {
    ...
    for (let i = 0; i < cellWidths.length; i++) {
      ...
      cssRules.push(`#vwContent .logEntry > div > span:nth-of-type(${i + 1}) {`, "  display: none;", "}");
    }
  ...
  const renderToDiv = function (vwEntry, i) {
    ...
    // Partyness
    if (cells[COLUMN_PARTYNESS] !== "" && details.realm === "network" && details.domain !== undefined) {
  ...
  const contentChanged = function (addedCount) {
    ...
    const newWholeHeight = Math.max(filteredLoggerEntries.length * lineHeight, vwRenderer.clientHeight);
...
const onLogBufferRead = function (response) {
  ...
  // Disable tooltips?
  if (popupLoggerTooltips === undefined && response.tooltips !== undefined) {
...
(() => {
  ...
  const onColorsReady = function (response) {
    ...
    for (const url in response.colors) {
      if (response.colors.hasOwnProperty(url) === false) {
        continue;
      }
  ...
  const updateWidgets = function () {
    ...
    dom.cl.toggle(qs$(dialog, "#createStaticFilter"), "disabled", createdStaticFilters.hasOwnProperty(value) || value === "");
  };
  ...
  const createPreview = function (type, url) {
    ...
    dom.cl.toggle(qs$(dialog, ".picker"), "hide", targetTabId < 0 || cantPreview);
  ...
  const fillSummaryPaneFilterList = async function (rows) {
    ...
    const handleResponse = function (response) {
      ...
      if (bestMatchFilter !== "" && Array.isArray(response[bestMatchFilter])) {
        ...
        rows[1].children[1].appendChild(nodeFromFilter(bestMatchFilter, response[bestMatchFilter]));
      }
      // https://github.com/gorhill/uBlock/issues/2179
      if (rows[1].children[1].childElementCount === 0) {
        i18n.safeTemplateToDOM("loggerStaticFilteringFinderSentence2", { filter: rawFilter }, rows[1].children[1]);
      }
    };
  ...
  const moveDialog = (ev) => {
    if (ev.button !== 0 && ev.touches === undefined) {
      return;
    }
  ...
  const fillDialog = function (domains) {
    ...
    dom.cl.toggle(dialog, "extendedRealm", dom.cl.has(targetRow, "extendedRealm"));
  ...
  vAPI.localStorage.getItemAsync("loggerUI.entryTools").then((response) => {
  ...
  dom.on("#netInspector", "click", ".canDetails > span:not(:nth-of-type(4)):not(:nth-of-type(8))", (ev) => {
    toggleOn(ev);
  });
...
const rowFilterer = (() => {
  ...
  const filterOne = function (logEntry) {
    if (
      logEntry.dead ||
      (selectedTabId !== 0 && (logEntry.tabId === undefined || (logEntry.tabId > 0 && logEntry.tabId !== selectedTabId)))
    ) {
      return false;
    }
...
const rowJanitor = (() => {
  ...
  const discard = function (deadline) {
    ...
    const obsolete = typeof opts.maxAge === "number" ? Date.now() - opts.maxAge * 60000 : 0;
...
const loggerStats = (() => {
  ...
  const toggleOn = function () {
    dialog = modalDialog.create("#loggerStatsDialog", () => {
      dialog = undefined;
      if (timer !== undefined) {
  ...
  return {
    processFilter: function (filter) {
      ...
      if (dialog === undefined || timer !== undefined) {
        return;
      }
...
(() => {
  ...
  const formatAsMarkdownTable = function () {
    ...
    if (fieldCount !== 0) {
      outputAll.unshift(`${" |".repeat(fieldCount - 1)} `, `${":--- |".repeat(fieldCount - 1)}:--- `);
    }
  ...
  const setRadioButton = function (group, value) {
    if (options.hasOwnProperty(group) === false) {
      return;
    }
    const groupEl = qs$(dialog, `[data-radio="${group}"]`);
    const buttonEls = qsa$(groupEl, "[data-radio-item]");
    for (const buttonEl of buttonEls) {
      dom.cl.toggle(buttonEl, "on", dom.attr(buttonEl, "data-radio-item") === value);
    }
    options[group] = value;
  };
  ...
  const onOption = function (ev) {
    ...
    setRadioButton(dom.attr(group, "data-radio"), dom.attr(item, "data-radio-item"));
    format();
    ev.stopPropagation();
  };

  const toggleOn = function () {
    dialog = modalDialog.create("#loggerExportDialog", () => {
      dialog = undefined;
      lines.length = 0;
    });
...
const loggerSettings = (() => {
  ...
  const toggleOn = function () {
    const dialog = modalDialog.create("#loggerSettingsDialog", (dialog) => {
      toggleOff(dialog);
    });
  ...
  const toggleOff = function (dialog) {
    ...
    vAPI.localStorage.setItem("loggerSettings", JSON.stringify(settings));
