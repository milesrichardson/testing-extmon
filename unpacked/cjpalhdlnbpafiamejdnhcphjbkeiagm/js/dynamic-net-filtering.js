...
import { decomposeHostname, domainFromHostname } from "./uri-utils.js";
...
class DynamicHostRuleFiltering {
  ...
  toArray() {
    ...
    for (const key of this.rules.keys()) {
      ...
      const srchnPretty = srchn.includes("xn--") && punycode ? punycode.toUnicode(srchn) : srchn;
      const deshnPretty = deshn.includes("xn--") && punycode ? punycode.toUnicode(deshn) : deshn;
      for (const type in typeBitOffsets) {
        if (typeBitOffsets[type] === undefined) {
          continue;
        }
  ...
  validateRuleParts(parts) {
    ...
    if (typeBitOffsets[parts[2]] === undefined) {
      return;
    }

    if (nameToActionMap[parts[3]] === undefined) {
      return;
    }
