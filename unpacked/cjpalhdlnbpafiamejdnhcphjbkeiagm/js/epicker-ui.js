...
*/
  ...
  const onSvgTouch = (() => {
    ...
    return (ev) => {
      ...
      const distance = Math.sqrt(Math.pow(stopX - startX, 2), Math.pow(stopY - startY, 2));
  ...
  const onCandidateChanged = function () {
    ...
    $id("resultsetModifiers").classList.toggle("hide", text === "" || text !== computedCandidate);
    vAPI.MessagingConnection.sendTo(epickerConnectionId, {
      what: "dialogSetFilter",
      filter,
      compiled: reCosmeticAnchor.test(filter) ? staticFilteringParser.result.compiled : undefined
    });
  };
  ...
  const onCreateClicked = function () {
    ...
    vAPI.MessagingConnection.sendTo(epickerConnectionId, {
      what: "dialogCreate",
      filter: candidate,
      compiled: reCosmeticAnchor.test(candidate) ? staticFilteringParser.result.compiled : undefined
    });
  };
  ...
  vAPI.MessagingConnection.connectTo(`epickerDialog-${epickerId}`, `epicker-${epickerId}`, onConnectionMessage);
