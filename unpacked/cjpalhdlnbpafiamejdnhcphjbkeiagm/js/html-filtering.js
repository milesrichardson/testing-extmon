...
class PSelectorXpathTask {
  ...
  transpose(node, output) {
    const xpr = docRegister.evaluate(this.xpe, node, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, null);
...
htmlFilteringEngine.fromCompiledContent = function (reader) {
  ...
  if (µb.canFilterResponseData === false) {
    return;
  }
...
htmlFilteringEngine.retrieve = function (details) {
  ...
  const entity = details.entity !== "" ? `${hostname.slice(0, -details.domain.length)}${details.entity}` : "*";
