...
import { domainFromHostname, hostnameFromURI, originFromURI } from "./uri-utils.js";
...
const onPopupUpdated = (() => {
  ...
  const popupMatch = function (fctxt, rootOpenerURL, localOpenerURL, targetURL, popupType = "popup") {
    ...
    if (µb.getNetFilteringSwitch(targetURL) === false || µb.getNetFilteringSwitch(µb.normalizeTabURL(0, targetURL)) === false) {
      return 0;
    }
    ...
    if (fctxt.getTabHostname() !== "" && targetURL !== "about:blank") {
      ...
      //   popunders.
      if (popupType === "popup" && sessionSwitches.evaluateZ("no-popups", fctxt.getTabHostname())) {
        fctxt.filter = {
      ...
      let result = sessionURLFiltering.evaluateZ(fctxt.getTabHostname(), targetURL, popupType);
      if ((result === 1 && sessionURLFiltering.type === popupType) || result === 2) {
      ...
      result = sessionFirewall.evaluateCellZY(fctxt.getTabHostname(), fctxt.getHostname(), popupType);
      if (result === 2) {
  ...
  const mapPopunderResult = function (fctxt, popunderURL, popunderHostname, result) {
    ...
    //   hostname in the URL.
    return beg >= pos && beg < pos + popunderHostname.length && end > pos ? result : 0;
  };

  const popunderMatch = function (fctxt, rootOpenerURL, localOpenerURL, targetURL) {
    let result = popupMatch(fctxt, targetURL, undefined, rootOpenerURL, "popunder");
    if (result === 1) {
      return result;
    }
    ...
    result = mapPopunderResult(fctxt, popunderURL, popunderHostname, popupMatch(fctxt, targetURL, undefined, popunderURL));
    if (result !== 0) {
      return result;
    }
    ...
    return mapPopunderResult(fctxt, popunderURL, popunderHostname, popupMatch(fctxt, targetURL, undefined, popunderURL));
  };
  ...
  return function (targetTabId, openerDetails) {
    ...
    let localOpenerURL = openerDetails.frameId !== 0 ? openerDetails.frameURL : undefined;
    ...
    if (µb.getNetFilteringSwitch(µb.normalizeTabURL(openerTabId, rootOpenerURL)) === false) {
      return;
    }
    ...
    //   legit.
    if (areDifferentURLs(targetURL, openerDetails.trustedURL) && areDifferentURLs(targetURL, µb.maybeGoodPopup.url)) {
    ...
    if (logger.enabled) {
      ...
      if (popupType === "popup") {
        ...
        fctxt.setURL(rootOpenerURL).setTabId(targetTabId).setTabOriginFromURL(targetURL).setDocOriginFromURL(targetURL);
      }
      fctxt.toLogger();
    }
...
µb.tabContextManager = (() => {
  ...
  const PopupCandidate = class {
    constructor(createDetails, openerDetails) {
      ...
      this.opener = {
        ...
        popunder: false,
        trustedURL: createDetails.sourceTabId === µb.maybeGoodPopup.tabId ? µb.maybeGoodPopup.url : ""
      };
  ...
  const onTabCreated = async function (createDetails) {
    ...
    if (popup === undefined) {
      ...
      popupCandidates.set(tabId, new PopupCandidate(createDetails, openerDetails));
    }
  ...
  const TabContext = function (tabId) {
    this.tabId = tabId;
    this.stack = [];
    this.rawURL = this.normalURL = this.origin = this.rootHostname = this.rootDomain = "";
  ...
  TabContext.prototype.update = function () {
    ...
    this.rootDomain = domainFromHostname(this.rootHostname) || this.rootHostname;
  };
  ...
  const Context = class {
    ...
    init(tabId) {
      ...
      this.pageHostname = this.pageDomain = this.requestURL = this.origin = this.requestHostname = this.requestDomain = "";
      return this;
    }
    dispose() {
  ...
  const updateBadge = (tabId) => {
    ...
    if (pageStore !== null) {
      ...
      if (state === 1) {
        ...
        if ((parts & 0b0100) !== 0) {
          color = computeBadgeColor(µb.blockingModeFromHostname(pageStore.tabHostname));
        }
      }
    }
...

{
  ...
  const pageStoreJanitor = function () {
    ...
    const n = Math.min(pageStoreJanitorSampleAt + pageStoreJanitorSampleSize, tabIds.length);
