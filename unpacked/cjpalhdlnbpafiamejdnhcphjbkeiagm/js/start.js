...
(async () => {
  ...
  const initializeTabs = async () => {
    ...
    const tabIds = [];
    {
      ...
      for (const tab of tabs) {
        ...
        toCheck.push(/^https?:\/\//.test(url) ? vAPI.tabs.executeScript(id, checker) : false);
        tabIds.push(id);
      }
    }
  ...
  const onNetWhitelistReady = (netWhitelistRaw, adminExtra) => {
    ...
    if (adminExtra instanceof Object && Array.isArray(adminExtra.trustedSiteDirectives)) {
  ...
  const onUserSettingsReady = (fetched) => {
    ...
    if (vAPI.Net.canSuspend() && fetched.suspendUntilListsAreLoaded === false) {
      vAPI.net.unsuspend({ all: true, discard: true });
      ubolog(`Unsuspend network activity listener at ${tnow} ms`);
      µb.supportStats.unsuspendAfter = `${tnow} ms`;
    } else if (vAPI.Net.canSuspend() === false && fetched.suspendUntilListsAreLoaded) {
      vAPI.net.suspend();
    ...
    if (fetched.importedLists.length === 0 && fetched.externalLists !== "") {
    ...
    if (vAPI.net.canUncloakCnames && µb.userSettings.cnameUncloakEnabled === false) {
  ...
  const onHiddenSettingsReady = async () => {
    ...
    if (vAPI.canWASM && µb.hiddenSettings.disableWebAssembly !== true) {
      const wasmModuleFetcher = function (path) {
        return fetch(`${path}.wasm`, { mode: "same-origin" })
    ...
    const cacheBackend = await cacheStorage.select(µb.hiddenSettings.cacheStorageAPI);
  ...
  try {
    ...
    const [, , lastVersion] = await Promise.all([
      ...
      cacheStorage.get({ compiledMagic: 0, selfieMagic: 0 }).then((fetched) => {
  ...
  µb.scheduleAssetUpdater(µb.userSettings.autoUpdate ? µb.hiddenSettings.autoUpdateDelayAfterLaunch * 1000 : 0);
  ...
  browser.runtime.onUpdateAvailable.addListener((details) => {
    ...
    if (µb.hiddenSettings.extensionUpdateForceReload === true || toInt(details.version) <= toInt(vAPI.app.version)) {
      vAPI.app.restart();
    }
  });
