...
vAPI.cantWebsocket =
  browser.webRequest.ResourceType instanceof Object === false || browser.webRequest.ResourceType.WEBSOCKET !== "websocket";
...
vAPI.browserSettings = (() => {
  ...
  return {
    ...
    canLeakLocalIPAddresses: vAPI.webextFlavor.soup.has("firefox") && vAPI.webextFlavor.soup.has("mobile"),
...

{
  ...
  vAPI.setIcon = async function (tabId, details) {
    ...
    const { badge, color } = hasUnprocessedRequest ? { badge: "!", color: "#FC0" } : details;
    ...
    if (browserAction.setTitle !== undefined) {
      const title = titleTemplate.replace("{badge}", state === 1 ? (badge !== "" ? badge : "0") : "off");
...
vAPI.messaging = {
  ...
  onPortConnect: function (port) {
    ...
    portDetails.privileged = origin !== undefined ? origin === this.PRIVILEGED_ORIGIN : url.startsWith(this.PRIVILEGED_ORIGIN);
    if (tab) {
  ...
  onPortMessage: function (request, port) {
    ...
    if (listenerDetails !== undefined && (listenerDetails.privileged === false || portDetails.privileged)) {
    ...
    ubolog(`vAPI.messaging.onPortMessage > unhandled request: ${JSON.stringify(request.msg)}`, request);
...

{
  ...
  const generateSecret = () => (Math.floor(Math.random() * 2176782336) + 2176782336).toString(36).slice(1);
...
vAPI.Net = class {
  ...
  addListener(which, clientListener, filters, options) {
    ...
    browser.webRequest[which].addListener(actualListener, actualFilters, options);
  }
...
vAPI.localStorage = {
  start: async function () {
    ...
    this.cache = webext.storage.local.get("localStorage").then((bin) => {
      this.cache = bin instanceof Object && bin.localStorage instanceof Object ? bin.localStorage : {};
    });
    return this.cache;
  },
  clear: function () {
...
vAPI.cloud = (() => {
  ...
  const evalMaxChunkSize = function () {
    return Math.floor(QUOTA_BYTES_PER_ITEM * (vAPI.webextFlavor.soup.has("firefox") ? 0.6 : 0.75));
  };
  ...
  const push = async function (details) {
    ...
    for (let i = 0; i < chunkCount; i++) {
      bin[datakey + i.toString()] = encoded.substr(i * maxChunkSize, maxChunkSize);
    }
  ...
  const used = async function (datakey) {
    ...
    let results;
    try {
      results = await Promise.all([webext.storage.sync.getBytesInUse(keys), webext.storage.sync.getBytesInUse(null)]);
    } catch (ex) {}
