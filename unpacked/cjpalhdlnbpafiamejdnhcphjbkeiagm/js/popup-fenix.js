...
const cachePopupData = function (data) {
  ...
  for (const hostname in hostnameDict) {
    ...
    hostnameToSortableTokenMap.set(hostname, domain + " " + prefix.split(".").reverse().join("."));
  }
  return popupData;
};
...
const hashFromPopupData = function (reset = false) {
  ...
  dom.cl.toggle(dom.body, "needReload", hash !== cachedPopupHash || popupData.hasUnprocessedRequest === true);
};
...
const formatNumber = function (count) {
  ...
  if (intlNumberFormat === undefined && Intl.NumberFormat instanceof Function) {
    ...
    if (intl.resolvedOptions instanceof Function && intl.resolvedOptions().hasOwnProperty("notation")) {
...
const updateFirewallCellCount = function (cells, allowed, blocked) {
  for (const cell of cells) {
    if (gtz(allowed)) {
      dom.attr(cell, "data-acount", Math.min(Math.ceil(Math.log(allowed + 1) / Math.LN10), 3));
    } else {
      dom.attr(cell, "data-acount", "0");
    }
    if (gtz(blocked)) {
      dom.attr(cell, "data-bcount", Math.min(Math.ceil(Math.log(blocked + 1) / Math.LN10), 3));
    } else {
...
const updateAllFirewallCells = function (doRules = true, doCounts = true) {
  ...
  for (const row of rows) {
    ...
    if (doRules) {
      updateFirewallCellRule(qsa$(row, ':scope > span[data-src="/"]'), "/", des, type, popupData.firewallRules[`/ ${des} ${type}`]);
    }
    const cells = qsa$(row, ':scope > span[data-src="."]');
    if (doRules) {
      updateFirewallCellRule(cells, ".", des, type, popupData.firewallRules[`. ${des} ${type}`]);
    }
    if (des === "*" || type !== "*") {
      continue;
    }
...
const buildAllFirewallRows = function () {
  ...
  for (const des of allHostnameRows) {
    ...
    const prettyDomainName = des.includes("xn--") ? punycode.toUnicode(des) : des;
...
const renderPopup = function () {
  ...
  // The hostname information below the power switch
  {
    ...
    if (pageDomain !== "") {
      ...
      dom.text(elemHn, pageHostname !== pageDomain ? safePunycodeToUnicode(pageHostname.slice(0, -pageDomain.length - 1)) + "." : "");
    } else {
      dom.text(elemDn, "");
      dom.text(elemHn, "");
    }
  ...
  dom.cl.toggle("#basicTools", "canPick", popupData.canElementPicker === true && isFiltering);
  ...
  dom.text("#no-popups .fa-icon-badge", total ? Math.min(total, 99).toLocaleString() : "");

  // Report large media count on badge
  total = popupData.largeMediaCount;
  dom.text("#no-large-media .fa-icon-badge", total ? Math.min(total, 99).toLocaleString() : "");

  // Report remote font count on badge
  total = popupData.remoteFontCount;
  dom.text("#no-remote-fonts .fa-icon-badge", total ? Math.min(total, 99).toLocaleString() : "");
...
const renderTooltips = function (selector) {
  for (const [key, details] of tooltipTargetSelectors) {
    ...
    const text = i18n$(details.i18n + (qs$(details.state) === null ? "1" : "2"));
...
const renderPopupLazy = (() => {
  ...
  return async function () {
    ...
    dom.text("#no-scripting .fa-icon-badge", (count || 0) !== 0 ? Math.min(count, 99).toLocaleString() : "");
...
const computedSections = () => (popupData.popupPanelSections & ~popupData.popupPanelDisabledSections) | popupData.popupPanelLockedSections;
...
const setSections = function (bits) {
  ...
  const max = sectionBitsToAttribute(((1 << maxNumberOfSections) - 1) & ~popupData.popupPanelDisabledSections);
...
const saveExpandExceptions = function () {
  vAPI.localStorage.setItem("popupExpandExceptions", Array.from(expandExceptions));
};
...
dom.on("#firewall", "click", '.isDomain[data-type="*"] > span:first-of-type', (ev) => {
  ...
  setSpecificExpand(dom.attr(div, "data-des"), dom.cl.has(div, "expandException") === false);
});
...

{
  ...
  const checkViewport = async function () {
    if (dom.cl.has(dom.root, "mobile") || selfURL.searchParams.get("portrait")) {
      ...
      if (selfURL.searchParams.get("portrait") || window.innerWidth < minWidth) {
        dom.cl.add(dom.root, "portrait");
      }
    }
