...
import { sessionFirewall, sessionSwitches, sessionURLFiltering } from "./filtering-engines.js";

import { domainFromHostname, hostnameFromURI, isNetworkURI } from "./uri-utils.js";
...
const NetFilteringResultCache = class {
  ...
  rememberResult(fctxt, result) {
    ...
    if (result !== 1 || fctxt.redirectURL !== undefined) {
      return;
    }
    const now = Date.now();
  ...
  rememberBlock(fctxt) {
    ...
    const now = Date.now();
    this.blocked.set(`${fctxt.getDocHostname()} ${fctxt.type} ${fctxt.url}`, now);
    this.hash = now;
  }
  ...
  lookupResult(fctxt) {
    const entry = this.results.get(fctxt.getDocHostname() + " " + fctxt.type + " " + fctxt.url);
    if (entry === undefined) {
      return;
    }
    // We need to use a new WAR secret if one is present since WAR secrets
    // can only be used once.
    if (entry.redirectURL !== undefined && entry.redirectURL.startsWith(this.extensionOriginURL)) {
...
const FrameStore = class {
  ...
  getCosmeticFilteringBits(tabId) {
    ...
    {
      const result = staticNetFilteringEngine.matchRequestReverse("specifichide", this.rawURL);
    ...
    {
      const result = staticNetFilteringEngine.matchRequestReverse("generichide", this.rawURL);
      if (result !== 0 && logger.enabled) {
        µb.filteringContext
          .duplicate()
          .fromTabId(tabId)
...
const CountDetails = class {
  ...
  reset() {
    ...
    blocked.any = blocked.frame = blocked.script = allowed.any = allowed.frame = allowed.script = 0;
  }
...
const PageStore = class {
  ...
  init(tabId, details) {
    ...
    if (typeof this.allowLargeMediaElementsUntil !== "number" || tabContext.rootHostname !== this.tabHostname) {
  ...
  titleFromDetails(details) {
    if (details instanceof Object === false || details.title === undefined) {
      return false;
    }
  ...
  async pruneFrames() {
    ...
    for (const [frameId, { t0 }] of this.frames) {
      if (toKeep.has(frameId) || t0 >= obsolete) {
        continue;
      }
  ...
  shouldApplyCosmeticFilters(frameId = 0) {
    if (this._noCosmeticFiltering === undefined) {
      ...
      if (this._noCosmeticFiltering === false) {
        this._noCosmeticFiltering = sessionSwitches.evaluateZ("no-cosmetic-filtering", this.tabHostname) === true;
    ...
    return this.shouldApplySpecificCosmeticFilters(frameId) || this.shouldApplyGenericCosmeticFilters(frameId);
  }

  shouldApplySpecificCosmeticFilters(frameId) {
    if (this.shouldApplyCosmeticFilters(-1) === false) {
      return false;
    }
    const frameStore = this.getFrameStore(frameId);
    if (frameStore === null) {
  ...
  getAllHostnameDetails() {
    if (this.hostnameDetailsMap.has(this.tabHostname) === false && isNetworkURI(this.rawURL)) {
      this.hostnameDetailsMap.set(this.tabHostname, new HostnameDetails(this.tabHostname));
    }
  ...
  journalAddRootFrame(type, url) {
    if (type === "committed") {
      ...
      if (this.journalLastUncommitted === -1 || this.journalLastUncommittedOrigin !== newOrigin) {
  ...
  filterRequest(fctxt) {
    ...
    if (fctxt.itype === fctxt.CSP_REPORT && this.filterCSPReport(fctxt) === 1) {
      return 1;
    }

    if ((fctxt.itype & fctxt.FONT_ANY) !== 0 && this.filterFont(fctxt) === 1) {
      return 1;
    }

    if (fctxt.itype === fctxt.SCRIPT && this.filterScripting(fctxt, true) === 1) {
      return 1;
    }

    const cacheableResult = this.cacheableResults.has(fctxt.itype) && fctxt.aliasURL === undefined;
    ...
    let result = sessionURLFiltering.evaluateZ(fctxt.getTabHostname(), fctxt.url, requestType);
    ...
    if (result === 0 && µb.userSettings.advancedUserEnabled) {
      result = sessionFirewall.evaluateCellZY(fctxt.getTabHostname(), fctxt.getHostname(), requestType);
    ...
    if (result === 0 || result === 3) {
      ...
      if (result !== 0) {
        ...
        if (result === 1 && fctxt.aliasURL !== undefined && snfe.isBlockImportant() === false && this.shouldExceptCname(fctxt)) {
          return 2;
        }
      }
    }
  ...
  filterOnHeaders(fctxt, headers) {
    ...
    // URL filtering
    if (result === 1 && sessionURLFiltering.evaluateZ(fctxt.getTabHostname(), fctxt.url, fctxt.type) === 2) {
      result = 2;
      if (loggerEnabled) {
        fctxt.filter = sessionURLFiltering.toLogData();
      }
    ...
    if (
      ...
      sessionFirewall.evaluateCellZY(fctxt.getTabHostname(), fctxt.getHostname(), fctxt.type) === 2
    ) {
      result = 2;
      if (loggerEnabled) {
        fctxt.filter = sessionFirewall.toLogData();
  ...
  redirectNonBlockedRequest(fctxt) {
    ...
    const pruneDirectives =
      (fctxt.redirectURL === undefined && staticNetFilteringEngine.hasQuery(fctxt) && staticNetFilteringEngine.filterQuery(fctxt)) ||
      undefined;
    if (transformDirectives === undefined && pruneDirectives === undefined) {
      return;
    }
    if (logger.enabled !== true) {
      return;
  ...
  filterCSPReport(fctxt) {
    if (sessionSwitches.evaluateZ("no-csp-reports", fctxt.getHostname())) {
      if (logger.enabled) {
  ...
  filterFont(fctxt) {
    ...
    if (sessionSwitches.evaluateZ("no-remote-fonts", fctxt.getTabHostname()) !== false) {
      if (logger.enabled) {
        fctxt.filter = sessionSwitches.toLogData();
      }
      return 1;
  ...
  filterScripting(fctxt, netFiltering) {
    ...
    if (netFiltering === false || sessionSwitches.evaluateZ("no-scripting", fctxt.getTabHostname()) === false) {
      return 0;
    }
    if (logger.enabled) {
      fctxt.filter = sessionSwitches.toLogData();
  ...
  filterLargeMediaElement(fctxt, size) {
    ...
    if (this.allowLargeMediaElementsRegex instanceof RegExp && this.allowLargeMediaElementsRegex.test(fctxt.url)) {
      return 0;
    }
    if (Date.now() < this.allowLargeMediaElementsUntil) {
      const sources = this.allowLargeMediaElementsRegex instanceof RegExp ? [this.allowLargeMediaElementsRegex.source] : [];
  ...
  clickToLoad(frameId, frameURL) {
    ...
    this.netFilteringCache.forgetResult(this.tabHostname, "sub_frame", frameURL);
  ...
  shouldExceptCname(fctxt) {
    ...
    if (exceptCname === undefined) {
      const result = staticNetFilteringEngine.matchRequestReverse(
        "cname",
        frameStore instanceof Object ? frameStore.rawURL : fctxt.getDocOrigin()
      );
      exceptCname = result === 2 ? staticNetFilteringEngine.toLogData() : false;
      if (frameStore instanceof Object) {
        frameStore.exceptCname = exceptCname;
      }
    }
    if (exceptCname === false) {
      return false;
    }
  ...
  getBlockedResources(request, response) {
    ...
    fctxt.fromTabId(this.tabId).setDocOriginFromURL(normalURL);
    ...
    if (Array.isArray(resources) && resources.length !== 0) {
      for (const resource of resources) {
        this.filterRequest(fctxt.setType(resource.type).setURL(resource.url));
      }
    }
    ...
    response.blockedResources = this.netFilteringCache.lookupAllBlocked(fctxt.getDocHostname());
  }
};

PageStore.prototype.cacheableResults = new Set([µb.FilteringContext.SUB_FRAME]);
