    ...
    datasetPromise = io
      ...
      .then((details) => {
        ...
        for (const request of requests) {
          ...
          if (!request.frameUrl || !request.url) {
            continue;
          }
...
µb.benchmarkDynamicNetFiltering = async function () {
  ...
  for (const request of requests) {
    ...
    fctxt.setType(request.cpt);
    sessionFirewall.evaluateCellZY(fctxt.getTabHostname(), fctxt.getHostname(), fctxt.type);
  }
  const t1 = performance.now();
  const dur = t1 - t0;
