...
µb.openNewTab = function (details) {
  if (details.url.startsWith("logger-ui.html")) {
    if (details.shiftKey) {
      this.changeUserSettings("alwaysDetachLogger", !this.userSettings.alwaysDetachLogger);
    }
    if (this.userSettings.alwaysDetachLogger) {
      ...
      let popupLoggerBox;
      try {
        popupLoggerBox = JSON.parse(vAPI.localStorage.getItem("popupLoggerBox"));
      } catch (ex) {}
...
µb.fireEvent = function (name, details = undefined) {
  if (self instanceof Object && self.dispatchEvent instanceof Function && self.CustomEvent instanceof Function) {
    self.dispatchEvent(new CustomEvent(name, { detail: details }));
  }
};

µb.onEvent = function (name, fn) {
  if (self instanceof Object && self.addEventListener instanceof Function) {
...
µb.settingValueFromString = function (orig, name, s) {
  if (typeof name !== "string" || typeof s !== "string") {
    return;
  }
  if (orig.hasOwnProperty(name) === false) {
    return;
  }
  let r;
