...
vAPI.Tabs = class extends vAPI.Tabs {
  ...
  synthesizeNavigationTargetEvent(details) {
    if (this.tabIds.has(details.tabId) === false) {
      return;
    }
    this.tabIds.delete(details.tabId);
    const isClientRedirect = Array.isArray(details.transitionQualifiers) && details.transitionQualifiers.includes("client_redirect");
    const isStartPage = details.transitionType === "start_page";
    if (isClientRedirect === false && isStartPage === false) {
      return;
    }
...

{
  ...
  vAPI.Net = class extends vAPI.Net {
    normalizeDetails(details) {
      ...
      if (typeof details.initiator === "string" && details.initiator !== "null") {
