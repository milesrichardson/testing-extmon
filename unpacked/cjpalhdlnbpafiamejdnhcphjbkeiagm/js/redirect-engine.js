...
const warSecret = typeof vAPI === "object" && vAPI !== null ? vAPI.warSecret.short : () => "";
...
class RedirectEntry {
  ...
  toURL(fctxt, asDataURI = false) {
    if (this.warURL !== undefined && asDataURI !== true && fctxt instanceof Object && fctxt.type !== "xmlhttprequest") {
      const params = [];
...
class RedirectEngine {
  ...
  tokenToURL(fctxt, token, asDataURI = false) {
  ...
  contentFromName(name, mime = "") {
    ...
    if (entry.mime.startsWith(mime) === false) {
      return;
    }
    return {
      js: entry.toContent(),
  ...
  resourcesFromString(text) {
    const lineIter = new LineIterator(removeTopCommentBlock(text) + "\n\n");
    ...
    while (lineIter.eot() === false) {
      ...
      const mime = fields[1];
      const data = orphanizeString(fields.slice(2).join(encoded ? "" : "\n"));
        ...
        for (const { prop, value } of details) {
          switch (prop) {
            case "alias":
              ...
              break;
            case "world":
              if (/^isolated$/i.test(value) === false) {
                break;
              }
              resource.world = "ISOLATED";
              break;
            case "dependency":
              if (this.resources.has(value) === false) {
                break;
              }
  ...
  loadBuiltinResources(fetcher) {
    ...
    const fetches = [
      import("/assets/resources/scriptlets.js").then((module) => {
        for (const scriptlet of module.builtinScriptlets) {
          ...
          if (Array.isArray(details.aliases) === false) {
            continue;
          }
    ...
    const process = (result) => {
      ...
      const name = match[1];
      return result.content instanceof Blob ? processBlob(name, result.content) : processText(name, result.content);
    };
  ...
  getResourceDetails() {
    const out = new Map([["none", { canInject: false, canRedirect: true, aliasOf: "" }]]);
  ...
  getTrustedScriptletTokens() {
    ...
    const isTrustedScriptlet = (entry) => {
      ...
      if (entry.name.endsWith(".js") === false) {
        return false;
      }
      return true;
    };
    for (const [name, entry] of this.resources) {
      if (isTrustedScriptlet(entry) === false) {
        continue;
      }
      out.push(name.slice(0, -3));
    }
    for (const [alias, name] of this.aliases) {
      if (out.includes(name.slice(0, -3)) === false) {
        continue;
      }
  ...
  async resourcesFromSelfie(storage) {
    ...
    if (selfie instanceof Object === false || selfie.version !== RESOURCES_SELFIE_VERSION || Array.isArray(selfie.resources) === false) {
      return false;
    }
