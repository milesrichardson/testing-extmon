...
CodeMirror.defineMode("ubo-static-filtering", function () {
  ...
  const nodeHasError = (node) => {
    return astParser.getNodeFlags(node || currentWalkerNode, sfp.NODE_FLAG_ERROR) !== 0;
  };

  const colorFromAstNode = function () {
    if (astParser.nodeIsEmptyString(currentWalkerNode)) {
      return "+";
    }
    if (nodeHasError()) {
      return "error";
    ...
    switch (nodeType) {
      ...
      case sfp.NODE_TYPE_EXT_OPTIONS_ANCHOR:
        return astParser.getFlags(sfp.AST_FLAG_IS_EXCEPTION) ? "tag strong" : "def strong";
...
function initHints() {
  ...
  const proceduralOperatorNames = new Map(Array.from(sfp.proceduralOperatorTokens).filter((item) => (item[1] & 0b01) !== 0));
  const excludedHints = new Set(["genericblock", "object-subrequest", "rewrite", "webrtc"]);
  ...
  const getCommentHints = function (cursor, line) {
    ...
    if (line.startsWith("!#if ")) {
      ...
      return pickBestHints(cursor, matchLeft[1], matchRight[0], preparseDirectiveHints);
    }
...

{
  ...
  const markerTemplates = {
    error: {
      node: null,
      html: ['<div class="CodeMirror-lintmarker" data-lint="error">&nbsp;', '<span class="msg"></span>', "</div>"]
    },
    "if-start": {
      node: null,
      html: [
