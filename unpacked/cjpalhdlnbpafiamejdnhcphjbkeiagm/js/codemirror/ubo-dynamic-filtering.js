...
CodeMirror.defineMode("ubo-dynamic-filtering", () => {
  ...
  const validSwitcheStates = new Set(["true", "false"]);
  const validHnRuleTypes = new Set(["*", "3p", "image", "inline-script", "1p-script", "3p-script", "3p-frame"]);
  const invalidURLRuleTypes = new Set(["doc", "main_frame"]);
  const validActions = new Set(["block", "allow", "noop"]);
