...

{
  ...
  const searchWidgetKeydownHandler = function (cm, ev) {
    ...
    CodeMirror.lookupKey(keyName, cm.getOption("keyMap"), function (command) {
  ...
  const getSearchCursor = function (cm, query, pos) {
    return cm.getSearchCursor(query, pos, { caseFold: queryCaseInsensitive(query), multiline: false });
  };
  ...
  const formatNumber = function (n) {
    ...
    return n > 10000 && intlNumberFormat instanceof Object ? intlNumberFormat.format(n) : n.toLocaleString();
  };
  ...
  const findNext = function (cm, dir, callback) {
    cm.operation(function () {
      ...
      let cursor = getSearchCursor(cm, state.query, dir <= 0 ? cm.getCursor("from") : cm.getCursor("to"));
      const previous = dir < 0;
      if (!cursor.find(previous)) {
        cursor = getSearchCursor(cm, state.query, previous ? CodeMirror.Pos(cm.lastLine()) : CodeMirror.Pos(cm.firstLine(), 0));
        if (!cursor.find(previous)) return;
      }
      cm.setSelection(cursor.from(), cursor.to());
      const { clientHeight } = cm.getScrollInfo();
      cm.scrollIntoView({ from: cursor.from(), to: cursor.to() }, clientHeight >>> 1);
  ...
  CodeMirror.defineInitHook(function (cm) {
    ...
    cm.on("linterDone", (details) => {
      ...
      dom.text(qs$(linterWidget, ".cm-linter-widget-count"), i18n$("linterMainReport").replace("{{count}}", count.toLocaleString()));
    });
  });
}
