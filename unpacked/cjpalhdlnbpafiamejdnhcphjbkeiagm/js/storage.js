...
import { permanentFirewall, permanentSwitches, permanentURLFiltering } from "./filtering-engines.js";
...
µb.getBytesInUse = async function () {
  ...
  promises.push(vAPI.storage.getBytesInUse instanceof Function ? vAPI.storage.getBytesInUse(null) : undefined);

  if (navigator.storage instanceof Object && navigator.storage.estimate instanceof Function) {
...
µb.loadUserSettings = async function () {
  ...
  const results = await Promise.all([vAPI.storage.get(Object.assign(usDefault)), vAPI.adminStorage.get("userSettings")]);

  const usUser = (results[0] instanceof Object && results[0]) || Object.assign(usDefault);
  ...
  if (Array.isArray(results[1])) {
    ...
    for (const entry of adminSettings) {
      ...
      const name = entry[0];
      if (usDefault.hasOwnProperty(name) === false) {
        continue;
      }
      const value = entry.length < 2 ? usDefault[name] : this.settingValueFromString(usDefault, name, entry[1]);
      if (value === undefined) {
        continue;
      }
...
µb.saveUserSettings = function () {
  ...
  this.userSettings.externalLists = this.userSettings.importedLists.join("\n");

  const toSave = this.getModifiedSettings(this.userSettings, this.userSettingsDefault);

  const toRemove = [];
  for (const key in this.userSettings) {
    if (this.userSettings.hasOwnProperty(key) === false) {
      continue;
    }
...
µb.loadHiddenSettings = async function () {
  ...
  const results = await Promise.all([
    vAPI.adminStorage.get(["advancedSettings", "disableDashboard", "disabledPopupPanelParts"]),
    vAPI.storage.get("hiddenSettings")
  ]);

  if (results[0] instanceof Object) {
    const { advancedSettings, disableDashboard, disabledPopupPanelParts } = results[0];
    if (Array.isArray(advancedSettings)) {
      for (const entry of advancedSettings) {
        ...
        const name = entry[0];
        if (hsDefault.hasOwnProperty(name) === false) {
          continue;
        }
        const value = entry.length < 2 ? hsDefault[name] : this.hiddenSettingValueFromString(name, entry[1]);
        if (value === undefined) {
          continue;
        }
    ...
    if (Array.isArray(disabledPopupPanelParts)) {
      ...
      hsDefault.popupPanelDisabledSections = hsAdmin.popupPanelDisabledSections = hsUser.popupPanelDisabledSections = bits;
    }
  }
  ...
  for (const key in hsDefault) {
    if (hsDefault.hasOwnProperty(key) === false) {
      continue;
    }
    if (hsAdmin.hasOwnProperty(name)) {
      continue;
    }
    if (typeof hs[key] !== typeof hsDefault[key]) {
      continue;
    }
...
µb.saveHiddenSettings = function () {
  vAPI.storage.set({
    hiddenSettings: this.getModifiedSettings(this.hiddenSettings, this.hiddenSettingsDefault)
  });
};
...
µb.hiddenSettingsFromString = function (raw) {
  ...
  while (lineIter.eot() === false) {
    ...
    if (this.hiddenSettingsAdmin.hasOwnProperty(name)) {
      continue;
    }
...
µb.isTrustedList = function (assetKey) {
  if (this.parsedTrustedListPrefixes.length === 0) {
    this.parsedTrustedListPrefixes = µb.hiddenSettings.trustedListPrefixes
      .split(/ +/)
      .map((prefix) => {
        if (prefix === "") {
          return;
...
µb.listKeysFromCustomFilterLists = function (raw) {
  const urls = typeof raw === "string" ? raw.trim().split(/[\n\r]+/) : raw;
  const out = new Set();
  const reIgnore = /^[!#]/;
  const reValid = /^[a-z-]+:\/\/\S+/;
  for (const url of urls) {
    if (reIgnore.test(url) || !reValid.test(url)) {
      continue;
    }
    // Ignore really bad lists.
    if (this.badLists.get(url) === true) {
      continue;
    }
    out.add(url);
  }
...
µb.appendUserFilters = async function (filters, options) {
  ...
  let comment = "";
  if (options instanceof Object && options.autoComment === true && this.hiddenSettings.autoCommentFilterTemplate.indexOf("{{") !== -1) {
    const d = new Date();
    // Date in YYYY-MM-DD format - https://stackoverflow.com/a/50130338
    const ISO8601Date = new Date(d.getTime() + d.getTimezoneOffset() * 60000).toISOString().split("T")[0];
  ...
  if (comment !== "") {
    ...
    if (end === -1 || details.content.startsWith("\n", end) === false || details.content.includes("\n!", end)) {
      filters = "\n" + comment + "\n" + filters;
    }
  }
...
µb.createUserFilters = function (details) {
  ...
  cosmeticFilteringEngine.removeFromSelectorCache(hostnameFromURI(details.docURL));
};
...
µb.addInMemoryFilter = async function (raw) {
  if (this.inMemoryFilters.includes(raw)) {
    return true;
  }
...
µb.clearInMemoryFilters = async function () {
  if (this.inMemoryFilters.length === 0) {
    return;
  }
...
µb.getAvailableLists = async function () {
  ...
  // Custom filter lists
  const importedListKeys = new Set(this.listKeysFromCustomFilterLists(this.userSettings.importedLists));
  ...
  for (const [stockAssetKey, stockEntry] of Object.entries(newAvailableLists)) {
    if (stockEntry.content !== "filters") {
      continue;
    }
    if (stockEntry.group === "user") {
      continue;
    }
    if (stockEntry.submitter === "user") {
      continue;
    }
    if (stockAssetKey.includes("://")) {
      continue;
    }
    const contentURLs = Array.isArray(stockEntry.contentURL) ? stockEntry.contentURL : [stockEntry.contentURL];
  ...
  for (const [assetKey, oldEntry] of Object.entries(oldAvailableLists)) {
    if (newAvailableLists[assetKey] !== undefined) {
      continue;
    }
    ...
    const listURL = Array.isArray(oldEntry.contentURL) ? oldEntry.contentURL[0] : oldEntry.contentURL;
    if (this.badLists.has(listURL)) {
      continue;
    }
    const newEntry = {
      content: "filters",
  ...
  for (const [assetKey, asset] of Object.entries(newAvailableLists)) {
    if (asset.submitter !== "user") {
      continue;
    }
    if (importedListKeys.has(assetKey)) {
      continue;
    }
...

{
  ...
  const onFilterListsReady = (lists) => {
    ...
    if (µb.inMemoryFilters.length !== 0) {
      if (µb.inMemoryFiltersCompiled === "") {
        µb.inMemoryFiltersCompiled = µb.compileFilters(µb.inMemoryFilters.join("\n"), {
          assetKey: "in-memory",
          trustedSource: true
        });
      }
      if (µb.inMemoryFiltersCompiled !== "") {
        toLoad.push(µb.applyCompiledFilters(µb.inMemoryFiltersCompiled, true));
      }
    }
  ...
  µb.loadFilterLists = function () {
    ...
    loadingPromise = Promise.all([this.getAvailableLists().then((lists) => onFilterListsReady(lists)), this.loadRedirectResources()]).then(
      () => {
        onDone();
      }
    );
...
µb.getCompiledFilterList = async function (assetKey) {
  ...
  //   format.
  if (this.compiledFormatChanged === false && this.badLists.has(assetKey) === false) {
...
µb.compileFilters = function (rawText, details = {}) {
  ...
  const lineIter = new LineIterator(sfp.utils.preparser.prune(rawText, vAPI.webextFlavor.env));
  ...
  const compiledContent = `${this.systemSettings.compiledMagic}\n` + writer.toString();
...
µb.loadRedirectResources = async function () {
  try {
    ...
    const fetchPromises = [redirectEngine.loadBuiltinResources(fetcher)];
    ...
    for (let i = 1; i < results.length; i++) {
      ...
      if (result instanceof Object === false || typeof result.content !== "string" || result.content === "") {
        continue;
      }
      content += "\n\n" + result.content;
    }
...
µb.loadPublicSuffixList = async function () {
  ...
  if (vAPI.canWASM && this.hiddenSettings.disableWebAssembly !== true) {
    ...
    let result = false;
    try {
      result = await psl.enableWASM(wasmModuleFetcher, "./lib/publicsuffixlist/wasm/");
    } catch (reason) {
      ubolog(reason);
    }
    if (result) {
...

{
  ...
  const create = async function () {
    ...
    if (Object.keys(µb.availableFilterLists).length === 0) {
      return;
    }
    await Promise.all([
      io.put(
      ...
      staticExtFilteringEngine.toSelfie("selfie/staticExtFilteringEngine"),
      staticNetFilteringEngine.toSelfie(io, "selfie/staticNetFilteringEngine")
    ]);
    lz4Codec.relinquish();
  ...
  const loadMain = async function () {
    ...
    if (details instanceof Object === false || typeof details.content !== "string" || details.content === "") {
      return false;
    }
    let selfie;
    try {
    ...
    if (selfie instanceof Object === false) {
      return false;
    }
    if (selfie.magic !== µb.systemSettings.selfieMagic) {
      return false;
    }
    if (selfie.availableFilterLists instanceof Object === false) {
      return false;
    }
    if (Object.keys(selfie.availableFilterLists).length === 0) {
      return false;
    }
  ...
  const load = async function () {
    ...
    try {
      const results = await Promise.all([
        ...
        staticNetFilteringEngine.fromSelfie(io, "selfie/staticNetFilteringEngine")
      ]);
...
µb.restoreAdminSettings = async function () {
  ...
  let data;
  try {
    const store = (await vAPI.adminStorage.get(["adminSettings", "toOverwrite"])) || {};
  ...
  if (typeof data.assetsBootstrapLocation === "string" && data.assetsBootstrapLocation !== "") {
  ...
  if (typeof data.userSettings === "object") {
    ...
    for (const name in µbus) {
      if (µbus.hasOwnProperty(name) === false) {
        continue;
      }
      if (adminus.hasOwnProperty(name) === false) {
        continue;
      }
  ...
  if (Array.isArray(toOverwrite.filterLists) && toOverwrite.filterLists.length !== 0) {
    const importedLists = [];
    for (const list of toOverwrite.filterLists) {
      if (/^[a-z-]+:\/\//.test(list) === false) {
        continue;
      }
  ...
  if (Array.isArray(toOverwrite.trustedSiteDirectives) && toOverwrite.trustedSiteDirectives.length !== 0) {
  ...
  if (Array.isArray(toOverwrite.filters) && toOverwrite.filters.length !== 0) {
...
µb.listMatchesEnvironment = function (details) {
  ...
  if (typeof details.lang === "string") {
    ...
    if (re !== undefined && re.test(details.lang)) {
      return true;
    }
  }
  // Matches user agent?
...

{
  ...
  µb.scheduleAssetUpdater = async function (updateDelay) {
    ...
    const fetchDelay = needEmergencyUpdate ? 2000 : this.hiddenSettings.autoUpdateAssetFetchPeriod * 1000 || 60000;
...
µb.assetObserver = function (topic, details) {
  ...
  if (topic === "after-asset-updated") {
    ...
    if (details.assetKey.startsWith("selfie/")) {
      return;
    }
  ...
  if (topic === "after-assets-updated") {
    if (details.assetKeys.length !== 0) {
      ...
      if (this.hiddenSettings.userResourcesLocation !== "unset" || vAPI.webextFlavor.soup.has("devbuild")) {
  ...
  if (topic === "assets.json-updated") {
    ...
    if (newDict["assets.json"] === undefined) {
      return;
    }
    if (oldDict["assets.json"] === undefined) {
      return;
    }
    ...
    if (newDefaultListset.size === 0) {
      return;
    }
    if (oldDefaultListset.size === 0) {
      Array.from(Object.entries(oldDict))
        .filter((a) => a[1].content === "filters" && a[1].off === undefined && /^https?:\/\//.test(a[0]) === false)
        .map((a) => a[0])
    ...
    for (const assetKey of oldDefaultListset) {
      if (newDefaultListset.has(assetKey)) {
        continue;
      }
      selectedListset.delete(assetKey);
    ...
    for (const assetKey of newDefaultListset) {
      if (oldDefaultListset.has(assetKey)) {
        continue;
      }
