...
const contextMenu = (() => {
  ...
  const onBlockElementInFrame = function (details, tab) {
    ...
    if (/^https?:\/\//.test(details.frameUrl) === false) {
      return;
    }
  ...
  const onSubscribeToList = function (details) {
    ...
    const hash = µb.selectedFilterLists.indexOf(parsedURL) !== -1 ? "#subscribed" : "";
    vAPI.tabs.open({
      url: `/asset-viewer.html` + `?url=${encodeURIComponent(url)}` + `&title=${encodeURIComponent(title)}` + `&subscribe=1${hash}`,
      select: true
    });
  };
