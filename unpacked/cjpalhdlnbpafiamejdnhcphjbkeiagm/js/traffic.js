...
import { entityFromDomain, isNetworkURI } from "./uri-utils.js";
...
const onHeadersReceived = function (details) {
  ...
  if (isRootDoc === false && fctxt.itype !== fctxt.SUB_FRAME) {
    return;
  }
  ...
  const filteredHTML = µb.canFilterResponseData && filterDocument(fctxt, details) === true;
...
const filterDocument = (() => {
  ...
  const onStreamStop = function () {
    ...
    if (charsetFound === undefined) {
      ...
      doc = domParser.parseFromString(utf8TextDecoder.decode(filterer.buffer.slice(0, 1024)), filterer.mime);
    ...
    const doctypeStr = doc.doctype instanceof Object ? xmlSerializer.serializeToString(doc.doctype) + "\n" : "";
  ...
  return function (fctxt, extras) {
    ...
    const stream = (request.stream = browser.webRequest.filterResponseData(extras.requestId));
