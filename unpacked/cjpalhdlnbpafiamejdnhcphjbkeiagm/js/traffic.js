...
let dontCacheResponseHeaders = vAPI.webextFlavor.soup.has("firefox");
...
window.addEventListener(
  "webextFlavor",
  function () {
    dontCacheResponseHeaders = vAPI.webextFlavor.soup.has("firefox");
  },
  { once: true }
);
...
const patchLocalRedirectURL = (url) => (url.charCodeAt(0) === 0x2f /* '/' */ ? vAPI.getURL(url) : url);
...
const onBeforeRequest = function (details) {
  ...
  // Not blocked
  if (fctxt.itype === fctxt.SUB_FRAME && details.parentFrameId !== -1 && details.aliasURL === undefined) {
...
const onBeforeRootFrameRequest = function (fctxt) {
  ...
  if (result === 0 && sessionSwitches.evaluateZ("no-strict-blocking", requestHostname)) {
    result = 2;
    if (logger.enabled) {
      logData = {
        engine: "u",
  ...
  vAPI.tabs.replace(fctxt.tabId, vAPI.getURL("document-blocked.html?details=") + query);
...
const validateStrictBlock = function (fctxt, logData) {
  ...
  return end === 0 || end === 1 || (end === 2 && url.charCodeAt(hnpos + hnlen) === 0x2e) /* '.' */;
};
...
const onBeforeBehindTheSceneRequest = function (fctxt) {
  ...
  if (
    (fctxt.tabOrigin.endsWith("-scheme") === false && isNetworkURI(fctxt.tabOrigin)) ||
    ...
    //   whitelisted.
    if (result === 1 && µb.getNetFilteringSwitch(fctxt.tabOrigin) === false) {
      result = 2;
...

{
  ...
  const gc = () => {
    if (pageStoresToken !== µb.pageStoresToken) {
      return reset();
    }
    gcTimer.on(30011);
  };
  ...
  onBeforeBehindTheSceneRequest.journalAddRequest = (fctxt, result) => {
    ...
    if (docHostname !== hostname || pageStoresToken !== µb.pageStoresToken) {
      hostname = docHostname;
      pageStores.clear();
      for (const pageStore of µb.pageStores.values()) {
        if (pageStore.tabHostname !== docHostname) {
          continue;
        }
...
const onHeadersReceived = function (details) {
  ...
  if (pageStore.getNetFilteringSwitch(fctxt) === false) {
    return;
  }
  ...
  if (bodyFilterer.canFilter(fctxt, details)) {
    ...
    const replaceDirectives = staticNetFilteringEngine.matchAndFetchModifiers(fctxt, "replace");
...
function textResponseFilterer(session, directives) {
  ...
  for (const directive of directives) {
    ...
    if (cache.re.test(session.getString()) !== true) {
      continue;
    }
    cache.re.lastIndex = 0;
    session.setString(session.getString().replace(cache.re, cache.replacement));
    applied.push(directive);
  }
  if (applied.length === 0) {
    return;
...
function htmlResponseFilterer(session, selectors) {
  ...
  const doc = htmlResponseFilterer.domParser.parseFromString(session.getString(), session.mime);
  ...
  const doctypeStr = [
    doc.doctype instanceof Object ? htmlResponseFilterer.xmlSerializer.serializeToString(doc.doctype) + "\n" : "",
...
const bodyFilterer = (() => {
  ...
  const onStreamData = function (ev) {
    ...
    const buffer = new Uint8Array(session.buffer.byteLength + ev.data.byteLength);
  ...
  return class Session extends µb.FilteringContext {
    ...
    static canFilter(fctxt, details) {
      ...
      mime = "text/plain";
      charset = "utf-8";
      const contentType = headerValueFromName("content-type", headers) || contentTypeFromDetails(details);
      if (contentType !== "") {
        ...
        if (mime.startsWith("text/") === false) {
          if (otherValidMimes.has(mime) === false) {
            return;
          }
        }
...
const injectCSP = function (fctxt, pageStore, responseHeaders) {
  ...
  if (cspSubsets.length !== 0 && sessionURLFiltering.evaluateZ(fctxt.getTabHostname(), fctxt.url, "csp") === 2) {
    if (logger.enabled) {
      fctxt.setRealm("network").setType("csp").setFilter(sessionURLFiltering.toLogData()).toLogger();
    }
    return;
  }
  ...
  if (
    cspSubsets.length !== 0 &&
    µb.userSettings.advancedUserEnabled &&
    sessionFirewall.evaluateCellZY(fctxt.getTabHostname(), fctxt.getTabHostname(), "*") === 2
  ) {
    if (logger.enabled) {
      fctxt.setRealm("network").setType("csp").setFilter(sessionFirewall.toLogData()).toLogger();
    }
    return;
  }
...
const strictBlockBypasser = {
  ...
  isBypassed: function (hostname) {
    if (this.hostnameToDeadlineMap.size === 0) {
      return false;
    }
...
const webRequest = {
  ...
  start: (() => {
    ...
    return () => {
      ...
      vAPI.net.addListener("onHeadersReceived", onHeadersReceived, { urls: ["http://*/*", "https://*/*"] }, [
        "blocking",
        "responseHeaders"
      ]);
      ...
      vAPI.defer
        .once({ sec: µb.hiddenSettings.toolbarWarningTimeout })
        .then(() => {
          if (vAPI.net.hasUnprocessedRequest() === false) {
            return;
          }
