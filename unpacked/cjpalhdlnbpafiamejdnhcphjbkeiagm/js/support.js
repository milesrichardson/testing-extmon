...
async function showSupportData() {
  ...
  uselessKeys.forEach((prop) => {
    removeKey(shownData, prop);
  });
  const redacted = true;
  if (redacted) {
    sensitiveValues.forEach((prop) => {
      redactValue(shownData, prop);
    });
    sensitiveKeys.forEach((prop) => {
      redactKeys(shownData, prop);
    });
  }
...
function reportSpecificFilterIssue() {
  ...
  githubURL.searchParams.set("url_address_of_the_web_page", "`" + qs$('select[name="url"]').value + "`");
...
async function updateFilterLists() {
  if (dom.body.dataset.shouldUpdateLists === undefined) {
    return false;
  }
