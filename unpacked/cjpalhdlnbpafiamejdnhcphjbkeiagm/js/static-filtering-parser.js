...
export const removableHTTPHeaders = new Set(["location", "refresh", "report-to", "set-cookie"]);
...
class argListParser {
  ...
  normalizeArg(s) {
    ...
    if (s.includes(this.separatorChar) === false) {
      return;
    }
...
export class AstFilterParser {
  ...
  parse(raw) {
    ...
    if (c1st === 0x7c /* | */) {
      if (clast === 0x5e /* ^ */ && this.reHnAnchoredHostnameAscii.test(this.raw)) {
        // ||example.com^
        this.astType = AST_TYPE_NETWORK;
        this.astTypeFlavor = AST_TYPE_NETWORK_PATTERN_HOSTNAME;
        const node = this.astFromTemplate(this.rootNode, astTemplates.netHnAnchoredHostnameAscii);
      ...
      if (this.raw.endsWith("$third-party") && this.reHn3pAnchoredHostnameAscii.test(this.raw)) {
        ...
        this.astType = AST_TYPE_NETWORK;
        this.astTypeFlavor = AST_TYPE_NETWORK_PATTERN_HOSTNAME;
        const node = this.astFromTemplate(this.rootNode, astTemplates.net3pHnAnchoredHostnameAscii);
        this.linkDown(this.rootNode, node);
        return;
      }
      if (this.reHnAnchoredPlainAscii.test(this.raw)) {
        ...
        const node = this.astFromTemplate(this.rootNode, astTemplates.netHnAnchoredPlainAscii);
        this.linkDown(this.rootNode, node);
        return;
      }
    } else if (c1st === 0x23 /* # */) {
      if (this.rePlainGenericCosmetic.test(this.raw)) {
        ...
        const node = this.astFromTemplate(this.rootNode, astTemplates.extPlainGenericSelector);
        this.linkDown(this.rootNode, node);
      ...
      if (this.reNetHosts1.test(this.raw)) {
        ...
        this.astType = AST_TYPE_NETWORK;
        this.astTypeFlavor = AST_TYPE_NETWORK_PATTERN_HOSTNAME;
        const node = this.astFromTemplate(this.rootNode, astTemplates.netHosts1);
        this.linkDown(this.rootNode, node);
        return;
      }
    } else if (c1st === 0x30 /* 0 */) {
      if (this.reNetHosts2.test(this.raw)) {
        ...
        this.astType = AST_TYPE_NETWORK;
        this.astTypeFlavor = AST_TYPE_NETWORK_PATTERN_HOSTNAME;
        const node = this.astFromTemplate(this.rootNode, astTemplates.netHosts2);
        this.linkDown(this.rootNode, node);
        return;
      }
      ...
      const node = this.astFromTemplate(this.rootNode, astTemplates.netPlainAscii);
      this.linkDown(this.rootNode, node);
      return;
    }
  ...
  parseRaw(parent) {
    ...
    if (l1 !== 0) {
      next = this.allocTypedNode(NODE_TYPE_WHITESPACE, parentBeg, parentBeg + l1);
    ...
    if (r0 !== l1) {
      next = this.allocTypedNode(NODE_TYPE_LINE_BODY, parentBeg + l1, parentBeg + r0);
      this.linkDown(next, this.parseFilter(next));
      prev = this.linkRight(prev, next);
    }
    if (r0 !== parentEnd) {
      next = this.allocTypedNode(NODE_TYPE_WHITESPACE, parentBeg + r0, parentEnd);
  ...
  parsePreparseDirective(parent, s) {
    ...
    const head = this.allocTypedNode(NODE_TYPE_PREPARSE_DIRECTIVE, parentBeg, directiveEnd);
  ...
  parseNet(parent) {
    ...
    if (anchorBeg !== parentEnd) {
      tail = this.allocTypedNode(NODE_TYPE_NET_OPTIONS_ANCHOR, anchorBeg, anchorBeg + 1);
      next = this.allocTypedNode(NODE_TYPE_NET_OPTIONS, anchorBeg + 1, parentEnd);
    ...
    next = this.allocTypedNode(NODE_TYPE_NET_PATTERN_RAW, patternBeg, anchorBeg);
      ...
      switch (type) {
        ...
        case NODE_TYPE_NET_OPTION_NAME_CSP:
          realBad =
            ...
            modifierType !== 0 ||
            this.reBadCSP.test(this.getNetOptionValue(NODE_TYPE_NET_OPTION_NAME_CSP));
          if (realBad) {
            break;
          }
          modifierType = type;
  ...
  indexOfNetAnchor(s, start = 0) {
    ...
    for (;;) {
      ...
      if (j !== start && before === 0x24 /* $ */) {
        return -1;
      }
      const after = s.charCodeAt(j + 1);
      if (after !== 0x29 /* ) */ && after !== 0x2f /* / */ && after !== 0x7c /* | */ && before !== 0x5c /* \ */) {
        return j;
      }
      if (j <= start) {
        break;
  ...
  parseNetPattern(parent) {
    ...
    if (parentEnd === parentBeg) {
      ...
      const node = this.allocTypedNode(NODE_TYPE_NET_PATTERN, parentBeg, parentEnd);
    ...
    if (
      ...
      this.astTypeFlavor = AST_TYPE_NETWORK_PATTERN_HOSTNAME;
      this.addFlags(AST_FLAG_NET_PATTERN_LEFT_HNANCHOR | AST_FLAG_NET_PATTERN_RIGHT_PATHANCHOR);
      next = this.allocTypedNode(NODE_TYPE_NET_PATTERN_LEFT_HNANCHOR, parentBeg, parentBeg + 2);
      prev = this.linkRight(prev, next);
      next = this.allocTypedNode(NODE_TYPE_NET_PATTERN, parentBeg + 2, parentEnd - 1);
      pattern = pattern.slice(2, -1);
      const normal = this.hasUnicode ? this.normalizeHostnameValue(pattern) : pattern;
      if (normal !== undefined && normal !== pattern) {
        this.setNodeTransform(next, normal);
      }
      this.addNodeToRegister(NODE_TYPE_NET_PATTERN, next);
      prev = this.linkRight(prev, next);
      next = this.allocTypedNode(NODE_TYPE_NET_PATTERN_PART_SPECIAL, parentEnd - 1, parentEnd);
      this.linkRight(prev, next);
      return this.throwHeadNode(head);
    }
    ...
    if (this.hasWhitespace && this.isException() === false && this.hasOptions() === false && this.reHostsSink.test(pattern)) {
      ...
      this.astTypeFlavor = AST_TYPE_NETWORK_PATTERN_HOSTNAME;
      this.addFlags(AST_FLAG_NET_PATTERN_LEFT_HNANCHOR | AST_FLAG_NET_PATTERN_RIGHT_PATHANCHOR);
      next = this.allocTypedNode(NODE_TYPE_NET_PATTERN, patternBeg, parentEnd);
      this.addNodeToRegister(NODE_TYPE_NET_PATTERN, next);
      this.linkRight(prev, next);
      return this.throwHeadNode(head);
    }
    ...
    // Regex?
    if (c1st === 0x2f /* / */ && clast === 0x2f /* / */ && pattern.length > 2) {
    ...
    if (exCharCodeAt(pattern, -1) === 0x7c /* | */) {
      const type = exCharCodeAt(pattern, -2) !== 0x2a /* * */ ? NODE_TYPE_NET_PATTERN_RIGHT_ANCHOR : NODE_TYPE_IGNORE;
    ...
    if (hasWildcard && this.rePointlessLeadingWildcards.test(pattern)) {
      ...
      next = this.allocTypedNode(NODE_TYPE_IGNORE, patternBeg, patternBeg + ignoreLen);
      prev = this.linkRight(prev, next);
    ...
    if (this.rePointlessTrailingSeparator.test(pattern)) {
      ...
      const ignoreLen = match[1].length;
      next = this.allocTypedNode(NODE_TYPE_IGNORE, patternEnd - ignoreLen, patternEnd);
    ...
    if (hasWildcard && this.rePointlessTrailingWildcards.test(pattern)) {
      ...
      const ignoreLen = match[1].length;
      const needWildcard = pattern.charCodeAt(0) === 0x2f && exCharCodeAt(pattern, -ignoreLen - 1) === 0x2f;
      ...
      if (badWildcardBeg !== patternEnd) {
        next = this.allocTypedNode(NODE_TYPE_IGNORE, badWildcardBeg, patternEnd);
        if (tail !== 0) {
          this.linkRight(next, tail);
        }
        tail = next;
  ...
  parsePatternParts(parent, pattern) {
    ...
    for (const match of matches) {
      ...
      if (plainPartEnd !== plainPartBeg) {
        next = this.allocTypedNode(NODE_TYPE_NET_PATTERN_PART, parentBeg + plainPartBeg, parentBeg + plainPartEnd);
        prev = this.linkRight(prev, next);
      }
      ...
      next = this.allocTypedNode(type, parentBeg + plainPartEnd, parentBeg + plainPartBeg);
      prev = this.linkRight(prev, next);
    }
    if (plainPartBeg !== pattern.length) {
      next = this.allocTypedNode(NODE_TYPE_NET_PATTERN_PART, parentBeg + plainPartBeg, parentBeg + pattern.length);
      this.linkRight(prev, next);
    }
    return this.throwHeadNode(head);
  }
  ...
  normalizePattern(pattern) {
    ...
    try {
      normal = normal.replace(this.reUnicodeChars, (s) => encodeURIComponent(s).toLowerCase());
    } catch (ex) {
      return;
    }
    return normal;
  ...
  parseNetOptions(parent) {
    ...
    while (optionBeg !== optionsEnd) {
      ...
      next = this.allocTypedNode(NODE_TYPE_NET_OPTION_RAW, parentBeg + optionBeg, parentBeg + optionEnd);
      ...
      next = this.allocTypedNode(NODE_TYPE_NET_OPTION_SEPARATOR, parentBeg + optionEnd, parentBeg + optionBeg);
    ...
    this.linkRight(prev, this.allocSentinelNode(NODE_TYPE_NET_OPTION_SENTINEL, parentEnd));
    return this.throwHeadNode(head);
  }
  ...
  parseNetOption(parent) {
    ...
    if (s.charCodeAt(0) === 0x7e) {
      ...
      next = this.allocTypedNode(NODE_TYPE_NET_OPTION_NAME_NOT, parentBeg, parentBeg + 1);
      prev = this.linkRight(prev, next);
      nameBeg += 1;
    }
    ...
    next = this.allocTypedNode(nodeOptionType, parentBeg + nameBeg, parentBeg + nameEnd);
    ...
    next = this.allocTypedNode(NODE_TYPE_NET_OPTION_ASSIGN, parentBeg + equalPos, parentBeg + valueBeg);
    prev = this.linkRight(prev, next);
    ...
    next = this.allocTypedNode(NODE_TYPE_NET_OPTION_VALUE, parentBeg + valueBeg, parentBeg + optionEnd);
  ...
  parseDomainList(parent, separator, mode = 0b00000) {
    const parentBeg = this.nodes[parent + NODE_BEG_INDEX];
    const parentEnd = this.nodes[parent + NODE_END_INDEX];
    const containerNode = this.allocTypedNode(NODE_TYPE_OPTION_VALUE_DOMAIN_LIST, parentBeg, parentEnd);
    if (parentEnd === parentBeg) {
    ...
    while (beg < listEnd) {
      ...
      if (end !== beg) {
        domainNode = this.allocTypedNode(NODE_TYPE_OPTION_VALUE_DOMAIN_RAW, parentBeg + beg, parentBeg + end);
      ...
      if (s.charCodeAt(end) === separatorCode) {
        ...
        separatorNode = this.allocTypedNode(NODE_TYPE_OPTION_VALUE_SEPARATOR, parentBeg + beg, parentBeg + end);
  ...
  normalizeDomainValue(s, modeBits) {
    if ((modeBits & 0b10000) === 0 || s.length <= 2 || s.charCodeAt(0) !== 0x2f /* / */ || exCharCodeAt(s, -1) !== 0x2f /* / */) {
  ...
  parseExt(parent, anchorBeg, anchorLen) {
    ...
    if (anchorBeg > parentBeg) {
      next = this.allocTypedNode(NODE_TYPE_EXT_OPTIONS, parentBeg, anchorBeg);
      this.addFlags(AST_FLAG_HAS_OPTIONS);
      this.addNodeToRegister(NODE_TYPE_EXT_OPTIONS, next);
      this.linkDown(next, this.parseDomainList(next, ",", 0b11110));
      prev = this.linkRight(prev, next);
    }
    next = this.allocTypedNode(NODE_TYPE_EXT_OPTIONS_ANCHOR, anchorBeg, anchorBeg + anchorLen);
    this.addNodeToRegister(NODE_TYPE_EXT_OPTIONS_ANCHOR, next);
    prev = this.linkRight(prev, next);
    next = this.allocTypedNode(NODE_TYPE_EXT_PATTERN_RAW, anchorBeg + anchorLen, parentEnd);
  ...
  parseExtPatternScriptletArgs(parent) {
    ...
    if (details.argBeg > 0) {
      next = this.allocTypedNode(NODE_TYPE_EXT_DECORATION, parentBeg, parentBeg + details.argBeg);
      prev = this.linkRight(prev, next);
    }
    const token = s.slice(details.argBeg, details.argEnd);
    const tokenEnd = details.argEnd - (token.endsWith(".js") ? 3 : 0);
    next = this.allocTypedNode(NODE_TYPE_EXT_PATTERN_SCRIPTLET_TOKEN, parentBeg + details.argBeg, parentBeg + tokenEnd);
    ...
    if (tokenEnd < details.argEnd) {
      next = this.allocTypedNode(NODE_TYPE_IGNORE, parentBeg + tokenEnd, parentBeg + details.argEnd);
      prev = this.linkRight(prev, next);
    }
    if (details.quoteEnd < argsEnd) {
      next = this.allocTypedNode(NODE_TYPE_EXT_DECORATION, parentBeg + details.argEnd, parentBeg + details.separatorEnd);
      prev = this.linkRight(prev, next);
    }
    // all args
    next = this.allocTypedNode(NODE_TYPE_EXT_PATTERN_SCRIPTLET_ARGS, parentBeg + details.separatorEnd, parentBeg + argsEnd);
  ...
  parseExtPatternScriptletArglist(parent) {
    ...
    for (;;) {
      ...
      if (decorationBeg < details.argBeg) {
        next = this.allocTypedNode(NODE_TYPE_EXT_DECORATION, parentBeg + decorationBeg, parentBeg + details.argBeg);
        prev = this.linkRight(prev, next);
      }
      if (i === argsEnd) {
        break;
      }
      next = this.allocTypedNode(NODE_TYPE_EXT_PATTERN_SCRIPTLET_ARG, parentBeg + details.argBeg, parentBeg + details.argEnd);
      if (details.transform) {
        const arg = s.slice(details.argBeg, details.argEnd);
        this.setNodeTransform(next, this.scriptletArgListParser.normalizeArg(arg));
      }
      prev = this.linkRight(prev, next);
      if (details.failed) {
        this.addNodeFlags(next, NODE_FLAG_ERROR);
  ...
  parseExtPatternHtml(parent) {
    ...
    const success = this.selectorCompiler.compile(this.result.raw, this.result, {
  ...
  parseExtPatternCosmetic(parent) {
    const parentBeg = this.nodes[parent + NODE_BEG_INDEX];
    const parentEnd = this.nodes[parent + NODE_END_INDEX];
    const head = this.allocTypedNode(NODE_TYPE_EXT_PATTERN_COSMETIC, parentBeg, parentEnd);
    this.result.exception = this.isException();
  ...
  isTokenCharCode(c) {
    return c === 0x25 || (c >= 0x30 && c <= 0x39) || (c >= 0x41 && c <= 0x5a) || (c >= 0x61 && c <= 0x7a);
  }
  ...
  normalizeHostnameValue(s, modeBits = 0b00000) {
    ...
    if (hasWildcard) {
      ...
      if (hn.length === 1) {
        if ((modeBits & 0b0100) === 0) {
          return "";
        }
        return;
      }
      if ((modeBits & 0b0010) !== 0) {
        if (this.rePlainEntity.test(hn)) {
          return;
        }
        if (this.reIsEntity.test(hn) === false) {
          return "";
        }
    ...
    if ((modeBits & 0b0001) === 0 && (hn.charCodeAt(0) === 0x2e /* . */ || exCharCodeAt(hn, -1) === 0x2e) /* . */) {
      return "";
    }
    return hn;
  }
  ...
  getDomainListIterator(root) {
    const iter =
      this.domainListIteratorJunkyard.length !== 0 ? this.domainListIteratorJunkyard.pop().reuse(root) : new DomainListIterator(this, root);
    return root !== 0 ? iter : iter.stop();
  }

  getNetFilterFromOptionIterator() {
    return this.getDomainListIterator(this.getBranchFromType(NODE_TYPE_NET_OPTION_NAME_FROM));
  }

  getNetFilterToOptionIterator() {
    return this.getDomainListIterator(this.getBranchFromType(NODE_TYPE_NET_OPTION_NAME_TO));
  }

  getNetFilterDenyallowOptionIterator() {
    return this.getDomainListIterator(this.getBranchFromType(NODE_TYPE_NET_OPTION_NAME_DENYALLOW));
  }

  getExtFilterDomainIterator() {
    return this.getDomainListIterator(this.getBranchFromType(NODE_TYPE_EXT_OPTIONS));
  }
...
class ExtSelectorCompiler {
  constructor(instanceOptions) {
    ...
    const cssSimple =
      ...
      ")";
    const cssCombinator = "(?: | [+>~] )";
    this.reCommonSelector = new RegExp(`^${cssSimple}(?:${cssCombinator}${cssSimple})*$`);
    ...
    this.knownPseudoElementsWithArgs = new Set(["part", "slotted"]);
    ...
    this.actionOperators = new Set([":remove", ":style"]);
    ...
    this.maybeProceduralOperatorNames = new Set(["has", "not"]);
    this.proceduralActionNames = new Set(["remove", "remove-attr", "remove-class", "style"]);
  ...
  compileSelector(raw) {
    ...
    if (this.astHasType(parts, "ProceduralSelector") === false && this.astHasType(parts, "ActionSelector") === false) {
...
export const utils = (() => {
  ...
  class regex {
    ...
    static tokenizableStrFromNode(node) {
      switch (node.type) {
        ...
        case 128: /* T_SPECIAL, 'Special' */ {
          ...
          return flags.MatchEnd === 1 || flags.MatchStart === 1 || flags.MatchWordBoundary === 1 ? "\x00" : "\x01";
        }
    ...
    static isValid(reStr) {
      try {
        ...
        if (regexAnalyzer !== null) {
          void this.tokenizableStrFromNode(regexAnalyzer(reStr, false).tree());
        }
      } catch (ex) {
        return false;
      }
    ...
    static toTokenizableStr(reStr) {
      ...
      let s = "";
      try {
        s = this.tokenizableStrFromNode(regexAnalyzer(reStr, false).tree());
      } catch (ex) {}
      ...
      for (;;) {
        ...
        s = left;
        s += this.firstCharCodeClass(right) === 1 || this.firstCharCodeClass(middle) === 1 ? "\x01" : "\x00";
        s += this.lastCharCodeClass(left) === 1 || this.lastCharCodeClass(middle) === 1 ? "\x01" : "\x00";
        s += right;
      }
      return s;
    }
