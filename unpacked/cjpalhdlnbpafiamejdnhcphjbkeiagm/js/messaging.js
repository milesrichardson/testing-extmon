...
import { domainFromHostname, domainFromURI, entityFromDomain, hostnameFromURI, isNetworkURI } from "./uri-utils.js";
...

{
  ...
  const getDomainNames = function (targets) {
    return targets.map((target) => {
      ...
      return target.indexOf("/") !== -1 ? domainFromURI(target) || "" : domainFromHostname(target) || target;
    });
  };
  ...
  const onMessage = function (request, sender, callback) {
    ...
    switch (request.what) {
      ...
      case "listsFromNetFilter":
        staticFilteringReverseLookup.fromNetFilter(request.rawFilter).then((response) => {
          callback(response);
        });
        return;

      case "listsFromCosmeticFilter":
        staticFilteringReverseLookup.fromExtendedFilter(request).then((response) => {
          callback(response);
        });
        return;
      ...
      case "snfeToDNR": {
        ...
        dnrRulesetFromRawLists(listPromises, options).then((result) => {
          ...
          const isUnsupported = (rule) => rule._error !== undefined;
          const isRegex = (rule) => rule.condition !== undefined && rule.condition.regexFilter !== undefined;
          const isRedirect = (rule) =>
            rule.action !== undefined && rule.action.type === "redirect" && rule.action.redirect.extensionPath !== undefined;
          const isCsp = (rule) => rule.action !== undefined && rule.action.type === "modifyHeaders";
    ...
    switch (request.what) {
      ...
      case "userSettings":
        ...
        if (response instanceof Object) {
          ...
          response.canLeakLocalIPAddresses = vAPI.browserSettings.canLeakLocalIPAddresses === true;
        }
        break;
...

{
  ...
  const getHostnameDict = function (hostnameDetailsMap, out) {
    ...
    for (const hnDetails of hostnameDetailsMap.values()) {
      ...
      const dnDetails = hostnameDetailsMap.get(domain) || { counts: createCounts() };
  ...
  const popupDataFromTabId = function (tabId, tabTitle) {
    ...
    if (pageStore) {
      ...
      r.noPopups = sessionSwitches.evaluateZ("no-popups", rootHostname);
      r.popupBlockedCount = pageStore.popupBlockedCount;
      r.noCosmeticFiltering = sessionSwitches.evaluateZ("no-cosmetic-filtering", rootHostname);
      r.noLargeMedia = sessionSwitches.evaluateZ("no-large-media", rootHostname);
      r.largeMediaCount = pageStore.largeMediaCount;
      r.noRemoteFonts = sessionSwitches.evaluateZ("no-remote-fonts", rootHostname);
      r.remoteFontCount = pageStore.remoteFontCount;
      r.noScripting = sessionSwitches.evaluateZ("no-scripting", rootHostname);
    } else {
      r.hostnameDict = {};
      getFirewallRules(undefined, r);
    }

    r.matrixIsDirty = sessionFirewall.hasSameRules(permanentFirewall, rootHostname, r.hostnameDict) === false;
    if (r.matrixIsDirty === false) {
      r.matrixIsDirty = sessionSwitches.hasSameRules(permanentSwitches, rootHostname) === false;
    }
    return r;
  };
  ...
  const launchReporter = async function (request) {
    ...
    if (filters.length !== 0) {
      request.popupPanel.extended = Array.from(new Set(filters.map((s) => (s.length <= 64 ? s : `${s.slice(0, 64)}…`))));
    }
  ...
  const onMessage = function (request, sender, callback) {
    ...
    switch (request.what) {
      ...
      case "revertFirewallRules":
        ...
        sessionFirewall.copyRules(permanentFirewall, request.srcHostname, Object.assign(Object.create(null), request.desHostnames));
        sessionSwitches.copyRules(permanentSwitches, request.srcHostname);
      ...
      case "saveFirewallRules":
        // TODO: use Set() to message around sets of hostnames
        if (permanentFirewall.copyRules(sessionFirewall, request.srcHostname, Object.assign(Object.create(null), request.desHostnames))) {
          µb.savePermanentFirewallRules();
        }
        if (permanentSwitches.copyRules(sessionSwitches, request.srcHostname)) {
...

{
  ...
  const retrieveContentScriptParameters = async function (sender, request) {
    ...
    const noSpecificCosmeticFiltering = pageStore.shouldApplySpecificCosmeticFilters(frameId) === false;
    ...
    const scf = (response.specificCosmeticFilters = cosmeticFilteringEngine.retrieveSpecificSelectors(request, response));
    ...
    if (
      ...
      (logger.enabled && (scf.convertedProceduralFilters.length !== 0 || scf.exceptedFilters.length !== 0))
    ) {
    ...
    if (logger.enabled || request.needScriptlets) {
      ...
      if (scriptletDetails !== undefined) {
        if (logger.enabled) {
          scriptletFilteringEngine.logFilters(tabId, request.url, scriptletDetails.filters);
        }
    ...
    //   cosmetic filters.
    if (logger.enabled) {
      if (noSpecificCosmeticFiltering === false || noGenericCosmeticFiltering === false) {
  ...
  const onMessage = function (request, sender, callback) {
    ...
    switch (request.what) {
      case "retrieveContentScriptParameters":
        return retrieveContentScriptParameters(sender, request).then((response) => {
          callback(response);
        });
      default:
        break;
    ...
    switch (request.what) {
      ...
      case "getCollapsibleBlockedRequests":
        response = {
          id: request.id,
          hash: request.hash,
          netSelectorCacheCountMax: cosmeticFilteringEngine.netSelectorCacheCountMax
        };
        if (µb.userSettings.collapseBlocked && pageStore && pageStore.getNetFilteringSwitch()) {
        ...
        return vAPI.tabs
          ...
          .then(() => {
            callback({
              ...
              pickerURL: vAPI.getURL(`/web_accessible_resources/epicker-ui.html?secret=${vAPI.warSecret.short()}`)
            });
...

{
  ...
  const toBase64 = function (data) {
    const value = data instanceof Uint8Array ? denseBase64.encode(data) : data;
...

{
  ...
  const backupUserData = async function () {
    ...
    const userData = {
      timeStamp: Date.now(),
      version: vAPI.app.version,
      userSettings: µb.getModifiedSettings(µb.userSettings, µb.userSettingsDefault),
      selectedFilterLists: µb.selectedFilterLists,
      hiddenSettings: µb.getModifiedSettings(µb.hiddenSettings, µb.hiddenSettingsDefault),
      whitelist: µb.arrayFromWhitelist(µb.netWhitelist),
    ...
    const filename = i18n$("aboutBackupFilename").replace("{{datetime}}", µb.dateNowToSensibleString()).replace(/ +/g, "_");
  ...
  const restoreUserData = async function (request) {
    ...
    // storages
    await Promise.all([cacheStorage.clear(), vAPI.storage.clear()]);
    ...
    if (hiddenSettings instanceof Object === false) {
      hiddenSettings = µb.hiddenSettingsFromString(userData.hiddenSettingsString || "");
    }
    // Discard unknown setting or setting with default value.
    for (const key in hiddenSettings) {
      if (µb.hiddenSettingsDefault.hasOwnProperty(key) === false || hiddenSettings[key] === µb.hiddenSettingsDefault[key]) {
    ...
    if (Array.isArray(whitelist) === false && typeof userData.netWhitelist === "string" && userData.netWhitelist !== "") {
  ...
  const getLists = async function (callback) {
    ...
    const [lists, metadata] = await Promise.all([µb.getAvailableLists(), io.metadata()]);
    r.available = lists;
  ...
  const getRules = function () {
    return {
      permanentRules: permanentFirewall.toArray().concat(permanentSwitches.toArray(), permanentURLFiltering.toArray()),
      sessionRules: sessionFirewall.toArray().concat(sessionSwitches.toArray(), sessionURLFiltering.toArray()),
  ...
  const getSupportData = async function () {
    ...
    const modifiedUserSettings = µb.getModifiedSettings(µb.userSettings, µb.userSettingsDefault);

    const modifiedHiddenSettings = µb.getModifiedSettings(µb.hiddenSettings, µb.hiddenSettingsDefault);
    ...
    for (const listKey in lists) {
      if (lists.hasOwnProperty(listKey) === false) {
        continue;
      }
    ...
    return {
      ...
      trustedset: diffArrays(µb.arrayFromWhitelist(µb.netWhitelist), µb.netWhitelistDefault),
      switchRuleset: diffArrays(sessionSwitches.toArray(), µb.hostnameSwitchesDefault),
      hostRuleset: diffArrays(sessionFirewall.toArray(), µb.dynamicFilteringDefault),
      urlRuleset: diffArrays(sessionURLFiltering.toArray(), []),
...

{
  ...
  const getLoggerData = async function (details, activeTabId, callback) {
    ...
    if (µb.pageStoresToken !== details.tabIdsToken) {
      ...
      for (const [tabId, pageStore] of µb.pageStores) {
        ...
        if (rawURL.startsWith(extensionOriginURL) === false || rawURL.startsWith(documentBlockedURL)) {
    ...
    if (activeTabId) {
      ...
      if (rawURL === null || (rawURL.startsWith(extensionOriginURL) && rawURL.startsWith(documentBlockedURL) === false)) {
  ...
  const getURLFilteringData = function (details) {
    ...
    for (const url of urls) {
      ...
      const pown = puf.r !== 0 && puf.context === context && puf.url === url && puf.type === type;
  ...
  const onMessage = function (request, sender, callback) {
    ...
    switch (request.what) {
      ...
      case "saveURLFilteringRules":
        response = permanentURLFiltering.copyRules(sessionURLFiltering, request.context, request.urls, request.type);
        if (response) {
          µb.savePermanentURLFilteringRules();
        }
        break;
...

{
  ...
  const logCSPViolations = function (pageStore, request) {
    ...
    const fctxt = µb.filteringContext.duplicate();
    fctxt.fromTabId(pageStore.tabId).setRealm("network").setDocOriginFromURL(request.docURL).setURL(request.docURL);
    ...
    for (const json of request.violations) {
      ...
      fctxt.setURL(violation.url).setType(type).setFilter(logData).toLogger();
    }
    ...
    switch (request.what) {
      case "inlinescriptFound":
        if (logger.enabled && pageStore !== null) {
          const fctxt = µb.filteringContext.duplicate();
          fctxt.fromTabId(tabId).setType("inline-script").setURL(request.docURL).setDocOriginFromURL(request.docURL);
      ...
      case "subscribeTo":
        ...
        const hash = µb.selectedFilterLists.indexOf(request.location) !== -1 ? "#subscribed" : "";
        vAPI.tabs.open({
