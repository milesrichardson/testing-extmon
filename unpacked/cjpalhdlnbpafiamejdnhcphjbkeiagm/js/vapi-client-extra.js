...
(() => {
  ...
  if (typeof vAPI !== "object" || vAPI.messaging instanceof Object === false || vAPI.MessagingConnection instanceof Function) {
    return;
  }
  ...
  vAPI.MessagingConnection = class {
    ...
    check() {
      this.checkTimer = undefined;
      if (connections.has(this.id) === false) {
        return;
      }
      const port = this.messaging.getPort();
      if (port === null) {
      ...
      switch (details.msg.what) {
        ...
        case "connectionRequested":
          ...
          if (result === true) {
            ...
            const connection = new vAPI.MessagingConnection(listener, details.msg);
            connections.set(connection.id, connection);
          } else {
...
(() => {
  ...
  if (typeof vAPI !== "object" || vAPI.messaging instanceof Object === false || vAPI.broadcastListener instanceof Object) {
    return;
  }
