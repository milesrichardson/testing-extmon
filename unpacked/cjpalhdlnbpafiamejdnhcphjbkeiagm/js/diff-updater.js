...
function parsePatch(patch) {
  ...
  while (i < n) {
    ...
    if (diffBlock.name === undefined) {
      return;
    }
    if (isNaN(diffBlock.lines) || diffBlock.lines <= 0) {
      return;
    }
    if (diffBlock.checksum === undefined) {
      return;
    }
...
async function applyPatchAndValidate(assetDetails, diffDetails) {
  ...
  const sha1Full = Array.from(arrayout)
...
async function fetchPatchDetailsFromCDNs(assetDetails) {
  ...
  if (Array.isArray(cdnURLs) === false) {
    return null;
  }
  if (cdnURLs.length === 0) {
    return null;
...
async function fetchAndApplyAllPatches(assetDetails) {
  ...
  if (patchData === null) {
    assetDetails.status = Date.now() - patchTime < 4 * assetDetails.diffExpires ? "nopatch-yet" : "nopatch";
    return assetDetails;
  }
...
bc.onmessage = (ev) => {
  ...
  switch (message.what) {
    case "update":
      fetchAndApplyAllPatches(message)
        .then((response) => {
