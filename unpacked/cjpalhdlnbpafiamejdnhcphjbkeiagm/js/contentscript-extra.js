...
if (typeof vAPI === "object" && typeof vAPI.DOMProceduralFilterer !== "object") {
  ...
  class PSelectorMatchesPathTask extends PSelectorTask {
    constructor(task) {
      super();
      this.needle = regexFromString(task[1].replace(/\P{ASCII}/gu, (s) => encodeURIComponent(s)));
    }
    transpose(node, output) {
    ...
    begin() {
      ...
      for (let keep of this.targets) {
        while (keep !== null && keep !== body) {
          ...
          while (discard !== null) {
            if (nonVisualElements[discard.localName] !== true && toKeep.has(discard) === false) {
  ...
  class PSelectorSpathTask extends PSelectorTask {
    ...
    static qsa(node, selector) {
      ...
      return parent.querySelectorAll(`:scope > :nth-child(${pos})${selector}`);
    }
  }
  ...
  class PSelectorXpathTask extends PSelectorTask {
    ...
    transpose(node, output) {
      this.xpr = this.xpe.evaluate(node, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE, this.xpr);
  ...
  class ProceduralFilterer {
    ...
    styleTokenFromStyle(style) {
      ...
      this.domFilterer.addCSS(`[${this.masterToken}][${styleToken}]\n{${style}}`, { silent: true, mustInject: true });
      return styleToken;
    }
    ...
    processNodes(nodes, action) {
      ...
      switch (op) {
        ...
        /* fall through */
        case "style": {
          const styleToken = this.styleTokenFromStyle(arg === "" ? vAPI.hideStyle : arg);
          for (const node of nodes) {
    ...
    createProceduralFilter(o) {
      return this.primeProceduralSelector(new PSelectorRoot(typeof o === "string" ? JSON.parse(o) : o));
    }
