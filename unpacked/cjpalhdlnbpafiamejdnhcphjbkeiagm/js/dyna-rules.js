...
const mergeView = new CodeMirror.MergeView(qs$(".codeMirrorMergeContainer"), {
...

{
  ...
  mergeGapObserver.observe(qs$(".CodeMirror-merge-copybuttons-left"), { attributes: true, attributeFilter: ["title"], subtree: true });
}
...
const updateOverlay = (() => {
  ...
  return function (filter) {
    reFilter = typeof filter === "string" && filter !== "" ? new RegExp(filter.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi") : undefined;
    return mode;
  };
})();
...
const rulesToDoc = function (clearHistory) {
  ...
  for (const key in thePanes) {
    if (thePanes.hasOwnProperty(key) === false) {
      continue;
    }
  ...
  if (isCollapsed) {
    for (let iline = 0, n = edit.lineCount(); iline < n; iline++) {
      ...
      const mark = edit.markText({ line: iline, ch: 0 }, { line: iline + 1, ch: 0 }, { atomic: true, readOnly: true });
...
const onPresentationChanged = (() => {
  ...
  const collapse = () => {
    ...
    const diffs = getDiffer().diff_main(thePanes.orig.modified.join("\n"), thePanes.edit.modified.join("\n"));
    const ll = [];
    let il = 0,
      lellipsis = false;
    const rr = [];
...
const onTextChanged = (() => {
  ...
  const process = (details) => {
    ...
    if (details === undefined && isClean === false && mergeView.editor().getValue().trim() === cleanEditText) {
      cleanEditToken = mergeView.editor().changeGeneration();
      isClean = true;
    }
...
const revertAllHandler = function () {
  ...
  for (const chunk of mergeView.leftChunks()) {
    const addedLines = left.getRange({ line: chunk.origFrom, ch: 0 }, { line: chunk.origTo, ch: 0 });
    const removedLines = edit.getRange({ line: chunk.editFrom, ch: 0 }, { line: chunk.editTo, ch: 0 });
...
const commitAllHandler = function () {
  ...
  const edit = mergeView.editor();
  for (const chunk of mergeView.leftChunks()) {
    const addedLines = edit.getRange({ line: chunk.editFrom, ch: 0 }, { line: chunk.editTo, ch: 0 });
    const removedLines = left.getRange({ line: chunk.origFrom, ch: 0 }, { line: chunk.origTo, ch: 0 });
    toAdd.push(addedLines.trim());
    toRemove.push(removedLines.trim());
  }
...
self.cloud.onPull = function (data, append) {
  ...
  applyDiff(false, data, append ? "" : mergeView.editor().getValue().trim());
};
