...
FilterContainer.prototype.compileGenericHideSelector = function (parser, writer) {
  ...
  if (key !== undefined) {
    writer.push([0, hashFromStr(key.charCodeAt(0), key.slice(1)), compiled]);
    return;
  }
...
FilterContainer.prototype.compileGenericUnhideSelector = function (parser, writer) {
...
FilterContainer.prototype.compileSpecificSelector = function (parser, hostname, not, writer) {
...
FilterContainer.prototype.fromCompiledContent = function (reader, options) {
  ...
  while (reader.next()) {
    ...
    switch (args[0]) {
      ...
      case 8:
        if (args[2] === 0b100) {
          if (this.reSimpleHighGeneric.test(args[3])) this.highlyGeneric.simple.dict.add(args[3]);
          else {
...
FilterContainer.prototype.fromSelfie = function (selfie) {
  if (selfie.version !== this.selfieVersion) {
    throw new Error(`cosmeticFilteringEngine: mismatched selfie version, ${selfie.version}, expected ${this.selfieVersion}`);
  }
...
FilterContainer.prototype.removeFromSelectorCache = function (targetHostname = "*", type = undefined) {
  ...
  for (let entry of this.selectorCache) {
    ...
    if (targetHostname !== "*") {
      ...
      if (hostname.length !== targetHostnameLength && hostname.charAt(hostname.length - targetHostnameLength - 1) !== ".") {
        continue;
      }
    }
    item.remove(type);
...
FilterContainer.prototype.cssRuleFromProcedural = function (pfilter) {
  if (pfilter.cssable !== true) {
    return;
  }
  const { tasks, action } = pfilter;
  let mq, selector;
  if (Array.isArray(tasks)) {
    if (tasks[0][0] !== "matches-media") {
      return;
    }
    mq = tasks[0][1];
    if (tasks.length > 2) {
  ...
  if (mq === undefined && style === undefined && selector === undefined) {
    return;
  }
  if (mq === undefined) {
    return `${selector}\n{${style}}`;
...
FilterContainer.prototype.retrieveGenericSelectors = function (request) {
  if (this.lowlyGeneric.size === 0) {
    return;
  }
  if (Array.isArray(request.hashes) === false) {
    return;
  }
...
FilterContainer.prototype.retrieveSpecificSelectors = function (request, options) {
  ...
  if (options.noSpecificCosmeticFiltering !== true || options.noGenericCosmeticFiltering !== true) {
    ...
    this.specificFilters.retrieve(hostname, options.noSpecificCosmeticFiltering ? discardSets : retrieveSets, 1);
    // Retrieve filters with a regex-based hostname value
    this.specificFilters.retrieve(hostname, options.noSpecificCosmeticFiltering ? discardSets : retrieveSets, 3);
    ...
    this.specificFilters.retrieve(hostname, options.noGenericCosmeticFiltering ? discardSets : retrieveSets, 2);
