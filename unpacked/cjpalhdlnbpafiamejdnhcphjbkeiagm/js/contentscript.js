...
*/
  ...
  vAPI.DOMFilterer = class {
    ...
    getAllSelectors(bits = 0) {
      ...
      const masterToken = hasProcedural ? `[${this.proceduralFilterer.masterToken}]` : undefined;
      for (const css of this.stylesheets) {
        for (const block of this.explodeCSS(css)) {
          if (includePrivateSelectors === false && masterToken !== undefined && block.startsWith(masterToken)) {
            continue;
          }
      ...
      if (excludeProcedurals === false) {
        out.procedural = [];
        if (hasProcedural) {
          out.procedural.push(...this.proceduralFilterer.selectors.values());
        }
...

  {
    ...
    const cachedBlockedSetClear = function () {
      cachedBlockedSet = cachedBlockedSetHash = cachedBlockedSetTimer = undefined;
    };
    ...
    const getCollapseToken = () => {
      if (collapseToken === undefined) {
        ...
        vAPI.userStylesheet.add(`[${collapseToken}]\n{display:none!important;}`, true);
      }
...

  {
    ...
    const domWatcherInterface = {
      onDOMCreated: function () {
        ...
        addPendingList(document.querySelectorAll("[id]:not(html):not(body),[class]:not(html):not(body)"));
        if (hasPendingNodes()) {
          surveyTimer.start();
        }
      },
      onDOMChanged: function (addedNodes) {
        ...
        for (const node of addedNodes) {
          ...
          addPendingList(node.querySelectorAll("[id]:not(html):not(body),[class]:not(html):not(body)"));
        }
        if (hasPendingNodes()) {
          surveyTimer.start(1);
        }
    ...
    const start = (details) => {
      ...
      if (self.vAPI.domFilterer instanceof Object === false) {
        return;
      }
      if (self.vAPI.domWatcher instanceof Object === false) {
        return;
      }
...

  {
    ...
    const onResponseReady = (response) => {
      ...
      const { noSpecificCosmeticFiltering, noGenericCosmeticFiltering, scriptletDetails } = response;
