...
(() => {
  ...
  vAPI.MessagingConnection.addListener(function (msg) {
    if (msg.from !== "domInspector" || msg.to !== "loggerUI") {
      return;
    }
    switch (msg.what) {
  ...
  const patchIncremental = function (from, delta) {
    ...
    var patchCosmeticHide = delta >= 0 && dom.cl.has(from, "isCosmeticHide") && dom.cl.has(li, "hasCosmeticHide") === false;
  ...
  const onClicked = function (ev) {
    ...
    if (dom.cl.has(target, "filter")) {
      ...
      dom.cl.toggle(qsa$(inspector, `[data-filter-id="${dom.attr(target, "data-filter-id")}"]`), "off", dom.cl.has(target, "off"));
    }
    // Toggle node
    else {
      vAPI.MessagingConnection.sendTo(inspectorConnectionId, {
  ...
  const revert = function () {
    ...
    vAPI.MessagingConnection.sendTo(inspectorConnectionId, { what: "resetToggledNodes" });
