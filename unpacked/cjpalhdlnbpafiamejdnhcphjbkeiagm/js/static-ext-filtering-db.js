...
const StaticExtFilteringHostnameDB = class {
  ...
  store(hn, bits, s) {
    ...
    let iHn = hnIsNotRegex ? this.hostnameToSlotIdMap.get(hn) : this.regexToSlotIdMap.get(hn);
    if (iHn === undefined) {
      if (hnIsNotRegex) {
  ...
  retrieve(hostname, out, modifiers = 0) {
    ...
    if (modifiers !== 0 && modifiers !== 3) {
      return;
    }
    if (this.regexToSlotIdMap.size === 0) {
      return;
    }
...
export { StaticExtFilteringHostnameDB };
