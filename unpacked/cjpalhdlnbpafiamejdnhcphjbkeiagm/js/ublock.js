...
const isHandcraftedWhitelistDirective = function (directive) {
  return (directive.startsWith("/") && directive.endsWith("/")) || (directive.indexOf("/") !== -1 && directive.indexOf("*") !== -1);
};
...
const matchDirective = function (url, hostname, directive) {
  ...
  if (directive.indexOf("/") === -1) {
    return (
      hostname.endsWith(directive) &&
      (hostname.length === directive.length || hostname.charAt(hostname.length - directive.length - 1) === ".")
    );
  }
  // Match URL exactly.
  ...
  if (re === undefined) {
    ...
    if (directive.startsWith("/") && directive.endsWith("/")) {
      ...
      reStr = directive.replace(whitelistDirectiveEscape, "\\$&").replace(whitelistDirectiveEscapeAsterisk, ".*");
    }
    re = new RegExp(reStr);
...
µb.changeUserSettings = function (name, value) {
  ...
  switch (name) {
    ...
    case "noCSPReports":
      ...
      switch (name) {
        case "noCosmeticFiltering":
          switchName = "no-cosmetic-filtering";
          break;
        case "noLargeMedia":
          switchName = "no-large-media";
          break;
        case "noRemoteFonts":
          switchName = "no-remote-fonts";
          break;
        case "noScripting":
          switchName = "no-scripting";
          break;
        case "noCSPReports":
          switchName = "no-csp-reports";
          break;
        default:
          break;
      }
...
µb.changeHiddenSettings = function (hs) {
  const mustReloadResources = hs.userResourcesLocation !== this.hiddenSettings.userResourcesLocation;
...
µb.elementPickerExec = async function (tabId, frameId, targetElement, zap = false) {
...
µb.toggleFirewallRule = function (details) {
  ...
  if (action !== 0) {
    sessionFirewall.setCell(srcHostname, desHostname, requestType, action);
  } else {
    sessionFirewall.unsetCell(srcHostname, desHostname, requestType);
  }

  // https://github.com/chrisaljoudi/uBlock/issues/731#issuecomment-73937469
  if (details.persist) {
    if (action !== 0) {
      permanentFirewall.setCell(srcHostname, desHostname, requestType, action);
    } else {
      permanentFirewall.unsetCell(srcHostname, desHostname, requestType);
    }
  ...
  // target page.
  if (srcHostname !== "*" && (requestType === "*" || requestType === "image" || requestType === "3p" || requestType === "3p-frame")) {
    srcHostname = "*";
  }
...
µb.toggleURLFilteringRule = function (details) {
  let changed = sessionURLFiltering.setRule(details.context, details.url, details.type, details.action);
  if (changed === false) {
    return;
  }
  ...
  if (details.persist !== true) {
    return;
  }

  changed = permanentURLFiltering.setRule(details.context, details.url, details.type, details.action);
...
µb.toggleHostnameSwitch = function (details) {
  const newState = typeof details.state === "boolean" ? details.state : sessionSwitches.evaluateZ(details.name, details.hostname) === false;
  let changed = sessionSwitches.toggleZ(details.name, details.hostname, !!details.deep, newState);
  if (changed === false) {
    return;
  }
  ...
  changed = permanentSwitches.toggleZ(details.name, details.hostname, !!details.deep, newState);
  if (changed) {
    this.saveHostnameSwitches();
  }
};
