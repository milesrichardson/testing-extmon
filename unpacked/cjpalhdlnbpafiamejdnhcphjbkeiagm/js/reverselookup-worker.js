...
const fromNetFilter = function (details) {
  ...
  for (const assetKey in listEntries) {
    ...
    for (;;) {
      ...
      if (notFound || (pos !== content.length && content.charCodeAt(pos) !== 0x0a)) {
        continue;
      }
      lists.push({
        assetKey: assetKey,
...
const fromExtendedFilter = function (details) {
  ...
  const regexFromLabels = (prefix, hn, suffix) => new RegExp(prefix + hn.split(".").reduce((acc, item) => `(${acc}\\.)?${item}`) + suffix);
  ...
  let reEntity;
  {
    ...
    if (pos !== -1) {
      reEntity = regexFromLabels("^(", hostname.slice(0, pos + hostname.length - domain.length), "\\.)?\\*$");
    }
  }
  ...
  for (const assetKey in listEntries) {
    ...
    while ((pos = content.indexOf(needle, pos)) !== -1) {
      ...
      if (filterType === 0 && details.ignoreGeneric) {
        continue;
      }

      // Do not confuse cosmetic filters with HTML ones.
      if ((filterType === 64) !== isHtmlFilter) {
        continue;
      }
      ...
      switch (filterType) {
        ...
        case 64: {
          if (exception !== ((fargs[2] & 0b001) !== 0)) {
            break;
          }
          const isProcedural = (fargs[2] & 0b010) !== 0;
          if ((isProcedural === false && fargs[3] !== selector) || (isProcedural && JSON.parse(fargs[3]).raw !== selector)) {
            break;
          }
          if (hostnameMatches(fargs[1]) === false) {
            break;
          }
          // https://www.reddit.com/r/uBlockOrigin/comments/d6vxzj/
          //   Ignore match if specific cosmetic filters are disabled
          if (filterType === 8 && exception === false && details.ignoreSpecific) {
            break;
          }
