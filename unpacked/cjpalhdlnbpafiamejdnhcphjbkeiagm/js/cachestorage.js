...
const cacheStorage = {
  ...
  select: function (selectedBackend) {
    ...
    if (actualBackend === undefined || actualBackend === "unset") {
      actualBackend = vAPI.webextFlavor.soup.has("firefox") ? "indexedDB" : "browser.storage.local";
    }
...
const selectIDB = async function () {
  ...
  const keepAlive = function () {
    dbTimer.offon(Math.max(µb.hiddenSettings.autoUpdateAssetFetchPeriod * 2 * 1000, 180000));
  };
  ...
  const getDb = function () {
    ...
    dbPromise = new Promise((resolve) => {
      ...
      req.onupgradeneeded = function (ev) {
        ...
        if (vAPI.webextFlavor.soup.has("firefox") && browser.extension.inIncognitoContext === true) {
          return req.onerror();
        }
        if (ev.oldVersion === 1) {
          return;
  ...
  const toBlob = function (data) {
    const value = data instanceof Uint8Array ? new Blob([data]) : data;
  ...
  const visitAllFromDb = async function (visitFn) {
    ...
    const transaction = db.transaction(STORAGE_NAME, "readonly");
    transaction.oncomplete = transaction.onerror = transaction.onabort = () => visitFn();
    const table = transaction.objectStore(STORAGE_NAME);
  ...
  const getAllFromDb = function (callback) {
    ...
    visitAllFromDb((entry) => {
      ...
      keyvalStore[key] = value;
      if (entry.value instanceof Blob === false) {
        return;
      }
      promises.push(decompress(keyvalStore, key, value));
    }).catch((reason) => {
...
const clearWebext = async function () {
  ...
  if (bin instanceof Object === false || bin.assetCacheRegistry instanceof Object === false) {
    return;
  }
  const toRemove = ["assetCacheRegistry", "assetSourceRegistry", "resourcesSelfie", "selfie"];
