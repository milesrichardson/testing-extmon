...
function setAccentColor(accentEnabled, accentColor, propagate, stylesheet = "") {
  if (accentEnabled && stylesheet === "" && self.hsluv !== undefined) {
    ...
    hsl[2] = 80;
    text.push(":root.light.accented {", `    --button-surface-rgb: ${toRGB(hsl)};`, "}");
    hsl[2] = 30;
    text.push(":root.dark.accented {", `    --button-surface-rgb: ${toRGB(hsl)};`, "}");
    text.push("");
...

{
  ...
  vAPI.messaging.send("dom", { what: "uiStyles" }).then((response) => {
    ...
    if (response.uiAccentCustom) {
      setAccentColor(true, response.uiAccentCustom0, false, response.uiAccentStylesheet);
    }
...
export { getActualTheme, setTheme, setAccentColor };
