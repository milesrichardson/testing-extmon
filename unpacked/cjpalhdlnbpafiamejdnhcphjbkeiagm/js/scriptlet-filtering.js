...
import { domainFromHostname, entityFromDomain, hostnameFromURI } from "./uri-utils.js";
...
const contentScriptRegisterer = new (class {
  constructor() {
    ...
    if (browser.contentScripts === undefined) {
      return;
    }
    µb.onEvent("filteringBehaviorChanged", (ev) => {
      ...
      if (details instanceof Object) {
        ...
        if (details.hostname) {
          return this.flush(details.hostname);
        }
      }
      this.reset();
    });
  ...
  register(hostname, code) {
    ...
    const promise = browser.contentScripts
      .register({
        js: [{ code }],
        allFrames: true,
        matches: [`*://*.${hostname}/*`],
  ...
  unregister(hostname) {
    if (this.hostnameToDetails.size === 0) {
      return;
    }
    const details = this.hostnameToDetails.get(hostname);
    if (details === undefined) {
  ...
  flush(hostname) {
    ...
    for (const hn of this.hostnameToDetails.keys()) {
      ...
      if (pos !== 0 && hn.charCodeAt(pos - 1) !== 0x2e /* . */) {
        continue;
      }
      this.unregister(hn);
    }
...
const isolatedWorldInjector = (() => {
  ...
  return {
    ...
    assemble: function (hostname, scriptlets) {
      ...
      return code.slice(0, match.index) + scriptlets + code.slice(match.index + match[0].length);
    }
  };
})();
...
const lookupScriptlet = function (rawToken, mainMap, isolatedMap) {
  ...
  while (dependencies.length !== 0) {
    ...
    if (targetWorldMap.has(token)) {
      continue;
    }
    const details = reng.contentFromName(token, "fn/javascript") || reng.contentFromName(token, "text/javascript");
    if (details === undefined) {
      continue;
    }
    targetWorldMap.set(token, details.js);
    if (Array.isArray(details.dependencies) === false) {
      continue;
    }
...
scriptletFilteringEngine.compile = function (parser, writer) {
  ...
  if (normalized === "[]" && isException === false) {
    return;
  }
...
scriptletFilteringEngine.retrieve = function (request) {
  ...
  if (µb.userSettings.advancedUserEnabled && sessionFirewall.evaluateCellZY(hostname, hostname, "*") === 2) {
    return;
  }
  ...
  if (cacheDetails === undefined) {
    ...
    const entity = request.entity !== "" ? `${hostname.slice(0, -request.domain.length)}${request.entity}` : "*";
    ...
    if ($exceptions.has("[]")) {
      return {
        filters: [{ tabId: request.tabId, url: request.url, filter: "#@#+js()" }]
      };
    }
