...
class BidiTrieContainer {
  constructor(extraHandler) {
    ...
    this.haystack = this.buf8.subarray(HAYSTACK_START, HAYSTACK_START + HAYSTACK_SIZE);
  ...
  add(iroot, aL0, n, pivot = 0) {
    ...
    for (;;) {
      ...
      inext = this.addCell(buf32[icell + CELL_AND], 0, ((bR - bi) << 24) | (bl + bi));
  ...
  addLeft(icell, aL0, pivot) {
    ...
    for (;;) {
      ...
      if (binfo <= BCELL_EXTRA_MAX) {
        ...
        iboundary = this.allocateCell();
        buf32[icell + CELL_AND] = this.addCell(iboundary, 0, toSegmentInfo(aL0, 0, ar));
      ...
      if (buf8[br - 1] !== buf8[aL + ar - 1]) {
        ...
        if (inext === 0) {
          iboundary = this.allocateCell();
          inext = this.addCell(iboundary, 0, toSegmentInfo(aL0, 0, ar));
          buf32[icell + CELL_OR] = inext;
          return iboundary;
        }
        icell = inext;
      ...
      if (br === bL) {
        ...
        // b:        vvvvvvv
          ...
          buf32[icell + CELL_AND] = inext;
          buf32[inext + CELL_AND] = this.addCell(0, 0, toSegmentInfo(aL0, 0, ar));
        }
      }
      ...
      // b: yyyyyyyvvvvvvv
        ...
        inext = this.addCell(buf32[icell + CELL_AND], 0, ((br - bL) << 24) | (bL - char0));
        // needle remainder: no = need boundary cell
        // a:        vvvvvvv
        // b: yyyyyyyvvvvvvv
        // r: yyyyyyy & 0 & vvvvvvv
        ...
        // r: (0 & wwww | yyyyyyy) & vvvvvvv
          ...
          buf32[icell + CELL_AND] = inext;
          iboundary = this.allocateCell();
          buf32[inext + CELL_OR] = this.addCell(iboundary, 0, toSegmentInfo(aL0, 0, ar));
          return iboundary;
        }
      }
      //debugger; // jshint ignore:line
  ...
  serialize(encoder) {
    if (encoder instanceof Object) {
      return encoder.encode(this.buf32.buffer, this.buf32[CHAR1_SLOT]);
    }
    return Array.from(new Uint32Array(this.buf32.buffer, 0, (this.buf32[CHAR1_SLOT] + 3) >>> 2));
  }

  unserialize(selfie, decoder) {
    const shouldDecode = typeof selfie === "string";
    let byteLength = shouldDecode ? decoder.decodeSize(selfie) : selfie.length << 2;
    if (byteLength === 0) {
      return false;
    }
  ...
  extractString(i, n) {
    ...
    return this.textDecoder.decode(this.buf8.subarray(offset, offset + n));
  }
    ...
    return {
      ...
      next() {
        ...
        for (;;) {
          ...
          if (len !== 0) {
            const s = this.textDecoder.decode(new Uint8Array(this.charBuf.buffer, 0, len));
            if (this.dir > 0) {
              this.pattern += s;
            } else if (this.dir < 0) {
  ...
  growBuf(trieGrow, charGrow) {
    ...
    const bufLen = Math.max(roundToPageSize(char1 + charGrow), this.buf8.length);
  ...
  reallocateBuf(newSize) {
    ...
    if (this.wasmMemory === null) {
      ...
      newBuf.set(newBuf.length < this.buf8.length ? this.buf8.subarray(0, newBuf.length) : this.buf8);
      this.buf8 = newBuf;
    } else {
      const growBy = ((newSize + 0xffff) >>> 16) - (this.buf8.length >>> 16);
      if (growBy <= 0) {
        return;
      }
