...
const extractMetadataFromList = (content, fields) => {
  ...
  for (let field of fields) {
    ...
    field = field.toLowerCase().replace(/-[a-z]/g, (s) => s.charAt(1).toUpperCase());
...
const resourceTimeFromXhr = (xhr) => {
  try {
    ...
    if (typeof xhr.response === "string") {
      const metadata = extractMetadataFromList(xhr.response, ["Last-Modified"]);
...
const resourceTimeFromParts = (parts, time) => {
  ...
  return goodParts.reduce((acc, part) => ((part.resourceTime || 0) > acc ? part.resourceTime : acc), time);
};

const resourceIsStale = (networkDetails, cacheDetails) => {
  if (typeof networkDetails.resourceTime !== "number") {
    return false;
  }
  if (networkDetails.resourceTime === 0) {
    return false;
  }
  if (typeof cacheDetails.resourceTime !== "number") {
    return false;
  }
  if (cacheDetails.resourceTime === 0) {
    return false;
  }
...
const isDiffUpdatableAsset = (content) => {
  ...
  const data = extractMetadataFromList(content, ["Diff-Path"]);
  return typeof data.diffPath === "string" && data.diffPath.startsWith("%") === false;
};
...
const getContentURLs = (assetKey, options = {}) => {
  ...
  if (options.favorLocal) {
    contentURLs.sort((a, b) => {
      if (reIsExternalPath.test(a)) {
        return 1;
      }
      if (reIsExternalPath.test(b)) {
        return -1;
      }
      return 0;
    });
...
assets.fetchText = async function (url) {
  ...
  if (isExternal && remoteServerFriendly !== true) {
    const cacheBypassToken = µb.hiddenSettings.updateAssetBypassBrowserCache
...
assets.fetchFilterList = async function (mainlistURL) {
  ...
  let rootDirectoryURL = toParsedURL(reIsExternalPath.test(mainlistURL) ? mainlistURL : vAPI.getURL(mainlistURL));
  if (rootDirectoryURL !== undefined) {
    const pos = rootDirectoryURL.pathname.lastIndexOf("/");
    if (pos !== -1) {
      rootDirectoryURL.pathname = rootDirectoryURL.pathname.slice(0, pos + 1);
    } else {
  ...
  const processIncludeDirectives = function (results) {
    ...
    for (const result of results) {
      ...
      const slices = sfp.utils.preparser.splitter(content, vAPI.webextFlavor.env);
  ...
  let allParts = [this.fetchText(mainlistURL)];
  // Abort processing `include` directives if at least one included sublist
  // can't be fetched.
  let resourceTime = 0;
  do {
    allParts = await Promise.all(allParts);
    const part = allParts.find((part) => typeof part === "object" && part.error !== undefined);
    if (part !== undefined) {
  ...
  return {
    url: mainlistURL,
    resourceTime,
    content: allParts.length === 1 ? allParts[0] : allParts.join("") + "\n"
  };
};
...
function getAssetSourceRegistry() {
  if (assetSourceRegistryPromise === undefined) {
    assetSourceRegistryPromise = cacheStorage.get("assetSourceRegistry").then((bin) => {
      if (bin instanceof Object && bin.assetSourceRegistry instanceof Object) {
        assetSourceRegistry = bin.assetSourceRegistry;
        return assetSourceRegistry;
      }
      return assets
        .fetchText(µb.assetsBootstrapLocation || µb.assetsJsonPath)
        .then((details) => {
          return details.content !== "" ? details : assets.fetchText(µb.assetsJsonPath);
        })
        .then((details) => {
...
function updateAssetSourceRegistry(json, silent = false) {
  ...
  try {
    ...
    newDict["assets.json"].defaultListset = Array.from(Object.entries(newDict))
  ...
  for (const assetKey in oldDict) {
    if (newDict[assetKey] === undefined && oldDict[assetKey].submitter === undefined) {
  ...
  for (const assetKey in newDict) {
    if (oldDict[assetKey] === undefined && !silent) {
      fireNotification("builtin-asset-source-added", { assetKey: assetKey, entry: newDict[assetKey] });
    }
...
function getAssetCacheRegistry() {
  if (assetCacheRegistryPromise === undefined) {
    assetCacheRegistryPromise = cacheStorage.get("assetCacheRegistry").then((bin) => {
      if (bin instanceof Object && bin.assetCacheRegistry instanceof Object) {
        if (Object.keys(assetCacheRegistry).length === 0) {
          assetCacheRegistry = bin.assetCacheRegistry;
        } else {
          console.error("getAssetCacheRegistry(): assetCacheRegistry reassigned!");
          if (Object.keys(bin.assetCacheRegistry).sort().join() !== Object.keys(assetCacheRegistry).sort().join()) {
            console.error("getAssetCacheRegistry(): assetCacheRegistry changes overwritten!");
          }
        }
      }
...
async function assetCacheRead(assetKey, updateReadTime = false) {
  ...
  const [, bin] = await Promise.all([getAssetCacheRegistry(), cacheStorage.get(internalKey)]);

  if (µb.readyToFilter !== true) {
    µb.supportStats.maxAssetCacheWait = Math.max(Date.now() - t0, parseInt(µb.supportStats.maxAssetCacheWait, 10) || 0) + " ms";
  }

  if (bin instanceof Object === false || bin.hasOwnProperty(internalKey) === false) {
    return reportBack("");
  }
...
async function assetCacheRemove(pattern) {
  ...
  if (removedContent.length !== 0) {
    await Promise.all([cacheStorage.remove(removedContent), cacheStorage.set({ assetCacheRegistry })]);
  }
...
const readUserAsset = async function (assetKey) {
  ...
  const content = bin instanceof Object && typeof bin[assetKey] === "string" ? bin[assetKey] : "";
...
assets.get = async function (assetKey, options = {}) {
  ...
  const reportBack = (content, url = "", err = undefined) => {
    ...
    if (options.needSourceURL) {
      if (url === "" && assetCacheRegistry instanceof Object && assetCacheRegistry[assetKey] instanceof Object) {
  ...
  for (const contentURL of contentURLs) {
    const details = assetDetails.content === "filters" ? await assets.fetchFilterList(contentURL) : await assets.fetchText(contentURL);
...
async function getRemote(assetKey, options = {}) {
  const [assetDetails = {}, cacheDetails = {}] = await Promise.all([assetSourceGetDetails(assetKey), assetCacheGetDetails(assetKey)]);
  ...
  for (const contentURL of getContentURLs(assetKey, options)) {
    if (reIsExternalPath.test(contentURL) === false) {
      continue;
    }

    const result = assetDetails.content === "filters" ? await assets.fetchFilterList(contentURL) : await assets.fetchText(contentURL);
    ...
    if (assetDetails.content === "filters") {
      const metadata = extractMetadataFromList(result.content, ["Last-Modified", "Expires", "Diff-Name", "Diff-Path", "Diff-Expires"]);
...
assets.put = async function (assetKey, content) {
  return reIsUserAsset.test(assetKey) ? await saveUserAsset(assetKey, content) : await assetCacheWrite(assetKey, content);
};

/******************************************************************************/

assets.metadata = async function () {
  await Promise.all([getAssetSourceRegistry(), getAssetCacheRegistry()]);
  ...
  for (const assetKey in assetDict) {
    ...
    const cacheEntry = cacheDict[assetKey];
    if (assetEntry.content === "filters" && assetEntry.external !== true) {
      assetEntry.isDefault = assetEntry.off === undefined || (assetEntry.off === true && µb.listMatchesEnvironment(assetEntry));
    }
    if (cacheEntry) {
    ...
    } else if (assetEntry.contentURL && assetEntry.contentURL.length !== 0) {
...
assets.getUpdateAges = async function (conditions = {}) {
  ...
  for (const [assetKey, asset] of Object.entries(assetDict)) {
    ...
    if (tokens.includes("*") === false) {
      if (tokens.includes(assetKey) === false) {
        continue;
      }
    }
...
const getAssetDiffDetails = (assetKey) => {
  ...
  if (Array.isArray(assetEntry.cdnURLs) === false) {
    return;
  }
...
async function diffUpdater() {
  if (updaterAuto === false) {
    return;
  }
  if (µb.hiddenSettings.differentialUpdate === false) {
    return;
  }
  ...
  return new Promise((resolve) => {
    ...
    const checkAndCorrectDiffPath = (data) => {
      ...
      if (metadata instanceof Object === false) {
        return;
      }
      if (metadata.diffPath === data.patchPath) {
        return;
      }
    ...
    bc.onmessage = (ev) => {
      ...
      if (data.status === "updated") {
        ...
        const metadata = extractMetadataFromList(data.text, ["Last-Modified", "Expires", "Diff-Name", "Diff-Path", "Diff-Expires"]);
...
async function getUpdateCandidates() {
  const [assetDict, cacheDict] = await Promise.all([getAssetSourceRegistry(), getAssetCacheRegistry()]);
  const toUpdate = [];
  for (const assetKey in assetDict) {
    const assetEntry = assetDict[assetKey];
...
assets.updateStart = function (details) {
  ...
  const newUpdateDelay = typeof details.delay === "number" ? details.delay : updaterAssetDelayDefault;
...
assets.isUpdating = function () {
  return updaterStatus === "updating" && updaterAssetDelay <= µb.hiddenSettings.manualUpdateAssetFetchPeriod;
};
