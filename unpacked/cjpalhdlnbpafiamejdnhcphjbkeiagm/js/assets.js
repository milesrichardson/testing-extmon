...
assets.fetchText = async function (url) {
  ...
  try {
    ...
    if (details.content.length !== 0 && details.content.endsWith("\n") === false) {
      details.content += "\n";
    }
  } catch (ex) {
    details = ex;
...
async function getRemote(assetKey) {
  ...
  for (let contentURL of contentURLs) {
    ...
    if (assetKey === "assets.json") {
      contentURL = contentURL.replace(/\/assets\/assets\.json$/, µb.assetsJsonPath);
    }
...
assets.metadata = async function () {
  ...
  for (const assetKey in assetDict) {
    ...
    if (cacheEntry) {
      ...
      const obsoleteAfter = cacheEntry.writeTime + assetEntry.updateAfter * 86400000;
...
const updateNext = async function () {
  ...
  for (const assetKey in assetDict) {
    ...
    const cacheEntry = cacheDict[assetKey];
    if (cacheEntry instanceof Object && cacheEntry.writeTime + assetEntry.updateAfter * 86400000 > now) {
      continue;
    }
    if (
  ...
  let result;
  if (toUpdate[0] !== "assets.json" || µb.hiddenSettings.debugAssetsJson !== true) {
