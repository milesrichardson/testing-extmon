...
const sparseBase64 = {
  ...
  encode: function (arrbuf, arrlen) {
    ...
    if (typeof TextDecoder === "undefined") {
      return JSON.stringify(Array.from(new Uint32Array(outbuf.buffer, 0, j >>> 2)));
    }
  ...
  decode: function (instr, arrbuf) {
    ...
    const outbuf = arrbuf instanceof ArrayBuffer === false ? new Uint32Array(outputLength) : new Uint32Array(arrbuf);
    ...
    let j = 0;
    for (;;) {
      if (j === outputLength || i >= inputLength) {
        break;
      }
      let v = 0,
        l = 0;
...
const denseBase64 = {
  ...
  decode: function (instr, arrbuf) {
    ...
    const outbuf = arrbuf instanceof ArrayBuffer === false ? new Uint8Array(outputLength) : new Uint8Array(arrbuf);
