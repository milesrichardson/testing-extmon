...
function beforeTRRequest(request) {
  ...
  for (const key of urlParams.keys()) {
    ...
    if (matchedCustomData) {
      customData[matchedCustomData[1]] = urlParams.get(matchedCustomData[0]);
    }
    if (matchedPiiData) {
      piiData[matchedPiiData[1]] = urlParams.get(matchedPiiData[0]);
    }
    if (matchedDpoData) {
      dpoData[key] = urlParams.get(key);
    }
  }
  // pixel event
  ...
  trDetails[tabId][pixelId][pixelEvent]["eventId"] = urlParams.get("eid");
  trDetails[tabId][pixelId][pixelEvent]["pixelLocation"] = urlParams.get("dl");
  trDetails[tabId][pixelId][pixelEvent]["requestType"] = urlParams.get("rqm");
...
chrome.webRequest.onBeforeRequest.addListener(beforeTRRequest, { urls: ["*://www.facebook.com/tr*"] }, ["extraHeaders", "requestBody"]);

// Feature parity to get loading time
chrome.webRequest.onCompleted.addListener(completeTRRequest, { urls: ["*://www.facebook.com/tr*"] }, ["extraHeaders"]);
