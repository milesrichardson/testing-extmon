  ...
  */
  ...
  const captureAJAXRequests = function (toContentScriptEventName, fromContentScriptEventName) {
    ...
    function processVideoData(data) {
      if (data && data.videoDetails && data.videoDetails.author) {
    ...
    window.addEventListener("message", (event) => {
      if (
        ...
        (event.data.eventName === fromContentScriptEventName || event.data.eventName === toContentScriptEventName)
      ) {
        updateURLWrapped(event.data.channelName);
      }
      if (event && event.data && event.data.removeChannelName && event.data.eventName === fromContentScriptEventName) {
  ...
  const runOnYT = function () {
    ...
    window.addEventListener("message", (event) => {
      if (event && event.data && event.data.channelName && event.data.eventName === toContentScriptRandomEventName) {
