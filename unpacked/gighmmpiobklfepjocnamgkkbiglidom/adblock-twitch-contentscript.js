...
if (window.top === window.self && /(^|\.)twitch\.tv$/.test(window.location.hostname) === true) {
  ...
  */
  ...
  const captureFetchRequests = function (settings, toContentScriptEventName, fromContentScriptEventName) {
    ...
    if (settings && settings.twitch_channel_allowlist) {
      ...
      myHistory.pushState = function thePushState(...args) {
        if (args && args.length > 2 && typeof args[2] === "string" && !args[2].startsWith("/directory/")) {
          document.dispatchEvent(new CustomEvent(toContentScriptEventName, { detail: { path: args[2] } }));
        }
    ...
    document.addEventListener(fromContentScriptEventName, (event) => {
      if (event && event.detail && event.detail.path === window.location.pathname) {
        ...
        if (!document.location.href.includes("ab_channel=") && window.location.href.match(/\//g).length === 1) {
          let channelName = event.detail.path;
          if (channelName.startsWith("/")) {
            channelName = channelName.substr(1);
          }
          updateURLWrapped(channelName);
          document.dispatchEvent(new CustomEvent(toContentScriptEventName, { detail: { channelName } }));
        }
      }
    });
    ...
    if (settings && settings.twitch_channel_allowlist) {
      preProcessCheck = function (input, params) {
        if (params.length >= 2 && typeof input === "string" && input.includes("https://gql.twitch.tv/gql")) {
          let body = {};
          try {
          ...
          if (
            ...
            document.dispatchEvent(new CustomEvent(toContentScriptEventName, { detail: { channelName: body[0].variables.channel } }));
          }
                ...
                for (let inx = 0; inx < respObj.length && nameFound === false; inx++) {
                  ...
                  if (
                    ...
                    document.dispatchEvent(
                      new CustomEvent(toContentScriptEventName, { detail: { channelName: entry.data.video.owner.displayName } })
                    );
                  }
                  ...
                  if (
                    ...
                    document.dispatchEvent(
                      new CustomEvent(toContentScriptEventName, { detail: { channelName: entry.data.clip.broadcaster.displayName } })
                    );
                  }
                  if (
                    entry &&
                    ...
                    document.dispatchEvent(
                      new CustomEvent(toContentScriptEventName, { detail: { channelName: entry.data.user.displayName } })
                    );
                  }
                  ...
                  if (
                    ...
                    document.dispatchEvent(
                      new CustomEvent(toContentScriptEventName, { detail: { channelName: entry.data.community.displayName } })
                    );
                  }
                }
              }
    ...
    document.addEventListener(toContentScriptRandomEventName, (event) => {
      ...
      if (event && event.detail && event.detail.path) {
        ...
        browser.runtime.sendMessage({ command: "pageIsWhitelisted", page: JSON.stringify(thePage) }).then((isWhitelisted) => {
          document.dispatchEvent(new CustomEvent(fromContentScriptRandomEventName, { detail: { path: event.detail.path, isWhitelisted } }));
        });
      }
    });
  };
  ...
  const injectWrappers = function (settings) {
    ...
    const scriptToInject = `(${captureFetchRequests.toString()})(${JSON.stringify(
      settings
    )}, '${toContentScriptRandomEventName}', '${fromContentScriptRandomEventName}');`;
