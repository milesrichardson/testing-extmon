...
async function cleanCustomFilter(filtersArg) {
  ...
  if (isPaused) {
    ...
    filters = filters.filter(
      (element) => !(element.text === pausedFilterText.pausedFilterText1 || element.text === pausedFilterText.pausedFilterText2)
    );
  }
  ...
  async function saveFilters() {
    ...
    for (let i = 0; i < uniqCustomFilters.length && !filterErrorMessage; i++) {
      ...
      if (!originalCustomFilters.includes(filterToAdd) && filterToAdd) {
        if (filterToAdd && isSelectorFilter(filterToAdd) && !hasValidQueryString(filterToAdd)) {
          filterErrorMessage = translate("customfilterserrormessage", [filterToAdd, translate("filter_invalid_css")]);
        }
    ...
    if (newFiltersToAdd.length) {
      ...
      if (errors) {
        ...
        if (filter) {
          filterErrorMessage = translate("customfilterserrormessage", [filter, translate(reason || type) || translate("filter_invalid")]);
        } else {
