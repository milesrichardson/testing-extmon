...
function getDefaultFilterUI(filterList, checkboxID, filterListType) {
  ...
  const $checkBox = $("<input>").attr("type", "checkbox").attr("adblockId", checkboxID).attr("id", checkboxID).prop("checked", isSelected);
  ...
  const $checkBoxWrapper = $("<span>").addClass("checkbox").addClass("md-stack").append($checkBox).append($checkBoxIcons);

  const $filterTitle = $("<h1>").text(filterList.label || filterList.title || filterList._title || `${filterList.url.substr(0, 40)}...`);
  ...
  const $label = $("<label>").attr("title", filterList.correctedURL).attr("for", checkboxID).append($filterInfo).append($infoSection);
...
CheckboxForFilterList.prototype = {
  ...
  bindActions() {
    ...
    this.checkBox.on("change", function filterListSelectionChanged(event) {
      ...
      if (checked) {
        if (!event.addedViaBackground && !SubscriptionUtil.validateOverSubscription(that.filterList)) {
...
CustomFilterListUploadUtil.performUpload = (url, subscribeTo, title) => {
  ...
  const checkbox = new CheckboxForFilterList(entry, "customFilterList", customFilterLists.array.length, customFilterLists.$container);
...
async function onFilterChangeHandler(action, items) {
  ...
  const updateEntry = async function (entryToUpdate, eventAction) {
    ...
    if (entry) {
      ...
      FilterListUtil.updateCheckbox(FilterListUtil.cachedSubscriptions[entry.adblockId], entry.adblockId);
  ...
  if (item && item.url) {
    ...
    if (action === "subscription.added" && !FilterListUtil.cachedSubscriptions["url:"] && !item.url.startsWith("~user~")) {
