...
(async () => {
  ...
  if ("panels" in browser.devtools) {
    let enabled = await browser.runtime.sendMessage({ type: "prefs.get", key: "show_devtools_panel" });

    if (enabled) {
      let panel = await browser.devtools.panels.create("AdBlock", "icons/ab-32.png", "devtools-panel.html");
