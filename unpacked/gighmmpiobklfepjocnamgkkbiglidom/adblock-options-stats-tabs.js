...
const showOrHideReplacedPanelCountNeeded = async function () {
  if (settings.local_data_collection && License.isActiveLicense() && channels && (await channels.isAnyEnabled())) {
    if (showReplacedData) {
...
function filterForAllData(chartDataArray) {
  ...
  for (let i = 0; i < chartDataArray.length; i++) {
    for (let j = 0; j < numBuckets - 1; j++) {
      if (chartDataArray[i].x >= monthDateBuckets[j] && chartDataArray[i].x < monthDateBuckets[j + 1]) {
        monthBuckets[j] += chartDataArray[i].y;
      }
    }
    if (chartDataArray[i].x >= monthDateBuckets[numBuckets - 1] && chartDataArray[i].x < nextMonth) {
...
const getLineChartConfig = async function (chartType, filterName, labelName) {
  ...
  if (showReplacedData && License.isActiveLicense() && channels && (await channels.isAnyEnabled())) {
  ...
  setFontSize(totalAdsBlocked, "#adsblocked_value", totalTrackersBlocked, "#trackers_blocked_value", totalReplaced, "#ads_replaced_value");
        ...
        for (const domain in rawData[timestamp].doms) {
          ...
          if (cleanDomain && cleanDomain.length > 1) {
            ...
            domainData[cleanDomain].total =
              domainData[cleanDomain].ads + domainData[cleanDomain].trackers + domainData[cleanDomain].adsReplaced;
          }
        }
      }
    }
...
$("#btnDelete").on("click", () => {
  if (!$("#local_data_collection_opt_out").is(":checked") && !$("#delete_all_stats_data").is(":checked")) {
    return;
  }
