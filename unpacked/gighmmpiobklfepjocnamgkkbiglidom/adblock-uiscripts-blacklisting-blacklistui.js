...
function BlacklistUi(clickedItem, advancedUser, isActiveLicense, showBlacklistCTA, $base, addCustomFilterRandomName) {
...
BlacklistUi.prototype.buildPage1 = function buildPage1(showBackButton) {
  ...
  $pageOneSlider.attr("max", Math.max(depth - 1, 1)).on("input change", function sliderInputChange() {
...
BlacklistUi.prototype.buildPage2 = function buildPage2() {
  ...
  $pageTwoHelpIcon.on("click", () => {
    browser.runtime.sendMessage({
      command: "openTab",
      urlToOpen: "https://help.getadblock.com/support/solutions/articles/6000246376-about-the-adblock-hiding-wizard-and-html-tags/"
    });
  });
...
BlacklistUi.prototype.makeFilter = function makeFilter() {
  ...
  if (!this.advancedUser || (this.advancedUser && $("input[type='checkbox']#cknodeName", $pageTwoDetails).is(":checked"))) {
  ...
  const attrs = ["id", "class", "name", "src", "href", "data"];
  for (const i in attrs) {
    if (el.attr(attrs[i]) && (!this.advancedUser || (this.advancedUser && $(`#ck${attrs[i]}`, $pageTwoDetails).is(":checked")))) {
