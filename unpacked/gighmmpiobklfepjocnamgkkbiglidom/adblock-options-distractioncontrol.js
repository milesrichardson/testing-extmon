...
function getDefaultDCFilterUI(entry, isActiveLicense) {
  const $filterTitle = $("<span>").addClass("dc_filter_list_title").text(entry.title);

  const $filterHeader = $("<div>").append($filterTitle);

  const $extraInformation = $("<div>").addClass("dc_extra_info").text(entry.description);
  ...
  const $lockOverlay = $("<span>").addClass("dc_locked_overlay").addClass("do-not-display").append($lockIcon);

  const $image = $("<img>").attr("src", entry.imageURL);

  const $imageWrapper = $("<div>").addClass("dc_image_wrapper").append($image).append($lockOverlay);

  const $leftWrapper = $("<span>").addClass("right_dc_subscription").append($label);
  ...
  const $filterWrapper = $("<div>").addClass("filter-subscription-wrapper").addClass(entry.topLineClass).append($checkboxHeaderLine);
