...
function displayTranslationCredit() {
  ...
  $.getJSON(browser.runtime.getURL("translators.json"), (response) => {
    ...
    for (const id in response) {
      ...
      const idEqualToLangSubstring = id.substring(0, 2) === langSubstring || id.substring(0, 2) === langSubstring.toLowerCase();
...
const requestSyncMessageRemoval = function (delayTime) {
  return new Promise((resolve) => {
    ...
    timeoutID = setTimeout(() => {
      ...
      $(".sync-header-message").removeClass("sync-message-good sync-message-error").addClass("sync-message-hidden");
...
const showNoLongerSyncError = function () {
  ...
  requestSyncMessageRemoval(0).then(() => {
    ...
    if ($("#sync").is(":visible")) {
      const maxHeight = Math.max($("#unsync-message-box-close-sync-tab").height(), $("#sync-reload-page-message").height());
...
const checkForUnSyncError = function () {
  if (!settings.sync_settings && (SyncService.getLastGetStatusCode() === 403 || SyncService.getLastPostStatusCode() === 403)) {
...
const showSyncMessage = function (msgText, doneIndicator, errorIndicator) {
  ...
  if (!doneIndicator && errorIndicator) {
    ...
    $(".sync-header-message").removeClass("sync-message-hidden sync-message-good").addClass("sync-message-error");
    requestSyncMessageRemoval(TWENTY_SECONDS);
  } else if (doneIndicator && !errorIndicator) {
    $(".sync-icon").text("check_circle");
    $(".sync-header-message").removeClass("sync-message-hidden sync-message-error").addClass("sync-message-good");
    ...
    chromeStorageGetHelper(mailCtaKey).then((alreadyClickedMailCTA) => {
      if (!alreadyClickedMailCTA) {
        ...
        $("#mail-cta-close-icon, #mail-cta-done-close-icon").on("click", (event) => {
          if (event && event.target && event.target.dataset && event.target.dataset.sendCloseEvent) {
