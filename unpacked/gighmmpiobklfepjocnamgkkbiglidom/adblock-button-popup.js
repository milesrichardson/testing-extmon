...
const start = async function () {
  ...
  if (info) {
    ...
    if (pageInfo.url && /ab_channel/.test(pageInfo.url.href) && pageInfo.url.hostname === "www.youtube.com" && info.youTubeChannelName) {
    ...
    if (pageInfo.url && /ab_channel/.test(pageInfo.url.href) && pageInfo.url.hostname === "www.twitch.tv" && info.twitchChannelName) {
    ...
    } else if (!info.settings.sync_settings && (info.lastGetStatusCode === 403 || info.lastPostStatusCode === 403)) {
    ...
    } else if ((info.lastPostStatusCode >= 400 || info.lastPostStatusCode === 0) && info.settings.sync_settings) {
    ...
    if (!info.settings.display_menu_stats || info.paused || info.domainPaused || info.disabledSite || info.whitelisted) {
      $("#block_counts").hide();
    }
  }
