...
const loadIconResources = function (base, callback) {
  ...
  fonts.push(
    new FontFace("Material Icons", `url(${browser.runtime.getURL("/icons/MaterialIcons-Regular.woff2")}`, {
      style: "normal",
      weight: "normal"
    })
  );
  fonts.push(
    new FontFace("AdBlock Icons", `url(${browser.runtime.getURL("/icons/adblock-icons.woff2")}`, { style: "normal", weight: "normal" })
  );
  Promise.all(fonts)
    .then((loaded) => {
...
const buildOverlay = function () {
  ...
  const overlaySuffix = `
    ...
    <div id="overlayIcon">
        <object id="iconlogo" type="image/svg+xml" style="height: 32px; width: 32px;" data='${browser.runtime.getURL(
...
const configureChildElements = function (parentNodeArg, adsAllowedArg) {
  ...
  const lineTwoClickHandler = function (event) {
    ...
    this.getRootNode().host.classList.remove("openedABPanel");
    browser.runtime
      .sendMessage({ command: "createAllowlistFilterForYoutubeChannelName", channelName: parentNode.dataset.parsedChannelName })
      .then(() => {
        browser.runtime.sendMessage({ command: "getAllAdsAllowedUserFilters" }).then((response) => {
          const ytRules = response;
          const { parsedChannelName } = parentNode.dataset;
...
const bodyClickHandler = function (event) {
  ...
  if (openedABPanel && openedABPanel !== event.target && !openedABPanel.contains(event.target) && openedPanels.length) {
...
const captureYTEvents = function (toContentScriptEventName) {
  document.addEventListener("yt-action", (event) => {
    if (event.detail && event.detail.actionName === "yt-append-continuation-items-action") {
      document.dispatchEvent(new CustomEvent(toContentScriptEventName, { detail: { actionName: "yt-append-continuation-items-action" } }));
    }
  });
};
