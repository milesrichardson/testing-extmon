...
const premiumFiltersUIitems = [
  {
    ...
    disclaimer: `${translate("please_note")} ${translate("distraction_control_new")} ${translate("your_participation")} ${translate(
      "dc_more_information"
    )}`,
...
async function getDefaultPFFilterUI(entry, isActiveLicense) {
  ...
  if (entry.disclaimer) {
    const $premiumDisclaimer = $("<div>").addClass("premium_disclaimer").addClass("top-line").text(entry.disclaimer);
    ...
    if (entry.disclaimer.includes("[[") && entry.disclaimer.includes("]]") && entry.urlToOpen) {
      ...
      const $replacementEl = $("<span>").attr("id", replacementElementId).addClass("link-text-color");
  ...
  const $lockOverlay = $("<span>").addClass("premium_locked_overlay").addClass("do-not-display").append($lockIcon);

  const $image = $("<img>").attr("src", entry.imageURL);

  const $imageWrapper = $("<div>").addClass("premium_image_wrapper").append($image).append($lockOverlay);
  ...
  if (isActiveLicense) {
    ...
    const $lensIcon = $("<i>lens</i>").attr("role", "img").attr("aria-hidden", "true").addClass("unchecked").addClass("material-icons");
    ...
    $iconWrapper = $("<span>").addClass("checkbox").addClass("md-stack").addClass("premium_icon").append($checkBoxIcons);
  }

  const $filterListRow = $("<div>").addClass("premium_filter_list_row").append($iconWrapper).append($description);
  ...
  const $filterWrapper = $("<div>").addClass("filter-subscription-wrapper").addClass(entry.topLineClass).append($premiumSubscription);
