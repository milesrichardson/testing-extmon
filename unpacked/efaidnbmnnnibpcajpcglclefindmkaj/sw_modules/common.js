...
let n = null,
  o = {
    prod: {
      ...
      uninstall_url: "https://acrobat.adobe.com/proxy/chrome-viewer/index.html?la=true#/uninstall",
      ...
      viewer_uri: {
        latest_uri: "https://acrobat.adobe.com/proxy/chrome-viewer/index.html",
        internal_uri: "https://acrobat.adobe.com/proxy/chrome-viewer/index.html",
        external_uri: "https://acrobat.adobe.com/proxy/chrome-viewer/1.0.4/index.html",
...
export function onMessageListener(e) {
  const t = e.env;
...
class a {
  ...
  GET_headers() {
    return (
      (this.$GET_headers["x-request-id"] = this.uuid()), (this.$GET_headers.Authorization = this.settings.auth_token), this.$GET_headers
    );
  }
  POST_headers() {
    return (
      (this.$POST_headers["x-request-id"] = this.uuid()), (this.$POST_headers.Authorization = this.settings.auth_token), this.$POST_headers
    );
  }
  noToken(e) {
    e && e.reject();
      ...
      this.settings.files_api
        ...
        : e
            .ajax({
              ...
              headers: { Accept: this.GET_headers().Accept, "x-api-client-id": this.GET_headers()["x-api-client-id"] }
            })
            .then(
              this.proxy(function (e) {
                (this.settings.files_api = e.api), (this.settings.files_upload = e.upload), t.resolve();
              }),
              function () {
                t.reject();
              }
          ...
          this.proxy(function () {
            if (this.settings.auth_code) {
              ...
              e.ajax({
                ...
                this.proxy(function (s) {
                  var r = new Date().getTime(),
                    n = /@adobe(test)?\.com$/i.test(s.email);
                  (this.settings.auth_code = null),
                    n
                      ? ((this.settings.auth_token = "Bearer " + s.access_token),
                        (this.settings.refresh_token = s.refresh_token),
                        (this.settings.token_expiry = r + s.expires_in),
                        (this.settings.refresh_time = r + s.expires_in / 2),
                        (this.settings.displayName = s.displayName),
                        t.resolve(),
                        e.consoleLog("got auth token"))
                      : (alert("PDF Helper Extension is available to Adobe Employees only"),
                        i.event(i.e.EXTENSION_FORCE_UNINSTALL),
        ...
        : this.connect().then(
            this.proxy(function () {
              this.settings.files_root
                ? i.resolve()
                : e.ajax({ url: this.settings.files_api + "root", type: "GET", headers: this.GET_headers() }).then(
                    this.proxy(function (e) {
  ...
  sso_url(t) {
    return e.ajax({ url: this.settings.cloud_api + "session/sso_uri?path=" + t, type: "GET", headers: this.GET_headers() });
  }
  authorize(e) {
    return (
  ...
  clearAuth() {
    this.settings.auth_token &&
      (e.ajax({ url: this.settings.cloud_api + "session", type: "DELETE", headers: this.$POST_headers }),
