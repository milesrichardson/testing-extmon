    ...
    (e = /\s+/),
      ...
      (r = {
        addClass(E, C) {
          if (C && (C = C.trim()))
            if (E.classList)
              C.indexOf(" ") > -1
                ? C.split(e).forEach(function (C) {
                    return E.classList.add(C);
                  })
                : E.classList.add(C);
            else {
              var B = " " + (E.getAttribute("class") || "") + " ";
              B.indexOf(" " + C + " ") < 0 && E.setAttribute("class", (B + C).trim());
            }
        },
        toogleClass(E, C) {
          (" " + (E.getAttribute("class") || "") + " ").indexOf(" " + C + " ") < 0 ? this.addClass(E, C) : this.removeClass(E, C);
        },
        hasClass: (E, C) => 0 != (C = C || "").replace(/\s/g, "").length && new RegExp(" " + C + " ").test(" " + E.className + " "),
        removeClass(E, C) {
          if (C && (C = C.trim()))
            if (E.classList)
              C.indexOf(" ") > -1
              ...
              for (var B = " " + (E.getAttribute("class") || "") + " ", D = " " + C + " "; B.indexOf(D) >= 0; ) B = B.replace(D, " ");
              (B = B.trim()) ? E.setAttribute("class", B) : E.removeAttribute("class");
            }
        },
        debounce(E, C) {
          let B = null;
      ...
      (a.prototype.render = function (E) {
        ...
        if (((B.className = "xl-slider"), C.step < 1 && (C.step = 1), C.max < C.min && (C.max = C.min + C.step), C.range)) {
          C.value = "object" == typeof C.value ? C.value : [C.min, C.value];
          var D = Math.min(C.value[0], C.value[1]),
            F = Math.max(C.value[0], C.value[1]);
          (C.value[0] = D > C.min ? D : C.min),
            (C.value[1] = F > C.min ? F : C.min),
            (C.value[0] = C.value[0] > C.max ? C.max : C.value[0]),
            (C.value[1] = C.value[1] > C.max ? C.max : C.value[1]);
          var A = Math.floor(((C.value[0] - C.min) / (C.max - C.min)) * 100),
            e = Math.floor(((C.value[1] - C.min) / (C.max - C.min)) * 100),
            t = e - A + "%";
          (A += "%"), (e += "%");
        } else
          "object" == typeof C.value && (C.value = Math.min.apply(null, C.value)),
            C.value < C.min && (C.value = C.min),
            C.value > C.max && (C.value = C.max),
            (t = Math.floor(((C.value - C.min) / (C.max - C.min)) * 100) + "%");
        var n = C.disabled ? "#c2c2c2" : C.theme,
          r = `<div class="xl-slider-bar" style="background:${n};width:${t};left:${
            A || 0
          };"></div>\n       <div class="xl-slider-wrap" style="left:${
            A || t
          };">\n        <div class="xl-slider-wrap-btn" style="border: 2px solid ${n};"></div>\n        <div class="xl-slider-wrap-value">${
            C.range ? C.value[0] : C.value
          }</div>\n       </div>\n      ${
            C.range
              ? `<div class="xl-slider-wrap" style="left: ${e};">\n             <div class="xl-slider-wrap-btn" style="border: 2px solid ${n};"></div>\n             <div class="xl-slider-wrap-value">${C.value[1]}</div>\n            </div>`
              : ""
          }\n       </div>`;
        (B.innerHTML = r),
          (this.container = document.querySelector(C.elem)),
          this.container.appendChild(B),
          (this.sliderInner = this.container.querySelector(".xl-slider")),
          (this.sliderBar = this.container.querySelector(".xl-slider-bar")),
          (this.sliderBtnWrap = this.container.querySelectorAll(".xl-slider-wrap")),
          C.range
            ? ((this.sliderBtnWrap[0].dataset.value = C.value[0]), (this.sliderBtnWrap[1].dataset.value = C.value[1]))
      ...
      (a.prototype.slide = function (E, C, B) {
        var D = this,
          ...
          o = function (E, C) {
            if (
              ...
              var e = Math.min(B, A),
                o = Math.abs(A - B);
              (D.sliderBar.style.width = o + "%"), (D.sliderBar.style.left = e + "%");
              var l = F.min + Math.round(((F.max - F.min) * E) / 100);
              if (((t[C].dataset.value = l), c(C, l), F.range)) {
                var d = [+t[0].dataset.value, +t[1].dataset.value];
                d[0] > d[1] && d.reverse();
              }
              F.change && F.change(F.range ? d : l);
            }
          },
          s = function (E) {
            var C = ((E / e()) * 100) / i,
          ...
          c = function (E, C) {
            ...
            B.innerText = C;
            var F = t[1].querySelector(".xl-slider-wrap-value").getBoundingClientRect(),
              e = t[0].querySelector(".xl-slider-wrap-value").getBoundingClientRect(),
        ...
        if ("set" === E) return o(C, B);
        D.sliderBtnWrap.forEach((E, C) => {
          var B = E.querySelector(".xl-slider-wrap-btn");
          B.addEventListener("mousedown", function (E) {
            E = E || window.event;
            var D = B.parentNode.offsetLeft,
              F = E.clientX;
            !(function (E, C) {
              var B = document.getElementById("LAY-slider-moving");
              if (!B) {
                var D = document.createElement("div");
                (D.id = "LAY-slider-moving"),
                  (D.className = "xl-auxiliar-moving"),
                  document.body.appendChild(D),
                  (B = document.getElementById("LAY-slider-moving"));
              }
              var F = function () {
                C && C(), B.parentNode.removeChild(B);
              };
              B.addEventListener("mousemove", E), B.addEventListener("mouseup", F), B.addEventListener("mouseleave", F);
            })(
              function (E) {
                E = E || window.event;
                var B = D + E.clientX - F;
      ...
      (window.onload = function () {
        var E = document.querySelector("#selected"),
          ...
          d = [],
          u = [
            { type: "默认", key: "all", count: 0, select: o },
            { type: "JPEG", key: "JPEG", count: 0, select: !1 },
            { type: "PNG", key: "PNG", count: 0, select: !1 },
            { type: "GIF", key: "GIF", count: 0, select: !1 },
            { type: "其他", key: "other", count: 0, select: !1 }
          ],
          m = [];
        function v(E) {
          !(function (E, C) {
            var B = m.filter((C) => E.indexOf(C.src) > -1),
              D = C.find((E) => "content-type" === E.name || "Content-Type" === E.name);
            if (B)
              for (let C of B) {
                var F = D ? D.value.split("/")[1].toLocaleUpperCase() : "其他";
                "JPG" === F && (F = "JPEG");
                var A = u.find((E) => E.type === F);
                if (A) {
                  var t = C.src.slice(C.src.lastIndexOf(".") + 1),
                    n = F.toLocaleLowerCase(),
        ...
        function p(t) {
          ...
          (l = [0, s]),
            ...
            B.addEventListener("click", (C) => {
              !(function (C) {
                if (C && "LI" === C.tagName) {
                  r.toogleClass(C, "is-checked"), r.toogleClass(C, "is-disabled");
                  var B = C.dataset.id;
                  (m[B].select = !m[B].select), (E.textContent = g().length);
                }
              })(C.target);
            }),
            e.addEventListener("click", (C) => {
              !(function (C) {
                if (C && ["SPAN", "SUP"].includes(C.tagName)) {
                  var D = C;
                  if (("SUP" === C.tagName && (D = C.parentNode), !r.hasClass(D, "is-disabled"))) {
                    r.toogleClass(D, "is-checked");
                    var F = D.dataset.id;
                    if (((u[F].select = !u[F].select), "0" === F)) {
                      if (u[F].select) {
                        var A = e.querySelectorAll("span");
                        [...A].slice(1).forEach((E, C) => {
                          r.removeClass(E, "is-checked"), (u[C + 1].select = !1);
                        });
                      }
                    } else if (u[F].select) {
                      var A = e.querySelectorAll("span");
        ...
        function f(E) {
          ...
          u.forEach((E, B) => {
            ...
            (D.dataset.id = B),
              E.select && r.addClass(D, "is-checked"),
              (D.innerHTML = E.type + (E.count ? ` <sup>${E.count}</sup>` : "")),
              C.appendChild(D);
          }),
            (E.innerHTML = ""),
            E.appendChild(C);
          ...
          if (E)
            if (C) {
              ...
              m.forEach((E, C) => {
                ...
                -1 === E.src.indexOf("?") ? (F.src = E.src + `?${t}=${Date.now()}`) : (F.src = E.src + `&${t}=${Date.now()}`),
                  E.select && r.addClass(D, "is-checked");
                let e = E.naturalWidth,
                  n = E.naturalHeight;
                (e > 250 || n > 250) &&
                  (e > n
                    ? (n = (e = e > 250 ? 250 : e) / (E.naturalWidth / E.naturalHeight))
                    : ((n = n > 250 ? 250 : n), (e = (E.naturalWidth / E.naturalHeight) * n))),
                  (F.width = e),
                  (F.height = n),
                  E.naturalWidth > s && (s = E.naturalWidth),
                  E.naturalHeight > c && (c = E.naturalHeight),
                  (A.textContent = `${E.naturalWidth}x${E.naturalHeight}`),
                  D.appendChild(F),
                  D.appendChild(A),
                  (D.dataset.id = C),
                  B.appendChild(D);
                ...
                (E.innerHTML = ""),
                m.length && E.appendChild(B);
            } else {
              var D = E.querySelectorAll("li img");
              m.forEach((E, C) => {
                if (D && D[C]) {
                  var B = D[C].parentNode;
                  E.select
                    ? (r.removeClass(B, "hidden"), r.removeClass(B, "is-disabled"), r.addClass(B, "is-checked"))
                    : (r.addClass(B, "hidden"), r.addClass(B, "is-disabled"), r.removeClass(B, "is-checked"));
                }
              });
            }
        }
        function g(E) {
          var C = u.filter((E) => E.select).map((E) => E.type);
          const B = C.includes("默认");
          var D = m.filter((D) => {
            var F = !!B || C.includes(D.type),
              A = D.select,
              e = D.select;
            return (
              E && E.width && (A = E.width[0] <= D.naturalWidth && D.naturalWidth <= E.width[1]),
              E && E.height && (e = E.height[0] <= D.naturalHeight && D.naturalHeight <= E.height[1]),
              (D.select = A && e && F),
              D.select
            );
          });
        ...
        !(function () {
          ...
          do {
            if (!E || !E.searchParams) break;
            let C = E.searchParams.get("tabId");
            if (!C) break;
            if (((C = Number(C)), isNaN(C))) break;
            chrome.tabs.get(C, (E) => {
              E &&
                ((n = E.url),
                chrome.tabs.onRemoved.addListener(function (E, B) {
                  E === C && window.close();
                }),
                chrome.scripting.executeScript(
                  ...
                  (E) => {
                    var C = [];
                    E && E[0] && E[0].result && (C = E[0].result);
                    let B = {},
                      D = [];
                    for (let E of C) B[E.src] || ((B[E.src] = !0), D.push(E));
                    p(D);
                  }
                ));
            });
          ...
          chrome.runtime.onMessage.addListener((E, C, B) => {
            "xlMultiPicUpdateDetail" == E.name && v(E.value);
          });
      });
  },
  function (E, C, B) {
