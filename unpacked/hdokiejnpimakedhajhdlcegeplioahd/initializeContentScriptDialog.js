!(function () {
  ...
  e.setAttribute("class", "backgroundFrame"),
    (e.src = "backgroundFrame.html?lplanguage=" + encodeURIComponent(LPTools.getURLParams().lplanguage)),
        ...
        e.contentWindow.Topics.get(e.contentWindow.Topics.REQUEST_FRAMEWORK_INITIALIZED).subscribe(function (e, t) {
          ...
          (csTop = Interfaces.createInstance(Interfaces.ContentScriptInterface, {
            ...
            requestFunction: function (e) {
              t.sendRequest({ type: "contentScriptRequest", data: e, frameID: o });
            }
          })).LPFrame.initializeRequestFramework(LPTools.getURLParams().dialogID);
        });
    }),
    document.body.appendChild(e),
    Topics.get(Topics.REPROMPT).subscribe(function (e) {
      bg.LPPlatform.openPopoverDialog("reprompt", { successCallback: e });
    }),
    ...
    Topics.get(Topics.ERROR).subscribe(function (e) {
      ...
      0 < Dialog.prototype.getDialogCount() && dialogs.alert.open({ title: Strings.translateString("Error"), text: e });
    });
})();
