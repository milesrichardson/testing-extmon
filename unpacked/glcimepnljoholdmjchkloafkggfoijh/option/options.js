...
function showPromoSwitchPannel(shown) {
  let isBrowserSuportCuivo = getBrowserSuportCuivo();
  if (shown && isBrowserSuportCuivo) {
...
function initPrivacyMsg(pid, postfix) {
  ...
  document.getElementById("checkbox-privacy-switch" + postfix).addEventListener("click", function (e) {
    ...
    document.getElementById("checkbox-privacy-switch" + postfix).src = curChecked
      ? "../images/ic_checkbox_normal.png"
      : "../images/ic_checkbox_checked.png";
  });
  getPref(Prop.agree_to_privacy_policy, true, function (value) {
    document.getElementById("checkbox-privacy-switch" + postfix).src = value
      ? "../images/ic_checkbox_checked.png"
      : "../images/ic_checkbox_normal.png";
  });
}
...
function initLayout(response) {
  ...
  if (response.state == CompatibleState.none || response.state == CompatibleState.upgrade_env) {
    // CompatibleState.none is canonical plugin
    ...
    if (getUILang() != "zh-CN") {
      ...
      if (true) {
        // 360safe don't support new features
        url360safe.style.display = "none";
      } else {
