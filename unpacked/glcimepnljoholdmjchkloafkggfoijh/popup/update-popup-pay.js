function updatePopup(document, msg) {
  ...
  if (msg.netpay != undefined) {
    ...
    if (msg.netpay == 0) {
      ...
      if (enableAntiTrack) statusDesc.textContent = chrome.i18n.getMessage("shopping_desc_antitrack");
      else statusDesc.textContent = chrome.i18n.getMessage("shopping_desc");
    }
  } else {
    // disabled by ts/360safe
