...
(function (root, factory) {
    ...
    function peg$parse(input) {
      var options = arguments.length > 1 ? arguments[1] : {},
        ...
        peg$c171 = function (char_) {
          return char_.replace("b", "\b").replace("f", "\f").replace("n", "\n").replace("r", "\r").replace("t", "\t").replace("v", "\x0B"); // IE does not recognize "\v".
        },
      ...
      function peg$buildException(message, expected, found, location) {
        ...
        function buildMessage(expected, found) {
          ...
          expectedDesc =
            expected.length > 1 ? expectedDescs.slice(0, -1).join(", ") + " or " + expectedDescs[expected.length - 1] : expectedDescs[0];
    ...
    return {
      ...
      log: function () {
        ...
        if (__window.console) {
          ...
          l.innerHTML =
            "<i>" + ts + "</i> " + (typeof arguments[0] === "string" && arguments.length === 1 ? arguments[0] : JSON.stringify(arguments));
          __dbg.appendChild(l);
        }
      }
    };
  });
  define("events", ["debug"], function (debug) {
   ...
   *
   */
  define("page", ["constants", "interpreter", "request", "utils", "events"], function (constants, interpreter, request, utils, events) {
   ...
   *
   */
  define("ui", ["mustache", "constants", "utils", "jquery", "plurals", "module", "events"], function (
    Mustache,
    constants,
    utils,
    jQuery,
    ...
    var Toolbar = function (document, settings) {
      ...
      (this.prefix = settings.tag + "-"), (this.doc = document);
    ...
    Toolbar.prototype = {
      ...
      render: function (offers, callback) {
        ...
        if (!this.templateBundle) {
          this.$.getJSON(self.api_url_bundle + constants.version + "/" + self.tag + "/bundle.json", function (data) {
      ...
      closeAllLists: function () {
        this.$("." + this.className("offer-list-container")).removeClass(this.className("visible"));
      },
      ...
      registerEvents: function () {
        ...
        this.$("." + self.className("dlg-close"))
          .unbind("click")
          .on("click", function () {
            self.hideDialog();
          });

        this.$("." + self.className("click-redirect"))
          .unbind("click")
          .on("click", function () {
      ...
      showDialog: function (id, params) {
        ...
        if (container.hasClass(this.className("expanded")) && iframe.attr("data-dialog-id") === id) {
          this.hideDialog();
        } else {
      ...
      closeDialogs: function () {
        this.$("#" + this.className("container")).removeClass(this.className("expanded"));
        this.$("#" + this.className("icontent"))
          .removeAttr("src")
          .removeAttr("data-dialog-id");
      },
  ...
  define("contentscript", ["constants", "utils", "xdstorage", "page", "ui"], function (constants, utils, xdstorage, page, ui) {
    ...
    ContentScript.prototype = {
      ...
      run: function (callback) {
        ...
        var self = this,
          storageUrl = self.api_url_storage + "?version=" + encodeURIComponent(constants.version);
      ...
      on_storage_return: function (storage, data) {
        var user_data = this.prep_user_data(storage, data),
          self = this,
   ...
   *
   */
  define("optin", ["contentscript", "xdstorage", "constants", "events"], function (contentscript, xdstorage, constants, events) {
