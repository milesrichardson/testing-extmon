...
if (document.location.pathname.match(/^\/course\/([0-9]+)\/materials\/link\/view\//)) {
  ...
  if (previewOnly) {
    ...
    newLink.setAttribute("href", pdfReaderServerAddress + "?file=" + activeLink + "?previewonly=true");
...
if (document.location.pathname.match(/^\/course\/([0-9]+)\/materials/)) {
  Drupal.behaviors.stextHelp = function (sender) {
    var i = document.querySelector(".s-grade-item-assignment-submission-apps");

    if (i != null) {
      var activeTHButton = document.getElementById("texthelpAssignment");
      ...
      if (activeTHButton == undefined) {
        ...
        textHelpButton.classList.add("s-grade-item-assignment-submission-app");
      ...
      <img class="app-logo" src="chrome-extension://feepmdlmhplaojabeoecaobfmibooaid/Chrome/Icons/orbitnote-icon-32x32.png"></div>\n    
        ...
        function receiveMessage(event) {
          ...
          if (event.origin == "chrome-extension://" + chromeId + "") {
            ...
            axios
              ...
              .then(function (t) {
                ...
                var transferFilename = event.data.file_name.toLowerCase();

                if (transferFilename.indexOf(".pdf") == -1) {
                  transferFilename = transferFilename + ".pdf";
                }
                this.SchoologyFilename = transferFilename;

                formSend.append("name", transferFilename);
                formSend.append("use_plain", 1);
                formSend.append("chunks", 0);
                formSend.append("file", t.data);
                formSend.append("originalfile", transferFilename);
                ...
                axios
                  .post(schoolurl + "/s_attachment_upload_chunked/2", formSend, {
                    onUploadProgress: function (t) {
                      console.log((t.loaded / t.total) * 0.5 + 0.5);
                    }
                  })
                  .then(function (response, transferFilename) {
                    console.log(response.request.response);

                    var fileCollect = document.querySelector("[name='file[files]'");

                    var fileUploadData = {};

                    var fileUploadResponse = JSON.parse(response.request.response);

                    fileUploadData[fileUploadResponse.file.fid] = {
                    ...
                    fileCollect.value = JSON.stringify(fileUploadData);

                    // Handle UI

                    HandleAssignmentUI(fileUploadResponse);
                    // Close Popup
                    Popups.close(e);
                  });
              });
        ...
        function HandleAssignmentUI(fileDetails) {
          //Set title
          var documentTitle = document.querySelector(".s-grade-item-assignment-submission-content-title");
          ...
          var appsButtons = document.querySelectorAll(".s-grade-item-assignment-submission-app");

          appsButtons.forEach(function (activeButton) {
            if (activeButton.getAttribute("data-app-nid") == "Texthelp") {
          ...
          var documentHolder = document.querySelector(".s-grade-item-assignment-submission-content");
        ...
        textHelpButton.addEventListener("click", function (t) {
          ...
          t.stopPropagation(), t.preventDefault(), t.stopImmediatePropagation(), Popups.saveSettings();
          var chromeId = "feepmdlmhplaojabeoecaobfmibooaid";

          if (typeof PDFJSDev !== "undefined" && PDFJSDev.test("GENERIC || QA")) {
            chromeId = "efeafadncamffgiohgiciboonbjldkfj";
          }
          ...
          e = Popups.openPathContent("assignment_submission_app", "Add Assignment From OrbitNote", a, t.target, s, o);
        });
      }
    }
  };
...
if (document.location.pathname.match(/^\/assignment\/([0-9]+)\/info/)) {
  ...
  if (thReturnURL != "") {
    var a = document.location.pathname.match(/^\/assignment\/([0-9]+)\/info/);
    var courseID = a[1];

    var c = document.querySelector("[href='/assignment/" + courseID + "/dropbox/submit']");
    c.click();
    ...
    schoologyAppContainer.src =
      `chrome-extension://` +
      chromeId +
      `/schoologyHandin/index.html?file=SchoologyHandin&th_return_url=` +
      thReturnURL +
      `#domain=` +
      schoologyDomain;
    ...
    function receiveMessage(event) {
      if (
        event.origin == "chrome-extension://efeafadncamffgiohgiciboonbjldkfj" ||
        event.origin == "chrome-extension://feepmdlmhplaojabeoecaobfmibooaid"
      ) {
        if (event.data.action == "TurnIn") {
          ...
          xhr.open("GET", "https://www.googleapis.com/drive/v2/files/" + docID + "?fields=downloadUrl,title", true);
          xhr.setRequestHeader("Authorization", event.data.authorization_header);
          ...
          xhr.onreadystatechange = function () {
            if (this.readyState == 4) {
              if (this.status == 200) {
                ...
                thFileTitle = getUrlParameter("th_return_file_name");

                axios
                  .get(jsonResponse.downloadUrl, {
                    responseType: "blob",
                    headers: headers
                  })
                  .then(function (t) {
                    var intervalSettings = setInterval(() => {
                      if (
                        Drupal &&
                        Drupal.settings &&
                      ...
                      var submissionForm = document.querySelector(".popups-dropbox-submit");

                      submissionForm.style.display = "none";
                    }, 100);
                    clearInterval(intervalSettings);

                    UploadTurnInAssignment(thFileTitle, t.data);
    ...
    var assignmentIDPath = document.location.pathname.match(/^\/assignment\/([0-9]+)\/info/);
  ...
  var attachmentFiles = document.querySelectorAll("#main-inner .attachments-file");
  ...
  attachmentFiles.forEach(function (attachmentItem) {
    var filenameArea = attachmentItem.querySelector(".attachments-file-name");
    var fileIcon = attachmentItem.querySelector(".attachments-file-icon .pdf-icon");
    var documentPDFhref = attachmentItem.querySelector(".sExtlink-processed").href;
    var classAssignmentID = document.getElementById("edit-nid");

    if (classAssignmentID == null) {
      // Try and get the assignment id from url
      var pageelem = document.location.pathname.match(/^\/assignment\/([0-9]+)\/info/);
    ...
    if (fileIcon != null) {
      var CourseID = document.getElementById("edit-node-realm-id");
      ...
      var filenameRoot = attachmentItem.querySelector(".attachments-file-name");

      var originalFileName = filenameRoot.querySelector(".infotip");
      ...
      if (originalFileName != null) {
        ...
        originalFileName = filenameRoot.querySelector(".sExtlink-processed");
        if (originalFileName != null) {
          originalFileName = originalFileName.innerText;
      ...
      tempDiv.setAttribute(
        ...
        "chrome-extension://" +
          chromeId +
          "/schoologyOpen/index.html?file=" +
          documentPDFhref +
          "?&nid=" +
          classAssignmentID.value +
          "&cid=" +
          CourseID.value +
          "&ofilename=" +
          originalFileName +
          ...
          PageTitleClean +
          "&CourseTitle=" +
          encodeURIComponent(courseNameTxt.innerText) +
          "&SCHOOLOGYURL=true"
      );
...
function UploadTurnInAssignment(SubmissionFileName, UploadData) {
  ...
  axios
    .post(
      ...
      {
        headers: {
          Authorization: "Bearer " + Drupal.settings.s_attachment.file_service_upload.token
        },
        onUploadProgress: function (t) {
          console.log((t.loaded / t.total) * 0.5 + 0.5);
        }
    ...
    .then(function (response) {
      ...
      var resolveAssignmentPath = document.querySelector("#dropbox-submit-upload-tab-content form");
      resolveAssignmentPath.action = resolveAssignmentPath.action.split("%3F")[0];
      ...
      var attachmentContainer = document.querySelector("#attachments-added-container");
      ...
      var activeUploadWindow = document.getElementById("thSubmission");

      try {
        activeUploadWindow.parentNode.removeChild(activeUploadWindow);
      } catch (e) {}
...
if (document.location.pathname.match(/^\/course\/([0-9]+)\/materials\//)) {
  ...
  if (previewOnly) {
    ...
    newLink.setAttribute("href", pdfReaderServerAddress + "?file=" + pdfLink + "?previewonly=true");
