...
function isPdfDownloadable(details) {
  ...
  if (details.type === "main_frame" && details.url.indexOf("=download") === -1) {
    return false;
  }
  var cdHeader = details.responseHeaders && getHeaderFromHeaders(details.responseHeaders, "content-disposition");
...
function isPdfFile(details) {
  ...
  if (header) {
    ...
    if (headerValue === "application/octet-stream") {
      ...
      var cdHeader = getHeaderFromHeaders(details.responseHeaders, "content-disposition");
...
chrome.webRequest.onHeadersReceived.addListener(
  function (details) {
    ...
    if (isOrbitDoc(details.url)) {
      if (typeof details.initiator == "undefined" || details.initiator.indexOf("classroom.google.com") == -1) {
