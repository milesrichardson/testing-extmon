...
if (location.pathname.match(/\-/g).length >= 2) {
    ...
    createElem() {
      switch (this.type) {
        ...
        case "checkbox":
          ...
          labelName.appendChild(document.createTextNode(this.options.labelData.name));
          ...
          checkboxElement.setAttribute("id", this.options.labelData.id + "-box");
          ...
          if (this.options.labelData.radio) {
            this.options.labelData.radio.radios.forEach((radio, i, radios) => {
              ...
              let id = radio.value.toLowerCase() + "-" + this.options.labelData.id + "-radio";
              let type = i == 0 ? "left" : i == radios.length - 1 ? "right" : "middle";

              if (i == 0 && !this.options.labelData.radio.checked) radioInput.setAttribute("checked", "checked");
              if (i + 1 == this.options.labelData.radio.checked) radioInput.setAttribute("checked", "checked");
              radioInput.setAttribute("type", "radio");
              radioInput.setAttribute("class", "optic-null optic-rinput");
              radioInput.setAttribute("name", this.options.labelData.radio.name);
              radioInput.setAttribute("id", id);
              radioLabel.setAttribute("for", id);

              radioLabel.setAttribute("class", id.replace("-radio", "-trigger ") + type + " optic-radio");
          ...
          if (this.options.labelData.slider) {
            ...
            let value =
              ...
              Math.floor(this.options.labelData.slider.min / 2 + this.options.labelData.slider.max / 2);
          ...
          if (this.options.labelData.input) {
            ...
            textInput.setAttribute("id", this.options.labelData.id + "-text-input");
            textInput.setAttribute("type", "text");
            textInput.setAttribute("class", "optic-input");
            textInput.setAttribute("placeholder", this.options.labelData.input.placeholder || "Lorem Ispum...");
            textInput.setAttribute("maxLength", this.options.labelData.input.maxLength || -1);
          ...
          if (this.options.labelData.upload) {
            ...
            fileInput.setAttribute("accept", this.options.labelData.upload.accept.join(", "));
            inputDisplay.setAttribute("class", "optic-file");
            inputDisplay.setAttribute("for", fileInput.id);

            inputDisplay.appendChild(document.createTextNode(this.options.labelData.upload.text || "Upload"));
    ...
    addListeners() {
      ...
      this.listeners.forEach((listenerData) => {
        listenerData.events.forEach((event) => {
          document.querySelector(listenerData.selector).addEventListener(event, listenerData.listener);
        });
      });
    }
  }
  ...
  class GoogleMeetOptics {
    ...
    async initThree(spec) {
      ...
      this.FX.Three.threeStuffs = THREE.JeelizHelper.init(
        spec,
        (a, c) => {}
      ...
      //  BUBBLES //
      this.FX.Three.Bubbles.stored.texture = new window.THREE.Texture(this.input);
      this.FX.Three.Bubbles.stored.geometry = new THREE.SphereBufferGeometry(0.1, 32, 16);
      this.FX.Three.Bubbles.stored.material = new window.THREE.MeshBasicMaterial({
        map: this.FX.Three.Bubbles.stored.texture
      });
      for (var i = 0; i < 200; i++) {
        let mesh = new THREE.Mesh(this.FX.Three.Bubbles.stored.geometry, this.FX.Three.Bubbles.stored.material);
      ...
      await (async () => {
        ...
        this.FX.Three.Halo.objects = (
          await this.loadOBJ(this.FX.Three.Halo.stored, [lightsource, lightsource2], this.FX.Three.Halo.url, (root) => {
      ...
      await (async () => {
        ...
        this.FX.Three.Graduate.objects = (
          await this.loadOBJ(this.FX.Three.Graduate.stored, [lightSource], this.FX.Three.Graduate.url, (root) => {
            root.position.y = 0.5;
      ...
      await (async () => {
        ...
        this.FX.Three.Sunglasses.objects = (
          await this.loadOBJ(this.FX.Three.Sunglasses.stored, [lightSource], this.FX.Three.Sunglasses.url, (root) => {
            root.rotateX((-90 * Math.PI) / 180);
    ...
    loadOBJ(faceMesh, lightSources, url, edit) {
      return new Promise((res) => {
        ...
        objLoader.load("https://visualeffectsbackend.herokuapp.com/api/static" + url, (root) => {
          root = edit(root);
    ...
    void main() {\n\
      ...
      // gl_FragColor=vec4(darkenCoeff, 255.0, 255.0, 1.);\n\
              ...
              root.add(FakeFace);
              lightSources.forEach((lightSource) => (lightSource.target = root));
    ...
    setBackground() {
      return new Promise((r) => {
        ...
        if (this.FX.Two.GreenScreen.IT.includes("video")) {
          ...
          background.addEventListener("loadedmetadata", useImage.bind(this), false);
          function useImage() {
    ...
    async updateGreen(tick) {
      ...
      try {
        if (this.FX.Two.GreenScreen.img) {
          ...
          this.FX.Two.GreenScreen.ctx.clearRect(0, 0, this.FX.Two.GreenScreen.canvas.width, this.FX.Two.GreenScreen.canvas.height);
      ...
      return new Promise((r, err) => {
        try {
          ...
          function addNodes() {
            ...
            this.FX.Two.GreenScreen.canvas = document.querySelector("#optic-img");
            this.FX.Two.GreenScreen.storageCanvas = document.querySelector("#optic-storage");
            this.FX.Two.GreenScreen.storageCtx = this.FX.Two.GreenScreen.storageCanvas.getContext("2d");
            this.FX.Two.SoftBlur.img = softblur;
            this.canvas = document.querySelector("#canvas");
            this.threeCanv = threeCanv;
            this.ctx = this.canvas.getContext("2d");
            this.FX.Two.GreenScreen.ctx = this.FX.Two.GreenScreen.canvas.getContext("2d");
        ...
        listeners: [
          ...
          },
          {
            ...
            listener: function (event) {
              // This'll be updated
              this.FX.Two.Bokeh.rate = 21 - parseInt(document.getElementById("bokeh-rate").value);
            }.bind(this)
          }
        ]
      });
      ...
      this.UI.Labels.Blur = new OpticElement("checkbox", {
        ...
        listeners: [
          ...
          },
          {
            ...
            listener: function (event) {
              // This'll be updated
              this.FX.Two.Blur.rate = 100 - parseInt(document.getElementById("blur-rate").value);
            }.bind(this)
          }
        ]
      });
      ...
      this.UI.Labels.Contrast = new OpticElement("checkbox", {
        ...
        listeners: [
          ...
          },
          {
            ...
            listener: function (event) {
              // This'll be updated
              this.FX.Two.Contrast.rate = 256 - parseInt(document.getElementById("contrast-rate").value);
            }.bind(this)
          }
        ]
      });
      ...
      this.UI.Labels.Pixelate = new OpticElement("checkbox", {
        ...
        listeners: [
          ...
          },
          {
            ...
            listener: function (event) {
              // This'll be updated
              this.FX.Two.Pixelate.rate = parseInt(document.getElementById("pixel-rate").value) / 300;
            }.bind(this)
          }
        ]
      });
      ...
      this.UI.Labels.SoftBlur = new OpticElement("checkbox", {
        ...
        listeners: [
          ...
          },
          {
            ...
            listener: function (event) {
              // This'll be updated
              this.FX.Two.SoftBlur.rate = 10 - document.getElementById("softblur-rate").value;
            }.bind(this)
          }
        ]
      });
      ...
      this.UI.Labels.GreenScreen = new OpticElement("checkbox", {
        ...
        listeners: [
          ...
          },
          {
            ...
            events: ["change"],
            listener: function (e) {
              this.FX.Two.GreenScreen.url = URL.createObjectURL(e.target.files[0]);
              document.querySelector("label.optic-file").classList.add("selected");
    ...
    editPixel(inputData, greenData, p) {
      if (this.FX.Two.GreenScreen.type == "old") {
        if (inputData[0] + inputData[2] < inputData[1] + this.FX.Two.GreenScreen.tolerance) {
        ...
        return this.blendPixel([this.storedImg[p], this.storedImg[p + 1], this.storedImg[p + 2], this.storedImg[p + 3]], greenData, alpha); // 0 = full green
      }
    }
    ...
    async findSegments(effect) {
      ...
      return await this.network.segmentPerson(this.input, this.FX.Two[effect].tensorConfig);
    }
    async addGreen(tick) {
      ...
      this.FX.Two.GreenScreen.storageCtx.drawImage(this.input, 0, 0, this.input.videoWidth, this.input.videoHeight);
      this.storedImg = this.FX.Two.GreenScreen.storageCtx.getImageData(0, 0, this.canvas.width, this.canvas.height).data;
      let segments = await this.findSegments("GreenScreen");
      let mask = bodyPix.toMask(segments, { r: 0, g: 0, b: 0, a: 255 }, { r: 255, g: 255, b: 255, a: 255 });
      bodyPix.drawMask(this.canvas, this.input, mask, 1, 2, false);
    }
    async addBokeh(tick) {
      // if (tick.toString().slice(-2) == "00") this.disableAll();
      let segments = await this.findSegments("Bokeh");
      bodyPix.drawBokehEffect(this.canvas, this.input, segments, +this.FX.Two.Bokeh.rate, this.FX.Two.Bokeh.edgeRate, false);
    }
    async addGreenScreen(tick) {
      return new Promise((res) => {
        try {
          ...
          this.FX.Two.GreenScreen.tolerance = this.FX.Two.GreenScreen.tolerance || 0;
          var len = imageData.length;

          for (var p = 0; p < len; p += 4) {
            let n = this.editPixel(
              [imageData[p], imageData[p + 1], imageData[p + 2], imageData[p + 3]],
    ...
    async addContrast(tick) {
      return new Promise((res) => {
        ...
        try {
          var imageData = this.ctx.getImageData(0, 0, this.canvas.width, this.canvas.height);
          var data = imageData.data;
          var factor = (259 * (this.FX.Two.Contrast.rate + 255)) / (255 * (259 - this.FX.Two.Contrast.rate));
          var len = data.length;
    ...
    async addText(text = "Lorem Ipsum") {
      ...
      this.ctx.fillText(text, this.canvas.width / 2, 100, this.canvas.width - 20);
      this.ctx.strokeText(text, this.canvas.width / 2, 100, this.canvas.width - 20);
    }
    ...
    async applySoftBlur(tick) {
      ...
      this.ctx.drawImage(this.FX.Two.SoftBlur.img, 0, 0, this.canvas.width, this.canvas.height);
      ...
      if (this.state >>> 2 && (this.state & 0b010) >>> 1) {
        try {
          ...
          if (this.FX.Two.Misha.active) {
            this.FX.Two.Misha.index = this.FX.Two.Misha.Misha.update(this.FX.Two.Misha.index || 1);
          }
          ...
          if (this.FX.Two.Blur.active) {
            await boxBlurCanvasRGB("canvas", 0, 0, this.canvas.width, this.canvas.height, this.FX.Two.Blur.rate, 1);
          }
          ...
          if (this.FX.Two.Flip.active) {
            ...
            this.ctx.drawImage(this.canvas, -this.canvas.width / 2, -this.canvas.height / 2, this.canvas.width, this.canvas.height);
            this.ctx.restore();
          }
        } catch (err) {
    ...
    convertThree() {
      if (!this.Freeze) {
        ...
        this.ctx.translate(this.canvas.width / 2, this.canvas.height / 2);
        this.ctx.rotate((180 * Math.PI) / 180);
        this.ctx.drawImage(this.threeCanv, -this.canvas.width / 2, -this.canvas.height / 2, this.canvas.width, this.canvas.height);
        this.ctx.restore();
      }
    }
    async updateRender(tick) {
      try {
        ...
        this.FX.Three.Renderer.setSize(this.threeCanv.width, this.threeCanv.height);
    ...
    async render(tick) {
      if (this.threeCanv.width !== this.input.videoHeight || this.threeCanv.height !== this.input.videoHeight) {
        this.updateRender(tick);
      }
      if (
        Object.keys(this.FX.Three).filter((key) => !this.FX.Three[key].uuid && this.FX.Three[key].active && this.FX.Three[key].objects)
          .length
      ) {
        if (this.FX.Three.Cube.active) {
        ...
        if (this.FX.Two.Bokeh.active || (this.FX.Two.GreenScreen.type == "new" && this.FX.Two.GreenScreen.active)) {
          if (this.FX.Two.Bokeh.active) {
            await this.addBokeh(tick);
          }
          if (this.FX.Two.GreenScreen.type == "new" && this.FX.Two.GreenScreen.active) {
