...
(function (_, EventEmitter) {
  ...
  if (typeof AvastWRC === "undefined") {
    AvastWRC = {};
  } //AVAST Online Security - namespace
  ...
  AvastWRC.bal = {
      ...
      Promise.all(shepherdPromise).then(() => {
        Q.fcall(
          ...
          .then(() => {
            ...
            AvastWRC.bal.sendHeartbeatInterval = setInterval(
              function () {
                AvastWRC.bal.sp.sendHeartbeat();
              },
    ...
    bootstrapInit: function (edition) {
      ...
      var bootstrapped = _.reduce(
        this._bootstrap_modules,
        function (bModules, moduleBootstrap) {
    ...
    updateOldUserSettings: function () {
      ...
      if (newSettings.advanced) {
        ...
        if (newSettings.general) {
          newSettings.notifications.offers.item1Selected =
            newSettings.advanced.offers.item1Selected && !newSettings.general.item2Selected ? true : false;
          newSettings.notifications.offers.item2Selected =
            newSettings.advanced.offers.item2Selected && !newSettings.general.item2Selected ? true : false;
          newSettings.notifications.offers.item3Selected =
            newSettings.advanced.offers.item3Selected && !newSettings.general.item2Selected ? true : false;
          newSettings.notifications.coupons.item1Selected =
            newSettings.advanced.coupons.item1Selected && !newSettings.general.item2Selected ? true : false;
          newSettings.notifications.coupons.item2Selected =
            newSettings.advanced.coupons.item2Selected && !newSettings.general.item2Selected ? true : false;
        }
    ...
    getFAQsUrl: function () {
      ...
      var data = {
        url: `https://ipm-provider.ff.avast.com/?action=2&p_pro=${brandingSpecifics.pPro}&p_elm=${brandingSpecifics.pElem}&p_lng=${bLang}&p_scr=${brandingSpecifics.pScr}`,
    ...
    getHostFromUrl: function (url) {
      ...
      if (
        lcUrl.toLowerCase().indexOf("http") !== 0 ||
...
(function (AvastWRC, _) {
  ...
  AvastWRC.Shepherd = {
    ...
    isValidTtl: function () {
      ...
      if (self.rules && self.rules.expireTime) return self.rules.expireTime > now;
      else return false;
    },
...
(function (_, Shepherd) {
  ...
  _.extend(Shepherd, {
    ...
    getIconBlinkAnimationConfig: function () {
      return Shepherd.rules && Shepherd.rules.spIconAnimation && Shepherd.rules.spIconAnimation.Configs
        ? Shepherd.rules.spIconAnimation.Configs
        : {
    ...
    getIconBlinkingAnimationAfterCloseTooltip: function () {
      return Shepherd.rules && Shepherd.rules.spIconAnimationCloseTooltip && Shepherd.rules.spIconAnimationCloseTooltip.Configs
        ? Shepherd.rules.spIconAnimationCloseTooltip.Configs
        : {
            icon: "common/ui/icons/logo-safeprice-128.png",
            color: AvastWRC.CONFIG.BRANDING_TYPE === AvastWRC.BRANDING_TYPE_AVAST ? "#0CB754" : "#556688",
            times: 22,
    ...
    getNotificationsConfig: function () {
      ...
      let notificationsConfig =
        Shepherd.rules && Shepherd.rules.notifications && Shepherd.rules.notifications.configs
          ? Shepherd.rules.notifications.configs
    ...
    updateEmptySearchConfig: function (rules) {
      ...
      if (rules.emptySearch.Configs.offers.domain) {
        rules.emptySearch.Configs.offers.trySearchIn = AvastWRC.bs.getLocalizedString("spTrySearchInAmazon", [
          rules.emptySearch.Configs.offers.domain
        ]);
      }
      if (rules.emptySearch.Configs.offers.domainName) {
        rules.emptySearch.Configs.offers.searchIn = AvastWRC.bs.getLocalizedString("spSearchInAmazon", [
          rules.emptySearch.Configs.offers.domainName
        ]);
      }
      if (rules.emptySearch.Configs.coupons.domain) {
        rules.emptySearch.Configs.coupons.trySearchIn = AvastWRC.bs.getLocalizedString("spTrySearchDeals", [
          rules.emptySearch.Configs.coupons.domain
        ]);
      }
      if (rules.emptySearch.Configs.coupons.domainName) {
        rules.emptySearch.Configs.coupons.searchIn = AvastWRC.bs.getLocalizedString("spSearchInAmazon", [
          rules.emptySearch.Configs.coupons.domainName
        ]);
      }
      return rules;
    },
    ...
    isSearchActive: function () {
      if (
        Shepherd.rules &&
        Shepherd.rules.searchConfigs &&
        Shepherd.rules.searchConfigs.Configs &&
    ...
    getDefaultPanelTabs: function () {
      if (Shepherd.rules && Shepherd.rules.defaultPanelTabs && Shepherd.rules.defaultPanelTabs.Configs) {
        return Shepherd.rules.defaultPanelTabs.Configs;
      }
      return Shepherd.defaultPanelTabs;
    },
    getAsbNotificationsString: function () {
      if (Shepherd.rules && Shepherd.rules.asbNotificationsString && Shepherd.rules.asbNotificationsString.Configs) {
...
(function (AvastWRC, _) {
  ...
  AvastWRC.ASDetector = {
    init: function () {
      ...
      self.pastEvents = {};
      self.listeners = [];
      self.blockref = afsrcConfig.blockref || [
      ...
      self.ciuvo_rex = afsrcConfig.ciuvo_rex || [
        new RegExp(".*ciuvo.com"), // ciuvo
      ...
      console.log(
        "DETECTOR initialized: blockref",
        self.blockref,
        " ciuvo_rex: ",
        self.ciuvo_rex,
        " NEW_EVENT_THRESHOLD_TIME: ",
        self.NEW_EVENT_THRESHOLD_TIME,
        " SESSION_TIMEOUT: ",
        self.SESSION_TIMEOUT,
        " TAB_EVENT_EXPIRATION_TIME: ",
        self.TAB_EVENT_EXPIRATION_TIME
      );
    },
...
(function (AvastWRC, _) {
  AvastWRC.YesNo = {
    ...
    isPowerUser: function () {
      return this.currentLanguageIsSupported() && !this.userAlreadyProvidedFeedback() && this.userHasInteractEnough() && this.isTimeToAsk();
    },
    ...
    userHasInteractEnough: function () {
      return (
        daysSinceInstallation() > this.powerUserData.serverConfig.minimumTime &&
        this.powerUserData.interactions.length >= this.powerUserData.serverConfig.minimumInteracionts
      );

      function daysSinceInstallation() {
        let date = AvastWRC.CONFIG.InstallDate.split(" ")[0],
          millisecondsOfOneDay = 86400000;
        return Math.round(
          new Date().getTime() / millisecondsOfOneDay -
    ...
    askLaterValid: function () {
      let askLaterValid =
          !this.powerUserData.askLaterEnabled ||
          this.isValidDelay(this.powerUserData.serverConfig.askLaterDelay * 86400, this.powerUserData.askedTTL.askLater),
        self = this;
    ...
    getHowToImproveLink: function () {
      return AvastWRC.Query.CONST.SAFESHOP_FEEDBACK_SERVER[
        AvastWRC.CONFIG.BRANDING_TYPE === AvastWRC.BRANDING_TYPE_AVAST ? "AVAST" : "AVG"
      ];
    },
    userAsked: function () {
    ...
    registerInteraction: function (tab) {
      ...
      function addInteraction(tab) {
        let domain = AvastWRC.bal.getDomainFromUrl(tab.url),
          tabId = tab.id,
          index = domain + tabId;

        if (
          self.powerUserData.interactions.length > self.powerUserData.serverConfig.minimumInteracionts ||
    ...
    keyName: "YesNo",
    ratingLink: "https://chrome.google.com/webstore/detail/avg-safeprice/mbckjcfnjmoiinpgddefodcighgikkgn/reviews?hl=__BROWSER_LANGUAGE__",
    ...
    config: {
      template: {
        strings: [
          "safePriceEnjoyQuestion_Avast",
          "safePriceEnjoyQuestion_AVG",
          "sasGladYouLike",
          "sasSorry",
          "safePriceLeaveRating",
          "sasAnswerYes",
          "sasAnswerNo",
          "safePriceAskMeLater",
          "sasRate"
  ...
  AvastWRC.Social = {
    ...
    getDisplayInfo: function (data) {
      ...
      return {
        ...
        showInBottomOffers:
          isPowerSocialUser &&
          (data.producstLength >= this.config.displayInfo.minimumOffers ||
            data.accommodationsLength >= this.config.displayInfo.minimumOffers),
    ...
    addInteraction: function () {
      ...
      if (this.config.socialData.interactionsTTL.length < maximumInteractions) {
        ...
        this.config.socialData.interactionsTTL = this.config.socialData.interactionsTTL
          .slice(maximumInteractions - 1)
          .concat(currentDateInSeconds);
      }
    ...
    config: {
      ...
      templateData: {
        strings: ["spSocialShare", "spSocialShareFb", "spSocialSharetttr", "avastAppName"],
        images: {
...
(function (AvastWRC, _) {
  ...
  AvastWRC.UpdateManager = {
        ...
        if (data) {
          ...
          if (self.config.localStorage.value.showNextTime && !excluded) {
            ...
            if (excluded && self.config.localStorage.value.currentVersion !== thisVersion) {
              ...
              console.log(
                "UpdateManager: isMajorVersionChange result ",
                change,
                self.config.localStorage.value.currentVersion,
                thisVersion
              );
    ...
    isMajorVersionChange: function (storageVersion, newVersion) {
      ...
      if (
        parseInt(storageVersionValues[0]) < parseInt(newVersionValues[0]) ||
        parseInt(storageVersionValues[1]) < parseInt(newVersionValues[1])
      ) {
        change.major = true;
    ...
    getUpdatePageLink: function () {
      var link =
        AvastWRC.CONFIG.BRANDING_TYPE === AvastWRC.BRANDING_TYPE_AVAST ? self.config.updatePageLinkAvast : self.config.updatePageLinkAVG;
    ...
    config: {
      ...
      updatePageLinkAvast:
        "https://www.avast.com/lp-safeprice-update-v2?version=__VERSION__&utm_medium=link&utm_source=safeprice&utm_campaign=safeprice-update",
      updatePageLinkAVG:
        "https://www.avg.com/en-ww/lp-safeprice-update-v3?version=__VERSION__&utm_medium=link&utm_source=safeprice&utm_campaign=safeprice-update"
    }
  };
...
(function (AvastWRC, _) {
  ...
  AvastWRC.NotificationsManager = {
    ...
    isDomainInSettingsWhiteList: function (urlDomain) {
      ...
      if (
        poppupSettings &&
        poppupSettings.customList &&
        poppupSettings.customList.whiteList.length > 0 &&
        poppupSettings.customList.whiteList.findIndex(checkList) !== -1
      ) {
        return true;
      }
      return false;
    ...
    isCategoryAvailableForDomain: function (domain, notificationName) {
      ...
      let closedCategoryData = this.config.onlyInMemoryValues.categoriesBlackListedOnDomain[domain] || {
        categoryFlag: 0,
        closedTimestamp: []
      };
      if ((closedCategoryData.categoryFlag & category) === category) {
        let closedTime = closedCategoryData.closedTimestamps[category];
        let closedCategoryTtlIsValid = closedTime
          ? new Date().getTime() - closedTime < AvastWRC.Shepherd.getNotificationsConfig().closeTTL * 1000
          : false;
        if (closedCategoryTtlIsValid) {
          console.log(
            ...
            category,
            " ----NOT AVAILABLE----",
            this.config.onlyInMemoryValues.categoriesBlackListedOnDomain
          );
          return false;
        } else {
          this.config.onlyInMemoryValues.categoriesBlackListedOnDomain[domain].closedTimestamps[category] = null;
          this.config.onlyInMemoryValues.categoriesBlackListedOnDomain[domain].categoryFlag =
            this.config.onlyInMemoryValues.categoriesBlackListedOnDomain[domain].categoryFlag - category;
          console.log(
            "NotificationName: ",
            notificationName,
            "Category: ",
    ...
    isTimeForRedirect: function (domain) {
      let currentTimeInMilliseconds = new Date().getTime(),
        redirectTTLIsValid =
          currentTimeInMilliseconds - this.getTTLForRedirectUrl(domain) >= AvastWRC.Shepherd.getNotificationsConfig().redirectTTL * 1000;
    ...
    cleanRedirectUrlTTl: function () {
      ...
      for (let key in this.config.values.redirectTTL) {
        if (
          currentTimeInMilliseconds - this.config.values.redirectTTL[key] >=
          AvastWRC.Shepherd.getNotificationsConfig().redirectTTL * 1000
        )
          delete this.config.values.redirectTTL[key];
      }
    },
    saveValues: function () {
    ...
    isdisabledForRedirect: function (domain) {
      return (
        !AvastWRC.NotificationsManager.isCategoryAvailableForDomain(domain, "SPECIAL_DEALS") ||
    ...
    isdisabledForOffers: function (domain) {
      return (
        !AvastWRC.NotificationsManager.isCategoryAvailableForDomain(domain, "OFFERS") ||
        AvastWRC.bal.settings.get().userSPPoppupSettings.notifications.offers.hide
      );
    },
    couponHaveBeenShowedInDomain: function (domain) {
      var value =
        AvastWRC.bal.settings.get().userSPPoppupSettings.notifications.coupons.showOnce &&
    ...
    isdisabledForCoupons: function (domain) {
      return (
        !AvastWRC.NotificationsManager.isCategoryAvailableForDomain(domain, "COUPONS") ||
    ...
    isdisabledForOffersCoupons: function (domain) {
      return (
        !AvastWRC.NotificationsManager.isCategoryAvailableForDomain(domain, "OFFERS_AND_COUPONS") ||
        AvastWRC.bal.settings.get().userSPPoppupSettings.notifications.offers.hide ||
        AvastWRC.bal.settings.get().userSPPoppupSettings.notifications.coupons.hide ||
        AvastWRC.NotificationsManager.couponHaveBeenShowedInDomain(domain)
      );
    ...
    isdisabledForCityHotels: function (domain) {
      return (
        !AvastWRC.NotificationsManager.isCategoryAvailableForDomain(domain, "POPULAR_HOTELS") ||
    ...
    isdisabledForSimilarHotels: function (domain) {
      return (
        !AvastWRC.NotificationsManager.isCategoryAvailableForDomain(domain, "ALTERNATIVE_HOTELS") ||
...
(function (AvastWRC, _) {
  ...
  AvastWRC.CloseTooltip = {
    ...
    setBadgeAnimation: function (data) {
      AvastWRC.bal.sp.setBadge(
        data.tabId,
        AvastWRC.bal.sp.getCurrentBadge(data.tabId),
        AvastWRC.bal.sp.currentAsbString[data.tabId],
        true,
        null,
        AvastWRC.Shepherd.getIconBlinkingAnimationAfterCloseTooltip()
      );
    },
    getTemplateData: function () {
      return {
...
(function (AvastWRC, _) {
  ...
  AvastWRC.SettingsTooltip = {
    ...
    isTimeToShow: function (offersNumber) {
      ...
      var now = parseInt(new Date().getTime()) / 1000;
      if (
        AvastWRC.YesNo.powerUserData.interactions &&
        AvastWRC.YesNo.powerUserData.interactions.length === 0 &&
    ...
    AvastWRC.bal.sp = _.extend(AvastWRC.bal.SP || {}, {
      ...
      affiliateIsContainedOnCurrentURL: function (url, urlToBeContained) {
        ...
        if (urlDomain === urlToBeContainedDomain) {
          console.log(
            "affiliateIsContainedOnCurrentURL: couponinTab: affiliate domain (" + urlToBeContained + ") equals to url domain (" + url + ")"
          );
          return true;
      ...
      isCouponInTab: function (url, tab) {
        ...
        if (couponInTabsToShow && couponInTabsToShow.coupons) {
          let _taburl = false,
            _url = false,
            _tabTedirectUrl = false;

          _taburl =
            couponInTabsToShow.toBeShownIn[tab.url] ||
            AvastWRC.bal.sp.affiliateIsContainedOnCurrentURL(tab.url, couponInTabsToShow.affiliateDomain) ||
            AvastWRC.bal.sp.siteNameIsContainedOnCurrentURL(tab.url, couponInTabsToShow.url) ||
            AvastWRC.bal.sp.affiliateIsContainedOnCurrentURL(tab.url, couponInTabsToShow.affiliateName);

          if (url !== tab.url) {
            _url =
              couponInTabsToShow.toBeShownIn[url] ||
              AvastWRC.bal.sp.affiliateIsContainedOnCurrentURL(tab.url, couponInTabsToShow.affiliateDomain) ||
              AvastWRC.bal.sp.siteNameIsContainedOnCurrentURL(tab.url, couponInTabsToShow.url) ||
              AvastWRC.bal.sp.affiliateIsContainedOnCurrentURL(tab.url, couponInTabsToShow.affiliateName);
          }
          ...
          if (tab.redirectUrl && tab.redirectUrl !== "") {
            _tabTedirectUrl =
              couponInTabsToShow.toBeShownIn[tab.redirectUrl] ||
              AvastWRC.bal.sp.affiliateIsContainedOnCurrentURL(tab.redirectUrl, couponInTabsToShow.affiliateDomain) ||
              AvastWRC.bal.sp.siteNameIsContainedOnCurrentURL(tab.redirectUrl, couponInTabsToShow.url) ||
              AvastWRC.bal.sp.affiliateIsContainedOnCurrentURL(tab.redirectUrl, couponInTabsToShow.affiliateName);
          }
      ...
      isShopDomain: function (url, response, tab, updateEvent) {
        ...
        function prepareData(response, url, tab) {
          ...
          data = _.extend(data, {
            ...
            clientInfo:
              cachedData && cachedData.clientInfo ? JSON.parse(JSON.stringify(cachedData.clientInfo)) : AvastWRC.Utils.getClientInfo()
          });
        ...
        if (AvastWRC.bal.sp.isInstallUrl(url)) {
          ...
          if (data.urlData.match) {
            ...
            if (
              (!data.urlData.match && data.urlData.isfakeShop && !data.urlData.isTrustedFakeShop) ||
      ...
      requestOffers: function (data) {
        ...
        var queryOptions = {
          ...
          callback: function (offersResponse) {
            ...
            cachedData.panelData.searchBack.offersSearchQueries = JSON.parse(
              JSON.stringify(AvastWRC.bal.sp.panelData.searchBack.offersSearchQueries)
            );
            ...
            cachedData.panelData.searchBack.couponsSearchQueries = JSON.parse(
              JSON.stringify(AvastWRC.bal.sp.panelData.searchBack.couponsSearchQueries)
            );
            ...
            if (cachedData.detailsToClosed.offerNumber.toString() > 0) {
              ...
              AvastWRC.bal.sp.setBadge(
                tab.id,
                cachedData.detailsToClosed.offerNumber.toString(),
                cachedData.detailsToClosed.asbString,
                cachedData.notifications.notificationName === "blinkIcon"
              );
      ...
      getUserSettings: function () {
        ...
        poppupSettings.strings.spSettingsPageCustomListItemAddSite =
          poppupSettings.strings.spSettingsPageCustomListItemAddSite.toUpperCase();
        ...
        switch (data.type) {
          ...
          case "COUPONS_CLICK":
            ...
            if (eventDetails.offer.isSearch || eventDetails.offer.isContextCoupon || eventDetails.isLinkClick) {
              ...
              console.log(
                "SHOW COUPON CONFIG: this is a search coupon or a context coupon or an appliedCoupon-> config: ",
                couponShowConfig
              );
            ...
            switch (data.action) {
              ...
              case "CLICKED_X":
                ...
                if (data.notificationName === "safeShopPanel" && data.category !== "COUPON_APPLIED_NOTIFICATION") {
                  AvastWRC.NotificationsManager.disableCategoryForDomain(
                    AvastWRC.bal.getDomainFromUrl(data.url),
                    AvastWRC.NotificationsManager.getFlagFromNotificationType(data.category)
                  );
                  if (data.category === "OFFERS") {
                    AvastWRC.NotificationsManager.disableCategoryForDomain(
                      AvastWRC.bal.getDomainFromUrl(data.url),
                      AvastWRC.NotificationsManager.getFlagFromNotificationType("OFFERS_AND_COUPONS")
                    );
                  }
                }
          ...
          case "SEARCH":
            ...
            switch (data.action) {
              ...
              case "EMPTY_SEARCH_REDIRECT":
                ...
                let serverSP =
                  AvastWRC.Query.CONST.SAFESHOP_SERVERS[AvastWRC.CONFIG.serverType] + AvastWRC.Query.CONST.SAFESHOP_ENDPOINTS[4];
      ...
      processSearchOffers: function (_tab, data) {
        ...
        var queryOptions = {
          ...
          query: data.query,
          provider_id:
            cachedData.search && cachedData.search.offersSearch && cachedData.search.offersSearch.providerId
              ? cachedData.search.offersSearch.providerId
              : cachedData.providerId || "",
          clientInfo: JSON.parse(cachedData.clientInfo),
      ...
      processSearchCoupons: function (_tab, data) {
        ...
        var queryOptions = {
          url: data.url,
          provider_id:
            cachedData.search && cachedData.search.couponsSearch && cachedData.search.couponsSearch.providerId
              ? cachedData.search.couponsSearch.providerId
              : cachedData.voucherProviderId || "",
          query: data.query,
      ...
      setBadge: function (tabID, badge, asbString, animation = true, color = null, config = null) {
        ...
        function setBadgeWithAnimation(tabID, badge) {
          ...
          for (let i = 0; i <= animationConfig.times; i++) {
            ...
            AvastWRC.bal.sp.badgeAnimationTimers.push(
              setAnimationTimeout(tabID, "", animationConfig.color, milliseconds + animationConfig.milliseconds)
            );
      ...
      isAnAffiliateDomain: function (tab, sendAffiliateDomainEvent = true, url = false) {
        function emitAFSRCMatchBurgerEvent(tab) {
          ...
          let eventDetails = {
            clientInfo: AvastWRC.Utils.getClientInfo(),
            url: `${eventTriggeredURL + (eventTriggeredURL.indexOf("?") < 0 ? "?" : "")}&${AFFILIATE_MATCHING_KEY}=${
              AvastWRC.ASDetector.isAffiliateSource(tab.id, false) ? "CIUVO" : "COMPRIGO"
            }`,
            offer: null,
            offerType: "",
        ...
        let eventTriggeredURL = url || tab.url,
          isAnAffiliateDomain =
            (AvastWRC.ASDetector && AvastWRC.ASDetector.isAffiliateSource(tab.id, false)) ||
      ...
      disableBadgeAnimation: function (tabId) {
        ...
        if (tabId) {
          AvastWRC.bal.sp.setBadge(tabId, AvastWRC.bal.sp.getCurrentBadge(tabId), AvastWRC.bal.sp.currentAsbString[tabId], false);
        }
      },
