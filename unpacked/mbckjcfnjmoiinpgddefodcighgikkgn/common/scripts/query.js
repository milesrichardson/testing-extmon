(function (_, AvastWRC, PROTO) {
  ...
  AvastWRC.Query.__MASTER__ = {
    ...
    post: function () {
      ...
      if (this.options.headers) this.headers = _.extend({}, this.headers, this.options.headers);
    ...
    parser: function (arrayBuffer) {
      ...
      } else this.response = arrayBuffer;
    },
    /**
  ...
  AvastWRC.Query.SafeShopCommon = {
    ...
    formatCoupons: function (vouchers, res, couponCategory) {
      var openShopString =
        AvastWRC.bs.getLocalizedString("spCouponApplied") +
        `<span class="asp-shopname-span">` +
        AvastWRC.bs.getLocalizedString("sasOpenShop") +
        `</span>`;
        ...
        for (var m = 0, n = vouchers.length; m < n; m++) {
          ...
          if (voucher !== undefined) {
            ...
            vouDetails = {
              ...
              spClickThenPaste: affiliateName
                ? AvastWRC.bs.getLocalizedString("spClickThenPasteSearchResult", [affiliateName])
                : AvastWRC.bs.getLocalizedString("spClickThenPaste"),
              spSearchCouponAppliedOnPage: affiliateName
                ? AvastWRC.bs.getLocalizedString("spSearchCouponAppliedOnPage", [affiliateName])
                : openShopString,
      ...
      if (resp && resp.redirect && resp.redirect.length > 0) {
        for (var m = 0, n = resp.redirect.length; m < n; m++) {
          ...
          if (redirect !== undefined) {
            var redirectImages = AvastWRC.Utils.resolveImageUrl(
              redirect.primary_image || "",
              redirect.secondary_image || "",
        ...
        for (var m = 0, n = resp.product.length; m < n; m++) {
          ...
          if (product !== undefined) {
            var prodDetails = {
              ...
              queryFormatedPrice:
                res.offerQuery && res.offerQuery[product.offer.values_.provider_id]
                  ? res.offerQuery[product.offer.values_.provider_id].formatted_price
                  : ""
            };
        ...
        for (var m = 0, n = resp.accommodation.length; m < n; m++) {
          ...
          if (accommodation !== undefined) {
            ...
            var accomDetails = {
              ...
              queryFormatedPrice:
                res.offerQuery && res.offerQuery[accommodation.offer.values_.provider_id]
                  ? res.offerQuery[accommodation.offer.values_.provider_id].formatted_price
                  : ""
            };
    ...
    updateFakeShopDataOnError: function (urlData, data) {
      if ((urlData.isfakeShop && !urlData.isTrustedFakeShop) || (urlData.isPhishing && !urlData.isTrustedPhishing)) {
  ...
  AvastWRC.Query.SafeShopOffer.prototype = _.extend({}, AvastWRC.Query.__MASTER__, {
    ...
    format: function () {
      ...
      if (resp && resp.notification) {
        res.notifications.notificationType =
          AvastWRC.gpb.All.SafeShopOffer.OfferResponse.Notification.NotificationType[resp.notification.type || 0];
      ...
      res.suppress_x_timeout =
        res.notifications.notificationType === AvastWRC.gpb.All.SafeShopOffer.OfferResponse.Notification.NotificationType[10]
          ? true
          : false;
      ...
      res.offersRequestTotalLength = res.producstLength + res.accommodationsLength + res.couponsLength + res.redirectLength;
