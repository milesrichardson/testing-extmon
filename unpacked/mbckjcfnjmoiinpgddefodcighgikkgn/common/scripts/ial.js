  ...
  AvastWRC.ial = AvastWRC.ial || {
    ...
    injectFonts: function () {
      if ($("#avast_os_ext_custom_font").length === 0) {
        $("head").append(
          `<link id='avast_os_ext_custom_font' href='${AvastWRC.bs.getLocalResourceURL(
    ...
    getTopHtmlElement: function () {
      ...
      if (docElement) {
        ...
        if (docElement && docElement[0]) {
          ...
          docElement = $("html");
          if (docElement.length > -1) {
            //drop this branch if no jQuery
            return docElement;
          } else {
    ...
    addRippleEffect: function (e, buttonClassName, bgColor, useCurrentTarget = false) {
      ...
      if (!bgColor) {
        if (
          (AvastWRC.ial.sp.data && AvastWRC.ial.sp.data.avastBranding) ||
          (AvastWRC.ial.sp.couponInTabData && AvastWRC.ial.sp.couponInTabData.avastBranding)
        ) {
          bgColor = "#087E3A";
        } else {
          bgColor = "#44cc88";
...
(function ($) {
  ...
  $(window).resize(function () {
    ...
    elementsToFixPosition.forEach((el) => {
      ...
      if (element && element[0]) {
        if (element.positionRight().right + $(el).width() > window.innerWidth || element.positionRight().top > window.outerHeight) {
  ...
  AvastWRC.ial.sp = {
    ...
    topBar: function (barHtml, barElementSelector, barHeight, topBarRules) {
      ...
      return {
        ...
        show: function () {
          ...
          AvastWRC.ial.getTopHtmlElement().css("margin-top", function (index, value) {
        ...
        remove: function () {
          ...
          if (_oldHtmlTopMargin) AvastWRC.ial.getTopHtmlElement().css("margin-top", _oldHtmlTopMargin);
          ...
          if (_oldGoogleTopElem !== null) {
            for (let i = 0, j = _oldGoogleTopElem.length; i < j; i++) {
              if (_oldGoogleTopElem[i].computedStyle) {
                ...
                $(_oldGoogleTopElem[i].styleName).css(
                  _oldGoogleTopElem[i].styleProperty,
                  _oldGoogleTopElem[i].oldValue + (_oldGoogleTopElem[i].oldValue === "" ? "" : "px")
                );
              }
            }
          }
    ...
    cleanScraperData: function () {
      ...
      AvastWRC.ial.sp.checkSafeShopDataConst = JSON.parse(
        JSON.stringify({
          data: null,
          completed: false,
          result: [],
    ...
    checkSafeShop: function (data) {
          ...
          provPromises.push(
            new Promise((resolve, reject) => {
              switch (scraper.providerId) {
                ...
                default:
                  resolve({
                    query: { title: document && document.title ? document.title : "", message: "error: no scraper for this provider" },
      ...
      Promise.all(provPromises).then((values) => {
        ...
        values.forEach((v) => {
          transactionId = AvastWRC.ial.sp.checkSafeShopDataConst.data.clientInfo
            ? JSON.parse(AvastWRC.ial.sp.checkSafeShopDataConst.data.clientInfo).transactionId
            : null;
        ...
        if (clientInfoChanged) {
          ...
          console.log(
            "Parser result with new Results",
            AvastWRC.ial.sp.checkSafeShopDataConst.rescan,
            AvastWRC.ial.sp.checkSafeShopDataConst.result
          );
        } else {
          AvastWRC.ial.sp.feedback({
          ...
          console.log(
            "Parser result with Same Results",
            AvastWRC.ial.sp.checkSafeShopDataConst.rescan,
            AvastWRC.ial.sp.checkSafeShopDataConst.result
          );
        }
    ...
    feedback: function (feedbackData) {
      ...
      let contsantValue = {
        ruleId:
          feedbackData && feedbackData.ruleId
            ? feedbackData.ruleId
            : AvastWRC.ial.sp.lastRuleClicked
            ? AvastWRC.ial.sp.lastRuleClicked
            : null,
        uiId: uiId || "",
    ...
    resizeTextElements: function (feedbackId) {
      ...
      if ($(container) && $(container)[0] && $(container)[0].clientHeight > 10 && $(text) && $(text)[0] && $(text)[0].clientHeight > 10) {
        ...
        if (AvastWRC.ial.sp.FEEDBACK_VALUES.lastPanelHeight > panelHeight) {
          var newValue =
            panelHeight -
            AvastWRC.ial.sp.FEEDBACK_VALUES[feedbackId].feedbackTopHeight -
            AvastWRC.ial.sp.FEEDBACK_VALUES.panelFixedElemHeight;
          console.log(
            "resizeTextElements: to newValue",
            newValue,
            containerHeigth,
            textAreaHeigth,
            AvastWRC.ial.sp.FEEDBACK_VALUES.lastPanelHeight,
            panelHeight
          );
          ...
          console.log(
            "resizeTextElements: text elem height and max-height to containersize-10",
            containerHeigth - 10,
            containerHeigth,
            textAreaHeigth
          );
          ...
          console.log(
            "resizeTextElements: text elem max-height to containersize-10",
            containerHeigth - 10,
            containerHeigth,
            textAreaHeigth
          );
    ...
    setSecurityHoverEffect: function (panel = false) {
      ...
      let timer = null;
      if (panel) {
        $(data.panel.securityHoveredId).hover(
          (e) => {
        ...
        $(data.panelMin.securityHoveredId).hover(
          (e) => {
        ...
        $(data.panelMin.sslHoveredId).hover(
          (e) => {
    ...
    createPanel: function () {
      ...
      if ($("#a-panel").length === 0) {
        let position = JSON.parse(JSON.stringify(AvastWRC.ial.sp.PANEL_DEFAULT_POSITION));
        if (
          AvastWRC.ial.sp.data.panelData &&
          AvastWRC.ial.sp.data.panelData.panelPosition &&
          AvastWRC.ial.sp.data.panelData.panelPosition.position
        ) {
          if (
            AvastWRC.ial.sp.data.panelData.panelPosition.position.right < document.body.clientWidth - 326 &&
        ...
        AvastWRC.ial.sp.makeDraggable(document.getElementById("a-panel-header"), document.getElementById("a-panel"), () => {
          AvastWRC.ial.sp.feedback({
            ...
            position:
              $(AvastWRC.ial.sp.notifications.config.notificationsContainer.panel).positionRight() ||
              JSON.parse(JSON.stringify(AvastWRC.ial.sp.PANEL_DEFAULT_POSITION))
          });
        });
        AvastWRC.ial.sp.makeResizeable(
          document.getElementById("panel-resize"),
          document.getElementById("a-panel"),
          AvastWRC.ial.sp.PANEL_MIN_HEIGHT
        );
    ...
    moveExternalPanels: function (size) {
      ...
      function getCurrentNotificationSize() {
        for (let key in AvastWRC.ial.sp.notifications.config.notificationsContainer) {
          if (
            $(AvastWRC.ial.sp.notifications.config.notificationsContainer[key]).length &&
            $(AvastWRC.ial.sp.notifications.config.notificationsContainer[key]).css("display") !== "none"
          ) {
            return (
              document.getElementById(AvastWRC.ial.sp.notifications.config.notificationsContainer[key].replace("#", "")).clientHeight +
              positionOffset
            );
          }
        }
    ...
    updatePanelWithSearch: function (data) {
      ...
      if (AvastWRC.ial.sp.data.search.lastSearch === "OFFERS") {
        ...
        AvastWRC.ial.sp.UpdateContent(
          ".a-sp-items-wrapper",
          true,
          "offersItemsWrapper",
          AvastWRC.Templates.offersItemsWrapper,
          function () {
        ...
        AvastWRC.ial.sp.UpdateContent(
          ...
          true,
          "couponsItemsWrapper",
          AvastWRC.Templates.couponsItemsWrapper,
          function () {
    ...
    bindSettingsTooltipEvents: function () {
      ...
      if (
        tooltip &&
        tooltip.length > 0 &&
        tooltip[0].className.indexOf("asp-hide-tooltip") === -1 &&
        tooltipButton &&
        tooltipButton.length > 0
      ) {
    ...
    BindPanelEvents: function () {
      $("#closePanel").click(function (e) {
        ...
        if (AvastWRC.ial.sp.data.feedbackInfo && AvastWRC.ial.sp.data.feedbackInfo.askForFeedback)
          AvastWRC.ial.sp.userFeedback.showFeedbackQuestion();
      });
    ...
    bindFeedbackEvents: function (ignoreFeedbackTextClick = false) {
      ...
      function feedbackShareEvents() {
        ...
        $(feedbackWhiteContainerGeneral).click(() => {
          ...
          AvastWRC.ial.sp.FEEDBACK_VALUES["#general"].feedbackTopHeight =
            $(".asp-general-container-title") && $(".asp-general-container-title")[0] && $(".asp-general-container-title")[0].clientHeight
              ? $(".asp-general-container-title")[0].clientHeight + 44
              : 0;
        ...
        $(buttonGeneralSend).click((e) => {
          ...
          if ($(".a-sp-rated-thanks-icon").length > 0) {
            ...
            $(".a-sp-rated-thanks-icon")[0].src =
              AvastWRC.ial.sp.data.panelData.images.checkGif.replace(/\?.*$/, "") + "?x=" + Math.random();
          }
        ...
        $(buttonShop1Next).click(() => {
          ...
          AvastWRC.ial.sp.FEEDBACK_VALUES["#shop2"].feedbackTopHeight =
            $(".asp-shop2-top") && $(".asp-shop2-top")[0] && $(".asp-shop2-top")[0].clientHeight ? $(".asp-shop2-top")[0].clientHeight : 0;
        ...
        $(buttonShop2Send).mousedown((e) => {
          if (
            (!$(radioButtonShop2SomethingDiff)[0].checked &&
              !$(radioButtonShop2PaidNotGet)[0].checked &&
              !$(radioButtonShop2Other)[0].checked) ||
        ...
        function sendFeedbackEvent(feedbackType) {
          ...
          if (feedbackType === feedbackGeneral) {
            ...
            reportReason = $(radioButtonShop2SomethingDiff)[0].checked
              ? "ORDERED_SOMETHING_DIFFERENT"
              : $(radioButtonShop2PaidNotGet)[0].checked
              ? "PAID_DID_NOT_GET"
              : $(radioButtonShop2Other)[0].checked
              ? "OTHER"
    ...
    toggleLoadingSearch: function (type, stop) {
      ...
      if (type === AvastWRC.ial.sp.searchTemplateClasses.offers.feedbackAction && !stop) {
        ...
        $(AvastWRC.ial.sp.searchTemplateClasses.offers.loadingicon).css(
          "background",
          "-webkit-radial-gradient(50% 45%,circle, #fff0 58%, #45485178 66%, #2c2f344f 66%, #9199ab 10%) no-repeat"
        );
        $(AvastWRC.ial.sp.searchTemplateClasses.offers.loadingicon).css(
          "background",
          "-moz-radial-gradient(50% 45%,circle, #fff0 58%, #45485178 66%, #2c2f344f 66%, #9199ab 10%) no-repeat"
        );
        $(AvastWRC.ial.sp.searchTemplateClasses.offers.loadingicon).css(
          "background",
          "-o-radial-gradient(50% 45%,circle, #fff0 58%, #45485178 66%, #2c2f344f 66%, #9199ab 10%) no-repeat"
        );
      }
      ...
      if (type === AvastWRC.ial.sp.searchTemplateClasses.coupons.feedbackAction && !stop) {
        ...
        $(AvastWRC.ial.sp.searchTemplateClasses.coupons.loadingicon).css(
          "background",
          "-webkit-radial-gradient(50% 45%,circle, #fff0 58%, #45485178 66%, #2c2f344f 66%, #9199ab 10%) no-repeat"
        );
        $(AvastWRC.ial.sp.searchTemplateClasses.coupons.loadingicon).css(
    ...
    BindSearchEvents: function () {
      ...
      $(AvastWRC.ial.sp.searchTemplateClasses.offers.expanded.backIcon).click(function (e) {
        ...
        var query =
          AvastWRC.ial.sp.data.panelData.searchBack.offersSearchQueries[AvastWRC.ial.sp.data.panelData.searchBack.lastOffersSearchPos - 1];
        if (query !== "") {
      ...
      $(AvastWRC.ial.sp.searchTemplateClasses.offers.collapsed.icon).hover(
        (e) => {
      ...
      $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.input).keypress(function (e) {
        if (searchCouponsFirstChar) {
          ...
          $(AvastWRC.ial.sp.searchTemplateClasses.coupons.onInstallTooltip).addClass(
            AvastWRC.ial.sp.searchTemplateClasses.hideTooltipClass
          );
        }
        var key = e.which;
        if (key === 13) {
      ...
      $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.backIcon).click(function (e) {
        ...
        var query =
          AvastWRC.ial.sp.data.panelData.searchBack.couponsSearchQueries[
            AvastWRC.ial.sp.data.panelData.searchBack.lastCouponsSearchPos - 1
          ];
        if (query !== "") {
      ...
      $(AvastWRC.ial.sp.searchTemplateClasses.coupons.collapsed.icon).hover(
        (e) => {
      ...
      function isOffersOrCouponsRequestQuery(query, action = null) {
        if (!action) return;
        if (
          action === AvastWRC.ial.sp.searchTemplateClasses.offers.feedbackAction &&
          AvastWRC.ial.sp.data.panelData.strings.searchTitleOffersRequest === query
        ) {
          ...
          action === AvastWRC.ial.sp.searchTemplateClasses.coupons.feedbackAction &&
          AvastWRC.ial.sp.data.panelData.strings.searchTitleCouponsRequest === query
        ) {
      ...
      function search(query, action = null, increasePos = true) {
        ...
        if (action === AvastWRC.ial.sp.searchTemplateClasses.offers.feedbackAction) {
          searchOffersFirstCLick = true;
          defaultSearchBoxStrings = [data.panelData.strings.spProductsSearchInput, data.panelData.strings.searchTitleOffers];
          ...
          if (increasePos) {
            ...
            AvastWRC.ial.sp.data.panelData.searchBack.lastOffersSearchPos =
              AvastWRC.ial.sp.data.panelData.searchBack.offersSearchQueries.length - 1;
          } else {
            AvastWRC.ial.sp.data.panelData.searchBack.lastOffersSearchPos--;
            if (AvastWRC.ial.sp.data.panelData.searchBack.lastOffersSearchPos === 0) {
              AvastWRC.ial.sp.data.panelData.searchBack.offersSearchQueries.splice(1);
            } else {
              AvastWRC.ial.sp.data.panelData.searchBack.offersSearchQueries.splice(
                AvastWRC.ial.sp.data.panelData.searchBack.lastOffersSearchPos
              );
            }
          }
          ...
          searchCouponsFirstCLick = true;
          defaultSearchBoxStrings = [data.panelData.strings.spCouponsSearchInput, data.panelData.strings.searchTitleCoupons];
          ...
          if (increasePos) {
            ...
            AvastWRC.ial.sp.data.panelData.searchBack.lastCouponsSearchPos =
              AvastWRC.ial.sp.data.panelData.searchBack.couponsSearchQueries.length - 1;
          } else {
            AvastWRC.ial.sp.data.panelData.searchBack.lastCouponsSearchPos--;
            if (AvastWRC.ial.sp.data.panelData.searchBack.lastCouponsSearchPos === 0) {
              AvastWRC.ial.sp.data.panelData.searchBack.couponsSearchQueries.splice(1);
            } else {
              AvastWRC.ial.sp.data.panelData.searchBack.couponsSearchQueries.splice(
                AvastWRC.ial.sp.data.panelData.searchBack.lastCouponsSearchPos
              );
            }
          }
      ...
      function expandSearch(type) {
        if (type === AvastWRC.ial.sp.searchTemplateClasses.offers.feedbackAction) {
          ...
          $(AvastWRC.ial.sp.searchTemplateClasses.offers.expanded.containerWithBack).toggleClass(
            AvastWRC.ial.sp.searchTemplateClasses.toggleClass
          );
          $(AvastWRC.ial.sp.searchTemplateClasses.offers.collapsed.container).toggleClass(
            AvastWRC.ial.sp.searchTemplateClasses.toggleClass
          );
          $(AvastWRC.ial.sp.searchTemplateClasses.offers.expanded.input).select();
        } else if (type === AvastWRC.ial.sp.searchTemplateClasses.coupons.feedbackAction) {
          $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.container).toggleClass(
            AvastWRC.ial.sp.searchTemplateClasses.toggleClass
          );
          $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.containerWithBack).toggleClass(
            AvastWRC.ial.sp.searchTemplateClasses.toggleClass
          );
          $(AvastWRC.ial.sp.searchTemplateClasses.coupons.collapsed.container).toggleClass(
            AvastWRC.ial.sp.searchTemplateClasses.toggleClass
          );
          $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.input).select();
        }
      ...
      function sendRedirectInEmptySearchClick(type) {
        ...
        if (type === AvastWRC.ial.sp.searchTemplateClasses.offers.feedbackAction) {
          ...
          value = $(AvastWRC.ial.sp.searchTemplateClasses.offers.expanded.input)[0]
            ? $(AvastWRC.ial.sp.searchTemplateClasses.offers.expanded.input)[0].value
            : "";
          ...
          offerCategory = "REDIRECT";
          value = $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.input)[0]
            ? $(AvastWRC.ial.sp.searchTemplateClasses.coupons.expanded.input)[0].value
            : "";
          querySearch = AvastWRC.ial.sp.clearInput(value);
        }
    ...
    BindCouponEvents: function () {
      let couponsSelector = ".asp-coupon-item",
        openShopSelector = ".asp-shopname-span",
      ...
      $(
        `${couponsSelector}, ${openShopSelector}, ${couponBottomWithCodeSelector}, ${couponBottomWithCodeSelector}, ${rateCouponPositive}, ${rateCouponNegative}`
      ).off();
    ...
    findAndUpdateRatedCouponInList: function (ratedPositive, e) {
      var data = {
        isSearch:
          e.currentTarget.attributes && e.currentTarget.attributes.isSearch && e.currentTarget.attributes.isSearch.value
            ? parseInt(e.currentTarget.attributes.isSearch.value)
            : false,
        url:
          e.currentTarget.attributes && e.currentTarget.attributes.resurl && e.currentTarget.attributes.resurl.value
            ? e.currentTarget.attributes.resurl.value
            : "",
        coupon: {}
      };
    ...
    activateShowMoreTextDivs: function (activeCoupons = null) {
      ...
      function showMore(e) {
        ...
        findAndReplace(
          ...
          "#couponsOneLineDesc",
          "asp-coupon-description-text asp-description-hidden"
        );
        AvastWRC.ial.sp.activateShowMoreTextDivs();
      }
      ...
      function showLess(e) {
        ...
        findAndReplace(
          e.currentTarget.parentElement.parentElement,
          "#couponsOneLineDesc",
          "asp-coupon-description-text  asp-more-info-to-show"
        );
        AvastWRC.ial.sp.activateShowMoreTextDivs();
      }
    },
    ...
    SetActiveCoupon: function (e, notificationType) {
      ...
      let isSearch =
        e.currentTarget.attributes.isSearch && e.currentTarget.attributes.isSearch.value
          ? parseInt(e.currentTarget.attributes.isSearch.value)
          : false;
      if (isSearch) {
        if (AvastWRC.ial.sp.ModifyInList(AvastWRC.ial.sp.data.search.couponsSearch.coupons, couponUrl, "selected", true)) {
          AvastWRC.ial.sp.data.search.couponsSearch.vouchersSelectedCounter =
            AvastWRC.ial.sp.data.search.couponsSearch.vouchersSelectedCounter + 1;
          ...
          AvastWRC.ial.sp.data.search.couponsSearch.vouchersAvailable =
            AvastWRC.ial.sp.data.search.couponsSearch.couponsLength - AvastWRC.ial.sp.data.search.couponsSearch.vouchersSelectedCounter > 0;
        }
      } else {
    ...
    addActiveCoupon: function (e, couponUrl, notificationType) {
      let isSearch =
        e.currentTarget.attributes.isSearch && e.currentTarget.attributes.isSearch.value
          ? parseInt(e.currentTarget.attributes.isSearch.value)
          : false;
      $(`#coupon[resurl="${couponUrl}"]`).replaceWith(
        Mustache.render(AvastWRC.Templates.selectedCoupon, this.getClickedCouponTemplateData(couponUrl, isSearch))
      );
      AvastWRC.ial.sp.BindCouponEvents();
    ...
    setCouponsSeparatorVisibility: function (isSearch) {
      ...
      if (onlyFirst) {
        $(AvastWRC.ial.sp.commonSelectors.secondCouponsSeparators).css("opacity", 0);
        $(`${AvastWRC.ial.sp.commonSelectors.firstCouponsSeparator}`).animate(
          { opacity: 1 },
          {
        ...
        $(`${AvastWRC.ial.sp.commonSelectors.firstCouponsSeparator}, ${AvastWRC.ial.sp.commonSelectors.secondCouponsSeparators}`).animate(
          { opacity: 1 },
          {
            duration: AvastWRC.ial.sp.data.vouchersSelectedCounter !== 1 ? 0 : 460,
            queue: false
    ...
    MaxPanel: function () {
      ...
      $(AvastWRC.ial.sp.notifications.config.notificationsContainer.minimizedCircle).removeClass(
        "a-sp-notifications-appear-pulse-animation"
      );
    ...
    OfferClick: function (e, uiSource = "UNKNOWN") {
      ...
      var isSearch =
        e.currentTarget && e.currentTarget.attributes.isSearch && e.currentTarget.attributes.isSearch.value
          ? parseInt(e.currentTarget.attributes.isSearch.value)
          : false;
      var offers = [];
      var offer = null;
    ...
    CouponClick: function (e, uiSource = "UNKNOWN") {
      ...
      if (e && !e.currentTarget) {
        ...
        e.currentTarget.offsetParent.attributes &&
        (e.currentTarget.offsetParent.attributes.resurl || e.currentTarget.offsetParent.attributes.isSearch)
      ) {
    ...
    showPanel: function (data) {
      ...
      if (!AvastWRC.ial.sp.couponInTabData || AvastWRC.ial.sp.couponInTabData.couponClosed) {
        ...
        AvastWRC.ial.sp.FEEDBACK_VALUES.lastPanelHeight =
          $("#a-panel") && $("#a-panel")[0] && $("#a-panel")[0].clientHeight ? $("#a-panel")[0].clientHeight : 442;
        ...
        if (
          (!AvastWRC.ial.sp.data.urlData || !AvastWRC.ial.sp.data.urlData.match || AvastWRC.ial.sp.data.transactionFinished) &&
    ...
    applyCouponInTab: function (data) {
      ...
      if (domCounter > 150) {
        ...
        AvastWRC.ial.sp.applyCouponInTabTimeOut = setTimeout(
          (_data, _locationUrl) => {
    ...
    initMinimizedNotification: function (isCLickInPanel, data) {
      if (!AvastWRC.ial.sp.couponInTabData || AvastWRC.ial.sp.couponInTabData.couponClosed) {
        ...
        if (AvastWRC.ial.sp.data.panelShown && !isCLickInPanel) {
          if (
            AvastWRC.ial.sp.data.notifications.notificationName === "safeShopMinimizedPanel" &&
            !AvastWRC.ial.sp.data.minimizedNotificationShown
          ) {
      ...
      function showNotification() {
        ...
        if (AvastWRC.ial.sp.data.panelData.minimizedPosition && AvastWRC.ial.sp.data.panelData.minimizedPosition.position) {
          if (
            AvastWRC.ial.sp.data.panelData.minimizedPosition.position.right < document.body.clientWidth - 326 &&
    ...
    showPanelMinNotifications: function (data) {
      if (!AvastWRC.ial.sp.couponInTabData || AvastWRC.ial.sp.couponInTabData.couponClosed) {
        ...
        if (AvastWRC.ial.sp.data.panelData.standardPosition && AvastWRC.ial.sp.data.panelData.standardPosition.position) {
          if (
            AvastWRC.ial.sp.data.panelData.standardPosition.position.right < document.body.clientWidth - 326 &&
        ...
        AvastWRC.ial.sp.makeDraggable(
          document.getElementById("a-panel-min-header"),
          document.getElementById("asp-panel-min-notification"),
          () => {
            AvastWRC.ial.sp.feedback({
              type: "UPDATE_POSITION",
              notificationType: "standard",
              position:
                $("#asp-panel-min-notification").positionRight() || JSON.parse(JSON.stringify(AvastWRC.ial.sp.PANEL_DEFAULT_POSITION))
            });
          }
        );
        ...
        $(".asp-header-close").click((e) => {
          AvastWRC.ial.sp.removeNotifications();
          if (e.target.id === "a-panel-min-close" && AvastWRC.ial.sp.data.feedbackInfo && AvastWRC.ial.sp.data.feedbackInfo.askForFeedback)
            AvastWRC.ial.sp.userFeedback.showFeedbackQuestion();
      ...
      if (!AvastWRC.ial.sp.couponInTabData || AvastWRC.ial.sp.couponInTabData.couponClosed) {
        ...
        $(notificationsButton).click((e) => {
          ...
          let url =
            ...
            e.currentTarget.attributes.resurl &&
            e.currentTarget.attributes.resurl.value
              ? e.currentTarget.attributes.resurl.value
              : null;
          if (url) {
    ...
    initDeepIntegration: function (data) {
      ...
      function applyRule(rule, targetUrl) {
        ...
        if (rule.search.length > 0) {
          // if search strings are defined, we must match them before applying the rule
    ...
    userFeedback: {
      ...
      config: {
        buttonsWithRippleEffect: [
          ".a-sp-feedback-question-buttons-yes",
          ".a-sp-feedback-question-buttons-no",
          ".a-sp-feedback-action-button-content"
        ],
    ...
    socialSharing: {
      ...
      registerEvents: function () {
        ...
        function changeSocialIconsOnHover() {
          $(AvastWRC.ial.sp.socialSharing.config.actionElements.tttr.button)
            .mouseenter(function () {
          ...
          $(AvastWRC.ial.sp.socialSharing.config.actionElements.fb.button)
            .mouseenter(function () {
      ...
      sendFeedback: function (data) {
        AvastWRC.ial.sp.feedback({
          ...
          category: data.category
            ? data.category
            : AvastWRC.ial.sp.socialSharing.config.isOnTop
            ? AvastWRC.ial.sp.socialSharing.config.events.categories.top
            : AvastWRC.ial.sp.socialSharing.config.events.categories.bottom,
      ...
      moveCardToTop: function () {
        ...
        function showOnTop() {
          let backgroundClass = "a-sp-social-top",
            closeIcon = "#couponsWrapper .a-sp-social-close-img",
  ...
  AvastWRC.ial.registerEvents(function (ee) {
    ee.on("message.removeRootTemplateAndCleanData", AvastWRC.ial.sp.removeRootTemplateAndCleanData.bind(AvastWRC.ial.sp));
    ee.on("message.removeAll", AvastWRC.ial.sp.removeAll.bind(AvastWRC.ial.sp));
    ee.on("message.checkSafeShop", AvastWRC.ial.sp.checkSafeShop.bind(AvastWRC.ial.sp));
    //notifications
    ee.on("message.none", AvastWRC.ial.sp.showNoneNotifications.bind(AvastWRC.ial.sp));
    ee.on("message.updateData", AvastWRC.ial.sp.updateData.bind(AvastWRC.ial.sp));
    ee.on("message.fakeShopPanel", AvastWRC.ial.sp.createFakeShopNotification.bind(AvastWRC.ial.sp));
    ee.on("message.safeShopPanel", AvastWRC.ial.sp.showPanel.bind(AvastWRC.ial.sp));
    ee.on("message.safeShopMinimizedPanel", AvastWRC.ial.sp.initMinimizedNotification.bind(AvastWRC.ial.sp, false));
    ee.on("message.safeShopPanelMinNotification", AvastWRC.ial.sp.showPanelMinNotifications.bind(AvastWRC.ial.sp));
    ee.on("message.notificationBar", AvastWRC.ial.sp.showBarNotification.bind(AvastWRC.ial.sp));
    ee.on("message.di", AvastWRC.ial.sp.initDeepIntegration.bind(AvastWRC.ial.sp));
    //other events
    ee.on("message.extensionIconClicked", AvastWRC.ial.sp.extensionIconClicked.bind(AvastWRC.ial.sp));
    ee.on("message.applyCouponInTab", AvastWRC.ial.sp.applyCouponInTab.bind(AvastWRC.ial.sp));
    ee.on("message.updatePanelWithSearch", AvastWRC.ial.sp.updatePanelWithSearch.bind(AvastWRC.ial.sp));
    ee.on("message.closeTooltipShown", AvastWRC.ial.sp.closeTooltip.disable);
    ee.on("message.showPanelInInstallPage", AvastWRC.ial.sp.showPanelInInstallPage.bind(AvastWRC.ial.sp));
    ee.on("message.hideSettingsTooltip", AvastWRC.ial.sp.hideSettingsTooltip.bind(AvastWRC.ial.sp));
    ee.on("message.cleanScraperData", AvastWRC.ial.sp.cleanScraperData.bind(AvastWRC.ial.sp));
  });
}).call(this, $);
