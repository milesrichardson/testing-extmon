(() => {
  var __webpack_modules__ = {
      651: () => {
        (() => {
          if (!self.Promise.prototype.finally)
            self.Promise.prototype.finally = function (onFinally) {
              return this.then(onFinally, onFinally), this;
            };
        })();
      }
    },
  ...
  (() => {
    ...
    const createElement = (preObject, classModification = (className) => className) => {
        let object = ((preObject) => Object.assign({}, preObject, { type: "string" == typeof preObject.tag ? "element" : "node" }))(
          preObject
        );
        if ("node" === object.type) {
          if ("string" != typeof object.text) throw new Error("createElement called without text property");
        ...
        if (attributes)
          for (const [attribute, value] of Object.entries(attributes)) {
            if (!(!possibleAttributes.includes(attribute) && !attribute.startsWith("data-")))
              if ("src" !== attribute || "img" === tag) element.setAttribute(attribute, value);
          }
      ...
      modifierOptions = { offset: 1 + Math.floor(Math.random() * (minClassLength - 1)), value: Math.floor(999999 * Math.random()) },
      modifier = (className) => {
      ...
      css = originalCss
        ...
        .map((part) => {
          ...
          return (
            ...
            (selectors = selectors
              .split(",")
              .map((selector) => selector.trim().replace(/\.([_a-zA-Z]+)/g, (match, className) => "." + modifier(className)))
              .join(",")),
            selectors + rules
          );
        })
    ...
    (async () => {
      let { html, promotionId, size } = await new Promise(async (resolve) => {
        const data = await sendMessage({ language, type: "notification:request:initial", url: location.href });
        if (null != data && data.show) resolve(data);
      });
      ((css) => {
        let style = document.createElement("style");
        (style.type = "text/css"), style.appendChild(document.createTextNode(css)), document.head.appendChild(style);
      })(css);
      let elements = (() => {
        let elements = {},
          structure = {
            tag: "div",
            class: "_Notification " + ("small" === size ? "_Notification_stSmall" : "_Notification_stBig"),
            children: [
              {
                tag: "div",
      ...
      const setVisibility = (visible) => {
        if ((sendMessage({ type: "notification:request:setVisibility", visible }), visible))
      ...
      elements.makeBig.addEventListener("click", () => {
        ...
        elements.parent.querySelectorAll("a").forEach((link) => {
          link.addEventListener("click", async (event) => {
            event.preventDefault(),
              sendMessage({ promotionId, type: "notification:request:click", url: location.href }),
        ...
        setTimeout(() => {
          if ((document.body.append(elements.parent), "big" !== size)) return;
          const rect = elements.parent.getClientRects()[0];
          if (rect.width && rect.height) sendMessage({ promotionId, type: "notification:request:popupShow", url: location.href });
        }, 1e4);
      const ping = async () => {
        try {
          await new Promise((resolve, reject) => {
            try {
              sendMessage({ type: "notification:ping" }).then((state) => {
                if ("ok" === state) resolve();
              }),
                setTimeout(() => {
