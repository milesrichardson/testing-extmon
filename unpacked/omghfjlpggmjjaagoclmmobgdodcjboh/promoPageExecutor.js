  ...
  (__webpack_require__.n = (module) => {
    ...
    (() => {
      ...
      var config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4019),
        config__WEBPACK_IMPORTED_MODULE_0___default = __webpack_require__.n(config__WEBPACK_IMPORTED_MODULE_0__);
      const configExperiments = new Map();
      for (const [name, { id, taskNumber }] of Object.entries(config__WEBPACK_IMPORTED_MODULE_0___default().experiments))
        configExperiments.set(id, { name, taskNumber });
      const sendMessage = (message) => {
        ...
        informationPromise = sendMessage({ type: "promo page: get information", url: location.href });
      (async () => {
        const [{ daysAfterInstall, experiments, internalExperiments, promotion }] = await Promise.all([informationPromise, domLoaded]),
          expvarid = [];
        for (const [id, variant] of Object.entries(experiments)) {
          if (!configExperiments.has(id)) continue;
          const { taskNumber } = configExperiments.get(id);
          expvarid.push(`exp${taskNumber}_${variant}`);
        }
        for (const [id, variant] of Object.entries(internalExperiments)) expvarid.push(`${id}_${String(variant)}`);
        const links = document.querySelectorAll('a[href*="/orders/new"]');
        for (const link of links) {
          const url = link.href,
            urlObject = new URL(url);
          if ((urlObject.searchParams.set("instd", String(daysAfterInstall)), expvarid.length))
            urlObject.searchParams.set("expvarid", expvarid.join(","));
          (link.href = urlObject.toString()),
            link.addEventListener("click", () => {
              sendMessage({ type: "promo page: click", promotion });
            });
        }
      })();
    })();
