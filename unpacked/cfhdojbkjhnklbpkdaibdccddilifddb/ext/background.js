...
// to avoid unexpected interference with code in other files
{
    ...
    (details) => {
      ...
      for (let header of details.responseHeaders) {
        ...
        if (headerName == "content-disposition") {
          ...
          if (disposition.toLowerCase() != "inline" && /^[\x21-\x7E]+$/.test(disposition) && !/[()<>@,;:\\"/[\]?={}]/.test(disposition))
            return;
        }
  ...
  browser.webNavigation.onCommitted.addListener((details) => {
    ...
    if (!frame || frame.url.href != url) updatePageFrameStructure(frameId, tabId, url, parentFrameId);
  });
  ...
  browser.webRequest.onBeforeRequest.addListener(
    (details) => {
      ...
      if (frameId > 0 && !ext.getFrame(tabId, frameId)) updatePageFrameStructure(frameId, tabId, "about:blank", parentFrameId);
    },
    {
      types: Object.values(browser.webRequest.ResourceType).filter((type) => type != "main_frame" && type != "sub_frame"),
      urls: ["<all_urls>"]
    }
  );
      ...
      if (details && details.length > 0) {
        ...
        for (let detail of details) {
          ...
          if (detail.parentFrameId > -1) {
            if (detail.frameId != detail.parentFrameId) frame.parent = frames.get(detail.parentFrameId);

            if (!frame.parent && detail.frameId != 0 && detail.parentFrameId != 0) frame.parent = frames.get(0);
          }
        }
      }
    }
