...
*/
  ...
  (() => {
    ...
    /* harmony default export */ const front = front_api; // CONCATENATED MODULE: ./js/i18n.mjs
    ...
    // EXTERNAL MODULE: ./node_modules/css-loader/dist/cjs.js??ruleSet[1].rules[1].use[1]!./css/pages/devtools-panel.css
    var devtools_panel = __webpack_require__(146); // CONCATENATED MODULE: ./css/pages/devtools-panel.css
    var options = {};
    ...
    /* harmony default export */ const pages_devtools_panel =
      devtools_panel /* default */.Z && devtools_panel /* default.locals */.Z.locals
        ? devtools_panel /* default.locals */.Z.locals
        : undefined; // CONCATENATED MODULE: ./js/pages/devtools-panel/index.mjs
    ...
    browser.runtime.sendMessage({ type: "types.get" }).then((filterTypes) => {
      ...
      for (const type of filterTypes) {
        filterStyleElem.innerHTML += `#items[data-filter-type=${type}] tr:not([data-type=${type}])` + "{display: none;}";
    ...
    function createRow(request, filter, options = {}) {
      ...
      if (filter) {
        ...
        if (filter.subscription) originElement.textContent = filter.subscription;
          ...
          if (filter.userDefined) originElement.textContent = getMessage("devtools_filter_origin_custom");
        ...
        if (!filter.allowlisted && request.type != "ELEMHIDE" && request.type != "SNIPPET" && !hasChanged) {
          actionWrapper.appendChild(
            createActionButton("add", "devtools_action_unblock", generateFilter(request, { allowlisted: true }), onFilterChanged)
          );
        }
      ...
      if (lastFilterQuery && shouldFilterRow(row, lastFilterQuery)) row.classList.add("filtered-by-search");
    ...
    document.addEventListener(
      ...
      () => {
        ...
        front.requests.listen(["hits", "reset"], browser.devtools.inspectedWindow.tabId);
