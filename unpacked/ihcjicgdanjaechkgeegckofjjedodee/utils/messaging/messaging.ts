...
export async function sendBackgroundMessage<T, U>(message: BackgroundMessageRequest<T>): Promise<BackgroundMessageResponse<U>> {
  console.debug(`MESSAGING: Sending message to script: ${message.type}`);
  return new Promise<BackgroundMessageResponse<U>>((resolve) => {
    chrome.runtime.sendMessage({ type: message.type, payload: message.payload }, function (response: unknown) {
