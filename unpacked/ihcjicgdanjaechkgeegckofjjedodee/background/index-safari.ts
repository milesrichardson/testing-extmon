...
import { clearLoggedUrls, exportLogs, hijackConsole, loggingReset } from "./logging";
import type { BackgroundMessageRequest, BackgroundMessageResponse } from "@/utils/messaging/messaging";
import { blockPreExistingTabs, msgTabDataGet, msgTabIdGet, msgTabInfoGet } from "./tabs";
import {
  MSG_ADD_ALLOW,
...
import { SETTING_SKIMMER_PROTECTION, SETTING_TELEMETRY_CONSENT, SETTING_VERBOSE_LOGGING, SettingKey } from "@/domain/types/settings";
...
async function init() {
  ...
  await SettingsStore.getInstance().setSetting(SETTING_TELEMETRY_CONSENT, true);
...
chrome.runtime.onMessage.addListener(
  (request: any, sender: chrome.runtime.MessageSender, sendResponse: (response?: BackgroundMessageResponse<unknown>) => void) => {
    ...
    switch (message.type) {
      ...
      case MSG_DETECTION:
        sendResponse({
          payload: {
            detect: (message.payload as DetectionRequest).type === EXCLUSION_SCAMS
...
async function msgLoadInlineScript(
  request: BackgroundMessageRequest<LoadInlineScriptRequest>,
...
async function msgIsAdProtectionActive(
  ...
  sendResponse: (response?: BackgroundMessageResponse<IsAdProtectionActiveResponse>) => void
) {
  ...
  const results = await Promise.all(domains.map((domain) => getExclusionService().isDomainExcluded(domain, EXCLUSION_ADS)));
...
async function msgAddAllowTemporary(
  ...
  sendResponse: (response?: BackgroundMessageResponse<AddAllowTemporaryResponse>) => void
) {
  if (request.payload!.domain) {
    await getExclusionService().excludeTemporarily(request.payload!.domain, request.payload!.tabId);
    console.log("Temporary exclusion set up for " + request.payload!.domain + " (tab " + request.payload!.tabId + " initiated this)");
...
async function msgAddAllow(
  ...
  sendResponse: (response?: BackgroundMessageResponse<AddAllowResponse>) => void
) {
  const exclusions = request.payload!.exclusions;
  const exclusionService = getExclusionService();
  const ok = await exclusionService.exclude(request.payload!.domain, exclusions);
  if (ok) {
    sendResponse({ payload: { success: true } });
...
async function msgRemAllowSingle(
  ...
  sendResponse: (response?: BackgroundMessageResponse<RemAllowSingleResponse>) => void
) {
  if (!request.payload) {
  ...
  try {
    const ok = await getExclusionService().removeExclude(request.payload.domain, request.payload.exclusions, request.payload!.override!);
    clearLoggedUrls();
...
async function msgIsExcluded(
  ...
  sendResponse: (response?: BackgroundMessageResponse<IsExcludedResponse>) => void
) {
  ...
  if (urlOrDomain && tabId) {
    ...
    const isExcludedTemporarily = await getExclusionService().isTemporarilyExcluded(urlOrDomain, tabId);
    ...
    const protectionStatus = await getProtectionService().getProtectionStatus();
    const isExcluded = await getExclusionService().isDomainExcluded(urlHost(urlOrDomain), EXCLUSION_SCAMS);
    const scamsProtectionEnabled = protectionStatus.get(EXCLUSION_SCAMS) ?? false;
    const response = {
...
async function msgTelemetryPhishingTunnel(
  ...
  const sendTelemetry = await SettingsStore.getInstance().getSetting(SETTING_TELEMETRY_CONSENT);
  if (sendTelemetry !== true) {
    console.debug("BKG: msgTelemetryPhishingTunnel: telemetry not consented");
...
async function msgDownloadLogs(
  ...
  sendResponse: (response?: BackgroundMessageResponse<DownloadLogsResponse>) => void
) {
  try {
...
async function msgSettingsGet(
  ...
  sendResponse: (response?: BackgroundMessageResponse<GetSettingResponse>) => void
) {
  if (isSafari && request.payload!.key === SETTING_SKIMMER_PROTECTION) {
    const protectionStatus = await getProtectionService().getProtectionStatus();
    const skimmerProtectionEnabled = protectionStatus.get(SETTING_SKIMMER_PROTECTION) ?? false;
