...
import { EXCLUSION_INDIVIDUAL, SAFE_CACHE, TAB_MALWARE_NOTIFIED, clearSafeCache } from "./definitions";
...
chrome.tabs.onReplaced.addListener((tabIdAdded, tabIdRemoved) => {
  if (!TABS[tabIdAdded]) {
    console.debug("Chrome has swapped tab " + tabIdRemoved + " for " + tabIdAdded + " => Replacing in namespace");
    TABS[tabIdAdded] = TABS[tabIdRemoved];
    TABS[tabIdAdded].tabId = tabIdAdded;
    TABS[tabIdRemoved] = null;
  } else {
    console.debug("Chrome has swapped tab " + tabIdRemoved + " for " + tabIdAdded + " => Removing unused tab");
    TABS[tabIdRemoved] = null;
  }
});
...
export function msgTabDataGet(sendResponse: (response?: BackgroundMessageResponse<unknown>) => void, tabId: number) {
  ...
  try {
    ...
    if (tab) {
      ...
      sendResponse({
        error: "Background replied: No tab data defined for: " + tabId + "!"
      });
    }
  } catch (e) {
    sendResponse({
...
function getTabsQueryAPI() {
  return new Promise((res, rej) => {
    ...
    while (tries < maxTries) {
      console.info("getTabsQueryAPI: chrome.tabs not available. Attempting to get it", { tries });
      tries++;
      if (chrome && chrome.tabs) {
        console.info("getTabsQueryAPI: chrome.tabs available. Returning it");
        return res(chrome.tabs.query);
      }
      // sleep
      sleep(300);
...
export function msgTabIdGet(sendResponse: (response?: BackgroundMessageResponse<GetTabIdResponse>) => void) {
  getTabsQueryAPI()
    .then((tabsApi) => {
...
export function msgTabInfoGet(sendResponse: (response: BackgroundMessageResponse<GetTabInfoResponse>) => void) {
  getTabsQueryAPI()
    .then((tabsApi) => {
...
function appendHeuristicAds(host: string, blockedItems: { ads: any }) {
  ...
  if (blockedItems.ads && blockedItems.ads[host] && blockedItems.ads[host].hasOwnProperty("heuristic-ad-block")) {
...
function keepTabInSync(tabId: number) {
  if (tabId >= 0 && TABS[tabId]) {
    ...
    (chrome.action || chrome.browserAction).getBadgeText({ tabId }, (text) => {
      if (!text) {
        return;
      }
      ...
      if (totalBlocked !== Number(text) && TABS[tabId].blockedHistory) {
        // revert back to the history because we overwrote it
        TABS[tabId].blocked = JSON.parse(JSON.stringify(TABS[tabId].blockedHistory));
      }
    });
  }
}
...
function tabsOnUpdateLoading(tabId: number, changeInfo: any, tab: chrome.tabs.Tab) {
  ...
  if (changeInfo.url) {
    if (!changeInfo.url.startsWith("chrome://")) {
      onNewTab(tabId, changeInfo.url, domain(changeInfo.url), urlDomains(changeInfo.url));
    }
  } else {
...
function onNewTab(tabId: number, url: string, urlSld: string, domains: string[]) {
  if (tabId >= 0) {
    let blockedHistory;
    ...
    TABS[tabId] = {
      ...
      site: {
        ...
        sld: urlSld,
        domains: domains,
        friendlyUrl: current && current.site && current.site.friendlyUrl
      },
      status: "loading",
      meta: {
    ...
    TABS[tabId].meta.history = TABS[tabId].meta.history.filter((item: any) => Date.now() - 10000 <= item.timestamp);
