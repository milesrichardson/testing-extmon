...
function mbwLogger(console: Console): Console & { logOnce: (url: string, ...message: any[]) => void } {
  const logToStorage = (message: string = "", level) => {
    ...
    CONSOLE_TRACE +=
      ...
      `"session": "${sessionId}", ` +
      `"message": "${message.toString().replace(doubleQuoteRegex, "'")}", ` +
      `"level": "${level}"}\n`;
  };
...
export async function exportLogs(fromIDB: boolean = false): Promise<string> {
  ...
  try {
    ...
    const clampedFullLog = fullLog.length > maxSize ? fullLog.slice(fullLog.length - maxSize) : fullLog;
