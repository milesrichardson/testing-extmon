...
import { ExclusionType, Exclusion, EXCLUSION_ADS } from "@/domain/types/exclusion";
...
export class SafariExclusionService implements ExclusionService {
  ...
  exclude(host: string, exclusions: ExclusionType[]): Promise<boolean> {
    const contentBlockerNames = exclusions.map((exclusion) => exclusionContentBlockerMap[exclusion]).join(",");
    const payload = `${host}:${contentBlockerNames}`;
    console.debug("addToAllowList payload: ", payload);
    return new Promise<boolean>((resolve, reject) => {
      chrome.runtime.sendNativeMessage("com.malwarebytes.browserguard", { action: MSG_SAFARI_ALLOW_LIST_ADD, payload }, (response) => {
  ...
  getAllExclusions(): Promise<Exclusion[]> {
    return new Promise<Exclusion[]>((resolve, reject) => {
      chrome.runtime.sendNativeMessage("com.malwarebytes.browserguard", { action: MSG_SAFARI_ALLOW_LIST_FETCH }, (response) => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
        }
        ...
        for (const [key, value] of Object.entries(allowListResp)) {
          exclusions.push({
            website: key,
            exclusions: (value as any[]).map((exclusion: string) => {
              return contentBlockerExclusionMap[exclusion];
            })
          });
        }
        resolve(exclusions);
  ...
  async getExclusionsForHost(host: string): Promise<Exclusion | undefined> {
    ...
    const res = allExclusions.find((exclusion) => exclusion.website === host);
  ...
  getExclusions_getExclusionsByNames(hostnames: string[]): Promise<Exclusion[]> {
    throw new Error("Method not implemented.");
  }

  removeAllExclusions(): Promise<boolean> {
    return new Promise((resolve) => {
      chrome.runtime.sendNativeMessage("com.malwarebytes.browserguard", { action: MSG_SAFARI_ALLOW_LIST_CLEAR }, (response) => {
        const resp = JSON.parse(response);
  ...
  removeExclude(host: string, exclusions: ExclusionType[], addOverride: boolean): Promise<boolean> {
    const contentBlockerNames = exclusions.map((exclusion) => exclusionContentBlockerMap[exclusion]).join(",");
    const payload = `${host}:${contentBlockerNames}`;
  ...
  async isTemporarilyExcluded(url: string, tabId: number): Promise<boolean> {
    ...
    if (tabId) {
      ...
      console.debug("No optional tabId specified for exclusionsMatchSync. Skipping isExcludedTemporary check");
    }
    return false;
  }
}
