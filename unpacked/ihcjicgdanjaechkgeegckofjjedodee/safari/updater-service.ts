...
export class SafariUpdaterService implements UpdaterService {
  ...
  private resetUpdatedProtections() {
    for (const key in this.updatedProtections) {
      if (Object.prototype.hasOwnProperty.call(this.updatedProtections, key)) {
  ...
  private async sendUpdateMessage(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendNativeMessage("com.malwarebytes.browserguard", { action: MSG_SAFARI_DATABASE_UPDATE }, (response) => {
        console.debug("SUM: Received response from safari" + JSON.stringify(response));
        resolve(response[0]);
      });
    });
  }
