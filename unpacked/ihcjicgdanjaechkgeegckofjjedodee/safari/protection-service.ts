...
import { EXCLUSION_ADS, EXCLUSION_MALWARE, EXCLUSION_SCAMS, ExclusionType } from "@/domain/types/exclusion";
import { SETTING_GTLD, SETTING_SKIMMER_PROTECTION, SettingKey } from "@/domain/types/settings";
import { chrome } from "@/utils/typed-polyfill";
import { contentBlockerExclusionMap, exclusionContentBlockerMap } from "./common";
import { exclusionTypeToSettingKey, settingKeyToExclusionType } from "@/domain/types/functions";
...
export class SafariProtectionService implements ProtectionService {
  async isProtectionActiveForUrl(exclusionType: ExclusionType, urlOfTab: string, urlOfAd: string, tabId?: string): Promise<boolean> {
  ...
  toggleAllProtections(enabled: boolean): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      chrome.runtime.sendNativeMessage("com.malwarebytes.browserguard", { action: MSG_TOGGLE_PROTECTION, payload: enabled }, (response) => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
        }

        console.debug("TAP: Received response from safari" + JSON.stringify(response));
        resolve(!!response);
      });
    });
  }

  toggleIndividualProtection(exclusionType: ExclusionType, enabled: boolean): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve, reject) => {
      chrome.runtime.sendNativeMessage(
        ...
        (response) => {
          ...
          console.debug("TIP: Received response from safari" + JSON.stringify(response));
          resolve(!!response);
        }
      );
    });
  ...
  toggleIndividualProtectionBySettingKey(settingKey: SettingKey, enabled: boolean): Promise<boolean> {
  ...
  getProtectionStatus(): Promise<Map<ExclusionType | SettingKey, boolean>> {
    return new Promise((resolve, reject) => {
      chrome.runtime.sendNativeMessage("com.malwarebytes.browserguard", { action: MSG_PROTECTION_STATUS }, (response) => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
        }
        ...
        const { adsProtectionEnabled, malwareProtectionEnabled, scamsProtectionEnabled, gtldProtectionEnabled, skimmerProtectionEnabled } =
        ...
        return resolve(
          new Map([
            ...
            [SETTING_SKIMMER_PROTECTION, skimmerProtectionEnabled]
          ])
        );
      });
    });
