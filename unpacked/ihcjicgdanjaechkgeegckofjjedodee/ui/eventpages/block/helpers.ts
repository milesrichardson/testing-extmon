import { MSG_ADD_ALLOW, MSG_ADD_ALLOW_TEMPORARY, MSG_REM_ALLOW_SINGLE } from "@/app/scripts/app-consts.js";
...
export function redirect(url: string, params: Record<any, any>) {
  ...
  console.debug("Add exclusions was successful. Will redirect to: ", redirectionPage);
...
export function getExclusionConst(type: string, params: any): ExclusionType {
  switch (type) {
    ...
    default:
      console.error("The block page exclusion checkbox has not handled '" + params.type + "'");
      throw new Error("Invalid type passed to getExclusionConst");
  }
}

export async function updateAllowListViaMessage(params: any, exclusionConst: ExclusionType, continueAlways: boolean) {
  if (exclusionConst) {
    let toAllow =
      ["full-url-malware", "malware-pattern"].includes(params.subtype) || params.rules === "specific" ? params.url : params.host;
    if (continueAlways) {
      //EXCLUDE FROM SCANS
      console.debug("Checkbox switched on - requesting allow for " + exclusionConst);
      const resp = await sendBackgroundMessage<AddAllowRequest, AddAllowResponse>({
        type: MSG_ADD_ALLOW,
      ...
      console.debug("Checkbox switched off - requesting block for " + exclusionConst);

      const resp = await sendBackgroundMessage<RemAllowSingleRequest, RemAllowSingleResponse>({
...
export async function excludeTemporary(exclusionConst: ExclusionType, params: any, continueAlways: boolean) {
  if (continueAlways) {
