...
import { signInStrings, susStrings, suspiciousDescs, suspiciousTitles, tunnels } from "./consts";
import {
...
function phishDetection() {
  ...
  if (window.location.protocol === "http:" && isValidIP(window.location.hostname)) {
    const fullUrl = window.location.href && window.location.href.toLowerCase();

    if (fullUrl && susStrings.some((s) => fullUrl.includes(s))) {
      text = text || (document.body.innerText && document.body.innerText.toLowerCase());
    ...
    (whitelisted) => {
      ...
      chrome.runtime.sendMessage(
        ...
        (enableSuspiciousTitleDetection) => {
          ...
          let description = "";
          let descriptionFound = Array.from(document.getElementsByTagName("meta"))
          ...
          if (descriptionFound) {
            // eslint-disable-next-line max-len
            console.log(`PHISH: (PAGE_BLOCK) Caught a Phishing page. Page has suspicious description: ${description}`);
            // eslint-disable-next-line max-len
            isSafari ? blockDetection("scam", "phishing") : onDetection("scam", "phishing");
            return;
          }
          ...
          if (suspiciousTitles.indexOf(document.title) > -1) {
            // eslint-disable-next-line max-len
            console.log(`PHISH: (PAGE_BLOCK) Caught a Phishing page. Page has suspicious title: ${document.title}`);
            // eslint-disable-next-line max-len
            isSafari ? blockDetection("scam", "phishing") : onDetection("scam", "phishing");
            return;
          }
      ...
      if (window.location.protocol === "http:") {
        document.querySelectorAll("form").forEach((form) => {
          if (form.action.includes("login")) {
            ...
            // eslint-disable-next-line max-len
            isSafari ? blockDetection("scam", "insecure login") : onDetection("scam", "insecure login");
            return;
          }
        });
      }
