...
export async function hideLinkedInAds(pageUrl: string) {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => afterDOMLoaded(pageUrl));
  } else {
...
function isPromotedFeedUnit(feedUnit) {
  const spans = feedUnit.querySelectorAll('div[data-control-name="actor"] span.t-12.t-black--light');
...
function blockInitialPromotedFeedItems(tab: GetTabInfoResponse) {
  const feedUnits = document.querySelectorAll("#main > div:nth-child(3) > div > div");
  // @ts-ignore
