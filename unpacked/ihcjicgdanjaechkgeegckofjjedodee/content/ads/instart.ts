...
export function blockInstart() {
      ...
      (response) => {
        if (response && response.isWhitelisted) {
          ...
          if (chrome.runtime.lastError) {
            console.error("INS: Failed with is-whitelisted request for " + property + ": " + chrome.runtime.lastError.message);
          }
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.type === "isInstartWhitelisted" && eventHost === window.location.host) {
