...
import {
  ...
  IsProtectionActiveRequest,
  IsProtectionActiveResponse
} from "@/utils/messaging/types";

export function isHidden(element) {
  return element && element.style && element.style.display && element.style.display.trim() == "none";
}

export async function isAdProtectionActive(tabId: number, pageUrl: string): Promise<boolean> {
  const resp = await sendBackgroundMessage<IsAdProtectionActiveRequest, IsAdProtectionActiveResponse>({
    type: MSG_IS_AD_PROTECTION_ACTIVE,
    payload: { domain: pageUrl }
  });
