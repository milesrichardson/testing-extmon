...
export async function hideFacebookAds(pageUrl: string) {
  ...
  const malwarebytesClearFbAds = (tab: GetTabInfoResponse) => {
    const hyperFeedDivs = document.querySelectorAll('#contentArea div[id^="hyperfeed_story_id_"]');

    // @ts-ignore
    hyperFeedDivs.forEach((div: HTMLElement) => {
      if (
        div.innerText.includes("Sponsored") &&
        Array.from(div.getElementsByTagName("span")).some((span) => span.innerText === "Sponsored") && // User text is in a `p` tag, so just to ensure we don't hide someone's post because they used the word "Sponsored"
    ...
    if (sponsoredPagelet && sponsoredPagelet.innerText.includes("Sponsored")) {
  ...
  const checkForPaidFeedUnit = (feedUnit) => {
    return checkForSponsoredFeedUnit(feedUnit) || checkForSuggestedPost(feedUnit);
  };

  const areNumbersClose = (n1, n2, threshold = 0.001) => Math.abs(n1 - n2) <= threshold;
  ...
  const checkForSponsoredFeedUnit = (feedUnit) => {
    ...
    let link = isPotentiallySponsoredLink(feedUnitRect, links[2]) || isPotentiallySponsoredLink(feedUnitRect, links[3]);
    if (!link) {
      return false;
    }
    ...
    const text3 = letters3 // @ts-ignore
      ...
      .map((x) => x.text)
      .join("");
    return adTexts[lang].isSponsored(text) || adTexts[lang].isSponsored(text3);
  };
  ...
  const adTexts = {
    ...
    pt: {
      ...
      isSuggested: (str) => str.endsWith("ugestões para você") || str.endsWith("ugestões para ti") //sugestões para você, sugestões para ti
    },
    de: {
    ...
    fr: {
      isSponsored: (str) => str.endsWith("ponsorisé") || str.endsWith("ommandité"), //sponsorisé, commandité
  ...
  const checkForSuggestedPost = (feedUnit) => {
    const span = feedUnit.querySelector("div.rq0escxv.l9j0dhe7 > span.d2edcug0.hpfvmrgz");
    if (!span) {
      return false;
    }
      ...
      mutations.forEach((mutation) => {
        if (mutation.type === "childList") {
          ...
          mutation.addedNodes.forEach((addedNode: Element) => {
            ...
            if (addedNode.parentElement && addedNode.parentElement.getAttribute("role") === "feed") {
              if (checkForPaidFeedUnit(addedNode) && !isHidden(addedNode)) {
  ...
  const blockSponsoredRightRail = (tab) => {
    ...
    const sponsoredSpan = document.querySelector("div[role='complementary'] > div > div > div > div > div > span");
    const title = sponsoredSpan && sponsoredSpan.querySelector("span");
    if (title && adTexts[lang].isSponsored(title.textContent) && !isHidden(sponsoredSpan)) {
  ...
  const getLang = () => {
    ...
    const searchText = (searchBox && (searchBox as HTMLInputElement).placeholder) || "";
    switch (searchText) {
