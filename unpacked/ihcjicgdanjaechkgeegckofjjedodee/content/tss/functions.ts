...
export function isSusAudioPlayer() {
  ...
  if (audio && audio.tagName === "AUDIO" && audio.hasAttribute("autoplay") && isSusAudioChild(audio.children)) {
...
export async function isExcluded(url: string, tabId: number): Promise<boolean> {
  const resp = await sendBackgroundMessage<IsExcludedRequest, IsExcludedResponse>({
