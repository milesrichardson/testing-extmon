...
import { MSG_DETECTION, MSG_SETTINGS_GET, MSG_TAB_ID_GET } from "@/app/scripts/app-consts.js";
import { DetectionRequest, DetectionResponse, GetSettingRequest, GetSettingResponse } from "@/utils/messaging/types";
import { createBlockUrl, isSafari } from "../helpers";
import { injectJavascript, urlHost } from "@/utils/utils.js";
import { checkoutRegex, latestTssBodyPatterns, latestTssTitlePatterns } from "@/utils/patterns.js";
import { SETTING_SKIMMER_PROTECTION } from "@/domain/types/settings";
import { isCheckoutSkimmer, isExcluded, isRepeated, isSusAudioPlayer, isTrojanScam } from "./functions";
...
async function onDetection(type, subtype) {
  if (!detected && !excluded) {
    const resp = await sendBackgroundMessage<DetectionRequest, DetectionResponse>({
      type: MSG_DETECTION,
...
function onFullScreenChange() {
  ...
  console.debug(`TSS: Checking if repeated ${10} times for interval ${10000} against data: `, lastFullScreenData);
...
function initScriptListener() {
  window.addEventListener(
    "message",
    (event) => {
      if (event.source === window && event.data.nonce === nonce && event.data.type === "scam") {
