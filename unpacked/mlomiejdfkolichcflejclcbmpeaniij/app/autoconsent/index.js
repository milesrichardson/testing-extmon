...
async function getCategories() {
  const tab = await new Promise((resolve) => {
    chrome.runtime.sendMessage({ name: "getTabInfo" }, resolve);
  });
  ...
  const { antiTracking, adBlock } = await new Promise((resolve) => {
    chrome.runtime.sendMessage(
      {
...
mount(document.body, {
  ...
  content: ({ categories }) => html`
    <ui-autoconsent categories="${categories}" onenable="${enable}" ondisable="${disable}" onclose="${close}"></ui-autoconsent>
  `
});
