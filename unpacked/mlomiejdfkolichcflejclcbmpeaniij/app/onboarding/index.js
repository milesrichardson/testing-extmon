...
getBrowserInfo().then(({ name }) => {
  if (name.toLowerCase() === "firefox") {
    mount(document.body, {
      content: () => html` <ui-onboarding-short platform="${name}" onsuccess="${complete}" onskip="${skip}"></ui-onboarding-short> `
    });
  } else {
    mount(document.body, {
      content: () => html` <ui-onboarding platform="${name}" renew=${renew} onsuccess="${complete}" onskip="${skip}"></ui-onboarding> `
    });
  }
});
