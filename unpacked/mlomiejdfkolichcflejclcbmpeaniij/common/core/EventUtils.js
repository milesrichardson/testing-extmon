...
function sendMouseEvent(aEvent, aTarget, aWindow) {
  ...
  var detailArg =
    aEvent.detail ||
    (aEvent.type == "click" || aEvent.type == "mousedown" || aEvent.type == "mouseup" ? 1 : aEvent.type == "dblclick" ? 2 : 0);
  ...
  if (!window.document || window.document.documentURIObject) return aTarget.dispatchEvent(event);
...
function sendChar(aChar, aWindow) {
  ...
  switch (aChar) {
    ...
    case "?":
      ...
      break;
    default:
      hasShift = aChar.toLowerCase() != aChar.toUpperCase() && aChar == aChar.toUpperCase();
      break;
  }
...
function _parseModifiers(aEvent, aWindow = window) {
  ...
  if (aEvent.accelKey) {
    mval |= _EU_isMac(aWindow) ? nsIDOMWindowUtils.MODIFIER_META : nsIDOMWindowUtils.MODIFIER_CONTROL;
  }
  if (aEvent.altGrKey) {
...
function synthesizeMouse(aTarget, aOffsetX, aOffsetY, aEvent, aWindow) {
  ...
  return synthesizeMouseAtPoint(rect.left + aOffsetX, rect.top + aOffsetY, aEvent, aWindow);
}
function synthesizeTouch(aTarget, aOffsetX, aOffsetY, aEvent, aWindow) {
  var rect = aTarget.getBoundingClientRect();
  return synthesizeTouchAtPoint(rect.left + aOffsetX, rect.top + aOffsetY, aEvent, aWindow);
}
...
function synthesizeMouseAtPoint(left, top, aEvent, aWindow = window) {
  ...
  if (utils) {
    ...
    if ("id" in aEvent) {
      ...
      id = isFromPen ? utils.DEFAULT_PEN_POINTER_ID : utils.DEFAULT_MOUSE_POINTER_ID;
    }

    var isDOMEventSynthesized = "isSynthesized" in aEvent ? aEvent.isSynthesized : true;
    var isWidgetEventSynthesized = "isWidgetEventSynthesized" in aEvent ? aEvent.isWidgetEventSynthesized : false;
    var buttons = "buttons" in aEvent ? aEvent.buttons : utils.MOUSE_BUTTONS_NOT_SPECIFIED;
...
function synthesizeMouseAtCenter(aTarget, aEvent, aWindow) {
  var rect = aTarget.getBoundingClientRect();
  return synthesizeMouse(aTarget, rect.width / 2, rect.height / 2, aEvent, aWindow);
}
...
function synthesizeWheelAtPoint(aLeft, aTop, aEvent, aWindow = window) {
  ...
  var lineOrPageDeltaX =
    aEvent.lineOrPageDeltaX != null ? aEvent.lineOrPageDeltaX : aEvent.deltaX > 0 ? Math.floor(aEvent.deltaX) : Math.ceil(aEvent.deltaX);
  var lineOrPageDeltaY =
    aEvent.lineOrPageDeltaY != null ? aEvent.lineOrPageDeltaY : aEvent.deltaY > 0 ? Math.floor(aEvent.deltaY) : Math.ceil(aEvent.deltaY);
...
function synthesizeWheel(aTarget, aOffsetX, aOffsetY, aEvent, aWindow) {
  var rect = aTarget.getBoundingClientRect();
  synthesizeWheelAtPoint(rect.left + aOffsetX, rect.top + aOffsetY, aEvent, aWindow);
}
...
function _sendWheelAndPaint(aTarget, aOffsetX, aOffsetY, aEvent, aCallback, aFlushMode = _FlushModes.FLUSH, aWindow = window) {
  var utils = _getDOMWindowUtils(aWindow);
  if (!utils) return;
  ...
  if (utils.isMozAfterPaintPending) {
    ...
    aWindow.waitForAllPaintsFlushed(function () {
      _sendWheelAndPaint(aTarget, aOffsetX, aOffsetY, aEvent, aCallback, aFlushMode, aWindow);
    });
    return;
  }
...
function sendWheelAndPaint(aTarget, aOffsetX, aOffsetY, aEvent, aCallback, aWindow = window) {
  _sendWheelAndPaint(aTarget, aOffsetX, aOffsetY, aEvent, aCallback, _FlushModes.FLUSH, aWindow);
}
 ...
 */
function sendWheelAndPaintNoFlush(aTarget, aOffsetX, aOffsetY, aEvent, aCallback, aWindow = window) {
  _sendWheelAndPaint(aTarget, aOffsetX, aOffsetY, aEvent, aCallback, _FlushModes.NOFLUSH, aWindow);
}
 ...
 */
function synthesizeAndWaitNativeMouseMove(aTarget, aOffsetX, aOffsetY, aCallback, aWindow = window) {
  ...
  let eventReceivedPromise = ContentTask.spawn(browser, [aOffsetX, aOffsetY], ([clientX, clientY]) => {
...
function synthesizeKey(aKey, aEvent = undefined, aWindow = window, aCallback) {
  ...
  var dispatchKeydown = !("type" in event) || event.type === "keydown" || !event.type;
  var dispatchKeyup = !("type" in event) || event.type === "keyup" || !event.type;
 ...
 * input events to chrome process and have some checks in content.
 */
function synthesizeAndWaitKey(aKey, aEvent, aWindow = window, checkBeforeSynthesize, checkAfterSynthesize) {
  let browser = gBrowser.selectedTab.linkedBrowser;
  let mm = browser.messageManager;
...
const KEYBOARD_LAYOUT_ARABIC = { name: "Arabic", Mac: 6, Win: 0x00000401, hasAltGrOnWin: false };
const KEYBOARD_LAYOUT_ARABIC_PC = { name: "Arabic - PC", Mac: 7, Win: null, hasAltGrOnWin: false };
const KEYBOARD_LAYOUT_BRAZILIAN_ABNT = { name: "Brazilian ABNT", Mac: null, Win: 0x00000416, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_DVORAK_QWERTY = { name: "Dvorak-QWERTY", Mac: 4, Win: null, hasAltGrOnWin: false };
const KEYBOARD_LAYOUT_EN_US = { name: "US", Mac: 0, Win: 0x00000409, hasAltGrOnWin: false };
const KEYBOARD_LAYOUT_FRENCH = { name: "French", Mac: 8, Win: 0x0000040c, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_GREEK = { name: "Greek", Mac: 1, Win: 0x00000408, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_GERMAN = { name: "German", Mac: 2, Win: 0x00000407, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_HEBREW = { name: "Hebrew", Mac: 9, Win: 0x0000040d, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_JAPANESE = { name: "Japanese", Mac: null, Win: 0x00000411, hasAltGrOnWin: false };
const KEYBOARD_LAYOUT_KHMER = { name: "Khmer", Mac: null, Win: 0x00000453, hasAltGrOnWin: true }; // available on Win7 or later.
const KEYBOARD_LAYOUT_LITHUANIAN = { name: "Lithuanian", Mac: 10, Win: 0x00010427, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_NORWEGIAN = { name: "Norwegian", Mac: 11, Win: 0x00000414, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_RUSSIAN_MNEMONIC = { name: "Russian - Mnemonic", Mac: null, Win: 0x00020419, hasAltGrOnWin: true }; // available on Win8 or later.
const KEYBOARD_LAYOUT_SPANISH = { name: "Spanish", Mac: 12, Win: 0x0000040a, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_SWEDISH = { name: "Swedish", Mac: 3, Win: 0x0000041d, hasAltGrOnWin: true };
const KEYBOARD_LAYOUT_THAI = { name: "Thai", Mac: 5, Win: 0x0002041e, hasAltGrOnWin: false };
...
function synthesizeNativeKey(aKeyboardLayout, aNativeKeyCode, aModifiers, aChars, aUnmodifiedChars, aCallback, aWindow = window) {
  var utils = _getDOMWindowUtils(aWindow);
  if (!utils) {
    return false;
  }
...
function _expectEvent(aExpectedTarget, aExpectedEvent, aTestName) {
  ...
  var type = aExpectedEvent.charAt(0) == "!" ? aExpectedEvent.substring(1) : aExpectedEvent;
  var eventHandler = function (event) {
    var epassed = !_gSeenEvent && event.originalTarget == aExpectedTarget && event.type == type;
 ...
 * aWindow is optional, and defaults to the current window object.
 */
function synthesizeMouseExpectEvent(aTarget, aOffsetX, aOffsetY, aEvent, aExpectedTarget, aExpectedEvent, aTestName, aWindow) {
 ...
 * aWindow is optional, and defaults to the current window object.
 */
function synthesizeKeyExpectEvent(key, aEvent, aExpectedTarget, aExpectedEvent, aTestName, aWindow) {
  var eventHandler = _expectEvent(aExpectedTarget, aExpectedEvent, aTestName);
...
const COMPOSITION_ATTR_RAW_CLAUSE = _EU_Ci.nsITextInputProcessor.ATTR_RAW_CLAUSE;
_defineConstant("COMPOSITION_ATTR_RAW_CLAUSE", COMPOSITION_ATTR_RAW_CLAUSE);
const COMPOSITION_ATTR_SELECTED_RAW_CLAUSE = _EU_Ci.nsITextInputProcessor.ATTR_SELECTED_RAW_CLAUSE;
_defineConstant("COMPOSITION_ATTR_SELECTED_RAW_CLAUSE", COMPOSITION_ATTR_SELECTED_RAW_CLAUSE);
const COMPOSITION_ATTR_CONVERTED_CLAUSE = _EU_Ci.nsITextInputProcessor.ATTR_CONVERTED_CLAUSE;
_defineConstant("COMPOSITION_ATTR_CONVERTED_CLAUSE", COMPOSITION_ATTR_CONVERTED_CLAUSE);
const COMPOSITION_ATTR_SELECTED_CLAUSE = _EU_Ci.nsITextInputProcessor.ATTR_SELECTED_CLAUSE;
...
function _getTIP(aWindow, aCallback) {
  ...
  if (TIPMap.has(aWindow)) {
    ...
    tip = _EU_Cc["@mozilla.org/text-input-processor;1"].createInstance(_EU_Ci.nsITextInputProcessor);
...
function _createKeyboardEventDictionary(aKey, aKeyEvent, aTIP = null, aWindow = window) {
  var result = { dictionary: null, flags: 0 };
  var keyCodeIsDefined = "keyCode" in aKeyEvent;
  var keyCode = keyCodeIsDefined && aKeyEvent.keyCode >= 0 && aKeyEvent.keyCode <= 255 ? aKeyEvent.keyCode : 0;
  ...
  if (aKeyEvent.doNotMarkKeydownAsProcessed) {
    result.flags |= _EU_Ci.nsITextInputProcessor.KEY_DONT_MARK_KEYDOWN_AS_PROCESSED;
  }
...
function synthesizeComposition(aEvent, aWindow = window, aCallback) {
  ...
  if (aEvent.key && typeof aEvent.key.key === "string") {
    keyEventDict = _createKeyboardEventDictionary(aEvent.key.key, aEvent.key, TIP, aWindow);
    keyEvent = new KeyboardEvent(
      aEvent.key.type === "keydown" ? "keydown" : aEvent.key.type === "keyup" ? "keyup" : "",
      keyEventDict.dictionary
    );
  } else if (aEvent.key === undefined) {
    keyEventDict = _createKeyboardEventDictionary("KEY_Process", {}, TIP, aWindow);
  ...
  try {
    switch (aEvent.type) {
      ...
      case "compositioncommit":
        ret = TIP.commitCompositionWith(aEvent.data, keyEvent, keyEventDict.flags);
        break;
    }
  } finally {
    _emulateToInactivateModifiers(TIP, modifiers, aWindow);
...
function synthesizeCompositionChange(aEvent, aWindow = window, aCallback) {
  ...
  if (!aEvent.composition || !aEvent.composition.clauses || !aEvent.composition.clauses[0]) {
    return;
  }
    ...
    for (var i = 0; i < aEvent.composition.clauses.length; i++) {
      switch (aEvent.composition.clauses[i].attr) {
        ...
        case TIP.ATTR_SELECTED_CLAUSE:
          TIP.appendClauseToPendingComposition(aEvent.composition.clauses[i].length, aEvent.composition.clauses[i].attr);
          break;
        case 0:
...
function synthesizeQueryTextContent(aOffset, aLength, aIsRelative, aWindow) {
  ...
  return utils.sendQueryContentEvent(utils.QUERY_TEXT_CONTENT, aOffset, aLength, 0, 0, flags);
}
...
function synthesizeQuerySelectedText(aSelectionType, aWindow) {
  ...
  return utils.sendQueryContentEvent(utils.QUERY_SELECTED_TEXT, 0, 0, 0, 0, flags);
}
...
function synthesizeNativeOSXClick(x, y) {
  ...
  var CGPoint = new ctypes.StructType("CGPoint", [{ x: CGFloat }, { y: CGFloat }]);

  // Functions
  var CGEventSourceCreate = CoreGraphics.declare("CGEventSourceCreate", ctypes.default_abi, CGEventSourceRef, CGEventSourceStateID);
  ...
  var CGEventPost = CoreGraphics.declare("CGEventPost", ctypes.default_abi, ctypes.void_t, CGEventTapLocation, CGEventRef);
  var CFRelease = CoreFoundation.declare("CFRelease", ctypes.default_abi, ctypes.void_t, CGEventRef);
  ...
  var event = CGEventCreateMouseEvent(source, kCGEventLeftMouseDown, loc, kCGMouseButtonLeft);
  if (!event) {
    throw new Error("CGEventCreateMouseEvent returns null");
  }
  CGEventSetIntegerValueField(event, kCGMouseEventClickState, new ctypes.Int64(1));
  CGEventPost(kCGHIDEventTap, event);
  CFRelease(event);

  event = CGEventCreateMouseEvent(source, kCGEventLeftMouseUp, loc, kCGMouseButtonLeft);
  if (!event) {
    throw new Error("CGEventCreateMouseEvent returns null");
  }
  CGEventSetIntegerValueField(event, kCGMouseEventClickState, new ctypes.Int64(1));
...
function synthesizeDragStart(element, expectedDragData, aWindow, x, y) {
  ...
  var trapDrag = function (event) {
    try {
      ...
      if (expectedDragData == null || dataTransfer.mozItemCount != expectedDragData.length) throw dataTransfer;
      for (var i = 0; i < dataTransfer.mozItemCount; i++) {
        ...
        for (var j = 0; j < dtTypes.length; j++) {
          ...
          if (expectedDragData[i][j].eqTest) {
            if (!expectedDragData[i][j].eqTest(dtData, expectedDragData[i][j].data)) throw dataTransfer;
...
function synthesizeCharAtPoint(aX, aY, aWindow) {
  ...
  return utils.sendQueryContentEvent(utils.QUERY_CHARACTER_AT_POINT, 0, 0, aX, aY, QUERY_CONTENT_FLAG_USE_NATIVE_LINE_BREAK);
}
 ...
 */
function createDragEventObject(aType, aDestElement, aDestWindow, aDataTransfer, aDragEvent) {
...
function synthesizeDragOver(aSrcElement, aDestElement, aDragData, aDropEffect, aWindow, aDestWindow, aDragEvent = {}) {
  ...
  var event = createDragEventObject("dragover", aDestElement, aDestWindow, dataTransfer, aDragEvent);
...
function synthesizeDropAfterDragOver(aResult, aDataTransfer, aDestElement, aDestWindow, aDragEvent = {}) {
  ...
  if (aResult) {
    ...
    event = createDragEventObject("drop", aDestElement, aDestWindow, aDataTransfer, aDragEvent);
...
function synthesizeDrop(aSrcElement, aDestElement, aDragData, aDropEffect, aWindow, aDestWindow, aDragEvent = {}) {
  ...
  try {
    var [result, dataTransfer] = synthesizeDragOver(aSrcElement, aDestElement, aDragData, aDropEffect, aWindow, aDestWindow, aDragEvent);
    return synthesizeDropAfterDragOver(result, dataTransfer, aDestElement, aDestWindow, aDragEvent);
  } finally {
...
async function synthesizePlainDragAndDrop(aParams) {
  ...
  try {
    ...
    if (destElement) {
      ...
      event = createDragEventObject("dragover", destElement, destWindow, dataTransfer, {});
      sendDragEvent(event, destElement, destWindow);

      await new Promise((r) => setTimeout(r, 0));

      event = createDragEventObject("drop", destElement, destWindow, dataTransfer, {});
      sendDragEvent(event, destElement, destWindow);
    }

    // dragend is fired, by definition, on the srcElement
    event = createDragEventObject("dragend", srcElement, srcWindow, dataTransfer, { clientX: finalX, clientY: finalY });
...
var PluginUtils = {
  withTestPlugin: function (callback) {
    var ph = _EU_Cc["@mozilla.org/plugin/host;1"].getService(_EU_Ci.nsIPluginHost);
...
class EventCounter {
  constructor(aTarget, aType, aOptions = {}) {
    ...
    if (aOptions.mozSystemGroup) {
      SpecialPowers.addSystemEventListener(aTarget, aType, this.handleEvent, aOptions.capture);
    } else {
  ...
  unregister() {
    if (this.options.mozSystemGroup) {
      SpecialPowers.removeSystemEventListener(this.target, this.type, this.handleEvent, this.options.capture);
    } else {
