(function () {
   ...
   */
  const eventWithResponse = ({ type, args = [] }) =>
  ...
  const removeHighlightsById = (ids) =>
    eventOnly({
  ...
  const getRangeText = /*#__PURE__*/ (function () {
    var _ref4 = _asyncToGenerator(function* ({ start, end }) {
  ...
  const insertData = (mimeType, data) =>
    eventOnly({
      type: Events.insertData,
      args: [mimeType, data]
    });
  const getParagraphStarts = () =>
    eventWithResponse({
  ...
  const getTextAtCursor = /*#__PURE__*/ (function () {
    var _ref6 = _asyncToGenerator(function* ({ side = "left", length = 1 } = {}) {
  ...
  const setSpellCheckVisibility = (visible) =>
    eventOnly({
      type: Events.setSpellCheckVisibility,
      args: [visible]
    });
  const getSpellCheckErrors = () =>
    eventWithResponse({
