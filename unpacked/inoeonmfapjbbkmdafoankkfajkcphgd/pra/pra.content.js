...
let { freeze, seal, create: create$2 } = Object; // eslint-disable-line import/no-mutable-exports
...
function addToSet(set, array, transformCaseFunc) {
  ...
  transformCaseFunc =
    (_transformCaseFunc = transformCaseFunc) !== null && _transformCaseFunc !== void 0 ? _transformCaseFunc : stringToLowerCase;
  if (setPrototypeOf) {
...
const checkExists = async (accessToken, id) => {
  ...
  const { trashed } = await result.json();
  if (trashed === true) {
    return false;
  }
  if (result.ok) {
...
const get = async (accessToken) => {
  const url = `https://content.googleapis.com/drive/v3/files?${encodeURI(
    `q=name='${configFileName}'&spaces=appDataFolder&fields=files/id,files/description,files/trashed`
  )}`;
...
class GoogleDriveUploader extends IUploader {
  ...
  async createFolderStructure(rootFolder, feature, accessToken, documentID = null) {
    const { configJson, configId } = await this.getConfigData(accessToken);
  ...
  async upload(blob, fileName, rootFolder, feature, accessToken, documentID = null, permissions = null) {
...
const uploadFile = async (
  ...
  try {
    ...
    const fileId = await uploader.upload(blob, fileName, rootFolder, feature, accessToken, documentID, permissions);
    return fileId;
  } catch (e) {
    return {
      fileId: null,
...
const drivePermission = async (accessToken, fileId, emailAddress, role, sendNotificationEmail, emailMessage) => {
  ...
  const response = await fetch(
    `https://www.googleapis.com/drive/v3/files/${fileId}/permissions?sendNotificationEmail=${sendNotificationEmail}&supportsAllDrives=true&emailMessage=${emailMessage}`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
const shareFile$1 = async (params) => {
  const { accessToken, fileId, emailAddress, role, sendNotificationEmail, emailMessage } = params;
...
const shareFile = async (params) => {
  const { accessToken, driveId, fileId, sendNotificationEmail, emailAddress, emailMessage } = params;
...
const shareLink$1 = async (params) => {
  const { accessToken, fileId, driveId } = params;
...
const getDataForShare = async () => {
  const { accessToken, error } = await getAccessTokenFromServiceWorker(window.thPra.loginType);
  const { driveId, fileId } = document.querySelector(".share-dialog").dataset;
  return {
    accessToken,
    fileId,
    driveId,
...
const share = async (e) => {
  ...
  const { accessToken, fileId, driveId, error } = await getDataForShare();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const sendToTeacherListener = async () => {
    ...
    const uploadResult = await fileuploader.uploadFile(
      ...
      title,
      "Texthelp",
      "PracticeReadingAloud",
      accessToken,
      null,
      window.thPra.loginType
    );
