(async function () {
  ...
  const mathjaxScripts = Object.freeze([
    {
      ...
      doNotInject: ["https://digitalemethode.be", "https://wezoozacademy.h5p.com"],
      injectRule: {
        target: {},
  ...
  if (
    mathjaxScripts[0].doNotInject.filter((x) => {
      if (x === window.location.origin) {
        return true;
      }
    }).length > 0
  ) {
    let settings = JSON.parse(
      window.localStorage.getItem("MathJax-Menu-Settings") ? window.localStorage.getItem("MathJax-Menu-Settings") : "{}"
    );
    if (settings.explorer === true) {
