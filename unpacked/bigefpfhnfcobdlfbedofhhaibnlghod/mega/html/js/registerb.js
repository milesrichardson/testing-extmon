...
BusinessRegister.prototype.initPage = function (preSetNb, preSetName, preSetTel, preSetFname, preSetLname, preSetEmail) {
  ...
  var fillPaymentGateways = function (status, list) {
    ...
    if (!window.businessVoucher) {
      ...
      for (var k = 0; k < list.length; k++) {
        var payRadio = radioHtml.replace("[x]", list[k].gatewayName).replace("[Y]", list[k].gatewayId).replace("[Z]", list[k].gatewayName);
    ...
    $paymentBlock.safeAppend(radioHtml.replace("[x]", "Voucher") + textHtml.replace("[x]", l[23494]) + "</div>");

    // setting the first payment provider as chosen
    $(".bus-reg-radio-block .bus-reg-radio", $pageContainer).first().removeClass("radioOff").addClass("radioOn");

    // event handler for radio buttons
    $(".bus-reg-radio-option", $paymentBlock).rebind("click.suba", function businessRegisterationCheckboxClick() {
  ...
  const isValidBillingData = () => {
    return (
      mySelf.planInfo.bd &&
      mySelf.planInfo.bd.us &&
      (mySelf.planInfo.bd.us.p || mySelf.planInfo.bd.us.lp) &&
      mySelf.planInfo.bd.sto &&
      (mySelf.planInfo.bd.sto.p || mySelf.planInfo.bd.sto.lp) &&
      mySelf.planInfo.bd.sto.s &&
      mySelf.planInfo.bd.trns &&
      (mySelf.planInfo.bd.trns.p || mySelf.planInfo.bd.trns.lp) &&
      mySelf.planInfo.bd.trns.t &&
      mySelf.planInfo.bd.ba.s &&
      mySelf.planInfo.bd.ba.t
    );
  };

  const isUsageCharges = () => {
    return mySelf.planInfo.bd.ba.s > 0 && mySelf.planInfo.bd.ba.t > 0 && mySelf.planInfo.bd.sto.s > 0 && mySelf.planInfo.bd.trns.t > 0;
  };
  ...
  const updatePriceGadget = function (users, quota) {
    ...
    if (quotaInfoPresent) {
      ...
      $(".bus-quota-note-body", $baseQuotaNote).text(
        l.base_stroage_note_desc.replace("%1", extraFares.storageBase / 1024).replace("%2", intl.format(mySelf.planInfo.bd.sto.p))
      );
      ...
      if (neededQuota > 0) {
        ...
        $(".bus-add-storage-body", $extraStroage).text(l.additional_storage.replace("%1", mySelf.extraStorage));
        $(".bus-add-storage-foot", $extraStroage).text(l.additional_storage_desc.replace("%1", extraFares.storageBase / 1024));
  ...
  $(".bus-reg-agreement", $pageContainer).rebind("click.suba", function businessRegisterationCheckboxClick() {
  ...
  $regChk.rebind("focus.chkRegisterb", function regsiterbInputFocus() {
    $(this).parent().addClass("focused");
  });

  $regChk.rebind("blur.chkRegisterb", function regsiterbInputBlur() {
  ...
  $(".bus-reg-info-block input", $pageContainer).rebind("input.suba", function nbOfUsersChangeEventHandler() {
    var $me = $(this);
    var valid = false;
  ...
  $regBtns.rebind("click.regBtns", function registerBusinessAccButtonClickHandler() {
  ...
  $regBtns.rebind("keydown.regBtns", function regBusinessKeyDownHandler(e) {
    if (e.keyCode === 9) {
      e.preventDefault();
      $nbUsersInput.focus();
  ...
  $regBtns.rebind("focus.regBtns", function regsiterbBtnFocus() {
    $(this).addClass("focused");
  });

  $regBtns.rebind("blur.regBtns", function regsiterbBtnBlur() {
...
BusinessRegister.prototype.processPayment = function (payDetails, businessPlan) {
  ...
  new BusinessAccount()
    .doPaymentWithAPI(payDetails, businessPlan)
    .then(({ result, saleId }) => {
      const redirectToPaymentGateway = () => {
        const isStrip = businessPlan.usedGatewayId ? (businessPlan.usedGatewayId | 0) === addressDialog.gatewayId_stripe : false;
