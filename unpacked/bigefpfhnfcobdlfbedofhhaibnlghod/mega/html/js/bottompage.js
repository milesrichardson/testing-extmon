...
var bottompage = {
  ...
  init: function () {
    ...
    if (page === "bird") {
      $(".top-bl .bottom-page.top-dark-button.rounded span.label", $content).safeHTML(l[20923].replace("%1", "Thunderbird"));
    }
  ...
  changeMenuItemsList: function ($content) {
    ...
    if (
      (u_attr && u_attr.b && u_attr.b.s !== pro.ACCOUNT_STATUS_EXPIRED) ||
  ...
  initNavButtons: function ($content) {
    ...
    $(".submenu-item", $topMenu).rebind("click.openpmenu", function () {
      ...
      $content.rebind("mousedown.closepmenu", function (e) {
        ...
        if (
          !$target.is(".submenu.active") &&
          !$target.closest(".submenu-item.active").length &&
  ...
  initSliderEvents: function ($sliderSection, $scrollBlock, $slides, passing) {
    ...
    const scrollToPlan = (slideNum) => {
      ...
      planPosition = $previousPlan.length ? $previousPlan.position().left + $scrollBlock.scrollLeft() + $previousPlan.outerWidth() : 0;
    ...
    $scrollBlock.rebind("scroll.scrollToPlan", function () {
      ...
      if (scrollVal > 0) {
        closestIndex = Math.round((scrollVal / (contentWidth - scrollAreaWidth)) * $slides.length);
      }
