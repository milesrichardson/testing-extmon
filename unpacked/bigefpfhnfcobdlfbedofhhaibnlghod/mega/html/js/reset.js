...
function recovery_reset_pw() {
  ...
  security
    .resetKey(recoverycode, base64_to_a32(recoverykey), recoveryemail, password)
    .then(() => {
...
function verify_key(key) {
  ...
  security
    .resetKey(recoverycode, base64_to_a32(recoverykey), recoveryemail)
    .then(() => {
...
function init_reset_pw() {
  ...
  var _checkInput = function ($input) {
    ...
    if (
      $firstInput.val() &&
      $confirmInput.val() &&
      $firstInput.val().length >= security.minPasswordLength &&
