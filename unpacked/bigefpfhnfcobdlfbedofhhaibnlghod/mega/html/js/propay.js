...
pro.propay = {
  ...
  init: function () {
    ...
    if (discountInfo && discountInfo.pd && !discountInfo.used) {
      const discountTitle = discountInfo.m === 12 ? l[24680] : discountInfo.m === 1 ? l[24849] : l[24850];
      $(".top-header.plan-title", this.$page).safeHTML(
        discountTitle.replace("%1", pro.propay.planName).replace("%2", formatPercentage(discountInfo.pd / 100))
      );
    ...
    $purchaseButton.rebind("click.purchase", () => {
      if (is_mobile) {
        pro.propay.userSubsGatewayId = M.account.sgwids && M.account.sgwids.length > 0 ? M.account.sgwids[0] : null;
      }
  ...
  preloadAnimation: function () {
    ...
    pro.propay.$loadingOverlay.find(".payment-animation").attr("src", staticpath + "/images/mega/payment-animation" + retina + ".gif");
  },
  ...
  renderPlanDurationOptions: function (discountInfo) {
    ...
    for (var i = 0, length = pro.membershipPlans.length; i < length; i++) {
      ...
      if (currentPlan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] === parseInt(pro.propay.planNum)) {
        ...
        if (discountInfo && discountInfo.md && discountInfo.pd) {
          ...
          if (
            (willRenewYearly && currentPlan[pro.UTQA_RES_INDEX_MONTHS] === 12) ||
        ...
        if (numOfMonths === 12) {
          ...
          if (discountedPriceY) {
            $(".oldPrice", $durationOption).text($(".price", $durationOption).text()).removeClass("hidden");
    ...
    let $selectedOption = $('.payment-duration[data-plan-months="' + selectedPeriod + '"]', ".duration-options-list");
  ...
  updateMainPrice: function (planIndex, discountInfo) {
    ...
    if (typeof planIndex === "undefined") {
      planIndex = $(".duration-options-list .membership-radio.checked", ".payment-section").parent().attr("data-plan-index");
    }
    ...
    if (discountInfo && discountInfo.md && discountInfo.pd && discountInfo.al === pro.propay.planNum) {
      ...
      $(".discount-header-text", $discountHeader).text(l[24670].replace("$1", formatPercentage(discountInfo.pd / 100)));
  ...
  updateTextDependingOnRecurring: function () {
    ...
    // Change the charge information below the recurring yes/no question
      ...
      chargeInfoDuration = l[10642].replace("%1", discountInfo.lmp ? discountInfo.lmp + "*" : discountInfo.emp);
    } else if (discountInfo && (discountInfo.lyp || discountInfo.eyp) && !recurringEnabled && numOfMonths === 12) {
      // You will be charged 0.00 one time.
      chargeInfoDuration = l[10642].replace("%1", discountInfo.lyp ? discountInfo.lyp + "*" : discountInfo.eyp);
    }
    ...
    if ((discountInfo && discountInfo.cs) || currentPlan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] === pro.ACCOUNT_LEVEL_PRO_FLEXI) {
  ...
  renderPaymentProviderOptions: function (gatewayOptions, primaryOrSecondary) {
    ...
    var selectedPlanIndex = $(".duration-options-list .membership-radio.checked", "body").parent().attr("data-plan-index");
    ...
    $(".payment-options-list." + primaryOrSecondary + " .payment-method:not(.template)", "body").remove();
    ...
    for (var i = 0, length = gatewayOptions.length; i < length; i++) {
      ...
      if (iconkey) {
        $(".provider-icon", $gateway).addClass("svgicon").safeHTML(`<i class="sprite-fm-uni ${svgicons[iconkey]}"></i>`);
      } else {
  ...
  preselectPaymentOption: function () {
    ...
    var $option =
      parseFloat(pro.propay.proBalance) > 0 ? $payOptions.first() : $payOptions.length > 1 ? $payOptions.eq(1) : $payOptions.eq(0);
  ...
  updateDurationOptionsOnProviderChange: function () {
    ...
    var selectedPlanIndex = $(".membership-radio.checked", $durationOptionsList).parent().attr("data-plan-index");
    ...
    $.each($durationOptions, function (key, durationOption) {
      ...
      if (
        (!selectedProvider.supportsMonthlyPayment && numOfMonths === 1) ||
        ...
        if (
          selectedProvider.minimumEURAmountSupported &&
    ...
    if ($newDurationOption.length && !$newDurationOption.hasClass("hidden") && !$newDurationOption.hasClass("disabled")) {
      newPlanIndex = selectedPlanIndex;
    } else {
      $newDurationOption = $(".payment-duration:not(.template, .hidden, .disabled)", $durationOptionsList).first();
  ...
  startPurchaseProcess: function () {
    ...
    if (u_type === false) {
      ...
      if (pro.propay.proPaymentMethod === "perfunctio") {
        ...
        String(pro.propay.proPaymentMethod).indexOf("ecp") === 0 ||
      ...
      if (offer.al && offer.pd && typeof offer.m !== "undefined") {
        ...
        pro.loadMembershipPlans(() => {
          const matchedPlan = pro.membershipPlans.find((plan) => {
            return plan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] === offer.al && plan[pro.UTQA_RES_INDEX_MONTHS] === (offer.m || 12);
          });
          if (matchedPlan) {
            const storageFormatted = numOfBytes(matchedPlan[pro.UTQA_RES_INDEX_STORAGE] * 1073741824, 0);
            const desc = l[24704].replace("%1", Math.round(storageFormatted.size) + " " + storageFormatted.unit);
            ...
            $("button.js-close, .close-btn", $discountDlg).rebind("click.discount", (ev) => {
              ...
              mBroadcaster.sendMessage("trk:event", "discountPopup", "closed", "notShowAgain", reTrigger ? 0 : 1);
            });
            ...
            $(".get-btn", $discountDlg).rebind("click.discount", () => {
              ...
              mBroadcaster.sendMessage("trk:event", "discountPopup", "requested", "notShowAgain", reTrigger ? 0 : 1);
            });
