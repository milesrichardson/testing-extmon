...
class DiscountPromo {
  ...
  getDiscountInfo(page, completeCallback) {
    ...
    api
      .req({ a: "dci", dc: mega.discountCode, su: mega.shortUrl, extra: true })
      .then(({ result: res }) => {
        loadingDialog.hide();
  ...
  getMatchedPlan() {
    return pro.membershipPlans.find((plan) => {
      return plan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] === mega.discountInfo.al && plan[pro.UTQA_RES_INDEX_MONTHS] === 1;
    });
  }
  ...
  renderDiscountDetails(matchedPlan) {
    ...
    let descriptionText = useYearString ? l.promotion_discount_description_year : l.promotion_discount_description_month;
    ...
    let footerTitleText = useYearString ? l.promotion_discount_footer_title_year : l.promotion_discount_footer_title_month;
