...
pro.proplan = {
  ...
  initPlanHoverHandlers: function () {
    ...
    $planBlock.rebind("mouseenter.showprotag tap.showprotag", function () {
      ...
      if ($this.data("tag")) {
        // Set tag information
        $("span", $planTag)
          .attr("class", "pro" + $this.data("payment"))
    ...
    $slider.slider({
      ...
      change: function (e, ui) {
        ...
        for (var i = 0, length = $resultBlocks.length; i < length; i++) {
          ...
          $(".price", $resultBlock).safeHTML(
            value === 1 ? planInfo[1] : formatCurrency(planInfo[1], planInfo[2]).replace("\u20ac", '<span class="currency">\u20ac</span>')
          );
  ...
  updateCurrentPlanData: function ($pricingBoxes) {
    ...
    if (u_type === 3) {
      ...
      if (
        !pro.proplan.planData ||
        pro.proplan.planData.utype !== u_attr.p ||
  ...
  processCurrentAndNextPlan: function (data, $pricingBoxes) {
    ...
    // Current plan
    if (data.srenew) {
      // This is subscription plan

      var renewTimestamp = data.srenew[0];
      if (renewTimestamp === 0) {
        $currPlan.addClass("renew");
      } else {
        $currPlan.addClass("renew").data("tag", l[20759].replace("%1", time2date(renewTimestamp, 2)));
      }
    } else {
    ...
    if (data.nextplan) {
      ...
      $nextPlan.data(
        "tag",
        l[20714].replace("%1", time2date(pro.proplan.nextPlan.t, 2)).replace("%2", pro.getProPlanName(pro.proplan.nextPlan.p))
      );
  ...
  handleDiscount: function (page) {
    ...
    delete mega.discountInfo;
    api
      .req({ a: "dci", dc: mega.discountCode })
      .then(({ result: res }) => {
        loadingDialog.hide();
  ...
  populateBusinessPlanData: function () {
    ...
    if (this.businessPlanData.isValidBillingData) {
      ...
      $(".js-main", $storageInfo).text(l.bsn_starting_storage.replace("%1", minStorageValue));
      $(".js-addition", $storageInfo).text(l.bsn_additional_storage.replace("%1", storagePrice));
      $("i", $storageInfo).attr(
        "data-simpletip",
        l.bsn_storage_tip.replace("%1", `${minStorageValue} ${l[20160]}`).replace("%2", storagePrice)
      );
      $(".js-main", $transferInfo).text(l.bsn_starting_transfer.replace("%1", minTransferValue));
      $(".js-addition", $transferInfo).text(l.bsn_additional_transfer.replace("%1", transferPrice));
      $("i", $transferInfo).attr(
        "data-simpletip",
        l.bsn_transfer_tip.replace("%1", `${minTransferValue} ${l[20160]}`).replace("%2", transferPrice)
      );
    ...
    // TODO: remove when new API is stable
      ...
      const storageAmount =
        this.businessPlanData.bd && this.businessPlanData.bd.ba && this.businessPlanData.bd.ba.s
          ? this.businessPlanData.bd.ba.s / 1024
          : 15;

      $businessCard = $(".js-business-card-old", $stepOne);
      $(".plan-feature.storage-b span", $businessCard).safeHTML(l[23789].replace("%1", `<span>${storageAmount} ${l[20160]}</span>`));
  ...
  initBusinessPlanCalculator: function () {
    ...
    const calculatePrice = (usersValue, storageValue, transferValue) => {
      ...
      totalPrice =
        userPrice * usersValue + storagePrice * (storageValue - minStorageValue) + transferPrice * (transferValue - minTransferValue);

      return this.businessPlanData.isLocalInfoValid
        ? formatCurrency(totalPrice, this.businessPlanData.l.lc)
        : formatCurrency(totalPrice) + astrisk;
    };
    ...
    $(".pr1", $calculator).text(l.bsn_calc_monthly_price.replace("%1", `100 ${l[20160]}`).replace("%2", calculatePrice(3, 97, 0)));
    $(".pr2", $calculator).text(l.bsn_calc_monthly_price.replace("%1", `1 ${l[23061]}`).replace("%2", calculatePrice(3, 997, 0)));

    // Set min values under each slider
    $(".js-min-users", $calculator).safeHTML(l.bsn_calc_min_users.replace("%1", planInfo.bd.minu));
    $(".js-min-storage", $calculator).safeHTML(l.bsn_calc_min_storage.replace("%1", minStorageValue));
    $(".js-min-transfer", $calculator).safeHTML(l.bsn_calc_min_transfer.replace("%1", minTransferValue));
  },
...
function completeProLogin(result) {
  ...
  // If successful result
    ...
    if (page === "chat") {
      ...
      const proNum =
        continuePlanNum === null ? pro.proplan2.selectedPlan || $(".pricing-page.plan.selected").data("payment") : continuePlanNum;
      ...
      M.getStorageQuota().then((storage) => {
        ...
        checkPlanStorage(storage.used, proNum).then((res) => {
          ...
          if (res) {
            ...
            msgDialog("warninga", l[135], l.warn_head_not_enough_storage, l.warn_body_not_enough_storage, () => {
              loadSubPage("pro");
...
var showSignupPromptDialog = function () {
  ...
  if (!signupPromptDialog) {
    ...
    signupPromptDialog.rebind("onBeforeShow", function () {
      ...
      $("header p", this.$dialog).safeHTML("@@", l[5842]);

      $(".pro-login", this.$dialog).rebind("click.loginrequired", function () {
