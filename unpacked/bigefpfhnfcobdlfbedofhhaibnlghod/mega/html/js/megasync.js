var megasync = (function () {
  ...
  function switchMegasyncUrlToHttpWhenPossible() {
    ...
    if (ua.details.browser === "Internet Explorer" || ua.details.browser === "Safari" || ua.details.browser === "Edge") {
      return ShttpMegasyncUrl;
  ...
  function megaSyncIsNotResponding(nextIfYes) {
    if (lastCheckStatus && lastCheckTime) {
      ...
      msgDialog("confirmation", "MEGA Desktop App is not responding", l[17795], l[17796], function (disableMsync) {
        if (disableMsync) {
  ...
  function api_handle(next, response, requestArgs) {
    ...
    if (response === 0) {
      ...
      } else if (requestArgs.a === "ufi" || requestArgs.a === "ufo") {
        // is upload file MEGAsync request
        return _uploadTick();
      ...
      if (requestArgs.a === "t") {
        // is get status request

        if (d > 1) {
          console.info("status: " + response.s + " progress: " + response.p + "  total: " + response.t + "  speed: " + response.v);
        }

        if (response.s && response.s == 2) {
          // allow implied convert
          ...
          if (currStatus !== l[17592]) {
            // 'Downloading with MEGAsync .'
        ...
        } else if (response.s && response.s == 1) {
          // allow implied convert
          // STATE_QUEUED = 1
        ...
        } else if (response.s && response.s == 0) {
          // allow implied convert
          // unknow STATE
        ...
        } else if (response.s && response.s == 6) {
          // allow implied convert
          // STATE_COMPLETED = 6
        ...
        } else if (response.s && response.s == 3) {
          // allow implied convert
        ...
        } else if (response.s && response.s == 4) {
          // allow implied convert
          ...
          setTimeout(_statusTick, 3000);
        } else if (response.s && response.s == 5) {
          // allow implied convert
          ...
          setTimeout(_statusTick, 1000);
        } else if (response.s && response.s == 8) {
          // allow implied convert
      ...
      } else if (requestArgs.a === "v") {
        // is get version MEGAsync request
        if (response.u) {
          ns.currUser = response.u;
        }
        lastCheckStatus = response;
        lastCheckTime = Date.now();
      } else if (requestArgs.a === "uss") {
        // is get upload status MEGAsync request
        var response = response.length ? response[0] : response;
        if (response.s && response.s == 1) {
          // selection done
          var toastTxt = "";
          var folderP = 0;
          if (response.fo) {
          ...
          showToast("megasync-transfer upload", toastTxt, l[865], l[823], ns.transferManager, function () {
            loadSubPage("fm/account/transfers");
          }); // Upload added toMEGAsync
  ...
  ns.isInstalled = function (next) {
    if (
      (!fmconfig.dlThroughMEGAsync && page !== "download" && page !== "fm/account/transfers" && !folderlink) ||
      ...
      if (diff >= defaultStatusThreshold) {
        ...
        if (lastCheckStatus && (!ns.currUser || ns.currUser !== u_handle) && diff >= statusThresholdWhenDifferentUsr) {
          SyncAPI({ a: "v" }, next);
