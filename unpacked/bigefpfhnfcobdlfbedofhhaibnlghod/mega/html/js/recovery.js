...
function AccountRecoveryControl() {
  ...
  $(".app-instruction-block .recovery-instructions select", this.$recoveryContents).rebind("change.recoverpagedevice", function () {
    var device = $(this).val();
    if (device) {
  ...
  var stepZero = function () {
    ...
    if (enteredEmail && !isValidEmail(enteredEmail)) {
      ...
      // Entered email is valid, or user click the button again withoout email.
...
AccountRecoveryControl.prototype.showStep = function _showStep(step, branch) {
  ...
  if (step === 0) {
    if (is_mobile) {
      this.$navigationControl.find(".fm-icon.left").addClass("mega").removeClass("back").removeClass("non-responsive");
    ...
    if (is_mobile) {
      this.$navigationControl.find(".fm-icon.left").addClass("back").removeClass("mega").addClass("non-responsive");
    }
  }
  ...
  $(".recover-image", this.$recoveryContents)
    .removeClass()
    .addClass("recover-image " + recoveryImg);
      ...
      { a: "erm", m: email, t: t },
      {
        callback: function (res) {
          ...
          if (res === ENOENT) {
            ...
            if (!is_mobile) {
              handleResetSuccessDialogs(l.reset_pwd_email_sent_msg.replace("%s", email), "resetsuccess");
            } else {
              closeDialog();
...
AccountRecoveryControl.prototype.showParkWarning = function _showParkWarning(easyPark) {
  ...
  $(".checkbox-block.park-account-checkbox .settings-row .checkdiv", $dialog)
    ...
    .removeClass("hidden");
  $(".checkbox-block.park-account-checkbox .settings-row .checkdiv .checkbox", $dialog).removeClass("checkboxOn").addClass("checkboxOff");
