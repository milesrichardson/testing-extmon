lazy(pro, "proplan2", () => {
  ...
  const initTabHandlers = () => {
    ...
    const proDivsSelector =
      ".pricing-pg.pick-period-container, .pricing-pg.pro-plans-cards-container, " + ".pricing-pg.pricing-estimation-note-container";
    ...
    const changeIndividualTeamTab = (target) => {
      ...
      $freeBanner.toggleClass("hidden", !visibilityState || (typeof u_handle !== "undefined" && !localStorage.keycomplete));
  ...
  const initBuyPlan = () => {
    ...
    $freeBtns.rebind("click.pricing", function () {
      ...
      if (!window.u_handle) {
        const destination = this.id === "tryMega" && this.textContent === l[24549] ? "registerb" : "register";
  ...
  const estimateBussPrice = (users = 3, storage = 3, transfer = 3) => {
    ...
    if (pro.proplan.businessPlanData.isLocalInfoValid) {
      ...
      totalPrice = formatCurrency(totalUsersCost + totalStorageCost + totalTransferCost, currency, "narrowSymbol");
    } else {
  ...
  const estimateFlexiPrice = (storage = 3, transfer = 3) => {
    ...
    if (
      ProFlexiFound[13] &&
      ProFlexiFound[15] &&
      ProFlexiFound[pro.UTQA_RES_INDEX_LOCALPRICECURRENCY] &&
      ...
      totalPrice = formatCurrency(totalStorageCost + totalTransferCost + ProFlexiFound[pro.UTQA_RES_INDEX_MONTHLYBASEPRICE]);
    }
  ...
  const initSlidersEvents = () => {
    ...
    $compareDP.text(formatCurrency(5.5));
    $compareGD.text(formatCurrency(3.7));
    ...
    const sliderEventHandler = (slider, ranges, $inputTxt) => {
      ...
      const styleVal = `linear-gradient(${direction}, var(--color-secondary-cobalt-900) ${val}%,` + `var(--color-grey-150) ${val}% 100%)`;
  ...
  const initProFlexi = () => {
    ...
    $(".pricing-plan-storage", $proFlexCard).text(l.bsn_base_stg_trs.replace("%1", baseStorage));
    ...
    if (flexiCurrency === "EUR") {
      $(".pricing-plan-trasfer .ex-desc", $proFlexCard).text(l.pr_flexi_extra.replace("%1", extraPrice));
    } else {
      $(".pricing-plan-trasfer .ex-desc", $proFlexCard).text(l.bsn_add_base_stg_trs.replace("%1", extraPrice));
    }

    const $buyBtn = $(".pricing-plan-btn", $proFlexCard).text(l.buy_plan.replace("%1", planName));
    ...
    $(".pricing-flexi-block-card-note, .pricing-flexi-block-card-note-s", $proflexiBlock).toggleClass("hidden", !hasLocalPrices);
  ...
  const fillPlansInfo = (period) => {
    ...
    for (const currentPlan of pro.membershipPlans) {
      ...
      const planPriceNoDiscount = includeNoDiscount ? 12 * currentPlan[pro.UTQA_RES_INDEX_MONTHLYBASEPRICE] * pro.conversionRate : null;
      ...
      const noDiscountText = planPriceNoDiscount ? formatCurrency(planPriceNoDiscount, priceCurrency, "narrowSymbol") : false;

      const monthlyPriceText = includeNoDiscount ? formatCurrency(planPrice / 12, priceCurrency, "narrowSymbol") : false;
      ...
      if (includeNoDiscount) {
        ...
        $(".pricing-plan-only", $planCard).text(noDiscountText).removeClass("hidden").addClass("strikethrough");
    ...
    $(".pricing-pg.pricing-banner-container .pricing-get-free-banner-price-val", $page).text(
      formatCurrency(0, localPriceInfo, "narrowSymbol", true)
    );
  ...
  const populateBusinessPlanData = () => {
    ...
    let pricePerUser = pro.proplan.businessPlanData.bd && pro.proplan.businessPlanData.bd.us && pro.proplan.businessPlanData.bd.us.p;
    let priceCurrency;
    let storagePrice;
    ...
    $(".pricing-plan-storage .business-base", $businessCard).text(l.bsn_base_stg_trs.replace("%1", minStorageValue));
    $(".pricing-plan-trasfer", $businessCard).text(
      l.bsn_add_base_stg_trs.replace("%1", storagePrice).replace("*", hasLocalPrice ? "*" : "")
    );
    ...
    $(".pricing-flexi-block-card-note, .pricing-flexi-block-card-note-s", $businessPlans).toggleClass("hidden", !hasLocalPrice);
  ...
  const fetchBusinessPlanInfo = async () => {
    ...
    return business.getBusinessPlanInfo(false).then((info) => {
      ...
      pro.proplan.businessPlanData.isValidBillingData =
        info.bd &&
        info.bd.us &&
        (info.bd.us.p || info.bd.us.lp) &&
        info.bd.sto &&
        (info.bd.sto.p || info.bd.sto.lp) &&
        info.bd.sto.s &&
        info.bd.trns &&
        (info.bd.trns.p || info.bd.trns.lp) &&
        info.bd.trns.t &&
        info.bd.ba.s &&
        info.bd.ba.t;

      // If local currency values exist
      pro.proplan.businessPlanData.isLocalInfoValid =
        info.l && info.l.lcs && info.l.lc && info.bd.us.lp && info.bd.sto.lp && info.bd.trns.lp;
  ...
  const updatePriceCards = () => {
    ...
    if (u_type > 1 && u_attr) {
      ...
      if (localStorage.keycomplete) {
        ...
        $(".pricing-pg.pricing-banner-container, .pricing-pg.pricing-get-started-container", $page).addClass("hidden");
      }
      ...
      if (pro.proplan2.storageData && pro.proplan2.storageData.used) {
        ...
        for (const plan of plans) {
          const currPlanLevel = plan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] <= 3 ? plan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] : 0;
