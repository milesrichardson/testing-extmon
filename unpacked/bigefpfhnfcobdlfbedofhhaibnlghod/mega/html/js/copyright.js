...
copyright.init_cn = function () {
    ...
    if ($("input.copyrightowner", ".cn-form").val() === "") {
      ...
      api_req(
        ...
        },
        {
          callback: function () {
            loadingDialog.hide();
            msgDialog("info", escapeHTML(l[1287]), escapeHTML(l[1288]), false, function () {
              loadSubPage("copyright");
            });
          }
        }
  ...
  for (var country in countries) {
    if (countries.hasOwnProperty(country)) {
      markup += '<option value="' + escapeHTML(country) + '">' + escapeHTML(countries[country]) + "</option>";
    }
  }
      ...
      api_req(requestParameters, {
        callback: function (response) {
          ...
          if (!isNaN(parseFloat(response)) && isFinite(response)) {
            ...
            if (response === -9) {
              // ENOENT error
              msgDialog("warninga", escapeHTML(l[135]), escapeHTML(l[8818]), false);
              return false;
            }
            if (response === -12) {
              // EEXIST error, they have already made a dispute for this link
              msgDialog("warninga", escapeHTML(l[135]), escapeHTML(l[8819]), false);
              return false;
            } else if (response === -11) {
              // Access
              msgDialog("warninga", escapeHTML(l[135]), escapeHTML(l[8820]), false);
              return false;
            } else {
              // Generic (probably args)
              msgDialog("warninga", escapeHTML(l[135]), escapeHTML(l[47]), false);
              return false;
            }
          }
...
copyright.markInputWrong = function (elm) {
  ...
  $(elm)
    ...
    .rebind("click", function () {
      $(this).off("click").removeClass("red").css("font-weight", "normal").parent().css("box-shadow", "none");
    })
    .parent()
    .css("box-shadow", "0px 0px 8px #f00");
};
