...
var loginFromEphemeral = {
  ...
  completeLogin: function (result) {
    ...
    if (security.login.checkForCommonErrors(result, loginFromEphemeral.old.startLogin, loginFromEphemeral.new.startLogin)) {
      return false;
    }
...
function registeraccount() {
  ...
  const signup = () => {
    security.register.startRegistration(rv.first, rv.last, rv.email, rv.password, false, continueNewRegistration);
  };
  ...
  if (u_type === 0) {
    ...
    M.getCopyNodes([M.RootID], null, names).then((nodes) => {
  ...
  if (!err) {
    if ($(".understand-check", $formWrapper).hasClass("checkboxOff")) {
      ...
      if (localStorage.businessSubAc) {
        ...
        u_checklogin(ctx, true, null, signupcode, $firstName.val() + " " + $lastName.val());
