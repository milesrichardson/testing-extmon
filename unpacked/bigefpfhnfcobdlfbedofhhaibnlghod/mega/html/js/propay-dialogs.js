...
var astroPayDialog = {
  ...
  updateDialogDetails: function () {
    ...
    this.$dialog
      .find(".provider-icon")
      .removeClass()
      .addClass("provider-icon " + gatewayName);
  ...
  taxNumberIsValid: function () {
    ...
    else if (taxLabel === "CPF" && (astroPayDialog.cpfIsValid(taxNumCleaned) || astroPayDialog.cpnjIsValid(taxNumCleaned))) {
      return true;
    } else {
      return false;
    }
...
var voucherDialog = {
  ...
  setDialogDetails: function () {
    ...
    this.$dialog
      .find(".plan-icon")
      .removeClass("pro1 pro2 pro3 pro4")
      .addClass("pro" + proNum);
    ...
    M.require("redeem_js")
      ...
      .then(({ data, res }) => {
        ...
        voucherDialog.getLatestBalance(function () {
          ...
          voucherDialog.$dialog.find(".voucher-account-balance .balance-amount").text(formatCurrency(balance));
          voucherDialog.$dialog.find(".voucher-account-balance .new-balance-amount").text(formatCurrency(newBalance));
    ...
    if (pro.propay.selectedProPackage !== null) {
      ...
      if (proPrice) {
        ...
        if (discountInfo && ((numOfMonths === 1 && discountInfo.emp) || (numOfMonths === 12 && discountInfo.eyp))) {
          proPrice = numOfMonths === 1 ? mega.intl.number.format(discountInfo.emp) : mega.intl.number.format(discountInfo.eyp);
        }
...
var addressDialog = {
  ...
  init: function (plan, userInfo, businessRegisterPage) {
    ...
    this.fetchBillingInfo().always(function (billingInfo) {
      ...
      const selectedState = ((billingInfo.country === "US" || billingInfo.country === "CA") && billingInfo.state) || false;
  ...
  showDialog: function () {
    ...
    if (!is_mobile) {
      ...
      if (specialLogos[gate]) {
        ...
        $(".payment-provider-icon", $paymentIcons).addClass("hidden").removeClass("stripeAE stripeJC stripeUP stripeDD");
      }
    }
    ...
    if (!this.businessPlan || !this.userInfo) {
      ...
      if (this.businessPlan.pastInvoice && this.businessPlan.currInvoice) {
        ...
        proPrice = (
          this.userInfo.nbOfUsers * this.businessPlan.userFare +
          (this.userInfo.quota ? this.userInfo.quota * this.businessPlan.quotaFare : 0)
        ).toFixed(2);
      }
    ...
    // Update template
    this.$dialog.find(".plan-icon").removeClass("pro1 pro2 pro3 pro4 pro101 business").addClass(proNum);
    ...
    var changeStates = function (selectedCountryCode) {
      ...
      if (addressDialog.localePostalCodeName[selectedCountryCode]) {
        if ($titleElemPostCode.length) {
          $postcodeInput.updateTitle(addressDialog.localePostalCodeName[selectedCountryCode]);
        } else {
  ...
  fetchBusinessInfo: function (requiredAttributes) {
    ...
    var promise = new MegaPromise();
    requiredAttributes = requiredAttributes || ["%name", "%address1", "%address2", "%city", "%state", "%country", "%zip"];
    var done = 0;
    var timeout = null;
  ...
  stripePaymentChecker: function (saleId) {
    ...
    api
      ...
      .then(({ result }) => {
        ...
        if (typeof result === "string") {
          ...
          if (addressDialog.userInfo && !addressDialog.userInfo.isUpgrade) {
            ...
            $("button.js-close, .btn-close-dialog", $stripeSuccessDialog).removeClass("hidden").rebind("click.stripeDlg", closeDialog);
      ...
      .catch((ex) => {
        if (ex === ENOENT) {
          pro.propay.paymentStatusChecker = setTimeout(addressDialog.stripePaymentChecker.bind(addressDialog, saleId), nextTick);
        } else {
          tell(ex);
        }
      });
  ...
  stripeFrameHandler: function (event) {
    ...
    const failHandle = (error) => {
      ...
      if (addressDialog.stripeSaleId === "EDIT") {
        $(is_mobile ? ".fail-head" : ".payment-stripe-failure-dialog-title", $stripeFailureDialog).text(l.payment_gw_update_fail);
        $(".err-txt", $stripeFailureDialog).safeHTML(
          l.payment_gw_update_fail_desc.replace("[A]", '<a href="mailto:support@mega.nz">').replace("[/A]", "</a>")
        );
      }
    ...
    if (event && event.origin === addressDialog.gatewayOrigin && event.data) {
      ...
      if (event.data.startsWith("payfail^")) {
        ...
        if (addressDialog.stripeSaleId === "EDIT") {
          ...
          pro.propay.paymentStatusChecker = setTimeout(
            addressDialog.stripePaymentChecker.bind(addressDialog, addressDialog.stripeSaleId),
            500
          );
        }
  ...
  stripeLocal: function () {
    ...
    switch (lang) {
      ...
      case "kr":
      case "vi":
        return "en"; // no support for Korean and Vietnamese
      default:
        return lang;
    }
  },
    ...
    if (isStripe) {
      ...
      if (utcResult.EUR) {
        ...
        const $iframeContainer = $(
          ".mobile.payment-stripe-dialog .iframe-container," + " .mega-dialog.payment-stripe-dialog .iframe-container"
        );
        let $stripeIframe = $("iframe#stripe-widget", $stripeDialog);
        $stripeIframe.remove();
        const sandBoxCSP = "allow-scripts allow-same-origin allow-forms" + (utcResult.edit ? " allow-popups" : "");
  ...
  discardCreditCardUpdate: function () {
    ...
    msgDialog("confirmation", "", l.close_credit_card_dialog, l.close_credit_card_dialog_confirmation, (status) => {
      if (status) {
        closeStripeDialog();
      }
...
var cardDialog = {
  ...
  getBillingDetails: function () {
    ...
    // Check all the card details are completed
      ...
      !billingData.expiry_date_month ||
      !billingData.expiry_date_year ||
      !billingData.cv2
    ) {
      msgDialog("warninga", l[6958], l[6959], "", function () {
        cardDialog.$backgroundOverlay.removeClass("hidden").addClass("payment-dialog-overlay");
...
var bitcoinDialog = {
  ...
  showInvoice: function (apiResponse) {
    ...
    const planMonths = (pro.propay.selectedProPackage[4] === 1 ? l.bcoin_plan_month_one : l.bcoin_plan_month_mul).replace(
      "%1",
      pro.propay.selectedProPackage[4]
    ); // %1-month purchase
    ...
    const discountInfo = pro.propay.getDiscount();
    if (discountInfo && ((numOfMonths === 1 && discountInfo.emp) || (numOfMonths === 12 && discountInfo.eyp))) {
      priceEuros = numOfMonths === 1 ? mega.intl.number.format(discountInfo.emp) : mega.intl.number.format(discountInfo.eyp);
    }
