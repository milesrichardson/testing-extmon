...
var troyhuntCampaign = {
  ...
  initTroyHuntPage: function (code, details, subpage) {
    ...
    $(".thunt-page:not(." + subpage + ")", this.$campaignPage)
      .addClass("hidden")
      .removeClass("active");
    $(".thunt-page." + subpage, this.$campaignPage)
      .removeClass("hidden")
      .addClass("active");
    if (subpage === "business") {
      this.lPage = "B";
    ...
    $(".thunt-page.business .thunt-content.top-header", this.$campaignPage).text(mega.icu.format(l.troy_business_try, details.bm));
    $(".thunt-page.business .get-voucher-label, .thunt-page.business .thunt-form-label", this.$campaignPage).text(
      mega.icu.format(l.troy_business_free, details.bm)
    );
    $(".thunt-page.business .feature-card.generous-storage .feature-card-explain.ext", this.$campaignPage).text(
      mega.icu.format(l.troy_generous_storage_desc4, details.bm)
    );

    $(".thunt-page.pro .thunt-content.top-header", this.$campaignPage).text(mega.icu.format(l.troy_pro_try, details.pm));
    $(".thunt-page.pro .get-voucher-label, .thunt-page.pro .thunt-form-label", this.$campaignPage).text(
      mega.icu.format(l.troy_pro_free, details.pm)
    );
  },
  ...
  showMessage: function (message, error) {
    ...
    var appendString = error ? '<i class="thunt-icon thunt-sprite white-triangle"></i><span> @@</span>' : "<span>@@</span>";
  ...
  sendFormData: function () {
    ...
    api_req(request, {
      callback: function (res) {
        if (res === 0) {
          troyhuntCampaign.showMessage(l.troy_success);
        } else {
