...
function dlinfo(ph, key, next) {
  ...
  if (dl_res) {
    setupSingleDownloadPage(dl_res).catch(tell);
...
async function setupSingleDownloadPage(res) {
  ...
  if (res === ETOOMANY) {
    ...
    if (fdl_file.name) {
      ...
      var $pageScrollBlock = $(is_mobile ? ".mega-overlay-view.mega-component" : ".download.download-page").removeClass(
        "video video-theatre-mode downloading resumable txtfile image"
      );
      ...
      $(".js-default-download, .js-standard-download, .js-megasync-download," + ".js-resume-download, .js-save-download").rebind(
        "click",
        (event) => {
          if (event.detail > 1) {
            return false;
          ...
          if (event.currentTarget.classList.contains("js-megasync-download")) {
            ...
            megasync.isInstalled((err, is) => {
              ...
              if (res.msd !== 0 && (!err || is)) {
                ...
                megasync.download(
                  dlpage_ph,
                  a32_to_base64(base64_to_a32(dlkey).slice(0, 8)),
                  (err) => {
                    if (err) {
                      setStandardDownloadOptions();
            ...
            watchdog.query("dling").always((res) => {
              var proceed = true;
              ...
              if (proceed) {
                dlmanager.getFileSizeOnDisk(dlpage_ph, filename).always((size) => {
                ...
                $(".download.state-text.resume").addClass("hidden");
                $(".download.download-page").removeClass("download-complete resumable").addClass("downloading");
      ...
      $(".info-block .block-view-file-type, .download .bar-cell .transfer-filetype-icon").addClass(fileIcon({ name: filename }));
      ...
      return mKeyDialog(dlpage_ph, false, key).catch(() => {
...
function dlprogress(fileid, perc, bytesloaded, bytestotal, kbps, dl_queue_num) {
  ...
  Object(fdl_queue_var).lastProgress = [fileid, perc, bytesloaded, bytestotal, kbps, dl_queue_num];
  ...
  if (!m) {
    ...
    if ($(".dark", $sizeBlock).length === 0) {
      $sizeBlock.safeHTML('<span class="dark">' + "</span>" + "<hr />" + '<span class="fs">' + fileSize + "</span>");
...
function sync_switchOS(os) {
  ...
  if (os === "windows") {
    ...
    $(".sync-bottom-txt").safeHTML(
      'Also available for <a href="" class="red windows">Windows</a> and <a href="" class="red linux">Linux</a>'
    );
...
function dlPageCleanup() {
  ...
  if (!is_mobile && typeof fdl_queue_var !== "undefined") {
    ...
    if (mega.tpw && $tpwRows.length) {
      mega.tpw.removeRow(
        $tpwRows.toArray().map((a) => String(a.id).replace("tpw_", "")),
        false
      );
    }
  }
