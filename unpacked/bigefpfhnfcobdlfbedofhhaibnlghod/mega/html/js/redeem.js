...
var redeem = {
  ...
  init: function () {
    ...
    this.getVoucherData()
      ...
      .then(() => {
        ...
        return api.req({ a: "promoter" in data ? "epcr" : "uavr", v: data.code, p: data.promoter }).then(() => {
          if (data.promotional) {
  ...
  displayDialog: function () {
    ...
    // Update information
    $(".plan-icon", redeem.$dialog)
      .removeClass("pro1 pro2 pro3 pro4 pro101 business")
      .addClass("pro" + proNum);
    $(".plan-title", redeem.$dialog).safeHTML(proName);
    $(".price", redeem.$dialog).text(intl.format(planPriceDollars) + decimalSeparator + intl.format(planPriceCents) + euroSign);
  ...
  processProPurchaseWithBalance: function () {
    ...
    api
      .screq({ a: "uts", it: 0, si: apiId, p: price, c: currency, aff })
      .then(({ result: saleId }) => {
  ...
  showSuccessfulPayment: function (promo) {
    ...
    if (vd.businessmonths && vd.proNum === pro.ACCOUNT_LEVEL_BUSINESS) {
      ...
      $(".promo-voucher-card", $voucherBlock).removeClass("red-block").removeClass("yellow-block").addClass("blue-block");
      ...
      $voucherIcon.removeClass("pro1 pro2 pro3 pro4 pro101 business").addClass("pro" + vd.proNum);

      if (vd.proNum === pro.ACCOUNT_LEVEL_PRO_LITE) {
        $(".promo-voucher-card", $voucherBlock).removeClass("red-block").removeClass("blue-block").addClass("yellow-block");
      } else {
        $(".promo-voucher-card", $voucherBlock).removeClass("yellow-block").removeClass("blue-block").addClass("red-block");
      }
      ...
      $(".storage-amount", $voucherBlock).safeHTML(l[23789].replace("%1", bytesToSize(vd.storage * 0x40000000, 0)));
      $(".transfer-amount", $voucherBlock).safeHTML(l[23790].replace("%1", bytesToSize(vd.bandwidth * 0x40000000, 0)));
    }
  ...
  showVoucherInfoDialog: function () {
    ...
    var infoFilling = function ($dlg) {
      ...
      if (mega.voucher.businessmonths && mega.voucher.promotional === redeem.PROMO_VOUCHER_BUSINESS) {
        ...
        let voucherType = pro.getProPlanName(mega.voucher.hasOwnProperty("item") ? mega.voucher.item.al : mega.voucher.proNum);
        ...
        if (mega.voucher.s4) {
          ...
          const $termsCheck = $(".s4-voucher-terms-check", $dlg).addClass("checkboxOff").removeClass("hidden checkboxOn");
          const termsToggle = () => {
      ...
      $whiteBtn.rebind("click", function () {
        if (!window.bCreatedVoucher && u_type !== 3) {
          login_txt = l[7712];
          loadSubPage("login");
        } else {
      ...
      $greenBtn.rebind("click", function () {
        if ($(this).hasClass("disabled")) {
          return false;
        }
        closeDialog();

        if (window.busUpgrade || window.bCreatedVoucher || (mega.voucher.businessmonths && u_attr && u_attr.b)) {
          loadSubPage("redeem");
          return false;
        }
      ...
      $("button.js-close, .close-voucher-redeem", $dlg).rebind("click.redeem", function () {
  ...
  setupVoucherInputbox: function (code) {
    ...
    $button.rebind("click", function () {
      ...
      redeem
        .getVoucherData($input.val(), promoter)
        .then(function (data) {
          mega.voucher = data;
          page = "";
      ...
      self
        ...
        .then(function (data) {
          var redeem = function () {
            api
              .req({ a: "uavr", v: code })
              .then(({ result: res }) => resolve({ res, data }))
              .catch(reject);
          };
