...
RepayPage.prototype.initPage = function () {
  ...
  if (u_attr.pf) {
    $(".plan-icon", $rightSection).removeClass("icon-crests-business-details").addClass("icon-crests-pro-flexi-details");
  ...
  $(".bus-reg-radio-option", $paymentBlock).rebind("click.suba", function businessRepayCheckboxClick() {
    var $me = $(this);
    $me = $(".bus-reg-radio", $me);
  ...
  $(".bus-reg-agreement.mega-terms .checkdiv", $leftSection).removeClass("checkboxOn").addClass("checkboxOff");

  // event handler for check box
  $(".bus-reg-agreement", $leftSection).rebind("click.suba", function businessRepayCheckboxClick() {
  ...
  const fillPaymentGateways = function (status, list) {
    ...
    const radioHtml =
      '<div class="bus-reg-radio-option"> ' + '<div class="bus-reg-radio payment-[x] radioOff" prov-id="[Y]" gate-n="[Z]"></div>';
    ...
    for (let k = 0; k < list.length; k++) {
      const payRadio = radioHtml.replace("[x]", list[k].gatewayName).replace("[Y]", list[k].gatewayId).replace("[Z]", list[k].gatewayName);
    ...
    $(".bus-reg-radio-block .bus-reg-radio", $repaySection).first().removeClass("radioOff").addClass("radioOn");

    // event handler for radio buttons
    $(".bus-reg-radio-option", $paymentBlock).rebind("click.suba", function businessRegCheckboxClick() {
    ...
    overduePromise.done(function (st, res) {
      ...
      const applyFormat = (val) => {
        if (sep !== res.l.sp[0]) {
          ...
          val = val.replace(reg1, "-").replace(reg2, res.l.sp[0]).replace(/-/g, res.l.sp[1]);
        }

        val = res.l.pl ? `${res.l.cs}${val}` : `${val}${res.l.cs}`;
      ...
      const showDetails = function () {
        ...
        if ($me.hasClass("expand")) {
          ...
          $me.nextUntil(".repay-breakdown-tb-content, .repay-breakdown-tb-total", ".repay-extra-details").removeClass("expand");
          $(".content-desc-container", $me).removeClass("icon-arrow-up-after").addClass("icon-arrow-down-after");
        } else {
          $me.addClass("expand");
          $me.nextUntil(".repay-breakdown-tb-content, .repay-breakdown-tb-total", ".repay-extra-details").addClass("expand");
          $(".content-desc-container", $me).removeClass("icon-arrow-down-after").addClass("icon-arrow-up-after");
        }
      };
      ...
      const $extraRowTemplate = $overdueExtraRow.length > 1 ? $($overdueExtraRow[0]).clone() : $overdueExtraRow.clone();
      ...
      const addDetailsRow = ($template, text, item, $parent) => {
        if (item && item[0] && item[1] && text && $template && $parent) {
          ...
          $(".repay-extra-desc", $row).text(mega.icu.isICUPlural(text) ? mega.icu.format(text, item[0]) : text.replace("%1", item[0]));
          $(".repay-extra-val", $row).text(item[2] ? applyFormat(intl.format(item[2])) : `${intl.format(item[1])} \u20ac`);
      ...
      if (res.inv[0].list) {
        ...
        if (u_attr.pf) {
          ...
          $overduePaymentRow.nextUntil(".repay-breakdown-tb-content, .repay-breakdown-tb-total", ".repay-extra-details").addClass("expand");
          $(".content-desc-container", $overduePaymentRow).removeClass("icon-arrow-down-after icon-arrow-up-after");
        } else {
      ...
      if ((res.nb || u_attr.pf) && futureAmount) {
        ...
        if (u_attr.pf) {
          ...
          $futurePaymentRow.nextUntil(".repay-breakdown-tb-content, .repay-breakdown-tb-total", ".repay-extra-details").addClass("expand");
          $(".content-desc-container", $futurePaymentRow).removeClass("icon-arrow-down-after icon-arrow-up-after");
        } else {
          // For Business, add a users row and make the row clickable
      ...
      business
        .getBusinessPlanInfo(false, u_attr.pf)
        .then((plan) => {
