...
var pro_json =
  '[[["N02zLAiWqRU",1,500,1024,1,"9.99","EUR"],["zqdkqTtOtGc",1,500,1024,12,"99.99","EUR"],["j-r9sea9qW4",2,2048,4096,1,"19.99","EUR"],["990PKO93JQU",2,2048,4096,12,"199.99","EUR"],["bG-i_SoVUd0",3,4096,8182,1,"29.99","EUR"],["e4dkakbTRWQ",3,4096,8182,12,"299.99","EUR"]]]';

pages.placeholder =
  '<div class="bottom-page scroll-block placeholder selectable-txt">' +
  "((TOP))" +
...
function topMenuDataUpdate(data) {
  ...
  if (u_attr && (u_attr.b || u_attr.pf)) {
    ...
    storageHtml = l[1607].replace("%1", "<span>" + space_used + "</span>").replace("%2", space);
  }
...
function topPopupAlign(button, popup, topPos) {
  ...
  const popupAlign = () => {
    ...
    if ($button.length && $popup.length) {
      ...
      popupLeftPos = $button.offset().left + $button.outerWidth() / 2 - $popup.outerWidth() / 2;
      if (topPos) {
      ...
      if (popupLeftPos > 10) {
        if (popupLeftPos + $popup.outerWidth() + 10 > pageWidth) {
          ...
          $popupArrow.css("left", $button.offset().left - $button.outerWidth() / 2);
        } else {
          $popup.css("left", popupLeftPos + "px");
        }
      } else {
        $popup.css("left", "10px");
        arrowLeftPos = $button.offset().left - $button.outerWidth() / 2;
        $popupArrow.css({
...
function init_page() {
  ...
  if (page.length > 2 && (page[0] === "!" || pageBeginLetters === "F!")) {
    ...
    page =
      page[0] === "F"
        ? page.replace("F!", "folder/").replace("!", "#").replace("!", "/folder/").replace("?", "/file/")
  ...
  if (page.substr(0, 5) === "file/") {
    ...
    if (M.hasPendingTransfers() && $.lastSeenFilelink !== getSitePath()) {
      ...
      M.abortTransfers()
        ...
        .catch(() => loadSubPage($.lastSeenFilelink));
      return;
    }
  ...
  if (is_chatlink || page.substr(0, 5) === "chat/") {
    if (fminitialized && megaChatIsReady) {
      ...
      assert(megaChat.initialChatId, "missing .initialChatId, did this page initialized from a standalone chat/meeting link?");
    ...
    if (!publicChatHandle || publicChatHandle.length !== 8 || !publicChatKey || publicChatKey.length !== 22) {
      return u_type ? loadSubPage("fm/chat") : mega.redirect("mega.io", "chatandmeetings", false, false);
    }
    ...
    const init = () => {
      init_chat(0x104df11e5)
        ...
        .then(() => {
          document.querySelector(".chat-links-preview .chat-links-logo-header a.logo").addEventListener("click", () => {
            is_chatlink = false;
    ...
    if (u_type) {
      // show loading
      for (const node of document.querySelectorAll(".section.chat-links-preview, .section.chat-links-preview .fm-chat-is-loading")) {
        node.classList.remove("hidden");
      }

      // init authring -> init chat
      authring.onAuthringReady().then(init, (ex) => {
  ...
  if (page.startsWith("folder/") || page.startsWith("collection/")) {
    ...
    if (!flhashchange || pfkey !== oldPFKey || pfkey.length !== 22 || pfid.length !== 8) {
      ...
      const data = JSON.stringify([
        ...
        pfkey.length | 0,
        pfid.length | 0,
        window[`preflight-folder-link-error:${pfid}`] | 0
      ]);
      ...
      if (pfkey.length === 22) {
        ...
        mKeyDialog(pfid, true, pfkey, newLinkSelector).catch(() => {
          loadSubPage("start");
        });
  ...
  // If user has been invited to join MEGA and they are not already registered
    ...
    if (hashChecksum === byteString) {
      ...
      // Show message
      alert(
        "We can't decipher your invite link, please check you copied the link correctly, or sign up manually with the same email address."
      );
    }
    ...
    loadingDialog.show();
    security.register
      .verifyEmailConfirmCode(confirmcode)
      .then(({ email }) => {
        page = "login";
        confirmok = true;
  ...
  } else if (page.substr(0, 9) === "registerb") {
    // business register
    ...
    api_req(
      { a: "ufpqfull", t: 0, d: 0 },
      {
        callback: function (gatewayOptions) {
          ...
          if (u_type === 3) {
            wireTransferDialog.init(function onClose() {
              loadSubPage("fm");
            });
          } else if (is_mobile) {
  ...
  // Page for mobile to let the user change their password and finish parking their account
    ...
    if (
      (typeof u_attr !== "undefined" && u_attr.b && u_attr.b.m && pro.isExpiredOrInGracePeriod(u_attr.b.s)) ||
  ...
  } else if (page === "mobile" || page === "android" || page === "ios" || page === "uwp" || page === "wp" || page === "mobileapp") {
      ...
      if (u_type === 0 && !u_attr.terms && !is_eplusplus) {
        ...
        if (!is_mobile) {
          M.safeShowDialog("terms", () => {
            msgDialog(`confirmation:!^${l[1037]}!${l[82]}`, "", l.terms_dialog_title, l.terms_dialog_text, (e) => {
              if (e) {
                $.termsAgree();
              } else {
                $.termsDeny();
...
function topbarUI(holderId) {
  ...
  $(".js-accountbtn", topbar).rebind("click.topAccBtn", function () {
    if (this.classList.contains("settings")) {
      ...
      window.open("https://survey.mega.co.nz/index.php?r=survey/index&sid=765691&lang=en", "_blank", "noopener,noreferrer");
    }
...
function topmenuUI() {
  ...
  if (u_type > 0) {
    ...
    if (u_attr.b || u_attr.pf) {
      $(".plan", $menuLoggedBlock).text(pro.getProPlanName(u_attr.b ? pro.ACCOUNT_LEVEL_BUSINESS : pro.ACCOUNT_LEVEL_PRO_FLEXI));
  ...
  $.hideTopMenu = function (e) {
    ...
    if (
      !e ||
      (!e.target.closest(".top-menu-popup, .js-more-menu") &&
        elements.length &&
    ...
    if (!e || (e.target.closest(".top-menu-popup") && (!c || (!c.includes("activity-status") && !c.includes("loading"))))) {
      $headerActivityBlock.removeClass("active");
    }

    if (!e || (!e.target.closest(".top-login-popup") && (!c || (!c.includes("top-login-popup") && !c.includes("top-login-button"))))) {
      $topHeader.find(".dropdown.top-login-popup").addClass("hidden");
    }

    if (!e || (!e.target.closest(".create-new-folder") && (!c || !c.includes("fm-new-folder")))) {
    ...
    if ((!e || !e.target.closest(".fm-add-user, .add-user-popup")) && (!c || c.indexOf("fm-add-user") === -1)) {
    ...
    if (!e || (!e.target.closest(".js-dropdown-notification") && ((c && c.indexOf("js-topbarnotification") === -1) || !c))) {
      notify.closePopup();
    }

    if (!e || (!e.target.closest(".js-dropdown-warning") && ((c && c.indexOf("js-dropdown-warning") === -1) || !c))) {
    ...
    if (!e || (!e.target.closest(".js-dropdown-account") && ((c && c.indexOf("js-topbaravatar") === -1) || !c))) {
  ...
  $(".top-user-status-popup .dropdown-item", $topHeader).rebind("click.topui", function (e) {
    var $this = $(this);

    if ($this.attr("class").indexOf("active") === -1) {
      $topHeader.find(".top-user-status-popup .dropdown-item").removeClass("active");
      $headerActivityBlock.removeClass("active");
  ...
  $(".nw-fm-left-icon, .js-top-buttons")
    .rebind("mouseover.nw-fm-left-icon", function () {
      var $this = $(this);
      ...
      $.liTooltipTimer = window.setTimeout(function () {
...
function is_fm() {
  ...
  if (!r && u_type !== false) {
    r = page === "" || page === "start" || page === "index" || page.substr(0, 2) === "fm" || page.substr(0, 7) === "account";
  }
...
window.addEventListener(
  ...
  () => {
    ...
    if (window.fmdb && window.currsn && fminitialized && Object(fmdb.pending).length && Object.keys(fmdb.pending[0] || {}).length) {
      setsn(currsn);
      return l[16168];
    }
...
mBroadcaster.once("startMega", () => {
  ...
  const onStateChange = (event) => {
    const nextState =
      event.type === "pagehide" ? (event.persisted ? "frozen" : "terminated") : event.type === "freeze" ? "frozen" : getState();
    ...
    Object.defineProperty(mega, "ensureAccessibility", {
      ...
      value: tryCatch(
        ...
        (ex) => {
          ...
          const data = tryCatch(() =>
            [...document.body.children].map((n) => n.tagName + (n.id ? `#${n.id}` : `.${n.classList[0] || ""}`))
          )();
  ...
  if (d) {
    ...
    (function memoryMeasurement() {
      var performMeasurement = tryCatch(function () {
        performance.measureMemory().then(function (result) {
