...
var buildVersion = {
  ...
  firefox: "4.24.0",
  firefoxSize: 8599527,
  commit: "20123c03515eea570ce1379ee12d3096666742bb",
  timestamp: 1702950640,
  dateTime: "Tue, 19 Dec 2023 14:50:40 +1300"
};
...
var is_livesite = location.host === "mega.nz" || location.host === "mega.io" || location.host === "smoketest.mega.nz" || is_extension;
...
function geoStaticPath(cms) {
  ...
  try {
    ...
    if (!sessionStorage.skipGeoStaticPath) {
      ...
      var northAmericaStaticCountries =
        "AG AI AR BB BL BO BR BS BZ CA CL CO CO CR CU DO EC FK GD GF GL GT GY HN HT IS JM KN LC MX NI PA PE PR PY SR SR TT US UY VC VE VE VG VI";
...
browserUpdate =
  browserUpdate ||
  (is_embed ? typeof ReadableStream === "undefined" || typeof IntersectionObserver === "undefined" : typeof BigInt === "undefined");
...
if (!browserUpdate)
  try {
    try {
      ...
      cookiesDisabled =
        (ex.code && ex.code === DOMException.SECURITY_ERR) || ex.message === "SecurityError: DOM Exception 18" || storageQuotaError;
    ...
    if (!m || !cookiesDisabled) {
      ...
      if (storageQuotaError) {
        extraInfo = "\n\nTip: We've detected this issue is likely caused by " + "browsing in private mode, please try turning it off.";
      } else if (cookiesDisabled) {
        extraInfo =
          "\n\nTip: We've detected this issue is likely related to " + "having Cookies disabled, please check your browser settings.";
      }
      alert(
        "Sorry, we were unable to initialize the browser's local storage, " +
          ...
          (e.message || e) +
          "\nBrowser: " +
          (typeof mozBrowserID !== "undefined" ? mozBrowserID : ua) +
          extraInfo
      );
      browserUpdate = 1;
    }
...
var mega = {
  ...
  browserBrand: [0, "Torch", "Epic", "Edgium"],
  whoami:
    "We make secure cloud storage simple. Create an account and get up to 50 GB " +
...
Object.defineProperty(mega, "paywall", {
  get: function () {
    "use strict";
    return (typeof u_attr === "object" && (u_attr.uspw || (u_attr.b && u_attr.b.s === -1) || (u_attr.pf && u_attr.pf.s === -1))) || false;
  }
});
...
var isStaticPage = function (page) {
  ...
  if (page) {
    ...
    if (
      excludedPages[page] ||
      page.indexOf("businesssignup") > -1 ||
      page.indexOf("businessinvite") > -1 ||
      page.indexOf("about/jobs") > -1
    ) {
      return false;
    }
  ...
  removeListener: function mBroadcaster_removeListenr(token, listener) {
    ...
    if (listener) {
      ...
      for (var id in this._topics[token]) {
        if (this._topics[token].hasOwnProperty(id) && this._topics[token][id].scope === listener) {
          found = id;
          break;
        }
      }
    ...
    if (this._topics[topic]) {
      ...
      for (var id in this._topics[topic]) {
        ...
        try {
          ...
          if (typeof reportError === "function" && buildVersion.timestamp * 1000 + 8e7 > Date.now()) {
            reportError(ex);
          }
        }
        if (ev.once || rc === 0xdead) idr.push(id);
      }
      if (idr.length) {
    ...
    initialize: function crossTab_init(cb) {
      var setup = function (ev) {
        ...
        if (cb && (!ev || msg === "pong")) {
          ...
          this.listen();
          if (d) {
            console.log("CROSSTAB COMMUNICATION INITIALIZED AS " + (this.master ? "MASTER" : "SLAVE"));
  ...
  if (hashFromWorker === hashFromDeployment) {
    ...
    console.error(
      "Hash mismatch on file: " +
        fileName +
        ". Hash from worker thread: " +
        hashFromWorker +
        " Hash from deployment script: " +
        hashFromDeployment
    );
    return false;
  }
...
function init_storage(storage) {
  ...
  if (v == 0) {
    ...
    if (storage.privk && storage.privk.substr(0, 1) == "[") {
      /* is json serialized array which need to be migrated */
      // Upgrade key format
      try {
        var privk = JSON.parse(storage.privk),
          str = "";
...
function logStaticServerFailure(errorType, filename, staticPathToLog) {
  ...
  onIdle(function () {
    ...
    xhr.send(JSON.stringify([{ a: "log", e: 99723, m: JSON.stringify([1, errorType, filename, staticPathToLog]) }]));
  });
}
...
function siteLoadError(error, filename) {
  ...
  message.push(
    "If the problem persists, please try disabling all third-party browser extensions, " +
  ...
  message.push(
    "BrowserID: " +
      ...
      staticpath +
      "\n" +
      "Flipped to default static: " +
      (staticServerLoading.flippedToDefault ? "yes" : "no") +
      "\n" +
      "Date/time: " +
...
var showLegacyMobilePage =
  m && (page.substr(0, 9) === "newsignup" || page.substr(0, 7) === "account" || (is_old_windows_phone && page.substr(0, 7) === "confirm"));
...
if (showLegacyMobilePage) {
  ...
  document.body.innerHTML =
    '<div class="bottom-page scroll-block selectable-txt"><div class="main-content-block">' +
  ...
  if (app) {
    ...
    document.getElementById("m_desc").innerHTML =
      "Otherwise, you can also open the link on a " + "desktop/laptop browser, or download the MEGA app.";
  } else {
    document.getElementById("m_desc").innerHTML = "Otherwise you can also open the link on a " + "desktop/laptop browser.";
  }
  ...
  if (ua.indexOf("chrome") > -1) {
    ...
    setTimeout(function () {
      var text = "This link should be opened in the MEGA app. " + "Click OK if you already have the MEGA app installed";
      if (confirm(text)) {
  ...
  if (!d && exTimeLeft && is_livesite) {
    ...
    window.onerror = function __MEGAExceptionHandler(msg, url, ln, cn, errobj) {
      ...
      var dump = {
          ...
          m: mTrim(msg)
            .replace(/'[a-z]+:\/+[^']+(?:'|$)/gi, function (url) {
              ...
              a.href = url;
              return "'" + ((a.origin !== "null" && a.origin) || a.protocol + "//" + a.hostname) + "...'";
            })
      ...
      dump.m =
        (is_mobile ? "[mobile] " : is_embed ? "[embed] " : window.pfid ? "[FL] " : mega.infinity ? "[INF] " : is_drop ? "[drop] " : "") +
        dump.m.replace(/\s+/g, " ");
      ...
      if (
        /NS_ERR|out[\s_]+of[\s_]+mem|dead\s+object|allocation\s+failed/i.test(dump.m) &&
      ...
      if (!expectedSourceOrigin) {
        ...
        if (!/SyntaxError|Script\serror/.test(dump.m)) {
          onIdle(function () {
            var xhr = getxhr();
            xhr.open("POST", apipath + "cs?id=0", true);
            xhr.send(JSON.stringify([{ a: "log", e: 99806, m: JSON.stringify([1, dump.m, url + ":" + ln]) }]));
          });
        }
        return;
      }
      ...
      if (errobj) {
        if (errobj.stack) {
          ...
          if (
            dump.s.indexOf("Unknown script code:") !== -1 ||
  ...
  if (is_drop) {
    ...
    if (
      typeof ReadableStreamDefaultController === "undefined" ||
  ...
  var jsl3 = {
    ...
    chat: {
      ...
      chat_history_retention_css: {
        f: "css/chat-history-retention.css",
        n: "chat_history_retention_css",
        j: 2,
        w: 2,
        c: 1,
        cache: 1,
  ...
  if (typeof Worker !== "undefined" && typeof window.URL !== "undefined" && !nocontentcheck) {
    var hashdata = [
      ...
      asmCryptoSha256Js,
      "self.onmessage = function(e) { try { var hashHex = asmCryptoSha256.SHA256.hex(e.data.text); e.data.hash = hashHex; self.postMessage(e.data); } catch(err) { e.data.error = err.message; self.postMessage(e.data);  } };"
    ];
    ...
    xhr_stack[xhri].onload = function () {
      try {
        ...
        if (!is_livesite) {
          ...
          if (entry && entry.j === 3 && entry.f.indexOf("_prod") === -1 && entry.text.indexOf("<!DOCTYPE html>") > -1) {
            console.warn("Lang file " + entry.f + " cannot be found. Switching to _prod lang file");
  ...
  function initall() {
    ...
    for (var i = 0; i < jsl.length; ++i) {
      if (jsl[i].j == 1 && !jj) {
        ...
        // new type to distinguish files to be used on iframes
        if (!window[jsl[i].n]) {
          var blobLink;
  ...
  mCreateElement("style", { type: "text/css" }, "body").textContent =
    ".div, span, input {outline: none;}.hidden {display: none;}.clear {clear: both;margin: 0px;padding: 0px;display: block;}.loading-main-block {width: 100%;height: 100%;position: fixed;z-index: 10000;font-family:Arial, Helvetica, sans-serif;}.main-blur-block,.bottom-page.scroll-block{display:none}.loading-mid-white-block {height: 100%;width:100%;}.loading-cloud {width: 222px;position: fixed;height: 158px;background-image: url(" +
    istaticpath +
    "loading-sprite_v4.png);background-repeat: no-repeat;background-position: 0 0;left:50%;top:50%;margin:-79px 0 0 -111px;}.loading-m-block{width:60px;height:60px;position:absolute; left:81px;top:65px;background-color:white;background-image: url(" +
    istaticpath +
    "loading-sprite_v4.png);background-repeat: no-repeat;background-position: -81px -65px;border-radius: 100%;-webkit-border-radius: 100%;border-radius: 100%;z-index:10;}.loading-percentage { width: 80px;height: 80px; background-color: #e1e1e1;position: absolute;-moz-border-radius: 100%;-webkit-border-radius: 100%;border-radius: 100%;overflow: hidden;background-image: url(" +
    istaticpath +
    "loading-sprite_v4.png);background-repeat: no-repeat;background-position: -70px -185px;left:71px;top:55px;}.loading-percentage ul {list-style-type: none;-moz-border-radius: 100%;-webkit-border-radius: 100%;border-radius: 100%;overflow: hidden;}.loading-percentage li {position: absolute;top: 0px;}.loading-percentage p, .loading-percentage li, .loading-percentage ul{width: 80px;height: 80px;padding: 0;margin: 0;}.loading-percentage span {display: block;width: 40px;height: 80px;}.loading-percentage ul :nth-child(odd) {clip: rect(0px, 80px, 80px, 40px);}.loading-percentage ul :nth-child(even) {clip: rect(0px, 40px, 80px, 0px);}.loading-percentage .right-c span {-moz-border-radius-topleft: 40px;-moz-border-radius-bottomleft: 40px;-webkit-border-top-left-radius: 40px;-webkit-border-bottom-left-radius: 40px;border-top-left-radius: 40px;border-bottom-left-radius: 40px;background-color:#dc0000;}.loading-percentage .left-c span {margin-left: 40px;-moz-border-radius-topright: 40px;-moz-border-radius-bottomright: 40px;-webkit-border-top-right-radius: 40px;-webkit-border-bottom-right-radius: 40px;border-top-right-radius: 40px;border-bottom-right-radius: 40px;background-color:#dc0000;}.loading-main-bottom {max-width: 940px;width: 100%;position: absolute;bottom: 20px;left: 50%;margin: 0 0 0 -470px;text-align: center;}.loading-bottom-button {height: 29px;width: 29px;float: left;background-image: url(" +
    istaticpath +
    "loading-sprite_v4.png);background-repeat: no-repeat;cursor: pointer;}.st-social-block-load {position: fixed;bottom: 20px;left: 0;width: 100%;height: 43px;text-align: center;}.st-bottom-button {height: 24px;width: 24px;margin: 0 8px;display: inline-block;background-image: url(" +
    istaticpath +
    "loading-sprite_v4.png);background-repeat: no-repeat;background-position:11px -405px;cursor: pointer;-moz-border-radius: 100%;-webkit-border-radius: 100%;border-radius: 100%;-webkit-transition: all 200ms ease-in-out;-moz-transition: background-color 200ms ease-in-out;-o-transition: background-color 200ms ease-in-out;-ms-transition: background-color 200ms ease-in-out;transition: background-color 200ms ease-in-out;background-color:#999999;}.st-bottom-button.st-google-button {background-position: 11px -405px;}.st-bottom-button.st-google-button {background-position: -69px -405px;}.st-bottom-button.st-twitter-button{background-position: -29px -405px;}.st-bottom-button:hover {background-color:#334f8d;}.st-bottom-button.st-twitter-button:hover {background-color:#1a96f0;}.st-bottom-button.st-google-button:hover {background-color:#d0402a;}@media only screen and (-webkit-min-device-pixel-ratio: 1.5), only screen and (-o-min-device-pixel-ratio: 3/2), only screen and (min--moz-device-pixel-ratio: 1.5), only screen and (min-device-pixel-ratio: 1.5) {.maintance-block, .loading-percentage, .loading-m-block, .loading-cloud, .loading-bottom-button,.st-bottom-button, .st-bottom-scroll-button {background-image: url(" +
    istaticpath +
    "loading-sprite_v4@2x.png);    background-size: 222px auto;}}";

  mCreateElement("div", { class: "loading-main-block", id: "loading" }, "body").innerHTML =
...
function makeUUID(a) {
  ...
  return a ? (a ^ ((Math.random() * 16) >> (a / 4))).toString(16) : ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, makeUUID);
}
