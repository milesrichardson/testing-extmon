    ...
    Object.defineProperty(self, "_connectionState", {
      ...
      set: function (newValue) {
        if (_connectionState !== newValue) {
          self.logger.warn("_connectionState = " + constStateToText(ConnectionRetryManager.CONNECTION_STATE, newValue));
...
ConnectionRetryManager.prototype.gotDisconnected = function () {
  ...
  if (self.options.functions.isDisconnected() === true && self.options.functions.isUserForcedDisconnect() === false) {
    self._connectionState = ConnectionRetryManager.CONNECTION_STATE.DISCONNECTED;
    ...
    $(window).rebind("online.megaChatRetry" + self._instanceIdx, function () {
      self._isOffline = false;
      if (!self.options.functions.isUserForcedDisconnect() && !self.options.functions.isConnected()) {
...
ConnectionRetryManager.prototype.startedConnecting = function (waitForPromise, delayed) {
  ...
  self._$connectingPromise
    ...
    .catch((ex) => {
      ...
      if (self._debug) {
        self.logger.warn("startedConnecting failed, retrying.", self.logger.name + "#" + self._connectionRetries, ex);
      }
      this._$connectingPromise = null;
...
ConnectionRetryManager.prototype.doConnectionRetryDelayed = function (base) {
  "use strict";
  tSleep((base | 1) + 3 * Math.random())
    .then(() => this.requiresConnection())
    .catch(dump);
};
...
ConnectionRetryManager.prototype.doConnectionRetry = function (immediately) {
  ...
  if (immediately !== true && self._connectionRetries > self.options.maxConnectionRetries) {
    ...
    var connectionRetryTimeout = Math.pow(self._connectionRetries, self.options.reconnectDelay / 1000) * 1000;

    // add some randomness
    connectionRetryTimeout =
      connectionRetryTimeout * rand_range(self.options.retryFuzzinesFactors[0], self.options.retryFuzzinesFactors[1]);
    ...
    if (self._debug) {
      self.logger.warn("calling startedConnecting: ", connectionRetryTimeout, self.logger.name + "#" + self._connectionRetries);
    }
