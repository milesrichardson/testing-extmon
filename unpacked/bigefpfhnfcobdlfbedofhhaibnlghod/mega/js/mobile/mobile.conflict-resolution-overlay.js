...
mobile.conflictResolutionDialog = {
  ...
  prompt: function (op, file, node, remaining, target) {
    ...
    const $replaceBtn = $(".action-replace-item", this.$overlay)
      .removeClass("hidden")
      .rebind("tap", () => {
    ...
    if (isFolder) {
      this.$overlay.find(".overlay-description-text").safeHTML(escapeHTML(l[17550]).replace("%1", "<strong>" + name + "</strong>"));
    } else {
      this.$overlay.find(".overlay-description-text").safeHTML(escapeHTML(l[16486]).replace("%1", "<strong>" + name + "</strong>"));
  ...
  _renderActionText: function (operation, isFolder) {
    ...
    var $actions = [".action-replace-item", ".action-skip-item", ".action-rename-item"].map(function (selector) {
      return {
