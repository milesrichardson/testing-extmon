...
mobile.overBandwidthQuota = {
  ...
  show: function (quotaExceeded) {
    ...
    contentsDiv.className = `obq-sheet-contents limited-bandwidth-dialog ${this.quotaExceeded ? "exceeded" : "limited"}`;
    ...
    if (!dlmanager.isDownloading && (dlmanager.isStreaming || videoPreview)) {
    ...
    const sheetBlurb = u_attr ? l[`${stringKey}${u_attr.p ? "_pro" : "_free"}`] : l[`${stringKey}_non_user`];
    ...
    if (Object(u_attr).p && this.quotaExceeded) {
      ...
      M.accountData((account) => {
        ...
        tbUsedDiv.append(parseHTML(escapeHTML(l.tq_percent_used).replace("%1", perc).replace("%2", tfsQuotaLimit[0])));
