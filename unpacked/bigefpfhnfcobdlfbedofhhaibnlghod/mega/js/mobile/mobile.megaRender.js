class MobileMegaRender {
  constructor(viewmode = M.viewmode) {
    ...
    if (this.container) {
      ...
      this.container.classList[
        M.currentdirid === "shares" || M.currentdirid === "out-shares" || M.currentdirid === "public-links" || !viewmode ? "remove" : "add"
      ]("grid-view");
    }
  ...
  getMListOptions() {
    ...
    if (!(M.currentdirid === "shares" || M.currentdirid === "out-shares" || M.currentdirid === "public-links") && M.viewmode) {
  ...
  handletouchend() {
    ...
    const {
      bottomBlock: { style, originalHeight }
    } = mega.ui.header;
    ...
    if (
      (originalHeight / 2 > parseInt(style.height || 0) || scrollTop + listHeight > scrollHeight - originalHeight / 2) &&
