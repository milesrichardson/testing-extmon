...
mobile.uploadOverlay = {
    ...
    if (Object.keys(this.uploadTransfers).length === 0) {
      ...
      if (this.actions) {
        for (let i = this.actions.length; i--; ) {
          const { type: type, text: text, className: className, onClick: onClick } = this.actions[i];
  ...
  getProgress: function () {
    ...
    const total =
      Object.keys(this.uploadTransfers).length + Object.keys(this.completedTransfers).length + Object.keys(this.erroredTransfers).length;
    ...
    return total === 1 ? l.upload_one : mega.icu.format(l.upload_total, completed).replace("%1", total);
  },
