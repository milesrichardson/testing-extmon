...
MobileContactLink.prototype.showContactLinkInfo = function _showContactLinkInfo() {
  ...
  var fillContactInfo = function _fillContactInfo(name, email, avatar, handle, ct) {
    ...
    if (avatar) {
      ...
      $(".avatar-wrapper.avatar img", $mobileContactInfoDlg).attr("src", myURL.createObjectURL(blob)).removeClass("hidden");
    } else {
      ...
      $(".mobile.red-button.first.add-contact", $mobileContactInfoDlg).rebind("click", function () {
        ...
        var appNotHere = function () {
          if (is_ios) {
            ...
            var googlePlay = "https://play.google.com/store/apps/details?" + "id=mega.privacy.android.app&referrer=meganzmobileapps";
    ...
    $(".mobile.text-button.third.cancel, .mobile.fm-dialog-close", $mobileContactInfoDlg).rebind("click", function () {
      var page = u_type ? "fm" : "start";
    ...
    api_req(req, {
      callback: function (res) {
        if (typeof res === "object") {
          ...
          fillContactInfo(self.contactName, self.contactEmail, self.contactAvatar, self.contactHandle, self.contactlink);
        } else {
