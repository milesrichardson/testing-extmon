  ...
  constructor() {
    ...
    if (fmblock) {
      ...
      this.selectionButton = new MegaMobileBottomBar({
        parentNode: fmblock,
        actions: [[["selection-action-btn", submitText, _submit, isDisabled]]]
      });
    }
  }
      ...
      copy: {
        ...
        onSubmit: (h, button) => {
          ...
          M.copyNodes([selected], h, false)
            ...
            .then((res) => {
              if (res && res.length) {
                mega.ui.toast.show(_getCompleteString(sn, h, "copy"), undefined, l[16797], { actionButtonCallback: () => M.openFolder(h) });
              }
            })
            .always(() => {
        ...
        buttonCondition: () => M.getNodeRights(M.currentdirid) && mobile.nodeSelector.selected !== M.currentdirid
      },
      move: {
        mode: "folder",
        submitText: l[62],
        onSubmit: (h, button) => {
          ...
          M.safeMoveNodes(h, [selected])
            .then(() => {
              mega.ui.toast.show(_getCompleteString(sn, h, aType), undefined, l[16797], { actionButtonCallback: () => M.openFolder(h) });
            })
            .catch(dump)
            .always(() => {
        ...
        tabs: moveCopyTabOpts,
        buttonCondition: () => M.getNodeRights(M.currentdirid) && !M.isCircular(mobile.nodeSelector.selected, M.currentdirid)
      },
      share: {
