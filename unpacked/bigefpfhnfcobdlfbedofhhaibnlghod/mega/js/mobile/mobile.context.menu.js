class MegaMobileContextMenu extends MegaMobileComponentGroup {
  ...
  async show(handle) {
    ...
    M.safeShowDialog("mobile-context-menu", () => {
      ...
      if (rubbishBinElm) {
        rubbishBinElm.component.text = sourceRoot === M.RubbishID ? l.delete_permanently : l.move_to_rubbish_bin;
      }
  ...
  MegaMobileContextMenu.menuItems = {
        ...
        if (!mobile.cloud.nodeInView(nodeHandle)) {
          ...
          const msg =
            nodeParent === M.RootID
              ...
              : restoredToFolder.replace("%1", node.name).replace("%2", M.getNodeByHandle(nodeParent).name);

          // toast message
          mega.ui.toast.show(msg, 4, l.show, {
            actionButtonCallback: () => {
              M.openFolder(nodeParent).finally(() => {
                $.selected = [nodeHandle];
              });
              return false;
            }
    ...
    ".leaveshare-item": {
      ...
      onClick: function (nodeHandle) {
        ...
        M.leaveShare(nodeHandle).catch((ex) => {
          if (ex === EMASTERONLY) {
            mega.ui.sheet.show({
