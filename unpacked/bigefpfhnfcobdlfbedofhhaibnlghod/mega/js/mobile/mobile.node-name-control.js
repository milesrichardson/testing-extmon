...
class MobileNodeNameControl {
  ...
  action(node) {
    ...
    } else if (!M.isSafeName(newName)) {
      // Check if folder/file name is valid
      errorMsg = newName.length > 250 ? (nodeType === 1 ? escapeHTML(l.LongName) : escapeHTML(l.LongName1)) : l[24708];
    } else if (this.duplicated(newName, node ? node.p : null)) {
      // Check if folder/file name already exists
  ...
  checkSpaces(newName, nodeType) {
    ...
    if (newName.trim() === newName) {
      // Check if folder/file name does not contain spaces at the beginning or end
      ...
      nameInput.showMessage(
        `${alertIcon}
                    ${nodeType === 1 ? escapeHTML(l.whitespaces_on_foldername) : escapeHTML(l.whitespaces_on_filename)}`
      );
    }
  }
      ...
      submit: (newName) => {
        ...
        M.createFolder(M.currentCustomView ? M.currentCustomView.nodeID : M.currentdirid, newName)
          .then((h) => {
            ...
            mega.ui.toast.show(escapeHTML(l.created_folder).replace("%1", newName), 4, escapeHTML(l[16797]), callbacks);
      ...
      submit: (newName, node) => {
        ...
        M.rename(node.h, newName)
          .then(() => {
            ...
            mega.ui.toast.show(
              (nodeType === 1 ? escapeHTML(l.renamed_folder_to) : escapeHTML(l.renamed_file_to))
                .replace("%1", nodeName)
                .replace("%2", newName)
            );
          })
