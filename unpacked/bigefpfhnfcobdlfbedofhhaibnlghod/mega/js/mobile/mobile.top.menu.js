class MegaMobileTopMenu extends MegaMobileComponent {
  ...
  static getPageRoot() {
    ...
    if (
      /\/(notifications|about|support|file-management|settings)$/.test(page) ||
      (page.includes("security") && !page.includes("change-password") && !page.includes("change-email") && !page.includes("verify"))
    ) {
  ...
  get visible() {
    return getComputedStyle(this.domNode).position === "relative" || this.domNode.classList.contains("active");
  }
  ...
  toggleActive() {
    ...
    for (const item of this.domNode.querySelectorAll(".menu-item")) {
      const isASharesPage =
        item.dataset.section === "/fm/shares" && (M.currentrootid === "out-shares" || M.currentrootid === "public-links");
