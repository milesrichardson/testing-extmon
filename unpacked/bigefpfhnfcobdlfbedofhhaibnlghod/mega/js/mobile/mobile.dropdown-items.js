class MegaMobileDropdownItemList {
  constructor(options) {
    ...
    if (options.dropdownOptions.search) {
      ...
      // megainputs
      this.$megaInput = new mega.ui.MegaInputs($(subNode).data("icon", "sprite-mobile-fm-mono icon-search-thin-outline"));
      this.$megaInput.$wrapper.addClass("box-style mobile");
      this.$megaInput.$input.rebind("input.search", (e) => this.searchList(_filterItems, e.target.value.trim()));
      ...
      this.noResults = mCreateElement("div", { class: "no-results hidden" }, [
        ...
        mCreateElement("span", {}, [document.createTextNode(options.resultText.caption || options.resultText || "")])
      ]);
      ...
      this.noResults.templates.caption = (options.resultText.caption || "").includes("%") && options.resultText.caption;
