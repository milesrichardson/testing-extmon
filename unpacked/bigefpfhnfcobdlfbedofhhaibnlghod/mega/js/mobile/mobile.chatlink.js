...
mobile.chatlink = {
  ...
  show: function (publicHandle, chatKey) {
    ...
    $(".chatlink-red-row > span", $overlay).html(htmlentities(l[20620]).replace("[S]", "<strong>").replace("[/S]", "</strong>"));
    ...
    this.linkInfo = new LinkInfoHelper(publicHandle, chatKey, false, true);

    init_chat(0x104df11e5).always(() => {
      this.linkInfo
        .getInfo()
        .then((res) => {
          if (res.mr) {
            $("p", $overlay).text("Install MEGA app to start a meeting. " + "Receive 20 GB of secure and private cloud storage for free.");
          }
          if (res.topic) {
