...
mobile.affiliate = {
  ...
  setCommissionIndexes: function ($page) {
    ...
    if (balance.localCurrency === "EUR") {
      ...
      $(".total.euro", $commissionBlocks).text(formatCurrency(balance.pending + balance.available));
  ...
  renderHistoryTable: function ($page) {
    ...
    if (this.historyList.length) {
      ...
      $list.rebind("tap.toRedemptionDetail", function () {
        ...
        const state = self.historyList[i].hasOwnProperty("state") ? self.historyList[i].state : self.historyList[i].s;
  ...
  initRedeemDetailsPage: function ($historyPage, data) {
    ...
    const state = data.hasOwnProperty("state") ? data.state : data.s;
  ...
  initRedeemPage: function () {
    ...
    Promise.all([M.affiliate.getRedemptionMethods(), M.affiliate.getBalance()])
      .then(function () {
        // Cache selector
        ...
        $(".default-green-button.redeem-button", self.$page).rebind("tap", function () {
          ...
          if ($this.is(".close")) {
            ...
            if (
              currentStep === 4 &&
              !$(".auto-fill-checkbox", this.$page).hasClass("hidden") &&
              isConfirmed &&
              affiliateRedemption.validateDynamicAccInputs()
            ) {
              mobile.messageOverlay
                .show(l[23374], l[23307], false, [l[78], l[79]])
                .then(() => {
            ...
            affiliateRedemption.processSteps().then(async (res) => {
              ...
              if (
                ...
                await new Promise((resolve) => {
                  mobile.messageOverlay
                    .show(l[23374], l.referral_bitcoin_update, false, [l[78], l[79]])
                    .then(() => {
                      affiliateRedemption.updateAccInfo();
              ...
              if (currentStep === 3 && affiliateRedemption.requests.first.m === 2) {
                if (affiliateRedemption.req1res[0].lf / affiliateRedemption.req1res[0].la > 0.1) {
                  ...
                  mobile.messageOverlay.show(l[24964], l[24965], false, [l[78], l[79]]).catch(() => {
                    self.closeRedeemPage();
                    return false;
                  });
                }
    ...
    for (var j = selectedGWData.data.$.length; j--; ) {
      ...
      contentHtml =
        '<option value="%1"%3>%2</option>'
          .replace("%1", c)
          .replace("%2", c)
          .replace("%3", c === activeCurrency ? " selected" : "") + contentHtml;
    }
  ...
  redeemStep5: function () {
    ...
    if (rdmreq1.c === "EUR") {
      ...
      $(".requested.price .euro", $step5).removeClass("hidden").text(formatCurrency(rdmreq1.p));
      $(".fee.price .euro", $step5).removeClass("hidden").text(formatCurrency(rdmreq1res[0].f));
      $(".received.price .euro", $step5)
        .removeClass("hidden")
    ...
    if (rdmreq1.m === 2) {
      $(".requested.price .local", $step5).text("BTC " + parseFloat(rdmreq1res[0].la).toFixed(8));
      ...
      $(".requested.price .local", $step5).text(formatCurrency(rdmreq1res[0].la, rdmreq1res[0].lc, "code"));
      $(".fee.price .local", $step5).text(formatCurrency(rdmreq1res[0].lf, rdmreq1res[0].lc, "code") + "*");
      $(".received.price .local", $step5).text(formatCurrency(rdmreq1res[0].la - rdmreq1res[0].lf, rdmreq1res[0].lc, "code"));
    }
      ...
      $select.rebind("change.showAdditionalInput", function () {
        ...
        M.affiliate.getExtraAccountDetail().then(function (res) {
          ...
          for (var j = 0; j < additions.length; j++) {
            ...
            if (
              $(".auto-fill-checkbox input", self.$page).prop("checked") &&
