...
var mobile = {
          ...
          var action = function () {
            if (data.EPAYWALL) {
              ...
              const bannerMsg = overlayTexts.fmBannerText
                .replace('<a href="/pro" class="clickurl">', "<strong>")
                .replace("</a>", "</strong>")
                .replace("<span>", "")
                .replace("</span>", "");

              const banner = mobile.banner.show("", parseHTML(bannerMsg), "", "error", false, undefined, true);
            ...
            } else if (mega.ui.sheet.name !== "resume-download") {
              // don't proceed if resume dialog is open
              if (data.isFull) {
                mobile.overStorageQuota.show();
              } else {
                ulmanager.ulOverStorageQuota = false;
                const banner = mobile.banner.show(l.storage_almost_full, l.storage_almost_full_msg, l.upgrade_now, "warning", true);
                banner.on("cta", () => {
                  mega.ui.sheet.hide();
                  loadSubPage("pro");
                });
      ...
      value: function (status) {
        ...
        if (status === 1) {
          mega.attr.get(u_handle, "cardalmostexp", -2, true).always((res) => {
            if (res !== "1") {
              const banner = mobile.banner.show(l.card_expiry_title, l.card_expiry_msg, l.update_card, "warning");
              banner.on("cta", () => loadSubPage("fm/account/paymentcard"));
              banner.on("close", () => mega.attr.set("cardalmostexp", "1", -2, true));
            }
          });
        } else if (status === 2) {
          const banner = mobile.banner.show(l.card_expired, l.card_expired_msg, l.update_card, "error", true);
          banner.on("cta", () => loadSubPage("fm/account/paymentcard"));
        }
      }
    });
  ...
  initPasswordVisibleToggle: ($page) => {
    ...
    if ($("i.pass-visible", $parent).length === 0) {
      $('input[type="password"]', $page).parent().safeAppend('<i class="sprite-fm-mono icon-eye-reveal pass-visible"></i>');
    }
  ...
  initNumberMaxlength: function ($page) {
    ...
    $('input[type="number"][maxlength]', $page).rebind("keydown.applyMaxlength", function (e) {
      if (
        (this.value.length === parseInt($(this).attr("maxlength")) &&
  ...
  albums: {
    ...
    initPublicAlbum: (parentNode) => {
      ...
      if (elCount) {
        ...
        api_getfileattr({ [coverNode.h]: coverNode }, 0, async (ctx, key, ab) => {
