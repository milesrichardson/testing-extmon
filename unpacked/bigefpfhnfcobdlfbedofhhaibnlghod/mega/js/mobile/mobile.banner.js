    ...
    if (u_attr) {
      ...
      if (u_attr.b) {
        if (u_attr.b.s === -1) {
          ...
          if (u_attr.b.m) {
            const banner = mobile.banner.show(l[20401], l.payment_failed_and_not_resolved, l.pay_and_reactivate, "error", false);
            ...
            if (u_attr.b.s === -1) {
              // expired
              mobile.banner.show(l.bsub_user_account_exp_title, l.bsub_user_account_exp_msg, "", "error", false);
            }
        ...
        if (u_attr.b.m) {
          if (u_attr.b.s === 2) {
            // grace
            const banner = mobile.banner.show(l.payment_not_processed_title, l.payment_not_processed_msg, l.update_card, "error", false);
          ...
          if (!u_attr["^buextra"] || sessionStorage.buextra) {
            if (sessionStorage.buextra) {
              const banner = mobile.banner.show(l.additional_storage_usage_title, l.additional_storage_usage_msg, l.read_more, "info");
              banner.on("cta", () => {
      ...
      if (u_attr.pf) {
        ...
        if (u_attr.pf.s === -1) {
          const banner = mobile.banner.show(l.pf_account_deactivated_title, l.pf_account_deactivated_msg, l.reactivate_account, "error");
          banner.on("cta", () => loadSubPage("repay"));
          ...
          // grace period
          const banner = mobile.banner.show(l.payment_not_processed_title, l.payment_not_processed_msg, l.update_card, "error");
          banner.on("cta", () => loadSubPage("fm/account/paymentcard"));
        }
      }
    }
 ...
 * @type {string[]}
 */
MegaMobileBanner.displayTypes = Object.freeze(["info", "warning", "success", "error", "advertisement"]);
