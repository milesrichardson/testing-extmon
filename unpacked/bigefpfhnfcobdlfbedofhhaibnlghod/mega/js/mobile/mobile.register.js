...
mobile.register = {
  ...
  initKeyupEvents: function () {
    ...
    var $allFields = $firstNameField.add($lastNameField).add($emailField).add($passwordField).add($confirmPasswordField);
  ...
  initRegisterButton: function () {
    ...
    $registerButton.rebind("tap", () => {
      ...
      $firstNameField.add($lastNameField).add($emailField).add($passwordField).add($confirmPasswordField).trigger("blur");
      ...
      if (registerInfo || localStorage.businessSubAc) {
        ...
        var ctx = {
          checkloginresult: function (u_ctx, r) {
            ...
            if (typeof r[0] === "number" && r[0] < 0) {
              ...
              security.login.checkLoginMethod(email, password, null, false, mobile.signin.old.startLogin, mobile.signin.new.startLogin);
        ...
        u_checklogin(ctx, true, null, signupcode, firstName + " " + lastName);
      } else {
  ...
  initCancelRegistrationButton: function ($button) {
    ...
    $button.off("tap").on("tap", function () {
      mobile.messageOverlay
        .show(l[5710], false, false, [l[78], l[79]])
        .then((a0) => mobile.register.abort(a0))
        .catch(dump);
    });
  },
