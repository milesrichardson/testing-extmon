  ...
  const render = (cardInfo) => {
    if (cardInfo) {
      if (cardInfo.brand === "visa") {
        ...
        $(".payment-card-icon i", $page).removeClass("sprite-fm-uni icon-mastercard-border");
      ...
      $(".payment-card-expiry .payment-card-expiry-val", $page).text(
      ...
      $(".payment-card-bottom a.payment-card-edit", $page).rebind("tap", () => {
        ...
        api
          ...
          .catch((ex) => {
            msgDialog("warninga", "", l.edit_card_error.replace("%1", ex), l.edit_card_error_des, loadSubPage.bind(null, "fm/account"));
          })
