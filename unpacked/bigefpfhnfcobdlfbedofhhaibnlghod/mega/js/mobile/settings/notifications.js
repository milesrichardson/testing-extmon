      ...
      mega.enotif.all().then((enotifStates) => {
        ...
        for (const notif of $EEach) {
          ...
          mobile.initSwitch($this, !enotifStates[eName], (newState) => {
            ...
            (newState || $EEach.hasClass("toggle-on") ? $.fn.addClass : $.fn.removeClass).apply($EAll, ["toggle-on"]);
        ...
        if (
          ...
          ((Array.isArray(M.account.sgw) && M.account.sgw.includes("Stripe")) ||
            (Array.isArray(M.account.sgwids) && M.account.sgwids.includes((addressDialog || {}).gatewayId_stripe || 19)))
        ) {
