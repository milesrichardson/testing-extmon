mobile.settings.history = Object.create(mobile.settingsHelper, {
  ...
  generateSessionNode: {
    value: function (session) {
      ...
      const nodeGenerator = {
        expired: (targetNode) =>
          mCreateElement("span", { class: "session-status expired" }, [document.createTextNode(l[25016])], targetNode),
        current: (targetNode) =>
          mCreateElement("span", { class: "session-status current" }, [document.createTextNode(l[7665])], targetNode),
        active: (targetNode) => {
  ...
  sortGroupSessions: {
    value: function (allSessions) {
      ...
      allSessions.sort((sessionA, sessionB) => sessionB[5] - sessionA[5] || sessionB[7] - sessionA[7] || sessionB[0] - sessionA[0] || -1);
      ...
      if (allSessions.length >= this.maxSessionsToDisplay) {
        ...
        return allSessions[this.maxSessionsToDisplay][7]
          ? allSessions.filter((x) => x[7])
