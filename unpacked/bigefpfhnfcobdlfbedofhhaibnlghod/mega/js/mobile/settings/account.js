mobile.settings.account = Object.create(mobile.settingsHelper, {
  ...
  init: {
    value: function () {
      ...
      this.domNode = useOverlay ? mCreateElement("div", { class: "mega-mobile-settings account" }) : this.generatePage("account");
      ...
      if (useOverlay) {
        ...
        linkComponents.forEach((c) =>
          c.rebind("beforeRedirect", () => {
  ...
  getStorageUsage: {
    value: function (data) {
      ...
      let percentageUsedText = new Intl.NumberFormat(lang, { style: "percent", maximumFractionDigits: 2 }).format(percentageUsed);
  ...
  renderDynamicInfo: {
    value: function (data) {
      ...
      const accountTop = mCreateElement("div", { class: "account-information" }, [avatarContainer, userInfoContainer]);
      ...
      percentageContainer.textContent =
        u_attr && (u_attr.b || u_attr.pf)
          ? mega.icu.format(l.mobile_account_storage_usage_flexi, storageInfo.spaceUsed).replace("%1", storageInfo.spaceUsed)
          : l.mobile_account_storage_usage.replace("%1", storageInfo.percentageUsed).replace("%2", storageInfo.spaceTotal);

      const planInfo = mCreateElement("div", { class: "text-box-wrapper storage-info" }, [planNameContainer, percentageContainer]);
      const planElem = mCreateElement("div", { class: "plan-information nav-elem full-width text-icon" }, [iconContainer, planInfo]);
      ...
      if (u_attr.uspw) {
        ...
        inlineODQBanner.domNode
          .querySelector(".message-text")
          .appendChild(parseHTML(overlayTexts.dlgFooterText.replace(/span/g, "strong")));
      }
  ...
  renderUpgradeButton: {
    value: function (userInfoContainer) {
      ...
      // If expired business master account show reactivate button
        ...
        u_attr &&
        ((u_attr.b && u_attr.b.m && u_attr.b.s !== pro.ACCOUNT_STATUS_ENABLED) || (u_attr.pf && u_attr.pf.s !== pro.ACCOUNT_STATUS_ENABLED))
      ) {
        btn = new MegaMobileLink({
          parentNode: userInfoContainer,
          href: "repay",
  ...
  render: {
    value: function () {
      ...
      if (this.overlayAccount) {
        api
          .req({ a: "maf", v: mega.achievem.RWDLVL })
          .then(({ result: res }) => {
  ...
  showAccCancelConfirmDialog: {
    value: function ($page, confirmMessage) {
      ...
      mobile.messageOverlay
        .show(l[6181], confirmMessage, false, false, false, true)
        .then(() => {
          loadingDialog.show();
