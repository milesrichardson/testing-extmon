(function ($) {
  ...
  var DEFAULT_SETTINGS = {
    ...
    resultsFormatter: function (item) {
      ...
      return (
        '<li class="share-search-result">' +
        (this.addAvatar ? avatar : "") +
        '<span class="fm-chat-user-info">' +
        '<span class="fm-chat-user">' +
        htmlentities(upperValue) +
        "</span>" +
        '<span class="fm-chat-user-email">' +
        htmlentities(lowerValue) +
        "</span>" +
  ...
  $.TokenList = function (input, url_or_data, settings) {
    ...
    function resize_input() {
      ...
      input_box.width(Math.min(token_list.outerWidth() || 30, Math.max(width_left, input_resizer.outerWidth() + 30)));
        ...
        switch (event.keyCode) {
          ...
          case KEY.DOWN:
            if (this.value.length === 0) {
              ...
              if (
                (previous_token.length && previous_token.get(0) === selected_token) ||
          ...
          case KEY.SEMICOLON:
            ...
            // If users press enter/return on empty input field behave like done/share button is clicked
              ...
              var cd;
              if ($.dialog === "share") {
                // if it is share dialog
                $addContactBtn = $(".share-dialog .dialog-share-button");
                cd = false;
              } else if ($.dialog === "add-user-popup") {
                // if it is add user dialog.
    ...
    input_resizer.insertAfter(input_box).css({
      position: "absolute",
      top: -9999,
      left: -9999,
      width: "auto",
      ...
      if ($(input).data("settings").preventDoublet) {
        ...
        if (Object.keys(M.opc).length > 0 && typeof itemFoundType === "undefined") {
          Object.keys(M.opc).forEach(function (g) {
            if (M.opc[g].m.toLowerCase() === item[tokenValue].toLowerCase() && !M.opc[g].hasOwnProperty("dts")) {
              itemFoundType = "opc";
              return false;
            }
          });
        ...
        if ($(input).data("settings").url) {
          ...
          ajax_params.success = function (results) {
            ...
            if (input_box.val() === query) {
              populate_dropdown(
                query,
                $(input).data("settings").jsonContainer ? results[$(input).data("settings").jsonContainer] : results
              );
            }
          };
