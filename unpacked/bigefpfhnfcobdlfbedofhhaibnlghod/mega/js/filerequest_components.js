...
lazy(mega, "fileRequestUI", () => {
  ...
  class EmbedCodeInputComponent extends ReadOnlyInputComponent {
    ...
    setContext(context) {
      ...
      this.setContent(generator.generateCode(this.puPagePublicHandle, this.lightTheme));
    }
  }
  ...
  class ShareLinkInputComponent extends ReadOnlyInputComponent {
    ...
    setContext(context) {
      ...
      this.setContent(generator.generateUrl(this.puPagePublicHandle));
    }
  }
  ...
  class BaseClickableComponent {
    ...
    disable() {
      return this.$input.addClass("disabled").attr("disabled", "disabled");
    }

    enable() {
      return this.$input.removeClass("disabled").removeAttr("disabled");
    }
    ...
    setOnClick() {
      this.eventOnClick(($input) => {
        ...
        if (typeof copyOptions.content === "function") {
          copyToClipboard(copyOptions.content(), this.getToastText(copyOptions.toastText), copyOptions.className);
        }
      });
    }
  ...
  class PreviewButtonComponent extends ButtonComponent {
    ...
    setOnClick() {
      this.eventOnClick(($input) => {
        ...
        const { name, title, description, theme, pupHandle } = optionCallback($input);

        const url = generator.generateUrlPreview(name, title, description, theme, pupHandle);

        if (url) {
          generator.windowOpen(url);
        }
      });
    }
  }
  ...
  class ValidatableInputComponent extends InputComponent {
      ...
      if (validationRules.required) {
        ...
        if (!this.getValue()) {
          if (validationMessage) {
            this.addErrorMessage(validationMessage);
          }
    ...
    addErrorMessage(message) {
      const $megaInputWrapper = this.$inputWrapper.closest(megaInputWrapperSelector);
    ...
    resetErrorMessage() {
      this.$inputWrapper.closest(megaInputWrapperSelector).removeClass("error msg");
    ...
    setValue(newValue) {
      ...
      return this.$input.val(newValue).trigger(`input${namespace}`);
    }

    reset() {
      this.setValue("");
      this.resetErrorMessage();
      this.getInput().closest(megaInputSelector).removeClass(activeClass);
    }
  }

  class ValidatableMobileComponent extends ValidatableInputComponent {
    addErrorMessage(message) {
      const $warningBlock = this.$inputWrapper.closest(".input-container").find(".input-warning-block");
    ...
    resetErrorMessage() {
      this.$inputWrapper.closest(".input-container").find(".input-warning-block").addClass("hidden");
    }
  }
  ...
  class SelectFolderComponent {
    constructor($dialog) {
      ...
      this.$inputFolder = new InputComponent($(".file-request-folder", this.$dialog));
    ...
    init() {
      ...
      this.$inputFolder.getInput().removeClass("disabled").closest(megaInputSelector).addClass(activeClass);
