  ...
  value: Object.create(null, {
    ...
    toString: {
      value: function toString(ach) {
        if (ach !== undefined) {
          var res = Object.keys(this).filter(
            function (v) {
              return this[v] === ach;
            }.bind(this)
          );
    ...
    if (data.rwds) {
      ...
      if (idx !== ach.ACH_INVITE) {
        ...
        if (data.rwd.expiry.unit === "d" && data.rwd.left > 0) {
          locFmt = mega.icu.format(l.ach_expires_days, data.rwd.left).replace("[S]", "<span>").replace("[/S]", "</span>");
        ...
        locFmt = "";
        switch (idx) {
          case ach.ACH_WELCOME:
            locFmt = l[16395];
            break;
          case ach.ACH_SYNCINSTALL:
            locFmt = l[16396];
            break;
          case ach.ACH_APPINSTALL:
            locFmt = l[16397];
            break;
        }
      }
      ...
      $(".expires-txt", $cell).removeClass("error").safeHTML("%n", locFmt, data.expiry.value, data.expiry.utxt);
...
mega.achievem.initInviteDialogMultiInputPlugin = function initInviteDialogMultiInputPlugin() {
  ...
  $this.tokenInput(contacts, {
    ...
    onReady: function () {
      // Called once on dialog initialization
  ...
  function resetInfoText(timeOut) {
    ...
    errorTimer = setTimeout(function () {
      ...
      $(".achievement-dialog.multiple-input").find("li input").eq(0).removeClass("red").trigger("focus");
    }, timeOut);
  }
...
mega.achievem.invitationStatusDialog = function invitationStatusDialog(close) {
  ...
  while (rlen--) {
    ...
    if (rwd.csu) {
      // csu - contact (invitee) signed up
      if (rwd.c) {
        // c - completed, time elapsed from time when app is installed

        $(".status", $tmpl).safeHTML('<strong class="green">@@</strong>' + '<span class="light-grey"></span>', l[16105]); // Quota Granted

        var expiry = rwd.expiry || maf.expiry;
        locFmt = l[16336].replace("[S]", "").replace("[/S]", "");
        $(".status .light-grey", $tmpl).safeHTML("%n", locFmt, expiry.value, expiry.utxt);

        $(".icon i", $tmpl).addClass("sprite-fm-mono icon-active granted-icon");
      } else {
        // Pending APP Install

        $(".status", $tmpl).safeHTML('<strong class="orange">@@</span>', l[16104]); // Pending App Install
      ...
      $(".status", $tmpl).safeHTML("<strong >@@</span>", l[7379]); // Pending
...
mega.achievem.parseAccountAchievements = function parseAccountAchievements() {
  ...
  $(".base-quota-note span", $bandwidthContent).text(l[19992].replace("%1", bytesToSize(transferBaseQuota, 0)));
  $(".achieve-quota-note span", $bandwidthContent).text(l[19993].replace("%1", bytesToSize(transferAchieveValue, 0)));
  ...
  $(".base-quota-note span", $storageContent).text(l[19992].replace("%1", bytesToSize(storageBaseQuota, 0)));
  $(".achieve-quota-note span", $storageContent).text(l[19993].replace("%1", bytesToSize(storageAchieveValue, 0)));
};
