function dashboardUI(updProcess) {
  ...
  $(".account.widget.storage .pr-item").rebind("click", function () {
    var section = String($(this).attr("class"))
      .replace(/account|pr-item|empty|ui-droppable/g, "")
      .trim();
  ...
  M.accountData(function (account) {
    ...
    if (u_attr.p || (u_attr.b && u_attr.b.s === pro.ACCOUNT_STATUS_EXPIRED) || (u_attr.pf && u_attr.pf.s === pro.ACCOUNT_STATUS_EXPIRED)) {
      var timestamp;
      // Subscription
      if (account.stype == "S") {
        ...
        if (account.nextplan) {
          $(".account.left-pane.plan-date-info").safeHTML(
            escapeHTML(l[20153]) +
      ...
      if (u_attr.b || (u_attr.pf && u_attr.pf.s === pro.ACCOUNT_STATUS_EXPIRED)) {
        ...
        if (u_attr.b && u_attr.b.s === pro.ACCOUNT_STATUS_ENABLED) {
          $businessLeft.find(".suba-status").addClass("active").removeClass("disabled pending").text(l[7666]);
        } else if (u_attr.b && u_attr.b.s === pro.ACCOUNT_STATUS_GRACE_PERIOD && u_attr.b.m) {
          $(".suba-status", $businessLeft).addClass("pending").removeClass("disabled active").text(l[19609]);
          ...
          $(".suba-status", $businessLeft).addClass("disabled").removeClass("pending active").text(l[19608]);
    ...
    for (var i = 0; i < $(".account.left-pane.small-txt:visible").length; i++) {
      var rowwidth =
        $(".account.left-pane.small-txt:visible").get(i).offsetWidth + $(".account.left-pane.big-txt:visible").get(i).offsetWidth;
    ...
    if (!u_attr.b && !u_attr.pf) {
      ...
      // Cloud drive
      $(".account.progress-size.cloud-drive").text(`(${bytesToSize(account.stats[M.RootID].bytes)})`);
      // Rubbish bin
      $(".account.progress-size.rubbish-bin").text(`(${bytesToSize(account.stats[M.RubbishID].bytes)})`);
      ...
      // Available
      $(".account.progress-size.available").text(`(${bytesToSize(Math.max(account.space - account.space_used, 0))})`);
      // Versions
      $(".account.progress-size.versions").text(`(${bytesToSize(account.stats[M.RootID].vbytes)})`);
      /* End of Used Storage */
      ...
      $(".storage-transfer-data, .storage-transfer-current", $storageBlk).text(
        bytesToSize(account.space_bus_used || account.space_used, 2)
      );
      $(".storage-transfer-data, .storage-transfer-current", $transferBlk).text(bytesToSize(account.tfsq_bus_used || account.tfsq.used, 2));
      ...
      if (u_attr.pf || (u_attr.b && u_attr.b.m)) {
        ...
        if (account.space_bus_base) {
          $(".storage-transfer-data", $storageBaseBlk).text(l[5816].replace("[X]", account.space_bus_base));
        ...
        if (account.tfsq_bus_base) {
          $(".storage-transfer-data", $transferBaseBlk).text(l[5816].replace("[X]", account.tfsq_bus_base));
      ...
      .then(() => {
        ...
        if (balance) {
          ...
          if (localCurrency === "EUR") {
            ...
            $(".referral-value.total.euro", $referralWidget).text(formatCurrency(balance.pending + balance.available));
            $(".referral-value.available.euro", $referralWidget).text(formatCurrency(balance.available));
          }

          $(".referral-value.total.local .value", $referralWidget).text(localTotal);
          $(".referral-value.available.local .value", $referralWidget).text(localAvailable);
...
dashboardUI.updateCloudDataWidget = function () {
  ...
  $(".data-item:not(.used-storage-info)", ".account.data-table.data").each(function (idx, elm) {
