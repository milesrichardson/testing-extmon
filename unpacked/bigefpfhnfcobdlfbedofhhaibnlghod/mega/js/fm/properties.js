(function _properties(global) {
  ...
  function fillPropertiesContactList($dialog, users) {
    var MAX_CONTACTS = 5;
    var shareUsersHtml = "";
    var $shareUsers = $dialog
      .find(".properties-body .properties-context-menu")
      .empty()
    ...
    for (var i = users.length; i--; ) {
      ...
      shareUsersHtml +=
        '<div class="properties-context-item ustatus ' +
        escapeHTML(userHandle) +
        " " +
        (status || "") +
        " " +
        hidden +
        ...
        "<span>" +
        escapeHTML(M.getNameByHandle(userHandle)) +
        "</span>" +
        "</div>";
    }
    ...
    if (users.length > MAX_CONTACTS) {
      shareUsersHtml +=
        ...
        "<span>..." +
        escapeHTML(l[10663]).replace("[X]", users.length - MAX_CONTACTS) +
        "</span>" +
        "</div>";
    }
  ...
  function _propertiesDialog(action) {
    ...
    var vhtml = versioningFlag
      ? '<div class="properties-float-bl' +
        p.t12 +
        '"><span class="properties-small-gray">' +
        p.t13 +
        "</span>" +
        '<span class="propreties-dark-txt">' +
        p.t14 +
        "</span></div>" +
        '<div class="properties-float-bl"><span class="properties-small-gray">' +
        p.t15 +
        "</span>" +
        '<span class="propreties-dark-txt">' +
        p.t16 +
    ...
    var singlenodeinfohtml =
      ...
      p.t6 +
      "</span>" +
      '<span class="propreties-dark-txt t7">' +
      p.t7 +
      "</span></div>";

    var shareinfohtml =
      typeof p.t10 === "undefined" && typeof p.t11 === "undefined"
        ? ""
        : '<div class="properties-float-bl"><div class="properties-small-gray t10">' +
          p.t10 +
          "</div>" +
          '<div class="propreties-dark-txt t11">' +
          p.t11 +
          "</div></div></div>";

    var html =
      '<div class="properties-name-container"><div class="properties-small-gray">' +
      p.t1 +
      "</div>" +
      '<div class="properties-name-block"><div class="propreties-dark-txt">' +
      p.t2 +
      "</div>" +
      "</div></div>" +
      ...
      (n.h === M.RootID || n.h === M.RubbishID || n.h === M.InboxID
        ? '<div class="contact-list-icon sprite-fm-mono icon-info-filled"></div>' + "</div>" + shareinfohtml
        : '<div class="properties-small-gray">' +
          p.t8 +
    ...
    if ($dialog.hasClass("shared")) {
      ...
      $(".properties-context-item.show-more").rebind("click", function () {
        // $('.properties-context-menu').fadeOut(200);
        $(".properties-dialog .properties-context-item").remove(".show-more").removeClass("hidden"); // un-hide rest of contacts
  ...
  global.propertiesDialog = function propertiesDialog(close) {
    if (close) {
      ...
      var shares = [];
      var nodes = ($.selected || []).filter(function (h) {
