        ...
        if (typeof M.columnsWidth.cloud[col].curr !== "number") {
          if (col === "fname") {
            ...
            if (document.body.offsetWidth > 1800) {
              ...
              M.columnsWidth.cloud[col].curr = header.style.width = `calc(100vw - ${$.leftPaneResizable.element.outerWidth() + 676}px)`;
            }
          }
...
FileManager.prototype.initFileManager = async function () {
  ...
  if (!pfid && !is_mobile && u_type) {
    ...
    if (s4e) {
      ...
      const s4load = Promise.resolve(M.require("s4"))
        .then(() => {
...
FileManager.prototype.initFileManagerUI = function () {
  ...
  $(document).rebind("contextmenu.doc", function (ev) {
    ...
    var ALLOWED_CLASSES = ["contact-details-user-name", "contact-details-email", "js-selectable-text", "nw-conversations-name"];
    var ALLOWED_PARENTS =
  ...
  $fmholder.rebind("ps-scroll-left.fm-x-scroll ps-scroll-right.fm-x-scroll", function (e) {
    if (!e || !e.target) {
  ...
  $.hideContextMenu = function (event) {
    ...
    if (event && event.target) {
      ...
      if (
        (currentNodeClass &&
          currentNodeClass.contains("dropdown") &&
          (currentNodeClass.contains("download-item") || currentNodeClass.contains("move-item")) &&
  ...
  if (!this.fmTabState || this.fmTabState["cloud-drive"].root !== M.RootID) {
    this.fmTabState = {
      "cloud-drive": {
        ...
        root: M.RootID,
        prev: null,
        subpages: [M.InboxID, M.RubbishID, "recents", "shares", "faves", "out-shares", "public-links", "file-requests"]
      },
  ...
  $(".js-fm-tab").rebind("click.fmTabState", function () {
    ...
    activeClass = ("" + $(activeClass + ".active:visible").attr("class")).split(" ").filter(function (c) {
    ...
    if (activeTab) {
      if (
        activeTab.root === M.currentrootid ||
        activeTab.root === "chat" ||
        M.isAlbumsPage() ||
    ...
    const isGalleryRedirect =
      this.dataset.locationPref && (M.isAlbumsPage() || M.isGalleryPage() || this.matches(".nw-fm-left-icon.gallery"));
    ...
    for (var tab in self.fmTabState) {
      if (~clickedClass.indexOf(tab)) {
        ...
        // Clicked on the currently active tab, should open the root (e.g. go back)
          ...
          if (tab.root === "chat" && String(M.currentdirid).substr(0, 5) === "chat/" && !M.currentdirid.startsWith("chat/contacts")) {
            targetFolder = M.currentdirid;
          }
        } else if (tab.prev && (M.d[tab.prev] || M.isCustomView(tab.prev) || (tab.subpages && tab.subpages.indexOf(tab.prev) > -1))) {
  ...
  if (ua.details.os === "Apple") {
    $(window).rebind("blur.ps-unfocus", function () {
      $(document).rebind("ps-scroll-y.ps-unfocus", function (e) {
        ...
        delay(
          "ps-out-focused-" + $(e.target).data("ps-id"),
          function __psOutFocused() {
...
FileManager.prototype.initShortcutsAndSelection = function (container, aUpdate, refresh) {
  ...
  if (!aUpdate) {
     ...
     */
    window.selectionManager = new SelectionManager2_DOM($(container), {
...
FileManager.prototype.updFileManagerUI = async function () {
  ...
  var buildtree = function (n) {
    delay(
      "updFileManagerUI:buildtree:" + n.h,
      function () {
  ...
  if (UItree) {
    ...
    if (this.currentdirid === "shares" && !this.viewmode) {
      renderPromise = Promise.resolve(renderPromise).then(() => this.openFolder("shares", true));
    }
  }
...
FileManager.prototype.initContextUI = function () {
  ...
  $(".dropdown-section").on("mouseover", ".dropdown-item", function () {
    ...
    if (!$this.hasClass("opened") && $this.hasClass("contains-submenu")) {
      ...
      $this.next(".submenu").css({ top: menuPos.top }).removeClass("hidden").addClass("active");
  ...
  $(c + ".startaudio-item," + c + ".startaudiovideo-item").rebind("click", function () {
    ...
    if (!$this.is(".disabled") && user_handle) {
      megaChat.createAndShowPrivateRoom(user_handle).then(function (room) {
        room.setActive();
        room.startAudioCall();
      });
    }
  ...
  $(".filter-block.body .close").rebind("click", function () {
    ...
    $(this)
      ...
      .find(".colour-label-ind")
      .remove(); // Remove all colors from it
  ...
  $(c + ".open-item").rebind("click", function () {
    ...
    if (M.currentrootid === "out-shares" || M.currentrootid === "public-links" || M.currentrootid === "file-requests") {
  ...
  $(`${c}.move-backup-item`).rebind("click.moveBckp", () => {
    if (!$.selected.length || !mega.backupCenter.selectedSync || mega.backupCenter.selectedSync.nodeHandle !== $.selected[0]) {
      return false;
    }
  ...
  $(`${c}.remove-backup-item`).rebind("click.removeBckp", () => {
    ...
    msgDialog("remove", l[882], l[13751], l[1007], (e) => {
      if (e) {
        loadingDialog.pshow();

        mega.backupCenter
          .stopSync(undefined, mega.backupCenter.selectedSync.nodeHandle)
          .then(nop)
          .catch((ex) => {
  ...
  $(`${c}.new-bucket-item`).rebind("click.createBucket", () => s4.ui.showDialog(s4.buckets.dialogs.create));

  $(`${c}.settings-item`).rebind("click.showSettings", () => s4.ui.showDialog(s4.buckets.dialogs.settings, M.d[$.selected[0]]));

  $(`${c}.managepuburl-item`).rebind("click.managePA", () => s4.ui.showDialog(s4.objects.dialogs.access, M.d[$.selected[0]]));
...
FileManager.prototype.initUIKeyEvents = function () {
  ...
  $(window).rebind("keydown.uikeyevents", function (e) {
    ...
    var is_transfers_or_accounts = M.currentdirid && (M.currentdirid.substr(0, 7) === "account" || M.currentdirid === "transfers");
     ...
     * other day :)
     */
    if (page === "fm/recents" && !slideshowid && !$.dialog) {
      // left or right
      ...
      return;
    } else if (is_selection_manager_available && !is_transfers_or_accounts && !$.dialog && !slideshowid && M.viewmode == 1) {
      if (e.keyCode == 37) {
        // left
    ...
    } else if (e.keyCode === 46 && selPanel.length > 0 && !$.dialog && M.getNodeRights(M.currentdirid) > 1) {
    ...
    // If the Esc key is pressed while the payment address dialog is visible, close it
    ...
    } else if (e.keyCode === 27 && ($.copyDialog || $.moveDialog || $.selectFolderDialog || $.copyrightsDialog || $.saveAsDialog)) {
      closeDialog();
      ...
      s.length > 0 &&
      !$.dialog &&
      M.getNodeRights(M.d[s[0]] && M.d[s[0]].h) > 1 &&
      M.currentrootid !== M.InboxID &&
      M.currentdirid !== "devices" &&
      M.getNodeRoot(M.d[s[0]].h) !== M.InboxID
    ) {
      renameDialog();
    } else if (is_selection_manager_available && e.keyCode == 65 && e.ctrlKey && !$.dialog && !M.isGalleryPage()) {
  ...
  var transferPanelContextMenu = function (target) {
    ...
    if (target === null && (target = $(".transfer-table tr.ui-selected")).length > 1) {
      ...
      if (finished === ids.length) {
        $menuitems.addClass("hidden").filter(".transfer-clear").removeClass("hidden");
    ...
    parent.children("hr").addClass("hidden").end().children("hr.pause").removeClass("hidden").end();
  ...
  $.transferHeader = function (tfse) {
    ...
    $tmp.rebind("dblclick", function () {
      if ($(this).hasClass("transfer-completed")) {
        ...
        if (path.length > 1) {
          M.openFolder(path[1], true).always(function () {
            $.selected = [id];
            reselect(1);
          });
        }
...
FileManager.prototype.addIconUI = function (aQuiet, refresh) {
  ...
  // user management ui update is handled in Business Account classes.
    ...
    $(".fm-blocks-view.fm", ".fmholder").removeClass("hidden out-shares-view public-links-view file-requests-view s4-view");
  ...
  $(".fm-blocks-view, .fm-empty-cloud, .fm-empty-folder," + ".shared-blocks-view, .out-shared-blocks-view, .fm-empty-s4-bucket").rebind(
    "contextmenu.fm",
    function (e) {
...
FileManager.prototype.addGridUI = function (refresh) {
  ...
  if (this.currentdirid === "shares") {
    ...
    $(".files-grid-view.fm", ".fmholder").removeClass("out-shares-view public-links-view file-requests-view");

    if (this.currentCustomView) {
      if (M.isGalleryPage() || M.isAlbumsPage()) {
        $(".files-grid-view.fm", ".fmholder").addClass("hidden");
      } else {
        $(".files-grid-view.fm", ".fmholder").addClass(`${this.currentCustomView.type}-view`);
      }
    } else {
      $(".files-grid-view.fm", ".fmholder").removeClass("out-shares-view public-links-view s4-view");
    }
  ...
  $(
    ".files-grid-view.fm .grid-scrolling-table,.files-grid-view.fm .file-block-scrolling,.fm-empty-cloud," +
      ".fm-empty-folder,.fm.shared-folder-content, .fm-empty-s4-bucket"
  ...
  $(".grid-table.shared-with-me tr td:first-child")
    .add(".grid-table.out-shares tr td:first-child")
    .add(".grid-table.fm tr td:nth-child(2)")
...
FileManager.prototype.onSectionUIOpen = function (id) {
  ...
  if ((M.previousdirid && M.isAlbumsPage(0, M.previousdirid)) || !$("#albums-view", $(".fm-right-files-block")).hasClass("hidden")) {
    if (M.isGalleryPage()) {
  ...
  if (
    ...
    M.initLeftPanel();
  } else if (id === "cloud-drive" || id === "dashboard" || id === "account" || id === "devices") {
...
FileManager.prototype.initLeftPanel = function () {
  ...
  elements = document.getElementsByClassName((isGallery && !isDiscovery) || isAlbums ? "js-lp-gallery" : "js-lp-myfiles");
  ...
  if (M.currentdirid === M.RootID) {
    ...
    // If gallery and is not Discovery
    ...
    dispatcher = (function (name, dsp) {
      return tryCatch(
        function () {
          ...
          if ($dialog) {
            if (!$dialog.hasClass("mega-dialog") && !$dialog.hasClass("fm-dialog-mobile") && !$dialog.hasClass("fm-dialog")) {
