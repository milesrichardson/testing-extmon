...
mega.tpw = new (function TransferProgressWidget() {
  ...
  var initEventsHandlers = function () {
    ...
    // close event handler
    $(".transfer-progress-icon.tpw-close", $rowsHeader)
      .off("click")
      .on("click", function tpw_CloseHandler() {
        isHiddenByUser = true;
        mega.tpw.hideWidget();
      });

    // close over quota
    $(".close-over", $overQuotaBanner)
      .off("click")
      .on("click", function overquota_bannerClose() {
        $overQuotaBanner.addClass("hidden");
      });

    // upgrade account
    $(".up-action", $overQuotaBanner)
      .off("click")
      .on("click", function overquota_bannerUpgrade() {
    ...
    // open dashboard
    $(".left-section.circle-dashboard", $overQuotaBanner)
      .off("click")
      .on("click", function overquota_bannerUpgrade() {
  ...
  var viewOverQuotaBanner = function (type) {
    ...
    M.accountData(function (acc) {
      if (type === mega.tpw.DOWNLOAD) {
        ...
        $(".quota-info-pr-txt", $overQuotaBanner).text(l[5775].replace("%1", bytesToSize(acc.tfsq.used)));
        ...
        $(".head-title", $overQuotaBanner).safeHTML(
          l.tfw_storage_exceeded_title.replace("[S]", '<span class="pct-used">').replace("[/S]", "</span>").replace("%1", perc)
        );
  ...
  const hideCompleteActions = (e) => {
    ...
    if (!(e.currentTarget.classList.contains("complete") && (actionsNode = e.currentTarget.querySelector(".transfer-complete-actions")))) {
      return;
    }

    const h = e.currentTarget.getAttribute("nhandle") || e.currentTarget.getAttribute("id").split("_").pop();
  ...
  var init = function () {
    ...
    $transferActionTemplate = $($("button.btn-icon.transfer-progress-btn", $downloadRowTemplate)[0])
      .clone()
    ...
    $transferActionTemplateIcon.removeClass(
      "pause cancel link cloud-folder restart" + "icon-pause icon-close-component icon-link icon-search-cloud icon-play-small"
    );
  ...
  this.addDownloadUpload = function (type, entry, specifiedSize) {
    ...
    for (var r = 0; r < entriesArray.length; r++) {
      ...
      if (!$row) {
        $row = type === this.DOWNLOAD ? $downloadRowTemplate.clone(true) : $uploadRowTemplate.clone(true);
      ...
      $("i.transfer-progress-icon", $enqueueAction).addClass("cancel icon-close-component").attr("data-simpletip", toolTipText);
      ...
      $row.find(".transfer-filetype-icon").attr("class", "transfer-filetype-icon").addClass(filetype(fName, true)[0].toLowerCase());
  ...
  this.updateDownloadUpload = function (type, id, perc, bytesLoaded, bytesTotal, kbps, queue_num, startTime) {
    ...
    if (!$targetedRow.attr("prepared")) {
      ...
      $("i.transfer-progress-icon", $progressAction).addClass("pause icon-pause").attr("data-simpletip", pauseText);
      $actionsRow.append($progressAction);
      $progressAction = $transferActionTemplate.clone(true).addClass("cancel");
      $("i.transfer-progress-icon", $progressAction).addClass("cancel icon-close-component").attr("data-simpletip", cancelText);
      $actionsRow.append($progressAction);
  ...
  this.finishDownloadUpload = function (type, entry, handle) {
    ...
    if (!$targetedRow.attr("zippo")) {
      ...
      if (root && root !== "shares") {
        $("i.transfer-progress-icon", $finishedAction)
          .removeClass("sprite-fm-mono")
          .addClass("sprite-fm-mono link icon-link")
          .attr("data-simpletip", l[5622]);
        $finishedActionsRow.append($finishedAction);
      }
      if (type === this.UPLOAD) {
        $finishedAction = $transferActionTemplate.clone(true).addClass("cloud-folder");
        $("i.transfer-progress-icon", $finishedAction).addClass("cloud-folder icon-search-cloud").attr("data-simpletip", l[20695]);
      } else {
        $finishedAction = $();
      }
      $finishedActionsRow.append($finishedAction);
    ...
    $("i.transfer-progress-icon", $finishedAction).addClass("cancel icon-close-component").attr("data-simpletip", l.tfw_transfer_remove);
  ...
  this.errorDownloadUpload = function (type, entry, errorStr, isOverQuota) {
    ...
    $("i.transfer-progress-icon", $errorCancelAction).addClass("cancel icon-close-component").attr("data-simpletip", cancelText);
  ...
  this.pauseDownloadUpload = function (type, entry) {
    ...
    $("i.transfer-progress-icon", $pausedAction).addClass("restart icon-play-small").attr("data-simpletip", l.tfw_transfer_start);
    $actionsRow.append($pausedAction);
    $pausedAction = $transferActionTemplate.clone(true).addClass("cancel");
    $("i.transfer-progress-icon", $pausedAction).addClass("cancel icon-close-component").attr("data-simpletip", l.tfw_transfer_cancel);
    $actionsRow.append($pausedAction);
    $targetedRow.removeAttr("prepared");
    delay(
      "tpwpauseallcheck",
      () => {
        if ($(".paused, .complete, .error, .overquota", $rowsContainer).length === $(tpwRowSelector, $rowsContainer).length) {
