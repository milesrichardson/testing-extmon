...
lazy(mega, "backupCenter", () => {
  ...
  const syncStatus = {
    inProgressSyncs(syncData, statusParentNode) {
      ...
      if (syncData.syncingPercs && syncData.syncsNumber === 1) {
        ...
        mCreateElement(
          ...
          percsNode
        ).textContent = `${syncData.syncingPercs} %`;
      } else {
        mCreateElement(
          "i",
          {
    ...
    offlineSyncs(syncData, statusParentNode, isDeviceCard) {
      ...
      if (isDeviceCard && (syncData.currentDate - syncData.lastHeartbeat * 1000) / (1000 * 3600 * 24) >= daysNum) {
        mCreateElement(
          "i",
          {
  ...
  return new (class {
    ...
    async getData(force) {
      ...
      if (Array.isArray(res) && res.length) {
        const uniqueHandles = res.map((a) => a.h).filter((val, i, self) => self.indexOf(val) === i);
    ...
    addNewBackup() {
      megasync.isInstalled((err, is) => {
        if (!err && is) {
          megasync.megaSyncRequest(
            { a: "ab", u: u_handle },
            (ev, res) => {
              // TODO: Remove the condition when no old versions left on users devices
              if (res === -2 && is.v && parseFloat(is.v) < 4.8) {
                msgDialog(`confirmation:!^${l[20826]}!${l[1597]}`, l[23967], l.outdated_app_ver, undefined, (e) => {
                  if (!e || typeof megasync === "undefined") {
                    return false;
                  }
                  open(megasync.getMegaSyncUrl() || "https://mega.io/desktop", "_blank", "noopener,noreferrer");
                });
              }
              // Invalid user handle
              else if (res === -2) {
    ...
    showStopConfirmationDialog(isBackup) {
      if (!$.selected.length || !this.selectedSync || this.selectedSync.nodeHandle !== $.selected[0]) {
        return false;
      }
      ...
      msgDialog(type, l[882], title, info, (e) => {
        if (!e) {
          return false;
        }
    ...
    scrollBlock() {
      ...
      if (this.scrollToSelected) {
        ...
        if (scrollToOffset - scrollBlockOffset + scrollToHeight > scrollBlockHeight) {
          $scrollBlock.scrollTop(scrollToOffset - scrollBlockOffset + scrollToHeight - scrollBlockHeight + scrollValue + 8);
        }
      }
    }
    ...
    getSyncStatus(folders) {
      ...
      for (var i = 0; i < folders.length; i++) {
        ...
        if (folders[i].hb) {
          ...
          folderHeartbeat = Math.max(folders[i].hb.ts || 0, folders[i].hb.lt || 0);

          // Set latest device heartbeat
          syncData.lastHeartbeat = Math.max(folderHeartbeat, syncData.lastHeartbeat);
        }
        ...
        else if (!folderHeartbeat || (syncData.isMobile && timeDifference > 60) || (!syncData.isMobile && timeDifference > 30)) {
          // Set Offline only if MEGA folder does not exist
          // or if MEGA folder was created > 10mins ago
          if (!M.d[folders[i].h] || (syncData.currentDate - M.d[folders[i].h].ts * 1000) / (1000 * 60) > 10) {
        ...
        // and there is no heartbeat state or Up to date/unknown/not active
          ...
          (folders[i].s === 1 || folders[i].s >= 5) &&
          (!folders[i].hb || (folders[i].hb && (folders[i].hb.s === 1 || folders[i].hb.s === 4)))
        ) {
    ...
    setSyncStatus(syncData, statusParentNode, isDeviceCard) {
      ...
      // Show backup state: Stalled
        ...
        mCreateElement("span", { class: "error" }, statusParentNode).textContent = mega.icu.format(
          l.stalled_sync_state,
          isDeviceCard ? syncData.stalledSyncs : 1
        );
      }
      // Show sync status: Up to date
        ...
        mCreateElement(
          ...
          statusParentNode
        ).textContent = l.up_to_date_status;
      }
    ...
    populateFolders(deviceNode) {
      ...
      const createFoldersTable = () => {
        ...
        mCreateElement("th", undefined, tableHeaderNode).textContent = l.last_updated_label; // 'Last updated'
        mCreateElement("th", undefined, tableHeaderNode).textContent = l.used_storage_label; // 'Used storage'
      ...
      for (let i = 0; i < deviceNode.folders.length; i++) {
        ...
        const folderName = (folder.id && decodedFolderName.bn && !syncStatus.isMobile) || !n.name ? decodedFolderName.bn : n.name;
        const isSelected = (folder.id && folder.id === this.selectedSync.id) || folder.h === this.selectedSync.id;
        let icon = "";
    ...
    openSection() {
      ...
      $(".fm-right-block, .fm-right-files-block, .section.conversations," + ".fm-right-account-block", ".fmholder").addClass("hidden");
