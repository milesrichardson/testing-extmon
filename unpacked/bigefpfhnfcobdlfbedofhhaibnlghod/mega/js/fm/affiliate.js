...
affiliateUI.commissionIndex = {
  ...
  calculateCommission: function () {
    ...
    if (balance.localCurrency === "EUR") {
      ...
      $(".commission-block.total .euro-price", this.$block).text(formatCurrency(balance.pending + balance.available));
...
affiliateUI.redemptionDialog = {
  show: function () {
    ...
    M.affiliate
      ...
      .then(function () {
        ...
        const $euroTemplate = $(".available-commission-euro", $availableComissionTemplates).clone().removeClass("hidden");
        const $localTemplate = $(".available-commission-local", $availableComissionTemplates).clone().removeClass("hidden");
        ...
        if (balance.localCurrency && balance.localCurrency !== "EUR") {
          const local = balance.localCurrency + " " + formatCurrency(balance.localAvailable, balance.localCurrency, "narrowSymbol") + "* ";
          $localTemplate.text(local);
          $availableComissionArea.safeAppend($localTemplate.prop("outerHTML")).safeAppend($euroTemplate.prop("outerHTML"));
        } else {
  ...
  showSubmitted: function () {
    ...
    var __closeSubmitted = function () {
      ...
      Promise.all([M.affiliate.getBalance(), M.affiliate.getRedemptionHistory()])
        .then(() => {
          affiliateUI.commissionIndex.init();
    ...
    if (affiliateRedemption.requests.first.m === 0) {
      ...
      $(".plan-name", $dialog).text(l.redemption_confirmation_pro_plan_name.replace("%1", affiliateRedemption.plan.planName));
  ...
  bindDialogEvents: function () {
    ...
    $(".prev-btn", this.$dialog).rebind("click", function () {
      ...
      if (affiliateRedemption.currentStep === 1) {
        $("#affi-bitcoin-address", this.$dialog).val("");
        const $checkbox = $(".step2 .checkdiv.checkboxOn", this.$dialog).removeClass("checkboxOn").addClass("checkboxOff");
    ...
    for (const type in M.affiliate.redeemGateways) {
      ...
      $clone
        .children(".radioOff")
        .children()
        .attr("id", "affiliate-payment-type" + type)
        .val(type);
      $clone
        .children("label")
        .attr("for", "affiliate-payment-type" + type)
        .text(radioText[type]);
    ...
    $amount.rebind("blur", function () {
      ...
      if (!val) {
        ...
        this.value = $this.attr("type") === "text" ? formatCurrency(val, "EUR", "number") : parseFloat(val).toFixed(2);
      }
    });
    ...
    $("#affi-bitcoin-address", $step2).rebind("input.changeBitcoinAddress", function () {
      if (!this.value) {
        ...
        M.affiliate.redeemAccDefaultInfo &&
        M.affiliate.redeemAccDefaultInfo.an &&
    ...
    var __fillupForm = function (empty) {
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        if ($target.is(".megaInputs.underlinedText")) {
          ...
          $activeOption = empty ? $(".option:first", $target) : $('.option[data-type="' + savedValue + '"]', $target);
    ...
    $saveDataTip.add($saveDataTipBitcoin).rebind("click.updateAccData", function (e) {
      ...
      const _validation =
        affiliateRedemption.requests.first.m === 2
          ? _bitcoinValidation
          : affiliateRedemption.validateDynamicAccInputs.bind(affiliateRedemption);
  ...
  displaySteps: function () {
    ...
    if ((affiliateRedemption.currentStep === 0 && u_attr.p === pro.ACCOUNT_LEVEL_BUSINESS) || u_attr.p === pro.ACCOUNT_LEVEL_PRO_FLEXI) {
      affiliateRedemption.currentStep = 1;
      affiliateRedemption.requests.first.m = 2;
    }
  ...
  displayStep1: function () {
    ...
    if (currentPlan === 0) {
      ...
      const megaInput =
        $amountInput.data("MegaInputs") ||
        new mega.ui.MegaInputs($amountInput, {
    ...
    // Method text
    $(".withdraw-txt .method-chosen", $currentStep).text(affiliateRedemption.getMethodString(affiliateRedemption.requests.first.m));
  },
  ...
  handleProPlans: function () {
    ...
    const updateRadioButtons = (selection) => {
      ...
      affiliateUI.redemptionDialog.__renderDropdown("redemption-plans", monthsOptions, defaultMonths, this.$dialog);
    };
    ...
    const fetchPlansData = () => {
      ...
      for (const currentPlan of pro.membershipPlans) {
        ...
        if (acceptedPlans.has(planNum) && months === 1) {
          ...
          $clone.children(".megaquota-option-label").children(".meqaquota-option-name").text(planName);
          $clone
            .children(".megaquota-option-label")
            .children(".meqaquota-option-information")
  ...
  getFormattedPlanData: (shortform, data) => {
    ...
    const { a, la, f, lf, c, m, s, t } = data === undefined ? affiliateRedemption.req1res[0] : data;
  ...
  getCurrentPlanInfo: (selection) => {
    ...
    for (const currentPlan of pro.membershipPlans) {
      ...
      if (
        planInfo.planNum === selection &&
        planInfo.planNum === planInfo.previousPlanNum &&
        planInfo.planNum !== -1 &&
        planInfo.previousPlanNum !== -1
      ) {
  ...
  updateQuotaSummaryTable: (selection) => {
    ...
    if (!selection || !numMonths) {
      ...
      $(".affiliate-redeem .summary-wrap .pro-price .plan-info", this.$dialog).text(
        formatCurrency("", M.affiliate.balance.localCurrency, "narrowSymbol").replace("NaN", "-")
      );
      $(".affiliate-redeem .summary-wrap .pro-price .euro", this.$dialog).addClass("hidden");
      $(".next-btn", this.$dialog).addClass("disabled");
      $(".insufficient-quota-warning", this.$dialog).addClass("hidden");
      $planSelectMessage.removeClass("hidden insufficient-quota-warning").addClass("under-price-warning").text(l.redemption_cost_too_low);
    ...
    if (numMonths === "Claim all commission") {
      const claimAllMonths = calculateClaimAllMonths(M.affiliate.balance.available, planInfo.yearlyPriceEuros, planInfo.monthlyPriceEuros);
    ...
    if (affiliateRedemption.plan.chosenPlan !== -1 && numMonths && cost >= minValue && cost <= M.affiliate.balance.available) {
      $nextBtn.removeClass("disabled");
    } else {
      $nextBtn.addClass("disabled");
    }
    if (cost > M.affiliate.balance.available) {
      $planSelectMessage
        .removeClass("hidden under-price-warning")
        .addClass("insufficient-quota-warning")
  ...
  setActive: function (type, activeItem, $dropdown, $currentStep) {
    ...
    if (type === "redemption-plans") {
      ...
      $("#affi-" + type + " > span", $currentStep).text(type === "country" ? M.getCountryName(activeItem) : activeItem);
    }
  },
  ...
  __renderDropdown: function (type, list, activeItem, $currentStep) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (type === "country") {
        ...
        displayName = item === "Claim all commission" ? l.redemption_claim_max_months : mega.icu.format(l[922], item);
        if (item % 12 === 0) {
  ...
  displayStep2: function () {
    ...
    // Summary table update
    $(".requested.price .euro", this.$dialog).addClass("hidden").text("------");
    $(".requested.price .local", this.$dialog).text(formatCurrency(affiliateRedemption.requests.first.p));
    $(".fee.price .euro", this.$dialog).addClass("hidden").text("------");
    $(".fee.price .local", this.$dialog).text("------");
    $(".received.price .euro", this.$dialog).addClass("hidden").text("------");
    $(".received.price .local", this.$dialog).text("------");
    ...
    var activeCountry = affiliateRedemption.requests.first.cc || seletedGWDefaultData[0] || selectedGWData.data.cc[0];
    var activeCurrency = affiliateRedemption.requests.first.c || seletedGWDefaultData[1] || selectedGWData.data.$[0];
  ...
  displayStep3: function () {
    ...
    // Summary table update
    if (req1.c !== "EUR") {
      $(".requested.price .euro", this.$dialog)
        .removeClass("hidden")
        .text(`(${formatCurrency(req1.p)})`);
      $(".fee.price .euro", this.$dialog)
        .removeClass("hidden")
        .text(`(${formatCurrency(req1res.f)})`);
      $(".received.price .euro", this.$dialog)
        .removeClass("hidden")
        .text(`(${formatCurrency(affiliateRedemption.requests.first.p - req1res.f)})`);
    }

    if (affiliateRedemption.requests.first.m === 2) {
      $(".requested.price .local", this.$dialog).text("BTC " + parseFloat(req1res.la).toFixed(8));
    ...
    $(".requested.price .local", this.$dialog).text(formatCurrency(req1res.la, req1res.lc, "code"));
    $(".fee.price .local", this.$dialog).text(formatCurrency(req1res.lf, req1res.lc, "code") + (req1.c === "EUR" ? "" : "*"));
    $(".received.price .local", this.$dialog).text(formatCurrency(req1res.la - req1res.lf, req1res.lc, "code"));
        ...
        M.affiliate
          ...
          .then(function (res) {
            ...
            for (var i = 0; i < subtractions.length; i++) {
              $("#m" + MurmurHash3(subtractions[i].key))
                .parent()
                .remove();
            }
            ...
            $(".option", $extraWrapper).rebind("click.showSaveTooltip", self.__showSaveDataTip.bind(self));
  ...
  bindEvents: function () {
    ...
    $("th.sortable", self.$block).rebind("click", function () {
      ...
      if ($icon.hasClass("desc")) {
        $(".mega-data-box th.sortable i", this.$block).removeClass("desc asc sprite-fm-mono icon-dropdown");
  ...
  drawTable: function () {
    ...
    if (this.list.length) {
      ...
      for (var i = 0; i < this.list.length; i++) {
        ...
        var $itemSummary = $itemSummaryTemplate.clone().removeClass("hidden template").addClass(itemStatus.class);
...
affiliateUI.purchaseIndex = {
    ...
    api_req(payload, {
      callback: function (results) {
        ...
        for (var i = 1; i < results.length; i++) {
          plans.push([results[i].id, results[i].al]);
        }
  ...
  count: function () {
    ...
    const updatePurchaseIndex = (plans) => {
      ...
      $(".list-item.prol .label", this.$purchaseChartBlock).text(formatPercentage(this.countedData[4] / this.totalCount || 0));
      $(".list-item.prol .num", this.$purchaseChartBlock).text(this.countedData[4] || 0);
      $(".list-item.pro1 .label", this.$purchaseChartBlock).text(formatPercentage(this.countedData[1] / this.totalCount || 0));
      $(".list-item.pro1 .num", this.$purchaseChartBlock).text(this.countedData[1] || 0);
      $(".list-item.pro2 .label", this.$purchaseChartBlock).text(formatPercentage(this.countedData[2] / this.totalCount || 0));
      $(".list-item.pro2 .num", this.$purchaseChartBlock).text(this.countedData[2] || 0);
      $(".list-item.pro3 .label", this.$purchaseChartBlock).text(formatPercentage(this.countedData[3] / this.totalCount || 0));
      $(".list-item.pro3 .num", this.$purchaseChartBlock).text(this.countedData[3] || 0);
      $(".list-item.pro101 .label", this.$purchaseChartBlock).text(formatPercentage(this.countedData[101] / this.totalCount || 0));
      $(".list-item.pro101 .num", this.$purchaseChartBlock).text(this.countedData[101] || 0);
      $(".list-item.business .label", this.$purchaseChartBlock).text(formatPercentage(this.countedData.b / this.totalCount || 0));
...
affiliateUI.geographicDistribution = {
  ...
  drawTable: function () {
    ...
    var _htmlFunc = function (item) {
      ...
      html += template
        .replace("$countryImg", staticpath + "images/flags/" + country.icon)
        .replace("$countryName", country.name || "Unknown")
        .replace("$count", countList[item]);
    };
