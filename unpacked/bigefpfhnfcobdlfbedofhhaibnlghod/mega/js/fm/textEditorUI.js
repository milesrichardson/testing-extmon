...
mega.textEditorUI = new (function TextEditorUI() {
  ...
  var validateAction = function (msg, submsg, callback) {
    if (!$saveButton.hasClass("disabled")) {
      msgDialog("confirmation", "", msg, submsg, function (e) {
        if (e) {
          callback();
        } else {
          editor.focus();
  ...
  var confirmSaveOrExit = () => {
    if ($saveButton.hasClass("disabled")) {
      ...
      msgDialog("save_discard_cancel", "", l.msg_dlg_modified_title, l.msg_dlg_modified_text, (e) => {
        if (e === 1) {
  ...
  var init = function (txt, $viewerContainer) {
    ...
    $(".file-btn", $menuBar).rebind("click.txt-editor", function textEditorMenuOpen() {
    ...
    $(".format-btn", $menuBar).rebind("click.txt-editor", function textEditorMenuOpen() {
      if ($(this).hasClass("disabled")) {
        return false;
      }
    ...
    $editorContainer.rebind("mouseup.txt-editor", function textEditorGlobalClick() {
      contextMenu.close(fileMenu);
      contextMenu.close(formatMenu);
      return false;
    });

    $("header .close-btn, .file-menu .close-f", $editorContainer).rebind("click.txt-editor", function textEditorCloseBtnClick() {
      if (editor) {
        confirmSaveOrExit();
      } else {
        history.back();
    ...
    $saveButton.rebind("click.txt-editor", function textEditorSaveBtnClick(e, cb) {
      if ($(this).hasClass("disabled")) {
        return false;
      }
      if (editor) {
        ...
        if (rights === 1 && M.getNodeRoot(fileHandle) === "shares") {
          ...
          (val ? mega.fileTextEditor.saveFileAs(name, M.d[fileHandle].p, val) : M.addNewFile(name, M.d[fileHandle].p))
            .then(getSavedFile)
            .catch(tell)
            .finally(() => {
              loadingDialog.hide();
    ...
    $(".file-menu .open-f", $menuBar).rebind("click.txt-editor", function openFileClick() {
      M.initFileAndFolderSelectDialog("openFile", selectedItemOpen);
    });

    $(".file-menu .save-f", $menuBar).rebind("click.txt-editor", function saveFileMenuClick() {
      $saveButton.trigger("click");
    });

    $(".file-menu .new-f", $menuBar).rebind("click.txt-editor", function newFileMenuClick() {
      validateAction(l[22750], l[22752], () => {
        // loadingDialog.show();
        openSaveAsDialog({ name: "New file.txt" }, "", (handle) => {
          M.getStorageQuota().then((data) => {
            loadingDialog.hide();
            if (data.isFull) {
              ulmanager.ulShowOverStorageQuotaDialog();
    ...
    $saveAsBtn.rebind("click.txt-editor", function saveAsMenuClick() {
      // loadingDialog.show();
      var editedTxt = editor.getValue();
      if (editedTxt === savedFileData) {
        editedTxt = null;
      }
      openSaveAsDialog(versionHandle || fileHandle, editedTxt, (handle) => {
        M.getStorageQuota().then((data) => {
          loadingDialog.hide();
          if (data.isFull) {
            ulmanager.ulShowOverStorageQuotaDialog();
    ...
    $(".file-menu .get-link-f", $menuBar).rebind("click.txt-editor", function getLinkFileMenuClick() {
    ...
    $(".file-menu .send-contact-f", $menuBar).rebind("click.txt-editor", function sendToContactMenuClick() {
      selectionManager.clear_selection();
      selectionManager.add_to_selection(versionHandle || fileHandle);
    ...
    $(".format-menu .wrap-text", $editorContainer).rebind("click.txt-editor", function wrapTextMenuClick() {
    ...
    $("footer .download-btn", $editorContainer).rebind("click.txt-editor", function downloadBtnClicked() {
    ...
    $editorContainer.rebind("keydown.txt-editor", function keydownHandler(event) {
      if (event[hotkey]) {
        switch (event.code) {
          case "KeyS":
            if (event.shiftKey) {
