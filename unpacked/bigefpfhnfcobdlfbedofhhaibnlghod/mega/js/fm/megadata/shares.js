...
MegaData.prototype.openSharingDialog = function (target) {
  ...
  const fillShareDlg = async (h) => {
    ...
    if ($folderName.get(0).offsetWidth < $folderName.get(0).scrollWidth) {
      $folderName.addClass("simpletip").attr("data-simpletip", name).attr("data-simpletipposition", "top");
    }
...
MegaData.prototype.initShareAddDialog = function (alreadyAddedContacts, $extraContent) {
  ...
  if (!megaChatIsReady) {
    if (megaChatIsDisabled) {
      ...
      loadingDialog.show();
      mBroadcaster.once("chat_initialized", this.initShareAddDialog.bind(this, alreadyAddedContacts, $extraContent));
    }
    return false;
  }
    ...
    if (!$(this).hasClass("disabled") && !$(this).parents(".share-add-dialog").is(".error")) {
      ...
      if ($newContacts.length) {
        ...
        for (var i = 0; i < $newContacts.length; i++) {
          ...
          var newContactHandle = M.findOutgoingPendingContactIdByEmail(newContactEmail) || "#new_" + MurmurHash3(newContactEmail);
  ...
  var prop = {
    ...
    onSelected: function (nodes) {
      ...
      if (
        JSON.stringify(clone($.contactPickerSelected).sort()) === JSON.stringify(alreadyAddedContacts.sort()) &&
