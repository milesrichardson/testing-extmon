(function (global) {
  ...
  var _openFolderCompletion = function (id, first) {
    ...
    this.fmsorting = id === "shares" || id === "out-shares" || id === "public-links" ? 0 : fmconfig.uisorting | 0;
    ...
    if (d) {
      console.log("previd=%s, currid=%s, currroot=%s", maph(this.previousdirid), maph(this.currentdirid), maph(this.currentrootid));
    }
    ...
    // Skip M.renderMain and clear folder nodes for sections without viewmode switchers
      ...
      if (this.overrideViewMode !== undefined) {
        ...
        typeof fmconfig.viewmodes !== "undefined" &&
        ...
        for (var i = Math.min(this.v.length, 200); i--; ) {
          ...
          if (String(n.fa).indexOf(":0*") > 0 || is_image2(n) || is_video(n) || MediaInfoLib.isFileSupported(n)) {
            viewmode = 1;
            break;
          }
        }
  ...
  MegaData.prototype.openFolder = async function (id, force) {
    ...
    if (d) {
      console.warn("openFolder(%s, %s), currentdir=%s, fmloaded=%s", maph(id), force, maph(this.currentdirid), loadfm.loaded);
    }
    ...
    if (this.gallery && (mega.gallery.sections[id] || (pfid && $viewIcons.filter(".media-view").hasClass("active")))) {
    ...
    if (id === "rubbish") {
      ...
      id &&
      (id.substr(0, 11) === "out-shares/" || id.substr(0, 13) === "public-links/" || id.substr(0, 14) === "file-requests/")
    ) {
      fetchDBNodes = true;
      id = cv.nodeID;
  ...
  lazy(MegaData.prototype, "dynContentLoader", () => {
    ...
    Object.keys(dynPages).reduce((o, k) => lazy(o, k, () => new DynContentLoader({ section: k, ...dynPages[k] })), obj);
    return obj;
  });
})(this);
