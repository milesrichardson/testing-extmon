...
MegaData.prototype.buildSubMenu = function (id) {
  ...
  if (!csb || parseInt(csb.dataset.folders) !== rootTreeLen) {
    if (rootTree) {
      ...
      sm = '<span class="dropdown body submenu" id="sm_' + rootID + '">' + '<span id="csb_' + rootID + '"></span>' + arrow + "</span>";
    }
    ...
    $(".dropdown-item.move-item").after(
      '<span class="dropdown body submenu" id="sm_move">' +
        '  <span id="csb_move">' +
        '    <span class="dropdown-item cloud-item' +
        cs +
        '" id="fi_' +
        rootID +
        '">' +
...
MegaData.prototype.getSelectedSourceRoot = function (isSearch) {
  ...
  let sourceRoot =
    isSearch || M.currentdirid === "recents" || M.currentdirid === "public-links" || M.currentdirid === "out-shares"
      ? M.getNodeRoot($.selected[0])
      : M.currentrootid;
...
MegaData.prototype.checkSendToChat = function (isSearch, sourceRoot) {
  ...
  if (!folderlink && window.megaChatIsReady && $.selected.length) {
    for (let i = $.selected.length; i--; ) {
      ...
      if (!n || (n.t && nRoot !== M.RootID && nRoot !== M.InboxID && nRoot !== "s4" && !M.isDynPage(nRoot)) || nRoot === M.RubbishID) {
        return false;
      }
    }
    return true;
...
MegaData.prototype.menuItems = async function menuItems() {
  ...
  if (selNode && $.selected.length === 1) {
    if (selNode.t) {
      ...
      if ((sourceRoot === M.RootID || sourceRoot === "s4" || M.isDynPage(M.currentrootid)) && !folderlink) {
        let exp = false;
        ...
        if (shares.length || M.ps[selNode.h]) {
          ...
          if (mega.fileRequest.publicFolderExists(selNode.h)) {
            ...
            if (!is_mobile) {
              fileRequestPageClass = M.currentrootid === "file-requests" ? ".file-request-page" : ":not(.file-request-page)";
            }
  ...
  if ($.selected.length > 1) {
    ...
    for (let i = 0; i < $.selected.length; i++) {
      ...
      if (!restrictedFolders && (sourceRoot === M.InboxID || M.getNodeRoot($.selected[i]) === M.InboxID)) {
  ...
  if (restrictedFolders || ($.selected.length === 1 && sourceRoot === M.InboxID)) {
    delete items[".open-cloud-item"];
    delete items[".open-in-location"];
    delete items[".move-item"];
    delete items[".rename-item"];
    ...
    if (
      ...
      $.selected.length === 1 &&
      (((selNode.devid || selNode.drvid) && selNode.td > 0) || M.d[selNode.p].devid || M.d[selNode.p].drvid || selNode.h === M.BackupsId)
    ) {
...
MegaData.prototype.contextMenuUI = function contextMenuUI(e, ll, items) {
  ...
  if (ll === 2) {
    ...
    if (M.currentdirid !== "shares" && M.currentdirid !== "out-shares") {
      ...
      if (nodeRights && M.currentrootid !== M.RubbishID && M.currentrootid !== M.InboxID && M.getNodeRoot(h) !== M.InboxID) {
    ...
    // we want just the download menu
    ...
    if (M && M.columnsWidth && M.columnsWidth.cloud) {
      ...
      var $currMenuItems = $(".files-menu.context a.dropdown-item").addClass("hidden").filter(".visible-col-select");
    ...
    if (id && id.length === 11) {
      ...
      // Enable All buttons
      $menuCmi.filter(".startaudiovideo-item, .send-files-item").removeClass("disabled disabled-submenu");
      ...
      if (M.viewmode) {
        ...
        // Set contact avatar
        $contactDetails.find(".dropdown-avatar").removeClass("hidden").find(".avatar").safeHTML(useravatar.contact(id, "context-avatar"));

        // Set username
        $contactDetails.find(".dropdown-user-name").removeClass("hidden").find(".name span").text(username);
      }
      ...
      (currNodeClass && (currNodeClass.includes("data-block-view") || currNodeClass.includes("folder"))) ||
      String(id).length === 8
    ) {
      asyncShow = true;
      const updateUIPerItems = ($menuCMI, items) => {
        ...
        if (items[".import-item"]) {
          ...
          if (u_type) {
            $("i", $importItem).removeClass("icon-mega-thin-outline").addClass("icon-upload-to-cloud-drive");

            $("span", $importItem).text(l.context_menu_import);
          } else {
            $("i", $importItem).removeClass("icon-upload-to-cloud-drive").addClass("icon-mega-thin-outline");
      ...
      M.menuItems()
        .then((items) => {
          ...
          if (!$currentTarget.attr("id")) {
            $menuCMI.filter(".download-item, .sh4r1ng-item, .send-to-contact-item," + ".getlink-item, .remove-item").addClass("hidden");
          }
...
MegaData.prototype.setContextMenuShareText = function () {
  ...
  let getLinkText = M.currentrootid === M.InboxID || M.getNodeRoot($.selected[0]) === M.InboxID ? l.read_only_share : l[5631];
...
MegaData.prototype.reCalcMenuPosition = function (m, x, y, ico) {
  ...
  if (typeof ico === "object") {
    // draw context menu relative to file-settings-icon
...
MegaData.prototype.labelSortMenuUI = function (event, rightClick) {
  ...
  $selectedItem = $sortMenuItems.filter("*[data-by=" + sorting.n + "]").addClass("active");
