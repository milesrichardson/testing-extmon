(function () {
  ...
  function getBreadcrumbDropdownHTML(items) {
    ...
    for (let item of items) {
      ...
      contents += `<a class="crumb-drop-link" data-id="${escapeHTML(item.id)}">
  ...
  function applyBreadcrumbEventHandlers(scope, dropdown, clickAction) {
    $(".breadcrumb-dropdown-link", scope).rebind("click.breadcrumb-dropdown", function (e) {
  ...
  function getPathHTML(items, dictionary, container) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (name !== "") {
        ...
        if ((!isDyhRoot && !isLastItem && !isSimpletip && currentPathLength > maxPathLength && !isInfoBlock) || (isInfoBlock && i > 1)) {
          extraItems.push({
            name,
            type: typeClass,
            id
        ...
        // otherwise, add it to the main breadcrumb
          ...
          html =
            `<a class="fm-breadcrumbs ${escapeHTML(typeClass)} ${isRoot || isDyhRoot ? "root" : ""} ui-droppable"
                            data-id="${item}" id="pathbc-${item}">
                            <span
                                class="right-arrow-bg simpletip simpletip-tc">
                                ${
                                  isRoot
                                    ? `<span class="not-loading selectable-txt">
                                        ${name}
                                    </span>
                                    <i class="loading sprite-fm-theme icon-loading-spinner"></i>`
                                    : name
                                }
                            </span>
  ...
  function removeSimpleTip($breadCrumbs) {
    ...
    for (let i = 0; i < $breadCrumbs.length; i++) {
      ...
      if (
        $("span", $currentBreadcrumb).get(0).offsetWidth >= $("span", $currentBreadcrumb).get(0).scrollWidth ||
  ...
  function breadcrumbClickHandler(id) {
    const n = this.d[id];
    const specialCases = ["shares", "out-shares", "public-links", "file-requests"];
