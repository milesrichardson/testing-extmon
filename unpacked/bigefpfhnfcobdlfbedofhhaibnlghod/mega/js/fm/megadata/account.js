...
MegaData.prototype.getAccountDetails = function () {
  ...
  return api.req({ a: "ug" }).then(({ result }) => {
    const { u_attr } = window;
...
MegaData.prototype.showRecoveryKeyDialog = function (version) {
  ...
  M.safeShowDialog("recovery-key-dialog", () => {
    ...
    $(".copy-recovery-key-button", $dialog)
      .removeClass("hidden")
      .rebind("click", () => {
...
MegaData.prototype.showContactVerificationDialog = function () {
  ...
  if (u_attr.since > 1697184000 || mega.keyMgr.getWarningValue("cvd") || mega.keyMgr.getWarningValue("cv") !== false) {
    return;
  }
...
MegaData.prototype.showPaymentCardBanner = function (status) {
  ...
  if (status === 1) {
    ...
    $(".update-payment-card", $dialog).rebind("click", () => {
      closeDialog();
...
MegaData.prototype.showOverStorageQuota = function (quota, options) {
  ...
  if (quota === EPAYWALL) {
    ...
    var maxStorage = bytesToSize(pro.maxPlan[2] * 1024 * 1024 * 1024, 0) + " (" + pro.maxPlan[2] + " " + l[17696] + ")";

    $(".body-p.long", $strgdlgBodyFull).safeHTML(
      l[22674].replace("%1", maxStorage).replace("%2", bytesToSize(pro.maxPlan[3] * 1024 * 1024 * 1024, 0))
    );
    ...
    if (Object(u_attr).p) {
      ...
      var minStorage = l[22669]
        .replace("%1", pro.minPlan[5])
        .replace("%2", pro.minPlan[2] + " " + l[17696])
    ...
    $(".fm-notification-block.almost-full").safeHTML(
      `<i class="notification-block-icon sprite-fm-mono icon-offline"></i>
  ...
  $(".fm-notification-block .fm-notification-close").rebind("click", function () {
    $(".fm-main").removeClass("fm-notification almost-full full");
    $.tresizer();
  });
