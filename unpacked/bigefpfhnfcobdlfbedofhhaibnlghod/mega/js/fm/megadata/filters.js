          ...
          .each(function () {
            ...
            if (node) {
              ...
              if (M.viewmode) {
                ...
                $this.find(".file-block-title").css({
                ...
                $this.find(".transfer-filetype-icon").css("background-color", "#" + color);
              }
            }
          });
        loadingDialog.hide();
        ...
        .then(() => {
          ...
          if (pfid && command.startsWith("ctag")) {
            for (let i = v.length; i--; ) {
              ...
              do {
                $(`#${n.h}`)
                  .removeClassWith("highlight")
                  .addClass(`highlight${(n.vhl = 1)}`);
              } while ((n = M.d[n.p]));
            }
          } else {
...
MegaData.prototype.getFilterBySearchFn = function (searchTerm) {
  ...
  if (regex) {
    return function (node) {
      return node.name && regex.test(node.name) && node.p !== "contacts" && !(node.s4 && node.p === M.RootID);
    };
  }

  return function (node) {
    return node.name && node.name.toLowerCase().includes(str) && node.p !== "contacts" && !(node.s4 && node.p === M.RootID);
  };
};
