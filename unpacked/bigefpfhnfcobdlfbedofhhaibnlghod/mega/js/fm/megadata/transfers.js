...
MegaData.prototype.putToTransferTable = function (node, ttl) {
  ...
  this.addToTransferTable(
    ...
    '<tr id="dl_' +
      htmlentities(handle) +
      '" class="transfer-queued transfer-download ' +
      state +
      '">' +
      '<td><div class="transfer-type download sprite-fm-mono-after icon-down-after">' +
      '<ul><li class="right-c"><p ' +
      rightRotate +
      ">" +
      '<span></span></p></li><li class="left-c"><p ' +
      leftRotate +
      "><span></span></p></li></ul>" +
      "</div></td>" +
      '<td><span class="transfer-filetype-icon ' +
      fileIcon(node) +
      '"></span>' +
      '<span class="tranfer-filetype-txt">' +
      htmlentities(node.name) +
      "</span></td>" +
      "<td>" +
      filetype(node) +
      "</td>" +
      '<td class="transfer-size">' +
      bytesToSize(node.s) +
      "</td>" +
      '<td><span class="downloaded-size">' +
      bytesToSize(dowloadedSize) +
      "</span></td>" +
      '<td><span class="eta"></span><span class="speed">' +
      pauseTxt +
      "</span></td>" +
      '<td><span class="transfer-status">' +
      l[7227] +
      "</span></td>" +
...
MegaData.prototype.addDownload = function (n, z, preview) {
  ...
  dbfetch.coll(n).always(function () {
...
MegaData.prototype.addWebDownload = function (n, z, preview, zipname) {
  ...
  if ($.totalDL > dlmanager.maxDownloadSize) {
    ...
    if (!fmconfig.dlThroughMEGAsync) {
      ...
      msgDialog("confirmation", "File Size is too big", msgMsg, msgSubMsg, function (activateMsync) {
        if (activateMsync) {
  ...
  if (z && zipsize) {
    this.addToTransferTable(
      ...
      '<tr id="zip_' +
        ...
        '"></span>' +
        '<span class="tranfer-filetype-txt">' +
        htmlentities(zipname) +
        "</span></td>" +
        "<td>" +
        filetype({ name: "archive.zip" }) +
        "</td>" +
        '<td class="transfer-size">' +
        bytesToSize(zipsize) +
        "</td>" +
  ...
  if (bt) {
    ...
    if (!uldl_hold) {
      ...
      if (retime > 0) {
        ...
        $tr.find(".eta").text(secondsToTime(retime)).removeClass("unknown").attr("title", title);
      } else {
    ...
    if (x != 233 || !prog.speed || !(prog.working || []).length) {
      ...
      if (page === "download") {
        if (error === EOVERQUOTA) {
          ...
          $(".download .pause-transfer").removeClass("hidden").addClass("active").find("span").text(l[1649]);
...
MegaData.prototype.addToTransferTable = function (gid, ttl, elem) {
  ...
  if (this.tfsResizeHandler) {
    M.getTransferElements().domScrollingTable.addEventListener("ps-y-reach-end", M, { passive: true });
  ...
  var startUpload = function (u) {
    ...
    for (var i = u.length; i--; ) {
      ...
      var template =
        '<tr id="' +
        gid +
        '" class="transfer-queued transfer-upload ' +
        pause +
        '">' +
  ...
  var makeDirProc = function () {
    ...
    (function _foreach() {
      ...
      if (entry) {
        ...
        fileconflict.prompt("upload", entry, node, conflicts.length, target).always(function (file, name, action, repeat) {
          if (file === -0xbadf) {
            if (d) {
...
function fm_tfsresume(gid) {
  ...
  if (ASSERT(typeof gid === "string" && "zdu".indexOf(gid[0]) !== -1, "Invalid GID to resume")) {
    ...
    if (gid[0] === "u") {
      ...
      if (
        (page === "download" && $(".download.download-page").hasClass("overquota")) ||
...
function tfsheadupdate(update) {
  ...
  if (!tfsheadupdate.stats) {
    ...
    tfsheadupdate.checkState = (type, id, state) =>
      tfsheadupdate.stats[type] && tfsheadupdate.stats[type][id] && tfsheadupdate.stats[type][id] === state ? -1 : 0;
  }
  if (update) {
    tfsheadcalc(update);
  }
  ...
  if (aul) {
    ...
    processStats(aul - ful, ul_total, ul_done, oul, eul, blocks);
  ...
  if (adl) {
    ...
    processStats(adl - fdl, dl_total, dl_done, odl, edl, blocks);
...
function tfsheadcalc(update) {
  ...
  switch (key) {
    ...
    default: {
      const id = update[key];
      if (typeof tfsheadupdate.stats[type][id] !== "undefined" && tfsheadupdate.checkState(type, id, STATE.FINISH)) {
