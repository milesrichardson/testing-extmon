...
MegaData.prototype.buildtree = function (n, dialog, stype, sSubMap) {
  ...
  if (!dialog || rebuild) {
    // dialog should not be filtered unless it is forced.
  ...
  if (n.h === M.RootID && !sSubMap) {
    ...
    if (typeof dialog === "undefined") {
      ...
      if (!folderlink && rebuild && (node = document.querySelector(".js-other-tree-panel .content-panel.cloud-drive ul"))) {
        node.remove();
      }

      if (rebuild || $(".content-panel.cloud-drive ul").length === 0) {
        $(`${wrapperClass} .content-panel.cloud-drive .tree`).safeHTML(`<ul id="treesub_${i}"></ul>`);
      }
    } else {
    ...
    if (typeof dialog === "undefined") {
      ...
      $("." + dialog + " .rubbish-bin .dialog-content-block").html('<ul id="mctreesub_' + escapeHTML(M.RubbishID) + '"></ul>');
    }
    stype = "rubbish-bin";
  ...
  if (tree) {
    ...
    if (inshares) {
      folders = folders.filter(function (n) {
        return !M.d[n.p];
      });
    }
    ...
    for (var h in labelhash) {
      if (labelhash[h]) {
        $(document.querySelector("#" + _li + h))
          .removeClass("tree-item-on-filter-hidden")
          .parents("li")
          .removeClass("tree-item-on-filter-hidden");
      }
    }
...
MegaData.prototype.redrawTreeFilterUI = function () {
  ...
  for (var i in M.filterTreePanel[type + "-label"]) {
    if (M.filterTreePanel[type + "-label"][i]) {
      ...
      $filterBlock.find(".content").append(fltIndicator.replace("%1", M.filterTreePanel[type + "-label"][i].txt));
    }
  }
};
  ...
  $(".nw-tree-panel-arrows").rebind("click", function () {
    ...
    if (!$self.hasClass("active")) {
      ...
      if (M.isLabelExistTree()) {
        ...
        menu.find(".dropdown-item-label").add(".fm-left-panel .filter-by .labels").removeClass("static disabled");
      } else {
        // lbl is not exist disable filter on DOM.
        menu.find(".dropdown-item-label").add(".fm-left-panel .filter-by .labels").addClass("static disabled");
      }
...
MegaData.prototype.addTreeUI = function () {
  ...
  $treeItem.draggable({
    ...
    start: function (e, ui) {
      ...
      if (id && M.d[id]) {
        html =
          '<div class="tree-item-dragger nw-fm-tree-item">' +
          '<span class="nw-fm-tree-folder ' +
          fileIcon(M.d[id]) +
          '"></span>' +
          '<span class="item-name">' +
          escapeHTML(M.d[id].name) +
          "</span>" +
          "</div>";
      }
  ...
  $treeItem.rebind("click.treeUI contextmenu.treeUI", function (e) {
    ...
    if (e.type === "contextmenu") {
      ...
      if ($uls.length > 1) {
        $.selected = $uls.attrs("id").map(function (id) {
...
MegaData.prototype.onTreeUIOpen = function (id, event, ignoreScroll) {
  ...
  if (target) {
    ...
    if (
      (fmconfig.uiviewmode | 0 && fmconfig.viewmode === 2) ||
      (typeof fmconfig.viewmodes !== "undefined" && typeof fmconfig.viewmodes[id] !== "undefined" && fmconfig.viewmodes[id] === 2)
    ) {
      target.classList.add("on-gallery");
    }
  }
