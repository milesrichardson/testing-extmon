lazy(s4, "buckets", () => {
  ...
  class S4CreateDialog extends S4Dialog {
    constructor(name) {
      ...
      this.megaInput = new mega.ui.MegaInputs(this.$bucketNameInput, { onShowError: true });
    }
    ...
    bindEvents() {
      ...
      this.$dialogProgress.rebind("click.s4dlg", () => {
        ...
        if (this.progress || !name || !this.containerId || this.$dialogProgress.hasClass("disabled")) {
          return false;
        }
  ...
  class S4SettingsDialog extends S4Dialog {
    ...
    bindEvents() {
      this.$tabs.rebind("click.s4dlg", (e) => {
        ...
        $(`.${$activeTab.removeClass("active").data("target")}`, this.$dialogContainer).addClass("hidden");
        $(`.${$clickedTab.addClass("active").data("target")}`, this.$dialogContainer).removeClass("hidden");
      ...
      $(".radio-wrap", this.$dialogContainer).rebind("click.s4dlg", (e) => {
        ...
        this.$radioInputs.prop("checked", false).parent().removeClass("radioOn").addClass("radioOff");
    ...
    changeProgressState() {
      ...
      if (
        (typeof this.publicAccess === "number" && this.origAccess !== this.publicAccess) ||
    ...
    validateStatement(obj) {
      const validKeys = new Set(["Action", "NotAction", "Condition", "Effect", "Resource", "Principal", "NotPrincipal", "Sid"]);
    ...
    async _setTabContent(n) {
      ...
      this.infoTab.$contains.text(
        mega.utils.trans.listToString(
          [mega.icu.format(l.folder_count, this.bucket.td), mega.icu.format(l.file_count, this.bucket.tf)],
          "%s"
        )
      );
      ...
      if (this.origPolicy) {
        this.origPolicy = JSON.stringify(this.origPolicy, (_key, value) => (value instanceof Set ? [...value] : value), 4);
      }
