lazy(s4, "userProperties", () => {
  ...
  class S4AddGroupDialog extends S4UsersCreateDialog {
      ...
      if ($("tbody tr", this.$steps[2]).length === 1) {
        ...
        if (html) {
          ...
          $(".group-table tbody input", this.$steps[2]).rebind("click.s4dlg", (e) => {
            ...
            this.changes[$this.attr("data-name").replace("cg_", "")] = $this.closest(".group-check").hasClass("checkboxOn");
    ...
    async _init() {
      ...
      this.container.groups = this.container.groups.filter((u) => !this.user.groups.some((ug) => u.gid === ug.gid));
    ...
    async _updateUserGroup(group) {
      ...
      this.user.groups = this.container.groups.filter((g) => group.includes(g.name)).map((g) => parseInt(g.gid));
      const promises = this.user.groups.map((g) => s4.kernel.group.addUsers(containerID, g, [s4.ui.lastPathPart]));
  ...
  class S4AddPolicyDialog extends S4UsersCreateDialog {
    ...
    async step3(finalise) {
      ...
      $(".head-title", this.$steps[3]).text(this.isGroup ? l.s4_group_attach_policy : l.s4_user_attach_policy);
      ...
      if ($("tbody tr", this.$steps[3]).length === 1) {
        ...
        if (html) {
          ...
          $(".policy-table tbody input", this.$steps[3]).rebind("click.s4dlg", (e) => {
            ...
            this.changes[$this.attr("data-name").replace("cp_", "")] = $this.closest(".policy-check").hasClass("checkboxOn");
            this.checkTaint(this.origSelected, this.changes);
          });
        }
      }
    ...
    async _init() {
      ...
      this.container.policies = this.container.policies.filter((p) => !this.user.policies.some((up) => p.policyName === up.name));
    ...
    async _attachPolicies(policies) {
      ...
      this.user.policies = this.container.policies.filter((p) => policies.includes(p.policyName)).map((p) => p.arn);
        ...
        asyncMsgDialog(
          ...
          async (yes) => {
            if (yes) {
              ...
              s4.kernel.group
                .removeUsers(containerID, groupId, [parseInt(s4.ui.lastPathPart)])
                .catch(tell)
  ...
  class S4PolicyList extends S4List {
    ...
    bindEvents() {
      ...
      $(".s4-grid-table tbody tr .delete-policy", this.$fmBlock).rebind("click.delete-policy", (e) => {
        const { containerID } = M.currentCustomView;
        const tr = e.target.closest("tr");
        ...
        msgDialog(
          ...
          (yes) => {
            if (yes) {
              ...
              s4.kernel.user
                .info(containerID, s4.ui.lastPathPart)
                .then((user) => s4.kernel.policies.detachUserPolicy(containerID, user.name, policyArn))
                .catch(tell)
