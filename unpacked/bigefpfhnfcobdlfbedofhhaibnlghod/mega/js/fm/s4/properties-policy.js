lazy(s4, "policyProperties", () => {
  ...
  class S4AddUserDialog extends S4GroupsCreateDialog {
      ...
      if ($("tbody tr", this.$steps[3]).length === 1) {
        ...
        if (html) {
          ...
          $(".group-users-table tbody input", this.$steps[3]).rebind("click.s4dlg", (e) => {
            ...
            this.changes[$this.attr("data-name").replace("cu_", "")] = $this.closest(".group-user-check").hasClass("checkboxOn");
    ...
    async _init() {
      ...
      await Promise.all(
        this.container.users.map(async (user) => {
    ...
    async _updatePolicyUsers(users) {
      ...
      for (let i = 0; i < users.length; ++i) {
        promises.push(s4.kernel.policies.attachUserPolicy(M.currentCustomView.containerID, users[i], this.policy.arn));
      }
  ...
  class S4AddGroupDialog extends S4UsersCreateDialog {
    ...
    async _init() {
      ...
      await Promise.all(
        this.container.groups.map(async (group) => {
    ...
    async _updatePolicyGroup(groups) {
      ...
      for (let i = 0; i < groups.length; ++i) {
        promises.push(s4.kernel.policies.attachGroupPolicy(M.currentCustomView.containerID, groups[i], this.policy.arn));
      }
      await Promise.all(promises)
        .catch(tell)
  ...
  const handlers = freeze({
    ...
    async showPolicyDoc() {
      ...
      const policyDoc = (p && (await s4.kernel.policies.getPolicyVersion(p.arn, p.v, containerID).catch(nop))) || {};
        ...
        asyncMsgDialog(
          ...
          async (yes) => {
            if (yes) {
              ...
              s4.kernel.group
                .info(containerID, groupId)
                .then((group) => s4.kernel.policies.detachGroupPolicy(containerID, group.name, this.policy.arn))
                .catch(tell)
        ...
        asyncMsgDialog(
          ...
          async (yes) => {
            if (yes) {
              ...
              s4.kernel.user
                .info(containerID, userId)
                .then((user) => s4.kernel.policies.detachUserPolicy(containerID, user.name, this.policy.arn))
                .catch(tell)
