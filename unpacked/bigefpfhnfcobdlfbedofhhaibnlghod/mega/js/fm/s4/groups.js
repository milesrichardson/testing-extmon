lazy(s4, "groups", () => {
  ...
  const handlers = freeze({
    ...
    remove() {
      return s4.ui.lists.groups.findSelected().then((item) => {
        const { gid, name } = item;
        return asyncMsgDialog(
          ...
          async (yes) => {
            if (yes) {
              ...
              return s4.kernel.group
                .remove(M.currentCustomView.containerID, parseInt(gid))
                .catch(tell)
    ...
    requiredLinks() {
      return s4.ui.lists.groups
        ...
        .then(() => {
          return {
            hide: ["options", "delete"],
            show: ["s4-group-copy-arn", "s4-group-rename", "s4-group-delete"]
          };
        })
        .catch(tell);
    },
  ...
  class S4CreateDialog extends S4PagedDialog {
    ...
    bindEvents() {
      ...
      const { $steps, $groupInput, $dialogProgress } = this;
      ...
      $("input.users-search", $steps[3]).rebind("input.s4dlg", (e) => {
        ...
        if (val) {
          ...
          $(".group-users-table .group-user-row.hidden:not(.group-user-template)", $steps[3]).removeClass("hidden");
        }
      });
    }
    ...
    async step1(finalise) {
      if (finalise) {
        ...
        return handlers
          .checkNameAvailable(this.group.name)
          .then(() => {
    ...
    async _initList() {
      ...
      const [users = [], policies = []] = await Promise.all([s4.kernel.user.list(containerID), s4.kernel.policies.list(containerID)]);
    ...
    async _createGroup() {
      ...
      const users = this.container.users.filter((u) => this.group.users.includes(u.name)).map((u) => parseInt(u.uid));

      const policies = this.container.policies.filter((u) => this.group.policies.includes(u.policyName)).map((u) => u.arn);

      return s4.kernel.group
        .create(containerID, this.group.name, policies)
        .then((g) => {
    ...
    bindEvents() {
      ...
      this.$tableData.rebind("contextmenu.s4klst", (e) => {
        this.handleSelection(e, () => {
          ...
          M.contextMenuUI(e, 8, [".s4-group-copy-arn", ".s4-group-rename", ".s4-group-delete"].join(","));
        });
      });
