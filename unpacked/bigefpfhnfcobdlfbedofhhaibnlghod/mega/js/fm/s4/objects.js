lazy(s4, "objects", () => {
  ...
  class S4AccessDialog extends S4Dialog {
    ...
    show(node) {
      ...
      s4.kernel.object
        .getPublicURL(this.object.h)
        .then((res) => {
          $("input", this.$linkContainer).val(res || "");
        })
        .catch(tell)
        .finally(() => {
          $(".transfer-filetype-icon", this.$fileInfoContainer).attr("class", `transfer-filetype-icon ${fileIcon(this.object)}`);
          $(".file-name", this.$fileInfoContainer).text(this.object.name);
          $(".file-size", this.$fileInfoContainer).text(bytesToSize(this.object.s));

          this.$notification = $(".bucket-access-warning", this.$dialogContainer).addClass("hidden");
    ...
    bindEvents() {
      this.$switch.rebind("click.s4dlg", () => {
        ...
        s4.kernel.object
          .publicURLAccess(this.object.h, this.publicAccess)
          .then(() => {
            this.togglePublicAccessUI();
          })
          .finally(() => {
