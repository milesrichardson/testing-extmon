lazy(s4, "keys", () => {
  ...
  const handlers = freeze({
    ...
    copyKeyConfig() {
      return s4.ui.lists.keys
        ...
        .then((item) => {
          const { ak, sk } = item;
          copyToClipboard(exportKey(ak, sk), l.s4_key_copied_toast_txt, "recoveryKey");
        })
        .catch(tell);
    },
    ...
    toggleVis(e) {
      ...
      if ($icon.hasClass("icon-eye-reveal")) {
        ...
        $("button.toggle-vis i.icon-eye-hidden", $table).removeClass("icon-eye-hidden").addClass("icon-eye-reveal");
    ...
    remove() {
      return s4.ui.lists.keys.findSelected().then((item) => {
        const { n, ak } = item;
        return asyncMsgDialog(
          ...
          async (yes) => {
            if (yes) {
              ...
              return s4.kernel.keys
                .remove(s4.ui.lists.keys.handle, ak)
                .catch(tell)
  ...
  class S4CreateDialog extends S4PagedDialog {
    ...
    hide() {
      ...
      this.$radioInputs.filter("[value=rootuser]").prop("checked", true).parent().removeClass("radioOff").addClass("radioOn");
    ...
    bindEvents() {
      ...
      const { $steps, $keyInput, $radioInputs, $dialogProgress } = this;
      // Step 1 events
    ...
    async step1(finalise) {
      if (finalise) {
        ...
        const ui = $(".radioOn input[value=rootuser]", this.$steps[1]).length
          ? null
          : $("tr.active", this.$steps[1]).prop("id").replace("cku_", "");
    ...
    _getUserError() {
      if (!($(".radioOn input", this.$steps[1]).val() === "rootuser" || $("tr.active", this.$steps[1]).length)) {
