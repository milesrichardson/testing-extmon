lazy(s4, "users", () => {
  ...
  const handlers = freeze({
    ...
    remove() {
      return s4.ui.lists.users.findSelected().then((item) => {
        const { uid, name } = item;
          ...
          async (yes) => {
            if (yes) {
              ...
              return s4.kernel.user
                .remove(M.currentCustomView.containerID, parseInt(uid))
                .catch(tell)
    ...
    requiredLinks() {
      return s4.ui.lists.users
        ...
        .then(() => {
          return {
            hide: ["options", "delete"],
            show: ["s4-user-copy-arn", "s4-user-rename", "s4-user-delete"]
          };
        })
        .catch(tell);
    },
  ...
  class S4CreateDialog extends S4PagedDialog {
    ...
    unbindEvents() {
      ...
      const { $steps, $userInput, $groupInput, $saveGroupButton } = this;
    ...
    bindEvents() {
      ...
      const { $steps, container, $userInput, $groupInput, $saveGroupButton, $dialogProgress } = this;
      ...
      $saveGroupButton.rebind("click.s4dlg", (e) => {
        ...
        if (group) {
          ...
          s4.groups.handlers
            .checkNameAvailable(group)
            .then(() =>
              s4.kernel.group
                .create(containerID, group, [])
                .then((gid) =>
                  s4.kernel.group.info(containerID, gid).then((g) => {
                    const newGroup = { ...g, gid };
                    container.groups.push(newGroup);
                    $(".group-table tbody", $steps[2]).safeAppend(this._createGroupRow(newGroup));
                    $(`.group-table tbody input[data-name="cg_${g.name}"]`, $steps[2]).rebind("click.s4dlg", (e) => {
    ...
    async step1(finalise) {
      if (finalise) {
        ...
        return handlers
          .checkNameAvailable(this.user.name)
          .then(() => {
    ...
    async step3(finalise) {
      if (finalise) {
        ...
        this.user.policies = $(".checkboxOn input[data-name]", this.$steps[3])
          .toArray()
          .map((row) => row.dataset.name.replace("cp_", ""));
        return;
      }
    ...
    async _initList() {
      ...
      const [groups = [], policies = []] = await Promise.all([s4.kernel.group.list(containerID), s4.kernel.policies.list(containerID)]);

      Object.assign(this.container, { groups, policies });

      if (this.container.groups && this.container.groups.length > 0) {
        await Promise.all(
          this.container.groups.map(async (g) => {
    ...
    async _createUser() {
      ...
      this.user.groups = this.container.groups.filter((g) => this.user.groups.includes(g.name)).map((g) => g.gid);

      this.user.policies = this.container.policies.filter((p) => this.user.policies.includes(p.policyName)).map((p) => p.arn);
    ...
    _createGroupRow(group) {
      ...
      if (this.container.groupPolicies[gid]) {
        policyString =
          this.container.groupPolicies[gid].length > 1
    ...
    bindEvents() {
      ...
      this.$tableData.rebind("contextmenu.s4klst", (e) => {
        this.handleSelection(e, () => {
          ...
          M.contextMenuUI(e, 8, [".s4-copy-arn", ".s4-user-rename", ".s4-user-delete"].join(","));
        });
      });
