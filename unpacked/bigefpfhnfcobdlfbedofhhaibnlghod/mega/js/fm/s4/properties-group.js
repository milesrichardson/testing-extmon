lazy(s4, "groupProperties", () => {
  ...
  class S4AddUserDialog extends S4CreateDialog {
    ...
    async _init() {
      ...
      this.container.users = (this.container.users || []).filter((u) => !this.group.users.some((ug) => u.uid === ug.uid));
    ...
    async _updateGroupUser(groups) {
      ...
      s4.kernel.group
        .addUsers(M.currentCustomView.containerID, s4.ui.lastPathPart, this.group.users)
        .catch(tell)
        ...
        asyncMsgDialog(
          ...
          async (yes) => {
            if (yes) {
              ...
              s4.kernel.group
                .removeUsers(containerID, s4.ui.lastPathPart, [userId])
                .catch(tell)
          ...
          async (yes) => {
            if (yes) {
              ...
              s4.kernel.group
                ...
                .then((group) => s4.kernel.policies.detachGroupPolicy(containerID, group.name, policyArn))
                .catch(tell)
