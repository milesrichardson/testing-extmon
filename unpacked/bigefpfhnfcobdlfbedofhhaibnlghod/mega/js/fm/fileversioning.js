  ...
  var ns = {
    ...
    getTopNodeSync: function (h) {
      ...
      while (M.d[h] && M.d[M.d[h].p]) {
        if (M.d[M.d[h].p].t !== 0) {
          // the node's parent is not a file.
          node = h;
          break;
        }
    ...
    clearPreviousVersions: function (h) {
      return fileversioning.getAllVersions(h).then((versions) => {
    ...
    updateVersionInfo: function () {
      ...
      const verionInfo = mega.icu
        .format(l.version_file_summary, data.versions.cnt)
        .replace("[X]", `<span class="versioning-text total-versions-size">${bytesToSize(data.versions.size)}</span>`);
    ...
    fileVersioningDialog: function (handle) {
      ...
      var fillVersionList = function (versionList) {
        ...
        for (var i = 0; i < versionList.length; i++) {
          ...
          var actionHtml = v.u === u_handle ? l[16480] : l[16476].replace("%1", (M.u[v.u] && M.u[v.u].m) || l[7381]);
          if (i < versionList.length - 1) {
            if (v.name !== versionList[i + 1].name) {
              actionHtml = l[17156].replace("%1", "<span>" + htmlentities(versionList[i + 1].name) + "</span>");
            }
          }
          ...
          var downBtnHtml = `<div class="mega-button small action download-file simpletip"
                            data-simpletip="${l[58]}"
                            aria-label="${l[58]}"
                            id="vdl_${v.h}">
                            <i class="sprite-fm-mono icon-download-small"></i>
                        </div>`;
          var viewBtnHtml = `<div class="mega-button small action preview-file simpletip"
                            data-simpletip="${l.version_preview}"
                            aria-label="${l.version_preview}"
                            id="vdl_${v.h}">
                            <i class="sprite-fm-mono icon-file-edit"></i>
                        </div>`;
          var revertBtnHtml = `<div class="mega-button small action revert-file simpletip"
          ...
          if ((nodeData && nodeData.r < 2) || (i === 0 && fileversioning.getTopNodeSync(current_sel_version[0]) === v.h)) {
            revertBtnHtml = `<div class="mega-button small action revert-file disabled nonclickable simpletip"
                                data-simpletip="${l[16475]}"
                                aria-label="${l[16475]}"
                                id="vrv_${v.h}">
          ...
          var deleteBtnHtml = `<div class="mega-button small action delete-file simpletip"
                            data-simpletip="${l[1730]}"
                            aria-label="${l[1730]}"
                            id="vde_${v.h}">
                            <i class="sprite-fm-mono icon-bin"></i>
                        </div>`;
          if ((nodeData && nodeData.r < 2) || !M.d[fh].tvf) {
            // if the user does not have full access of the shared folder.
            deleteBtnHtml = `<div class="mega-button small action delete-file disabled nonclickable"
                                data-simpletip="${l[1730]}"
                                aria-label="${l[1730]}"
                                id="vde_${v.h}">
      ...
      var refreshHeader = function (fileHandle) {
        ...
        if (current_sel_version.length > 1 || current_sel_version[0] === topNodeHandle || (nodeData && nodeData.r < 2)) {
      ...
      $(".fm-versioning .pad .top-column button.js-delete").rebind("click", function () {
        if (M.isInvalidUserStatus()) {
          return;
        }
      ...
      fileversioning.getAllVersions(fh).then((versions) => {
        ...
        $(".fm-versioning .pad .top-column button.js-clear-previous").rebind("click", function () {
          if (M.isInvalidUserStatus()) {
            return;
          }
        ...
        $(window).rebind(
          "resize.fileversioning",
          SoonFc(() => {
            initPerfectScrollbar($scrollBlock);
          })
        );
        if (!is_video(M.d[window.versiondialogid]) && !is_image2(M.d[window.versiondialogid]) && !is_text(M.d[window.versiondialogid])) {
