...
MegaUtils.prototype.hasPendingTransfers = function megaUtilsHasPendingTransfers() {
  ...
  return (fminitialized && ulmanager.isUploading) || dlmanager.isDownloading || typeof dlmanager.isStreaming === "object";
};
  ...
  M.logoutAbortTransfers().then(function () {
    ...
    var finishLogout = function () {
      const afterLogout = () => {
        ...
        if (sitePath.includes("fm/search/") || sitePath.includes("/chat") || sitePath.includes("keybackup")) {
...
MegaUtils.prototype.vtol = function megaUtilsVTOL(version, hex) {
  ...
  version =
    (((version[0] | 0) & 0xff) << 24) | (((version[1] | 0) & 0xff) << 16) | (((version[2] | 0) & 0xff) << 8) | ((version[3] | 0) & 0xff);
...
MegaUtils.prototype.getSiteVersion = function () {
  ...
  if (buildVersion.timestamp !== "") {
    ...
    if (is_extension) {
      version = mega.chrome
        ? buildVersion.chrome + " " + (ua.details.browser === "Edgium" ? l[23326] : l[957])
...
MegaUtils.prototype.checkForDuplication = function (id) {
  "use strict";
  if (M.currentrootid === M.RubbishID || !M.d[id] || M.getNodeRights(id) < 2) {
    return;
  }
  ...
  if (M.v.length > Object.keys(names).length) {
    ...
    if (!Object.keys(dups).length && !Object.keys(dupsFolders).length) {
      if (d) {
        console.warn(
          "No Duplications were found in the time when" +
            "we have a mismatch in lengths " +
            id +
            ". We have names intersected between files and folders"
        );
      }
      return;
    }
  ...
  if (urlParts) {
    ...
    if (urlParts) {
      ...
      // hash they came from e.g. a folder link may not be valid for this account so just load the file manager.
        ...
        msgDialog("warninga", l[882], l.megasync_transferred_different_user, "", function () {
      ...
      var _rawXHR = function (url, data, callback) {
        M.xhr(url, JSON.stringify([data])).always(function (ev, data) {
          var resp = data | 0;
        ...
        var ctx = {
          checkloginresult: function (ctx, result) {
            ...
            if (toPage.substr(0, 1) === "!" && toPage.length > 7) {
              _rawXHR(apipath + "cs?id=0&domain=meganz", { a: "g", p: toPage.substr(1, 8) }, function (data) {
                if (data) {
                  dl_res = data;
                }
                _goToPage();
...
MegaUtils.prototype.checkGoingOverStorageQuota = function (opSize) {
  ...
  return M.getStorageQuota().then((data) => {
    if (opSize === -1) {
      opSize = data.mstrg;
    }
    ...
    if (opSize > data.mstrg - data.cstrg) {
      ...
      return M.showOverStorageQuota(data, options).always(() => {
...
MegaUtils.prototype.checkLeftStorageBlock = async function (data) {
  ...
  if (u_attr && (u_attr.b || u_attr.pf)) {
    ...
    storageHtml = l[1607]
      .replace("%1", `<span class="lp-sq-used">${space_used}</span>`)
...
MegaUtils.prototype.getFileSystemEntry = promisify(function (resolve, reject, filename, create) {
  ...
  this.getFileSystemAccess(create, seqno).then(function (fs) {
...
MegaUtils.prototype.delPersistentData = function (k) {
  ...
  return Promise.allSettled([this.onPersistentDB("rem", k), this.onPersistentDB.fallback("rem", k)]);
};
