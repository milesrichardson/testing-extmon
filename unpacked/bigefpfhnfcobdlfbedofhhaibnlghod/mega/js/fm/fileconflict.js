  ...
  var ns = {
    ...
    check: function (files, target, op, defaultAction, defaultActionFolders) {
      ...
      if (conflicts.length) {
          ...
          (function promptRecursion(a) {
            ...
            if (file) {
              ...
              if (action) {
                ...
                self.prompt(op, file, node, a.length, node.p || target).always(function (file, name, action, checked) {
                  if (file === -0xbadf) {
                    result = [];
                    resolve();
                  } else {
        ...
        applyCheck(conflicts).always(function _traversTree() {
          if (merges && merges.length && !breakOP) {
            // merge mode ! --> checking everything
            var conflictedNodes = [];
            var okNodes = [];
    ...
    filesFolderConfilicts: function _filesFolderConfilicts(nodesToCopy, target) {
      ...
      if (folderFound) {
        ...
        for (var k2 = 0; k2 < conflictedNodes.length; k2++) {
          if (conflictedNodes[k2][0].t) {
            // array contains either files or folders only
    ...
    prompt: function (op, file, node, remaining, target, dupsNB) {
      ...
      if (file.t) {
        ...
        $(".info-txt-fn", $dialog).safeHTML(escapeHTML(l[17550]).replace("%1", "<strong>" + name + "</strong>"));
      } else {
      ...
      switch (op) {
        case "dups":
          ...
          if (file.t) {
            $(".info-txt-fn", $dialog).safeHTML(l[22104].replace("{0}", "<strong>" + name + "</strong>"));
            ...
            $(".info-txt-fn", $dialog).safeHTML(l[22109].replace("{0}", "<strong>" + name + "</strong>"));
          }
          break;
        case "copy":
          if (file.t) {
      ...
      $("#duplicates-checkbox", $dialog).switchClass("checkboxOn", "checkboxOff").parent().switchClass("checkboxOn", "checkboxOff");

      var $aside = $("aside", $dialog).addClass("hidden");

      if (remaining) {
        var remainingConflictText = remaining > 1 ? escapeHTML(l[16494]).replace("%1", "<span>" + remaining + "</span>") : l[23294];
      ...
      var resolveDup = function (duplicateEntries, keys, kIndex, type, applyToAll) {
        ...
        if (applyToAll) {
          ...
          fileconflict
            .prompt(
              "dups",
              M.d[duplicateEntries[type][name][0]],
              M.d[duplicateEntries[type][name][1]],
              allDups - 1,
              target,
              duplicateEntries[type][name].length
            )
            .always(contuineResolving);
        }
      };
