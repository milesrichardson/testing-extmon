function accountUI() {
  ...
  if (u_type === 0) {
    msgDialog("confirmation", l[998], l[17146] + " " + l[999], l[1000], function (e) {
      if (e) {
  ...
  $(".fm-right-files-block, .section.conversations, .fm-right-block.dashboard", $fmContainer).addClass("hidden");
...
accountUI.renderAccountPage = function (account) {
  ...
  var showOrHideBanner = function (sectionName) {
    ...
    if (sectionName === "/fm/account" || sectionName === "/fm/account/plan" || sectionName === "/fm/account/transfers") {
...
accountUI.inputs = {
  ...
  radioCard: {
    ...
    set: function (identifier, $container, newVal, onChangeCb) {
      ...
      $(identifier + ".radioOn", $container)
        .addClass("radioOff")
        .removeClass("radioOn");
      $(identifier + ".radioOff", $container)
        .next()
        .addClass("radioOff")
        .removeClass("radioOn");
      $(identifier + ".radioOff", $container)
        .next()
        .children(".chat-selected")
        .removeClass("checked");
      $input.removeClass("radioOff").addClass("radioOn").prop("checked", true);
      $input.parent().addClass("radioOn").removeClass("radioOff");
  ...
  switch: {
    init: function (identifier, $container, currentValue, onChangeCb, onClickCb) {
      ...
      if ((currentValue && !$switch.hasClass("toggle-on")) || (!currentValue && $switch.hasClass("toggle-on"))) {
...
accountUI.account = {
  ...
  profiles: {
    ...
    renderPhoneBanner: async function () {
      ...
      if (u_attr.flags.smsve !== 2 || typeof u_attr.smsv !== "undefined" || fmconfig.skipsmsbanner || (ach && ach[9] && ach[9].rwd)) {
        hideOrDisplayBanner();
        return false;
      }
      ...
      if (
        ...
        (phoneBannerTimeChecker && !phoneBannerTimeChecker.shouldShow() && !phoneBannerTimeChecker.hasUpdated())
      ) {
        hideOrDisplayBanner();
        return false;
      }
    ...
    renderBirth: function () {
      ...
      $(".mega-input.birth." + $.dateTimeFormat.structure, accountUI.$contentBlock).removeClass("hidden");
    ...
    renderBirthYear: function () {
      ...
      var $input = $(".mega-input.birth" + formatClass, accountUI.$contentBlock).attr("max", i);
    ...
    bindEvents: function () {
      ...
      var self = this;
      var $personalInfoBlock = $(".profile-form", accountUI.$contentBlock);
      var $birthdayBlock = $(".mega-input.title-ontop.birth." + $.dateTimeFormat.structure, $personalInfoBlock);
      ...
      // Avatar
      $(".avatar-wrapper, .settings-sub-section.avatar .avatar", $personalInfoBlock).rebind("click.showDialog", function () {
        avatarDialog();
      });
      ...
      $firstNameField
        .add($lastNameField)
        .add(".byear, .bmonth, .bdate", $birthdayBlock)
        .rebind("input.settingsGeneral change.settingsGeneral", function () {
          ...
          if (
            enteredFirst.length > 0 &&
            enteredLast.length > 0 &&
      ...
      $(".mega-input-dropdown .option", $countryDropdown).rebind("click.showSave", function () {
        if ($firstNameField.val() && $firstNameField.val().trim().length > 0 && !$personalInfoBlock.find(".errored").length) {
          $saveBlock.removeClass("closed");
        } else {
          $saveBlock.addClass("closed");
        }
      ...
      $saveButton.rebind("click", function () {
        ...
        if ($(".bdate", $birthdayBlock).val()) $(".fm-account-avatar").safeHTML(useravatar.contact(u_handle, "", "div", false));
        ...
        var checklist = {
          ...
          birthyear: String(by || ""),
          country: String(getDropdownValue($("#account-country", $personalInfoBlock)))
        };
        ...
        if (checkUpdated()) {
          api
            .screq(userAttrRequest)
            .then(({ result: res }) => {
              if (res === u_handle) {
                $(".user-name").text(u_attr.name);
                $(".name", ".account-dialog").text(u_attr.fullname).attr("data-simpletip", u_attr.fullname);
      ...
      accountUI.inputs.switch.init(
        ...
        function (val) {
          if (val) {
            ...
            $(".access-qr-container", accountUI.account.qrcode.$QRSettings).add(".qr-settings .qr-block").parent().addClass("closed");
    ...
    reset: function () {
      ...
      msgDialog("confirmation", l[18227], l[18228], l[18229], function (regenQR) {
        if (regenQR) {
          ...
          api
            .req({ a: "cld", cl: M.account.contactLink.substring(2) })
            .then(({ result }) => {
              assert(result === 0);
              return api.send("clc");
            })
  ...
  preference: {
    render: function () {
      ...
      accountUI.inputs.radio.init(
        ...
        function (val) {
          $("body")
            .removeClass("fontsize1 fontsize2")
            .addClass("fontsize" + val);
          mega.config.setn("font_size", parseInt(val), l[16168]);
        }
      );

      // Theme
      accountUI.inputs.radio.init(".uiTheme", $(".uiTheme", accountUI.$contentBlock).parent(), u_attr["^!webtheme"] || 0, function (val) {
          ...
          api_req(request, {
            callback: function (res) {
              ...
              // Check for incorrect email
                ...
                handleResetSuccessDialogs(l.ac_closure_email_sent_msg.replace("%s", u_attr.email), "deleteaccount");
              } else {
...
accountUI.plan = {
      ...
      if (
        ...
        if (
          ...
          const renderBars = (used, base, extra, $container, msg, $overall) => {
            ...
            if (base) {
              spaceTxt += `/${l[5816].replace("[X]", base + (extra || 0))}`;
            ...
            $(".settings-sub-bar", $container).css("width", `${100 - (storageConsume * 100) / storageQuota}%`);
          ...
          renderBars(
            M.account.space_bus_used || M.account.space_used,
            M.account.space_bus_base,
            M.account.space_bus_ext,
            $storageContent,
            l.additional_storage,
            $(".plan-info.storage > span", $planContent)
          );

          renderBars(
            M.account.tfsq_bus_used || M.account.tfsq.used,
            M.account.tfsq_bus_base,
            M.account.tfsq_bus_ext,
            $bandwidthContent,
            l.additional_transfer,
    ...
    cancelSubscriptionDialog: {
      ...
      fillBenefits: async function ($keepPlanBtn) {
        ...
        const callback = async (data) => {
          ...
          const plan = pro.membershipPlans.find(
            (plan) => plan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] === subscription && plan[pro.UTQA_RES_INDEX_MONTHS] === duration
          );
          ...
          $("span", $keepPlanBtn).text(l.cancel_pro_keep_current_plan.replace("%1", pro.getProPlanName(subscription)));
      ...
      sendSubCancelRequestToApi: function (skippedReason) {
        ...
        const requests = [{ a: "cccs", r: reason }];
        ...
        api_req(requests, {
          callback: () => {
            ...
            this.$expiryDateBlock.safeHTML('<span class="red">@@</span>', time2date(account.expiry, 2));
  ...
  paymentCard: {
    ...
    validateCardResponse: function (res) {
      "use strict";
      return res && (res.gw === (addressDialog || {}).gatewayId_stripe || 19) && res.brand && res.last4 && res.exp_month && res.exp_year;
    },
    ...
    validateUser: function (account) {
      ...
      return (
        ...
        ((Array.isArray(account.sgw) && account.sgw.includes("Stripe")) ||
          (Array.isArray(account.sgwids) && account.sgwids.includes((addressDialog || {}).gatewayId_stripe || 19)))
      );
    },
    ...
    init: function (account, $planSection) {
      ...
      const hideCardSection = () => {
        ...
        $(".settings-button .acc-setting-menu-card-info", ".content-panel.account").addClass("hidden");
      };
    ...
    render: function (cardInfo) {
      ...
      if (cardInfo && this.$paymentSection) {
        if (cardInfo.brand === "visa") {
          ...
          $(".payment-card-icon i", this.$paymentSection).removeClass("sprite-fm-uni icon-mastercard-border");
        ...
        $(".payment-card-expiry .payment-card-expiry-val", this.$paymentSection).text(
          `${String(cardInfo.exp_month).padStart(2, "0")}/${String(cardInfo.exp_year).substr(-2)}`
        );
  ...
  balance: {
    render: function (account) {
      ...
      $(".account.plan-info.balance span", accountUI.$contentBlock).safeHTML("&euro; @@", mega.intl.number.format(account.balance[0][0]));
    },
    ...
    renderTransaction: function (account) {
      ...
      if (account.transactions.length) {
        ...
        $(account.transactions).each(function (i, el) {
          ...
          html +=
            "<tr><td>" + time2date(el[1]) + "</td><td>" + htmlentities(el[0]) + "</td><td>" + credit + "</td><td>" + debit + "</td></tr>";
        });
      } else {
...
accountUI.notifications = {
  ...
  render: function () {
    ...
    for (let i = $NToggle.length; i--; ) {
      ...
      accountUI.inputs.switch.init(`#${el.id}`, sectionEl, notif.has(el.getAttribute("name"), section), (val) => {
        ...
        if (val) {
          ...
          ($NToggle.hasClass("toggle-on") ? $.fn.addClass : $.fn.removeClass).apply($NToggleAll, ["toggle-on"]);
        }
    ...
    accountUI.inputs.switch.init("#" + $NToggleAll[0].id, $NToggleAll.parent(), $NToggle.hasClass("toggle-on"), function (val) {
    ...
    mega.enotif.all().then(function (enotifStates) {
      ...
      $EToggle.each(function () {
        ...
        accountUI.inputs.switch.init("#" + this.id, $section, !enotifStates[emailId], function (val) {
          mega.enotif.setState(emailId, !val);
          (val || $EToggle.hasClass("toggle-on") ? $.fn.addClass : $.fn.removeClass).apply($EToggleAll, ["toggle-on"]);
  ...
  getSectionName: function ($section) {
    ...
    var section = String($section.attr("class"))
      .split(" ")
      .filter(function (c) {
        return {
          chat: 1,
  ...
  onPermissionsGranted() {
    ...
    msgDialog(
      ...
      l.notifications_permissions_granted_info
        .replace("[A]", `<a href="${l.mega_help_host}/${this.helpURL}" target="_blank" class="clickurl">`)
        .replace("[/A]", "</a>")
    );
    this.renderNotification();
  },
...
accountUI.security = {
  ...
  metadata: {
    render: function () {
      ...
      accountUI.inputs.switch.init(".dbDropOnLogout", $(".personal-data-container"), fmconfig.dbDropOnLogout, function (val) {
  ...
  session: {
    ...
    render: function () {
      ...
      var html =
        ...
        l[482] +
        "</th>" +
        '<th class="no-border session-status">' +
        l[7664] +
        "</th>" +
          ...
          if (e) {
            ...
            api.screq({ a: "usr", ko: 1 }).then(() => {
              ...
              $(".active-session-txt", $activeSessionsRows)
                .removeClass("active-session-txt")
                .addClass("expired-session-txt")
                .text(l[25016]);
              $(".fm-close-all-sessions", $securitySection).addClass("hidden");
              loadingDialog.hide();
            });
      ...
      $(".settings-logout", $securitySection).rebind("click.accountSection", function () {
        ...
        if (sessionId === "current") {
          ...
          api.screq({ a: "usr", s: [sessionId] }).then(() => {
            ...
            $this.find(".active-session-txt").removeClass("active-session-txt").addClass("expired-session-txt").text(l[25016]);
            loadingDialog.hide();
          });
        }
      });
    ...
    getHtml: function (el) {
      ...
      var html =
        ...
        htmlentities(browserName) +
        "</span></div></td>" +
        '<td><span class="break-word" title="' +
        ipAddress +
        '">' +
        ipAddress +
        "</span></td>" +
        '<td><div class="label-with-icon"><img alt="" src="' +
        staticpath +
        "images/flags/" +
        country.icon +
        '" title="' +
      ...
      if (activeSession) {
        html +=
          ...
          '<div><i class="sprite-fm-mono icon-logout"></i></div><span>' +
          l[967] +
          "</span>" +
          "</button></td></tr>";
      } else {
    ...
    update: function (force) {
      ...
      if (page === "fm/account/security") {
        ...
        M.refreshSessionList(function () {
          ...
          if ((fSession && (($(domList[1]).hasClass("current") && !fSession[5]) || !$(domList[1]).hasClass(fSession[6]))) || force) {
            if (d) {
...
accountUI.fileManagement = {
  ...
  rubsched: {
    render: function (account) {
      ...
      var initRubschedSwitch = function (defaultValue) {
        accountUI.inputs.switch.init("#rubsched", $("#rubsched", accountUI.$contentBlock).parent(), defaultValue, function (val) {
          if (val) {
      ...
      if (u_attr.flags.ssrs > 0) {
        ...
        if (u_attr.p) {
          ...
          $(".account.rubbish-cleaning .settings-right-block", accountUI.$contentBlock).addClass("slide-in-out");
          ...
          $(".account.rubbish-cleaning .settings-right-block", accountUI.$contentBlock).removeClass("slide-in-out");
        }
      } else {
        // Client side scheduler - old
        ...
        if (fmconfig.rubsched) {
          ...
          accountUI.inputs.radio.init(".rubschedopt", $(".rubschedopt", accountUI.$contentBlock).parent(), opt[0], function (val) {
  ...
  userInterface: {
    _initOption: function (name) {
      ...
      accountUI.inputs.radio.init(selector, $(selector).parent(), fmconfig[name] | 0, function (val) {
  ...
  dragAndDrop: {
    render: function () {
      ...
      accountUI.inputs.switch.init("#ulddd", $("#ulddd", accountUI.$contentBlock).parent(), !mega.config.get("ulddd"), function (val) {
  ...
  chatDialogs: {
    render: function () {
      ...
      for (let i = $switches.length; i--; ) {
        ...
        accountUI.inputs.switch.init(".mega-switch", $(elm).parent(), !currVal, (val) => _set(id, val, subtype));
      }
    }
  },
  ...
  publicLinks: {
    render: function () {
      ...
      accountUI.inputs.switch.init(warnplinkId, $(warnplinkId, accountUI.$contentBlock).parent(), !mega.config.get("nowarnpl"), (val) =>
        mega.config.setn("nowarnpl", val ^ 1)
      );
    }
  }
};
...
accountUI.transfers = {
  ...
  uploadAndDownload: {
    setSlider($container, sliderSelector, sliderOptions) {
      ...
      $(".ui-slider-handle", $slider).addClass("sprite-fm-mono icon-arrow-left sprite-fm-mono-after icon-arrow-right-after");
      ...
      render: function (account) {
        ...
        if (u_attr.p && !u_attr.b) {
          ...
          var $slider = $("#bandwidth-slider").slider({
                  ...
                  if (bandwidthLimit > 99) {
                    msgDialog("warningb:!" + l[776], l[882], l[12689], 0, function (e) {
                      if (e) {
                        ...
                        $(".slider-percentage").safeHTML(l.transfer_quota_pct.replace("%1", formatPercentage(0)));
            ...
            slide: function (e, ui) {
              $(".slider-percentage", accountUI.$contentBlock).safeHTML(
                l.transfer_quota_pct.replace("%1", formatPercentage(ui.value / 100))
              );
          ...
          $(".slider-percentage", accountUI.$contentBlock).safeHTML(
            l.transfer_quota_pct.replace("%1", formatPercentage(bandwidthLimit / 100))
          );
      ...
      render: function () {
        ...
        accountUI.transfers.uploadAndDownload.setSlider($uploadSettings, "#slider-range-max", {
          ...
          slide: function (e, ui) {
            ...
            $(".numbers:nth-child(" + ui.value + ")", $uploadSettings).addClass("active");
          }
        });
      }
    },
      ...
      render: function () {
        ...
        accountUI.transfers.uploadAndDownload.setSlider($downloadSettings, "#slider-range-max2", {
          ...
          slide: function (e, ui) {
            ...
            $(".numbers:nth-child(" + ui.value + ")", $downloadSettings).addClass("active");
          }
        });
      }
    }
...
accountUI.contactAndChat = {
  ...
  status: {
    ...
    render: function (presenceInt, autoaway, autoawaylock, autoawaytimeout, persist, persistlock, lastSeen) {
      ...
      accountUI.inputs.radio.init(".chatstatus", $(".chatstatus").parent(), presenceInt.getPresence(u_handle), function (newVal) {
        presenceInt.setPresence(parseInt(newVal));
        showToast("settings", l[16168]);
      });

      // Last seen switch
      accountUI.inputs.switch.init("#last-seen", $sectionContainerChat, lastSeen, function (val) {
      ...
      if (autoawaytimeout !== false) {
        // Auto-away switch
        accountUI.inputs.switch.init("#auto-away-switch", $sectionContainerChat, autoaway, function (val) {
        ...
        if (autoawaylock === true) {
          $("#auto-away-switch", $sectionContainerChat).addClass("disabled").parent().addClass("hidden");
        } else {
          $("#auto-away-switch", $sectionContainerChat).removeClass("disabled").parent().removeClass("hidden");
        }
        ...
        accountUI.controls.enableElement($(`input#${this.AWAY_REFS.hours}, input#${this.AWAY_REFS.minutes}`, $sectionContainerChat));

        // Persist switch
        accountUI.inputs.switch.init("#persist-presence-switch", $sectionContainerChat, persist, function (val) {
        ...
        // Prevent changes to autoaway if autoawaylock is set
        if (persistlock === true) {
          $("#persist-presence-switch", $sectionContainerChat).addClass("disabled").parent().addClass("hidden");
        } else {
          $("#persist-presence-switch", $sectionContainerChat).removeClass("disabled").parent().removeClass("hidden");
        }
      }
    },
  ...
  chatList: {
    render: function () {
      ...
      accountUI.inputs.radioCard.init(".card", $(".card", accountUI.$contentBlock).parent(), curr, (val) => {
  ...
  dnd: {
    ...
    hasDnd: function () {
      "use strict";
      return (pushNotificationSettings && pushNotificationSettings.getDnd(this.group)) || pushNotificationSettings.getDnd(this.group) === 0;
    },
    ...
    setMorningOption: function () {
      ...
      $label.safeHTML(targetTomorrow ? l[23671] || "Until tomorrow morning, 08:00" : l[23670] || "Until this morning, 08:00");
  ...
  delayRender: function (presenceInt, autoaway) {
    ...
    if (autoaway === undefined) {
      presenceInt.rebind(
        "settingsUIUpdated.settings",
        function (e, autoaway, autoawaylock, autoawaytimeout, persist, persistlock, lastSeen) {
  ...
  voucher: {
    render: function (account) {
      ...
      $(account.vouchers).each(function (i, el) {
        ...
        html +=
          ...
          time2date(el.date) +
          "</span></td>" +
          '<td class="selectable"><span class="break-word">' +
          voucherLink +
          "</span></td>" +
          "<td><span>&euro; " +
      ...
      for (var j = 0; j < prices.length; j++) {
        voucheroptions +=
          '<div class="option" data-value="' + htmlentities(prices[j]) + '">&euro;' + htmlentities(prices[j]) + " voucher</div>";
      }
      $(".vouchertype .dropdown-scroll", $resellerSection).safeHTML(voucheroptions);
    },
    ...
    bindEvents: function () {
      ...
      $(".vouchercreate", $resellerSection).rebind("click.voucherCreateClick", function () {
        var vouchertype = $('.option[data-state="active"]', $voucherTypeSelect).attr("data-value");
        ...
        loadingDialog.show();
        api_req(
          { a: "uavi", d: vouchertype, n: voucheramount, c: "EUR" },
          {
            callback: function () {
              M.account.lastupdate = 0;
              accountUI();
