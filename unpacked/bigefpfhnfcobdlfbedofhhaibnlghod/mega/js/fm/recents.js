...
RecentsRender.prototype._initialRender = function (actions) {
  ...
  if (!this._showRecents) {
    ...
    this.$buttonEnableRecents.rebind("click.enableRecents", () => this._setConfigShow(1));
    ...
    this._dynamicList = new MegaDynamicList(this.scrollDiv, {
      ...
      itemRenderFunction: function (id) {
        return self._doRenderWorker(id);
      },
      itemHeightCallback: function (id) {
        return self._getItemHeight(id);
      },
      onNodeInjected: function () {
        return self._onNodeInjected();
      },
      onResize: function () {
        return self.thottledResize();
      },
      onScroll: function () {
...
RecentsRender.prototype.keySelectUpDown = function (dir, shift) {
  ...
  if (dir < 0 && this._dynamicList.getScrollTop() >= this._dynamicList.getItemOffsets(itemIndex)) {
    ...
    dir > 0 &&
    this._dynamicList.getScrollTop() + this._dynamicList.listContainer.offsetHeight <=
...
RecentsRender.prototype.populateBreadCrumb = function ($container, action) {
  ...
  var getActionUserString = function (isOtherUser, isCreated) {
    ...
    if (isOtherUser) {
      ...
      actionUserString = actionUserString.replace("%3", '<span class="link action-user-name"></span>');
    } else {
...
RecentsRender.prototype.handleByUserHandle = function ($newRow, action) {
  ...
  $userNameContainer.removeClass("hidden").text(M.getNameByHandle(action.user) || l[24061]);
...
RecentsRender.prototype.handleInOutShareState = function ($newRow, action) {
  ...
  $newRow
    .find(".transfer-filetype-icon")
    .removeClass("hidden")
    .addClass(action.outshare ? "folder-shared" : "inbound-share");
  $newRow.find(".in-out-tooltip span").text(action.outshare ? l[5543] : l[5542]);
};
...
RecentsRender.prototype._renderFiles = function ($newRow, action, actionId) {
  ...
  if (
    action.length === 1 &&
    ((iconClass === "image" && is_image2(action[0])) || (iconClass === "video" && is_video(action[0])) || iconClass === "pdf")
  ) {
    ...
    if (iconClass === "video") {
      $icon.safeAppend('<div class="video-thumb-details theme-dark-forced">' + '<i class="sprite-fm-mono icon-play"></i>' + "</div>");
    }
  }
  ...
  var clickFunction = (e) => {
    if ((e.detail === 2 || $newRow.hasClass("ui-selected")) && ($newRow.hasClass("expanded") || $newRow.hasClass("collapsed"))) {
  ...
  if (isMore) {
    ...
    $(".expand-collapse-toggle, .more-less-toggle", $newRow).rebind("click.recents", () => {
      expandCollapseHelper();
      return false;
    });
...
RecentsRender.prototype._renderMedia = function ($newRow, action, actionId) {
  ...
  var $contextMenuButton = $newRow.find(".context-menu-button");
  $contextMenuButton.rebind("dblclick", () => false).rebind("click contextmenu", triggerContextMenu);
