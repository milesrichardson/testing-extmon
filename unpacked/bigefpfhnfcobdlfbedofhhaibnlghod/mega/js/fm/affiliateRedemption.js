...
affiliateRedemption.getRedemptionStatus = function (code, proSuccessful) {
  ...
  if (proSuccessful !== undefined) {
    return proSuccessful ? { c: "", s: l[25002], class: "complete", m: l[23373] } : { c: "red", s: l[22819], class: "failed", m: l[23369] };
  }
...
affiliateRedemption.processSteps = function () {
  ...
  return Promise.all(
    steps[this.currentStep].map((func) => {
      return self[func]();
    })
  ).catch(function (ex) {
    if (ex) {
...
affiliateRedemption.__processBlock3 = function () {
  ...
  return Promise.all([M.affiliate.redeemStep1(), this.requests.first.m % 2 === 0 ? false : M.affiliate.getRedeemAccountInfo()]).then(
    function (res) {
      self.req1res = res;
    },
    function (ex) {
...
affiliateRedemption.recordSecondReqValues = function () {
  ...
  if (this.req1res[0].data.length > 1) {
    ...
    var activeTypeValue = $accountType.hasClass("dropdown-input") ? $(".option.active", $accountType).data("type") : $accountType.val();
  ...
  var uiSelectString = is_mobile
    ? ".affi-dynamic-acc-select select, .affi-dynamic-acc-input input"
...
affiliateRedemption.fillBasicHistoryInfo = function ($wrapper, item, status) {
  ...
  const proSuccessful = item.gw === 0 ? status === 4 : undefined;
  ...
  if (item.c === "EUR") {
    ...
    $(".request-amount", $wrapper).safeHTML(
      '<span class="euro">' + formatCurrency(a) + "</span>" + '<span class="local">BTC ' + parseFloat(la).toFixed(8) + "</span>"
    );
    $(".fee-charged", $wrapper).safeHTML(
      '<span class="euro">' + formatCurrency(f) + "</span>" + '<span class="local">BTC ' + parseFloat(lf).toFixed(8) + "</span>"
    );
    $(".recived-amount", $wrapper).safeHTML(
      '<span class="euro">' + formatCurrency(a - f) + "</span>" + '<span class="local">BTC ' + parseFloat(la - lf).toFixed(8) + "</span>"
    );
  } else {
    $(".request-amount", $wrapper).safeHTML(
      '<span class="euro">' + formatCurrency(a) + '</span><span class="local">' + formatCurrency(la, item.c, "code") + "</span>"
    );
    $(".fee-charged", $wrapper).safeHTML(
      '<span class="euro">' + formatCurrency(f) + '</span><span class="local">' + formatCurrency(lf, item.c, "code") + "</span>"
    );
    $(".recived-amount", $wrapper).safeHTML(
      '<span class="euro">' +
        formatCurrency(receivedAmount) +
        '</span><span class="local">' +
        formatCurrency(receivedAmount, item.c, "code") +
        "</span>"
    );
  }
...
affiliateRedemption.redemptionAccountDetails = function ($wrapper, method, data) {
  ...
  if (method === 2) {
    ...
    html += itemTemplate.replace("%1", l[23361]).replace("%2", extra.an).replace("%3", "bitcoin-address");
  } else if (method === 1) {
...
affiliateRedemption.updateAccInfo = function () {
  ...
  if (this.requests.first.m === 1) {
    ...
    accInfo = Object.assign({ ccc: this.requests.first.cc + this.requests.first.c }, this.requests.second.extra);
