(function _dialogs(global) {
  ...
  var disableFolders = function () {
    ...
    if ($.moveDialog) {
      ...
      const $sharedAndFileRequestFolders = $('*[id^="mctreea_"]', $dialog).children(".file-request-folder, .shared-folder");

      // All parent file request and shared folder
      $sharedAndFileRequestFolders.closest(".nw-fm-tree-item").addClass("disabled");

      // Filter shared folder and disable descendants
      const filteredSharedFolders = $sharedAndFileRequestFolders.filter(".shared-folder").closest(".nw-fm-tree-item");
      ...
      const filteredLinkedFolders = $allFolders.filter(".linked").addClass("disabled");
  ...
  var setDialogBreadcrumb = function (aTarget) {
    ...
    if ($.mcselected && !path.length) {
      ...
      if (el) {
        path.push(aTarget);
        names[aTarget] = el.querySelector(".nw-fm-tree-folder, .nw-fm-tree-icon-wrap").textContent;

        $(el)
          .parentsUntil(".dialog-content-block", "ul")
          .each(function (i, elm) {
            var h = String(elm.id).split("_")[1];
            path.push(h);

            elm = dialog.querySelector(`#mctreea_${h} .nw-fm-tree-folder, #mctreea_${h} .nw-fm-tree-icon-wrap`);
            if (elm) {
    ...
    if (path.length) {
      ...
      filetypeIcon.classList.add("sprite-fm-mono", section === "conversations" ? "icon-chat-filled" : "icon-folder-filled");
    }
  ...
  var setSelectedItems = function (single) {
    var $icon = $(".summary-items-drop-icon", $dialog).removeClass("icon-arrow-up drop-up icon-arrow-down drop-down hidden");
    ...
    if ($.nodeSaveAs) {
      ...
      var rowHtml =
        ...
        '<input id="f-name-input" class="summary-ff-name" type="text" value="' +
        escapeHTML($.nodeSaveAs.name) +
        '" placeholder="' +
        l[17506] +
        '" autocomplete="off"/> &nbsp; ' +
        "</div>" +
  ...
  var handleConversationTabContent = function _handleConversationTabContent() {
    ...
    if (myChats && myChats.length) {
      ...
      for (var chati = 0; chati < sortedChats.length; chati++) {
        ...
        } else if (chatRoom.type === "public" && chatRoom.membersSetFromApi && chatRoom.membersSetFromApi.members[u_handle] >= 2) {
          isValidGroupOrPubChat = true;
        }
        ...
        if (isValidGroupOrPubChat) {
          ...
          if (gNames.length) {
            if (nbOfRecent < top5) {
              var gElem = createGroupEntry(gNames, Object.keys(chatRoom.members).length, chatRoom.roomId, chatRoom);
              contactGeneratedList += gElem;
            } else {
              myContacts.push({
                id: Object.keys(chatRoom.members).length,
                name: gNames[0],
                handle: chatRoom.roomId,
                isG: true,
                gMembers: gNames
              });
            }
          ...
          if (contactHandle && M.u[contactHandle] && M.u[contactHandle].c === 1 && M.u[contactHandle].m) {
            addedContactsByRecent.push(contactHandle);
            var ctElemC = createContactEntry(M.getNameByHandle(contactHandle), M.u[contactHandle].m, contactHandle);
    ...
    if (myContacts && myContacts.length) {
      ...
      for (var a = 0; a < myContacts.length; a++) {
        ...
        if (!myContacts[a].isG) {
          ...
          ctElem = createGroupEntry(myContacts[a].gMembers, myContacts[a].id, myContacts[a].handle, megaChat.chats[myContacts[a].handle]);
        }
        contactGeneratedList += ctElem;
      }
    }

    if ((myChats && myChats.length && isActiveMember) || (myContacts && myContacts.length)) {
  ...
  const handleDialogTabContent = (tabClass, parentTag, htmlContent) => {
    ...
    const html = String(htmlContent)
      .replace(/treea_/gi, "mctreea_")
      .replace(/treesub_/gi, "mctreesub_")
      .replace(/treeli_/gi, "mctreeli_");
  ...
  var buildDialogTree = function () {
    ...
    if (!treesearch) {
      $(".nw-fm-tree-item", ".fm-picker-dialog").removeClass("expanded active opened selected");
    ...
    onIdle(() => {
      ...
      const folderNames = $dialog[0].querySelectorAll(".nw-fm-tree-folder:not(.inbound-share), .nw-fm-tree-icon-wrap");
  ...
  var handleDialogContent = function (dialogTabClass, buttonLabel) {
    ...
    if (
      ...
      !(
        ...
        ($.dialog === "copy" && $.selected.filter((n) => !M.isFileNode(M.getNodeByHandle(n)) && M.getNodeRoot(n) === "shares").length)
      )
    ) {
    ...
    if (!u_type || $.copyToShare || $.mcImport || $.selectFolderDialog || $.saveAsDialog) {
      $rubbishBin.addClass("hidden");
      $conversations.addClass("hidden");
    }
    ...
    if (nodeRoot === M.RubbishID || $.copyToShare || $.selectFolderDialog || !u_type || M.currentdirid === "devices" || $.albumImport) {
    ...
    if (!$.saveToDialog && section === "cloud-drive" && (M.currentrootid === "chat" || $.copyToShare)) {
      ...
      if ($.fileRequestNew) {
        ...
        $(".fm-picker-dialog-desc", $dialog).removeClass("hidden");
        $(".fm-picker-dialog-desc p", $dialog).text(l.file_request_select_folder_desc);
  ...
  mBroadcaster.addListener("fm:initialized", function copyMoveDialogs() {
    ...
    $dialog.rebind("click.dialog", (e) => {
      if (!e.target.closest(".breadcrumb-dropdown, .breadcrumb-dropdown-link") && $(".breadcrumb-dropdown", $dialog).hasClass("active")) {
    ...
    $(".dialog-newfolder-button", $dialog).rebind("click", function () {
      ...
      $.cftarget = target;
      ($.cfpromise = mega.promise).then((h) => callback(h)).catch(nop);
    ...
    $dialog.rebind("click", ".nw-fm-tree-item", function (e) {
      ...
      $(this).parentsUntil(".dialog-content-block", "ul").addClass("opened").prev(".nw-fm-tree-item").addClass("expanded");
    ...
    $btn.rebind("click", function () {
      ...
      const { saveToDialogCb, saveToDialogNode, shareToContactId, noOpenChatFromPreview } = $;
      ...
      if ($.nodeSaveAs) {
        ...
        M.getStorageQuota().then((data) => {
          ...
          mega.fileTextEditor
            .saveFileAs(saveAsName, $.mcselected, $.saveAsContent, nodeToSave)
            .then((handle) => {
              if ($.saveAsCallBack) {
      ...
      // Get active tab
      if (section === "cloud-drive" || section === "folder-link" || section === "rubbish-bin" || section === "s4") {
        // If copying from contacts tab (Ie, sharing)
        if ($(this).text().trim() === l[1344]) {
          ...
          mega.keyMgr
            .setShareSnapshot(target)
            .then(() => doShare(target, [user]))
            .catch(tell);
        } else if ($.albumImport) {
          // This is a set to be imported
          mega.sets.copyNodesAndSet(selectedNodes, $.mcselected).catch((ex) => {
            if (ex === EBLOCKED) {
          ...
          M.copyNodes(selectedNodes, $.mcselected).catch((ex) => ex !== EBLOCKED && tell(ex));
        }
      } else if (section === "shared-with-me") {
