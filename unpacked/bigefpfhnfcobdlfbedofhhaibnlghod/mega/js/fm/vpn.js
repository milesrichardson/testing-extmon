...
class VpnCredsManager {
  ...
  static async createCredential() {
    ...
    return api
      .req({ a: "vpnp", k: ab_to_base64(keypair.publicKey) })
      .then(({ result }) => {
        assert(typeof result === "object");
      ...
      .catch((ex) => {
        if (ex === EACCESS) {
          console.warn("[VPN Manager] You are not authorised to create VPN credentials. " + "Upgrade to Pro, then try again.");
          return ex;
        }
        if (ex === ETOOMANY) {
          console.warn("[VPN Manager] You have exceeded your credential limit. " + "Please deactivate one credential, then try again.");
          return ex;
        }
  ...
  constructor() {
    ...
    $(".disclaimer1", this.page).safeHTML(
      escapeHTML(l.vpn_page_welcome_disclaimer1)
        .replace("[A1]", '<a href="https://mega.io/terms" ' + 'target="_blank" rel="noopener" class="clickurl vpn-link">')
        .replace("[A2]", '<a href="https://mega.io/vpn-terms" ' + 'target="_blank" rel="noopener" class="clickurl vpn-link">')
        .replace("[/A1]", "</a>")
        .replace("[/A2]", "</a>")
    );
    ...
    $(".tnc-agree-checkbox .radio-txt", this.page).safeHTML(
      escapeHTML(l.vpn_page_step2_tnc_checkbox_label)
        .replaceAll("[A]", '<a href="https://mega.io/vpn-terms" ' + 'target="_blank" rel="noopener" class="clickurl">')
        .replaceAll("[/A]", "</a>")
    );
