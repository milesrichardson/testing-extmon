...
function BusinessAccountUI() {
  ...
  this.initUItoRender = function () {
    ...
    if (
      !u_attr ||
      !u_privk ||
      (!u_attr.pf && (!u_attr.b || (u_attr.b.s === pro.ACCOUNT_STATUS_EXPIRED && M.currentdirid !== "user-management/account")))
    ) {
      loadSubPage("start");
      return false;
    }
    ...
    $(".inv-det-arrow, .inv-det-id", ".fm-right-header-user-management .user-management-breadcrumb").addClass("hidden");
    ...
    $(".fm-right-header-user-management .user-management-main-page-buttons .add-sub-user")
      .off("click.subuser")
      .on("click.subuser", function addSubUserHeaderButtonHandler() {
        mySelf.showAddSubUserDialog();
      });
    $(".fm-right-header-user-management .user-management-main-page-buttons .ba-account")
      .off("click.subuser")
      .on("click.subuser", function addSubUserHeaderButtonHandler() {
  ...
  this.updateSubUserLeftPanel = function (subUserLeftPanelHeader) {
    ...
    var $subUsers = $(".fm-tree-panel .content-panel.user-management .nw-user-management-item[id]", $subUserLeftPanel);
...
BusinessAccountUI.prototype.viewSubAccountListUI = function (subAccounts, isBlockView, quickWay) {
  ...
  if (!Object.keys(currSubAccounts).length) {
    // no subs
  ...
  var unhideUsersListSection = function () {
    ...
    $(".user-management-main-page-buttons", ".fm-right-header-user-management").removeClass("hidden"); // unhide head
  ...
  var fillSubUsersTable = function (subUsers, uiBusiness) {
    ...
    for (var h in subUsers) {
      ...
      if (subUsers[h].s === 0) {
        ...
        $currUserLeftPane.find(".user-management-status").removeClass("pending disabled").addClass("enabled");
      } else if (subUsers[h].s === 10) {
        $currUser.find(".user-management-status").addClass("pending");
        $currUserLeftPane.find(".user-management-status").removeClass("enabled disabled").addClass("pending");
      } else {
        $currUser.find(".user-management-status").addClass("disabled");
        $currUser.addClass("hidden");
        $currUserLeftPane.find(".user-management-status").removeClass("enabled pending").addClass("disabled");
    ...
    $(
      ...
      if ($me.hasClass("nw-user-management-item")) {
        // left pane
        userHandle = $me.attr("id");
      } else if ($me.hasClass("view-icon")) {
        // user row
    ...
    $(".grid-table-user-management .edit-icon").rebind("click.subuser", function editSubUserClickHandler() {
      if ($(this).hasClass("disabled")) {
        return false;
      }
    ...
    $(".grid-table-user-management .dis-en-icon").rebind("click.subuser", function disableEnableSubUserClickHandler() {
      const $this = $(this);
  ...
  var fillSubUsersUsage = function (st, quotas) {
    ...
    $(".info-block.bandwidth-sub-users .number", ".user-management-overview-bar").text(totalBandwidthFormatted.size);
    $(".info-block.bandwidth-sub-users .title2", ".user-management-overview-bar").text(totalBandwidthFormatted.unit);
...
BusinessAccountUI.prototype.showLinkPasswordDialog = function (invitationLink) {
  ...
  var prepareSubAccountLinkDialog = function () {
    ...
    $(".link-sub-user-pass input", $dialog)
      .off("keydown")
...
BusinessAccountUI.prototype.viewLandingPage = function () {
  ...
  $(".adding-subuser", $landingContainer).rebind("click.subuser", function addSubUserClickHandler() {
    mySelf.showAddSubUserDialog(null, function () {});
  });

  // handler account setting page
  $(".suba-account-setting", $landingContainer).rebind("click.subuser", function accountSettingClickHandler() {
    mySelf.viewBusinessAccountPage();
  });
...
BusinessAccountUI.prototype.viewSubAccountInfoUI = function (subUserHandle) {
  ...
  const decodedUser = mySelf.decodeFields(subUser, ["firstname", "lastname", "idnum", "position", "phonenum", "location"]);
  const uName = `${decodedUser.firstname} ${decodedUser.lastname}`.trim();
  ...
  if (subUser.isAdmin) {
    $(
      ".profile-button-container .migrate-data," +
        ...
        ".profile-button-container .reset-sub-user-password",
      $subAccountContainer
    ).addClass("disabled");
    ...
    $subAccountContainer
      .find(
        ".profile-button-container .migrate-data, .profile-button-container .edit-profile, " +
          ".profile-button-container .resend-verification, .profile-button-container .disable-account," +
          ".profile-button-container .reset-sub-user-password"
      )
  ...
  $(".profile-button-container .disable-account", $subAccountContainer)
    ...
    .addClass("negative")
    .addClass("sub-disable")
    .removeClass("sub-enable");
  ...
  $(".user-management-title", $subHeader).rebind("click.subuser", function navigationHeaderClickHandler() {
    mySelf.viewSubAccountListUI();
  });

  // event handler for enable/disable account
  $subAccountContainer
    .find(".profile-button-container .disable-account")
    .off("click.subuser")
    .on("click.subuser", function enable_disableClickHandler() {
      if ($(this).hasClass("disabled")) {
        return false;
      }
      if ($(this).hasClass("sub-disable")) {
        // button now in disable status
  ...
  $subAccountContainer
    .find(".profile-button-container .migrate-data")
    .off("click.subuser")
    .on("click.subuser", function migrateData_ClickHandler() {
      ...
      mySelf
        .migrateSubUserData(subUserHandle)
        .then((n) => {
          M.safeShowDialog("migration-success-dlg", () => {
            ...
            $(".dialog-text-one", $dialog).safeHTML(
              l[19149].replace("{0}", `<b>${M.suba[subUserHandle].e}</b>`).replace("{1}", `<b>${escapeHTML(n.name)}</b>`)
            );
  ...
  $subAccountContainer
    .find(".profile-button-container .edit-profile")
    .off("click.subuser")
    .on("click.subuser", function editSubUserClickHandler() {
      if ($(this).hasClass("disabled")) {
        return;
      }
  ...
  $subAccountContainer
    .find(".profile-button-container .reset-sub-user-password")
    .off("click.subuser")
    .on("click.subuser", function resetPasswordSubUserClickHandler() {
      if ($(this).hasClass("disabled")) {
        return;
      }
  ...
  $subAccountContainer
    .find(".profile-button-container .resend-verification")
    .off("click.subuser")
    .on("click.subuser", function resendInvitation_ClickHandler() {
      ...
      // success
      resendPromise.done(function resendSuccess(st, res) {
        if (st === 1) {
      ...
      // failure
      resendPromise.fail(function resendFail() {
  ...
  var fillQuotaInfo = function (st, quotas) {
    ...
    if (subUser.s === 11 && totalStorage > 0) {
      $(".profile-button-container .migrate-data", $subAccountContainer).safeHTML("<span>@@</span>", l[19095]).removeClass("hidden");
    }
    ...
    var versionsTotalFormatted = numOfBytes(
      rootInfo[3] + rubbishInfo[3] + inshareInternalInfo[3] + inshareExternalInfo[3] + backupsInfo[3],
      2
    );

    // fill in UI
    $(".user-management-view-data .user-management-storage .storage-transfer-data", $subAccountContainer).text(
      totalStorageFormatted.size + " " + totalStorageFormatted.unit
    );
    $(".user-management-view-data .user-management-transfer .storage-transfer-data", $subAccountContainer).text(
      totalBandwidthFormatted.size + " " + totalBandwidthFormatted.unit
    );

    var $cloudDriveSection = $(".user-management-view-data .subaccount-view-used-data .used-storage-info.ba-root", $subAccountContainer);
    var $inShareSection = $(".user-management-view-data .subaccount-view-used-data .used-storage-info.ba-inshare", $subAccountContainer);
    var $inShareExSection = $(
      ".user-management-view-data .subaccount-view-used-data" + " .used-storage-info.ba-inshare-ex",
      $subAccountContainer
    );
    var $outShareSection = $(
      ".user-management-view-data .subaccount-view-used-data" + " .used-storage-info.ba-outshare",
      $subAccountContainer
    );
    var $outShareExternalSection = $(
      ".user-management-view-data .subaccount-view-used-data" + " .used-storage-info.ba-outshare-ex",
      $subAccountContainer
    );
    var $backupsSection = $(
      ".user-management-view-data .subaccount-view-used-data" + " .used-storage-info.ba-backups",
      $subAccountContainer
    );
    var $rubbishSection = $(
      ".user-management-view-data .subaccount-view-used-data" + " .used-storage-info.ba-rubbish",
      $subAccountContainer
    );
    const $linksSection = $(".user-management-view-data .subaccount-view-used-data .used-storage-info.ba-pub-links", $subAccountContainer);
    var $versionsSection = $(
      ".user-management-view-data .subaccount-view-used-data" + " .used-storage-info.ba-version",
      $subAccountContainer
    );
    ...
    $(".ff-occupy", $inShareSection).text(inshareInternalTotalFormatted.size + " " + inshareInternalTotalFormatted.unit);
    ...
    $(".ff-occupy", $inShareExSection).text(inshareExternalTotalFormatted.size + " " + inshareExternalTotalFormatted.unit);
    ...
    $(".ff-occupy", $outShareExternalSection).text(outshareTotalFormatted.size + " " + outshareTotalFormatted.unit);
    ...
    $(".ff-occupy", $outShareSection).text(outshareTotalInternalFormatted.size + " " + outshareTotalInternalFormatted.unit);
    ...
    $(".ff-occupy", $backupsSection).text(backupsTotalFormatted.size + " " + backupsTotalFormatted.unit);
    ...
    var versionsFileNumText = ffNumText(rootInfo[4] + rubbishInfo[4] + inshareInternalInfo[4] + inshareExternalInfo[4], "file");
...
BusinessAccountUI.prototype.viewAdminDashboardAnalysisUI = function () {
  ...
  const populateStoragePieAndData = function (st, quotas) {
    ...
    $(".storage-summary .total-storage-number", $storageAnalysisPie).text(totalStorageFormatted.size + " " + totalStorageFormatted.unit);

    $(".storage-division-container.cloud-node .storage-division-num", $storageAnalysisPie).text(
      rootTotalFormatted.size + " " + rootTotalFormatted.unit
    );
    $(".storage-division-container.cloud-node .storage-division-per", $storageAnalysisPie)
      .text(`${rootPercentage}%`)
      .addClass(cloudNodeDigitClass);
    $(".storage-division-container.inbox-node .storage-division-num", $storageAnalysisPie).text(
      outshareTotalFormatted.size + " " + outshareTotalFormatted.unit
    );
    $(".storage-division-container.inbox-node .storage-division-per", $storageAnalysisPie)
      .text(`${outsharePercentage}%`)
      .addClass(outshareNodeDigitClass);
    $(".storage-division-container.inshare-node .storage-division-num", $storageAnalysisPie).text(
      inshareTotalFormatted.size + " " + inshareTotalFormatted.unit
    );
    $(".storage-division-container.inshare-node .storage-division-per", $storageAnalysisPie)
      .text(`${insharePercentage}%`)
      .addClass(inshareNodeDigitClass);
    $(".storage-division-container.backups-node .storage-division-num", $storageAnalysisPie).text(
      backupsTotalFormatted.size + " " + backupsTotalFormatted.unit
    );
    $(".storage-division-container.backups-node .storage-division-per", $storageAnalysisPie)
      .text(`${backupsPercentage}%`)
      .addClass(backupsNodeDigitClass);
    $(".storage-division-container.rubbish-node .storage-division-num", $storageAnalysisPie).text(
      rubbishTotalFormatted.size + " " + rubbishTotalFormatted.unit
    );
    $(".storage-division-container.rubbish-node .storage-division-per", $storageAnalysisPie)
      .text(`${rubbishPercentage}%`)
      .addClass(rubbishNodeDigitClass);
    ...
    M.require("charts_js").done(function usagePieChartDataPopulate() {
      ...
      const usagePieChart = new Chart($pieChart, {
        ...
        data: {
          ...
          labels: [l[164], l[19187], l[16770], l.restricted_folder_button, l[167]]
        },
        options: {
      ...
      const $customChartLegend = $(".storage-pie-data-container .storage-division-container", $storageAnalysisPie).rebind(
        "click.subuser",
        function chartLegendClickHandler(e) {
          const $me = $(this);
          let ix = 0;
  ...
  const setBarChartScaleUnit = function (res, isTransferData) {
    ...
    const $barChartUnit = isTransferData
      ? $("#trf-bar-chart-unit", $stgeTrfAnalysisContainer)
  ...
  const populateNextBillInfo = function () {
    ...
    $(".next-bill-value.local .value", $nbPriceContainer).text(
      formatCurrency(billData.price_local.amount, billData.price_local.name, "number")
    );
    ...
    if (billData.price_local.name === "EUR") {
      ...
      $(".next-bill-value.euro", $nbPriceContainer).removeClass("hidden").text(formatCurrency(billData.price_eur.amount));
    }
  ...
  const fillStgeTrfBillInfo = function (isTrfField) {
    ...
    if (noLastBillItem) {
      ...
      if (lastBillItem !== 0 && valDiff !== 0) {
        ...
        $(".diff-perc span", $billDiff).text(l.bsn_last_renew_diff_ratio.replace("[X]", `(${valDiffPerc})`));
      }
    }
  };
...
BusinessAccountUI.prototype.initBusinessAccountHeader = function ($accountContainer) {
  ...
  $(".settings-menu-bar .settings-menu-item", $accountContainer)
    .off("click.suba")
    .on("click.suba", function settingHeaderClickHandler() {
      var $me = $(this);
...
BusinessAccountUI.prototype.showExp_GraceUIElements = function () {
  ...
  var msg = "";
  if ((u_attr.b && u_attr.b.s === pro.ACCOUNT_STATUS_EXPIRED) || (u_attr.pf && u_attr.pf.s === pro.ACCOUNT_STATUS_EXPIRED)) {
    ...
    (u_attr.b && u_attr.b.s === pro.ACCOUNT_STATUS_GRACE_PERIOD && u_attr.b.m) ||
...
BusinessAccountUI.prototype.showExpiredDialog = function (isMaster) {
  ...
  if (isMaster) {
    ...
    $(".ok-action", $dialog).rebind("click.subuser", function closeExpiredSubAccountDialog() {
      closeDialog();
    });
...
BusinessAccountUI.prototype.viewBusinessAccountPage = SoonFc(60, function () {
  ...
  const $pageHeader = $(`.user-management-breadcrumb.${$breadcrumbsClass}`, ".fm-right-header-user-management");
  ...
  var unhideSection = function () {
    ...
    if (!u_attr["%name"] && !u_attr["%phone"]) {
      mySelf.business.updateSubUserInfo(u_attr.b.bu, [
        "%name",
        "%phone",
        "%email",
        "%taxnum",
  ...
  var inputs = [$cNameInput, $cTelInput, $cEmailInput, $cVatInput, $cAddressInput, $cAddress2Input, $cCityInput, $cStateInput, $cZipInput];
  ...
  $saveButton.rebind("click.suba", function companyProfileSaveButtonClick() {
    ...
    var settingResultHandler = function (st) {
      if (st) {
        $saveButton.removeClass("active").addClass("disabled");
        toaster.main.neutral(l[19633], "sprite-fm-uni icon-check", {
          hasClose: false
        });
        ...
        if (mega && mega.buinsessAccount && mega.buinsessAccount.invoicesList && mega.buinsessAccount.invoicesList.list) {
  ...
  $(".cnt-ddl .option", $profileContainer).rebind("click.suba", function countrySelectChangingHandler(se) {
...
BusinessAccountUI.prototype.viewInvoiceDetail = function (invoiceID) {
  ...
  var validateInvoice = function (invoice) {
    ...
    if (!invoice.mega.cnum || !invoice.mega.taxnum || !invoice.mega.cname || !invoice.mega.phaddr || !invoice.mega.poaddr) {
      return false;
    }
    ...
    for (var k = 0; k < invoice.items.length; k++) {
      if (
        typeof invoice.items[k].ts === "undefined" ||
        typeof invoice.items[k].gross === "undefined" ||
        typeof invoice.items[k].net === "undefined" ||
        typeof invoice.items[k].tax === "undefined" ||
  ...
  var fillInvoiceDetailPage = function (st, invoiceDetail) {
    ...
    if (invoiceDetail.u.taxnum) {
      $invoiceDetailContainer
        .find(".billed-vat")
        .text(l.taxname_label.replace("%TAXNAME", invoiceDetail.u.taxnum[0]) + " " + invoiceDetail.u.taxnum[1])
        .removeClass("hidden");
    }
    ...
    for (var k = invoiceDetail.items.length - 1; k >= 0; k--) {
      ...
      if (invoiceDetail.items[k].v && invoiceDetail.items[k].list) {
        if (invoiceDetail.items[k].list.t) {
          ...
          $(".inv-pay-desc", $invItem).text(l.additional_transfer.replace("%1", invoiceDetail.items[k].list.t[0]));
        ...
        if (invoiceDetail.items[k].list.s) {
          $invItem = $invItemContentTemplate.clone(true);
          $(".inv-pay-date", $invItem).text(" ");
          $(".inv-pay-desc", $invItem).text(l.additional_storage.replace("%1", invoiceDetail.items[k].list.s[0]));
    ...
    if (invoiceDetail.u.taxnum) {
      ...
      if (oldInvoice || taxExcluded) {
        taxText =
          l.taxname_label.replace("%TAXNAME", invoiceDetail.taxname || invoiceDetail.u.taxnum[0]) +
          ` ${formatPercentage(Number(invoiceDetail.taxrate) / 100, true)}`;
      } else {
        taxText = l.tax_on_invoice
          .replace("%1", invoiceDetail.taxname || invoiceDetail.u.taxnum[0])
    ...
    $(".inv-payment-price.inv-li-total .inv-total-val", $invoiceItemsContainer).text(formatCurrency(invoiceDetail.tot));
    ...
    $(".inv-detail-export", $invoiceDetailContainer).rebind("click.subuser", function invoiceDetailExportClickHandler() {
      M.require("business_invoice").done(function exportOverviewPageToPDF() {
        ...
        myPage = myPage.replace("{2VATTXT}", escapeHTML(l.taxname_label.replace("%TAXNAME", taxnum[0])));
        ...
        if (u.taxnum && u.taxnum[1]) {
          cVat = l.taxname_label.replace("%TAXNAME", u.taxnum[0]) + " " + u.taxnum[1];
        }
            ...
            if (rowStart) {
              ...
              if (rowEnd) {
                ...
                if ($allItems.length) {
                  ...
                  myPage = myPage.slice(0, rowStart) + newRows + myPage.slice(rowEnd + 5, myPage.length);
                }
              }
            }
          } else {
...
BusinessAccountUI.prototype.showDisableAccountConfirmDialog = function (actionFuncHandler, userName, isEnable) {
  ...
  dialogQuestion = dialogQuestion.replace("[B]", "<b>").replace("[/B]", "</b>").replace("{0}", escapeHTML(userName));
  ...
  $("footer .dlg-btn", $dialog).rebind("click.subuser", function () {
    closeDialog();
...
BusinessAccountUI.prototype.showWelcomeDialog = function () {
  ...
  var showTheDialog = function (isViewed) {
    ...
    $("button.js-close", $dialog).rebind("click.subuser", function closeWelcomeDialogHandler() {
      closeDialog();
    });

    $(".add-subuser, .go-to-landing", $dialog).rebind("click.subuser", function welcomeDlgGoToUsersManagement() {
      closeDialog();
...
BusinessAccountUI.prototype.showAddSubUserDialog = function (result, callback) {
  ...
  var clearDialog = function () {
    ...
    $("footer .mega-switch .mega-feature-switch", $dialog).attr("style", "");
  ...
  if (result && result.u && result.m) {
    ...
    if (result.lp) {
      ...
      if (is_extension || M.execCommandUsable()) {
        ...
        $(".copy-pw-btn", $resultContianer).rebind("click.suba", function () {
      ...
      $(".sub-p", $resultContianer).rebind("copy.suba", function passwordTextTouchHandler() {
        $("footer .add-sub-user", $dialog).removeClass("disabled");
        $("footer .add-more", $dialog).removeClass("disabled");
      });
  ...
  $("button.js-close", $dialog).rebind("click.subuser", function exitIconClickHandler() {
    closeDialog();
  });

  // event handler for clicking on "add more"
  $("footer .add-more", $dialog).rebind("click.subuser", function addMoreClickHandler() {
    if ($(this).hasClass("disabled")) {
      return;
    }
    clearDialog();
  ...
  $(".dialog-input-container .opti-add-suba", $dialog).rebind("click.subuser", function showMoreMoreClickHandler() {
    var $me = $(this);
    if ($me.hasClass("show")) {
  ...
  $("footer .mega-switch", $dialog).rebind("click.subuser", function protectLinkClickHandler() {
    var $me = $(this);
  ...
  $("footer .add-sub-user", $dialog).rebind("click.subuser", function addSubUserClickHandler() {
    if ($(this).hasClass("disabled")) {
      return;
    }
    ...
    var subPromise = mySelf.business.addSubAccount(subEmail, subName, subLastName, addUserOptionals, isProtectLink);
...
BusinessAccountUI.prototype.showResetPasswordSubUserDialog = function (subUserHandle) {
  ...
  var subTitle = l[22077].replace("[S]", '<span class="green strong">').replace("[S]", "</span>").replace("{0}", escapeHTML(uName));

  $subTitle.safeHTML(subTitle);

  $("button.js-close, .cancel-dlg", $dialog).rebind("click.subuser", function closeResetSubUserPassword() {
    closeDialog();
  });

  $generateButton.off("click.subuser").on("click.subuser", function generatePasswordBtn() {
  ...
  $passVisibility.off("click.subuser").on("click.subuser", function passVisibiltyChange() {
    if ($passVisibility.hasClass("show-pw")) {
  ...
  $confirmBtn.rebind("click.subuser", function confirmResetPassBtn() {
    ...
    mySelf.business
      .resetSubUserPassword(subUserHandle, mySelf.lastGeneratedPass)
      .then(() => {
        M.safeShowDialog("pass-reset-success-subuser-dlg", () => {
          ...
          $(".dialog-text-one", $resetDialog).safeHTML(l[22081].replace("{0}", `<b>${subUser.e}</b>`));
...
BusinessAccountUI.prototype.showEditSubUserDialog = function (subUserHandle) {
  ...
  const decodedUser = mySelf.decodeFields(subUser, ["firstname", "lastname", "position", "idnum", "phonenum", "location"]);
    ...
    if (st === 0) {
      ...
      if (!res) {
        ...
        if (subUser && subUser.s === 11) {
          msgDialog("info", "", l[24073], "", function () {
            mySelf.showAddSubUserDialog(res);
          });
        } else {
          mySelf.showAddSubUserDialog(res);
  ...
  // close event handler
  $("footer .btn-edit-close, button.js-close", $dialog).rebind("click.subuser", closeDialog);

  // event handler for save button clicking
  $("footer .btn-edit-save", $dialog).rebind("click.subuser", function () {
      ...
      if ("email" in changedVals) {
        ...
        for (const u of Object.values(M.suba)) {
          if (u.e === changedVals.email) {
            $emailInput.megaInputsShowError(u.s !== 0 && u.s !== 10 ? l.err_bus_sub_exists_deactive : l[19562]);
            return;
          }
        }
      }
...
BusinessAccountUI.prototype.showAddSubUserResultDialog = function (results) {
  ...
  $("footer .ok-done", $dialog).rebind("click.subuser", function addSubUserDoneClickHandler() {
    $("footer .ok-done", $dialog).off("keydown.subuserresd");
    closeDialog();
  });
  $("footer .ok-done", $dialog).rebind("keydown.subuserresd", function addSubUserDoneKeydownHandler(key) {
...
BusinessAccountUI.prototype.migrateSubUserData = async function (subUserHandle) {
  ...
  const [f, { k, k2 }] = await Promise.all([this.business.getSubUserTree(subUserHandle), this.business.getSubAccountMKey(subUserHandle)]);
  changePercentage(20);
...
BusinessAccountUI.prototype.UIEventsHandler = function (subuser) {
  ...
  var updateLeftSubUserPanel = function (subuser) {
    ...
    if (!$userRow.length) {
      ...
      if (self && !self.business.hasSubs) {
        $userRow.rebind("click.subuser", function () {
          $(".content-panel.user-management .nw-user-management-item").removeClass("selected");
    ...
    if (subuser.s === 0) {
      $userRow.find(".user-management-status").removeClass("pending disabled").addClass("enabled");
      // leftPanelClass = 'enabled-accounts';
    } else if (subuser.s === 10) {
      $userRow.find(".user-management-status").removeClass("enabled disabled").addClass("pending");
    } else {
      $userRow.find(".user-management-status").removeClass("enabled pending").addClass("disabled");
  ...
  if (!$(".user-management-list-table").hasClass("hidden") || !$(".user-management-landing-page.user-management-view").hasClass("hidden")) {
  ...
  else if (!$(".files-grid-view.user-management-view .user-management-subaccount-view-container").hasClass("hidden")) {
    var viewedUserId = $(".files-grid-view.user-management-view .user-management-subaccount-view-container").attr("id");
  ...
  } else if (!$(".files-grid-view.user-management-view .user-management-account-settings").hasClass("hidden")) {
    if (!subuser.u) {
      // dummy attr
      // safe to create new object.
