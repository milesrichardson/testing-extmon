    ...
    if (ptr === -1) {
      ...
      if (nextIndex > -1 && nextIndex - 1 >= 0) {
        ...
        if (!shiftKey) {
          ...
          if (this.selected_list.length > 0 && this.selected_list.includes(nextId)) {
      ...
      nextIndex = nextIndex + 1 >= currentViewIds.length ? -1 : nextIndex;
      ...
      if (nextIndex + 1 < currentViewIds.length) {
        ...
        if (!shiftKey) {
          ...
          if (this.selected_list.length > 1 && this.selected_list.includes(nextId)) {
...
class SelectionManager2_DOM extends SelectionManager2Base {
  ...
  get items_per_row() {
    return Math.floor($(".data-block-view:visible").parent().outerWidth() / $(".data-block-view:visible:first").outerWidth(true));
  }
  ...
  bindJqSelectable(target) {
     ...
     */
    $jqSelectable.rebind("selectableunselecting.sm selectableunselected.sm", (e, data) => {
    ...
    if ($jqSelectable.is(`${$.selectddUIgrid}:not(.hidden)`)) {
      ...
      $jqSelectable.rebind("mousedown.sm", (e) => {
        ...
        if (
          $target.is(`${$.selectddUIgrid}:not(.hidden)`) &&
          e.button === 0 &&
          !e.shiftKey &&
          !e.metaKey &&
          !e.ctrlKey &&
  ...
  _get_selectable_container() {
    ...
    if (!targetScope || !targetScope.parentNode || targetScope.classList.contains("hidden") || !$(targetScope).is(":visible")) {
  ...
  select_all() {
    ...
    const currentNode = M.d[this.currentdirid] || (M.currentrootid === "s4" && M.d[this.currentdirid.split("/").pop()]);
    ...
    if (itemsNum === 0) {
      ...
      if (totalNodes) {
        if (totalNodes === 1) {
          ...
          notificationText = mega.icu
            .format(l[24672], totalNodes)
  ...
  showSelectionBar(notificationText, itemSelected, itemsTotalSize, totalNodes) {
    ...
    if (this.currentdirid === "out-shares") {
      scrollBarYClass = M.viewmode
        ? ".out-shared-blocks-scrolling.ps--active-y"
        : ".out-shared-grid-view .grid-scrolling-table.ps--active-y";
    } else if (this.currentdirid === "shares") {
      scrollBarYClass = M.viewmode ? ".shared-blocks-scrolling.ps--active-y" : ".shared-grid-view .grid-scrolling-table.ps--active-y";
    } else {
      scrollBarYClass = M.viewmode === 1 ? ".file-block-scrolling.ps--active-y" : ".grid-scrolling-table.ps--active-y";
    }

    if ((!M.gallery || M.isAlbumsPage()) && (this.currentdirid.substr(0, 7) !== "search/" || this.selected_list.length > 0)) {
  ...
  showRequiredLinks() {
    ...
    if (isAlbums && mega.gallery.albums.grid && mega.gallery.albums.grid.timeline) {
      ...
      shareButton.dataset.simpletip = (sourceRoot === "s4" && M.geS4NodeType(selNode) === "bucket" && l.s4_share_bucket) || l[5631];
      ...
      if ((sourceRoot === M.RootID || sourceRoot === "s4" || M.isDynPage(sourceRoot)) && !folderlink) {
      ...
      if (M.getNodeRoot(M.currentdirid) !== M.RubbishID && (!mega.lite.inLiteMode || !mega.lite.containsFolderInSelection($.selected))) {
        __showBtn("download");
      }
      ...
      if (sourceRoot === M.InboxID || restrictedFolders) {
        ...
        (!folderlink && M.currentrootid !== "shares" && M.currentdirid !== "shares") ||
      ...
      M.dyh("required-links").then((links) => {
        if (links) {
