lazy(mega.gallery, "albums", () => {
  ...
  scope.isInSelectArea = (domEl, [left, right, top, bottom], containerPadding = 0) => {
    ...
    return (
      (fitVert && (fitHoriz || (offsetLeft < left && rightEdge > right))) ||
  ...
  scope.playSlideshow = (albumId, useFullscreen, autoplay) => {
    ...
    if (album && album.nodes.length > 0) {
      const selHandles = scope.albums.grid && scope.albums.grid.timeline ? Object.keys(scope.albums.grid.timeline.selections) : [];
      ...
      delay("toggle:album_slideshow_on", () => {
        ...
        for (let i = 0; i < selectModifiers.length; i++) {
          eventsToDisposeOnClose.push(MComponent.listen(selectModifiers[i], "click", modifySelection));
        }
        ...
        mBroadcaster.once("slideshow:close", () => {
          ...
          if (M.isAlbumsPage(1) || !selHandles.length) {
            ...
            window.selectionManager.showSelectionBar(
              mega.icu.format(l.album_selected_items_count, album.nodes.length).replace("%1", selHandles.length)
            );
          }
        });
      });
  ...
  scope.getAlbumsHandles = (albumIds) => {
    ...
    if (albumIds.length === 1 && albumIds[0] === scope.getAlbumIdFromPath() && scope.albums.grid.timeline.selCount > 0) {
   ...
   * @returns {String[]}
   */
  const unwantedHandles = () => MegaGallery.handlesArrToObj([...M.getTreeHandles(M.RubbishID), ...M.getTreeHandles("shares")]);
    ...
    delay("album:" + albumId + ":update_placeholder", () => {
      ...
      if (album.nodes.length) {
        const shouldUpdateCover = album.filterFn
          ...
          : !album.node || !album.at.c || !album.eIds[album.at.c] || album.eIds[album.at.c] !== album.node.h;
    ...
    return [
      ...
      },
      {
        ...
        click: () => {
          ...
          if (handles.length) {
            scope.reportDownload();
            M.addDownload(handles, true, false, albumIds.length > 1 ? "Album-archive-1" : scope.albums.store[albumIds[0]].label);
          }
        }
      }
    ];
  ...
  const sortStore = () => {
    ...
    albumKeys.sort((keyA, keyB) => sortAlbumsArray(scope.albums.store[keyA], scope.albums.store[keyB]));
   ...
   * @returns {void}
   */
  const albumNameExists = (name, ignoreId) => Object.values(scope.albums.store).some(({ label, id }) => label === name && id !== ignoreId);
  ...
  const insertAlbumElement = (albumId, domElement, domContainer, siblingComponentKey) => {
    ...
    if (aIndex === aKeys.length - 1) {
      ...
      domContainer.insertBefore(domElement, scope.albums.store[aKeys[aIndex + 1]][siblingComponentKey].el);
    }
  };
      ...
      if (M.isAlbumsPage(1)) {
        ...
        if (grid.timeline) {
          ...
          if (grid.timeline.selCount > 0) {
            window.selectionManager.showSelectionBar(
              mega.icu.format(l.album_selected_items_count, album.nodes.length).replace("%1", grid.timeline.selCount)
            );
          } else {
            window.selectionManager.hideSelectionBar();
          }
   ...
   * @returns {Boolean}
   */
  const checkIfExpandable = () => Object.values(scope.albums.store).some((album) => albumIsRenderable(album));
  ...
  class ToCopyInput extends MComponent {
    constructor(isKey) {
      ...
      this.copyResponse = isKey ? mega.icu.format(l.toast_copy_key, 1) : mega.icu.format(l.toast_copy_link, 1);
  ...
  class AlbumShareLinksList extends MComponent {
    ...
    set albumIds(albumIds) {
      ...
      for (let i = 0; i < ids.length; i++) {
        this._shares.push(
          new AlbumShareLinkBlock(this.el.firstElementChild, ids[i], this.isMultiple, () => {
  ...
  class AlbumShareDialog extends MDialog {
    ...
    setContent(albumIds) {
      ...
      this.list = new AlbumShareLinksList(albumIds, () => {
        this.hide();
      });
  ...
  class AlbumItemsDialog extends TimelineDialog {
            ...
            if (this.timeline && album) {
              ...
              if (handles.length > 0) {
                ...
                if (addedCount > 0) {
                  ...
                  mega.sets.elements
                    .bulkAdd(handlesToAdd, albumId, k)
                    .then(() => {
                      toaster.main.show({
                        icons: ["sprite-fm-mono icon-check-circle text-color-medium"],
                        content: mega.icu.format(l.album_added_items_status, addedCount).replace("%s", limitNameLength(label))
                      });
    ...
    onMDialogShown() {
      ...
      if (galleryNodes.cu.length > 0 && galleryNodes.cd.length > 0) {
        ...
        div.textContent = galleryNodes.cu.length > 0 ? l.on_camera_uploads : l.on_cloud_drive;
  ...
  class AlbumCoverDialog extends TimelineDialog {
        ...
        ok: {
          ...
          callback: () => {
            if (this.timeline && this.timeline.selCount) {
              scope.albums.updateAlbumCover(scope.albums.store[this.albumId], Object.keys(this.timeline.selections)[0]);
            }
          }
        },
        cancel: true,
    ...
    onMDialogShown() {
      ...
      if (nodes && nodes.length) {
        this.timeline.selectNode(c && eIds[c] && M.d[eIds[c]] && M.getNodeRoot(M.d[eIds[c]].p) !== M.RubbishID ? M.d[eIds[c]] : nodes[0]);
      }
  ...
  class AlbumsEmpty {
    ...
    setContents(title, btnLabel, buttonFn) {
      ...
      const button = new MButton(btnLabel, null, buttonFn, "mega-button large positive");
    ...
    constructor(albumId, isPublic) {
      ...
      if (isPublic) {
        ...
        if (isUserAlbum) {
          ...
          if (album.nodes.length) {
            options.push(generateDownloadMenuItem([albumId]), {
  ...
  class AlbumCell extends MComponent {
    constructor(albumId) {
      ...
      this.attachEvent("dblclick", () => {
        M.openFolder("albums/" + albumId);
      });

      this.attachEvent("contextmenu", ({ pageX, pageY }) => {
        if (!$.dialog) {
          ...
          const contextMenu = selectedCells.length > 1 ? new MultipleAlbumsContextMenu(selectedCells) : new AlbumContextMenu(albumId);
        ...
        const optionsBtn = AlbumsGridHeader.attachButton(
          ...
          () => {
            const contextMenu = new AlbumOptionsContextMenu(
              buttons.slice(headerBtnLimit).map(([label, icon, click, parent, isDisabled, children, classes]) => {
                return {
                  label,
                  click,
  ...
  AlbumsGridHeader.attachButton = (label, icon, clickFn, parent, isDisabled) => {
    const button = new MButton(label, `icon-${icon}`, clickFn, "mega-button action ml-5");
  ...
  class AlbumsGrid {
    ...
    setPendingCell(label) {
      ...
      this.updateGridState(Object.values(scope.albums.store).filter((album) => albumIsRenderable(album)).length + 1);
      this.el.scrollTop = 0;
    }
    ...
    showEmptyAlbumPage(albumId) {
      ...
      if (scope.albums.isPublic) {
        ...
        this.addEmptyBlock(
          new AlbumsEmpty(l.album_no_media, l.add_album_items, () => {
            const dialog = new AlbumItemsDialog(albumId);
            dialog.show();
          })
        );
    ...
    showAlbumContents(albumId) {
          ...
          delay(
            ...
            () => {
              ...
              if (this.timeline.selCount) {
                window.selectionManager.showSelectionBar(
                  mega.icu.format(l.album_selected_items_count, album.nodes.length).replace("%1", this.timeline.selCount)
                );
      ...
      delay("render:album_content_timeline", () => {
        if (this.timeline && this.timeline.el && albumId === scope.getAlbumIdFromPath()) {
          window.selectionManager = new AlbumsSelectionManager(albumId, this.timeline.el).reinitialize();
        }
      });
    ...
    updateGridState(count, useDefaultEmptyPad = true) {
      ...
      if (useDefaultEmptyPad) {
        if (isEmpty) {
          this.addEmptyBlock(
            new AlbumsEmpty(l.no_albums, l.create_new_album, () => {
              const dialog = new AlbumNameDialog();
              dialog.show();
            })
          );
    ...
    refresh() {
      this.updateGridState(Object.values(scope.albums.store).filter((album) => albumIsRenderable(album)).length);
    }
    ...
    attachKeyboardEvents() {
      ...
      scope.disposeKeyboardEvents = (() => {
        const disposeKeydown = MComponent.listen(document, "keydown", (evt) => {
          ...
          const lastSelIndex = this.lastSelected ? albums.findIndex(({ cellEl }) => cellEl.el === this.lastSelected) : -1;
          ...
          if (curIndex < 0) {
            ...
            curIndex =
              isCtrl || shiftKey || (curIndex - lastSelIndex > 1 && curIndex - (albums.length - 1) < albumsPerRow) ? albums.length - 1 : 0;
          }
          ...
          const adjustScrollTop = () => {
            if (albumCell.el.offsetTop < scope.albums.grid.el.scrollTop) {
              ...
              const bottomOverlap =
                albumCell.el.offsetTop + albumCell.el.offsetHeight - (scope.albums.grid.el.scrollTop + scope.albums.grid.el.clientHeight);
          ...
          delay("album:" + s + ":add_items", () => {
            if (timeline) {
              ...
              if (timeline.selCount > 0) {
                window.selectionManager.showSelectionBar(
                  mega.icu.format(l.album_selected_items_count, album.nodes.length).replace("%1", timeline.selCount)
                );
              }
            }
          });
  ...
  class AlbumsTree {
    ...
    setPendingButton(label) {
      this.pendingBtn = new MSidebarButton(label + " " + l.album_name_creating, "icon-album", nop, "pending-btn subalbum-btn");
    ...
    setHeader() {
      this.headButton = new MSidebarButton(l.albums, "icon-album", () => {
        storeLastActiveTab();
  ...
  AlbumsTree.createButton = (albumId, label, isShared) => {
    ...
    btn.attachEvent("contextmenu", (evt) => {
      evt.preventDefault();
  ...
  class Albums {
    constructor() {
      ...
      this.grid = null;
      this.store = {
        // The length of the key should be always as per predefinedKeyLength
        ...
        raw: {
          ...
          filterFn: (n) => n.fa && is_rawimage(n.name) !== undefined && !ignoreRaws[fileext(n.name || "", true, true)]
        }
      };
    ...
    async initPublicAlbum() {
      ...
      if (handles.length) {
        $(".fm-import-to-cloudrive span", ".folder-link-btns-container").text(u_type ? l.context_menu_import : l.btn_imptomega);
      } else {
    ...
    init(handles) {
      ...
      const gallerySidebarWrap = document.querySelector(".js-lp-gallery.lp-gallery .js-gallery-panel .lp-content-wrap-wrap");
    ...
    createAlbumData({ e, at, k, id, ts, p, cts }, ignoreHandles, isPublic) {
      const attr = isPublic ? mega.sets.decryptPublicSetAttr(at, k) : mega.sets.decryptSetAttr(at, k);
    ...
    onCDNodeRemove(node) {
      ...
      const albumKeys = Object.keys(this.store).filter((k) => Array.isArray(this.store[k].nodes) && this.store[k].nodes.length > 0);
          ...
          delay("album:" + albumId + ":add_items", () => {
            if (this.grid && this.grid.timeline) {
              ...
              if (this.grid.timeline.selCount > 0) {
                window.selectionManager.showSelectionBar(
                  mega.icu.format(l.album_selected_items_count, album.nodes.length).replace("%1", this.grid.timeline.selCount)
                );
              }
            }
          });
    ...
    updateAlbumDataByUpdatedNode(albumId, node) {
      const { h: handle } = node;
      const album = this.store[albumId];
      const additionIsNeeded =
        album && ((album.filterFn && album.filterFn(node)) || (!album.filterFn && album.eHandles[handle]))
    ...
    removeElementsByHandles(handles) {
      ...
      for (let i = 0; i < album.nodes.length; i++) {
        statsObj[(handlesCache[album.nodes[i].h] ? "del" : "left") + (mega.gallery.isVideo(album.nodes[i]) ? "Vid" : "Img")]++;
      }

      mega.sets.elements
        .bulkRemove(ids, album.id)
        .then(() => {
          const content = generateToastContent(
            mega.icu.format(l.album_items_removed_status, handles.length).replace("%s", limitNameLength(album.label))
          );
          ...
          delay("albums_stat_99917", eventlog.bind(null, 99917, JSON.stringify(statsObj)));
        })
        .catch(() => {
    ...
    updateAlbumCover({ at, id, k, eHandles }, handle) {
      ...
      mega.sets
        .updateAttrValue({ at, k, id }, "c", eHandles[handle] || "")
        .then(() => {
          if (this.grid && this.grid.timeline) {
     ...
     */
    addShare(albumIds) {
      mega.Share.initCopyrightsDialog([], false, async () => {
        delete $.itemExport;
