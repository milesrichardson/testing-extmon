...
var security = {
  ...
  fetchAccountVersionAndSalt: async function (email) {
    ...
    const {
      result: { s, v }
    } = await api.req({ a: "us0", user: email });
  ...
  resetUser: function (recoveryCode, recoveryEmail, newPassword, completeCallback) {
    ...
    security.fetchAccountVersionAndSalt(recoveryEmail).then(({ version }) => {
      ...
      if (version === 2) {
        ...
        security
          .deriveKeysFromPassword(newPassword, u_k)
  ...
  verifyPassword: promisify(function (resolve, reject, aPassword, aMasterKey, aPrivateKey, aSalt) {
    ...
    (function _next() {
      ...
      security
        .getDerivedEncryptionKey(aPassword, salt || false)
  ...
  completeVerifyEmail: promisify(function (resolve, reject, pwd, code) {
    ...
    if (u_attr.aav > 1) {
      ...
      security
        .deriveKeysFromPassword(pwd, u_k)
        .then(({ clientRandomValueBytes, encryptedMasterKeyArray32, hashedAuthenticationKeyBytes }) => {
      ...
      if (aStep === "login-to-account") {
        ...
        api
          ...
          .then(({ result: res }) => {
            ...
            if (is_mobile) {
              ...
              $(".cancel-email-verify", $dialog)
                .removeClass("hidden")
                .rebind("click.cancel", function () {
                  loadSubPage("start");
                });
            } else {
            ...
            $("button.next", $dialog).rebind("click.ve", function () {
              ...
              showLoading();
              security
                .verifyPassword(pwd, u_attr.evk, u_attr.privk)
                .then(function (res) {
                  u_k = res.k;
                  u_attr.aav = 1 + !!res.s;
        ...
        $("button.finish", $dialog).rebind("click.ve", function () {
          ...
          var error = function (msg) {
            hideLoading();
            $("input", $dialog).val("").trigger("blur").first().trigger("input").megaInputsShowError(msg).trigger("focus");
            return false;
          };
          ...
          showLoading();
          security
            .verifyPassword(pw1, u_attr.evk, u_attr.privk)
            .then(function () {
            ...
            .catch(function (ex) {
              ...
              security
                .completeVerifyEmail(pw1, u_attr.evc)
                .then(function () {
                ...
                .catch(function (ex) {
                  hideLoading();
                  msgDialog("warninga", l[135], l[47], ex < 0 ? api_strerror(ex) : ex, reset.bind(null, false));
                });
            });
...
security.register = {
  ...
  startRegistration: function (firstName, lastName, email, password, fromProPage, completeCallback) {
    ...
    security.register.createEphemeralAccount(function () {
      ...
      security
        .deriveKeysFromPassword(password, u_k)
        .then(({ clientRandomValueBytes, encryptedMasterKeyArray32, hashedAuthenticationKeyBytes }) => {
  ...
  async verifyEmailConfirmCode(confirmCode) {
    ...
    return api.req({ a: "ud2", c: confirmCode }).then(({ result }) => {
...
security.login = {
  ...
  checkLoginMethod: function (email, password, pinCode, rememberMe, oldStartCallback, newStartCallback) {
    ...
    // Fetch the user's salt from the API
    security
      .fetchAccountVersionAndSalt(email)
      .then(({ version, salt }) => {
      ...
      callback: function (result) {
        ...
        if (typeof result === "object") {
          ...
          if (typeof temporarySessionIdBase64 !== "undefined") {
            ...
            security.login.decryptRsaKeyAndSessionId(
   ...
   */
  decryptRsaKeyAndSessionId: function (masterKeyArray32, encryptedSessionIdBase64, encryptedPrivateRsaKeyBase64, userHandle) {
    "use strict";
