 ...
 */
SharedLocalKVStorage.DEFAULT_QUERY_TIMEOUT = mega.chrome ? 10000 : 1000;
...
SharedLocalKVStorage.prototype._setupPersistance = function () {
  ...
  if (self.broadcaster.crossTab.master) {
    ...
    self.persistAdapter = new SharedLocalKVStorage.Utils.DexieStorage(self.name, self.manualFlush, self.wdog.wdID);
...
SharedLocalKVStorage.prototype.getItem = function (k) {
  ...
  if (self.broadcaster.crossTab.master) {
    ...
    self.wdog
      .query("slkv_get_" + self.name, SharedLocalKVStorage.DEFAULT_QUERY_TIMEOUT, false, { k: k }, true)
      .then((response) => {
...
SharedLocalKVStorage.prototype.eachPrefixItem = function __SLKVEachItem(pfx, each) {
  ...
  return new MegaPromise((resolve, reject) => {
    this.wdog
      .query(`slkv_getby_${this.name}`, SharedLocalKVStorage.DEFAULT_QUERY_TIMEOUT, false, { pfx }, true)
      .then(([res]) => {
        if (!res) {
...
SharedLocalKVStorage.prototype.setItem = function (k, v, meta) {
  ...
  return new MegaPromise((resolve, reject) => {
    ...
    this.wdog
      .query(`slkv_set_${this.name}`, SharedLocalKVStorage.DEFAULT_QUERY_TIMEOUT, false, { k, v }, true)
      .then(resolve)
      .catch(reject)
      .finally(() => {
...
SharedLocalKVStorage.prototype.clear = function () {
  ...
  return new MegaPromise((resolve, reject) => {
    this.wdog.query(`slkv_clear_${this.name}`, SharedLocalKVStorage.DEFAULT_QUERY_TIMEOUT, false, false, true).then(resolve).catch(reject);
  });
};
...
SharedLocalKVStorage.prototype.destroy = function () {
  ...
  return new MegaPromise((resolve, reject) => {
    self.wdog
      .query(`slkv_destroy_${this.name}`, SharedLocalKVStorage.DEFAULT_QUERY_TIMEOUT, false, false, true)
      .then(resolve)
      .catch(reject);
  });
};
...
SharedLocalKVStorage.Utils.DexieStorage.prototype._OpenDB = function () {
  ...
  return self.db.kv.toArray().then(function (r) {
...
SharedLocalKVStorage.Utils.DexieStorage.prototype.flush = function () {
  ...
  var bulkDelete = Object.keys(self.delcache).map(function (k) {
    delete self.dbcache[k];
    return self.db.kv.where("k").equals(self._encryptKey(k)).delete();
  });

  var bulkPut = Object.keys(self.newcache).map(function (k) {
  ...
  Promise.all(bulkDelete)
    ...
    .catch(function (ex) {
      if (d || is_karma) {
        self.db.kv.toArray().then(function (o) {
