...
var notify = {
  ...
  getInitialNotifications: function () {
    ...
    api
      .req(`c=${this.numOfNotifications}`, 3)
      .then(({ result }) => {
  ...
  isUnwantedNotification: function (notification) {
    ...
    switch (notification.a || notification.t) {
      ...
      case "c":
        ...
        if ((action === 0 && !mega.notif.has("contacts_fcrdel")) || (action === 1 && !mega.notif.has("contacts_fcracpt"))) {
          return true;
        }
        break;
      ...
      if (notify.notifications[i].seen === false) {
        ...
        if (notify.notifications[i].type === "mcsmp") {
          ...
          if (megaChatIsReady) {
            ...
            if (res !== false && (res === 0 || !(res.mode === ScheduleMetaChange.MODE.CREATED && data.ou === u_handle))) {
              newNotifications++;
            }
          }
        } else {
  ...
  getUserEmailByTheirHandle: function (userHandle, data, skipFetch) {
    ...
    const promises = [M.syncUsersFullname(userHandle), M.syncContactEmail(userHandle, true)];
    this.userEmails[userHandle] = Promise.allSettled(promises).then(() => {
  ...
  initTakedownClickHandler: function () {
    ...
    this.$popup.find(".nt-takedown-notification, .nt-takedown-reinstated-notification").rebind("click", function () {
      ...
      if (parentFolderId) {
        // Open the folder
        M.openFolder(parentFolderId).always(function () {
          reselect(true);
        });
      }
    });
    ...
    this.$popup.find(".notification-item.nt-payment-notification").rebind("click", function () {
      ...
      mBroadcaster.once("settingPageReady", function () {
        ...
        if ($scrollBlock.length) {
          $scrollBlock.scrollTop($target.offset().top - $scrollBlock.offset().top + $scrollBlock.scrollTop());
        }
      });
    });
  },
  ...
  initAcceptContactClickHandler: function () {
    ...
    this.$popup.find(".notification-item .notifications-button.accept").rebind("click", function () {
      ...
      M.acceptPendingContactRequest(pendingContactId).catch(() => {
    ...
    if (notification.data.orig) {
      ...
      if (notificationOwner === u_handle) {
        if (notificationOrginating === u_handle) {
          console.error("receiving a wrong notification, this notification shouldnt be sent to me", notification);
        } else {
  ...
  renderNewSharedNodes: function ($notificationHtml, notification, email) {
    ...
    if (folderCount >= 1 && fileCount >= 1) {
      title = email
        ? mega.icu.format(l.user_item_added_count, folderCount + fileCount).replace("[X]", email)
        : mega.icu.format(l.item_added_count, folderCount + fileCount);
    } else if (folderCount > 0) {
      title = email ? l[836].replace("[X]", email).replace("[DATA]", folderText) : mega.icu.format(l.folder_added_count, folderCount);
    } else if (fileCount > 0) {
      title = email ? l[836].replace("[X]", email).replace("[DATA]", fileText) : mega.icu.format(l.file_added_count, fileCount);
    }
  ...
  renderPayment: function ($notificationHtml, notification) {
    ...
    if (!notification.seen && !notify.welcomeDialogShown && mega.flags.ab_wdns && !(u_attr.pf || u_attr.b) && u_attr["^!welDlg"] | 0) {
    ...
    M.accountData((account) => {
      ...
      const getPlansEndingAfterPurchase = () => {
        ...
        account.purchases.forEach((purchase) => {
          const days = purchase[6] === 1 ? 32 : purchase[6] === 12 ? 367 : purchase[6] * 31;
      ...
      if (account.purchases[0][5] !== u_attr.p) {
        ...
        const bodyText = plansEndingAfterPurchase < 2 ? l.welcome_dialog_active_until : l.welcome_dialog_active_check;
        msgDialog(
          "warninga",
          "",
          l.welcome_dialog_thanks_for_sub.replace("%1", newPlan),
          bodyText.replace("%1", currentPlan).replace("%3", time2date(account.suntil, 1)).replace("%2", newPlan)
        );
        return;
      }
      ...
      pro.loadMembershipPlans(() => {
        const plan = pro.membershipPlans.find((currentPlan) => {
          return currentPlan[pro.UTQA_RES_INDEX_ACCOUNTLEVEL] === account.purchases[0][5] && currentPlan[pro.UTQA_RES_INDEX_MONTHS] === 1;
        });
        ...
        $(".more-quota .info-text", $dialog).text(
          l.welcome_dialog_quota_details.replace("%1", bytesToSize(storage, 0)).replace("%2", bytesToSize(transfer, 0))
        );
  ...
  renderTakedown: function ($notificationHtml, notification) {
    ...
    if (notification.data.down === 1) {
      header = l[8521];
      title =
        node.t === 0 ? l.publicly_shared_file_taken_down.replace("%1", name) : l.publicly_shared_folder_taken_down.replace("%1", name);
    ...
    // Your taken down %1 (%2) has been reinstated.
      ...
      header = l[8524];
      title = node.t === 0 ? l.taken_down_file_reinstated.replace("%1", name) : l.taken_down_folder_reinstated.replace("%1", name);
        ...
        if (
          (meta.prevTiming &&
            (meta.timeRules.startTime !== meta.prevTiming.startTime ||
              ...
              (meta.recurring && !megaChat.plugins.meetingsManager.areMetaObjectsSame(meta.timeRules, meta.prevTiming)))) ||
      ...
      M.u.forEach(function (contact) {
        ...
        if (contactEmail === email) {
          ...
          if (M.u[contactHandle].nickname !== "") {
            ...
            displayName = (M.u[contactHandle].firstName + " " + M.u[contactHandle].lastName).trim() + " (" + email + ")";
          }
