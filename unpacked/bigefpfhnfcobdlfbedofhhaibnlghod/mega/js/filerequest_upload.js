...
lazy(mega, "fileRequestUpload", () => {
  ...
  class FileRequestUploadEventHandler {
    ...
    handle(optionReference, event) {
      ...
      const targetId = mega.fileRequestUpload.getUploadPagePuHandle();

      const ownerHandle = mega.fileRequestUpload.getUploadPageOwnerHandle();
      ...
      for (let i = 0; files[i]; i++) {
        ...
        if (file.webkitRelativePath) {
          path = String(file.webkitRelativePath).replace(RegExp(`[\\/]${String(file.name).replace(/(\W)/g, "\\$1")}$`), "");
        }
  ...
  class FileRequestUploadPage {
    ...
    init() {
      ...
      this.$uploadButton = new mega.fileRequestUI.ButtonComponent($(".upload-btn, .upload-more", this.$wrapper), {
        namespace: "frup"
      });
      this.$languageButton = new mega.fileRequestUI.ButtonComponent($(".language-wrapper", this.$wrapper), {
        namespace: "frup"
      });
      this.$dialogOverlay = new mega.fileRequestUI.ButtonComponent($(".fm-dialog-overlay", document.body), {
        namespace: "frup"
      });
    ...
    addEventHandlers() {
      ...
      this.$wrapperItems.off("click.frup", uploadItemReupload).on("click.frup", uploadItemReupload, () => {
        this.$selectFileButton.click();
      });
    ...
    handleResize() {
      ...
      const footerCaptionHeight = $pageCaption.outerHeight(true) + $(".page-footer", this.$wrapper).outerHeight(true);
    ...
    getAndSetUploadHandler(optionReference) {
      ...
      this.$selectFileButton.rebind("change.frup", (...args) => {
    ...
    setLabel(name, title, description, error) {
      this.$labelName.addClass("hidden").text("");

      if (name) {
        const formattedName = l.file_request_upload_content_title.replace("%1", name);

        this.$labelName.removeClass("hidden").text(formattedName);
      }

      this.$wrapperDetail.removeClass("hidden content-error");
      this.$labelTitle.addClass("hidden").text("");
      this.$labelDescription.addClass("hidden").text("");
    ...
    updateUploadInfo() {
      const formattedInfo = mega.icu.format(l.file_request_upload_summary_total_plural, this.stats.total).replace("%1", this.stats.current);

      this.$labelUploadInfo.text(formattedInfo);
    }

    onItemUploadProgress(ul, bps, time, perc, bl) {
      // Item Upload
    ...
    onItemUploadError(abort, uid, error, ignoreGeneral) {
      // Item Upload
      ...
      let gid = null;
      if (errorType && taskType === ERROR_TASK_TYPE_GENERAL && !ignoreGeneral) {
        // Show stoppers
        switch (errorType) {
    ...
    createItem(id, name, size, status) {
      ...
      $newItem.removeClass("hidden").attr("id", id);
    ...
    addItem(fileId, fileName, status, fileSize) {
      ...
      if (this.queue.total === 1) {
        // Add last-item class on first element
        $newItem.addClass("last-item");
      }

      this.$uploadItems.prepend($newItem);
      this.$wrapperEmptyBlock.removeClass("block-empty-error").addClass("hidden");
    ...
    handleError(type, error) {
      ...
      this.setLabel(name, messageObject.title, messageObject.description, true);
    ...
    initUploadErrorEventHandler() {
      ...
      mBroadcaster.addListener(`upload:abort`, this.onItemUploadError.bind(this, true));
      mBroadcaster.addListener(`upload:error`, this.onItemUploadError.bind(this, false));
    ...
    showEmptyBlockError() {
      this.$wrapperEmptyBlock.addClass("block-empty-error").removeClass("hidden");
    }
  }
  ...
  return new (class FileRequestUploadPageHandler {
    ...
    async handlePublicUploadPage(uploadPagePath) {
      ...
      if (!data) {
        ...
        data = await this.fileRequestApi
          .getPuPage(puPageId)
          .catch((ex) => {
            logger.error(ex);
    ...
    async handleChecked() {
      ...
      return this.fileRequestApi.getOwnerPublicKey(this.ownerHandle).then((pubk) => {
    ...
    handleException(exception, puPagePublicHandle) {
      ...
      if (d) {
        logger.info("Upload - PageHandler.handleException - Exception", exception, puPagePublicHandle);
      }
    }
    ...
    parseParameters(pagePath) {
      ...
      if (pagePath.substr(0, 1) !== "!") {
        ...
        if (isPreview) {
          // It has a pup handle and has preview parameters
          isUpdate = true;
        }
      }
