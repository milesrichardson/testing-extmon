...
(function _dataStruct(global) {
  ...
  function MegaDataEvent(src, target) {
    if (typeof src === "object") {
      ...
      if (
        ...
        (src.isDefaultPrevented && src.isDefaultPrevented())
      ) {
  ...
  MegaDataEmitter.prototype.off = function (event, handler) {
    ...
    for (var j = emitter.types.length; j--; ) {
      ...
      for (var i = handlers.length; i--; ) {
        ...
        if (type === tmp.type && (!handler || handler.pid === tmp.pid) && (!namespace || namespace === tmp.namespace)) {
          handlers.splice(i, 1);
        }
      }
  ...
  MegaDataEmitter.prototype.trigger = function (event, data) {
    ...
    while ((tmp = handlers[idx++]) && !event.isPropagationStopped()) {
      ...
      if ((!namespace || namespace === tmp.namespace) && (res = tmp.handler.apply(this, data)) !== undefined) {
        event.result = res;
        if (res === false) {
  ...
  function MegaDataBitMap(name, isPub, keys) {
    ...
    this._readyPromise = new Promise((resolve, reject) => {
      mega.attr
        .get(u_handle, name, this.isPublic() ? true : -2, true)
        .then((r) => {
          if (typeof r !== "string") {
            throw r;
          }
  ...
  MegaDataBitMap.prototype.setSync = function (key, v, ignoreDataChange, defaultVal = 0) {
    ...
    if (
      (typeof this._data[keyIdx] === "undefined" && typeof defaultVal !== "undefined" && _cmp(defaultVal, v) === true) ||
    ...
    for (let i = 0, strLen = str.length; i < strLen; i++) {
      ...
      if (this._data[i] !== newVal) {
        if (this._updatedMask[i] && this._updatedMask[i] === 1) {
          ...
          this._data[i] = newVal;
          this.trackDataChange(this._data, this._keys[i], newVal);
          if (requiresCommit) {
  ...
  MegaDataBitMap.prototype.maskToString = function () {
    return base64urlencode(String.fromCharCode.apply(null, this._updatedMask));
  };
  ...
  MegaIntBitMap.prototype.load = function () {
    var self = this;
    return new MegaPromise(function (resolve, reject) {
      mega.attr.get(u_attr.u, self.attribute, self.pub, self.nonHistoric).then(
        function (value) {
  ...
  MegaIntBitMap.prototype.save = function () {
    return mega.attr.set(this.attribute, this.value, this.pub, this.nonHistoric);
  };
