(function (scope) {
  ...
  ChatdPersist.prototype.init = function () {
    ...
    self.chatd.rebind("onMessageKeysDone.chatdPersist", function (e, data) {
      ...
      data.keys.forEach(function (key) {
        if (isFromHist) {
          self.chatd.chatdPersist.addToQueue(data.chatId, ChatdPersist.QUEUE_TYPE.KEY, [key.keyid, key.userId, key.key]);
        } else {
          self.chatd.chatdPersist.persistKey(data.chatId, key.keyid, key.userId, key.key).catch((ex) => {
            if (d && ex) {
              self.logger.warn(ex);
            }
          });
    ...
    megaChat.rebind("onRoomDestroy.chatdPersist", function (e, chatRoom) {
      ...
      self
        .transaction(function () {
          self
            ._encryptedWhere("keys", { chatId: chatId })
          ...
          self
            ._encryptedWhere("msgs", { chatId: chatId })
            .then(function (results) {
              results.forEach(function (r) {
  ...
  ChatdPersist.DONT_ENCRYPT = {
    keys: [],
    ptrs: [],
    msgs: ["id", "msgId", "orderValue", "[chatId+msgId]", "[chatId+orderValue]"]
  };
  ...
  ChatdPersist.prototype._OpenDB = function () {
    ...
    return self.db.ptrs.toArray().then(function (r) {
  ...
  ChatdPersist.prototype._encryptedGet = async function (table, keyValue, keyName) {
    ...
    return this.db[table].get(v).then((result) => {
      if (!result) {
  ...
  ChatdPersist.prototype.addKey = function (chatId, keyId, userId, key) {
    ...
    return self._encryptedAdd("keys", {
      ...
      .then(function (found) {
        if (found && found.userId !== userId) {
          self.logger.error(
            ...
            chatId,
            keyId,
            "already exists, but its assigned to different user:",
            userId,
            "!=",
            found.userId,
            "Halting."
  ...
  ChatdPersist.prototype.getPointer = promisify(function (resolve, reject, chatId, pointerName, defaultValue) {
    ...
    self
      ._encryptedGet("ptrs", chatId + "_" + pointerName)
  ...
  [
    ...
    ChatdPersist.prototype[`getMessageBy${meta[0]}`] = async function (chatId, val) {
      if (!ChatdPersist.DONT_ENCRYPT.msgs.includes(meta[1]) && !ChatdPersist.DONT_ENCRYPT.msgs.includes(`[chatId+${meta[1]}]`)) {
   ...
   * @returns {Promise}
   */
  ChatdPersist.prototype.addMessage = function (chatId, msgId, userId, orderValue, keyId, msgObject) {
    ...
    return this._encryptedAdd("msgs", {
      chatId: chatId,
      msgId: msgId,
      userId: userId,
      orderValue: orderValue,
  ...
  ChatdPersist.prototype.persistMessage = function (chatId, msgObject, isEdit) {
    ...
    return this.getMessageByMessageId(chatId, messageId)
      .then((found) => {
        ...
        if (keyid !== 0 && base64urlencode(userId) !== mega.BID && found && found.userId !== userId) {
          this.logger.error(
        ...
        return this.db.msgs.delete(found.id).then(() => this.persistMessage(chatId, msgObject, isEdit));
      })
      .catch((ex) => {
  ...
  ChatdPersist.prototype.unpersistMessage = function (chatId, msgIdOrObj) {
    ...
    return this.getMessageByMessageId(chatId, msgId).then((r) => {
   ...
   * @returns {void}
   */
  ChatdPersist.prototype.persistMessageBatched = function (action, chatId, args) {
    var self = this;
    ...
    if (msgObject.keyid === true || msgObject.keyid === "true" || (msgObject.keyid & 0xffff0000) >>> 0 === 0xffff0000 >>> 0) {
  ...
  ChatdPersist.prototype._flushMessageActionsQueue = mutex("chatdpersist:mutex", function (resolve, reject, chatId) {
    ...
    (function next() {
      ...
      if (queueEntry[0] === "push") {
        self.persistMessage.apply(self, [chatId].concat(queueEntry[1])).catch(fail).finally(next);
      } else if (queueEntry[0] === "replace") {
        self.persistMessage
          .apply(self, [chatId].concat(queueEntry[1][1]).concat([true]))
          .catch(fail)
          .finally(next);
      } else if (queueEntry[0] === "remove") {
        if (queueEntry[1][1] !== true) {
          self.unpersistMessage.apply(self, [chatId].concat(queueEntry[1])).catch(fail).finally(next);
        } else {
  ...
  ChatdPersist.prototype._queryMessages = function (startFromOrderValue, limit, chatId) {
    ...
    var q = self.db.msgs.where("[chatId+orderValue]").between([encChatId, ordValueMin], [encChatId, ordValueMax]).reverse().limit(limit);
  ...
  ChatdPersist.prototype._paginateMessages = promisify(function (resolve, reject, offset, pageSize, chatId) {
    ...
    self
      ._queryMessages(offset, pageSize, chatId)
      .toArray()
      .then(function (results) {
        ...
        self
          .getKeys(Object.keys(keysThatRequireLoading))
          .then(function (results) {
  ...
  ChatdPersist.prototype.retrieveChatHistory = promisify(function (resolve, reject, chatId, len, highNum, lowNum) {
    ...
    waitingPromises.push(
      self
        ._paginateMessages(lastRetrOrdValue, parseInt(len, 10), chatId)
    ...
    Promise.allSettled(waitingPromises).always(function () {
      if (messages && messages.length > 0) {
        resolve([messages, keys, lownum, highnum, lastSeen]);
      } else {
  ...
  ChatdPersist.prototype.getLowHighIds = function (chatId) {
    ...
    return new MegaPromise(function (resolve, reject) {
      ...
      Promise.all([
        self.db.msgs.where("[chatId+orderValue]").between([encChatId, -Infinity], [encChatId, Infinity]).last(),
        self.db.msgs.where("[chatId+orderValue]").between([encChatId, -Infinity], [encChatId, Infinity]).first()
      ])
        .then(function (result) {
          var low = result[1];
          var high = result[0];
  ...
  ChatdPersist.prototype.persistQueue = mutex("chatdpersist:mutex", function (resolve, reject, chatId) {
    ...
    self
      .transaction(function (msgs, keys, trans) {
        var promises = [];
  ...
  ChatdPersist.prototype.persistTruncate = promisify(function (resolve, reject, chatId, firstNumToStartFrom) {
    ...
    self
      .transaction(function () {
        return self._queryMessages(firstNumToStartFrom, Infinity, chatId).eachPrimaryKey(function (pk) {
          self.db.msgs
            .delete(pk)
            .then(function () {
              stats.delmsgs++;
  ...
  ChatdPersist.prototype.clearChatHistoryForChat = mutex("chatdpersist:mutex", function (resolve, reject, chatId) {
    ...
    self
      .transaction(function () {
        return self._queryMessages(0, Infinity, chatId).eachPrimaryKey(function (pk) {
  ...
  ChatdPersist.prototype.retrieveAndLoadKeysFor = promisify(function (resolve, reject, chatId, userId, keyId) {
    ...
    self.chatd.chatdPersist
      ...
      .catch((ex) => {
        if (d) {
          console.error("Failed to retrieve key for retrieveAndLoadKeysFor", [chatId, userId, keyId], ex);
        }
        reject(ex);
      });
  });
  ...
  ChatdPersist.prototype.modifyPersistedMessage = function (chatId, messageInfo) {
    ...
    self
      .getMessageByMessageId(chatId, messageInfo.messageId)
      .then(function (r) {
        var msg = r[0];
        if (!msg) {
        ...
        return chatRoom.protocolHandler.decryptFrom(messageInfo.message, messageInfo.userId, messageInfo.keyid, false);
      })
      .then(function (r) {
        var msgInstance = new Message(chatRoom, self, {
          messageId: msg.msgId,
          userId: msg.userId,
          keyid: msg.keyId,
      ...
      this.getMessageByMessageId(chatId, message.messageId)
        .then((r) => {
          if (r && r[0]) {
            ...
            this.persistMessageBatched("replace", chatId, [undefined, Message.fromPersistableObject(megaChat.getChatById(chatId), r[0])]);
            return r[0].msgId;
          }
        })
        .then(resolve)
  ...
  ChatdPersist.prototype.clear = function () {
    return Promise.allSettled([this.db.msgs.clear(), this.db.keys.clear(), this.db.ptrs.clear()]);
  };
  ...
  ChatdPersist.prototype.debugValidate = function () {
    megaChat.chats.forEach(function (chatRoom) {
      ...
      megaChat.plugins.chatdIntegration.chatd.chatdPersist
        ._paginateMessages(0, 99999, chatRoom.chatId)
        .then(function (r) {
          ...
          msgs.forEach(function (v) {
            if (orderValues[v.orderValue]) {
              ...
              console.error("Found dup orderValue: ", v.msgId, v.orderValue, "already found as:", orderValues[v.orderValue]);
            }
            if (prevOrderValue !== false && v.orderValue + 1 !== prevOrderValue) {
              foundErrors = foundErrors === false ? 1 : foundErrors + 1;
              console.error(
                "Consistency error in buffer: ",
                v.msgId,
                "expected orderValue of",
                v.orderValue + 1,
                "got",
                v.orderValue,
            ...
            if (prevMsg !== false && v.msgObject.delay > prevMsg.msgObject.delay) {
              foundErrors = foundErrors === false ? 1 : foundErrors + 1;
              console.error("Consistency error in buffer: ", v.msgId, "expected delay of", prevMsg.msgObject.delay, "<", v.msgObject.delay);
            }
            prevMsg = v;
          ...
          if (foundErrors !== false) {
            console.error("Found", foundErrors, "errors in", chatRoom.chatId, "in total of", msgs.length, "messages");
          } else {
            console.error("Validation success, everything is ok for chat", chatRoom.chatId, "checked", msgs.length, "messages");
          }
        })
        .catch(dump);
    });
