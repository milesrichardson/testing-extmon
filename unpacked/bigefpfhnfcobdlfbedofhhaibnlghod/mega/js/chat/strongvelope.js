...
(function () {
  ...
  var _KEYED_MESSAGES = [MESSAGE_TYPES.GROUP_KEYED, MESSAGE_TYPES.ALTER_PARTICIPANTS];
  ...
  strongvelope._symmetricEncryptMessage = function (message, key, nonce) {
    ...
    var nonceBytes = asmCrypto.string_to_bytes(strongvelope.deriveNonceSecret(nonce).substring(0, NONCE_SIZE));
  ...
  strongvelope.deriveSharedKey = function (sharedSecret, context) {
    ...
    context = typeof context === "undefined" ? "strongvelope pairwise key" : context;
    var hmac = asmCrypto.HMAC_SHA256.bytes;

    return asmCrypto.bytes_to_string(hmac(context + "\x01", hmac(sharedSecret, "")));
  };
  ...
  strongvelope.deriveNonceSecret = function (masterNonce, recipient) {
    // Equivalent to first block of HKDF, see RFC 5869.
    recipient = typeof recipient === "undefined" ? "payload" : base64urldecode(recipient);

    return asmCrypto.bytes_to_string(asmCrypto.HMAC_SHA256.bytes(recipient, masterNonce));
  };
  ...
  strongvelope._parseMessageContent = function (binaryMessage) {
    ...
    try {
      ...
      while (rest.length > 0) {
        part =
          parsedContent.protocolVersion > PROTOCOL_VERSION_V1 ? tlvstore.splitSingleTlvElement(rest) : tlvstore.splitSingleTlvRecord(rest);
        if (part === false) {
        ...
        switch (tlvType) {
          ...
          case TLV_TYPES.KEY_BLOB:
            ...
            while (pos < value.length) {
              ...
              parsedContent[_TLV_MAPPING[TLV_TYPES.RECIPIENT]].push(base64urlencode(value.substring(pos, pos + USER_HANDLE_SIZE)));
   ...
   */
  strongvelope.ProtocolHandler = function (ownHandle, myPrivCu25519, myPrivEd25519, myPubEd25519, chatMode, chatUnifiedKey) {
    this.keyId = null;
  ...
  strongvelope.ProtocolHandler.prototype.reinitWithNewData = function (ownHandle, myPrivCu25519, myPrivEd25519, myPubEd25519, chatMode) {
        ...
        verifyPromise
          .always(function () {
            ...
            if (keyIndex >= 0) {
              ...
              var encryptedKey =
                isOwnMessage && parsedMessage.keys[keyIndex].length >= strongvelope._RSA_ENCRYPTION_THRESHOLD
                  ? parsedMessage.ownKey
                  : parsedMessage.keys[keyIndex];
              ...
              crypt
                .getAllPubKeys(otherHandle)
                .then(function () {
                  var decryptedKeys = self._legacyDecryptKeysFor(encryptedKey, parsedMessage.nonce, otherHandle, isOwnMessage);
  ...
  strongvelope.ProtocolHandler.prototype._batchParseAndExtractKeys = function (messages) {
    ...
    MegaPromise.allDone(proxyPromises).always(function () {
  ...
  strongvelope.ProtocolHandler.prototype._computeSymmetricKey = function (userhandle) {
    ...
    if (!DERIVED_SHARED_KEYS[userhandle]) {
      var sharedSecret = nacl.scalarMult(asmCrypto.string_to_bytes(this.myPrivCu25519), asmCrypto.string_to_bytes(pubKey));
  ...
  strongvelope.ProtocolHandler.prototype._encryptKeysTo = function (keys, destination) {
    ...
    var keyBytes = asmCrypto.string_to_bytes(this._computeSymmetricKey(destination).substring(0, KEY_SIZE));
    var cipherBytes = asmCrypto.AES_ECB.encrypt(clearBytes, keyBytes, false);
   ...
   */
  strongvelope.ProtocolHandler.prototype._legacyDecryptKeysFor = function (encryptedKeys, nonce, otherParty, iAmSender) {
    var clear = "";
    ...
    if (encryptedKeys.length < strongvelope._RSA_ENCRYPTION_THRESHOLD) {
      ...
      var ivBytes = asmCrypto.string_to_bytes(strongvelope.deriveNonceSecret(nonce, receiver).substring(0, IV_SIZE));
      var keyBytes = asmCrypto.string_to_bytes(this._computeSymmetricKey(otherParty).substring(0, KEY_SIZE));
      var clearBytes = asmCrypto.AES_CBC.decrypt(cipherBytes, keyBytes, false, ivBytes);
      clear = asmCrypto.bytes_to_string(clearBytes);
    } else {
      throw new SecurityError("Got RSA encrypted sender keys.");
    }

    assert(clear.length % KEY_SIZE === 0, "Length mismatch for decoding sender keys.");
   ...
   *     All symmetric keys in an array.
   */
  strongvelope.ProtocolHandler.prototype._decryptKeysFrom = function (encryptedKeys, otherParty) {
    var clear = "";
    ...
    if (encryptedKeys.length < strongvelope._RSA_ENCRYPTION_THRESHOLD) {
      ...
      var cipherBytes = asmCrypto.string_to_bytes(encryptedKeys);
      var keyBytes = asmCrypto.string_to_bytes(this._computeSymmetricKey(otherParty).substring(0, KEY_SIZE));
      var clearBytes = asmCrypto.AES_ECB.decrypt(cipherBytes, keyBytes, false);
      clear = asmCrypto.bytes_to_string(clearBytes);
    } else {
      throw new SecurityError("Got RSA encrypted sender keys.");
    }
   ...
   */
  strongvelope.ProtocolHandler.prototype._assembleBody = function (
    message,
    type,
    senderKey,
    trackedParticipants,
    ...
    var encryptedMessage = ns._symmetricEncryptMessage(messageIdentity + ns.pack16le(refids.length) + refids + to8(message), senderKey);
    // Assemble message content.
    if (encryptedMessage.nonce !== null && encryptedMessage.nonce.length > 0) {
      content += tlvstore.toTlvElement(String.fromCharCode(TLV_TYPES.NONCE), encryptedMessage.nonce);
    }
    ...
    if (encryptedMessage.ciphertext !== null && encryptedMessage.ciphertext.length > 0) {
      content += tlvstore.toTlvElement(String.fromCharCode(TLV_TYPES.PAYLOAD), encryptedMessage.ciphertext);
    }
    if (withKeyIncluded) {
      content += tlvstore.toTlvElement(String.fromCharCode(TLV_TYPES.KEY_BLOB), this.getKeyBlob(senderKey, trackedParticipants));
    }
    content += tlvstore.toTlvElement(String.fromCharCode(TLV_TYPES.INVITOR), base64urldecode(this.ownHandle));
    ...
    // Sign message.
    content = this._signContent(String.fromCharCode(PROTOCOL_VERSION) + String.fromCharCode(type) + senderKey + content) + content;
  ...
  strongvelope.ProtocolHandler.prototype._signContent = function (content) {
    var signature = ns._signMessage(content, this.myPrivEd25519, this.myPubEd25519);
    var signatureRecord = tlvstore.toTlvElement(String.fromCharCode(TLV_TYPES.SIGNATURE), signature);
   ...
   */
  strongvelope.ProtocolHandler.prototype.embeddedEncryptTo = function (message, type, participants, forceUpdate, openMode) {
    // this is a fixed id.
  ...
  strongvelope.ProtocolHandler.prototype.refreshKey = function (refs) {
    ...
    encryptedMessages.push({
      ...
      message: this.getKeyBlob(this.participantKeys[this.ownHandle][this.keyId], trackedParticipants)
    });
  ...
  strongvelope.ProtocolHandler.prototype.encryptTo = function (message, refs) {
    ...
    if (this.chatMode !== CHAT_MODE.PUBLIC) {
      ...
      if (this.keyId === null || this.participantChange === true) {
        var newSenderKey = this.updateSenderKey();
        this.keyId = newSenderKey.keyId;
        this.participantKeys[this.ownHandle][this.keyId] = newSenderKey.senderKey;
    ...
    assembledMessage = this._assembleBody(message, MESSAGE_TYPES.GROUP_FOLLOWUP, senderKey, trackedParticipants, refs, messageIdentity);
   ...
   * @private
   */
  strongvelope.ProtocolHandler.prototype._getSenderKeyAndUpdateCache = function (sender, parsedMessage, senderKeys) {
    var storedKey;
    ...
    if (parsedMessage.keys.length > 0 && parsedMessage.recipients.length > 0) {
      ...
      if (this.participantKeys[sender] && this.participantKeys[sender][keyId]) {
        ...
        if (this._logRsaAwareCritical("Encryption key for message from *** with ID *** unavailable.")) {
          this.logger.error(`Encryption key for message from ${sender} with ID ${base64urlencode(keyId)} unavailable.`);
        }
    ...
    if (parsedMessage) {
      switch (parsedMessage.type) {
        ...
        case MESSAGE_TYPES.OPEN_MODE_CLOSED:
          // Sanity checks.
          if (setutils.intersection(new Set(parsedMessage.includeParticipants), new Set(parsedMessage.excludeParticipants)).size > 0) {
   ...
   */
  strongvelope.ProtocolHandler.prototype.legacyDecryptFrom = function (message, sender, historicMessage) {
    ...
    extractContentPromise.done(function (extractedContent) {
      ...
      // Get sender key.
      var senderKey = self._getSenderKeyAndUpdateCache(sender, parsedMessage, senderKeys);
      ...
      } else if (parsedMessage.recipients.length > 0 && parsedMessage.recipients.indexOf(self.ownHandle) === -1) {
  ...
  strongvelope.ProtocolHandler.prototype.decryptMessage = function (parsedMessage, sender, keyId) {
    var self = this;
      ...
      if (parsedMessage.keys.length > 0) {
        ...
        if (myIndex >= 0) {
          if (parsedMessage.keys && parsedMessage.keys.length === parsedMessage.recipients.length) {
            ...
            var decryptedKeys = this._decryptKeysFrom(encryptedKey, sender, isOwnKey);
            if (decryptedKeys) {
    ...
    if (parsedMessage) {
      ...
      var verifyPromise =
        self.chatMode === CHAT_MODE.PUBLIC
          ? MegaPromise.resolve(true)
          : ns._verifyMessage(
              String.fromCharCode(parsedMessage.protocolVersion) +
                String.fromCharCode(parsedMessage.type) +
                senderKey +
                parsedMessage.signedContent,
              parsedMessage.signature,
              pubEd25519[sender],
   ...
   */
  strongvelope.ProtocolHandler.prototype.decryptFrom = function (message, sender, keyId, historicMessage) {
    // jshint maxcomplexity: 11
  ...
  strongvelope.ProtocolHandler.prototype.batchDecrypt = function (messages, historicMessages) {
    ...
    messages.forEach(function (message, i) {
      message = messages[i];
      var decryptPromise = self.decryptFrom(message.message, message.userId, message.keyid, historicMessages);
    ...
    var proxyPromise = new MegaPromise();
    MegaPromise.allDone(waitingPromises).always(function () {
