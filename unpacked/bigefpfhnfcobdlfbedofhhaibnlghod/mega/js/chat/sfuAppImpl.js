(() => {
  ...
  function loadPubCu25519ForPeer(peerHandle) {
    return crypt.getPubCu25519(peerHandle).then(() => {
  ...
  function loadPubEd25519ForPeer(peerHandle) {
    return crypt.getPubEd25519(peerHandle).then(() => {
  ...
  async function webrtcCalculateSharedKey(userId) {
    ...
    var sharedSecret = nacl.scalarMult(asmCrypto.string_to_bytes(u_privCu25519), asmCrypto.string_to_bytes(pubKey));
    ...
    return asmCrypto.string_to_bytes(asmCrypto.bytes_to_string(hmac("webrtc pairwise key\x01", hmac(sharedSecret, ""))).substring(0, 16));
  }
  Call.prototype.signString = async function (str) {
    var detachedSignature = nacl.sign.detached(asmCrypto.string_to_bytes(str), asmCrypto.string_to_bytes(u_privEd25519 + u_pubEd25519));
  ...
  Call.prototype.verifySignature = async function (msg, signature, userId) {
    ...
    return backgroundNacl.sign.detached.verify(
      asmCrypto.string_to_bytes(msg),
      SfuClient.base64DecodeToArr(signature),
  ...
  Call.prototype.encryptKeyToUser = async function (key, userId) {
    ...
    return base64urlencode(asmCrypto.bytes_to_string(asmCrypto.AES_ECB.encrypt(key, userSharedKey, false)));
  };
  /* Used for v0 clients, to be removed soon */
  ...
  Call.rxStatsToText = function (track, info, raw) {
    ...
    text += `${raw.framesPerSecond || 0}fps ${Math.round(info.keyfps)}kfs ${Math.round(info.kbps)}kbps rtt: ${
      sfuClient.rtcStats.rtt
    }, pl: ${Math.round(info.plost)}, rxq: ${sfuClient.rxQuality}`;
    return text;
  };
    ...
    Call.prototype.onVideoTxStat = function (isHiRes, stats, raw) {
      ...
      let text =
        isHiRes === null
          ...
          : `loc: ${stats.vtxw}x${stats.vtxh}:${sfuClient.sentTracksString()} ${Math.round(stats._vtxkbps)}kbps\n${
              stats.vtxfps || 0
            }fps ${Math.round(stats._vtxkfps)}kfs rtt: ${stats.rtt} dly: ${stats.vtxdly}`;
