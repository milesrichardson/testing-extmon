...
var Chatd = function (userId, megaChat, options) {
  ...
  self.identity =
    Chatd.pack32le((Math.random() * 0xffffffff) | 0) + Chatd.pack16le((Math.random() * 0xffff) | 0) + Chatd.pack16le(Date.now() & 0xffff);
  ...
  [
    ...
    self.rebind(evt + ".chatd", function (e) {
      if (arguments[1].shard) {
        ...
        console.error(evt, unixtime(), JSON.stringify(tmp));
      } else {
...
Chatd.LOGGER_LEVEL = Chatd.LOGGER_ENABLED ? MegaLogger.LEVELS.DEBUG : d > 1 ? MegaLogger.LEVELS.WARN : MegaLogger.LEVELS.ERROR;
  ...
  self.connectionRetryManager = new ConnectionRetryManager(
    {
      functions: {
        reconnect: function (connectionRetryManager) {
          ...
          self
            .retrieveMcurlAndExecOnce(base64urlencode(Object.keys(self.chatIds)[0]))
            .then(function (mcurl) {
        ...
        isConnectedOrConnecting: function () {
          return self.s && (self.s.readyState === self.s.CONNECTING || self.s.readyState === self.s.OPEN);
        },
        /**
        ...
        isConnected: function () {
          return self.s && self.s.readyState === self.s.OPEN;
        },
        /**
        ...
        isDisconnected: function () {
          return !self.s || self.s.readyState === self.s.CLOSED;
        },
        /**
        ...
        isUserForcedDisconnect: function () {
          return self.chatd.destroyed === true || self.destroyed === true;
        }
      }
    },
    self.logger
...
Chatd.Shard.prototype.markAsJoinRequested = function (chatId) {
  ...
  if (!this.userRequestedJoin[chatId]) {
    ...
    Reactions.getSn(base64urlencode(chatId))
      .then(function (sn) {
        if (sn) {
          self.cmd(Chatd.Opcode.REACTIONSN, chatId + base64urldecode(sn));
        }
      })
      .catch(function (ex) {
...
Chatd.Shard.prototype.retrieveMcurlAndExecOnce = async function (chatId) {
  ...
  return megaChat.plugins.chatdIntegration._retrieveShardUrl(publicChatHandle, chatHandleOrId).then((res) => {
...
Chatd.Shard.prototype.getRelatedChatIds = function () {
  ...
  Object.keys(this.chatIds).forEach(function (v) {
    chatIds.push(base64urlencode(v));
  });
  return chatIds;
};
...
Chatd.cmdToString = function (cmd, tx) {
  ...
  switch (opCode) {
    ...
    case Chatd.Opcode.NEWKEY:
      ...
      result += " chatid:" + base64urlencode(cmd.substr(1, 8)) + " keyId: " + keyId + " keyLen: " + keyLength;
    ...
    case Chatd.Opcode.JOIN:
      ...
      result += " chatId: " + chatId + " userId: " + userId + " priv: " + constStateToText(Chatd.Priv, priv) + "(" + priv + ")";
      return [result, 18];
      ...
      if (sender === strongvelope.COMMANDER) {
        ...
        switch (parsed.type) {
          case strongvelope.MESSAGE_TYPES.CALL_END: {
            ...
            result +=
              " callend-type: " +
              constStateToText(CallManager2.CALL_END_REMOTE_REASON, reason) +
              " dur: " +
              Chatd.unpack32le(parsed.payload.substr(9, 4));
            break;
          }
        }
      } else {
    ...
    case Chatd.Opcode.RETENTION:
      result +=
        ...
        base64urlencode(cmd.substr(9, 8)) +
        "': " +
        Chatd.unpack32le(cmd.substr(17, 2)) +
        " second(s)";
      return [result, 21];

    case Chatd.Opcode.NUMBYHANDLE:
      result += " num by handle in '" + base64urlencode(cmd.substr(1, 8)) + "' with " + Chatd.unpack32le(cmd.substr(9, 4)) + " user(s)";
    ...
    case Chatd.Opcode.NODEHIST:
      ...
      result +=
        " chatId: " + base64urlencode(cmd.substr(1, 8)) + " " + "msgId: " + base64urlencode(cmd.substr(9, 8)) + " " + "count: " + count;
      return [result, 21];
    ...
    case Chatd.Opcode.DELREACTION:
      ...
      result +=
        ...
        " " +
        "emoji: " +
        base64urlencode(emoji);

      return [result, 26 + leng];

    case Chatd.Opcode.REACTIONSN:
      // chatid.8  sn.8
      result += " chatId: " + base64urlencode(cmd.substr(1, 8)) + " " + "sn: " + base64urlencode(cmd.substr(9, 8));

      return [result, 17];

    case Chatd.Opcode.JOINEDCALL:
    case Chatd.Opcode.LEFTCALL:
      result += " chatId: " + base64urlencode(cmd.substr(1, 8)) + " callId: " + base64urlencode(cmd.substr(9, 8)) + " participants: ";
    ...
    case Chatd.Opcode.CALLEND:
      result += " chatId: " + base64urlencode(cmd.substr(1, 8)) + " callId: " + base64urlencode(cmd.substr(9, 8));
...
Chatd.Shard.prototype.sendKeepAlive = function (forced) {
  ...
  if ((forced || now - lastKeepAliveSent > Chatd.KEEPALIVE_PING_INTERVAL) && this.s && this.s.readyState === this.s.OPEN) {
...
Chatd.Shard.prototype.addReaction = function (chatId, msgId, userId, emoji) {
  ...
  this.cmd(Chatd.Opcode.ADDREACTION, chatId + userId + msgId + String.fromCharCode(len) + emoji);
};
...
Chatd.Shard.prototype.delReaction = function (chatId, msgId, userId, emoji) {
  ...
  this.cmd(Chatd.Opcode.DELREACTION, chatId + userId + msgId + String.fromCharCode(len) + emoji);
};
...
Chatd.Shard.prototype.hist = function (chatId, count, isInitial) {
  ...
  if (self.chatd.chatdPersist) {
    ...
    self.chatd.chatdPersist
      .retrieveChatHistory(base64urlencode(chatId), count * -1)
      .then(function (result) {
        ...
        if (chatRoom && chatIdMessagesObj) {
          ...
          if (isInitial) {
            ...
            self.chatd.chatdPersist
              .getLowHighIds(base64urlencode(chatId))
              .then(function (result) {
                var chatIdMessagesObj = self.chatd.chatIdMessages[chatId];
                ...
                self.markAsJoinRequested(chatId);
                self.chatd.cmd(Chatd.Opcode.JOINRANGEHIST, chatId, base64urldecode(result[0]) + base64urldecode(result[1]));
              })
              .catch(function (ex) {
                if (ex && d) {
                  self.logger.warn(ex);
  ...
  while (cmd.length) {
    ...
    switch (opcode) {
        ...
        if (self.loggerIsEnabled) {
          self.logger.log(
            "Message '" +
              ...
              base64urlencode(cmd.substr(9, 8)) +
              " with " +
              Chatd.dumpToHex(cmd, 39, len)
          );
        }
        len += 39;

        self.chatd.msgmodify(
          cmd.substr(1, 8),
          cmd.substr(9, 8),
          cmd.substr(17, 8),
          Chatd.unpack32le(cmd.substr(25, 4)),
      ...
      case Chatd.Opcode.CLIENTID:
        ...
        if (self.loggerIsEnabled) {
          self.logger.log("Assigned CLIENTID " + Chatd.clientIdToString(self.clientId), " for shard", self.shard);
        }
        break;
      case Chatd.Opcode.SEEN:
        self.keepAlive.restart();
        if (self.loggerIsEnabled) {
          self.logger.log(
            "Newest seen message on '" + base64urlencode(cmd.substr(1, 8)) + "': '" + base64urlencode(cmd.substr(9, 8)) + "'"
          );
        }
      ...
      case Chatd.Opcode.RECEIVED:
        ...
        if (self.loggerIsEnabled) {
          self.logger.log(
            "Newest delivered message on '" + base64urlencode(cmd.substr(1, 8)) + "': '" + base64urlencode(cmd.substr(9, 8)) + "'"
          );
        }
      ...
      case Chatd.Opcode.RETENTION:
        ...
        if (self.loggerIsEnabled) {
          self.logger.log(
            "Retention policy change on '" + handle + "' by '" + base64urlencode(cmd.substr(9, 8)) + "': " + retentionTime + " second(s)"
          );
        }
      ...
      case Chatd.Opcode.REJECT:
        ...
        } else if (rejectedOpCode === Chatd.Opcode.DELREACTION || rejectedOpCode === Chatd.Opcode.ADDREACTION) {
      ...
      case Chatd.Opcode.NEWKEY:
        ...
        if (self.loggerIsEnabled) {
          self.logger.log("Set keys: " + base64urlencode(cmd.substr(1, 8)) + " length: " + Chatd.unpack32le(cmd.substr(13, 4)));
        }
      ...
      case Chatd.Opcode.NUMBYHANDLE:
        ...
        if (self.loggerIsEnabled) {
          self.logger.log(
            "Num by handle in '" + base64urlencode(cmd.substr(1, 8)) + "' with " + Chatd.unpack32le(cmd.substr(9, 4)) + " user(s)"
          );
        }
      ...
      case Chatd.Opcode.MSGIDTIMESTAMP:
        ...
        self.chatd.msgreject(cmd.substr(1, 8), cmd.substr(9, 8), Chatd.unpack32le(cmd.substr(17, 4)));
      ...
      case Chatd.Opcode.NEWMSGIDTIMESTAMP:
        ...
        self.chatd.msgconfirm(cmd.substr(1, 8), cmd.substr(9, 8), Chatd.unpack32le(cmd.substr(17, 4)));
      ...
      case Chatd.Opcode.LEFTCALL:
        ...
        self.chatd.trigger(cmd.charCodeAt(0) === Chatd.Opcode.JOINEDCALL ? "onChatdPeerJoinedCall" : "onChatdPeerLeftCall", {
          chatId: base64urlencode(cmd.substr(1, 8)),
      ...
      case Chatd.Opcode.CALLSTATE:
        ...
        self.chatd.trigger("onCallState", {
          chatId: base64urlencode(cmd.substr(1, 8)),
          userId: base64urlencode(cmd.substr(9, 8)),
      ...
      case Chatd.Opcode.CALLEND:
        ...
        self.chatd.trigger("onChatdCallEnd", {
          chatId: base64urlencode(cmd.substr(1, 8)),
          callId: base64urlencode(cmd.substr(9, 8)),
    ...
    if (cmd.length < len) {
      self.logger.error(
        "FATAL: Short WebSocket frame - got " +
          cmd.length +
          ", expected " +
          len +
          ". To stop potential loop-forever case, the next commands in the buffer were rejected!"
      ...
      self.logger.error(
        "FATAL: Internally got nextlen == NaN, with queued cmdlen: " +
          cmd.length +
          ". " +
          "To stop potential loop-forever case, the next commands in the buffer were rejected!"
      );
...
Chatd.prototype.join = function (chatId, shardNo, url) {
  if (this.chatIdShard[chatId]) {
    // this.chatidMessages[chatid] may be still missing!
    return false;
  }
  // unknown chat
...
Chatd.Shard.prototype.msg = function (chatId, messages) {
  ...
  for (var i = 0; i < messages.length; i++) {
    ...
    var cmd = "";
    if (type === Chatd.MsgType.KEY) {
      // this is key message;
      cmd = [Chatd.Opcode.NEWKEY, chatId + Chatd.pack32le(keyid) + Chatd.pack32le(message.length) + message];
    } else if (type === Chatd.MsgType.EDIT) {
      // this is edit message;
      cmd = [
        Chatd.Opcode.MSGUPD,
        chatId +
          Chatd.Const.UNDEFINED +
          msgxid +
          Chatd.pack32le(0) +
          Chatd.pack16le(updated) +
          Chatd.pack32le(keyid) +
          Chatd.pack32le(message.length) +
          message
      ];
    } else {
      cmd = [
...
Chatd.Shard.prototype.msgupdx = function (chatId, msgxid, updatedelta, message, keyxid) {
  this.cmd(
    Chatd.Opcode.MSGUPDX,
    chatId +
      Chatd.Const.UNDEFINED +
      msgxid +
      Chatd.pack32le(0) +
      Chatd.pack16le(updatedelta) +
      Chatd.pack32le(keyxid) +
      Chatd.pack32le(message.length) +
      message
  );
};
...
Chatd.Messages.prototype._setLoginState = function (state) {
  ...
  if (
    oldState !== null &&
    state !== LoginState.DISCONN &&
    state !== null &&
    state !== oldState + 1 &&
    !(oldState === LoginState.HISTDONE && state === LoginState.JOIN_SENT)
  ) {
    console.error(
      "chat %s: Invalid login state transition from %s to %s",
      base64urlencode(this.chatId),
      constStateToText(LoginState, oldState),
      constStateToText(LoginState, state)
    );
  }
...
Chatd.Messages.prototype.join = function () {
  ...
  if (Object.keys(self.buf).length === 0 && (!chatRoom.messagesBuff || chatRoom.messagesBuff.messages.length === 0)) {
    ...
    if (chatRoom.type === "public" && !chatRoom.members.hasOwnProperty(base64urlencode(shard.chatd.userId)) && chatRoom.publicChatHandle) {
...
Chatd.Messages.prototype.submit = function (messages, keyId, isAttachment) {
  ...
  for (var i = 0; i < messages.length; i++) {
    ...
    this.sendingbuf[++Chatd.sendingnum] = [msgxid, this.chatd.userId, timestamp, message.message, keyId >>> 0, 0, message.type];
  ...
  if (this.chatd.chatIdShard[this.chatId].isOnline() && chatRoom && chatRoom.messagesBuff && chatRoom.messagesBuff.sendingListFlushed) {
...
Chatd.Messages.prototype.modify = function (msgnum, message) {
  ...
  if (self.sendingbuf[msgnum]) {
    ...
    if (self.chatd.chatIdShard[self.chatId].isOnline()) {
      ...
      self.chatd.chatIdShard[self.chatId].msgupdx(
        self.chatId,
        self.sendingbuf[msgnum][Chatd.MsgField.MSGID],
    ...
    if (self.chatd.chatIdShard[self.chatId].isOnline()) {
      self.chatd.chatIdShard[self.chatId].msgupd(
        self.chatId,
        self.buf[msgnum][Chatd.MsgField.MSGID],
        updated,
        message,
...
Chatd.Messages.prototype._joinrangehistViaMessagesBuff = function () {
  ...
  if (chatRoom && chatRoom.messagesBuff && chatRoom.messagesBuff.messages.length > 0) {
    ...
    if (firstLast) {
      ...
      self.chatd.cmd(Chatd.Opcode.JOINRANGEHIST, chatId, base64urldecode(firstLast[0]) + base64urldecode(firstLast[1]));
      return;
    }
  }
  ...
  self.chatd.cmd(Chatd.Opcode.JOIN, chatId, self.chatd.userId + String.fromCharCode(Chatd.Priv.NOCHANGE));
  self.chatd.trigger("onMessagesHistoryRequest", {
...
Chatd.Messages.prototype.handlejoinrangehist = function (chatId, handle) {
  ...
  for (low = this.lownum; low <= this.highnum; low++) {
    if (
      this.buf[low] &&
      !this.sending[this.buf[low][Chatd.MsgField.MSGID]] &&
      ...
      this.chatd.chatIdShard[chatId].cmd(
        Chatd.Opcode.OPCODE_HANDLEJOINRANGEHIST,
...
Chatd.prototype._reinitChatIdHistory = async function (chatId, resetNums) {
  ...
  if (oldChatIdMessages) {
    ["lownum", "highnum", "sending", "sentid", "receivedid", "seenid", "sendingbuf", "sending", "sendingList", "expired"].forEach(
      function (k) {
...
Chatd.Messages.prototype.store = function (newmsg, userId, msgid, timestamp, updated, keyid, msg, skipPersist, isSharedFile) {
  var id;
  ...
  // store message
  (isSharedFile ? this.sharedBuf : this.buf)[id] = [msgid, userId, timestamp, msg, keyid, updated, Chatd.MsgType.MESSAGE];
...
Chatd.Messages.prototype.msgmodify = function (userid, msgid, ts, updated, keyid, msg, isRetry) {
  ...
  if (this.chatd.chatdPersist) {
    if (!origMsgNum || !this.buf[origMsgNum]) {
      ...
      this.chatd.chatdPersist
        .getMessageByMessageId(encChatId, base64urlencode(msgid))
        .then(function (r) {
          if (!r) {
            console.error(
          ...
          if (msgObj.keyId !== 0) {
            self.chatd.chatdPersist.retrieveAndLoadKeysFor(encChatId, msgObj.userId, msgObj.keyId).catch(dump).finally(done);
          } else {
            done();
          }
        })
        .catch((ex) => {
          if (d) {
            console.warn(
              "msgmodify failed, can't find",
              base64urlencode(msgid),
              " in chatdPersist, maybe the " + "message was not yet retrieved?",
              ex
            );
          }
        });
  ...
  for (var i = this.highnum; i >= this.lownum; i--) {
    if (this.buf[i] && this.buf[i][Chatd.MsgField.MSGID] === msgid) {
      ...
      if (keyid === 0 && base64urlencode(userid) === strongvelope.COMMANDER) {
        ...
        if (
          this.sending[messagekey] &&
          this.sendingbuf[this.sending[messagekey]] &&
  ...
  if (keyid === 0 && base64urlencode(userid) === strongvelope.COMMANDER) {
    ...
    if (this.chatd.chatdPersist) {
      ...
      this.chatd.chatdPersist.persistTruncate(chatId, origMsgNum).always(function () {
...
Chatd.Messages.prototype.confirmkey = function (keyid) {
  ...
  promises.push(
    self.chatd.messagesQueueKvStorage.getItem(cacheKey).then((v) => {
      if (v) {
...
Chatd.Messages.prototype.restore = function () {
    ...
    self.chatd.messagesQueueKvStorage.eachPrefixItem(prefix, function (v) {
      if (v.userId === self.chatd.userId) {
        ...
        if (v.type !== Chatd.MsgType.KEY || (v.type === Chatd.MsgType.KEY && pendingkey)) {
          ...
          if (!self.sending[messagekey]) {
            self.sendingbuf[++Chatd.sendingnum] = [v.messageId, v.userId, v.timestamp, v.message, v.keyId, v.updated, v.type];
            self.sending[messagekey] = Chatd.sendingnum;
            self.sendingList.push(messagekey);
            count++;
          }
  ...
  Promise.allSettled(promises).then(() => {
    if (d > 2) {
      console.debug(`Chatd.Messages.restore(${prefix}):`, count, iskey, trivialkeys.length, Object(self.sendingList).length);
    }
    if (iskey) {
      trivialkeys.push(self.getmessagekey(previouskeyid, Chatd.MsgType.KEY));
    }
