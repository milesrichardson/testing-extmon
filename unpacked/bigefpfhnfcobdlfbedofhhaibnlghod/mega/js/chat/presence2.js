  ...
  if (self.url) {
    ...
    self.s.onopen = function () {
      if (!self.canceled) {
        ...
        self.sendstring(String.fromCharCode(1) + String.fromCharCode(1) + String.fromCharCode(self.capabilities));
    ...
    self.s.onmessage = function (m) {
      if (!this.canceled) {
        if (PRESENCE2_DEBUG) {
          console.error(new Date().toISOString(), "PRES recv:", UserPresence.commandDebugDataAsPrettyString(self.commandsToString(m.data)));
        }
          ...
          switch (op) {
            ...
            case 6: // OPCODE_PEERSTATUS
              ...
              if (self.peerstatuscb) {
                self.peerstatuscb(user, presence, isWebrtcFlag);
              }
              ...
              if (presence !== UserPresence.PRESENCE.ONLINE) {
                // if not green, query last seen time
...
UserPresence.prototype.sendstring = function presence_sendstring(s) {
  ...
  if (PRESENCE2_DEBUG) {
    console.error(
      ...
      "PRES send:",
      UserPresence.commandDebugDataAsPrettyString(this.commandsToString(u.buffer, true))
    );
  }
...
UserPresence.prototype.sendprefs = function presence_sendprefs() {
  ...
  if (this.open) {
    ...
    this.sendstring(String.fromCharCode(7) + String.fromCharCode(prefs & 0xff) + String.fromCharCode(prefs >> 8));
  }
};
...
UserPresence.prototype.ui_signalactivity = function presence_ui_signalactivity(force) {
  var timeout = !this.persist && this.presence == UserPresence.PRESENCE.ONLINE && this.autoawaytimeout && this.autoawayactive;
  ...
  if (timeout && !this.autoawaytimer) {
    ...
    (this.autoawaytimer = tSleep(this.autoawaytimeout)).then(() => {
...
UserPresence.prototype.commandsToString = function (ab, tx) {
  ...
  while (p < u.length) {
    ...
    switch (op) {
      ...
      case 3: // FLAGS
        output.push(["FLAGS", "online = " + !!(u[p + 1] & 1), "dnd = " + !!(u[p + 1] & 2)]);
      ...
      case 5: // DELPEERS
        ...
        var numpeers = u[p + 1] + (u[p + 2] << 8) + (u[p + 3] << 16) + (u[p + 4] << 24);
        assert(!isNaN(numpeers));
        var peers = [];
        for (var i = 0; i < numpeers; i++) {
          peers.push(ab_to_base64(u.subarray(p + 5 + i * 8, p + 5 + (i + 1) * 8)));
        }
        if (sn != null) {
          line.push("sn = " + sn);
        }
      ...
      case 7: // PREFS
        ...
        if (!tx && newprefs === this.prefs()) {
          output.push(["PREFS", "prefs unchanged"]);
          break;
        }
