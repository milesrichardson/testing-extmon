(function (scope) {
  ...
  ChatStats.prototype.initDataCollectors = function () {
    ...
    self.unwrapOnDone.push(
      self.wrapFunction("Chatd.Shard.prototype.reconnect", false, function () {
    ...
    self.unwrapOnDone.push(
      self.wrapFunction("Chatd.Shard.prototype.hist", false, function (args) {
    ...
    self.unwrapOnDone.push(
      self.wrapFunction("Chatd.Messages.prototype.store", false, function (args) {
        var newmsg = args[0];
    ...
    chatd.rebind("onMessagesHistoryDone.chatStats", function (e, data) {
      ...
      if (stats) {
        ...
        if (startTime && messagesCount > 0) {
          self.data["chr#" + data.chatId] = [messagesCount, microtime() - startTime];
        }
      }
    });

    self.eventsForUnbinding.push([chatd, "onMessagesHistoryDone.chatStats"]);
    ...
    self.megaChat.rebind("onRoomInitialized.chatStats", function (e, chatRoom) {
      ...
      self.eventsForUnbinding.push([chatRoom, "onChatdIntegrationReady.chatStats"]);
      ...
      chatRoom.rebind("onHistoryDecrypted.chatStats", function () {
        ...
        if (startTime && messagesCount > 0) {
          self.data["hd#" + base64urlencode(chatId)] = [messagesCount, microtime() - startTime];
        }
      });
      self.eventsForUnbinding.push([chatRoom, "onHistoryDecrypted.chatStats"]);
      ...
      self.eventsForUnbinding.push([chatRoom, "onHistoryDone.chatStats"]);

      chatRoom.rebind("onHistoryDecryptedDone.chatStats", function () {
        self.sendIfChatsReady();
      });

      self.eventsForUnbinding.push([chatRoom, "onHistoryDecryptedDone.chatStats"]);
    });
    self.eventsForUnbinding.push([self.megaChat, "onRoomInitialized.chatStats"]);
  };
