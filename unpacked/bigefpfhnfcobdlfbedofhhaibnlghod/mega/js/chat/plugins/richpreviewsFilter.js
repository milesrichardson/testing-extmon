...
var RichpreviewsFilter = function (megaChat) {
  ...
  megaChat.rebind("onRoomInitialized.richpreviewsFilter", function (e, megaRoom) {
    ...
    megaRoom.rebind("onMessageUpdateDecrypted.richpreviewsFilter", function (e, msgObj) {
      [msgObj.chatRoom.roomId + "_" + msgObj.messageId, msgObj.chatRoom.roomId + "_" + msgObj.pendingMessageId].forEach(function (k) {
...
RichpreviewsFilter.prototype.processMessage = function (e, eventData, forced, isEdit) {
  ...
  if (isEdit) {
    textContents = megaChat.plugins.btRtfFilter.escapeAndProcessMessage(e, eventData, ["messageHtml", "textContents"], "messageHtml", true);
  ...
  var key = eventData.chatRoom.roomId + "_" + (eventData.messageId ? eventData.messageId : eventData.pendingMessageId);
  ...
  if (haveLinks && (forced || RichpreviewsFilter.previewGenerationConfirmation === true)) {
    ...
    if (
      eventData.getState &&
      (eventData.getState() === Message.STATE.SENT || (eventData.getState() === Message.STATE.DELIVERED && eventData.source === undefined))
    ) {
...
RichpreviewsFilter._updateMessageToPreview = function (chatRoom, msgObj, responses, isRetry) {
  ...
  var keys = [chatRoom.roomId + "_" + msgObj.pendingMessageId, chatRoom.roomId + "_" + msgObj.messageId];
  ...
  if (foundValidResponses && meta.extra.length > 0) {
    var messageContents =
      ...
      Message.MESSAGE_META_TYPE.RICH_PREVIEW +
      JSON.stringify(meta);

    chatRoom.megaChat.plugins.chatdIntegration.updateMessage(chatRoom, msgObj.orderValue, messageContents);
  } else {
    msgObj.meta = {};
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (RichpreviewsFilter._waitConfirm[key]) {
      ...
      for (var x = 0; x < urls.length; x++) {
        retrievePromises.push(RichpreviewsFilter.retrievePreview(urls[x]).then(urlLoaded).catch(urlFailedToLoad.bind(null, urls[x])));
...
RichpreviewsFilter.prototype.revertToText = function (chatRoom, msgObj) {
  ...
  if (msgObj.meta && !msgObj.meta.isLoading && msgObj.isEditable()) {
    ...
    if (textMessage) {
      chatRoom.megaChat.plugins.chatdIntegration.updateMessage(chatRoom, msgObj.orderValue, textMessage);
    }
  }
};
