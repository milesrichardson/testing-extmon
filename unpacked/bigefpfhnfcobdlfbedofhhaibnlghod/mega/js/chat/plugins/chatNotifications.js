  ...
  megaChat
      ...
      megaRoom
        ...
        .rebind("onMessagesBuffAppend.chatNotifications", async (e, message) => {
          ...
          if (message.userId !== u_handle) {
            ...
            var unreadFlag = message.getState() === Message.STATE.NOT_SEEN && !mega.active;

            if (pushNotificationSettings.isAllowedForChatId(megaRoom.chatId) && message.source === Message.SOURCE.CHATD) {
    ...
    .rebind("onLocalMediaError.chatNotifications", (e, errObj) => {
      if (errObj && errObj.screen) {
        self.notifications.notify("screen-share-error", { anfFlag: "chat_enabled" }, true);
      }
    })
