...
class ChatToastIntegration {
      ...
      .rebind("onRoomInitialized.cTI", (e, megaRoom) => {
        ...
        megaRoom
          .rebind("onCallPeerJoined.cTI", (e, userHandle) => {
            ...
            this.batchNotifications("onCallPeerJoined", this.getTrimmedName(name), {
              close: true,
              cb: () => {
          ...
          .rebind("onCallPeerLeft.cTI", (e, { userHandle }) => {
            if (navigator.onLine) {
              ...
              this.batchNotifications("onCallPeerLeft", this.getTrimmedName(name), {
                close: true,
                cb: () => {
          ...
          .rebind("onCallPrivilegeChange", (e, userHandle, privilege) => {
            const name = nicknames.getNickname(userHandle);
            const role =
              privilege === ChatRoom.MembersSet.PRIVILEGE_STATE.OPERATOR ? l.chat_user_role_change_op : l.chat_user_role_change_std;
          ...
          .rebind("onNoMicInput.cTI", () => {
            if (megaRoom.call) {
              ChatToast.quick(l.chat_mic_off_toast /* Your mic is not working */, "sprite-fm-mono icon-audio-off");
            }
          })
          .rebind("onBadNetwork.cTI", (e) => {
            if (e.data) {
              ...
              this.poorConnToast = new ChatToast(l.poor_connection, {
                persistent: true,
          ...
          .rebind("onRetryTimeout.cTI", () => {
            ...
            const retryFailToast = new ChatToast(l.reconnect_failed /* `Unable to reconnect` */, {
              icon: "sprite-fm-uni icon-hazard",
              close: true,
              timeout: 5000
            });
  ...
  sfuConnClose() {
    if (!this.reconnectingSfu) {
      this.reconnectingSfu = new ChatToast(l.reconnecting /* `Reconnecting...` */, {
        icon: "sprite-fm-uni icon-hazard",
        persistent: true,
        onEnd: () => {
          if (this.reconnectingSfu) {
...
class BatchChatToast extends ChatToast {
  ...
  constructor(id, content = [], options = {}) {
    ...
    this.setJoiner(
      options.joiner && typeof options.joiner === "function"
        ? options.joiner
        : (arr) => {
...
ChatToastIntegration.prototype.batchNotifications = (function () {
  ...
  return function (batchId, content, { joiner, icon, cb, timeout } = {}) {
    ...
    BatchChatToast.BATCHES[batchId].addContent(content).setJoiner(joiner).initDispatcher().setTimeout(timeout).setIcon(icon).setOnShown(cb);
  };
})();
