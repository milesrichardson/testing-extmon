...
var BacktickRtfFilter = function (megaChat) {
  ...
  self.regexps = {};
  self.regexps["(^|\\s)`{3}(\\n?)(\\n|.*)`{3}"] = ["gis", '$1<pre class="rtf-multi">$2$3</pre>', "$1 $2$3"];

  self.regexps["(^|\\s)`{1}([^\\n|^`]{1,})`{1}"] = ["gim", '$1<pre class="rtf-single">$2</pre>', "$1 $2"];
  self.escaped = {};
  self.index = 0;
...
BacktickRtfFilter.prototype.processBackticks = function (msgString, replaceGenCb) {
  ...
  for (var i = 0; i < len; i++) {
    if (
      msgString.substr(i, 3) === "```" &&
      (state === BacktickRtfFilter.PARSER_STATE.REGULAR || state === BacktickRtfFilter.PARSER_STATE.IN_PLACEHOLDER_MULTI) &&
      ...
      msgString[i] === "`" &&
      (state === BacktickRtfFilter.PARSER_STATE.REGULAR || state === BacktickRtfFilter.PARSER_STATE.IN_PLACEHOLDER_SINGLE) &&
    ...
    } else if (state === BacktickRtfFilter.PARSER_STATE.IN_PLACEHOLDER_SINGLE && (msgString[i] === "\n" || msgString[i] === "\r")) {
...
BacktickRtfFilter.prototype.unescapeAndProcessMessage = function (e, eventData, props, mainProp) {
  ...
  props.forEach(function (prop) {
    ...
    messageContents = messageContents.replace(new RegExp(BacktickRtfFilter.UNESCAPE_REGEXP, "gi"), function (match, id) {
