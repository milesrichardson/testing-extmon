...
var EmoticonShortcutsFilter = function (megaChat) {
  ...
  $.each(self.shortcuts, function (shortcut) {
    escapedRegExps.push("(^|\\W)(" + RegExpEscape(shortcut) + ")(?=(\\s|\\.|\\?|,|\\/|!|:|$))");
  });
...
EmoticonShortcutsFilter.prototype.processMessage = function (e, eventData) {
  ...
  var tmpReplacements = [];
  messageContents = messageContents.replace(/(\:[a-zA-Z\_]{1,}\:)/g, function ($0) {
  ...
  messageContents = messageContents.replace(self.emoticonsRegExp, function (match) {
    ...
    return self.shortcuts[foundSlug] ? prefix + self.shortcuts[foundSlug] + suffix : match;
  });

  // revert back the :*: emoticons
  messageContents = messageContents.replace(/(\$ee\:(\d{1,})\$)/g, function ($0, $1, $2) {
