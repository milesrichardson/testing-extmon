...
var EmoticonsFilter = function (megaChat) {
  ...
  self.emoticonsLoading = megaChat
    .getEmojiDataSet("emojis")
    .then((emojis) => {
      self.emojis = emojis;
...
EmoticonsFilter.prototype.processHtmlMessage = function (messageContents) {
  ...
  messageContents = messageContents.replace(self.emoticonsRegExp, function (match, p1) {
    ...
    if (self.reservedEmotions[foundSlug]) {
      return (
        ...
        self.reservedEmotions[foundSlug] +
        '" src="' +
        staticpath +
        "images/mega/twemojis/2_v2/" +
        twemoji.size +
        "/" +
        twemoji.convert.toCodePoint(self.reservedEmotions[foundSlug]) +
        twemoji.ext +
        '"/>'
      );
    }
  ...
  messageContents = messageContents.replace('class="emoji"', 'class="emoji"');

  // if only one emoji, make it big
  if (messageContents.substr(0, 4) === "<img" && messageContents.substr(-1) === ">" && messageContents.indexOf("<img", 1) === -1) {
    messageContents = messageContents.replace('class="emoji"', 'class="emoji big"');
  }
  ...
  messageContents = messageContents.replace(/alt="([^"]+)"/g, function (match, p1) {
    if (megaChat._emojiData.emojisUtf[p1]) {
      ...
      return match.replace(match, 'alt="' + p1 + '" data-simpletip=":' + megaChat._emojiData.emojisUtf[p1].n + ':"');
    }
