(function (scope) {
  ...
  var ChatdIntegration = function (megaChat) {
    ...
    megaChat.rebind("onNewGroupChatRequest.chatdInt", function (e, contactHashes, opts) {
      ...
      var protocolHandler = new strongvelope.ProtocolHandler(u_handle, u_privCu25519, u_privEd25519, u_pubEd25519, chatMode, true);
      var promises = [];
      ...
      var _createChat = () => {
        ...
        var topic = null;
        if (opts.topic) {
          topic = ChatRoom.encryptTopic(protocolHandler, opts.topic, participants, chatMode === strongvelope.CHAT_MODE.PUBLIC);
        }
        ...
        asyncApiReq(mccPacket).catch((ex) => {
          this.logger.error("Failed to retrieve chatd ID from API, while trying to create a new room, mccReq:", mccPacket, ex);
        });
      };
  ...
  ChatdIntegration._waitForProtocolHandler = async function (chatRoom, cb = nop) {
    ...
    return createTimeoutPromise(() => !!chatRoom.protocolHandler, 500, 15000, false, name)
      .then(done)
      .catch(fail);
  };
  ...
  ChatdIntegration.prototype.joinChatViaPublicHandle = function (chatRoom) {
    ...
    return new Promise((resolve, reject) => {
      if (chatRoom.protocolHandler && chatRoom.protocolHandler.unifiedKey) {
        ...
        asyncApiReq(this.getMciphRequest(chatRoom))
          .then((res) => {
  ...
  ChatdIntegration.prototype._finalizeMcurlResponseHandling = function (ret, chatInfo, publicChatHandle, finishProcess) {
    ...
    var chatRoom;
    if (publicChatHandle) {
      var keys = ["url", "id", "cs", "ct", "ts", "ck", "mr", "callId", "w"];
  ...
  ChatdIntegration.prototype._retrieveShardUrl = function (isPublic, chatIdOrHandle, chatShard) {
    ...
    if (isPublic) {
      ...
      if (this.chatd && ((this.chatd.shards[chatShard] || false).s || false).readyState === WebSocket.OPEN) {
  ...
  ChatdIntegration.prototype.openChat = promisify(function (resolve, reject, chatInfo, isMcf, missingMcf) {
    ...
    var getShardURL = function (onSuccess) {
      self
        ._retrieveShardUrl(publicChatHandle, chatInfo.id, chatInfo.cs)
        .then(function (ret) {
          ...
          createTimeoutPromise(validate1, 500, 10000)
            .then(() => {
              if (setAsActive.createChatLink === true) {
                createTimeoutPromise(validate2, 300, 2000).always(() => {
            ...
            chatInfo.n.forEach(function (v) {
              if (v.p === -1) {
                ...
                if (v.u === u_handle) {
                  if (typeof chatRoom.megaChat.plugins.presencedIntegration !== "undefined") {
                    Object.keys(chatRoom.members).forEach(function (user_handle) {
                      ...
                      if (!M.u[user_handle].c) {
                        chatRoom.megaChat.plugins.presencedIntegration.eventuallyRemovePeer(user_handle, chatRoom);
                      }
                  ...
                  if (!M.u[v.u].c && typeof chatRoom.megaChat.plugins.presencedIntegration !== "undefined") {
                    chatRoom.megaChat.plugins.presencedIntegration.eventuallyRemovePeer(v.u, chatRoom);
                  }
                }
              } else {
                included.push(v.u);
                ...
                if (M.u[v.u] && M.u[v.u].c === 1 && typeof chatRoom.megaChat.plugins.presencedIntegration !== "undefined") {
                  chatRoom.megaChat.plugins.presencedIntegration.eventuallyAddPeer(v.u, undefined);
                }
              }
            });
  ...
  ChatdIntegration._waitForShardToBeAvailable = function (name, fn) {
    return async function (...args) {
      ...
      if (!this.chatd.chatIdShard[chatIdDecoded]) {
        ...
        await createTimeoutPromise(() => !!this.chatd.chatIdShard[chatIdDecoded], 500, 1e4, 0, `wfShard.${name}`).catch((ex) => {
  ...
  ChatdIntegration._waitUntilChatIdIsAvailable = function (name, fn) {
    return async function (...args) {
      ...
      if (!chatRoom.chatId) {
        ...
        await this._retrieveChatdIdIfRequired(chatRoom).catch((ex) => {
  ...
  ChatdIntegration.prototype._parseMessage = function (chatRoom, message) {
    ...
    if (!textContents || textContents === "") {
      ...
      if (messageHasAttachment || messageIsVoiceClip) {
        ...
        return null;
      } else if (textContents[1] === Message.MANAGEMENT_MESSAGE_TYPES.CONTAINS_META) {
      ...
      message.wrappedChatDialogMessage = chatRoom.megaChat.plugins.callManager2.remoteEndCallToDialogMsg(chatRoom, message);
    } else if (message.dialogType === "remoteCallStarted") {
      message.wrappedChatDialogMessage = chatRoom.megaChat.plugins.callManager2.remoteStartedCallToDialogMsg(chatRoom, message);
    }
  };
  ...
  ChatdIntegration.prototype._attachToChatRoom = promisify(function (resolve, reject, chatRoom) {
    ...
    if (chatRoom.messagesBuff) {
      ...
      chatRoom.messagesBuff.rebind("onHistoryFinished.chatd", function () {
        ...
        var mIdx = -1;
        var parseMessage = tryCatch(
          function (msg) {
            var messageId = hist[mIdx].k;
            var msgInstance =
              containerBatchFromHist[messageId] || containerMessages[messageId] || chatRoom.messagesBuff.getMessageById(messageId);
      ...
      chatRoom.rebind("onAddUserRequest.chatdInt", function (e, contactHashes) {
        contactHashes.forEach(function (h) {
          if (chatRoom.topic) {
            ...
            ChatdIntegration._ensureKeysAreLoaded(0, participants, chatRoom.publicChatHandle)
              .then(() => {
                ...
                if (topic) {
                  if (chatRoom.type === "public") {
                    ...
                    assert(chatKeyEncoded && chatKeyEncoded.length === 32, "onAddUserRequest: invalid chat key passed.");
        ...
        if (msgObject.message && msgObject.message.length && msgObject.message.length > 0) {
          ChatdIntegration.decryptMessageHelper(msgObject)
            .then(function (decrypted) {
              ...
              var succeeded = self._processDecryptedMessage(chatRoom, msg, decrypted, false, true);
      ...
      if ((chatRoom.type === "public" || chatRoom.type === "group") && chatRoom.ck) {
        ...
        if (parsedKey) {
          waitingForPromises.push(ChatdIntegration._ensureKeysAreLoaded(undefined, [parsedKey.sender], chatRoom.publicChatHandle));
        }
      }
      ...
      chatRoom.strongvelopeSetupPromises = Promise.allSettled(waitingForPromises)
        .then((res) => {
          ...
          if (is_chatlink) {
            chatRoom.protocolHandler = new strongvelope.ProtocolHandler(null, null, null, null, strongvelope.CHAT_MODE.PUBLIC, chatRoom.ck);
          } else {
          ...
          if (chatRoom.ck && chatRoom.type === "public") {
            if (!chatRoom.protocolHandler.gotRsaFailure) {
              ...
              self
                .decryptUnifiedkey(chatRoom)
                .then(() => self.decryptTopic(chatRoom))
                .catch(nop);
            }
        ...
        .catch((ex) => {
          if (d && ex instanceof Error) {
            self.logger.error(
              "Failed to pre-load keys before initialising strongvelope. " + "Can't initialise strongvelope for this chat: ",
              chatRoom.roomId,
              waitingForPromises,
              ex
            );
    ...
    if (decryptedResult && msgInstance) {
      if (decryptedResult.type === strongvelope.MESSAGE_TYPES.GROUP_FOLLOWUP) {
        ...
        if (decryptedResult.identity && decryptedResult.references) {
          ...
          if (mb.verifyMessageOrder(decryptedResult.identity, decryptedResult.references, messageOrdersSource) === false) {
        ...
        decryptedResult.type === strongvelope.MESSAGE_TYPES.PUBLIC_HANDLE_CREATE ||
        ...
        msgInstance.meta = megaChat.plugins.meetingsManager.getFormattingMeta(decryptedResult.handle, decryptedResult, chatRoom);
      } else {
  ...
  ChatdIntegration.prototype.decryptTopic = function (chatRoom) {
    return this._decryptTopicHelper(chatRoom).catch((ex) => {
  ...
  ChatdIntegration.prototype.sendMessage = async function (chatRoom, messageObject) {
    ...
    for (var i = 0; i < refs.length; i++) {
      ...
      } else if (chatRoom.messagesBuff.messages[refs[i]] && chatRoom.messagesBuff.messages[refs[i]].msgIdentity) {
    ...
    return this.chatd.submit(base64urldecode(chatRoom.chatId), result, keyid, messageObject.isPostedAttachment);
  };
  ...
  ChatdIntegration.prototype.updateMessage = function (chatRoom, msgnum, newMessage) {
    ...
    if (!msg) {
      ...
      if (!msg && self.chatd.chatdPersist) {
        return self.chatd.chatdPersist
          .getMessageByOrderValue(chatRoom.chatId, msgnum)
          .then((r) => {
          ...
          .catch((ex) => {
            console.error("Update message failed, msgNum was not found in either .buf, .sendingbuf or messagesBuff", msgnum, ex);
          });
      }
  ...
  ChatdIntegration.prototype.updateScheduledMeeting = async function (options, scheduledMeetingId, chatId) {
    ...
    if (!scheduledMeeting.isSameAsOpts(options)) {
      ...
      const res = await asyncApiReq({
        ...
        this.logger.error(`Failed to update scheduled meeting ${scheduledMeetingId}, chatId: ${chatId}, error: ${ex}`);
        throw ex;
      });
    ...
    const res = await asyncApiReq({
      ...
      ...(scheduledMeeting.recurring && {
        r: {
          ...
          mwd:
            scheduledMeeting.recurring.offset && Object.keys(scheduledMeeting.recurring.offset).length
      ...
      this.logger.error(`Failed to cancel scheduled meeting ${scheduledMeeting.id}, chatId: ${chatId}, error: ${ex}`);
      throw ex;
    });
  ...
  ChatdIntegration.prototype.deleteScheduledMeeting = async function (scheduledMeetingId, chatId) {
    const res = await asyncApiReq({ a: "mcsmr", id: scheduledMeetingId, i: chatId }).catch((ex) => {
  ...
  ["retrieveHistory", "markMessageAsSeen", "markMessageAsReceived", "sendMessage", "updateMessage"].forEach((fnName) => {
    ChatdIntegration.prototype[fnName] = ChatdIntegration._waitForShardToBeAvailable(fnName, ChatdIntegration.prototype[fnName]);
  });

  // decorate ALL functions which require a valid chat ID
  ["join", "retrieveHistory", "markMessageAsSeen", "markMessageAsReceived", "sendMessage", "updateMessage"].forEach((fnName) => {
    ChatdIntegration.prototype[fnName] = ChatdIntegration._waitUntilChatIdIsAvailable(fnName, ChatdIntegration.prototype[fnName]);
  });
