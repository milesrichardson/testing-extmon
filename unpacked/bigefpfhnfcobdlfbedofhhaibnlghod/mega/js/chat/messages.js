...
Message._getTextContentsForDialogType = function (message) {
  if (!message.textContents || (typeof message.textContents.length !== "undefined" && message.textContents.length === 0)) {
    var textMessage =
      mega.ui.chat.getMessageString(
    ...
    if (message.dialogType === "privilegeChange" && message.meta) {
      ...
      if (message.meta.excluded && message.meta.excluded.length > 0) {
        ...
        if (otherContact) {
          if (otherContact.u === contact.u) {
            textMessage = message.chatRoom.isMeeting ? l.meeting_mgmt_left /* `Left the meeting.` */ : l[8908] /* `Left the group chat.` */;
          } else {
            textMessage = (
              message.chatRoom.isMeeting ? l.meeting_mgmt_kicked /* `Was removed from the meeting by %s.` */ : l[8906]
            ) /* `Was removed from the group chat by %s.` */
      ...
      textMessage = l[9081].replace("%s", '"' + htmlentities(message.meta.topic) + '"');
      ...
      textMessage = message.meta.handleUpdate === 1 ? l[20570] : l[20571];
      ...
      if (
        meta.reason === CallManager2.CALL_END_REMOTE_REASON.CALL_ENDED ||
    ...
    if (textMessage) {
      return textMessage.replace(/\[\[/g, "").replace(/\]\]/g, "");
    } else {
      return false;
    }
  }
...
Message.prototype._onAttachmentReceived = function (data) {
  ...
  if (this.source === Message.SOURCE.CHATD && this.hasAttachments() && !this.chatRoom.messagesBuff.sharedFiles[this.messageId]) {
...
Message.prototype._onAttachmentRevoked = function (h, thisMsg) {
  ...
  var a = Object.values(M.chc[roomId] || {}).filter(function (n) {
...
Message.prototype.isSentOrReceived = function () {
  ...
  return (
    state === Message.STATE.DELIVERED || state === Message.STATE.SENT || state === Message.STATE.SEEN || state === Message.STATE.NOT_SEEN
  );
};
...
Message.fromPersistableObject = function (chatRoom, msgData) {
  ...
  var msg = new Message(chatRoom, chatRoom.messagesBuff, {
  ...
  ["meta", "dialogType", "references", "msgIdentity", "metaType"].forEach(function (k) {
  ...
  return function msgBufSMWrap(msg, ignoreDB) {
    const persist = () => {
      ...
      if (ignoreDB !== true && chatdPersist) {
        ...
        if (!(chatRoom.type === "public" && parent.joined === true && (members[u_handle] === undefined || members[u_handle] === -1))) {
          chatdPersist.persistMessageBatched(method === "removeByKey" ? "remove" : method, chatRoom.chatId, toArray.apply(null, arguments));
        }
      }
    };
...
MessageBuffSortedMap.prototype.push = function (msg) {
  ...
  if (chatRoom.scrolledToBottom === true && chatRoom.activeSearches === 0 && this.length > Chatd.MESSAGE_HISTORY_LOAD_COUNT * 2) {
...
function MessagesBuff(chatRoom, chatdInt) {
  ...
  chatRoom.rebind("onHistoryDecrypted.mb", function () {
    ...
    if (!self.isRetrievingSharedFiles && self.haveMoreHistory() && self.messages.length <= Chatd.MESSAGE_HISTORY_LOAD_COUNT_INITIAL) {
  ...
  chatRoom.rebind("onMembersUpdated.messagesBuff" + chatRoomId, function (e, eventData) {
    ...
    if (eventData.userId === u_handle || chatRoom.type === "public") {
      self.joined = true;
  ...
  self.chatRoom.rebind("onMessagesHistoryDone.messagesBuff" + chatRoomId, function () {
    ...
    if (self.isRetrievingSharedFiles) {
      ...
      } else if (self.expectedMessagesCount >= requestedMessagesCount) {
        // this is an empty/new chat/the last message is an attachment.
        self.expectedMessagesCount = 0;
        self.haveMoreSharedFiles = false;
      } else if (self.expectedMessagesCount > 0) {
  ...
  self.chatRoom.rebind("onMessageStore.messagesBuff" + chatRoomId, function (e, eventData) {
    ...
    var msgObject = new Message(chatRoom, self, {
    ...
    if (!eventData.isNew) {
      ...
      if (eventData.pendingid) {
        ...
        ["meta", "metaType", "pendingMessageId"].forEach(function (k) {
          if (foundMessage[k]) {
  ...
  self.chatRoom.rebind("onMessageUpdated.messagesBuff" + chatRoomId, function (e, eventData) {
    ...
    if (eventData.state === "EDITED" || eventData.state === "TRUNCATED") {
      ...
      var editedMessage = new Message(chatRoom, self, {
        messageId: eventData.messageId,
        userId: eventData.userId,
        keyid: eventData.keyid,
        message: eventData.message,
          ...
          if (decrypted.type === strongvelope.MESSAGE_TYPES.GROUP_FOLLOWUP) {
            ...
            if (decrypted.identity && decrypted.references) {
              ...
              if (chatRoom.messagesBuff.verifyMessageOrder(decrypted.identity, decrypted.references) === false) {
                ...
                self.logger.critical("message order tampering detected: ", chatRoom.chatId, eventData.messageId);
              }
            }
      ...
      if (foundMessage) {
        var confirmedMessage = new Message(chatRoom, self, {
          messageId: eventData.messageId,
          userId: eventData.userId,
          keyid: eventData.keyid,
          message: eventData.message,
      ...
      if (foundMessage.messageId !== eventData.messageId) {
        ...
        self.messages.push(foundMessage);
      }
      ...
      var outgoingMessage = new Message(chatRoom, self, {
        userId: eventData.userId,
      ...
      ChatdIntegration.decryptMessageHelper(outgoingMessage)
        .then(function (decrypted) {
          ...
          chatRoom.megaChat.plugins.chatdIntegration._parseMessage(chatRoom, chatRoom.messagesBuff.messages[eventData.messageId]);
        })
        .catch(function (ex) {
          self.logger.error("Failed to decrypt message!", ex);
        });
...
MessagesBuff.prototype.setLastSeen = function (msgId, isFromChatd, force) {
  ...
  if (!self.lastSeen || lastMsg.orderValue < targetMsg.orderValue || force) {
    ...
    if (
      force ||
      (!isFromChatd && self.joined && !self.isRetrievingHistory && typeof self.chatRoom.members[u_handle] !== "undefined" && !is_chatlink)
    ) {
      delay(
        "MessagesBuff.setLastSeen:" + this,
        function () {
...
MessagesBuff.prototype.retrieveSharedFilesHistory = async function (len) {
  ...
  await Promise.all([this.$msgsHistoryLoading, this.$sharedFilesLoading, this.$isDecryptingSharedFiles]).catch(dump);
  ...
  if (this.haveMoreSharedFiles || !this.sharedFilesLoadedOnce) {
    ...
    var lastMsgId =
      self.sharedFiles.length === 0 ? (lastKnownMessage ? lastKnownMessage.messageId : "") : self.sharedFiles.getItem(0).messageId;
...
MessagesBuff.prototype.retrieveChatHistory = function (isInitialRetrivalCall) {
  ...
  var len = Number.isInteger(isInitialRetrivalCall)
    ? isInitialRetrivalCall
    : isInitialRetrivalCall
    ? Chatd.MESSAGE_HISTORY_LOAD_COUNT_INITIAL
  ...
  if (self.$msgsHistoryLoading) {
    ...
    self.isDecrypting.finally(function () {
      proxyPromise.linkDoneAndFailTo(self.retrieveChatHistory(isInitialRetrivalCall));
    });
  ...
  self.chatdInt.retrieveHistory(self.chatRoom, len * -1);
...
MessagesBuff.prototype.getLastMessageFromServer = function () {
  ...
  if (this.messages.length > 0) {
    ...
    for (var i = msgs.length - 1; i >= 0; i--) {
      ...
      if (msg && ((msg.textContents && msg.textContents.length > 0) || msg.dialogType) && msg.messageId && msg.messageId.length === 11) {
        if ((msg.isManagement && msg.isManagement() === true && msg.isRenderableManagement() === false) || msg.revoked === true) {
          continue;
        }
        return msg;
      }
...
MessagesBuff.prototype.getLatestTextMessage = function () {
  if (this.messages.length > 0) {
    ...
    for (var i = msgs.length - 1; i >= 0; i--) {
      var msg = msgs.getItem(i);
      if (msg && ((msg.textContents && msg.textContents.length > 0) || msg.dialogType)) {
        if ((msg.isManagement && msg.isManagement() === true && msg.isRenderableManagement() === false) || msg.revoked === true) {
          continue;
        }
        return msg;
...
MessagesBuff.prototype.restoreMessage = function (msgObject) {
  ...
  self.chatRoom.megaChat.plugins.chatdIntegration._parseMessage(self.chatRoom, msgObject);
  ...
  self.messages.push(msgObject, true);
...
MessagesBuff.prototype._removeMessagesBefore = function (messageId) {
  ...
  var lengthBefore = self.messages.length;
  self.logger.info("messages in memory now: ", lengthBefore);
  ...
  self.logger.info("messages removed from memory: ", lengthBefore - self.messages.length);
};
  ...
  if (author) {
    if (lastMessage.chatRoom.type === "private") {
      ...
      if (author.u === u_handle) {
        ...
        const name = rawContent === 0xbadf ? M.getNameByHandle(author.u) : megaChat.html(M.getNameByHandle(author.u));
        if (name) {
...
MessagesBuff.prototype.encryptReaction = promisify(function (resolve, reject, msgId, meta, key, msg) {
  ...
  if (!key) {
    ...
    var self = this;
    cdp
      .getKey(self.chatRoom.chatId, msg.userId, msg.keyid)
      .then(function (r) {
  ...
  emoji = MessagesBuff._padString(emoji, MessagesBuff._roundToNearestMultipleOf(emoji.length, 4));
...
MessagesBuff.prototype.decryptReaction = promisify(function (resolve, reject, msgId, s, key, isRetry, msg) {
  ...
  if (!(msg = msg || this.getMessageById(msgId))) {
    if (!isRetry && isRetrievingHistory) {
      ...
      megaChat
        .getMessageByMessageId(chatRoom.chatId, msgId)
  ...
  if (!key) {
    ...
    chatdPersist
      .getKey(chatRoom.chatId, msg.userId, msg.keyid)
      .then((r) => {
...
MessagesBuff.prototype._userReaction = function (op, msgId, slug, meta) {
  ...
  var self = this;
  return self
    .encryptReaction(msgId, meta)
    .then(function (r) {
      Reactions.sendAndQueueOperation(op, self.chatRoom, self.getMessageById(msgId), msgId, slug, meta, r);
    })
    .catch(dump);
};
...
MessagesBuff.prototype._onReactionEvent = async function (eventName, eventData) {
  ...
  return this.decryptReaction(msgId, emoji).then(({ msg, res }) => {
  ...
  this.messages.forEach((msg) => {
    ...
    if (msg.reacts && $.len(msg.reacts.reactions)) {
      for (const [emoji, handles] of Object.entries(msg.reacts.reactions)) {
        content.push(mega.utils.trans.listToString(nameList(Object.keys(handles)), emojiString.replace("%s2", emoji)));
      }
    }
