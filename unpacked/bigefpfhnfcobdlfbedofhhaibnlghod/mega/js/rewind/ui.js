lazy(mega, "rewindUi", () => {
  ...
  class RewindSidebar {
        ...
        onRenderCell: (date, cellType, oldHtml) => {
          ...
          if (cellType === DATEPICKER_CELL_TYPE_DAY) {
            ...
            if (hasRecord) {
              ...
              const dateString = formatLine(`item-date`, `${this.formatTooltipDate(date)}`, false, true);
              const divAdded = formatLine(
                `sprite-fm-mono icon-add-filled item-added`,
                mega.icu.format(l.rewind_label_added, record.added).replace("%1", this.formatNumber(record.added)),
                true
              );
              const divModified = formatLine(
                `sprite-fm-mono icon-changed item-modified`,
                mega.icu.format(l.rewind_label_modified, record.modified).replace("%1", this.formatNumber(record.modified)),
                true
              );
              const divRemoved = formatLine(
                `sprite-fm-mono icon-removed item-removed`,
                mega.icu.format(l.rewind_label_removed, record.removed).replace("%1", this.formatNumber(record.removed)),
                true
              );
    ...
    addDatepickerOverlay(type) {
      ...
      if (overlayCaption) {
        overlayCaption = overlayCaption.replace("[A]", '<a href="#" class="rewind-datepicker-upgrade">').replace("[/A]", "</a>");
      }
    ...
    close(currentDirectory, newListContainer) {
      ...
      if (currentDirectory) {
        ...
        if (this.currentHandle && this.isSameOrAncestor(M.d, currentDirectory, this.currentHandle)) {
          isOpenFolder = true;
          closeRewind = false;
        }
    ...
    addEventHandlers() {
      ...
      this.$filterSelectButton.rebind("click.rewind", (e) => {
        ...
        if (this.$filterSelectButton.hasClass("focused") && ($target.closest(".filter-label") || $target.is(".filter-icon"))) {
      ...
      this.$infoButton.rebind("click.rewind", () => {
        ...
        mega.rewind.injectNodes(
          currentHandle,
          selectedNodeRawHandle,
          async () => {
      ...
      this.$openFolderButton.rebind("click.rewind", () => {
        ...
        mega.rewind
          .openSidebar(null, currentHandle, true)
          .then(() => {
    ...
    updateFilterLabel(date, totalFiles, isDefault) {
      ...
      const allLabelCount = isDefault
        ? `${l.rewind_label_all_default}`
        : mega.icu.format(l.rewind_label_all, totalFiles).replace("%1", this.formatNumber(totalFiles));

      $allLabel.text(`${allLabelCount}`);
      $addedLabel.text(mega.icu.format(l.rewind_label_added, record.added).replace("%1", this.formatNumber(record.added)));
      $modifiedLabel.text(mega.icu.format(l.rewind_label_modified, record.modified).replace("%1", this.formatNumber(record.modified)));
      $removedLabel.text(mega.icu.format(l.rewind_label_removed, record.removed).replace("%1", this.formatNumber(record.removed)));
    ...
    async getFilterNodes(type, selectedHandle) {
      const dayData = (mega.rewind.dateData && mega.rewind.dateData[this.selectedDateString]) || Object.create(null);
        ...
        itemHeightCallback: (handle) => {
          const type =
            (mega.rewind.dateData &&
              ...
              mega.rewind.dateData[this.selectedDateString].type[handle]) ||
            TYPE_NONE;
    ...
    toggleDownloadPanel() {
      if (this.hasSelectedNodes()) {
        ...
        const formattedInfo = `${l.rewind_folder_download_caption.replace("%1", this.formatDownloadCaptionDate(this.selectedDate))}`;
    ...
    selectTreeCheckbox(handle, status) {
      ...
      if (oldStatus === TREE_SELECT_PARTIAL && !isSelected && this.selectedNodesPartial[handle]) {
    ...
    hasSelectedNodes() {
      return (
        (this.selectedNodes && Object.keys(this.selectedNodes).length) ||
      ...
      if (handleEnd) {
        ...
        while (currentNodeHandle !== handleEnd) {
          ...
          if (this.selectedNodes[currentNodeHandle]) {
            ...
            this.selectedNodesPartial[selectedHandle][currentNodeHandle] = this.selectedNodes[currentNodeHandle];
            delete this.selectedNodes[currentNodeHandle];
          }

          if (currentNodeHandle === handleEnd && handleEndFolder) {
            const nextNode = nodeDictionary[handleEnd];
            this.closeTreeChildren(nextNode.h, nodesToRemove, nextNode);
          } else {
            index++;
          }
        }
    ...
    renderFileTemplate(node) {
      ...
      const nodeDateData =
        (mega.rewind.dateData && mega.rewind.dateData[this.selectedDateString] && mega.rewind.dateData[this.selectedDateString]) || null;
      ...
      if (type !== TYPE_NONE) {
        detailsElement.classList.remove("hidden");
        tsIconElement.classList.add.apply(tsIconElement.classList, this.getTypeIconClass(type));
        tsLabelElement.textContent = this.getTypeTimestamp(type, currentTimestamp);
      }
    ...
    getTypeTimestamp(type, timestamp) {
      ...
      // If time is 1:00 AM
      const isSingular = timestampDate.getHours() === 1 && timestampDate.getMinutes() === 0;
      switch (type) {
        case TYPE_ADDED:
    ...
    getTemplate(isFolder, isFilter) {
      if (isFilter) {
        if (!this.rewindFilterListFileTemplate) {
          this.rewindFilterListFileTemplate = document.querySelector("#rewind-filter-list-file");
        }
