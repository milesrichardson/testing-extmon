...
var ulmanager = {
  ...
  ulShowOverStorageQuotaDialog: function (aFileUpload) {
    ...
    M.safeShowDialog("upload-overquota", () => {
      ...
      $(".p-after-icon.msg-overquota", $dialog).text(
        is_mobile
          ? l[22673]
              .replace("%1", bytesToSize(pro.maxPlan[2] * 1024 * 1024 * 1024, 0))
  ...
  retry: function UM_retry(file, chunk, reason, xhr) {
    ...
    file.__umRetryTimer[tid] = setTimeout(
      function () {
    ...
    onUploadError(file, l[20918], reason.substr(0, 2) === "IO" ? "IO Failed" : reason, xhr);
  ...
  networkErrorCheck: function UM_network_error_check() {
    ...
    for (i = 0; i < dl_queue.length; i++) {
      if (dl_queue[i] && dl_queue[i].dl_failed) {
        if (d) {
          dlmanager.logger.info(
            "Failed download:",
            dl_queue[i].zipname || dl_queue[i].n,
            "Retries: " + dl_queue[i].retries,
            dl_queue[i].zipid
          );
        }
  ...
  ulUpload: function UM_ul_upload(File) {
    ...
    file.ul_aes = new sjcl.cipher.aes([file.ul_key[0], file.ul_key[1], file.ul_key[2], file.ul_key[3]]);
    ...
    if (!file.faid && !window.omitthumb) {
      ...
      if (img || vid) {
        ...
        createthumbnail(file, file.ul_aes, file.faid, null, null, { raw: img !== 1 && img, isVideo: vid }).catch(nop);
  ...
  ulCompletePending2: function UM_ul_completepending2(res, ctx) {
    ...
    if (typeof res === "object" && "st" in res) {
      ...
      if (res !== EOVERQUOTA && res !== EGOINGOVERQUOTA) {
        srvlog("Unexpected upload completion server response (" + res + " @ " + hostname(ctx.file.posturl) + ")");
      }
    }
        ...
        callback: function (res, ctx) {
          ...
          if (oIsFrozen(File)) {
            ...
            if (!(uq.skipfile = !File.file.chatid)) {
              ...
              if (eventData) {
                if (d) {
                  ulmanager.logger.info("[%s] Cleaning efa on deduplication " + "for the chat to be aware...", ctx.n.h, eventData.efa);
                }
                eventData.efa = 0;
              }
            }
            ...
            if (eventData) {
              ...
              if (eventData.efa !== efa) {
                if (d) {
                  ulmanager.logger.info(
                    "[%s] Fixing up efa on deduplication " + "for the chat to be aware... (%s != %s)",
                    ctx.n.h,
                    eventData.efa,
                    efa
                  );
  ...
  ulIdentical: function UM_ul_Identical(file) {
    ...
    if (nodes) {
      for (var node in nodes) {
        ...
        if (node && file.size === node.s && file.name === node.name && file.hash === node.hash) {
          return node;
        }
      }
    }
...
ChunkUpload.prototype.onXHRready = function (xhrEvent) {
    ...
    if (!response.byteLength || response.byteLength === 36) {
      ...
      if (response.byteLength === 36) {
        ...
        if (u_k_aes && this.gid && !ulmanager.ulCompletingPhase[this.gid]) {
          ...
          this.file.response = u8[35] === 1 ? ab_to_base64(response) : ab_to_str(response);
      ...
      if (estr !== "EKEY") {
        return this.onXHRerror("$FATAL", null, estr ? estr + " error" : "IUR[" + String(resp).trim().substr(0, 5) + "]");
      }
    }
  }
  ...
  if (d) {
    this.logger.warn("Bad response from server, status(%s:%s)", xhr.status, xhr.statusText, isValidType, this.file.name);
  }
...
ChunkUpload.prototype.upload = function () {
  ...
  if (!this.file) {
    if (d) {
      logger.error("This upload was cancelled while the Encrypter was working," + " prevent this aborting it beforehand");
    }
    return;
  }
...
var ulQueue = new TransferQueue(
  ...
  is_ios && isMediaSourceSupported() ? 1 : 4,
  "uploader"
);
