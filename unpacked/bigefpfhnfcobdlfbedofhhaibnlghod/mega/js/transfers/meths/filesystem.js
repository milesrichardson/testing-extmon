...
(function (window) {
            ...
            if (!dlmanager.isTrasferActive(file.name) && deltime < Date.now() && deltime < lastactive) {
              file.remove(
                function () {
                  ...
                  if (d) {
                    console.log("temp file removed", file.name, bytesToSize(metadata.size));
                  }
                ...
                function () {
                  if (d) {
                    console.log("temp file removal failed", file.name, bytesToSize(metadata.size));
                  }
                  if (++del == entries.length && callback) {
                    callback(totalsize);
                  }
    ...
    if (window.requestFileSystem) {
      queryUsageAndQuota(
        ...
        function (used, remaining) {
          if (used + remaining) {
            if (d) {
              console.log("Cleaning %s Storage...", storage_n2s(storagetype), bytesToSize(used), bytesToSize(remaining));
            }
  ...
  function dl_getspace(reqsize, callback, max_retries) {
    ...
    queryUsageAndQuota(
      PERSISTENT,
      function onPQU(used, remaining) {
        if (d) {
          console.log("Used persistent storage: %s, remaining: %s", bytesToSize(used), bytesToSize(remaining));
        }

        queryUsageAndQuota(
          TEMPORARY,
          function onTQU(tused, tremaining) {
            if (d) {
              console.log("Used temporary storage: %s, remaining: %s", bytesToSize(tused), bytesToSize(tremaining));
            }
            ...
            if (used > 0 || remaining > 0) {
              ...
              if (tremaining > reqsize * 5 + 1024 * 1024 * 100) {
                ...
                requestQuota(
                  PERSISTENT,
                  HUGE_QUOTA,
                  function onRQ(grantedBytes) {
                    if (d) {
                      console.log("Granted persistent storage: %s", bytesToSize(grantedBytes));
                    }
  ...
  window.FileSystemAPI = function fsIO(dl_id, dl) {
                    ...
                    if (dl_fw.position == targetpos) {
                      ...
                      var onSpaceFreed = function () {
                        ...
                        if (!(++chrome_write_error_msg % 21) && !$.msgDialog) {
                          ...
                          msgDialog("warningb:" + l[103], WRITERR_DIAGTITLE, l[20817], str_mtrunc(dl_filename), function (cancel) {
                            if (cancel) {
    ...
    this.fsInitOp = function check() {
      dl_getspace(
        dl_filesize,
        function (aStorageType, aEvent, aFail) {
          if (wTimer === null) {
            return;
          }
          ...
          if (d) {
            logger.info("Using Storage: " + storage_n2s(dl_storagetype), aStorageType, aEvent, aFail);
          }
          ...
          try {
            var onWriteEnd = (function (writer, entry) {
              return function () {
                ...
                if (entry) {
                  entry.remove(_logger.debug.bind(_logger), _logger.error.bind(_logger));
                } else if (writer) {
                  writer.truncate(0);
                }
