...
ClassChunk.prototype.shouldIReportDone = function (report_done) {
  ...
  if (!report_done) {
    report_done = !this.done && dlQueue.canExpand() && (pbx[1] - pbx[0]) / this.Progress.speed <= dlmanager.dlDoneThreshold;
  }
...
ClassChunk.prototype.onXHRready = function (xhrEvent) {
  ...
  if (r && r.byteLength === this.size) {
    ...
    this.dl.decrypter++;
    Decrypter.push([[this.dl, this.task.offset], this.dl.nonce, this.task.offset / 16, new Uint8Array(r)]);
    this.dl.retries = 0;
    this.finish_download();
    this.destroy();
  } else if (!this.dl.cancelled) {
    if (d) {
      dlmanager.logger.error("HTTP FAILED", this.dl.n, xhr.status, "am i done? " + this.done, r && r.byteLength, this.size);
    }
...
ClassFile.prototype.destroy = function () {
  if (d) {
    dlmanager.logger.info("Destroying " + this, this.dl ? (this.dl.cancelled ? "cancelled" : "finished") : "expunged");
  }
  if (!this.dl) {
    return;
  }
...
ClassFile.prototype.run = function (task_done) {
  ...
  this.dl.ready = function () {
    if (d) {
      this.dl.writer.logger.info(this + " readyState", this.chunkFinished, this.dl.writer.isEmpty(), this.dl.decrypter);
    }
  ...
  this.dl.io.begin = function (newName, resumeOffset) {
    ...
    if (!this.dl || this.dl.cancelled) {
      ...
      if (this.dl.byteOffset && resumeOffset !== this.dl.byteOffset) {
        if (d) {
          dlmanager.logger.info(this + " cannot resume at offset %s, %s given", this.dl.byteOffset, resumeOffset);
        }
      ...
      if (tasks.length > 0) {
        dlQueue.pushAll(
          ...
          dlmanager.failureFunction.bind(dlmanager)
        );
      }
    }
    ...
    if (cancelOnInit()) {
      ...
      if (this.dl.zipid && (fatal || this.dlGetUrlErrors > 20)) {
        ...
        if (error === EOVERQUOTA) {
          ...
          dlmanager.logger.warn(this + " Retry to fetch url in %dms, error:%s", dlmanager.dlRetryInterval, error);
        }
      }
    } else {
      const init = (resumeInfo) => {
        dlmanager
          .initDownload(this, res, resumeInfo)
          .then((info) => {
