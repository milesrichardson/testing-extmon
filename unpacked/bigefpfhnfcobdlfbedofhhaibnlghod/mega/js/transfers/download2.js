...
var dlmanager = {
    ...
    dlmanager.dlGetUrl(
      ...
      function (error, res, o) {
        ...
        if (Object(this._newUrlQueue).hasOwnProperty(gid)) {
          this._newUrlQueue[gid].forEach(function (callback) {
            callback(res.g, res);
          });
  ...
  dlGetUrlDone: function DM_dlGetUrlDone(res, ctx) {
    ...
    if (!res.e) {
      ...
      if (typeof attr === "object" && typeof attr.n === "string") {
        ...
        if (
          page !== "download" &&
          (!res.fa || !String(res.fa).includes(":0*") || !String(res.fa).includes(":1*") || ctx.object.preview === -1) &&
          res.s > minSize &&
          ...
          if ((res.s < limit && (image || audio)) || video) {
            if (d) {
              this.logger.warn("[%s] Missing thumb/prev, will try to generate...", attr.n, [res], [attr]);
            }
  ...
  failureFunction: function DM_failureFunction(task, args) {
    ...
    if (d) {
      dlmanager.logger.error("Fai1ure", dl.zipname || dl.n, code, task.task.chunk_id, task.task.offset, task.onQueueDone.name);
    }
  ...
  verifyIntegrity: function (dl) {
    ...
    const macs = Object.keys(dl.macs)
      .map(Number)
      .sort((a, b) => a - b)
      .map((v) => dl.macs[v]);
    const aes = new sjcl.cipher.aes([dl.key[0] ^ dl.key[4], dl.key[1] ^ dl.key[5], dl.key[2] ^ dl.key[6], dl.key[3] ^ dl.key[7]]);
  ...
  dlWriter: function DM_dl_writer(dl, is_ready) {
    ...
    dl.writer = new MegaQueue(
      function dlIOWriterStub(task, done) {
        ...
        var ready = function _onWriterReady() {
          ...
          if (dl.misThumbData && task.offset + abLen <= dl.misThumbData.byteLength) {
            new Uint8Array(dl.misThumbData, task.offset, abLen).set(task.data);
          }
          ...
          dlmanager.setResumeInfo(dl, dl.writer.pos).always(function () {
  ...
  _overquotaInfo: function () {
    ...
    const onQuotaInfo = (res) => {
      ...
      let $dlPageCountdown = $(".download.transfer-overquota-txt", "body").text(String(l[7100]).replace("%1", ""));
  ...
  _overquotaClickListeners: function ($dialog, flags, preWarning) {
    ...
    if (preWarning) {
      ...
      $(".continue, .continue-download", $dialog).removeAttr("style").rebind("click", this.onLimitedBandwidth.bind(this));
    } else {
  ...
  _setOverQuotaState: function DM_setOverQuotaState(dlTask) {
    ...
    this.getCurrentDownloads().forEach(function (gid) {
  ...
  showRegisterDialog4ach: function DM_showRegisterDialog($dialog, flags) {
    ...
    mega.ui.showRegisterDialog({
      ...
      onDialogClosed: function () {
        if ($dialog) {
          fm_showoverlay();
          $dialog.removeClass("hidden").find("button.js-close, .fm-dialog-close").rebind("click.quota", closeDialog);
        }
  ...
  getCurrentDownloadsSize: function (sri) {
    ...
    dl_queue.filter(isQueueActive).map(function (dl) {
      size += dl.size;
      ...
      megasync.isInstalled(function (err, is) {
        if (err || !is) {
          ...
          if (minVersion) {
            ...
            if (typeof minVersion !== "number" || parseInt(minVersion) !== minVersion) {
    ...
    $(".mega-button", $elm).rebind("click", function () {
      if (typeof megasync === "undefined") {
        ...
        if (typeof dlpage_ph === "string") {
          ...
          window.open(megasync.getMegaSyncUrl() || "https://mega.io/desktop", "_blank", "noopener,noreferrer");
        }
      }
...
var dlQueue = new TransferQueue(
  function _downloader(task, done) {
    if (!task.dl) {
