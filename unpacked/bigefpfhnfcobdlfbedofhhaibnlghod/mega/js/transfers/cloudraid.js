...
(function megaCloudRaid(global) {
  ...
  CloudRaidRequest.prototype.onPartFailure = function (failedPartNum, partStatus) {
    ...
    if (DEBUG) {
      this.logger.warn(
        "Recovering from fail, part %s",
        failedPartNum,
        this.part[0].failCount,
        this.part[1].failCount,
        this.part[2].failCount,
        this.part[3].failCount,
        this.part[4].failCount,
        this.part[5].failCount
      );
    }
    ...
    if (sumFails > 2 || (partStatus > 200 && !this.EC_SWITCH_SOURCE[partStatus | 0])) {
      ...
      if (DEBUG) {
        this.logger.error("%s, aborting chunk download and retrying...", sumFails > 2 ? "too many fails" : "network error");
      }
      this.response = false;
  ...
  CloudRaidRequest.prototype.start5Channel = function (failedPartNum) {
    ...
    if (DEBUG) {
      this.logger.debug("Begin reading file from position %s, each part from %s", this.wholeFileDatalinePos, partStartPos);
    }
  ...
  CloudRaidRequest.prototype.combineSectors = function (piecesPos, piecesLen, pieces, eofexcess) {
    ...
    var dataToDeliver = deliverByteLen === fileBuf.byteLength ? fileBuf : fileBuf.subarray(skipFileFront, deliverByteLen + skipFileFront);
    ...
    if (this.outputByteCount === this.expectedBytes) {
      ...
      if (DEBUG_VERBOSE) {
        ...
        this.logger.log(
          "Chunk complete, %d bytes (Took %dms, %d bytes/sec) " +
            "bandwidth efficiency: %f, total channel pauses: %d (%dms) - %d requests sent.",
          this.outputByteCount,
          ms,
          (this.outputByteCount * 1000) / ms,
          this.outputByteCount / this.bytesProcessed,
          channelPauseCount,
          channelPauseMs,
          this.totalRequests
        );
  ...
  CloudRaidRequest.prototype.loadPart = function (partNum, pos, endpos, skipchunkcount) {
    ...
    this._loadPart(part, pos, endpos, skipchunkcount).catch((ex) => {
      part.reading = false;
  ...
  CloudRaidRequest.prototype._loadPart = async function (part, pos, endpos, skipchunkcount) {
    ...
    part.reading = true;
    return fetch(pieceUrl, { signal: part.signal }).then((response) => {
      part.reading = false;
  ...
  CloudRaidRequest.prototype.readFetchData = function (partNum) {
    ...
    this._readFetchData(part).catch((ex) => {
      part.reading = false;
      this.fetchReadExceptionHandler(ex, partNum);
    });
  };
  ...
  CloudRaidRequest.prototype._readFetchData = async function (part) {
    ...
    part.reading = true;
    return part.reader.read().then((r) => {
      part.reading = false;
