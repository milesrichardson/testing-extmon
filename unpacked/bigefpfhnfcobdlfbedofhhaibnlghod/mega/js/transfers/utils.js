...
function setTransferStatus(dl, status, ethrow, lock, fatalError) {
  ...
  if (page === "download") {
    ...
    $dlTopBarErrorBlock.removeClass("hidden").attr("title", status).find("span").text(text);
...
(function __FileFingerprint(scope) {
  ...
  function i2s(i) {
    return String.fromCharCode.call(String, (i >> 24) & 0xff, (i >> 16) & 0xff, (i >> 8) & 0xff, i & 0xff);
  }
  ...
  scope.getFingerprint = function (file) {
    return new Promise(function (resolve, reject) {
      var logger = d && new MegaLogger("fingerprint:" + file.size + ":.." + String(file.name).substr(-8), false, ulmanager.logger);
    ...
    if (!$this.hasClass("disabled")) {
      ...
      if ($this.hasClass("active")) {
        ...
        if (u_type || folderlink || Object(u_attr).terms || $(".transfer-table", ".fmholder").length === 0) {
          ...
          const $otherButton = $this.parent().hasClass("transfer-widget-footer")
