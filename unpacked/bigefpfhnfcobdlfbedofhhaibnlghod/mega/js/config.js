(function __fmconfig_handler() {
  ...
  const moveLegacySettings = function () {
    const prefs = [
      "agreedToCopyrightWarning",
      "dl_maxSlots",
      "font_size",
      "leftPaneWidth",
      "mobileGridViewModeEnabled",
      "ul_maxSlots",
      "ul_maxSpeed"
    ];
    const replacements = {
  ...
  const shrink = (cfg) => {
    ...
    cfg.xs1 = stringify(((cfg.obVer & 0xfff) << 20) | ((cfg.font_size & 15) << 12) | (cfg.leftPaneWidth & 0xfff));
  ...
  filter.tree = async (config, types) => {
    ...
    const handles = array.unique(
      Object.keys(types)
        .reduce(
          (s, v) =>
            Object.keys(config[v] || {})
              .map((h) => M.isCustomView(h).nodeID || h)
              .concat(s),
          []
        )
    ...
    return (tree) => {
      ...
      for (let handle in tree) {
        if (hasOwn(tree, handle) && handle.substr(0, 7) !== "search/" && isValid(handle)) {
  ...
  refresh.ui = () => {
    ...
    if ((view !== undefined && M.viewmode !== view) || (sort !== undefined && (sort.n !== M.sortmode.n || sort.d !== M.sortmode.d))) {
