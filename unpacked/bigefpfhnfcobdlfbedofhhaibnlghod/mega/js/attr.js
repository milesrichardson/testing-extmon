...
(function _userAttributeHandling(global) {
   ...
   *     dependencies.
   */
  ns.get = function _getUserAttribute(userhandle, attribute, pub, nonHistoric, callback, ctx, chathandle, decodeValues) {
    ...
    if ((attribute[0] === "^" || attribute[0] === "+") && userhandle in M.u) {
      const data = (Object(window.u_attr).u === userhandle && u_attr[attribute]) || M.getUserByHandle(userhandle)[attribute];
    ...
    var settleFunctionDone = function _settleFunctionDone(res, cached) {
      ...
      if (typeof res !== "number") {
        ...
        if (DEBUG || is_karma) {
          ...
          logger.info(tag + 'Attribute "%s" for user "%s" is %s.', attribute, userhandle, loggerValueOutput);
        }
        thePromise.resolve(res);
      } else {
        // Got back an error (a number).
        if (DEBUG || is_karma) {
          logger.warn(tag + 'attribute "%s" for user "%s" could not be retrieved: %d!', attribute, userhandle, res);
        }
    ...
    var doApiReq = function _doApiReq(promise) {
      ...
      if (promise instanceof MegaPromise) {
        ...
        if (chathandle) {
          api_req({ a: "mcuga", ph: chathandle, u: userhandle, ua: attribute, v: 1 }, myCtx, ATTR_REQ_CHANNEL[attribute]);
        } else {
          api_req({ a: "uga", u: userhandle, ua: attribute, v: 1 }, myCtx, ATTR_REQ_CHANNEL[attribute]);
        }
      }
    };
  ...
  ns.remove = promisify(function (resolve, reject, attribute, pub, nonHistoric, encodeValues) {
    ...
    attribCache.removeItem(cacheKey).always(() => {
      api
        .screq({ a: "upr", ua: attribute, v: 1 })
        .then(({ result }) => {
  ...
  ns.set2 = async function (cmds, attribute, value, pub, nonHistoric, useVersion, options) {
    ...
    if (res === EEXPIRED && useVersion && this._conflictHandlers[attribute]) {
      logger.error(`Server returned version conflict for attribute "${attribute}"`, this._versions[cacheKey]);
      attribCache.removeItem(cacheKey, false);
   ...
   * @deprecated
   */
  ns.set = function _setUserAttribute(attribute, value, pub, nonHistoric, callback, ctx, mode, useVersion, encodeValues) {
    var self = this;
    var myCtx = ctx || {};
    ...
    if (attribute[0] === "*") {
      ...
      savedValue = base64urlencode(tlvstore.blockEncrypt(tlvstore.containerToTlvRecords(value), u_k, mode, false));
    }
    ...
    var settleFunction = function (res) {
      if (typeof res !== "number") {
        ...
        if (res === EEXPIRED && useVersion) {
          ...
          if (!self._conflictHandlers[conflictHandlerId] || self._conflictHandlers[conflictHandlerId].length === 0) {
            logger.error(
              'Server returned version conflict for attribute "' + attribute + '", result: ' + res + ", local version:",
              self._versions[cacheKey]
            );
            thePromise.reject(res);
          } else {
            ...
            self
              .get(u_handle, attrName, pub, nonHistoric, false, false, false, encodeValues)
              .done(function (attrVal) {
                ...
                if (matched) {
                  thePromise.linkDoneAndFailTo(
                    self.set(attrName, valObj.mergedValue, pub, nonHistoric, callback, ctx, mode, useVersion, encodeValues)
                  );
                }
              })
              .fail(function (failResult) {
                logger.error("This should never happen:", attribute, res, failResult, self._versions[cacheKey]);
    ...
    if (useVersion) {
      ...
      apiCall["a"] = "upv";
      if (version) {
        apiCall[attribute] = [savedValue, version];
  ...
  QueuedSetArrayAttribute.prototype.toString = function () {
    ...
    return "QueuedSetArrayAttribute: " + buildAttribute(self.attributeName, self.pub, self.nonHistoric) + "(" + setOps.join(",") + ")";
  };
  ...
  QueuedSetArrayAttribute.prototype.exec = function () {
    ...
    var _setArrayAttribute = function (r) {
      if (r === EINTERNAL) {
        ...
        logger.error(
          "Found number value for attribute: ",
          self.attributeName,
          " when trying to use it as " + "attribute array. Halting .setArrayAttribute"
        );
      ...
      proxyPromise.linkDoneAndFailTo(
        self.attr.set(self.attributeName, serializedValue, self.pub, self.nonHistoric, undefined, undefined, undefined, true)
      );
    };

    self.attr
      .get(u_handle, self.attributeName, self.pub, self.nonHistoric)
      .done(function (r) {
        try {
          if (r === -9) {
            _setArrayAttribute({});
  ...
  ns.getArrayAttribute = function (userId, attributeName, subkey, pub, nonHistoric) {
    ...
    var $getPromise = self.get(userId, attributeName, pub, nonHistoric).done(function (r) {
      try {
        var arr = r ? r : {};
        proxyPromise.resolve(arr[subkey]);
      } catch (e) {
        proxyPromise.reject(e, r);
      }
    });
  ...
  ns.handleBitMapAttribute = function (attrName, version) {
    ...
    if (bitMapInstance.getVersion() !== version) {
      mega.attr.get(u_handle, attributeStringName, attrName.substr(0, 2) === "+!" ? true : -2, true).done(function (r) {
  ...
  ns.uaPacketParser = function uaPacketParser(attrName, userHandle, ownActionPacket, version) {
    ...
    removeItemPromise.always(function _uaPacketParser() {
  ...
  mBroadcaster.once("boot_done", function () {
    ...
    uaPacketParserHandler["*!cam"] = function (userHandle) {
      mega.attr.get(userHandle, "cam", false, true, (res, ctx) => {
        u_attr[ctx.ua] = base64urlencode(
          tlvstore.blockEncrypt(tlvstore.containerToTlvRecords(res), u_k, tlvstore.BLOCK_ENCRYPTION_SCHEME.AES_GCM_12_16)
        );
    ...
    uaPacketParserHandler["^!keys"] = (userHandle) => {
      ...
      if (userHandle === u_handle && !pfid && "keyMgr" in mega) {
        ...
        if (shouldLoad) {
          mega.keyMgr.fetchKeyStore().catch((ex) => {
  ...
  ns.factory = function (attribute, pub, nonHistoric, storeKey, decode, encode) {
    ...
    var cacheValue = function (value) {
      ...
      if (key[0] === "*") {
        value = base64urlencode(
          tlvstore.blockEncrypt(tlvstore.containerToTlvRecords(value), u_k, tlvstore.BLOCK_ENCRYPTION_SCHEME.AES_GCM_12_16)
        );
      } else if (key[0] === "^") {
    ...
    var factory = {
      ...
      get: promisify(function (resolve, reject, force) {
        ...
        mega.attr
          .get(u_handle, attribute, pub, nonHistoric)
          .then(function (value) {
  ...
  ns.getFolderFactory = function (attribute, pub, nonHistoric, storeKey, name, decode, encode) {
    ...
    ns.init = function () {
      factory
        .get()
        .then(factory.notify.bind(factory))
        .catch(function () {
  ...
  ns.registerConflictHandler("lstint", false, true, false, function (valObj) {
  ...
  ns.registerConflictHandler("alias", false, true, true, function (valObj) {
