    ...
    if (u_type === 0) {
      ...
      var pushAsAddedEmails = function () {
        if (!currentContactsEmails.includes(email) && !M.findOutgoingPendingContactIdByEmail(email)) {
          ...
          promises.push(M.inviteContact(M.u[u_handle].m, email, emailText).then((res) => addedEmails.push(res)));
        }
      };
...
function setContactLink($container) {
  ...
  $publicLink.rebind("mouseout.publiclnk", function () {
    $(".dropdown.tooltip.small").removeClass("visible").addClass("hidden");
  });
...
function fmtopUI() {
  ...
  $(".fm-s4-new-bucket, .fm-s4-new-key, .fm-s4-new-user, .fm-s4-new-group", $header).addClass("hidden");
  ...
  var showUploadBlock = function _showUploadBlock() {
    if (M.InboxID && (M.currentrootid === M.InboxID || M.getNodeRoot(M.currentdirid.split("/").pop()) === M.InboxID)) {
      return false;
    }
  ...
  if (M.currentrootid === M.RubbishID) {
    ...
    } else if (String(M.currentdirid).length === 8 && M.getNodeRights(M.currentdirid) > 0) {
      $(".fm-right-files-block").addClass("visible-notification");
      showUploadBlock();
    }
  }
...
function initTreeScroll() {
  ...
  if (folderlink || M.currentTreeType !== "cloud-drive") {
    ...
    $(".js-other-tree-panel .section-title", fmholder).text(folderlink ? (pfcol ? l.shared_album : l.folderlink_lp_title) : l[24682]);
  }
...
function handleResetSuccessDialogs(dialogText, dlgString) {
  ...
  $(".reset-success-title", $resetSuccessDlg).text(dlgString === "deleteaccount" ? l.ac_closure_link_sent : l.pwd_link_sent);
...
function FMShortcuts() {
  ...
  $(window).rebind("keydown.fmshortcuts", function (e) {
    ...
    if (charTyped === "a" && (e.ctrlKey || e.metaKey)) {
    ...
    } else if ((charTyped === "c" || charTyped === "x") && (e.ctrlKey || e.metaKey) && !isShareRoot) {
      ...
      return false; // stop prop.
    } else if (charTyped === "v" && (e.ctrlKey || e.metaKey) && !isShareRoot) {
      ...
      return false; // stop prop.
    } else if (charCode === 8 && !isShareRoot) {
      ...
      if (
        remItems.length === 0 ||
        (M.getNodeRights(M.currentdirid || "") | 0) < 2 ||
...
function renameDialog() {
  ...
  if ($.selected.length > 0) {
    ...
    $(".transfer-filetype-icon", $dialog).attr("class", "transfer-filetype-icon " + fileIcon(n));
...
function msgDialog(type, title, msg, submsg, callback, checkboxSetting) {
  ...
  var $dialog = $("#msgDialog").removeClass(
    "confirmation warning info error question " + "delete-contact loginrequired-dialog multiple with-close-btn"
  );
  ...
  if (type === "clear-bin") {
    ...
    $("#msgDialog aside").safeHTML(
      `<div class="checkbox-block top-pad">
    ...
    if (checkboxSetting) {
      assert(checkboxSetting === "cslrem" || checkboxSetting === "nowarnpl" || checkboxSetting === "skipDelWarning", checkboxSetting);

      $("#msgDialog .checkbox-block .checkdiv," + "#msgDialog .checkbox-block input")
    ...
    $("footer .footer-container", $dialog).safeHTML(
      `<div class="space-between">
                    <button class="mega-button cancel">
                        <span>@@</span>
                    </button>
...
function shareDialogContentCheck() {
  ...
  if (
    Object.keys($.addContactsToShare).length ||
  ...
  if (!cv && u_attr.since < 1697184000 && !mega.keyMgr.getWarningValue("cvb")) {
    ...
    $(cvnText).safeHTML(
      escapeHTML(l.contact_verification_notif_banner)
        .replace("[D]", '<div class="contact-verification-settings">')
        .replace("[/D]", "</div>")
    );
  }
...
function fillShareDialogWithContent(readonly) {
  ...
  userHandles.forEach(function (handle) {
    ...
    if (!user.m) {
      ...
      generateShareDialogRow(name, user.m, share.r | 0, handle, handle in pendingShares, readonly);
      seen[user.m] = 1;
    }
  });
  ...
  for (var newContact in $.addContactsToShare) {
    ...
    if (!seen[newContactEmail]) {
      ...
      generateShareDialogRow(newContactName, newContactEmail, shareRights, newContact, pendingContact, readonly);
...
function showShareDialogPermMenu($this, x, y) {
  ...
  const $permissionMenu = $(".share-dialog-permissions-menu", $shareDialog).removeClass("hidden").addClass("o-hidden");
...
function initShareDialogMultiInput(alreadyAddedContacts) {
  ...
  $input.tokenInput([], {
    ...
    onAdd: function (email) {
      if (listedContacts.indexOf(email.id) > -1) {
        ...
        $(".token-input-token-mega ." + $listedItemHandle, $scope)
          .siblings(".token-input-delete-token-mega")
          .trigger("click");
      } else {
    ...
    onDelete: function () {
      ...
      if (JSON.stringify(clone($.contactPickerSelected).sort()) === JSON.stringify(alreadyAddedContacts.sort()) && newEmailsNum === 0) {
...
function initShareDialog() {
  ...
  $dialog.rebind("click", function (e) {
    ...
    if (!$target.is(".share-dialog-permissions-menu") && !$target.closest(".share-dialog-permissions-menu").length) {
      hideShareDialogPermMenu();
    }
  });
  ...
  $(".share-dialog-permissions-menu .option", $dialog).rebind("click", function (e) {
    ...
    if (e.shiftKey) {
      ...
      $(".share-dialog-access-node:not(.owner)", $dialog)
        .get()
      ...
      $nodes.removeClass("full-access read-and-write read-only simpletip").addClass(newPermLevel[0]);
      ...
      $node.removeClass("full-access read-and-write read-only simpletip").addClass(newPermLevel[0]);
    ...
    } else if (Object.keys($.removedContactsFromShare).length === 0 && Object.keys($.addContactsToShare).length === 0) {
  ...
  $(".share-dialog-access-add", $dialog).rebind("click", function () {
    ...
    if (!$shareAddFooterElement) {
      $shareAddFooterElement = $(document.querySelector(".share-add-dialog-bottom-template").content.firstElementChild.cloneNode(true));
    }
...
function showLoseChangesWarning() {
  ...
  if ($tokenItems.length) {
    ...
    ($.dialog === "share" && Object.keys($.addContactsToShare || []).length > 0) ||
    Object.keys($.changedPermissions || []).length > 0 ||
    Object.keys($.removedContactsFromShare || []).length > 0 ||
    ($.dialog === "file-request-create-dialog" && mega.fileRequest.dialogs.createDialog.checkLoseChangesWarning()) ||
    ($.dialog === "file-request-manage-dialog" && mega.fileRequest.dialogs.manageDialog.checkLoseChangesWarning())
  ) {
...
function closeDialog(ev) {
  ...
  if ($.dialog === "sub-user-adding-dlg") {
    if ($(".user-management-add-user-dialog.user-management-dialog footer .add-sub-user").hasClass("disabled")) {
      return false;
    }
  }
  ...
  var doCreateFolder = function (v) {
    ...
    M.createFolder(target, v)
      .then((h) => {
        ...
        return M.openFolder(Object(M.d[h]).p || target).always(() => {
          $.selected = [h];
          reselect(1);
        });
      })
    ...
    action = function (name, t) {
      ...
      M.addNewFile(name, t)
        .done(function (nh) {
          ...
          if ($.selectddUIgrid.indexOf(".grid-scrolling-table") > -1 || $.selectddUIgrid.indexOf(".file-block-scrolling") > -1) {
...
function bottomPageDialog(close, pp, hh, tickbox) {
  ...
  M.safeShowDialog(pp, function _showDialog() {
    ...
    $bottomPageDialogMain.safeHTML(
      translate(String(pages[pp]).split("((TOP))")[1].split("((BOTTOM))")[0].replace("main-mid-pad new-bottom-pages", ""))
    );
...
function fm_resize_handler(force) {
  ...
  if (M.currentdirid === "shares") {
    ...
    if (
      $dashboardContent.width() < 780 ||
      (!$(".business-dashboard", $dashboardContent).hasClass("hidden") && $dashboardContent.width() < 915)
    ) {
    ...
    initDashboardScroll();
  } else if (M.currentdirid && M.currentdirid.startsWith("user-management") && typeof initBusinessAccountScroll === "function") {
  ...
  if (nodeData) {
    ...
    $(".shared-details-block").prepend(
      '<div class="shared-top-details">' +
        ...
        '<div class="shared-details-folder-name">' +
        ...
        '<span class="fm-chatbutton-arrow inshare-dl-button0">' +
        escapeHTML(l[58]) +
        "</span>" +
        "</button>" +
...
function userFingerprint(userid, callback) {
  ...
  fingerprintPromise.done(function (response) {
    callback(response.toUpperCase().match(/.{4}/g), response);
  });
}
...
function showAuthenticityCredentials(user, $wrapper) {
  var $fingerprintContainer = $wrapper.length ? $wrapper.find(".contact-fingerprint-txt") : $(".contact-fingerprint-txt");
  ...
  $(".download-app", $dialog).rebind("click.downloadDesktopAppDialog", () => {
    switch (ua.details.os) {
      ...
      case "Windows":
        ...
        window.location = megasync.getMegaSyncUrl(ua.details.is64bit && !ua.details.isARM ? "windows" : "windows_x32");
