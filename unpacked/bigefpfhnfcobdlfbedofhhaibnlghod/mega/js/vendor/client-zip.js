var clientZip = (() => {
  ...
  function normalizeInput(input, modDate) {
    if (modDate !== void 0 && !(modDate instanceof Date)) modDate = new Date(modDate);
    ...
    if (modDate === void 0) modDate = /* @__PURE__ */ new Date();
    else if (isNaN(modDate)) throw new Error("Invalid modification date.");
    if (input === void 0) return { isFile: false, modDate };
  ...
  function contentLength(files) {
    ...
    for (const file of files) {
      ...
      if (file.uncompressedSize === void 0) throw new Error(`Missing size for file "${new TextDecoder().decode(file.encodedName)}".`);
    ...
    if (archiveNeedsZip64 || offset >= 0xffffffffn) centralLength += BigInt(zip64endRecordLength + zip64endLocatorLength);
