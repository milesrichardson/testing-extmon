(function (scope, $) {
  ...
  var MegaDynamicList = function (listContainer, options) {
    ...
    this.$listContainer.css({ position: "relative" }).addClass("MegaDynamicList");
  ...
  MegaDynamicList.prototype._viewChanged = function (forceDOMCheck) {
    ...
    if (
      forceDOMCheck ||
      this._calculated["firstItemPos"] !== this._lastFirstPos ||
      this._calculated["lastItemPos"] !== this._lastLastPos ||
      this._calculated["firstItem"] !== this._lastFirstItem ||
      this._calculated["lastItem"] !== this._lastLastItem
    ) {
