...
})()(
  {
    1: [
      function (require, module, exports) {
        var inherits = require("/bundle/utils").inherit,
        ...
        function AudioStream(file, mediaElem, opts) {
          ...
          (self._fileStream = fileStream).on(
            "data",
            tryCatch(function (data) {
              ...
              if (opts.partial) {
                ...
                var read = self._bytesRead || self._buffer.byteLength,
                  size = Math.min(file.fetchChunkSize << 1, AudioStream.maxChunkSize);
                if (read > size) {
          ...
          self._onPlay = function () {
            self._audioStream &&
              self._pauseOffset &&
              self._play(self._pauseOffset >= (self._audioBuffer || !1).duration ? 0 : self._pauseOffset);
          };
          ...
          if (self._buffer && !self._decoding) {
            ...
            self._audioContext
              ...
              .finally(function () {
                ...
                var c,
                  b,
                  again = !done && self._complete;
                !again &&
                  self._buffer &&
                  ((c = self._buffer), (again = ((b = self._bytesRead) > 0 ? Math.min(b, c.byteLength) : c.byteLength) !== len));
        ...
        AudioStream.prototype._setup = function (autoplay, time, buffer) {
          var elm = this._elem;
          if (this.destroyed) log("This instance has been destroyed.", [this]);
          else {
            ...
            if (this._audioBuffer) {
              ...
              var audioContext = this._audioContext,
                ...
                videoStream = this._captureStream(videoCanvas),
                tracks = [audioStream.stream.getTracks()[0], videoStream.getTracks()[0]],
        ...
        AudioStream.prototype._stop = function () {
          var audioSource = this._audioSource,
            visualiser = this._visualiser;
          if (audioSource)
            try {
        ...
        Object.defineProperty(AudioStream.prototype, "currentTime", {
          get: function () {
            ...
            if (result >= (this._audioBuffer || !1).duration)
              if (this.ready) this._elem.pause();
              else {
                this._onPause();
        ...
        AudioVisualiser.prototype._draw = function () {
          var self = this,
            tick = ++self._tick,
            stream = self._stream,
            ctx = stream._videoContext,
            canvas = stream._videoCanvas,
            analyser = stream._audioAnalyser,
            $video = $(stream._elem).parent();
        ...
        Visualiser.prototype.draw = function (ctx, width, height) {
          ...
          if (this._hasFocus) {
            var data = this._byteData,
              analyser = this._stream._audioAnalyser;
        ...
        Star.prototype.drawStar = function () {
          var distanceFromCentre = Math.sqrt(Math.pow(this.x, 2) + Math.pow(this.y, 2)),
            brightness = 200 + Math.min(Math.round(5 * this.high), 55);
          this.ctx.lineWidth = 0.5 + (distanceFromCentre / 2e3) * Math.max(this.starSize / 2, 1);
          this.ctx.strokeStyle = "rgba(" + brightness + ", " + brightness + ", " + brightness + ", 0.8)";
          this.ctx.beginPath();
          this.ctx.moveTo(this.x, this.y);
          var lengthFactor =
              1 + Math.min(((Math.pow(distanceFromCentre, 2) / 3e4) * Math.pow(this.parent.volume, 2)) / 6e6, distanceFromCentre / 16),
            toX = Math.cos(this.angle) * -lengthFactor,
            toY = Math.sin(this.angle) * -lengthFactor;
      ...
      { "/bundle/utils": 4, buffer: 10, eventemitter3: 15, "promise-decode-audio-data": 26 }
    ],
    2: [
      function (require, module, exports) {
        var bcp47iso639 = {
        ...
        ns.decodeMP4LangCode = function (code) {
          return String.fromCharCode.apply(null, [96 + ((code >> 10) & 31), 96 + ((code >> 5) & 31), 96 + (31 & code)]);
        };
        ns.bcp47match = function (lang) {
          "number" === typeof lang && (lang = ns.decodeMP4LangCode(lang));
          if ((lang = bcp47iso639[lang]) && lang === (localStorage.vslang || $.vslang)) return lang;
          for (
            var locales = ["nn-" + mega.ipcc, mega.intl.locale, navigator.language].concat(navigator.languages), i = 0;
            i < locales.length;
            ++i
          ) {
            var locale = String(locales[i]).split("-");
            if (locale[1] && String(locale[1]).toLowerCase() === lang) return lang;
            if (lang && lang === locale[0]) return lang;
          }
          return !1;
        };
        module.exports = ns;
    ...
    3: [
      function (require, module, exports) {
        ...
        var MAX_CACHE = 0 | localStorage.maxStreamingCache || 524288e3,
          MIN_CACHE = 0 | localStorage.minStreamingCache || 419430400,
          REQUEST_SIZE = 4194304,
          MAX_BUF_SECONDS = 25,
          SIMULATE_RESUME_FROM_STALLED = !1,
          VideoStream = require("../videostream"),
          AudioStream = require("../audiostream"),
          inherits = require("/bundle/utils").inherit,
        ...
        CacheStream.prototype._read = function (size) {
          if (this._file && this._file._vs) {
            ...
            var videoFile = this._file,
              videoStream = videoFile._vs,
              fileSize = videoFile.filesize,
              currentTime = videoStream._elem.currentTime,
              bufTime = currentTime && !(videoFile.seeking || videoFile.streamer.playbackSeeking) && videoStream.bufTime;
            if (bufTime > MAX_BUF_SECONDS) {
              videoFile.throttle = currentTime + bufTime;
              d &&
                console.debug(
                  "[CacheStream._read()] Max buffered seconds reached, throttling until %s (current playback time is %s)...",
              ...
              while (size) {
                ...
                var offset = this.pos - (p = videoFile.cachepos[p]),
                  t = size;
        ...
        VideoFile.prototype.cacheadd = function (pos, data) {
          ...
          if (gap >= 0) console.error("*** Mediacache: Internal error - clash");
          else if ((gap = -gap - 1) < this.cachepos.length && pos + data.byteLength > this.cachepos[gap])
        ...
        VideoFile.prototype.fetcher = function (data, byteOffset, byteLength) {
          return SIMULATE_RESUME_FROM_STALLED && byteOffset > 8 * REQUEST_SIZE
            ? Promise.reject(
                {
                  target: {
                    status: 509
        ...
        VideoFile.prototype.fetch = function (startpos, recycle) {
          var p,
            ...
            self = this,
            pos = startpos;
          if (recycle && this.paused && this.cachesize >= this.minCache && this.canplay)
            d &&
              console.debug(
                "[VideoFile.fetch()] MIN_CACHE reached, will not fetch more data until no longer %s...",
                this.paused ? "paused" : "throttled"
              );
          else if (void 0 !== this.mru) {
        ...
        Streamer.prototype.initTypeGuess = function (data) {
          ...
          var self = this,
            file = new VideoFile(data, self);
          file
            .fetcher(data, 0, 16)
            .then(function (chunk) {
              var dv = new DataView(chunk.buffer),
                long = dv.getUint32(0, !1);
              440786851 === long
                ...
                : 1295270176 === dv.getUint32(8, !1)
                ? (self.options.type = "M4A ")
                : 1463899717 === dv.getUint32(8, !1) && (self.options.type = "Wave");
        ...
        Streamer.prototype.handleEvent = function (ev) {
          ...
          switch (ev.type) {
            ...
            case "stalled":
              ...
              if (!this.isOverQuota && stream instanceof VideoStream)
                if (videoFile.playing) stream.findTimeGAPs() && stream._enqueueForcePump();
            ...
            case "timeupdate":
              if (videoFile.throttle && videoFile.throttle - target.currentTime < MAX_BUF_SECONDS / 3) {
                d &&
                  console.debug(
                    "[Streamer.timeupdate] Throttle threshold %s reached at playback time %s, resuming...",
                    secondsToTime(videoFile.throttle),
          ...
          if (this.evs[ev.type]) {
            if ("error" === ev.type) {
              var mediaError = target.error || !1,
                streamError = Object(this.stream._elemWrapper).detailedError,
              ...
              promise
                .then(function () {
                  if (self.file) {
                    d &&
                      console.debug(
                        "Playing, current time: %s, duration: %s",
        ...
        Streamer.prototype._setActivityTimer = function (force) {
          var self = this;
          (!force && self.stream instanceof AudioStream) ||
            delay(
              "vs:activity-timer",
              function () {
        ...
        Streamer.prototype.getImage = function (w, h) {
          ...
          return new Promise(function _(resolve, reject) {
            if (video.videoWidth) {
              ...
              d &&
                console.debug(
                  "[Streamer.getImage()] Taking %sx%s image from %sx%s at %s",
                  dim.width,
                  dim.height,
                  video.videoWidth,
                  video.videoHeight,
              ...
              if (Math.round((100 * bp) / len) > 70) {
                ...
                video.paused || video.ended || window.safari
        ...
        Streamer.getThumbnail = function (data) {
          return new Promise(function (resolve, reject) {
            ...
            Object.isExtensible(data) &&
              Object.defineProperty(data, "__getVTNPid", {
                value: Streamer.pid
              });
        ...
        Object.defineProperty(Streamer.prototype, "goAudioStream", {
          get: function () {
            ...
            return "M4A " === type
              ? mega.fullAudioContextSupport
              : "MPEG Audio" === type || "Wave" === type || "Ogg" === type || "FLAC" === type;
          }
        });
      ...
      { "../audiostream": 1, "../videostream": 46, "/bundle/utils": 4, buffer: 10, "readable-stream": 41 }
    ],
    4: [
      function (require, module, exports) {
        var Buffer = require("buffer").Buffer,
          cbs = [],
          tbsp = Promise.resolve(),
    ...
    5: [
      function (require, module, exports) {
        ...
        var inherits = require("/bundle/utils").inherit,
          ...
          EventEmitter = require("eventemitter3").EventEmitter,
          EBMLDecoder = require("ebml/lib/ebml/decoder");

        module.exports = EBMLRemuxer;

        var DEBUG = 0 | localStorage.vsd,
          DEBUG_INFO = DEBUG || window.d,
          DEBUG_TRACE = DEBUG > 2,
          DEBUG_VERBOSE = DEBUG > 1;
        ...
        EBMLRemuxer.prototype.emitInitSegment = function (segment) {
          ...
          for (
            ...
            var track = tracks[i],
              codec = String(track.CodecID).replace(/\W/g, ""),
              type = track.TrackType,
              mime = (function (codec, type) {
                switch (codec) {
                  case "A_VORBIS":
                  case "A_OPUS":
                  case "V_VP8":
          ...
          if (segment.Cues) {
            ...
            for (var c = segment.Cues, y = segment.playtime, t = (this._seekTable = []), j = c.length; j--; ) {
              var p = c[j],
                x = p.CueTrackPositions,
                z = Math.round(p.CueTime * segment.timescale * 1e3) / 1e3;
        ...
        EBMLRemuxer.prototype._shrink = function (segment, props) {
          for (var k in props)
            segment[k] && (segment[k] = segment[k][props[k]]) && (Array.isArray(segment[k]) || (segment[k] = [segment[k]]));
        };
        ...
        EBMLReader.prototype._onData = function (state, data) {
          ...
          if ("start" === state) {
            if (!this._byteOffset)
              if ("EBML" === data.name) this._ebmlOffset = data.start;
              else if ("Segment" === data.name) this._segmentOffset = -1;
              else if (this._segmentOffset < 0) this._segmentOffset = data.start;
              else if ("Cluster" === data.name) {
                ...
                if (dd && this._clearDefaultDuration) {
                  ...
                  if (size > 4) console.warn("Unexpected default duration...", dd);
                  else {
            ...
            this._segment[data.name]
              ? Array.isArray(this._segment[data.name])
            ...
            if (data.end - data.start > 0) {
              ...
              if (this.listenerCount(name)) {
                ...
                if ("cluster" === name) this._cluster = chunk;
                else {
                  if ("cues" === name) {
        ...
        EBMLReader.prototype._read = function (start, end) {
          var length = end - start,
            ...
            offset = start % this.$buffer.length,
            left = this.$buffer.length - offset;
          if (length > this.$buffer.length) this.destroy(new Error("ebml:read buffer overflow."));
          else if (length > left) {
        ...
        function MediaSegment(muxer, offset) {
          ...
          this._async(function () {
            self._reader = muxer._createReader(offset, self, "cluster", function (cluster) {
              if (!self.destroyed)
                if (cluster) self.append.apply(self, arguments);
                else {
        ...
        MediaSegment.prototype.append = function (cluster, timecode, duration) {
          ...
          var timecodeOffset = -1,
            cs = cluster.readUInt32BE(4);
          if (16777216 === cs) timecodeOffset = 12;
          else if (cs >> 24 === 255) timecodeOffset = 5;
          else if (DEBUG_TRACE) {
      ...
      { "/bundle/utils": 4, buffer: 10, "ebml/lib/ebml/decoder": 11, eventemitter3: 15, "readable-stream": 41 }
    ],
    6: [
      function (require, module, exports) {
        var bs = require("binary-search"),
          EventEmitter = require("eventemitter3").EventEmitter,
          inherits = require("/bundle/utils").inherit,
          mp4 = require("mp4-stream"),
          Box = require("mp4-box-encoding"),
          RangeSliceStream = require("range-slice-stream"),
          Buffer = require("buffer").Buffer,
          locale = require("./bundle/locale"),
          FIND_MOOV_SEEK_SIZE = 4096;
        ...
        MP4Remuxer.prototype._findMoov = function (offset) {
          ...
          if (file.filesize >= 0 && offset >= file.filesize)
            return self._moov ? self._parseMoov() : self.emit("error", RangeError("Offset out of bound."));
          var fileStream = file.createReadStream({
            start: offset
          });
        ...
        MP4Remuxer.prototype._processTracks = function (traks) {
          ...
          for (
            var mime,
              ...
              self = this,
              audio = [],
              hevc = Object.assign(Object.create(null), {
                hvc1: 1,
                hev1: 1
              }),
            ...
            var trak = traks[i],
              mdia = trak.mdia || !1,
              stsd = trak.mdia.minf.stbl.stsd.entries[0],
              lang = Object(mdia.mdhd).language,
              type = Object(mdia.hdlr).handlerType;
            if ("vide" === type && vide[stsd.type]) {
              ...
              stsd.avcC
                ...
                : stsd.hvcC && (codec += stsd.hvcC.mimeCodec);
          ...
          if (audio.length < 2) self._hasAudio = validateAudioTrack(audio[0]);
          else {
        ...
        MP4Remuxer.prototype._processMoov = function (moov) {
          ...
          d &&
            console.debug(
              ...
              0 | this._hasAudio.idx,
              this._hasAudio.codec || "N/A"
            );
          for (var i = 0; i < traks.length; i++) {
            var mime,
              trak = traks[i],
              stbl = trak.mdia.minf.stbl,
              stco = stbl.stco || stbl.co64;
            ...
            var currChunkEntry,
              ...
              dts = 0,
              decodingTimeEntry = new RunLengthIndex(stbl.stts.entries),
              presentationOffsetEntry = null;
            ...
            if (decodingTimeEntry.value) {
              while (1) {
                ...
                var size = stbl.stsz.sample_size || stbl.stsz.entries[sample],
                  duration = (decodingTimeEntry.value && decodingTimeEntry.value.duration) || 0,
                  sync = !0;
          ...
          if (0 !== this._tracks.length) {
            ...
            var ftypBuf = Box.encode(this._ftyp),
              data = this._tracks.map(function (track) {
        ...
        MP4Remuxer.prototype._writeFragment = function (fragment, i) {
          function wrap(cb) {
            return function (err) {
              err
                ? self.emit("error", err)
                : outStream.destroyed || inStream.destroyed
                ? d > 1 && console.debug("writeFragment stream destroyed", outStream.destroyed, inStream.destroyed)
                : cb();
            };
          }
          var self = this,
            track = self._tracks[i],
            inStream = track.inStream,
            outStream = track.outStream,
            parser = wrap(function () {
              if (!(fragment = self._generateFragment(i))) return outStream.finalize();
              writeFragment();
            }),
            boxer = wrap(function () {
              var slicedStream = inStream.slice(fragment.ranges),
                mediaDataStream = outStream.mediaData(fragment.length, parser);
        ...
        MP4Remuxer.prototype.seek = function (time) {
          ...
          return self._tracks.map(function (track, i) {
            ...
            if (track.fragment) {
              ...
              self._fileStream ||
                (self._fileStream = self._file.createReadStream({
                  start: startOffset
                }));
        ...
        MP4Remuxer.prototype._findSampleBefore = function (trackId, time) {
          var track = this._tracks[trackId],
            scaledTime = Math.floor(track.timeScale * time),
            sample = bs(track.samples, scaledTime, function (sample, t) {
        ...
        MP4Remuxer.prototype._generateFragment = function (track, time) {
          var currTrack = this._tracks[track],
            firstSample = void 0 !== time ? this._findSampleBefore(track, time) : currTrack.currSample;
          if (firstSample >= currTrack.samples.length) return null;
          for (
            var startDts = currTrack.samples[firstSample].dts, totalLen = 0, ranges = [], currSample = firstSample;
            ...
            currRange < 0 || ranges[currRange].end !== sample.offset
              ? ranges.push({
                  start: sample.offset,
        ...
        Box.boxes.co64.decode = function (buf, offset) {
          for (var num = (buf = buf.slice(offset)).readUInt32BE(0), entries = new Array(num), i = 0; i < num; i++) {
            var pos = 8 * i + 4,
              hi = buf.readUInt32BE(pos),
              lo = buf.readUInt32BE(4 + pos);
        ...
        Box.boxes.av1C.decode = function (buf, offset, end) {
          ...
          if ((tmp >> 7) & !1) console.warn("Invalid av1C marker.");
          else if (1 !== this.version) console.warn("Unsupported av1C version %d.", this.version);
          else {
            tmp = readUint8();
            ...
            r.mimeCodec = [
              "av01",
              r.seq_profile,
              ("0" + r.seq_level_idx_0).slice(-2) + (r.seq_tier_0 ? "H" : "M"),
              ("0" + tmp).slice(-2)
            ].join(".");
          }
          return r;
        ...
        Box.boxes.vpcC.decode = function (buf, offset, end) {
          ...
          var tmp = readUint8();
          this.version = 127 & tmp;
          if (1 !== this.version) console.warn("Unsupported/deprecated vpcC version %d.", this.version);
          else {
            p += 3;
            r.profile = readUint8();
            r.level = readUint8();
        ...
        Box.boxes.hvcC.decode = function (buf, offset, end) {
          ...
          mime += r.profile_idc + ".";
          var j = (tmp = 0),
            cpl = r.profile_compatibility_indications;
          while (1) {
            tmp = 1 & cpl;
            if (++j > 30) break;
            tmp <<= 1;
        ...
        Box.boxes.stsz.decode = function (buf, offset) {
          var size = (buf = buf.slice(offset)).readUInt32BE(0),
    ...
    7: [
      function (require, module, exports) {
        var AudioContext = window.AudioContext || window.webkitAudioContext,
          ...
          BaseAudioContext = window.BaseAudioContext || (OfflineAudioContext && Object.getPrototypeOf(OfflineAudioContext));
    ...
    8: [
      function (require, module, exports) {
        ...
        function encodeChunk(uint8, start, end) {
          for (var output = [], i = start; i < end; i += 3)
            output.push(tripletToBase64(((uint8[i] << 16) & 16711680) + ((uint8[i + 1] << 8) & 65280) + (255 & uint8[i + 2])));
          return output.join("");
        }
        ...
        function fromByteArray(uint8) {
          ...
          1 === extraBytes
            ...
            : 2 === extraBytes &&
              parts.push(
                lookup[(tmp = (uint8[len - 2] << 8) + uint8[len - 1]) >> 10] + lookup[(tmp >> 4) & 63] + lookup[(tmp << 2) & 63] + "="
              );
          return parts.join("");
        }
      },
    ...
    9: [
      function (require, module, exports) {
        module.exports = function (haystack, needle, comparator, low, high) {
          var mid, cmp;
          if (void 0 === low) low = 0;
          else if ((low |= 0) < 0 || low >= haystack.length) throw new RangeError("invalid lower bound");
          if (void 0 === high) high = haystack.length - 1;
          else if ((high |= 0) < low || high >= haystack.length) throw new RangeError("invalid upper bound");
          while (low <= high)
            if ((cmp = +comparator(haystack[(mid = low + ((high - low) >>> 1))], needle, mid, haystack)) < 0) low = mid + 1;
            else {
              if (!(cmp > 0)) return mid;
              high = mid - 1;
            }
    ...
    10: [
      function (require, module, exports) {
        ...
        function fromArrayLike(array) {
          for (var length = array.length < 0 ? 0 : 0 | array.length, buf = createBuffer(length), i = 0; i < length; i += 1)
            buf[i] = 255 & array[i];
          return buf;
        }
              ...
              switch (bytesPerSequence) {
                ...
                case 2:
                  128 === (192 & (secondByte = buf[i + 1])) &&
                    (tempCodePoint = ((31 & firstByte) << 6) | (63 & secondByte)) > 127 &&
        ...
        Buffer.prototype.readUIntBE = function (offset, byteLength) {
          byteLength >>>= 0;
          var val = this[(offset >>>= 0) + --byteLength],
            mul = 1;
        ...
        Buffer.prototype.fill = function (val, start, end, encoding) {
          ...
          var i;
          if ("number" === typeof (val = val || 0)) for (i = start; i < end; ++i) this[i] = val;
          else {
    ...
    11: [
      function (require, module, exports) {
        ...
        EbmlDecoder.prototype.readTag = function () {
          ...
          var start = this._total,
            tag = tools.readVint(this._buffer, this._cursor);
      ...
      { "./schema.js": 12, "./tools.js": 13, "/bundle/utils": 4, buffer: 10, "readable-stream": 41 }
    ],
    12: [
      function (require, module, exports) {
        var schema = {
          80: { name: "ChapterDisplay", level: "4", type: "m", multiple: "1", minver: "1", webm: "1" },
          83: { name: "TrackType", level: "3", type: "u", mandatory: "1", minver: "1", range: "1-254" },
          85: { name: "ChapString", level: "5", type: "8", mandatory: "1", minver: "1", webm: "1" },
          86: { name: "CodecID", level: "3", type: "s", mandatory: "1", minver: "1" },
          88: { name: "FlagDefault", level: "3", type: "u", mandatory: "1", minver: "1", default: "1", range: "0-1" },
          89: { name: "ChapterTrackNumber", level: "5", type: "u", mandatory: "1", multiple: "1", minver: "1", webm: "0", range: "not 0" },
          91: { name: "ChapterTimeStart", level: "4", type: "u", mandatory: "1", minver: "1", webm: "1" },
          92: { name: "ChapterTimeEnd", level: "4", type: "u", minver: "1", webm: "0" },
          96: { name: "CueRefTime", level: "5", type: "u", mandatory: "1", minver: "2", webm: "0" },
          97: { name: "CueRefCluster", level: "5", type: "u", mandatory: "1", webm: "0" },
          98: { name: "ChapterFlagHidden", level: "4", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0", range: "0-1" },
          4254: {
          ...
          4255: { name: "ContentCompSettings", level: "6", type: "b", minver: "1", webm: "0" },
          4282: { name: "DocType", level: "1", type: "s", mandatory: "1", default: "matroska", minver: "1" },
          4285: { name: "DocTypeReadVersion", level: "1", type: "u", mandatory: "1", default: "1", minver: "1" },
          4286: { name: "EBMLVersion", level: "1", type: "u", mandatory: "1", default: "1", minver: "1" },
          4287: { name: "DocTypeVersion", level: "1", type: "u", mandatory: "1", default: "1", minver: "1" },
          4444: { name: "SegmentFamily", level: "2", type: "b", multiple: "1", minver: "1", webm: "0", bytesize: "16" },
          4461: { name: "DateUTC", level: "2", type: "d", minver: "1" },
          4484: { name: "TagDefault", level: "4", type: "u", mandatory: "1", minver: "1", webm: "0", default: "1", range: "0-1" },
          4485: { name: "TagBinary", level: "4", type: "b", minver: "1", webm: "0" },
          4487: { name: "TagString", level: "4", type: "8", minver: "1", webm: "0" },
          4489: { name: "Duration", level: "2", type: "f", minver: "1", range: "> 0" },
          4598: { name: "ChapterFlagEnabled", level: "4", type: "u", mandatory: "1", minver: "1", webm: "0", default: "1", range: "0-1" },
          4660: { name: "FileMimeType", level: "3", type: "s", mandatory: "1", minver: "1", webm: "0" },
          4661: { name: "FileUsedStartTime", level: "3", type: "u", divx: "1" },
          4662: { name: "FileUsedEndTime", level: "3", type: "u", divx: "1" },
          4675: { name: "FileReferral", level: "3", type: "b", webm: "0" },
          5031: { name: "ContentEncodingOrder", level: "5", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0" },
          5032: {
          ...
          5033: { name: "ContentEncodingType", level: "5", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0", br: ["", ""] },
          5034: { name: "ContentCompression", level: "5", type: "m", minver: "1", webm: "0" },
          5035: { name: "ContentEncryption", level: "5", type: "m", minver: "1", webm: "0" },
          5378: { name: "CueBlockNumber", level: "4", type: "u", minver: "1", default: "1", range: "not 0" },
          5654: { name: "ChapterStringUID", level: "4", type: "8", mandatory: "0", minver: "3", webm: "1" },
          5741: { name: "WritingApp", level: "2", type: "8", mandatory: "1", minver: "1" },
          5854: { name: "SilentTracks", level: "2", type: "m", minver: "1", webm: "0" },
          6240: { name: "ContentEncoding", level: "4", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "0" },
          6264: { name: "BitDepth", level: "4", type: "u", minver: "1", range: "not 0" },
          6532: { name: "SignedElement", level: "3", type: "b", multiple: "1", webm: "0" },
          6624: { name: "TrackTranslate", level: "3", type: "m", multiple: "1", minver: "1", webm: "0" },
          6911: { name: "ChapProcessCommand", level: "5", type: "m", multiple: "1", minver: "1", webm: "0" },
          6922: { name: "ChapProcessTime", level: "6", type: "u", mandatory: "1", minver: "1", webm: "0" },
          6924: { name: "ChapterTranslate", level: "2", type: "m", multiple: "1", minver: "1", webm: "0" },
          6933: { name: "ChapProcessData", level: "6", type: "b", mandatory: "1", minver: "1", webm: "0" },
          6944: { name: "ChapProcess", level: "4", type: "m", multiple: "1", minver: "1", webm: "0" },
          6955: { name: "ChapProcessCodecID", level: "5", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0" },
          7373: { name: "Tag", level: "2", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "0" },
          7384: { name: "SegmentFilename", level: "2", type: "8", minver: "1", webm: "0" },
          7446: { name: "AttachmentLink", level: "3", type: "u", minver: "1", webm: "0", range: "not 0" },
          258688: { name: "CodecName", level: "3", type: "8", minver: "1" },
          18538067: { name: "Segment", level: "0", type: "m", mandatory: "1", multiple: "1", minver: "1" },
          "447a": { name: "TagLanguage", level: "4", type: "s", mandatory: "1", minver: "1", webm: "0", default: "und" },
          "45a3": { name: "TagName", level: "4", type: "8", mandatory: "1", minver: "1", webm: "0" },
          "67c8": { name: "SimpleTag", level: "3", recursive: "1", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "0" },
          "63c6": { name: "TagAttachmentUID", level: "4", type: "u", multiple: "1", minver: "1", webm: "0", default: "0" },
          "63c4": { name: "TagChapterUID", level: "4", type: "u", multiple: "1", minver: "1", webm: "0", default: "0" },
          "63c9": { name: "TagEditionUID", level: "4", type: "u", multiple: "1", minver: "1", webm: "0", default: "0" },
          "63c5": { name: "TagTrackUID", level: "4", type: "u", multiple: "1", minver: "1", webm: "0", default: "0" },
          "63ca": { name: "TargetType", level: "4", type: "s", minver: "1", webm: "0", strong: "informational" },
          "68ca": { name: "TargetTypeValue", level: "4", type: "u", minver: "1", webm: "0", default: "50" },
          "63c0": { name: "Targets", level: "3", type: "m", mandatory: "1", minver: "1", webm: "0" },
          "1254c367": { name: "Tags", level: "1", type: "m", multiple: "1", minver: "1", webm: "0" },
          "450d": { name: "ChapProcessPrivate", level: "5", type: "b", minver: "1", webm: "0" },
          "437e": { name: "ChapCountry", level: "5", type: "s", multiple: "1", minver: "1", webm: "0" },
          "437c": { name: "ChapLanguage", level: "5", type: "s", mandatory: "1", multiple: "1", minver: "1", webm: "1", default: "eng" },
          "8f": { name: "ChapterTrack", level: "4", type: "m", minver: "1", webm: "0" },
          "63c3": { name: "ChapterPhysicalEquiv", level: "4", type: "u", minver: "1", webm: "0" },
          "6ebc": { name: "ChapterSegmentEditionUID", level: "4", type: "u", minver: "1", webm: "0", range: "not 0" },
          "6e67": { name: "ChapterSegmentUID", level: "4", type: "b", minver: "1", webm: "0", range: ">0", bytesize: "16" },
          "73c4": { name: "ChapterUID", level: "4", type: "u", mandatory: "1", minver: "1", webm: "1", range: "not 0" },
          b6: { name: "ChapterAtom", level: "3", recursive: "1", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "1" },
          "45dd": { name: "EditionFlagOrdered", level: "3", type: "u", minver: "1", webm: "0", default: "0", range: "0-1" },
          "45db": { name: "EditionFlagDefault", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0", range: "0-1" },
          "45bd": { name: "EditionFlagHidden", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0", range: "0-1" },
          "45bc": { name: "EditionUID", level: "3", type: "u", minver: "1", webm: "0", range: "not 0" },
          "45b9": { name: "EditionEntry", level: "2", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "1" },
          "1043a770": { name: "Chapters", level: "1", type: "m", minver: "1", webm: "1" },
          "46ae": { name: "FileUID", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0", range: "not 0" },
          "465c": { name: "FileData", level: "3", type: "b", mandatory: "1", minver: "1", webm: "0" },
          "466e": { name: "FileName", level: "3", type: "8", mandatory: "1", minver: "1", webm: "0" },
          "467e": { name: "FileDescription", level: "3", type: "8", minver: "1", webm: "0" },
          "61a7": { name: "AttachedFile", level: "2", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "0" },
          "1941a469": { name: "Attachments", level: "1", type: "m", minver: "1", webm: "0" },
          eb: { name: "CueRefCodecState", level: "5", type: "u", webm: "0", default: "0" },
          "535f": { name: "CueRefNumber", level: "5", type: "u", webm: "0", default: "1", range: "not 0" },
          db: { name: "CueReference", level: "4", type: "m", multiple: "1", minver: "2", webm: "0" },
          ea: { name: "CueCodecState", level: "4", type: "u", minver: "2", webm: "0", default: "0" },
          b2: { name: "CueDuration", level: "4", type: "u", mandatory: "0", minver: "4", webm: "0" },
          f0: { name: "CueRelativePosition", level: "4", type: "u", mandatory: "0", minver: "4", webm: "0" },
          f1: { name: "CueClusterPosition", level: "4", type: "u", mandatory: "1", minver: "1" },
          f7: { name: "CueTrack", level: "4", type: "u", mandatory: "1", minver: "1", range: "not 0" },
          b7: { name: "CueTrackPositions", level: "3", type: "m", mandatory: "1", multiple: "1", minver: "1" },
          b3: { name: "CueTime", level: "3", type: "u", mandatory: "1", minver: "1" },
          bb: { name: "CuePoint", level: "2", type: "m", mandatory: "1", multiple: "1", minver: "1" },
          "1c53bb6b": { name: "Cues", level: "1", type: "m", minver: "1" },
          "47e6": { name: "ContentSigHashAlgo", level: "6", type: "u", minver: "1", webm: "0", default: "0", br: ["", ""] },
          "47e5": { name: "ContentSigAlgo", level: "6", type: "u", minver: "1", webm: "0", default: "0", br: "" },
          "47e4": { name: "ContentSigKeyID", level: "6", type: "b", minver: "1", webm: "0" },
          "47e3": { name: "ContentSignature", level: "6", type: "b", minver: "1", webm: "0" },
          "47e2": { name: "ContentEncKeyID", level: "6", type: "b", minver: "1", webm: "0" },
          "47e1": { name: "ContentEncAlgo", level: "6", type: "u", minver: "1", webm: "0", default: "0", br: "" },
          "6d80": { name: "ContentEncodings", level: "3", type: "m", minver: "1", webm: "0" },
          c4: { name: "TrickMasterTrackSegmentUID", level: "3", type: "b", divx: "1", bytesize: "16" },
          c7: { name: "TrickMasterTrackUID", level: "3", type: "u", divx: "1" },
          c6: { name: "TrickTrackFlag", level: "3", type: "u", divx: "1", default: "0" },
          c1: { name: "TrickTrackSegmentUID", level: "3", type: "b", divx: "1", bytesize: "16" },
          c0: { name: "TrickTrackUID", level: "3", type: "u", divx: "1" },
          ed: { name: "TrackJoinUID", level: "5", type: "u", mandatory: "1", multiple: "1", minver: "3", webm: "0", range: "not 0" },
          e9: { name: "TrackJoinBlocks", level: "4", type: "m", minver: "3", webm: "0" },
          e6: { name: "TrackPlaneType", level: "6", type: "u", mandatory: "1", minver: "3", webm: "0" },
          e5: { name: "TrackPlaneUID", level: "6", type: "u", mandatory: "1", minver: "3", webm: "0", range: "not 0" },
          e4: { name: "TrackPlane", level: "5", type: "m", mandatory: "1", multiple: "1", minver: "3", webm: "0" },
          e3: { name: "TrackCombinePlanes", level: "4", type: "m", minver: "3", webm: "0" },
          e2: { name: "TrackOperation", level: "3", type: "m", minver: "3", webm: "0" },
          "7d7b": { name: "ChannelPositions", level: "4", type: "b", webm: "0" },
          "9f": { name: "Channels", level: "4", type: "u", mandatory: "1", minver: "1", default: "1", range: "not 0" },
          "78b5": { name: "OutputSamplingFrequency", level: "4", type: "f", minver: "1", default: "Sampling Frequency", range: "> 0" },
          b5: { name: "SamplingFrequency", level: "4", type: "f", mandatory: "1", minver: "1", default: "8000.0", range: "> 0" },
          e1: { name: "Audio", level: "3", type: "m", minver: "1" },
          "2383e3": { name: "FrameRate", level: "4", type: "f", range: "> 0", strong: "Informational" },
          "2fb523": { name: "GammaValue", level: "4", type: "f", webm: "0", range: "> 0" },
          "2eb524": { name: "ColourSpace", level: "4", type: "b", minver: "1", webm: "0", bytesize: "4" },
          "54b3": { name: "AspectRatioType", level: "4", type: "u", minver: "1", default: "0" },
          "54b2": { name: "DisplayUnit", level: "4", type: "u", minver: "1", default: "0" },
          "54ba": { name: "DisplayHeight", level: "4", type: "u", minver: "1", default: "PixelHeight", range: "not 0" },
          "54b0": { name: "DisplayWidth", level: "4", type: "u", minver: "1", default: "PixelWidth", range: "not 0" },
          "54dd": { name: "PixelCropRight", level: "4", type: "u", minver: "1", default: "0" },
          "54cc": { name: "PixelCropLeft", level: "4", type: "u", minver: "1", default: "0" },
          "54bb": { name: "PixelCropTop", level: "4", type: "u", minver: "1", default: "0" },
          "54aa": { name: "PixelCropBottom", level: "4", type: "u", minver: "1", default: "0" },
          ba: { name: "PixelHeight", level: "4", type: "u", mandatory: "1", minver: "1", range: "not 0" },
          b0: { name: "PixelWidth", level: "4", type: "u", mandatory: "1", minver: "1", range: "not 0" },
          "53b9": { name: "OldStereoMode", level: "4", type: "u", maxver: "0", webm: "0", divx: "0" },
          "53c0": { name: "AlphaMode", level: "4", type: "u", minver: "3", webm: "1", default: "0" },
          "53b8": { name: "StereoMode", level: "4", type: "u", minver: "3", webm: "1", default: "0" },
          "9a": { name: "FlagInterlaced", level: "4", type: "u", mandatory: "1", minver: "2", webm: "1", default: "0", range: "0-1" },
          e0: { name: "Video", level: "3", type: "m", minver: "1" },
          "66a5": { name: "TrackTranslateTrackID", level: "4", type: "b", mandatory: "1", minver: "1", webm: "0" },
          "66bf": { name: "TrackTranslateCodec", level: "4", type: "u", mandatory: "1", minver: "1", webm: "0" },
          "66fc": { name: "TrackTranslateEditionUID", level: "4", type: "u", multiple: "1", minver: "1", webm: "0" },
          "56bb": { name: "SeekPreRoll", level: "3", type: "u", mandatory: "1", multiple: "0", default: "0", minver: "4", webm: "1" },
          "56aa": { name: "CodecDelay", level: "3", type: "u", multiple: "0", default: "0", minver: "4", webm: "1" },
          "6fab": { name: "TrackOverlay", level: "3", type: "u", multiple: "1", minver: "1", webm: "0" },
          aa: { name: "CodecDecodeAll", level: "3", type: "u", mandatory: "1", minver: "2", webm: "0", default: "1", range: "0-1" },
          "26b240": { name: "CodecDownloadURL", level: "3", type: "s", multiple: "1", webm: "0" },
          "3b4040": { name: "CodecInfoURL", level: "3", type: "s", multiple: "1", webm: "0" },
          "3a9697": { name: "CodecSettings", level: "3", type: "8", webm: "0" },
          "63a2": { name: "CodecPrivate", level: "3", type: "b", minver: "1" },
          "22b59c": { name: "Language", level: "3", type: "s", minver: "1", default: "eng" },
          "536e": { name: "Name", level: "3", type: "8", minver: "1" },
          "55ee": { name: "MaxBlockAdditionID", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0" },
          "537f": { name: "TrackOffset", level: "3", type: "i", webm: "0", default: "0" },
          "23314f": {
          ...
          "234e7a": { name: "DefaultDecodedFieldDuration", level: "3", type: "u", minver: "4", range: "not 0" },
          "23e383": { name: "DefaultDuration", level: "3", type: "u", minver: "1", range: "not 0" },
          "6df8": { name: "MaxCache", level: "3", type: "u", minver: "1", webm: "0" },
          "6de7": { name: "MinCache", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0" },
          "9c": { name: "FlagLacing", level: "3", type: "u", mandatory: "1", minver: "1", default: "1", range: "0-1" },
          "55aa": { name: "FlagForced", level: "3", type: "u", mandatory: "1", minver: "1", default: "0", range: "0-1" },
          b9: { name: "FlagEnabled", level: "3", type: "u", mandatory: "1", minver: "2", webm: "1", default: "1", range: "0-1" },
          "73c5": { name: "TrackUID", level: "3", type: "u", mandatory: "1", minver: "1", range: "not 0" },
          d7: { name: "TrackNumber", level: "3", type: "u", mandatory: "1", minver: "1", range: "not 0" },
          ae: { name: "TrackEntry", level: "2", type: "m", mandatory: "1", multiple: "1", minver: "1" },
          "1654ae6b": { name: "Tracks", level: "1", type: "m", multiple: "1", minver: "1" },
          af: { name: "EncryptedBlock", level: "2", type: "b", multiple: "1", webm: "0" },
          ca: { name: "ReferenceTimeCode", level: "4", type: "u", multiple: "0", mandatory: "1", minver: "0", webm: "0", divx: "1" },
          c9: { name: "ReferenceOffset", level: "4", type: "u", multiple: "0", mandatory: "1", minver: "0", webm: "0", divx: "1" },
          c8: { name: "ReferenceFrame", level: "3", type: "m", multiple: "0", minver: "0", webm: "0", divx: "1" },
          cf: { name: "SliceDuration", level: "5", type: "u", default: "0" },
          ce: { name: "Delay", level: "5", type: "u", default: "0" },
          cb: { name: "BlockAdditionID", level: "5", type: "u", default: "0" },
          cd: { name: "FrameNumber", level: "5", type: "u", default: "0" },
          cc: { name: "LaceNumber", level: "5", type: "u", minver: "1", default: "0", divx: "0" },
          e8: { name: "TimeSlice", level: "4", type: "m", multiple: "1", minver: "1", divx: "0" },
          "8e": { name: "Slices", level: "3", type: "m", minver: "1", divx: "0" },
          "75a2": { name: "DiscardPadding", level: "3", type: "i", minver: "4", webm: "1" },
          a4: { name: "CodecState", level: "3", type: "b", minver: "2", webm: "0" },
          fd: { name: "ReferenceVirtual", level: "3", type: "i", webm: "0" },
          fb: { name: "ReferenceBlock", level: "3", type: "i", multiple: "1", minver: "1" },
          fa: { name: "ReferencePriority", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0", default: "0" },
          "9b": { name: "BlockDuration", level: "3", type: "u", minver: "1", default: "TrackDuration" },
          a5: { name: "BlockAdditional", level: "5", type: "b", mandatory: "1", minver: "1", webm: "0" },
          ee: { name: "BlockAddID", level: "5", type: "u", mandatory: "1", minver: "1", webm: "0", default: "1", range: "not 0" },
          a6: { name: "BlockMore", level: "4", type: "m", mandatory: "1", multiple: "1", minver: "1", webm: "0" },
          "75a1": { name: "BlockAdditions", level: "3", type: "m", minver: "1", webm: "0" },
          a2: { name: "BlockVirtual", level: "3", type: "b", webm: "0" },
          a1: { name: "Block", level: "3", type: "b", mandatory: "1", minver: "1" },
          a0: { name: "BlockGroup", level: "2", type: "m", multiple: "1", minver: "1" },
          a3: { name: "SimpleBlock", level: "2", type: "b", multiple: "1", minver: "2", webm: "1", divx: "1" },
          ab: { name: "PrevSize", level: "2", type: "u", minver: "1" },
          a7: { name: "Position", level: "2", type: "u", minver: "1", webm: "0" },
          "58d7": { name: "SilentTrackNumber", level: "3", type: "u", multiple: "1", minver: "1", webm: "0" },
          e7: { name: "Timecode", level: "2", type: "u", mandatory: "1", minver: "1" },
          "1f43b675": { name: "Cluster", level: "1", type: "m", multiple: "1", minver: "1" },
          "4d80": { name: "MuxingApp", level: "2", type: "8", mandatory: "1", minver: "1" },
          "7ba9": { name: "Title", level: "2", type: "8", minver: "1", webm: "0" },
          "2ad7b2": { name: "TimecodeScaleDenominator", level: "2", type: "u", mandatory: "1", minver: "4", default: "1000000000" },
          "2ad7b1": { name: "TimecodeScale", level: "2", type: "u", mandatory: "1", minver: "1", default: "1000000" },
          "69a5": { name: "ChapterTranslateID", level: "3", type: "b", mandatory: "1", minver: "1", webm: "0" },
          "69bf": { name: "ChapterTranslateCodec", level: "3", type: "u", mandatory: "1", minver: "1", webm: "0" },
          "69fc": { name: "ChapterTranslateEditionUID", level: "3", type: "u", multiple: "1", minver: "1", webm: "0" },
          "3e83bb": { name: "NextFilename", level: "2", type: "8", minver: "1", webm: "0" },
          "3eb923": { name: "NextUID", level: "2", type: "b", minver: "1", webm: "0", bytesize: "16" },
          "3c83ab": { name: "PrevFilename", level: "2", type: "8", minver: "1", webm: "0" },
          "3cb923": { name: "PrevUID", level: "2", type: "b", minver: "1", webm: "0", bytesize: "16" },
          "73a4": { name: "SegmentUID", level: "2", type: "b", minver: "1", webm: "0", range: "not 0", bytesize: "16" },
          "1549a966": { name: "Info", level: "1", type: "m", mandatory: "1", multiple: "1", minver: "1" },
          "53ac": { name: "SeekPosition", level: "3", type: "u", mandatory: "1", minver: "1" },
          "53ab": { name: "SeekID", level: "3", type: "b", mandatory: "1", minver: "1" },
          "4dbb": { name: "Seek", level: "2", type: "m", mandatory: "1", multiple: "1", minver: "1" },
          "114d9b74": { name: "SeekHead", level: "1", type: "m", multiple: "1", minver: "1" },
          "7e7b": { name: "SignatureElementList", level: "2", type: "m", multiple: "1", webm: "0", i: "Cluster|Block|BlockAdditional" },
          "7e5b": { name: "SignatureElements", level: "1", type: "m", webm: "0" },
          "7eb5": { name: "Signature", level: "1", type: "b", webm: "0" },
          "7ea5": { name: "SignaturePublicKey", level: "1", type: "b", webm: "0" },
          "7e9a": { name: "SignatureHash", level: "1", type: "u", webm: "0" },
          "7e8a": { name: "SignatureAlgo", level: "1", type: "u", webm: "0" },
          "1b538667": { name: "SignatureSlot", level: "-1", type: "m", multiple: "1", webm: "0" },
          bf: { name: "CRC-32", level: "-1", type: "b", minver: "1", webm: "0" },
          ec: { name: "Void", level: "-1", type: "b", minver: "1" },
          "42f3": { name: "EBMLMaxSizeLength", level: "1", type: "u", mandatory: "1", default: "8", minver: "1" },
          "42f2": { name: "EBMLMaxIDLength", level: "1", type: "u", mandatory: "1", default: "4", minver: "1" },
          "42f7": { name: "EBMLReadVersion", level: "1", type: "u", mandatory: "1", default: "1", minver: "1" },
          "1a45dfa3": { name: "EBML", level: "0", type: "m", mandatory: "1", multiple: "1", minver: "1" }
        };
          ...
          tools = {
            readVint: function (buffer, start) {
              ...
              for (var value = buffer[start] & ((1 << (8 - length)) - 1), i = 1; i < length; i++) {
                if (7 === i && value >= Math.pow(2, 45) && buffer[start + 7] > 0)
                  return {
                    length: length,
                    value: -1
                  };
        ...
        var once = require("once"),
          ...
          eos = function (stream, opts, callback) {
            ...
            function onclosenexttick() {
              if (!cancelled)
                return (!readable || (rs && rs.ended)) && (!writable || (ws && ws.ended && !ws.destroyed))
                  ? void 0
    ...
    15: [
      function (require, module, exports) {
        ...
        function addListener(emitter, event, fn, context, once, pp) {
          ...
          emitter._events[evt]
            ? emitter._events[evt].fn
              ? (emitter._events[evt] = pp ? [listener, emitter._events[evt]] : [emitter._events[evt], listener])
              : emitter._events[evt][pp ? "unshift" : "push"](listener)
        ...
        EventEmitter.prototype.removeAllListeners = function (event) {
          var evt;
          if (event) this._events[(evt = prefix ? prefix + event : event)] && clearEvent(this, evt);
          else {
            this._events = new Events();
            this._eventsCount = 0;
          }
    ...
    17: [
      function (require, module, exports) {
        ...
        var inherits = require("/bundle/utils").inherit,
          ...
          MediaSource = "undefined" !== typeof window && window.MediaSource,
        ...
        function MediaSourceStream(wrapper, obj) {
          ...
          if ("string" === typeof obj) {
            self._type = obj;
            "open" === self._mediaSource.readyState
        ...
        MediaSourceStream.prototype._flow = function () {
          if (
            ...
            !("open" === this._mediaSource.readyState && this._getBufferDuration() > this._bufferDuration) &&
            this._cb
          ) {
            var cb = this._cb;
            this._cb = null;
        ...
        MediaSourceStream.prototype._getBufferDuration = function () {
          for (
            var buffered = this._sourceBuffer.buffered, currentTime = this._elem.currentTime, bufferEnd = -1, i = 0;
            i < buffered.length;
            i++
          ) {
            var start = buffered.start(i),
              end = buffered.end(i) + EPSILON;
      ...
      { "/bundle/utils": 4, "readable-stream": 41, "to-arraybuffer": 43 }
    ],
    18: [
      function (require, module, exports) {
        var Buffer = require("buffer").Buffer,
        ...
        exports.ftyp.decode = function (buf, offset) {
          for (
            var brand = (buf = buf.slice(offset)).toString("ascii", 0, 4), version = buf.readUInt32BE(4), compatibleBrands = [], i = 8;
            i < buf.length;
            i += 4
          )
        ...
        exports.VisualSampleEntry.encode = function (box, buf, offset) {
          ...
          var compressorName = box.compressorName || "",
            nameLen = Math.min(compressorName.length, 31);
        ...
        exports.VisualSampleEntry.decode = function (buf, offset, end) {
          buf = buf.slice(offset);
          var length = end - offset,
            nameLen = Math.min(buf.readUInt8(42), 31),
            box = {
        ...
        exports.esds.decode = function (buf, offset, end) {
          ...
          var desc = Descriptor.Descriptor.decode(buf, 0, buf.length),
            dcd = ("ESDescriptor" === desc.tagName ? desc : {}).DecoderConfigDescriptor || {},
            oti = dcd.oti || 0,
            dsi = dcd.DecoderSpecificInfo,
            audioConfig = dsi ? (248 & dsi.buffer.readUInt8(0)) >> 3 : 0,
            mimeCodec = null;
          if (oti) {
        ...
        exports.stsz.decode = function (buf, offset) {
          for (
            var size = (buf = buf.slice(offset)).readUInt32BE(0), num = buf.readUInt32BE(4), entries = new Array(num), i = 0;
            i < num;
            i++
          )
            entries[i] = 0 === size ? buf.readUInt32BE(4 * i + 8) : size;
          return {
            entries: entries
          };
        };
        ...
        exports.stco.decode = function (buf, offset) {
          for (var num = (buf = buf.slice(offset)).readUInt32BE(0), entries = new Array(num), i = 0; i < num; i++)
            entries[i] = buf.readUInt32BE(4 * i + 4);
          return {
            entries: entries
          };
        };
        ...
        exports.co64.decode = function (buf, offset) {
          for (var num = (buf = buf.slice(offset)).readUInt32BE(0), entries = new Array(num), i = 0; i < num; i++)
            entries[i] = uint64be.decode(buf, 8 * i + 4);
          return {
            entries: entries
          };
        };
        ...
        exports.dref.decode = function (buf, offset) {
          for (var num = (buf = buf.slice(offset)).readUInt32BE(0), entries = new Array(num), ptr = 4, i = 0; i < num; i++) {
            var size = buf.readUInt32BE(ptr),
              type = buf.toString("ascii", ptr + 4, ptr + 8),
              tmp = buf.slice(ptr + 8, ptr + size);
            ptr += size;
            entries[i] = {
              type: type,
              buf: tmp
      ...
      { "./descriptor": 19, "./index": 20, buffer: 10, uint64be: 44 }
    ],
    19: [
      function (require, module, exports) {
        var Buffer = require("buffer").Buffer,
        ...
        exports.Descriptor.decode = function (buf, start, end) {
          ...
          (obj = exports[tagName]
            ? exports[tagName].decode(buf, ptr, end)
            : {
        ...
        exports.DecoderConfigDescriptor.decode = function (buf, start, end) {
          var oti = buf.readUInt8(start),
            obj = exports.DescriptorArray.decode(buf, start + 13, end);
          obj.oti = oti;
          return obj;
        };
      },
    ...
    20: [
      function (require, module, exports) {
        ...
        Box.readHeaders = function (buffer, start, end) {
          ...
          var version,
            ...
            type = buffer.toString("ascii", start + 4, start + 8),
            ptr = start + 8;
          if (1 === len) {
        ...
        Box.decode = function (buffer, start, end) {
          ...
          return !headers || headers.length > end - start
            ? new Error("Unsupported media format, data too short...")
            : Box.decodeWithoutHeaders(headers, buffer, start + headers.headersLen, start + headers.length);
        };
        ...
        Box.decodeWithoutHeaders = function (headers, buffer, start, end) {
          ...
          if (containers[type]) {
            ...
            while (end - ptr >= 8) {
              ...
              ptr += child.length;
              if (contents.indexOf(child.type) >= 0) obj[child.type] = child;
            ...
            boxes[type]
              ? (obj = boxes[type].decode.call(headers, buffer, start, end))
        ...
        Box.encodingLength = function (obj) {
          ...
          } else if (boxes[type]) len += boxes[type].encodingLength(obj);
          else {
            if (!obj.buffer) throw new Error("Either `type` must be set to a known type (not'" + type + "') or `buffer` must be set");
      ...
      { "./boxes": 18, buffer: 10, uint64be: 44 }
    ],
    21: [
      function (require, module, exports) {
        var Buffer = require("buffer").Buffer,
          ...
          if (!this.destroyed) {
            ...
            while (data.length && !this.destroyed) {
              ...
              this._buf
                ...
                : this._str && (drained = this._str.write(consumed === data.length ? data : data.slice(0, consumed)));
            ...
            if (!this._pending || this._missing) drained ? next() : this._ondrain(next);
            else {
              this._writeBuffer = data;
              this._writeCb = next;
            }
        ...
        Decoder.prototype._readBox = function () {
          ...
          !(function bufferHeaders(len, buf) {
            self._buffer(len, function (additionalBuf) {
              ...
              if ("number" === typeof headers) bufferHeaders(headers - buf.length, buf);
              else {
                self._pending++;
      ...
      { "/bundle/utils": 4, buffer: 10, "mp4-box-encoding": 20, "next-event": 24, "readable-stream": 41 }
    ],
    22: [
      function (require, module, exports) {
        ...
        var nextTick = require("/bundle/utils").nextTick,
          Buffer = require("buffer").Buffer;
      ...
      { "/bundle/utils": 4, buffer: 10, "mp4-box-encoding": 20, "readable-stream": 41 }
    ],
    23: [
      function (require, module, exports) {
      ...
      function (require, module, exports) {
        ...
        if (OfflineAudioContext) {
          var silent = new Uint32Array([
              1179011410, 56, 1163280727, 544501094, 16, 65537, 44100, 88200, 1048578, 1635017060, 20, 0, 0, 0, 0, 0
            ]).buffer,
            isPromiseBased = new OfflineAudioContext(1, 128, 44100).decodeAudioData(
              silent,
              function () {},
              function () {}
            );
    ...
    28: [
      function (require, module, exports) {
        ...
        var _require = require("readable-stream"),
          ...
          RangeSliceStream = (function (_Writable) {
            ...
            _proto._write = function (chunk, encoding, cb) {
              ...
              while (1) {
                ...
                var currRange = this._queue[0],
                  writeStart = Math.max(currRange.start - this._position, 0),
                  writeEnd = currRange.end - this._position;
      ...
      { "/bundle/utils": 4, "readable-stream": 41 }
    ],
    29: [
      function (require, module, exports) {
        "use strict";
        ...
        createErrorType(
          ...
          function (name, expected, actual) {
            ...
            return (
              (endsWith(name, " argument")
                ? "The " + name + " " + determiner + " " + oneOf(expected, "type")
                : 'The "' +
                  name +
                  '" ' +
                  (includes(name, ".") ? "property" : "argument") +
                  " " +
                  determiner +
                  " " +
                  oneOf(expected, "type")) +
    ...
    30: [
      ...
      { "./_stream_readable": 32, "./_stream_writable": 34, "/bundle/utils": 4 }
    ],
    31: [
      function (require, module, exports) {
        "use strict";
      ...
      { "./_stream_transform": 33, "/bundle/utils": 4 }
    ],
    32: [
      function (require, module, exports) {
        "use strict";
        ...
        var StringDecoder,
          ...
          from,
          EE = require("eventemitter3").EventEmitter,
          EElistenerCount = function (emitter, type) {
            return emitter.listeners(type).length;
          },
          Stream = require("./internal/streams/stream"),
          _isUint8Array = require("/bundle/utils").isU8,
          Buffer = require("buffer").Buffer,
          debug = require("/bundle/utils").debuglog("stream"),
        ...
        Readable.prototype.push = function (chunk, encoding) {
          ...
          if (state.objectMode) skipChunkCheck = !0;
        ...
        Readable.prototype.read = function (n) {
          ...
          if (
            0 === n &&
            state.needReadable &&
            ((0 !== state.highWaterMark ? state.length >= state.highWaterMark : state.length > 0) || state.ended)
          ) {
          ...
          if (state.ended || state.reading) doRead = !1;
          else if (doRead) {
            state.reading = !0;
            state.sync = !0;
      ...
      },
      {
        "../errors": 29,
        "./_stream_duplex": 30,
        "./internal/streams/buffer_list": 35,
    ...
    33: [
      function (require, module, exports) {
        ...
        Transform.prototype._read = function (n) {
          var ts = this._transformState;
          if (null === ts.writechunk || ts.transforming) ts.needTransform = !0;
          else {
            ts.transforming = !0;
      ...
      { "../errors": 29, "./_stream_duplex": 30, "/bundle/utils": 4 }
    ],
    34: [
      function (require, module, exports) {
        "use strict";
        ...
        var realHasInstance,
          deprecate = require("/bundle/utils").deprecate,
          Stream = require("./internal/streams/stream"),
          _isUint8Array = require("/bundle/utils").isU8,
          Buffer = require("buffer").Buffer,
          destroyImpl = require("./internal/streams/destroy"),
        ...
        if ("function" === typeof Symbol && Symbol.hasInstance && "function" === typeof Function.prototype[Symbol.hasInstance]) {
          ...
          Object.defineProperty(Writable, Symbol.hasInstance, {
            value: function (object) {
              return (
                !!realHasInstance.call(this, object) || (this === Writable && object && object._writableState instanceof WritableState)
              );
            }
          });
        } else
        ...
        function validChunk(stream, state, chunk, cb) {
          ...
          null === chunk
            ...
            : "string" === typeof chunk || state.objectMode || (er = new ERR_INVALID_ARG_TYPE("chunk", ["string", "Buffer"], chunk));
          if (er) {
            errorOrDestroy(stream, er);
            vsNT(cb.bind(null, er));
            return !1;
        ...
        function clearBuffer(stream, state) {
          ...
          if (stream._writev && entry && entry.next) {
            var buffer = new Array(state.bufferedRequestCount),
        ...
        function prefinish(stream, state) {
          if (!state.prefinished && !state.finalCalled)
            if ("function" !== typeof stream._final || state.destroyed) {
              state.prefinished = !0;
              stream.emit("prefinish");
            } else {
              state.pendingcb++;
          ...
          this._destroy(err || null, function (err) {
            if (!cb && err)
              if (_this._writableState)
                if (_this._writableState.errorEmitted) vsNT(emitCloseNT.bind(null, _this));
                else {
                  _this._writableState.errorEmitted = !0;
                  vsNT(emitErrorAndCloseNT.bind(null, _this, err));
                }
              else vsNT(emitErrorAndCloseNT.bind(null, _this, err));
            else if (cb) {
    ...
    38: [
      ...
      { "../../../errors": 29, "./end-of-stream": 37, once: 25 }
    ],
    39: [
      function (require, module, exports) {
        "use strict";
    ...
    41: [
      ...
      },
      {
        ...
        "./lib/internal/streams/end-of-stream.js": 37,
    ...
    42: [
      function (require, module, exports) {
        ...
        function normalizeEncoding(enc) {
          ...
          if ("string" !== typeof nenc && (Buffer.isEncoding === isEncoding || !isEncoding(enc)))
    ...
    44: [
      function (require, module, exports) {
        ...
        exports.decode = function (buf, offset) {
          var top = buf.readUInt32BE((offset = offset || 0)),
            rem = buf.readUInt32BE(offset + 4);
    ...
    46: [
      function (require, module, exports) {
        ...
        var pump = require("pump"),
          MP4Remuxer = require("./mp4-remuxer"),
          EBMLRemuxer = require("./ebml-remuxer"),
          MediaElementWrapper = require("mediasource");
        ...
        function VideoStream(file, mediaElem, opts) {
          ...
          self.flushSourceBuffers = opts.sbflush
            ? self._flushSourceBuffers
            : function () {
        ...
        VideoStream.prototype.createWriteStream = function (obj) {
          var videoStream = this,
            videoFile = videoStream._file,
            mediaSource = videoStream._elemWrapper.createWriteStream(obj),
            mediaSourceDestroy = mediaSource.destroy,
            trace = localStorage.vsd
              ? function (time, sb, chunk) {
                  for (var start = time, end = time + chunk.duration, b = [], i = 0; i < sb.buffered.length; ++i)
                    b.push(sb.buffered.start(i) + "-" + sb.buffered.end(i));
                  console.debug(
                    "ranges(%s), timecode=%s, duration=%s, time=%s, start=%s(%s), end=%s(%s)",
                    b.join(", "),
                    chunk.timecode,
                    chunk.duration,
                    time,
                    start,
                    sb.appendWindowStart,
                    end,
                    sb.appendWindowEnd,
                    sb.timestampOffset,
                    chunk.seektime
            ...
            if (this._cb && !this.destroyed) {
              ...
              if (sb && !sb.updating) {
                if (
                  ...
                  sb.buffered.length &&
                  sb.buffered.end(sb.buffered.length - 1) - this._elem.currentTime > this._bufferDuration
                )
                  return;
                var cb = this._cb;
                this._cb = null;
        ...
        VideoStream.prototype._pump = function (time) {
          try {
            ...
            "WebM" === this._type && this.withinBufferedRange(time + 1)
              ? d && console.debug("Ignoring pump within buffered range.", time)
              : this._tryPump(time);
          } catch (ex) {
        ...
        VideoStream.prototype._tryPump = function (time) {
          var i,
            ...
            video = self._elem,
            muxer = self._muxer,
            muxed = muxer.seek(time),
            timeStampFixup = muxer._seekTimeFixup;
          d && console.debug("Seeking to %s, fixup=%s", time, timeStampFixup);
          if (time - timeStampFixup > 0.4 || 0 === timeStampFixup) {
          ...
          if (!this.destroyed) {
            ...
            if (this._tracks) {
              for (
                var track,
                  tracks = this._tracks,
                  destroy = function (track, instance) {
                    try {
        ...
        VideoStream.prototype.forEachSourceBuffer = function (cb) {
          if (this._tracks)
            for (var sb, startRange, endRange, ms, src, currentTime = this._elem.currentTime, i = this._tracks.length; i--; ) {
        ...
        VideoStream.prototype._forcePump = function (offset) {
          ...
          delay.cancel("vs.force-track-pump");
          if (this._tracks)
            for (var ms, sb, t = this._tracks, i = t.length; i--; )
              if ((sb = (ms = t[i].mediaSource) && ms._sourceBuffer) && ms) {
                ++step;
                ms._cb = null;
                ms.destroy();
                if (sb.updating) {
        ...
        VideoStream.prototype.removeBuffered = function (sb, currentTime, callback) {
          if (sb.buffered.length) {
            var startRange = sb.buffered.start(0),
              endRange = sb.buffered.end(sb.buffered.length - 1);
        ...
        VideoStream.prototype.forEachBufferedRanges = tryCatch(function (cb) {
          if (this._tracks)
            for (
              var j, sb, startRange, endRange, ms, src, currentTime = this._elem.currentTime, trackId = this._tracks.length;
              trackId--;

            ) {
              ms = (src = this._tracks[trackId].mediaSource || !1)._mediaSource;
              for (j = (sb = src._sourceBuffer) && sb.buffered.length; j--; )
                try {
        ...
        VideoStream.prototype._flushSourceBuffers = function (mode, track) {
          ...
          this.forEachSourceBuffer(function (sb, startRange, endRange, currentTime, mediaSource) {
            d &&
              console.debug(
                "[VideoStream.flushSourceBuffers] ct=%s sr=%s er=%s",
                currentTime,
                startRange,
                endRange,
                sb.updating,
            ...
            if (!sb.updating) {
              if (-1 !== mode)
                endRange > currentTime
                  ...
                  : currentTime >= startRange && currentTime <= endRange && (endRange = currentTime - 1);
              else {
        ...
        Object.defineProperty(VideoStream.prototype, "bufTime", {
          get: function () {
            ...
            if (this._muxer instanceof MP4Remuxer) {
              var trak = this._muxer._tracks[0] || !1,
                smpl = (trak && trak.samples[trak.currSample]) || !1;
              return (time = (smpl.dts + smpl.duration) / trak.timeScale) - this._elem.currentTime;
            }
            if (this._muxer instanceof EBMLRemuxer)
              for (var mediaSourceStream, i = Object(this._tracks).length; i--; )
                "open" === ((mediaSourceStream = this._tracks[i].mediaSource) && mediaSourceStream._mediaSource).readyState &&
                  (time = Math.max(time, mediaSourceStream._getBufferDuration()));
            return time;
          }
        });
      },
      { "./ebml-remuxer": 5, "./mp4-remuxer": 6, mediasource: 17, pump: 27 }
    ]
  },
  {},
  [3]
