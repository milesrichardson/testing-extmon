...
(function (global, factory) {
  ...
  const _global = typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : global;
  ...
  props(DexiePromise.prototype, {
    ...
    always: NativePromise.prototype.always,
      ...
      return typeof type === "function"
        ? this.then(null, (err) => (err instanceof type ? handler(err) : PromiseReject(err)))
        : this.then(null, (err) => (err && err.name === type ? handler(err) : PromiseReject(err)));
    },
  ...
  function callListener(cb, promise, listener) {
    try {
      ...
      if (promise._state) {
        ...
        ret = cb(value);
        if (!rejectingErrors.includes(value)) markErrorAsHandled(promise);
      }
  ...
  class Table {
    ...
    bulkAdd(objects, keysOrOptions, options) {
      ...
      return this._trans("readwrite", (trans) => {
        ...
        return this.core
          .mutate({ trans, type: "add", keys: keys, values: objectsToAdd, wantResults })
    ...
    bulkPut(objects, keysOrOptions, options) {
      ...
      return this._trans("readwrite", (trans) => {
        ...
        return this.core
          .mutate({ trans, type: "put", keys: keys, values: objectsToPut, wantResults })
          .then(({ numFailures, results, lastResult, failures }) => {
            const result = wantResults ? results : lastResult;
            if (numFailures === 0) return result;
  ...
  function getDatabaseNames({ indexedDB, IDBKeyRange }) {
    return hasDatabasesNative(indexedDB)
      ? Promise.resolve(indexedDB.databases()).then((infos) =>
          !Array.isArray(infos) ? [] : infos.map((info) => info.name).filter((name) => name !== DBNAMES_DB)
        )
    ...
    function createDbCoreTable(tableSchema) {
      ...
      function mutate({ trans, type, keys, values, range }) {
        return new Promise((resolve, reject) => {
          ...
          if (type === "deleteRange") {
            if (range.type === 4) return resolve({ numFailures, failures, results: [], lastResult: undefined });
  ...
  function dexieOpen(db) {
    ...
    return DexiePromise.race([
      openCanceller,
      safari14Workaround(indexedDB).then(
        () =>
          new DexiePromise((resolve, reject) => {
            throwIfCancelled();
