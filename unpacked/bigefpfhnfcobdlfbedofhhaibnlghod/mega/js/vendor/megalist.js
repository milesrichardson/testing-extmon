(function (scope, $) {
  ...
  var MegaList = function (listContainer, options) {
    ...
    this.$listContainer.css({ position: "relative" }).addClass("megaList");
  ...
  MegaList.prototype.rebind = function (eventName, cb) {
    if (eventName.indexOf(".") === -1) {
      if (typeof console !== "undefined" && console.error) {
        console.error("MegaList.rebind called with eventName that does not have a namespace, which is an" + "anti-pattern");
      }
      return;
    }
  ...
  MegaList.prototype.scrollToItem = function (itemId) {
    ...
    else if (itemOffsetTopPlusHeight + this.options.headerHeight > this._calculated["scrollTop"] + this._calculated["scrollHeight"]) {
      scrollToY = itemOffsetTopPlusHeight - this._calculated["scrollHeight"] + this.options.headerHeight + this.options.bottomSpacing;
    }
  ...
  MegaList.prototype._recalculate = function () {
    ...
    lazy(calculated, "contentHeight", function () {
      return (
        Math.ceil(self.items.length / this.itemsPerRow) * self.options.itemHeight + self.options.headerHeight + self.options.bottomSpacing
      );
    });
    ...
    lazy(calculated, "visibleLastItemNum", function () {
      var value = Math.min(
        self.items.length,
        Math.ceil(Math.ceil(this.scrollTop / self.options.itemHeight) * this.itemsPerRow + this.itemsPerPage)
      );
  ...
  MegaList.prototype._contentUpdated = function (forced) {
    if (this._wasRendered || forced) {
      ...
      if (this._lastContentHeight !== this._calculated["contentHeight"]) {
        ...
        this.content.style.height = this._calculated.contentHeight - this.options.headerHeight - this.options.bottomSpacing + "px";
      }
  ...
  MegaList.prototype._applyDOMChanges = function (contentWasUpdated) {
    ...
    if (contentWasUpdated === true) {
      ...
      const tick = 192;
      delay(
        `megalist:content-updated:${this.listId}`,
        () => {
          this._isUserScroll = false;
          this.scrollUpdate();
          this._isUserScroll = true;
  ...
  MegaList.RENDER_ADAPTERS.Table = class TableRenderAdapter {
    ...
    _rendered() {
      const megaList = this.megaList;
      megaList.content.style.height =
        megaList._calculated["contentHeight"] - megaList.options.headerHeight - megaList.options.bottomSpacing + "px";
      if (!this.options.usingNativeScroll) {
  ...
  MegaList.RENDER_ADAPTERS.Grid = class GridRenderAdapter extends MegaList.RENDER_ADAPTERS.List {
    ...
    _itemsRepositioned() {
      ...
      this.postPusherDOMNode.style.height = `${_calcSpace(megaList.items.length - calculated.visibleLastItemNum)}px`;
