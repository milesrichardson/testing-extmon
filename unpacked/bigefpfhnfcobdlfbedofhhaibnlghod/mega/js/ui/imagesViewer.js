...
(function _imageViewerSlideShow(global) {
  ...
  const events = ["mega:openfolder", "updFileManagerUI", "chat_image_preview", "mega:gallery:view:after", "mega:close_fileversioning"];
    ...
    if (
      ...
      $favButton.rebind("click.mediaviewer", function () {
        ...
        if (newFavState) {
          ...
          if (is_video(n)) {
            $("i", $button).removeClass("icon-favourite").addClass("icon-heart-broken-small-regular-outline");
          } else {
          ...
          if (is_video(n)) {
            $("i", $button).removeClass("icon-heart-broken-small-regular-outline").addClass("icon-favourite");
          } else {
  ...
  function slideshow_remove(n, $overlay) {
    ...
    if (
      ...
      !n.p ||
      root === M.InboxID ||
      (root === "shares" && M.getNodeRights(n.p) < 2) ||
      folderlink ||
      (M.getNodeByHandle(n.h) && !M.getNodeByHandle(n.h).u && M.getNodeRights(n.p) < 2) ||
      M.chat
    ) {
  ...
  function slideshow_node(id, $overlay) {
    ...
    if ($overlay) {
      ...
      if (
        ...
        root === M.RubbishID ||
        pfcol ||
        (!folderlink && M.getNodeByHandle(n.h) && !M.getNodeByHandle(n.h).u && M.getNodeRights(n.p) < 2)
      ) {
  ...
  const slideshow_zoomSlider = is_mobile
    ? nop
    : (value = 100) => {
  ...
  function slideshow_imgPosition($overlay) {
    ...
    if (zoom_mode) {
      ...
      else if (fitToWindow[id] && origImgHeight < viewerHeight && origImgWidth < viewerWidth && origImgHeight * w_perc <= viewerHeight) {
        imgWidth = viewerWidth;
        imgHeight = origImgHeight * w_perc;
        newImgWidth = switchedSides ? imgHeight : imgWidth;
      }
      // Check if preview and original imgs are loading and width fits browser window after increasing height
      else if (fitToWindow[id] && imgHeight < viewerHeight && origImgWidth < viewerWidth && origImgWidth * h_perc <= viewerWidth) {
        imgWidth = origImgWidth * h_perc;
        imgHeight = viewerHeight;
        newImgWidth = switchedSides ? imgHeight : imgWidth;
      } else {
  ...
  function slideshow(id, close, hideCounter, filteredNodeArr) {
    ...
    if (close) {
      ...
      $overlay.removeClass("video video-theatre-mode mouse-idle slideshow fullscreen fullimage").addClass("hidden");
      ...
      $(".speed i", $videoControls).removeClass().addClass("sprite-fm-mono icon-playback-1x-small-regular-outline");
    ...
    if ($document.fullScreen()) {
      $(".v-btn.fullscreen i", $imageControls).addClass("icon-fullscreen-leave").removeClass("icon-fullscreen-enter");

      $content.attr("data-fullscreen", "true");
      $(".v-btn.fs", $videoControls).addClass("cancel-fullscreen").removeClass("go-fullscreen");
      $(".v-btn.fs i", $videoControls).removeClass().addClass("sprite-fm-mono icon-minimize-02-small-regular-outline");
      $(".fs-wrapper .tooltip", $videoControls).text(l.video_player_exit_fullscreen);
    } else {
      $(".v-btn.fullscreen i", $imageControls).removeClass("icon-fullscreen-leave").addClass("icon-fullscreen-enter");

      $content.attr("data-fullscreen", "false");
      $(".v-btn.fs", $videoControls).removeClass("cancel-fullscreen").addClass("go-fullscreen");
      $(".v-btn.fs i", $videoControls).removeClass().addClass("sprite-fm-mono icon-maximize-02-small-regular-outline");
    ...
    if (!slideshowplay) {
      ...
      $document.rebind("keydown.slideshow", function (e) {
        ...
        } else if ((e.keyCode === 8 || e.key === "Backspace") && !isDownloadPage && !$.copyDialog && !$.dialog && !$.msgDialog) {
          history.back();
          return false;
        }
      });
      ...
      if (steps.backward.length) {
        ...
        $prevNextButtons.filter(".previous").removeClass("hidden").addClass("opacity-50").attr("disabled", "disabled");
      }
      if (steps.forward.length) {
        $prevNextButtons.filter(".next").removeClass("hidden opacity-50").removeAttr("disabled");
      } else if (is_video(n)) {
        $prevNextButtons.filter(".next").removeClass("hidden").addClass("opacity-50").attr("disabled", "disabled");
      }
    ...
    $dlBut.rebind("click.media-viewer", function _dlButClick() {
      ...
      if (p && p.full && Object(p.buffer).byteLength) {
        M.saveAs(p.buffer, n.name).fail(function (ex) {
          if (d) {
            console.debug(ex);
          }
  ...
  function fetchsrc(id) {
    ...
    if (loadOriginal) {
      ...
      M.gfsfetch(n.link || n.h, 0, -1, progress)
        .then((data) => {
  ...
  function slideshow_videostream(id, $overlay) {
    ...
    $playVideoButton.rebind("click", function () {
      ...
      initVideoStream(n, $overlay, destroy)
        .done((streamer) => {
          preqs[n.h] = streamer;
  ...
  function previewsrc(id) {
    ...
    img.onload = img.onerror = function (ev) {
      ...
      if (slideshowplay) {
        if (previews[id].full || previews[id].ffailed || ev.type === "error" || is_image(M.getNodeByHandle(slideshowid)) !== 1) {
          slideshow_timereset();
        }
      }
      ...
      $img
        .removeClassWith("exif-rotation-")
        .addClass("exif-rotation-" + rot)
        .attr("data-exif", rot);
