...
(function (scope) {
  ...
  const recentlyOpened = {
    ...
    refresh: async function () {
      ...
      await M.getPersistentData(`${u_handle}!ro`)
        .then(async (recentlyOpened) => {
          this.files = new Map(
            JSON.parse(tlvstore.decrypt(recentlyOpened)).map((v) => {
              return [v.h, v];
            })
          );
        })
    ...
    save: function () {
      ...
      M.setPersistentData(`${u_handle}!ro`, tlvstore.encrypt(JSON.stringify([...recentlyOpened.files.values()])));
    },
    /**
    ...
    fetchNodes: async function () {
      ...
      return dbfetch.acquire(toFetch).always(() => {
        ...
        for (const h of this.files.keys()) {
          if (!M.getNodeByHandle(h) || M.getNodeRoot(h) === M.RubbishID || typeof h === "undefined") {
            this.files.delete(h);
            anyDeleted = true;
          }
        }
  ...
  function initSearch(currentPage) {
    ...
    $(".mini-search input", $topbar).rebind("keyup.mini-searchbar", function () {
      if ($(this).val().length) {
  ...
  function addVisibilityListeners() {
    ...
    $fileSearch.rebind("focus", () => {
      showEmptyState = recentlySearched.terms.size !== 0 && recentlyOpened.files.size !== 0;
      renderUpdatedDropdown();
    });
    ...
    $(".fmholder").rebind("click.searchbar", (event) => {
      ...
      if ($target.closest(".dropdown-search").length === 0 && $target.closest(".js-topbar-searcher").length === 0) {
        $dropdownSearch.addClass("hidden");
      }
    });
  ...
  function renderRecentlySearchedItems() {
    ...
    for (let i = recentlySearchedArr.length - 1, nb = 0; i >= 0 && nb < recentlySearched.numTermsInView; i--, nb++) {
  ...
  function renderRecentlyOpenedItems() {
    ...
    const makeRecentlyOpenedFileItem = ({ h: handle, e: editable }) => {
      ...
      const parentName = parentNode.h === M.RootID ? l[1687] : parentNode.name;
    ...
    for (let i = recentlyOpenedArr.length - 1, nb = 0; i >= 0 && nb < recentlyOpened.numFilesInView; i--, nb++) {
  ...
  function closeMiniSearch() {
    const $miniSearch = $(".mini-search", $topbar);
    if ($miniSearch && ($miniSearch.hasClass("active") || $miniSearch.hasClass("highlighted"))) {
