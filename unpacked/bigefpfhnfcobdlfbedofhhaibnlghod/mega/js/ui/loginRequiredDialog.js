  ...
  function showLoginRequiredDialog(options) {
    ...
    if (u_type !== false && (!options.minUserType || u_type >= options.minUserType)) {
      ...
      loginRequiredDialog.bind("onBeforeShow", function () {
        $("header h2", this.$dialog).text(this.options.title);

        $("header p", this.$dialog).text(options.textContent || l[7679]);

        $("button.pro-login", this.$dialog).rebind("click.loginrequired", function () {
        ...
        if (options.showRegister) {
          $("button.pro-register", this.$dialog).rebind("click.loginrequired", () => {
            loginRequiredDialog.hide();
