(function (scope) {
  ...
  PasswordReminderAttribute.prototype.loadFromAttribute = function () {
    ...
    self.loading = mega.attr
      .get(u_handle, "prd", -2, true)
      .done(function (r) {
        if (isString(r)) {
   ...
   * @type {Array}
   */
  PasswordReminderAttribute.PROPERTIES = ["lastSuccess", "lastSkipped", "masterKeyExported", "dontShowAgain", "lastLogin"];
  ...
  PasswordReminderDialog.prototype.bindEvents = function () {
    ...
    $(
      this.dialog.querySelectorAll(
        is_mobile ? ".button-prd-confirm, .button-prd-skip, .change-password-button, .button-prd-backup" : "button.mega-button"
      )
  ...
  PasswordReminderDialog.prototype.onConfirmClicked = function (element, evt) {
    ...
    security
      .getDerivedEncryptionKey(enteredPassword)
  ...
  PasswordReminderDialog.prototype.recheck = function (hideIfShown) {
    ...
    if (
      ...
      var skipShowingDialog =
        !self.showIcon() || $("textarea:focus, input:focus, select:focus, .dropdown:visible:first, .mega-dialog:visible:first").length > 0;

      if (
        !skipShowingDialog &&
        is_fm() &&
        !pfid &&
        (!self.passwordReminderAttribute.lastSkipped || unixtime() - self.passwordReminderAttribute.lastSkipped > SHOW_AFTER_LASTSKIP)
      ) {
        self.isLogout = false;
        self.show();
      } else {
  ...
  PasswordReminderDialog.prototype._initInternals = function () {
    this.dialog = document.querySelector(is_mobile ? ".mobile.password-reminder-overlay" : ".mega-dialog.pass-reminder");
  ...
  PasswordReminderDialog.prototype.onGenericClick = function (e) {
    ...
    if ($(e.target).parents(".pass-reminder").length === 0 && !$(e.target).is(".pass-reminder")) {
      if (this.isShown) {
        this.onSkipClicked();
      }
    }
