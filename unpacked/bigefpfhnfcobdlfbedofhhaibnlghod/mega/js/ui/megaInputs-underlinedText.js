...
mega.ui.MegaInputs.prototype.underlinedText = function () {
  ...
  if (!(this.type === "text" || this.type === "password" || this.type === "tel" || this.type === "number" || this.type === "email")) {
    console.error("Class binding and input type mismatch! " + "classname: mega-input, input type: " + this.type + ", Required type: text.");
    return;
  }
...
mega.ui.MegaInputs.prototype.underlinedText._withIconOrPrefix = function () {
  ...
  if ($input.hasClass("copyButton")) {
    const icon = is_mobile ? "sprite-mobile-fm-mono icon-copy-thin-outline" : "sprite-fm-mono icon-copy-2";
    ...
    $copyBtn.rebind("click.copyInputValue tap.copyInputValue", () => {
      copyToClipboard($input.val(), escapeHTML(this.options.copyToastText) || l[371]);
      return false;
    });
  }

  if ($input.hasClass("clearButton")) {
    const icon = is_mobile ? "sprite-mobile-fm-mono icon-x-thin-outline icon-size-16" : "sprite-fm-mono icon-close-component";
...
mega.ui.MegaInputs.prototype.underlinedText._strengthChecker = function () {
  ...
  if (this.type === "password" && $input.hasClass("strengthChecker")) {
    ...
    if (is_mobile) {
      ...
      $wrapper.safeAppend('<div class="account-pass-lines">' + '<div class="register-pass-status-line"></div>' + "</div>");
    }
    ...
    var _bindStrengthChecker = function () {
      ...
      $input.rebind("keyup.strengthChecker input.strengthChecker change.strengthChecker", function (e) {
        ...
        $passStatus.add($passStatusBar).add($messageContainer).removeClass("good1 good2 good3 good4 good5 checked");
        ...
        if (typeof strength === "object") {
          ...
          if (is_mobile) {
            ...
            $input.data("MegaInputs").showMessage(`${alert} <span>${strength.string2}</span>`);
          } else {
...
mega.ui.MegaInputs.prototype.underlinedText._botSpaceCalc = function () {
  ...
  if ($wrapper.hasClass("msg")) {
    ...
    $wrapper.css(
      "margin-bottom",
      this.origBotSpace + $(".message-container", $wrapper).outerHeight() + ($wrapper.hasClass("fix-msg") ? 4 : 9)
    );
  }
};
