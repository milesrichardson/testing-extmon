...
function bindDropdownEvents($select, saveOption, contentBlock) {
  ...
  $select.rebind("click.inputDropdown", function (e) {
    ...
    if ($this.hasClass("disabled")) {
      ...
      ($target.closest(".option").length || $target.is(".option")) &&
  ...
  $hiddenInput.rebind("keyup.inputDropdown", function (e) {
    ...
    if ((charCode > 64 && charCode < 91) || (charCode > 96 && charCode < 123)) {
      ...
      const $current = $activeOption.length ? $activeOption : $(".option:not(.template)", $select).first();

      var $prev = $current.prev(".option:not(.template)");
      var $next = $current.next(".option:not(.template)");

      if (charCode === 38 && $prev.length) {
        // Up key
        $filteredItem = $prev;
      } else if (charCode === 40 && $next.length) {
        // Down key
        $filteredItem = $next;
      } else if (charCode === 13) {
        // Enter
    ...
    if ($filteredItem.length) {
      ...
      const $scrollBlock = $dropdownScroll.length ? $dropdownScroll : $(".dropdown-scroll", $(this).closest(".dropdown-input"));
...
function setDropdownValue($container, selectedOptionCallback) {
  ...
  let $selectedOption =
    typeof selectedOptionCallback === "function"
  ...
  $dropdownInputOptions.removeClass("active").removeAttr("data-state");
  ...
  if (!$selectedOption.length) {
    // Abnormal situation, dropdown may not be initialized
    return;
  }

  $selectedOption.addClass("active").attr("data-state", "active");
...
function createDropdown($container, options) {
  ...
  const __prepareDropdownOptions = ($megaInputDropdown, $dropdownInput, options) => {
    ...
    const selectedOptionCallback = typeof selectedOption === "function" ? selectedOption : null;

    const postActionCallback = typeof options.postAction === "function" ? options.postAction : null;
    ...
    for (const option in optionList) {
      ...
      const $dropdownOptionItem = $("<div/>", {
        class: "option"
      });

      $dropdownOptionItem.attr(attr).text(value);

      if (selectedOption) {
        const result = selectedOptionCallback ? selectedOptionCallback($dropdownOptionItem, value, index) : selectedOption === index;

        if (result) {
          $dropdownOptionItem.addClass("active").attr("data-state", "active");
...
function getDropdownValue($container, attributeName) {
  ...
  if (!attributeName) {
    return $dropdownInputSelectedOption.attr("data-value") || defaultValue;
  }

  return $dropdownInputSelectedOption.attr(`data-${attributeName}`) || $dropdownInputSelectedOption.attr("data-value") || defaultValue;
}
