(function ($, scope) {
  ...
  Dialog.prototype._initGenericEvents = function () {
    ...
    $("textarea", self.$dialog).rebind("mouseup mousemove", function () {
  ...
  Dialog.prototype.show = function ($toggleButton) {
    ...
    if (self.options.closable) {
      $(document.body).rebind("mousedown.dialogClose" + self.dialogIdx, function (e) {
        if (
          $(self.$dialog).find(e.target).length == 0 &&
          $(self.$dialog).is(e.target) === false &&
          !$(self.$dialog).is(".fm-mega-dialog")
        ) {
          self.hide();
          return false;
        }
  ...
  Dialog.prototype.collapse = function ($toggleButton) {
    var self = this;
    self.expanded = false;
    $(self.options.expandableButtonClass).addClass("short-size").removeClass("full-size");
  ...
  Dialog.prototype.expand = function () {
    ...
    $(self.options.expandableButtonClass).removeClass("short-size").addClass("full-size");
