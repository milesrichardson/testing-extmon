((scope) => {
  ...
  class DragSelect {
     ...
     */
    constructor(el, { allowedClasses, onDragStart, onDragMove, onDragEnd, scrollMargin, onScrollUp, onScrollDown, getScrollTop }) {
      if (el) {
        this.el = el;
    ...
    startListening(el, cl) {
      ...
      this._disposeDown = MComponent.listen(document, "mousedown", (evt) => {
        ...
        if (target !== el && (!Array.isArray(cl) || !cl.length || !cl.some((c) => target.classList.contains(c)))) {
          return true;
        }
