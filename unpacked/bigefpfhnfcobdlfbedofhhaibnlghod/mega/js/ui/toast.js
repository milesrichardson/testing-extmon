...
window.toastRack = (() => {
   ...
   */
  function createToastSlot(rack, { timeout, icons, buttons, hasClose = true, onClose, classes, content, groups, zIndex = true }) {
   ...
   */
  function createToast(rack, toastSlotId, { icons, buttons, hasClose, onClose, content }) {
  ...
  function createButtons(buttons) {
    return buttons.map(({ text, classes, onClick }) => {
    ...
    return {
      ...
      batch: (batchId, content, { level, joiner, icon, overrideOptions, cb } = {}) => {
        ...
        if (typeof batches[batchId].listener === "undefined") {
          ...
          batches[batchId].listener = setTimeout(dsp, batches[batchId].maxTime - batches[batchId].dispTime);
 ...
 * @returns {undefined}
 */
window.showToast = function (type, content, firstButtonText, secondButtonText, firstButtonOnClick, secondButtonOnClick, timeout) {
  "use strict";
...
lazy(window, "toaster", () => {
  ...
  lazy(toaster, "alerts", () => window.toastRack(document.querySelector(".alert-toast-container"), "alert-toast-container", "top"));
