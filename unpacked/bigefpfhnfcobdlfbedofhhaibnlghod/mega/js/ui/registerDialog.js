(function (scope) {
  ...
  function doProRegister($dialog, aPromise) {
    ...
    const registeraccount = function () {
      ...
      security.register.startRegistration(rv.first, rv.last, rv.email, rv.password, fromProPage, continueNewProRegistration);
    };
  ...
  function showRegisterDialog(opts, aPromise) {
    ...
    if (M.chat) {
      ...
      $("aside .login-text a, .register-side-pane.header a", $dialog).rebind("click.doSignup", function () {
        closeRegisterDialog($dialog, true);
        megaChat.loginOrRegisterBeforeJoining(undefined, false, true, undefined, opts.onLoginSuccessCb);
      });
      ...
      $("aside .login-text a, .register-side-pane.header a", $dialog).rebind("click.doSignup", function () {
        ...
        closeRegisterDialog($dialog, true);
        mega.ui
          .showLoginRequiredDialog({ minUserType: 3, skipInitialDialog: 1 })
          .then(function () {
            if (typeof onAccountCreated === "function") {
