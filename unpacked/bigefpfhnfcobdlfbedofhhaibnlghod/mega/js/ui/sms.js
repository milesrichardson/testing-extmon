...
var sms = {
  ...
  resetPhoneInputForm: function ($dialog) {
    ...
    if ($phoneInputPage.length > 0) {
      ...
      $(".verify-ph-country .js-country-list .option", $phoneInputPage).removeClass("active").attr("data-state", "");
...
sms.phoneInput = {
  ...
  initChangeAndKeyupHandler: function () {
    ...
    var toggleButtonState = function () {
      ...
      if ($countrySelect.length && $countrySelect.attr("data-country-iso-code").length > 1 && validateResult && !checkOldNum) {
...
sms.verifyCode = {
  ...
  initVerifyButton: function () {
    ...
    $verifyButton.rebind("click", function () {
      ...
      api
        .send({ a: "smsv", c: verificationCode })
        .then(() => {
          // Hide the current page
