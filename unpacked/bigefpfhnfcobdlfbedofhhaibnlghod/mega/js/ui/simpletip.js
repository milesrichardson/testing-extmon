(function ($) {
  ...
  $(document.body).rebind("mouseenter.simpletip", ".simpletip", function () {
    ...
    if (contents || isBreadcrumb) {
      ...
      $this.rebind(SIMPLETIP_UPDATED_EVENT, () => {
        $textContainer.safeHTML(sanitize($this.attr("data-simpletip")));
      });
      ...
      $node.position({
        ...
        at: at,
        collision: "flipfit",
        within: $this.hasClass("simpletip-breadcrumb")
          ? $this.parents("body").first()
  ...
  $(document.body).rebind("mouseover.simpletip touchmove.simpletip", function (e) {
    if (
      $currentNode &&
      !e.target.classList.contains("simpletip") &&
