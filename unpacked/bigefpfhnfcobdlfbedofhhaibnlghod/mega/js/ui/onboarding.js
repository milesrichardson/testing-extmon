...
mBroadcaster.addListener("fm:initialized", () => {
  ...
  const upgradeFrom = fmconfig.obVer ? (fmconfig.obVer < ONBOARD_VERSION ? fmconfig.obVer : false) : -1;
  if (upgradeFrom) {
        ...
        },
        {
          ...
          actions: [
            ...
            },
            {
              ...
              targetElmClass: [".dropdown.context.files-menu a.sh4r1ng-item", ".dropdown.context.files-menu a.getlink-item"],
              targetDescription: [l.onboard_v4_manage_file_context_desc_1, l.onboard_v4_manage_file_context_desc_2],
          ...
          actions: [
            {
              ...
              postComplete: () => {
                ...
                megaChat.plugins.chatOnboarding.handleFlagChange(null, null, OBV4_FLAGS.CHAT_SCHEDULE_ADDED, 1);
    ...
    get currentSectionName() {
      switch (M.currentrootid) {
        ...
        case "chat":
          return "chat";
        default:
          return M.currentrootid === undefined ? M.currentdirid : M.currentrootid;
      }
    }
  ...
  class OnboardV4Section {
    ...
    bindControlPanelEvents() {
      ...
      $(".onboarding-control-panel-content .js-close", this.$obControlPanel).rebind("click.onboarding", this.showConfirmDismiss.bind(this));
      $(".onboarding-control-panel-complete .js-close", this.$obControlPanel).rebind(
        "click.onboarding",
        this.markSectionComplete.bind(this)
      );
      $(".js-dismiss", this.$obControlPanel).rebind("click.onboarding", this.markSectionComplete.bind(this));
      $(".js-dismiss-cancel", this.$obControlPanel).rebind("click.onboarding", this.hideConfirmDismiss.bind(this));
  ...
  class OnboardV4Step {
    ...
    removePrerequisiteMessage() {
      this.$stepButton
        ...
        .removeAttr("data-simpletip data-simpletipposition data-simpletip-class data-simpletip-display-duration");
    }
      ...
      delay(
        ...
        () => {
          if (this.parentSection.currentStepIndex !== this.index) {
            $(".onboarding-step-link", this.$controlPanel).eq(this.index).removeClass("active").addClass("hotspot");
          }
        },
        1000
      );
  ...
  class OnboardV4Action {
    ...
    positionDialog() {
      ...
      if (this.map.targetHotSpot) {
        this.parentStep.parentSection.parent.$hotSpotNode = $(this.map.targetElmClass).addClass("onboarding-hotspot-animation-rect");
      }
      ...
      if (arrowAt) {
        $("#ob-dialog-arrow", this.$dialog).removeClass("hidden top bottom left right top-left").addClass(arrowAt);
      } else {
    ...
    bindDialogEvents() {
      ...
      $("#fmholder").rebind("mouseup.onboarding", (e) => {
        ...
        if (!this.map.nextActionTrigger || !$target.is(this.map.targetElmClass) || $target.parents(this.map.targetElmClass).length) {
    ...
    markContextMenu() {
      mBroadcaster.once("showcontextmenu", () => {
        ...
        mBroadcaster.once("contextmenuclose", () => {
          $(".onboarding-highlight-dot", targetSelector)
            .parent()
            .removeClass("simpletip")
