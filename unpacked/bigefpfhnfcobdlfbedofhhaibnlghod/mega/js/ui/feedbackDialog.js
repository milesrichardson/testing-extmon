(function ($, scope) {
  ...
  var FeedbackDialog = function (opts) {
    ...
    self.bind("onBeforeShow", function () {
      $(".rating a", self.$dialog).removeClass("active colored");

      self.$textarea = $("textarea", self.$dialog);
      self.$textarea.val("").next().text("");

      initTextareaScrolling($(".feedback-dialog-scr textarea", self.$dialog));

      $(".collected-data", self.$dialog).html("");
      ...
      $('input[name="send_stats"]', self.$dialog).prop("checked", true).trigger("change");

      $('input[name="contact_me"]', self.$dialog).prop("checked", false).trigger("change");
    });
