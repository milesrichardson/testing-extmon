lazy(mega.slideshow, "manager", () => {
  ...
  return new (class SlideshowManager {
     ...
     * @returns {void}
     */
    setState({ nodes, currentNodeId, isPlayMode, isReset, isAbortFetch, isChangeOrder, isNotBuildPlaylist }) {
    ...
    next(nodeId) {
      ...
      if (playlist.items.length > 1) {
        ...
        const nextStep = node === undefined || (this.state.isPlayMode && this.state.isReset) ? step.reset(playIndex) : step.next(playIndex);
    ...
    isLast(playIndex) {
      ...
      return playlist.items.length < 2 || (file.state.isReady && !settings.repeat.getValue() && playIndex === playlist.items.length - 1);
    }
    ...
    _isFetchAllowed() {
      ...
      return this.state.isPlayMode && !utils.isCurrentDirFlat() && settings.sub.getValue() && !playlist.isFull();
    }
  })();
});
