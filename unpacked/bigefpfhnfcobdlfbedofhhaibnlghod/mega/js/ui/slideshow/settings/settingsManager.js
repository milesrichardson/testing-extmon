  ...
  return new (class SlideshowSettingsManager {
    ...
    render($container, onConfigChange) {
      const setSettingsDefaults = (exclude) => {
        ...
        for (let i = 0; i < this._settings.length; i++) {
          const setting = this._settings[i];
          assert(typeof setting.getDefaultCfg === "function", `"${setting.name}" setting must implement "getDefaultCfg" method`);
      ...
      for (let i = 0; i < this._settings.length; i++) {
        const setting = this._settings[i];
        assert(typeof setting.render === "function", `"${setting.name}" setting must implement "render" method`);
