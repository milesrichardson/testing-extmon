(function (scope) {
  ...
  var versionColumnPrepare = function (versionsNb, VersionsSize) {
    var versionsTemplate =
      '<div class="ver-col-container">' +
      '<div class="ver-nb">' +
      versionsNb +
      "</div>" +
      ...
      "</div>" +
      '<div class="ver-size">' +
      '<div class="ver-size-nb">' +
      bytesToSize(VersionsSize) +
      "</div>" +
      "</div>" +
      "</div>";
  ...
  MegaRender.prototype = Object.freeze({
    ...
    cleanupLayout: function (aUpdate, aNodeList, aListSelector) {
      ...
      if (nodeListLength || (M.currentdirid && M.currentdirid.includes("user-management"))) {
        ...
        if (M.RubbishID && M.currentdirid === M.RubbishID) {
          ...
          /*} */
        } else if (M.currentrootid === M.RootID || M.currentrootid === M.RubbishID || M.currentrootid === M.InboxID) {
    ...
    insertDOMNode: function (aNode, aNodeIndex, aDOMNode, aUpdate, aDynCache) {
      if (!aUpdate || !this.container.querySelector(aDOMNode.nodeName)) {
        ...
        if (aUpdate && (prevNode = this.nodeList[aNodeIndex - 1]) && (domNode = document.getElementById(prevNode.h))) {
        ...
        } else if (aUpdate && (nextNode = this.nodeList[aNodeIndex + 1]) && (domNode = document.getElementById(nextNode.h))) {
      ...
      shares: function (aNode, aHandle, aExtendedInfo) {
        ...
        if (this.viewmode) {
          ...
          if (this.chatIsReady) {
            ...
            if (contact) {
              props.onlineStatus = M.onlineStatusClass(contact.presence ? contact.presence : "unavailable");
            }
          }
        ...
        if (this.viewmode) {
          if (aExtendedInfo !== false) {
            for (i = 0; i < props.userHandles.length && i < 4; i++) {
              props.avatars.push(parseHTML(useravatar.contact(props.userHandles[i], "", "span")).firstChild);
            }
          }
        } else {
          props.shareInfo = fm_contains(aNode.tf, aNode.td);
    ...
    builders: freeze({
      ...
      "cloud-drive": function (aNode, aProperties, aTemplate) {
        ...
        var title = [];
        let elm;
        const isBackup = this.location === "backups" || (this.location === "mixed-content" && M.getNodeRoot(aNode.h) === M.InboxID);
        ...
        if (this.viewmode) {
          ...
          if (aProperties.playtime !== undefined) {
            ...
            aTemplate.querySelector(".video-thumb-details span").textContent = secondsToTimeShort(aProperties.playtime);
          }
      ...
      shares: function (aNode, aProperties, aTemplate) {
        ...
        if (this.viewmode) {
          ...
          aTemplate.querySelector(".shared-folder-info").textContent = l[17590].replace("%1", aProperties.userName);
      ...
      "out-shares": function (aNode, aProperties, aTemplate) {
        ...
        if (this.viewmode) {
          ...
          if (aProperties.userNames.length > 3) {
            ...
            aTemplate.querySelector(".fm-chat-users-other").textContent = mega.icu.format(l.users_share_other_count, otherCount);
          }
    ...
    finalizers: freeze({
      ...
      "cloud-drive": function (aUpdate, aNodeList, aUserData) {
        if (DYNLIST_ENABLED) {
          if (!aUpdate) {
            var container = document.querySelector(viewModeContainers[this.section][this.viewmode]);
            this.addClasses(document.querySelector(viewModeContainers[this.section][0 + !this.viewmode]), ["hidden"]);
