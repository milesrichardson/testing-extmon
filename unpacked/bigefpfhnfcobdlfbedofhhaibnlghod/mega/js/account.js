...
function u_checklogin(ctx, force, passwordkey, invitecode, invitename) {
  ...
  if ((u_sid = u_storage.sid)) {
    ...
    api_createuser(ctx, invitecode, invitename)
      .then(({ result }) => {
        assert(
          typeof result === "string" && (localStorage.p = ctx.passwordkey) && (localStorage.handle = result),
  ...
  if (typeof res !== "object") {
    ...
    const log99810 = async (ex, tag = 0) => {
      ...
      if (!window.buildOlderThan10Days) {
        ...
        const stack = String(ex && ex.stack)
          .trim()
          .replace(/\s+/g, " ")
          .replace(msg, "")
          .substr(0, 512);

        const payload = [4, msg, stack, tag | 0, is_mobile | 0, is_extension | 0, buildVersion.website || "dev"];
    ...
    (window.M && typeof M.getPersistentData === "function" ? M.getPersistentData("e++ck") : Promise.reject())
      .then((data) => {
        ...
        return M.getPersistentDataEntries("e++", true).then((records) => {
          if (d) {
      ...
      .then(() => {
        ...
        // We've got keys?
        if (keys) {
          return mega.keyMgr.initKeyManagement(keys).catch((ex) => {
        ...
        // @todo Transifex
        const gone = `Your cryptographic keys have gone missing. It is not safe to use your account at this time.`;

        // We don't - are we supposed to?
        // otherwise, write them later, when the insecure state is fully loaded
        return mega.keyMgr.getGeneration().then((gen) => {
          if (gen > 0) {
          ...
          api
            ...
            .then(({ result: u_attr }) => {
              if (u_attr.b && u_attr.b.m === 0 && u_attr.b.bu) {
                crypt
                  .getPubKeyAttribute(u_attr.b.bu, "RSA")
                  .then(function (res) {
          ...
          M.setPersistentData("e++ck", data)
            .then(() => {
              return Promise.allSettled([
                mega.attr.set("firstname", base64urlencode(to8(firstName)), -1, false),
                mega.attr.set("lastname", base64urlencode(to8(lastName)), -1, false)
              ]);
            })
            .then(() => {
...
(function (exportScope) {
  ...
  var getLastInteractionWith = function (u_h, triggeredBySet, noRender) {
    ...
    var _renderLastInteractionDone = noRender
      ? nop
      : function (r) {
          r = r.split(":");
          ...
          $elem.removeClass("never").removeClass("cloud-drive").removeClass("conversations").removeClass("unread-conversations");
    ...
    var _renderLastInteractionFail = noRender
      ? nop
      : function (r) {
          var $elem = $(".li_" + u_h);
    ...
    $promise.done(_renderLastInteractionDone).fail(_renderLastInteractionFail);
    ...
    if (_lastUserInteractionCacheInFlight[u_h] && _lastUserInteractionCacheInFlight[u_h] !== -1) {
    ...
    } else if (_lastUserInteractionPromiseCache[u_h] && _lastUserInteractionPromiseCache[u_h].state() === "pending") {
      ...
      (!_lastUserInteractionPromiseCache[u_h] || _lastUserInteractionPromiseCache[u_h].state() !== "pending")
    ) {
      if (_lastUserInteractionCacheInFlight[u_h] === -1) {
        delete _lastUserInteractionCacheInFlight[u_h];
      }
      _lastUserInteractionPromiseCache[u_h] = mega.attr
        .getArrayAttribute(u_handle, "lstint", u_h, false, true)
        .always(function () {
    ...
    getLastInteractionWith(u_h, true)
      .done(function (timestamp) {
        if (_compareLastInteractionStamp(v, timestamp) === false) {
          ...
          mega.attr.setArrayAttribute("lstint", u_h, _lastUserInteractionCache[u_h], false, true);
        }
      })
      .fail(function (res) {
        if (res === false || res === -9) {
