...
function createthumbnail(file, aes, id, imagedata, node, opt) {
  ...
  return (async () => {
    ...
    (timer = tSleep(120)).then(() => webgl.sendTimeoutError()).catch(dump);
...
async function exifImageRotation(source, orientation) {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.src = source instanceof Blob ? URL.createObjectURL(source) : mObjectURL([source], source.type || "image/jpeg");
  });
}
