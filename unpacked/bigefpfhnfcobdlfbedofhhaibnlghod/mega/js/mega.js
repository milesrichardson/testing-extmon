  ...
  loadingDialog.show = function (subject, label) {
    ...
    if (!this.quiet) {
      ...
      if (is_mobile && $overlay.length && $spinner.length) {
        document.getElementById("loading-overlay").addEventListener(
          "touchmove",
          function (e) {
            e.preventDefault();
          },
        ...
        document.getElementById("loading-spinner").addEventListener(
          "touchmove",
          function (e) {
            e.preventDefault();
          },
...
scparser.$add("t", function (a, scnodes) {
  ...
  // notification logic
  if (fminitialized && !pfid && a.ou && a.ou !== u_handle && rootNode && rootNode.p && !rootNode.su) {
...
scparser.$add("opc", (a) => {
  ...
  if (fminitialized && M.chat && megaChatIsReady && megaChat.routingSection === "contacts" && megaChat.routingSubSection === "sent") {
...
async function fm_fullreload(q, logMsg) {
  ...
  return Promise.allSettled([fmdb && fmdb.invalidate(), logMsg && eventlog(99624, logMsg)]).finally(() => location.reload(true));
}
...
async function fetchfm(sn) {
  ...
  return api.req(payload, options).then(({ result }) => {
    if (!mega.infinity) {
...
function process_businessAccountSubUsers_SC(packet) {
  ...
  if (!M.suba) {
    // no sub-users in memory
    return;
  }
  if (!packet.a) {
    // no packet type/operation
    return;
  }
  if (!packet.u) {
    // no user handle
  ...
  if (!subUser) {
    // sub-user not found --> it's new one
  ...
  if ("s" in packet && packet.s !== subUser.s) {
    // new status
    subUser.s = packet.s;
    valChanged = true;
  }
  if (packet.e && packet.e !== subUser.e) {
    // new email
    subUser.e = packet.e;
    valChanged = true;
  }
  if (packet.firstname && packet.firstname !== subUser.firstname) {
    // new first-name
    subUser.firstname = packet.firstname;
    valChanged = true;
  }
  if (packet.lastname && packet.lastname !== subUser.lastname) {
    // new last-name
    subUser.lastname = packet.lastname;
    valChanged = true;
  }
  if (packet.position && packet.position !== subUser.position) {
    // new position
    subUser.position = packet.position;
    valChanged = true;
  }
  if (packet.idnum && packet.idnum !== subUser.idnum) {
    // new id number
    subUser.idnum = packet.idnum;
    valChanged = true;
  }
  if (packet.phonenum && packet.phonenum !== subUser.phonenum) {
    // new phone number
    subUser.phonenum = packet.phonenum;
    valChanged = true;
  }
  if (packet.location && packet.location !== subUser.location) {
    // new location
...
function folderreqerr(c, e) {
  ...
  if ($.dialog) {
    return mBroadcaster.once(
      "closedialog",
      SoonFc(90, () => folderreqerr(c, e))
    );
  }
...
function init_chat(action) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (action === 0x104df11e5) {
      M.require("chat").always(function () {
...
function loadfm_done(mDBload) {
  ...
  if (!pfid && u_type == 3) {
    ...
    var clen = Object.keys(M.c[M.RootID] || {}).filter(function (h) {
      return M.c[M.RootID][h] > 1;
    }).length;
...
function fmtreenode(id, e) {
  if (M.getNodeRoot(id) === "contacts") return false;
  var treenodes = {};
...
function fm_thumbnails(mode, nodeList, callback) {
  ...
  const onTheFly =
    !is_mobile && M.viewmode && mode !== "standalone" && !exclude && !mega.config.get("noflytn") ? Object.create(null) : false;
