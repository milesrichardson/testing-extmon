...
async function mKeyDialog(ph, fl, keyr, selector) {
  ...
  if (keyr) {
    ...
    $(".instruction-message", dialog).text(pfcol ? l.album_decr_key_descr : l[9048]);
    ...
    $(".mega-dialog.dlkey-dialog .instruction-message").safeHTML(l.album_decr_key_descr);
    $input[0].placeholder = "";
    if (document.body.classList.contains("theme-dark")) {
      $(".fm-dialog-new-folder-input", $dialog)[0].style.background = "black";
    }
    ...
    $(".mega-dialog.dlkey-dialog .instruction-message").safeHTML(l[7945] + "<br/>" + l[7972]);
  }
  ...
  $(".new-download-file-icon").addClass(
    fileIcon({
  ...
  while (elem) {
    ...
    if (pNode && pNode.tagName && !/html|body/i.test(pNode.tagName)) {
      if (typeof n === "undefined") {
        // count this in, except for overflow huge menu
        sl = elem.scrollLeft;
        st = elem.scrollTop;
      }
      cl = elem.clientLeft;
...
function secToDuration(s, sep) {
  ...
  var durStr = "";
  sep = sep || ", ";
  if (!secToDuration.regExp) {
    //regexp compile cache
...
function assertStateChange(currentState, newState, allowedStatesMap, enumMap) {
  ...
  if (!allowed) {
    assert(
      false,
      "State change from: " +
        constStateToText(enumMap, currentState) +
        " to " +
        constStateToText(enumMap, newState) +
        " is not in the allowed state transitions map."
    );
  }
}
...
mBroadcaster.once("startMega", function () {
  ...
  var isSafeTarget = function (link) {
    ...
    var allowed = [getBaseUrl(), getAppBaseUrl()];
...
function odqPaywallDialogTexts(user_attr, accountData) {
  ...
  var totalFiles =
    (accountData.stats[M.RootID] ? accountData.stats[M.RootID].files : 0) +
    ...
    if (user_attr.uspw.wts && user_attr.uspw.wts.length) {
      ...
      if (user_attr.uspw.wts.length === 2) {
        dialogText = dialogText
          .replace("%2", time2date(user_attr.uspw.wts[0], 1))
          .replace("%3", "")
  ...
  if (pro.membershipPlans && pro.membershipPlans.length) {
    ...
    for (var h = 0; h < pro.membershipPlans.length; h++) {
      if (pro.membershipPlans[h][4] === 1 && pro.membershipPlans[h][2] > spaceUsedGB && pro.membershipPlans[h][2] < minPlan) {
