...
var u_authring = { Ed25519: undefined, Cu25519: undefined, RSA: undefined };
...
var authring = (function () {
  ...
  const safeStateAssert = (promise, cond) => {
    ...
    if (cond === undefined || cond) {
      cond = window.u_attr && isUserHandle(window.u_handle) && typeof u_attr === "object" && u_attr.u === u_handle;
    }
  ...
  ns._PROPERTIES = { Ed25519: "authring", Cu25519: "authCu255", RSA: "authRSA" };
  ...
  ns._serialiseRecord = function (userhandle, fingerprint, method, confidence) {
    ...
    return base64urldecode(userhandle) + fingerprintString + String.fromCharCode((confidence << 4) | method);
  };
  ...
  ns.serialise = function (authring) {
    ...
    for (var userhandle in authring) {
      ...
      if (
        record.fingerprint.length % 20 !== 0 ||
      ...
      result += this._serialiseRecord(userhandle, record.fingerprint, record.method, record.confidence);
    }
  ...
  ns._deserialiseRecord = function (serialisedRing) {
    ...
    return { userhandle: userhandle, value: { fingerprint: fingerprint, method: method, confidence: confidence }, rest: rest };
  };
  ...
  ns.deserialise = function (serialisedRing) {
    ...
    while (rest.length > 0) {
      ...
      // Skip obviously faulty records.
      if (
        result.value.fingerprint.length % 20 !== 0 ||
  ...
  ns.setContacts = function (keyType) {
    ...
    return this.onAuthringReady("setContacts").then(() => {
      ...
      if (!mega.keyMgr.secure) {
        promises.push(Promise.resolve(mega.attr.set(ns._PROPERTIES[keyType], { "": ns.serialise(u_authring[keyType]) }, false, true)));
      }
   ...
   *     (e. g. UNSURE).
   */
  ns.setContactAuthenticated = function (userhandle, fingerprint, keyType, method, confidence) {
    assertUserHandle(userhandle);
    ...
    if (
      ...
      return ns.setContacts(keyType).then(() => {
  ...
  ns.computeFingerprint = function (key, keyType, format) {
    ...
    if (keyType === "Ed25519" || keyType === "Cu25519") {
      if (key.length !== 32) {
        logger.error("Unexpected key length for type " + keyType + ": " + key.length);
  ...
  ns.signKey = function (pubKey, keyType) {
    ...
    var detachedSignature = nacl.sign.detached(
      asmCrypto.string_to_bytes(keyString),
  ...
  ns.resetFingerprintsForUser = async function (userHandle) {
    ...
    return this.setContactAuthenticated(userHandle, fingerprint, "Ed25519", this.AUTHENTICATION_METHOD.SEEN, this.KEY_CONFIDENCE.UNSURE);
  };
  ...
  ns.initAuthenticationSystem = function () {
    ...
    if (ns._initialisingPromise !== false) {
      if (ns._initialisingPromise === true) {
        ...
        logger.error("Failed to initAuthSystem because of invalid _initialisingPromise state of: ", ns._initialisingPromise);
  ...
  ns._initKeyringAndEd25519 = function () {
    ...
    attributePromise.done(function __attributePromiseResolve(result) {
      ...
      u_pubEd25519 = asmCrypto.bytes_to_string(nacl.sign.keyPair.fromSeed(asmCrypto.string_to_bytes(u_privEd25519)).publicKey);
      ...
      Promise.all([Promise.resolve(authring.getContacts("Ed25519")), Promise.resolve(authring.getContacts("Cu25519"))])
        .then((res) => {
    ...
    attributePromise.fail(function __attributePromiseReject(result) {
      if (result === ENOENT) {
        ...
        var pubkeyPromise = mega.attr.set("puEd255", base64urlencode(u_pubEd25519), true, false);
        var authringPromise = authring.getContacts("Ed25519");

        var comboPromise = MegaPromise.all([keyringPromise, pubkeyPromise, authringPromise]);
        masterPromise.linkDoneAndFailTo(comboPromise);
      } else {
  ...
  ns._setupKeyPair = function (keyType) {
    ...
    var keyringPromise = mega.attr.set("keyring", u_keyring, false, false);
    var pubkeyPromise = mega.attr.set(crypt.PUBKEY_ATTRIBUTE_MAPPING[keyType], base64urlencode(pubKey), true, false);
    var signaturePromise = mega.attr.set(crypt.PUBKEY_SIGNATURE_MAPPING[keyType], base64urlencode(pubKeySignature), true, false);
    var authringPromise = authring.getContacts(keyType);

    return MegaPromise.all([keyringPromise, pubkeyPromise, authringPromise, signaturePromise]);
  };
  ...
  ns._initKeyPair = function (keyType) {
    ...
    var privKey = keyType === "RSA" ? u_privk : u_keyring[crypt.PRIVKEY_ATTRIBUTE_MAPPING[keyType]];

    if (privKey) {
      // Fire off various API calls we need downstream.
      var authringPromise = ns.getContacts(keyType);
      var signaturePromise = mega.attr.get(u_handle, crypt.PUBKEY_SIGNATURE_MAPPING[keyType], true, false);
      ...
      var sigKeyComboPromise = MegaPromise.all([gotSignaturePromise, pubkeyPromise]);
      sigKeyComboPromise.done(function __signatureComboResolve(result) {
        ...
        if (signature) {
          ...
          ns.verifyKey(signature, pubKey, keyType, u_pubEd25519).done(function (isVerified) {
            if (isVerified) {
              masterPromise.resolve();
            } else {
              // Signature fails, make a good one and save it.
              var pubKeySignature = authring.signKey(pubKey, keyType);
              var setSignaturePromise = mega.attr.set(
                crypt.PUBKEY_SIGNATURE_MAPPING[keyType],
                base64urlencode(pubKeySignature),
                true,
                false
              );
              var comboPromise = MegaPromise.all([authringPromise, setSignaturePromise]);
              masterPromise.linkDoneAndFailTo(comboPromise);
            }
          });
        } else {
          ...
          mega.attr.set(crypt.PUBKEY_SIGNATURE_MAPPING[keyType], base64urlencode(signature), true, false);
          masterPromise.resolve();
        }
  ...
  ns._checkPubKey = function (pubKey, keyType) {
    ...
    var attributePromise = mega.attr.get(u_handle, crypt.PUBKEY_ATTRIBUTE_MAPPING[keyType], true, false);
    ...
    attributePromise.done(function (result) {
      ...
      if (storedPubKey === pubKey) {
        ...
        masterPromise.linkDoneAndFailTo(mega.attr.set(crypt.PUBKEY_ATTRIBUTE_MAPPING[keyType], base64urlencode(pubKey), true, false));
      }
    });
