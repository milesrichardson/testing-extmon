...
function browserdetails(useragent) {
  ...
  if (useragent.indexOf("windows phone") > 0) {
    ...
    os = "iPad";
  } else if (useragent.indexOf("mac") > 0 || useragent.indexOf("darwin") > 0) {
    os = "Apple";
  ...
  if (os && browser) {
    ...
    nameTrans = String(l && l[7684])
      .replace("%1", displayName || brand || browser)
      .replace("%2", os);
  } else if (os) {
    name = os;
  ...
  details.version = (useragent.match(RegExp(verSep + (verTag || brand || browser) + "/([\\d.]+)", "i")) || [])[1] || 0;
