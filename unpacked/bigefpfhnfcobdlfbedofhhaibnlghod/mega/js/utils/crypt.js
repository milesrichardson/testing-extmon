...
var crypt = (function () {
  ...
  ns.getPubKeyAttribute = function (userhandle, keyType, userData) {
    ...
    if (keyType === "RSA") {
      ...
      var pubKeyPromise =
        userData && userData.chatHandle
          ? mega.attr.get(userhandle, ns.PUBKEY_ATTRIBUTE_MAPPING[keyType], true, false, undefined, undefined, userData.chatHandle)
  ...
  ns._getPubKeyAuthentication = function (userhandle, pubKey, keyType) {
    ...
    if (recorded && authring.equalFingerprints(recorded.fingerprint, fingerprint) === false) {
      logger.warn("Error verifying authenticity of " + keyType + " pub key: " + "fingerprint does not match previously authenticated one!");
  ...
  ns.getPubKey = function (userhandle, keyType, callback) {
    ...
    var __finish = function (pubKey, authMethod, newAuthMethod) {
      if (typeof newAuthMethod !== "undefined" && newAuthMethod !== authMethod) {
        authring.setContactAuthenticated(userhandle, fingerprint, keyType, newAuthMethod, authring.KEY_CONFIDENCE.UNSURE);
      }
      __callbackAttachAfterDone(masterPromise);
    ...
    getPubKeyPromise.done(function __resolvePubKey(result) {
      ...
      if (keyType === "Ed25519") {
        ...
        if (authMethod === false) {
          ...
          ns._showFingerprintMismatchException(userhandle, keyType, authMethod, authRecord.fingerprint, fingerprint);
    ...
    var __resolveSignatureVerification = function (result) {
      ...
      ns._checkSignature(signature, pubKey, keyType, signingKey).done(function (signatureVerification) {
        if (authMethod === false) {
    ...
    if (keyType === "Ed25519") {
      ...
      var signatureVerificationPromise = MegaPromise.all([getPubKeyPromise, pubEd25519KeyPromise, signaturePromise]);
  ...
  ns.getFingerprintEd25519 = function (userhandle, format) {
    ...
    if (pubEd25519[userhandle]) {
      ...
      var value = authring.computeFingerprint(pubEd25519[userhandle], "Ed25519", format);
  ...
  ns.getPubKeyFromPrivKey = function (privKey, keyType) {
    var result;
    if (keyType === "Ed25519") {
      result = nacl.sign.keyPair.fromSeed(asmCrypto.string_to_bytes(privKey)).publicKey;

      return asmCrypto.bytes_to_string(result);
    } else if (keyType === "Cu25519") {
      result = nacl.scalarMult.base(asmCrypto.string_to_bytes(privKey));
   ...
   *     The new fingerprint.
   */
  ns._showFingerprintMismatchException = function (userHandle, keyType, method, prevFingerprint, newFingerprint) {
    ...
    authring.showCryptoWarningDialog("credentials", userHandle, keyType, prevFingerprint, newFingerprint).dump("cred-fail");
    ...
    logger.warn(
      keyType +
        " fingerprint does not match the previously authenticated one!\n" +
