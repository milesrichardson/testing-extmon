lazy(mega.utils, "subtitles", () => {
  ...
  const selectTableRow = (dialog, { currentTarget }) => {
    ...
    currentTarget.classList.toggle(
      "ui-selected",
      !currentlySelected || currentlySelected.id === selectedHandle || !currentlySelected.classList.remove("ui-selected") // If landed here, it's always true
    );
  ...
  const subtitlesGridTable = document.getElementById("subtitles-dialog-subtitles-grid-table").content.querySelector("div");
      ...
      if (this.nodes.length) {
        ...
        $(document).rebind("keydown.subtitle_row", (e) => {
          ...
          const currentTarget = selectedHandle
            ? rows.querySelector(`tr.ui-selected`)[e.keyCode === 38 ? "previousElementSibling" : "nextElementSibling"]
  ...
  class Subtitles {
    ...
    removeTags(cue) {
      ...
      return doc.body.textContent.replace(/<(?!\/?([biu]|br))[^>]*>/gi, "").replace(/(\[|{)(\/?)([biu]|br)(]|})/gi, "<$2$3>");
    }
  }
  ...
  class SubtitlesFactory {
    ...
    initParsers() {
      this._parsers = [new SRTSubtitles(), new VTTSubtitles()];
    }
  ...
  class SubtitlesManager {
    ...
    async fetch(index) {
      return new Promise((resolve, reject) => {
        M.gfsfetch(this._nodes[index].h, 0, -1)
          .then((data) => {
            if (data.buffer === null) {
              return reject();
            }
