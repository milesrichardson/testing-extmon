...
lazy(self, "csp", () => {
  ...
  return Object.freeze({
    ...
    showCookiesDialog: promisify((resolve, reject, step, chg) => {
      ...
      const forEachCell = (cb, p = "") =>
        qsa(".settings-cell" + p, (cell) => {
      ...
      const save = async (ev) => {
        ...
        if (!window.buildOlderThan10Days) {
          const type =
            ev.type === "dialog-closed" ? 1 : $(ev.target).hasClass("accept-cookies") ? 2 : ev === -0xbadf ? 3 : first ? 4 : chg ? 5 : 0;

          const ctx = is_extension ? 1 : location.host === "mega.nz" ? 2 : location.host === "mega.io" ? 3 : location.host;
