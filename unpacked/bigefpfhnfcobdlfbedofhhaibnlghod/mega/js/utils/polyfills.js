...
mBroadcaster.once("startMega", function () {
  ...
  if (typeof document.exitFullscreen !== "function") {
    document.exitFullscreen =
      document.mozCancelFullScreen || document.webkitCancelFullScreen || document.msExitFullscreen || function () {};
  }
});
    ...
    if (window.Intl && !Intl.NumberFormat.prototype.formatToParts) {
      Intl.NumberFormat.prototype.formatToParts = function (n) {
        ...
        tryCatch(
          function () {
            result = Number(n)
              .toLocaleString(getCountryAndLocales().locales)
              .match(/(\d+)(\D)(\d+)/);
          },
          function () {
            result = Number(n)
...
(() => {
  ...
  Promise.prototype.dump = function (tag) {
    ...
    this.then(console.debug.bind(console, tag || "OK")).catch(console.warn.bind(console, tag || "FAIL"));
    return this;
  };
      ...
      value: function flat(depth = 1) {
        return depth < 2
          ? depth
            ? concat(...this)
            : this
          : reduce.call(this, (a, o) => ((Array.isArray(o) && a.push(...flat.call(o, depth - 1))) || a.push(o)) && a, []);
      }
    });
  }
...
mBroadcaster.once(
  ...
  tryCatch(() => {
    ...
    scriptTest("megaecmastest = window.foo?.bar ?? -1", (error) => {
    ...
    if (typeof BigInt === "undefined") {
      ...
      scriptTest("megaecmabitest = BigInt(Math.pow(2, 48)) << 16n === 18446744073709551616n", (error) => {
