...
lazy(mega, "icu", () => {
    ...
    for (let k = start; k < msg.length; k++) {
      ...
      if (msg[k] === "{") {
        ...
        if (openFound === 0) {
          ...
          reportError(`---- Parsing error, failed to parse Plural set, not matched brackets. ` + `value = ${msg}`);
          return null;
        }
      }
    }
  ...
  return Object.freeze({
    ...
    format: (msg, count, localizeCount) => {
      ...
      reportError(`---- Parsing error, Plural values for count ${count} not found.` + `value = ${JSON.stringify(stringDic)}`);
