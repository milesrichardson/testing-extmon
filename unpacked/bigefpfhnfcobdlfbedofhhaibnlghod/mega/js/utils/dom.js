...
function parseHTML(markup) {
  ...
  $.parseHTML(markup, doc).forEach((node) => {
    ...
    if (!invalid) {
      invalid = domNodeForEach(node, (n) => {
        ...
        return (
          dumb[nn] ||
          domAttributeForEach(n, ({ name }) => {
            // console.warn('domAttrForEach(%s:%s)', a.name, a.value, [a], [n]);

            return (name[0] === "o" && name[1] === "n") || (nn !== "IMG" && name[0] === "s" && name[1] === "r" && name[2] === "c");
          })
        );
      });
    }
...
Object.defineProperty(xmlParser, "fetch", {
  value: async function xmlParserFetch(...args) {
    ...
    res = Object.assign(Object.create(null), xmlParser[headers["content-type"] === "application/json" ? "fromJSON" : "fromString"](text));
