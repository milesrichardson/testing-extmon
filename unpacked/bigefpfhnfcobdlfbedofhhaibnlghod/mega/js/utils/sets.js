...
lazy(mega, "sets", () => {
        ...
        for (let i = 0; i < commands.length; i++) {
          ...
          switch (command) {
            ...
            case "ba":
              ...
              break;
            case "u":
              bulks.u[id] = bulks.u[id] ? { ...bulks.u[id], ...data } : data;
              break;
            case "d":
              bulks.d.push(id);
              break;
   ...
   */
  const groupById = (array) => array.reduce((obj, v) => Object.assign(obj, { [v.id]: v }), {});
  ...
  const encryptSetAttr = (attrData, key = undefined) => {
    const keyArr = typeof key === "string" ? decrypt_key(u_k_aes, base64_to_a32(key)) : [...crypto.getRandomValues(new Uint32Array(4))];
  ...
  const elements = {
    ...
    add: (h, setId, setKey) => {
      ...
      return sendScReq("aep", {
        h,
        s: setId,
        k: ab_to_base64(asmCrypto.AES_CBC.encrypt(a32_to_ab(n.k), a32_to_ab(decrypt_key(u_k_aes, base64_to_a32(setKey))), false))
      });
    },
    /**
    ...
    bulkAdd: async (handles, setId, setKey) => {
      ...
      for (let i = 0; i < handles.length; i++) {
        ...
        const el = {
          h,
          o,
          k: ab_to_base64(asmCrypto.AES_CBC.encrypt(a32_to_ab(n.k), setPubKey, false))
        };
  ...
  const getPublicSetTree = (s, e, n, sn) => {
    ...
    if (Array.isArray(e) && e.length) {
      ...
      const decrNodeKey = tryCatch((k) => base64_to_a32(ab_to_base64(asmCrypto.AES_CBC.decrypt(base64_to_ab(k), psKeyAb, false))));
  ...
  return {
    ...
    parseAsp: async (payload) => {
      ...
      if (isExisting) {
        // The album is already stored, hence needs an update only
