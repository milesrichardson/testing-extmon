...
function CreateWorkers(url, message, size) {
  ...
  return new MegaQueue(
    function _(task, done) {
      var i = size;
      var self = this;
    ...
    size,
    url.split("/").pop().split(".").shift() + "-worker"
  );
}
  ...
  backgroundNacl.sign = {
    detached: {
      ...
      verify: function (msg, sig, publicKey, tagName) {
        ...
        backgroundNacl.workers.push([taskId, ["verify", msg, sig, publicKey]], function () {
          masterPromise.resolve(arguments[1][0]);
        });
