  ...
  constructor(ex, data = null, name = "InvalidStateError") {
    ...
    if (!stack) {
      ...
      if (stack && typeof stack === "string") {
        stack = stack.replace("Error:", `${name}:`).split("\n").filter(MEGAException.stackFilter).join("\n");
      }
    }
...
class MEGAImageElement {
  ...
  async createImageBitmap(source) {
    ...
    if (self.supImageBitmap) {
      const bitmap = await createImageBitmap(source).catch((ex) => {
        if (self.d) {
...
class MEGACanvasElement extends MEGAImageElement {
  ...
  isTainted(aImageData) {
    ...
    if (!result && width > 7 && data.byteLength > 63) {
      ...
      for (let i = width << 3, p = data.length - i, ed = 0; i > 4; ) {
        ed += Math.pow(data[i - 4] - data[p], 2) + Math.pow(data[i - 3] - data[p + 1], 2) + Math.pow(data[i - 2] - data[p + 2], 2);
  ...
  async alignTargetSize({ sw, sh, maxWidth, maxHeight }) {
    ...
    } else if (maxWidth > MEGAImageElement.THUMBNAIL_SIZE && (await this.contentAwareCroppingMethod()) === "SmartCrop") {
    ...
    if (source.width >= maxWidth || source.height >= maxHeight) {
      ...
      if (method === "FaceDetector") {
        ...
        if (faces && faces.length) {
          ...
          if (sx | sy | sw | sh) {
            [sx, sy, sw, sh] = this.getScaledCropArea(source, sx, sy, sw - sx, sh - sy, maxWidth, maxHeight);
  ...
  async getBoundingRect({ source, maxWidth, maxHeight, ...options }) {
    const dest = {};
    let { sx, sy, sw, sh, dx, dy, dw, dh } = await this.getCropRect({ source, maxWidth, maxHeight, ...options, dest });
  ...
  async createPreviewImage(source, options = false) {
    ...
    const { crop = "none", type = defType, maxWidth = defSize, maxHeight = defSize, quality = defQuality } = options;

    source = await this.getCanvasImageSource(source);
    const { sx, sy, sw, sh, dx, dy, dw, dh } = await this.getBoundingRect({ crop, source, maxWidth, maxHeight, ...options });
    ...
    /** @todo https://crbug.com/1082451
      ...
      this.debug(`Creating ${dw}x${dh} image using ${this.renderingContextType}...`, { sx, sy, sw, sh }, { dx, dy, dw, dh }, source);
    }
  ...
  drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh, type, quality = MEGAImageElement.DEFAULT_QUALITY) {
    ...
    return this._drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh, type, quality).catch((ex) => {
...
class WebGLMEGAContext extends MEGACanvasElement {
  ...
  bindBuffers(gl, image, sx, sy, sw, sh, dx, dy, dw, dh) {
    ...
    const tex = this.enableVertexAttrib(gl, texcoord, new Float32Array([u0, v0, u1, v0, u0, v1, u0, v1, u1, v0, u1, v1]));
    ...
    const pos = this.enableVertexAttrib(gl, position, new Float32Array([x1, y1, x2, y1, x1, y2, x1, y2, x2, y1, x2, y2]));
  ...
  getErrorString(code) {
    if (!WebGLMEGAContext.glContextErrorMap) {
      WebGLMEGAContext.glContextErrorMap = Object.keys(WebGL2RenderingContext).reduce((o, p) => {
  ...
  async _drawImage(image, sx, sy, sw, sh, dx, dy, dw, dh, type, quality = MEGAImageElement.DEFAULT_QUALITY) {
    ...
    if (!error && type) {
      result = await this.convertTo(type, quality).catch((ex) => {
        error = ex;
      });
    }
...
class MEGAWorker extends Array {
  ...
  get stats() {
    const res = [this.running.size, this.pending.length, this.wkCompletedJobs, this.wkDisposalThreshold, Object.isFrozen(this) | 0];
...
Object.defineProperties(MEGAWorker, {
  ...
  getTransferable: {
    value: (payload, refs = new WeakSet()) => {
      ...
      if (payload) {
        const nonIterable = typeof payload !== "object" || payload instanceof Blob || "BYTES_PER_ELEMENT" in payload || "width" in payload;
...
((self) => {
  ...
  const dump = (() => {
    ...
    const rgb = [`color:#${((r) => ((r() << 16) | (r() << 8) | r()).toString(16))(() => ~~(Math.random() * 0x9f + 96))}`, "color:inherit"];
  ...
  const handlers = {
    ...
    scissor: async (data) => {
      ...
      const thumbnail = createThumbnail && (await webgl.createThumbnailImage(source).then((blob) => webgl.readAsAESCBCBuffer(blob)));
      const preview = createPreview && (await webgl.createPreviewImage(source).then((blob) => webgl.readAsAESCBCBuffer(blob)));
  ...
  lazy(self, "webgl", () => {
    ...
    waiter = (async () => {
      ...
      await (async () => {
        ...
        v = !v && typeof ImageBitmapRenderingContext !== "undefined" && new MEGACanvasElement(1, 1, "bitmaprenderer").ctx;
