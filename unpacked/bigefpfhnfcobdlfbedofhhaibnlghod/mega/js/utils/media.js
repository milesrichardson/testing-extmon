...
function is_image2(n, ext) {
  "use strict";
  ext = ext || fileext((n && n.name) || n, true, true);
  return (is_image(n, ext) && (!mega.chrome || !n || n.s < 6e8)) || (filemime(n).startsWith("image") && String(n.fa).indexOf(":1*") > 0);
}
...
mThumbHandler.add("SVG", function SVGThumbHandler(ab, cb) {
  ...
  image.src = "data:image/svg+xml;charset=utf-8," + encodeURIComponent(ab_to_str(ab).replace(/foreignObject|script/g, "desc"));
});
...
mBroadcaster.once(
  ...
  tryCatch(() => {
    "use strict";
    const images = [
      ["JXL", "image/jxl", "/woAkAEAEogCAMAAPeESAAAVKqOMG7yc6/nyQ4fFtI3rDG21bWEJY7O9MEhIOIONi4LdHIrhMyApVJIA"],
      ["WEBP", "image/webp", "UklGRjoAAABXRUJQVlA4IC4AAACyAgCdASoCAAIALmk0mk0iIiIiIgBoSygABc6WWgAA/veff/0PP8bA//LwYAAA"],
      [
        "AVIF",
        "image/avif",
...
function setImage(n, ab, options) {
  "use strict";
  var aes = new sjcl.cipher.aes([n.k[0] ^ n.k[4], n.k[1] ^ n.k[5], n.k[2] ^ n.k[6], n.k[3] ^ n.k[7]]);
  ...
  return new Promise(function (resolve, reject) {
    var parse = tryCatch(function (ab) {
      ...
      var readFrame = function (offset) {
        var id = String.fromCharCode.apply(String, [u8[offset], u8[offset + 1], u8[offset + 2], u8[offset + 3]]);
      ...
      if (u8[0] === 0x49 && u8[1] === 0x44 && u8[2] === 0x33 && u8[3] === 3) {
        ...
        while (offset < size) {
          ...
          if (frame.id === "APIC") {
            ...
            while (u8[offset++]) {}
            ++offset;
            if ((u8[offset] === 0xff && u8[offset + 1] === 0xfe) || (u8[offset] === 0xfe && u8[offset + 1] === 0xff)) {
              while (u8[offset]) {
                offset += 2;
              }
              offset += 3;
...
(function _initVideoStream(global) {
  ...
  var _initVideoControls = function (wrapper, streamer, node, options) {
    ...
    var changeButtonState = function (type) {
      ...
      if (type === "playpause" && !timeDrag) {
        if (is_embed && (videoElement.paused || videoElement.ended)) {
          ...
          $("i", $playpause).removeClass("icon-pause-small-regular-outline").addClass("icon-rotate-ccw-small-regular-outline");
          ...
          if (is_embed) {
            ...
            $("i", $playpause).removeClass("icon-pause-small-regular-outline").addClass("icon-play-small-regular-outline");
          }
          ...
          $("i", $playPauseButton).removeClass("icon-play-regular-solid").addClass("icon-pause-small-regular-solid");
          ...
          if ($("i", $playPauseButton).hasClass("icon-pause-small-regular-solid")) {
            $playPauseButton.removeClass("hidden");
            $("i", $playPauseButton).addClass("icon-play-regular-solid").removeClass("icon-pause-small-regular-solid");
            tSleep(2.5).then(() => $playPauseButton.addClass("hidden"));
          }
        }
      }
      // Mute button
        ...
        if (videoElement.muted) {
          ...
          if (is_embed) {
            ...
            $("i", $mute)
              .removeClass()
              .addClass(`sprite-fm-mono ${volumeIcon(streamer.volume)}`);
          }
    ...
    if (options.filter) {
      applyVideoFilter.apply(
        this,
        options.filter.map((v) => v / 10)
      );
    }
    ...
    $video.rebind("play pause", function () {
      if (
        ...
        ($.dialog === "subtitles-dialog" ||
          ($(".context-menu.subtitles", $wrapper).get(0) && $(".context-menu.subtitles", $wrapper).get(0).classList.contains("visible")))
      ) {
        videoElement.pause();
      } else {
    ...
    $progress.rebind("mousemove.videoprogress", function (e) {
      $(".video-progress-time", $videoControls)
        .css("left", getX(e) - 15.4 + "px")
        .text(secondsToTimeShort(getTimeOffset(getX(e)).time, 1));
      $(".video-expected-time-bar", $videoControls).css("width", (100 * (getX(e) - $progress.offset().left)) / $progress.width() + "%");
    });
    ...
    var setFullscreenData = function (state) {
      ...
      if (state) {
        $("i", $fullscreen).removeClass("icon-maximize-02-small-regular-outline").addClass("icon-minimize-02-small-regular-outline");
        $(".fs-wrapper .tooltip", $wrapper).text(l.video_player_exit_fullscreen);
      } else {
        $("i", $fullscreen).removeClass("icon-minimize-02-small-regular-outline").addClass("icon-maximize-02-small-regular-outline");
    ...
    $(".context-menu.playback-speed button", $wrapper).rebind("click.media-viewer", function () {
      ...
      if ($this.hasClass("05x")) {
        $("i", $speed).removeClass().addClass("sprite-fm-mono icon-playback-05x-small-regular-outline icon-size-36");
        ...
        $("i", $speed).removeClass().addClass("sprite-fm-mono icon-playback-15x-small-regular-outline icon-size-36");
        $speed.addClass("margin-2");
        $(".context-menu.playback-speed button i", $wrapper).addClass("hidden");
  ...
  var _initVideoStream = function (node, $wrapper, destroy, options) {
    ...
    if (d && options.bitrate && typeof bytesToSize === "function") {
      console.info("%s, %s", node.name, bytesToSize(node.s), secondsToTime(options.playtime), bytesToSpeed(options.bitrate));
    }
    ...
    s.on("inactivity", function (ev) {
      ...
      if (this.isOverQuota) {
        ...
        var data = [
          ...
          s.hasVideo,
          s.hasAudio,
          Math.round(s.getProperty("bitrate")),
          s.getProperty("server"),
          s.playbackTook,
          node.ph || node.h
        ];
    ...
    s.on("error", function (ev, error) {
      ...
      if (!d && String(Object.entries).indexOf("native") > 0 && !window.buildOlderThan10Days && emsg && emsg !== l[23]) {
  ...
  global.initVideoStream = function (node, wrapper, destroy, options) {
    ...
    return new MegaPromise(function (resolve, reject) {
      ...
      M.require("videostream").tryCatch(
        function () {
          resolve(_init());
        },
        function (ex) {
  ...
  global.iniVideoStreamLayout = function (node, $wrapper, options) {
    ...
    return new MegaPromise(function (resolve, reject) {
      MediaAttribute.canPlayMedia(node)
        .then(function (yup) {
          var $video = $("video", $wrapper);
          var c = MediaAttribute.getCodecStrings(node);
          if (c) {
...
(function _MediaInfoLib(global) {
  ...
  if (d > 7) {
    mediaInfoExtensions += ".aa3.amr.ape.asf.au.caf.dif.drc.dv.jpx.la.lxf.mpc.mp+.mtv.nsv.nut.oma.pss.qcp.rka.shn.tak.tp.vqf.wv.y4m.";
  }
    ...
    return new Promise(function (resolve) {
      ...
      var saveNode = function (n, err, res) {
        if (err) {
          ...
          summary[n.h] = n.h + ":..." + String(n.name).substr(-7) + ":" + res.container + ":" + res.vcodec + ":" + res.acodec;
        }
  ...
  MediaInfoLib.test = function (files) {
    ...
    MediaInfoLib.collect(files, {
      ...
      .then(function (res) {
        ...
        console.log(
          "MediaInfoLib.test finished, processed %s/%s (%s%% media) files, %s%% were unrecognized",
          cnt,
          files.length,
          ((cnt * 100) / files.length) | 0,
          ((unknown.length * 100) / cnt) | 0
        );
  ...
  MediaInfoLib.prototype.getMetadata = function (file) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      if (mediaInfoTasks) {
        ...
        Promise.all([self._loadMediaInfo(), MediaInfoLib.getMediaCodecsList()]).then(self._parseMediaFile.bind(self, task)).catch(reject);
      }
    });
  };
  ...
  MediaInfoLib.prototype._loadMediaInfo = function () {
    return new Promise(function (resolve, reject) {
      if (mediaInfoLib) {
        ...
        M.require("mediainfo").tryCatch(function () {
          var options = {
  ...
  MediaInfoLib.prototype._parseMediaFile = function (task) {
    ...
    (function _nextChunk(byteLength) {
      self
        .fetcher(entry, byteOffset, byteLength)
        .then(function (chunk) {
          var file = entry.file;
          ...
          if (state & 8 || !length) {
            ...
            if (d) {
              console.debug("MediaInfoLib(%s:%s) len=%s, state=%s, offset=%s", file.name, file.size, length, state, byteOffset, seekTo);
            }
          ...
          if (finished) {
            ...
            if (d) {
              ...
              console.debug(
                "MediaInfoLib(%s:%s) Parsing finished, state=%s, bytesRead=%s",
                file.name,
                file.size,
                state,
                bytesRead,
...
(function _MediaAttribute(global) {
  ...
  MediaAttribute.setAttribute = function (entry) {
    return new Promise(function (resolve, reject) {
      if (!(entry instanceof MegaNode)) {
        if (typeof entry !== "object" || String(entry.h).length !== 8 || !Array.isArray(entry.k) || entry.k.length !== 8) {
        ...
        if (n) {
          ...
          if (res.playtime && M.viewmode && n.p === M.currentdirid) {
            $("#" + n.h)
              .find(".data-block-bg")
              .addClass("video")
              .find(".video-thumb-details span")
              .text(secondsToTimeShort(res.playtime));
          }
        }
        self.fa = fa;
