...
async function megaUtilsGFSFetch(aData, aStartOffset, aEndOffset, aProgress) {
  ...
  if ((!aStartOffset && aStartOffset !== 0) || aStartOffset > aData.s || !aEndOffset || aEndOffset < aStartOffset) {
  ...
  if (typeof aProgress === "function") {
    aProgress = (
      (cb) => (ev) =>
        ev.lengthComputable && cb((ev.loaded / ev.total) * 100, ev.loaded, ev.total)
    )(aProgress);
  ...
  const ev = await (Array.isArray(aData.g) ? CloudRaidRequest.fetch(aData, aStartOffset, aEndOffset, aProgress) : M.xhr(request));
  aData.macs = {};
  aData.writer = [];
  ...
  if (!aData.nonce) {
    ...
    aData.nonce = JSON.stringify([key[0] ^ key[4], key[1] ^ key[5], key[2] ^ key[6], key[3] ^ key[7], key[4], key[5]]);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    Decrypter[method](
      [[aData, aStartOffset], aData.nonce, aStartOffset / 16, uint8],
      tryCatch(() => {
