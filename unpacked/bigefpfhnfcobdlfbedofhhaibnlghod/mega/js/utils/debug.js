...
lazy(self, "runKarmaTests", () => {
  ...
  const chai = {
    ...
    instanceOf: (v) => assert(value instanceof v, `Expected '${name(value)}' to be instance of ${name(v)}`),
    ...
    setInterval(function () {
      ...
      for (var uri in usages) {
        ...
        if (now - data.ts > intv) {
          ...
          if (warn && !(data.r++ % 10)) {
            console.warn(
              "[$%s] a call to createObjectURL was not revoked" + " in too long, possible memory leak?",
              data.id.toString(16),
              uri,
              data
            );
...
mBroadcaster.once("boot_done", function radSetup() {
  ...
  "debug,error,info,log,warn,table,group,groupEnd,assert".split(",").map(
    tryCatch((fn) => {
