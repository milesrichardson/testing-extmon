...
(function (global) {
  ...
  array.pack = function (input, enc) {
    ...
    if (d) {
      console.debug("array.pack saved %s bytes (%s%)", ilen - result.length, 100 - Math.round((result.length * 100) / ilen));
    }
  ...
  array.diff = function (old_arr, new_arr) {
    return {
      removed: old_arr.filter(function (v) {
        return new_arr.indexOf(v) < 0;
      }),
      added: new_arr.filter(function (v) {
        return old_arr.indexOf(v) < 0;
      })
    };
  };
...
function makeEnum(aEnum, aPrefix, aTarget, aNorm) {
  ...
  while (len--) {
    Object.defineProperty(aTarget, (aPrefix || "") + String(aEnum[len]).toUpperCase(), {
