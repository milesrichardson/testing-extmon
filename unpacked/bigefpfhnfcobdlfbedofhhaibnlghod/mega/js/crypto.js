...
var crypto_rsagenkey = promisify(function _crypto_rsagenkey(resolve, reject, aSetRSA) {
  ...
  function _done(k) {
    ...
    logger.debug("Key generation took " + (endTime.getTime() - startTime.getTime()) / 1000.0 + " seconds!");
    ...
    api_req(
      { a: "whyamiblocked" },
      {
        callback: function whyAmIBlocked(reasonCode) {
          ...
          msgDialog("warninga", dialogTitle, reasonText, false, function () {
...
async function api_createuser(ctx, invitecode, invitename) {
  ...
  if (invitecode) {
    ...
    const { clientRandomValueBytes, encryptedMasterKeyArray32, hashedAuthenticationKeyBytes, derivedAuthenticationKeyBytes } =
    ...
    if (typeof res.k === "string") {
      ...
      if (k.length === 4) {
        ...
        if (typeof res.tsid === "string") {
          ...
          if (a32_to_str(encrypt_key(aes, str_to_a32(t.substr(0, 16)))) === t.substr(-16)) {
            r = [k, res.tsid];
          }
          ...
          if (sessionIdUserHandle !== res.u) {
            console.error("User handle in Session ID did not match user handle from the 'us' request", res.u, sessionIdUserHandle);
...
function crypto_decodeprivkey(privk, errobj) {
  ...
  if (!(p.bitLength > 1000 && q.bitLength > 1000 && d.bitLength > 2000 && u.bitLength > 1000 && hexU === hexInv)) {
    return false;
  }
...
fa_handler.prototype = {
  ...
  GetFA: function (response) {
    ...
    const decrypt = tryCatch(
      (k) => {
        const ts = new Uint8Array(response, p, l);

        const data = asmCrypto.AES_CBC.decrypt(ts, a32_to_ab([k[0] ^ k[4], k[1] ^ k[5], k[2] ^ k[6], k[3] ^ k[7]]), false);
...
function api_faretry(ctx, error, host) {
  ...
  if (ctx.errfa && ctx.errfa.timeout && ctx.faRetryI > ctx.errfa.timeout) {
    ...
    logger.debug("Attribute " + (ctx.p ? "retrieval" : "storage") + " failed (" + error + "), retrying...", ctx.faRetryI);
...
function crypto_procsr(sr) {
  ...
  ctx.callback = function (res, ctx) {
    if (ctx.sr) {
      ...
      if (typeof res === "object" && typeof res.pubk === "string") {
...
function crypto_rsacheck(n) {
  ...
  if (
    typeof n.k == "string" && // must be undecrypted
