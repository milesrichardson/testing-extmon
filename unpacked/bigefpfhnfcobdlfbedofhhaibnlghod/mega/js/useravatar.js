...
var useravatar = (function () {
  ...
  function _getAvatarSVGDataURI(user) {
    ...
    var $template = $("#avatar-svg")
      .clone()
      .removeClass("hidden")
      .find("svg")
      .addClass("color" + s.colorIndex)
      .end()
      .find("text")
      .text(s.letters)
      .end();
  ...
  function _getAvatarImageContent(url, id, className, type, bg) {
    ...
    if (bg) {
      bgBlock = '<div class="avatar-bg colorized">' + '<span class="colorized"></span>' + "</div>";
    }
  ...
  function isUserVerified(userHandle) {
    ...
    authring
      .onAuthringReady("avatar-v")
      .then(function isUserVerified_Callback() {
  ...
  ns.loaded = function (user) {
    ...
    if (user === u_handle) {
      ...
      $(".fm-avatar img,.fm-account-avatar img, .top-menu-popup .avatar-block img", "body").attr("src", myavatar);
    ...
    var updateAvatar = function () {
      ...
      $this.removeClass($this.data("color")).addClass($avatar.data("color")).data("color", $avatar.data("color")).safeHTML($avatar.html());
    };

    $(".avatar-wrapper." + user.replace(/[^\w-]/g, "") + ":not(.in-chat)").each(updateAvatar);

    if ((M.u[user] || {}).m) {
      var eem = String(M.u[user].m)
        .replace(/[^\w@.,+-]/g, "")
        .replace(/\W/g, "\\$&");
  ...
  (function loadAvatarStub(ns) {
    ...
    ns.loadAvatar = function (handle, chathandle) {
      ...
      mega.attr
        .get(handle, "a", true, false, undefined, undefined, chathandle)
        .fail(reject)
        .done(function (res) {
          var error = res;
