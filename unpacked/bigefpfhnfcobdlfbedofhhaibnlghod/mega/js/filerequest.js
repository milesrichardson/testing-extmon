...
lazy(mega, "fileRequest", () => {
  ...
  const openNewDialogHandler = () => {
    openNewFileRequestDialog().then(openCreateDialogFromSelect).catch(dump);
    return false;
  };
  ...
  class CommonDialog extends BaseDialog {
    constructor() {
      ...
      this.$previewButtonFooter = new mega.fileRequestUI.PreviewButtonComponent($("footer .file-request-preview-button", this.$dialog));
      this.$previewButtons = new mega.fileRequestUI.PreviewButtonComponent($(".file-request-preview-button", this.$dialog));

      this.$inputTitle = new mega.fileRequestUI.ValidatableInputComponent($(".file-request-title", this.$sectionPrimary), {
        validations: {
          limit: {
            max: 80,
      ...
      this.$inputDescription = new mega.fileRequestUI.ValidatableInputComponent($(".file-request-description", this.$sectionPrimary), {
        validations: {
          limit: {
            max: 500,
      ...
      this.$themeButton = new mega.fileRequestUI.RadioComponent($(this.themeButtonSelector, this.$dialog), { namespace: "frm" });
      this.themeButtonWrapper = ".embed-button-select";
      this.$themeButtonWrapper = new mega.fileRequestUI.ButtonComponent($(this.themeButtonWrapper, this.$dialog), { namespace: "frm" });

      this.$embedCode = new mega.fileRequestUI.EmbedCodeInputComponent($(".file-request-embed-code", this.$dialog));
      this.$shareLink = new mega.fileRequestUI.ShareLinkInputComponent($(".file-request-share-link", this.$dialog));
      ...
      this.$closeButton = new mega.fileRequestUI.CloseButtonComponent($("button.close, .file-request-close-button", this.$dialog), {
      ...
      const titleDescInputPostCallback = function (selfObject, options) {
        ...
        const limit = (options && options.validations && options.validations.limit) || 0;
        ...
        if (selfObject.getValue()) {
          ...
          selfObject.getInput().closest(megaInputSelector).addClass(activeClass);
          return;
        }

        $charCount.text(``);
        selfObject.getInput().closest(megaInputSelector).removeClass(activeClass);
      };
  ...
  class CommonMobileDialog extends BaseDialog {
    createTitleDescInput(options) {
      ...
      this.$inputTitle = new mega.fileRequestUI.ValidatableMobileComponent($(".file-request-title", this.$dialog), {
        validations: {
          limit: {
            max: 80,
            message: l.file_request_dialog_label_title_invalid
      ...
      this.$inputDescription = new mega.fileRequestUI.ValidatableMobileComponent($(".file-request-description", this.$dialog), {
        validations: {
          limit: {
            max: 500,
            message: l.file_request_dialog_label_desc_invalid,
  ...
  class CreateDialog {
    ...
    checkLoseChangesWarning() {
      if (this.commonDialog.$inputTitle.getValue().length || this.commonDialog.$inputDescription.getValue().length) {
        return true;
      }
    }
  }
  ...
  class ManageDialog {
    ...
    checkLoseChangesWarning() {
      ...
      if (this.commonDialog.$inputTitle.getValue() !== title || this.commonDialog.$inputDescription.getValue() !== description) {
        return true;
      }
    }
  }
  ...
  class FileRequestContextMenu {
    constructor() {
      ...
      this.$createButton = new mega.fileRequestUI.ButtonComponent($(".dropdown-item.file-request-create", this.$contextMenu), {
        namespace: "frcm"
      });
      this.$manageButton = new mega.fileRequestUI.ButtonComponent($(".dropdown-item.file-request-manage", this.$contextMenu), {
        namespace: "frcm"
      });

      this.$copyLinkButton = new mega.fileRequestUI.CopyButtonComponent($(".dropdown-item.file-request-copy-link", this.$contextMenu), {
        namespace: "frcm",
        toastText: l.file_request_action_copy_link
      });

      this.$removeButton = new mega.fileRequestUI.ButtonComponent($(".dropdown-item.file-request-remove", this.$contextMenu), {
        namespace: "frcm"
      });
    ...
    addEventHandlers() {
      if (d > 1) {
        logger.info("#file-request #context-menu - Add Folder context menu event handlers", this.$createButton.length);
      }
      ...
      this.$copyLinkButton.setOptions({
        callback: () => {
          const selectedNodeHandle = $.selected[0];
          const puPagePublicHandle = mega.fileRequest.storage.getPuHandleByNodeHandle(selectedNodeHandle);
      ...
      const showRemoveDialog = (selectedNodeHandle, title, message, description) => {
        ...
        const _remove = () => {
          ...
          mega.fileRequest
            .remove(selectedNodeHandle, true)
            .catch((ex) => {
              if (ex !== ENOENT) {
                logger.error(ex);
                return;
  ...
  class CreateMobileDialog extends CommonMobileDialog {
    constructor() {
      ...
      this.$closeButton = new mega.fileRequestUI.CloseMobileComponent($(".cancel.text-button, .fm-dialog-close", this.$dialog), {
        $dialog: this.$dialog,
        propagate: false
      });
  ...
  class ManageMobileDialog extends CommonMobileDialog {
    ...
    addEventHandlers() {
      this.$updateButton.eventOnClick(async () => {
        ...
        await mega.fileRequest.update(this.puHandleObject.h, this.$inputTitle.getValue(), this.$inputDescription.getValue());
  ...
  class SuccessMobileDialog extends CommonMobileDialog {
    constructor() {
      ...
      this.$closeButton = new mega.fileRequestUI.CloseMobileComponent($(".close.text-button, .fm-dialog-close", this.$dialog), {
        $dialog: this.$dialog,
        post: () => {
  ...
  return new (class FileRequest {
    ...
    async create(handle, title, description) {
      ...
      if (!puHandleObject) {
        puHandleObject = await this.fileRequestApi.create(handle, title, description).then((res) => {
          const {
            pkt: {
              pup: { p }
            },
    ...
    async update(handle, title, description) {
      ...
      await this.fileRequestApi.update(puHandleObject.p, title, description).catch(dump);
    ...
    async remove(handle, quiet) {
      ...
      return this.fileRequestApi.remove(handle).finally(() => {
        if (!quiet) {
          loadingDialog.hide();
        }
      });
    ...
    async refreshPuPageList() {
      ...
      for (let index = puPageList.length; index--; ) {
        ...
        if (!puPageId) {
          if (d) {
            logger.error("FileRequest.refreshPuPageList - Abnormal state - no puPageId", puPageList[index]);
          }
    ...
    async processUploadedPuHandles(fetchNodesResponse) {
      return this.actionHandler.processUploadedPuHandles(fetchNodesResponse).then(() => this.refreshPuPageList());
    }
    ...
    rebindListManageIcon(options) {
      ...
      $(".grid-scrolling-table .grid-file-request-manage", document).rebind("click.frlm", function (ev) {
    ...
    rebindTopMenuCreateIcon() {
      $(".fm-header-buttons .fm-new-file-request", document).rebind("click.frtmc", openNewDialogHandler);
    }

    rebindPageEmptyCreateButton() {
      $(".fm-empty-file-requests .fm-new-file-request", document).rebind("click.frpec", openNewDialogHandler);
    }

    showRemoveWarning(list) {
      return new Promise((resolve, reject) => {
        const fldName = list.length > 1 ? l[17626] : l[17403].replace("%1", escapeHTML(M.d[list[0]].name));
    ...
    async updatePuHandleAttribute(nodeHandle, type, value) {
      ...
      await this.fileRequestApi.update(puHandleObject.p, msg, description, name).catch((ex) => {
        dump(ex);
