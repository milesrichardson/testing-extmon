(function (scope) {
  ...
  function addUpload(files, emptyFolders) {
    ...
    if (M.InboxID && M.currentrootid && (M.currentrootid === M.InboxID || M.getNodeRoot(M.currentdirid.split("/").pop()) === M.InboxID)) {
  ...
  function pushUpload() {
    if (!--dir_inflight && $.dostart) {
      var emptyFolders = Object.keys(filedrag_paths).filter(function (p) {
    ...
    if (item.isFile) {
      ...
      getFile(item)
        ...
        .catch(function (error) {
          if (d) {
            ...
            console[fn]('Failed to get File from FileEntry for "%s", %s', item.name, Object(error).name, error, item);
          }
      ...
      var dirReaderIterator = function () {
        dirReader.readEntries(
          ...
          function (error) {
            console.warn('Unable to traverse folder "%s", %s', item.name, Object(error).name, error, item);
  ...
  function start_upload() {
    if (u_type && u_attr) {
      // logged in user landing on start-page
      loadSubPage("fm");
      return;
    }
    if (u_wasloggedin()) {
  ...
  function FileSelectHandlerMegaSyncClick(e) {
    ...
    if (window.useMegaSync === 2) {
      ...
      var elem = e.target;
      if (
        elem.hasAttribute("webkitdirectory") ||
        elem.hasAttribute("mozdirectory") ||
        elem.hasAttribute("msdirectory") ||
        elem.hasAttribute("odirectory") ||
  ...
  function FileDragLeave(e) {
    ...
    if (optionReference.touchedElement <= 0 || (optionReference.touchedElement === 1 && ua.details.browser === "Firefox")) {
      removeOverlay();
  ...
  function FileSelectHandler(e) {
    ...
    if (
      (folderlink || (currentDir && currentDir !== "dashboard" && currentDir !== "transfers" && (M.getNodeRights(currentDir) | 0) < 1)) &&
    ...
    if (window.d && (e.ctrlKey || e.metaKey) && MediaInfoLib.isFileSupported(files[0])) {
      ...
      return M.require("videostream")
        .then(() => Streamer(files[0], video))
        .catch(dump);
    }
    ...
    if (e.dataTransfer && e.dataTransfer.items && e.dataTransfer.items.length > 0 && e.dataTransfer.items[0].webkitGetAsEntry) {
      ...
      for (var i = 0, f; (f = files[i]); i++) {
        if (f.webkitRelativePath) {
          f.path = String(f.webkitRelativePath).replace(RegExp("[\\/]" + String(f.name).replace(/([^\w])/g, "\\$1") + "$"), "");
        }
        if (gecko) {
          f.gecko = true;
        }
      ...
      $fileAndFolderUploadWrap.safeAppend(
        '<input type="file" class="hidden" id="fileselect1" title="' +
          l[99] +
          '" multiple="">' + // File input
      ...
      $(".files-menu .fileupload-item").after('<input type="file" id="fileselect3" class="hidden" name="fileselect3" multiple="">');
      $("input#fileselect4").remove();
      $(".files-menu .folderupload-item").after(
  ...
  function onDragStartHandler(e) {
    if ((e.target && e.target.toString && e.target.toString() === "[object Text]") || page.indexOf("/fm/") === -1) {
      e.stopPropagation();
      e.preventDefault();
      return false;
    }
  ...
  function isFileDragAllowed() {
    ...
    return !(
      is_fm() && // if page is fm,
      (window.slideshowid ||
        !$(".feedback-dialog").hasClass("hidden") || // preview and feedback dialog show
  ...
  scope.InitFileDrag = function () {
    ...
    $(".drag-n-drop.overlay").rebind("click.dnd", function dragDropLayoutClickHndler() {
      removeOverlay();
    });
    ...
    if (is_mobile && ((ua.details.engine === "Gecko" && parseInt(ua.details.version) < 83) || (is_ios && is_ios < 13))) {
...
async function ulDummyImages(count, type) {
  ...
  for (let i = count || 210; i--; ) {
    ...
    const buf = await webgl.createImage(i % 2 ? rnd * Date.now() : null, wdh, (wdh / 1.777) | 0, type).catch((ex) => {
