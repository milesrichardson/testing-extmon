...
lazy(mega, "fileRequestCommon", () => {
  ...
  const addFileRequestIcon = (puHandlePublicHandle) => {
    if (fminitialized && puHandlePublicHandle) {
      const puHandleObject = M.d[puHandlePublicHandle] || mega.fileRequestCommon.storage.getPuHandleByPublicHandle(puHandlePublicHandle);
      ...
      if ($nodeId.length === 0 && !String(M.currentdirid).includes("chat") && M.megaRender && M.megaRender.hasDOMNode(nodeId)) {
      ...
      if (d) {
        logger.info("common.addFileRequestIcon - Added node icon", nodeId, puHandleObject, puHandlePublicHandle);
      }
    }
  };
  ...
  const removeFileRequestIcon = (selectedNodeHandle) => {
    if (fminitialized && selectedNodeHandle) {
      ...
      if (node && M.megaRender && M.megaRender.hasDOMNode(nodeId)) {
        ...
        const viewModeClass = M.viewmode ? "span.block-view-file-type" : "span.transfer-filetype-icon";
        $(viewModeClass, node).removeClass(folderClass);
      }

      $(`#treea_${nodeId} span.nw-fm-tree-folder`).removeClass(folderClass);
    }
  };
  ...
  class FileRequestApi {
    create(handle, title, description) {
      ...
      eventlog(99773);
      return api.screq({ n: handle, a: "ul", d: 0, s: 2, data }).finally(() => {
    ...
    remove(handle) {
      ...
      return api.screq({ a: "ul", d: 1, n: handle }).finally(() => {
    ...
    update(puPagePublicHandle, title, description, name, email) {
      ...
      return api.screq({ a: "ps", p: puPagePublicHandle, d }).finally(() => {
  ...
  class FileRequestStorage {
    ...
    async addPuHandle(puHandleNodeHandle, puHandlePublicHandle, data, pagePublicHandle) {
      ...
      if (d) {
        logger.info("Storage.addPuHandle - puf add", puHandlePublicHandle, puHandleNodeHandle);
      }
    ...
    saveOrUpdatePuHandle(options, update) {
      if (!options.publicHandle) {
        logger.info("Storage.saveOrUpdatePuHandle - PUF Save/Update", options.folderName, options.nodeHandle);
      }

      const { nodeHandle, title, description, folderName, state, publicHandle, pagePublicHandle } = this.setPuHandleValues(options, update);
    ...
    setPuHandleValues(options, update) {
      let { nodeHandle, title, description, folderName, state, publicHandle, pagePublicHandle } = options;
    ...
    updatePuHandlePageId(puHandlePublicHandle, puPagePublicHandle, puHandleState) {
      if (d) {
    ...
    addPuPage(puPageObject) {
      ...
      if (puPageObject.d) {
        // We override title and description and use what is stored in the API
    ...
    saveOrUpdatePuPage(options, update) {
      if (!options.pagePublicHandle) {
        logger.info("Storage.saveOrUpdatePuPage - PUF Save/Update", options.folderName, options.nodeHandle);
      }

      const { nodeHandle, title, description, message, folderName, state, publicHandle, pagePublicHandle, name } = this.setPuPageValues(
        options,
        update
      );
    ...
    setPuPageValues(options, update) {
      let { nodeHandle, title, description, message, folderName, state, publicHandle, pagePublicHandle, name } = options;
  ...
  class FileRequestGenerator {
    ...
    windowOpen(url) {
      ...
      window.open(
        url,
        "_blank",
        "noopener,noreferrer," + "width=770, height=770, resizable=no," + "status=no, location=no, titlebar=no, toolbar=no"
      );
    }
  }
