...
if (typeof importScripts !== "undefined") {
  ...
  if (typeof delay === "undefined") {
    ...
    delay = (tag, cb, t) => {
      ...
      let e = q.get(tag);
      if (!e) {
        (e = Object.create(null)).tid = setTimeout(
          (e) => {
            q.delete(e.pun);
      ...
      if (key.length < 44) {
        ...
        if (k.length === 4 || k.length === 8) {
          ...
          if (!self.allowNullKeys) {
            const invalidFileKey = k.length === 8 && k[0] === k[4] && k[1] === k[5] && k[2] === k[6] && k[3] === k[7];
...
function crypto_clearattr(n) {
  ...
  const dattrs = ["ar", "devid", "drvid", "f", "fav", "gps", "hash", "lbl", "mtime", "name", "rr", "s4", "sds"];
  const old = {};
...
(function _console(global) {
  ...
  var funcs =
    "clear,count,debug,dir,dirxml,error,exception,group,groupCollapsed,groupEnd," +
