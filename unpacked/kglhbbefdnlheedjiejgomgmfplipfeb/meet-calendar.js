...
class EventContainer {
  ...
  static getInstance() {
    ...
    if (eventEditPage) return new GEvent(eventEditPage);
    else if (document.querySelector("body").dataset.viewfamily) return new G2Event(document.querySelector("body"));
    else return new MSLiveEvent();
  }
  ...
  updateMeetingId() {
    ...
    if (ix != -1 && (url = inviteText.substring(ix)) && url.length > 0) {
      ...
      } else this.meetingId = generateRoomWithoutSeparator();

      if (NUMBER_RETRIEVE_SCRIPT) {
        // queries a predefined location for settings
        $.getJSON(NUMBER_RETRIEVE_SCRIPT, (jsonobj) => {
          ...
          if (jsonobj.roomNameDictionary) {
            this.meetingId = generateRoomWithoutSeparator(jsonobj.roomNameDictionary);
          }

          if (!jsonobj.numbersEnabled) return;

          this.numbers = jsonobj.numbers;
          this.inviteNumbersTextTemplate = jsonobj.inviteNumbersTextTemplate;
...
class Description {
  ...
  update(location) {
    ...
    if (this.element != undefined) {
      var descriptionContainsURL = this.value && this.value.length >= 1 && this.value.indexOf(BASE_URL) !== -1;
      isDescriptionUpdated =
        descriptionContainsURL ||
        // checks whether there is the generated name in the location
        // input if there is a location
        (location != null && location.text && location.text.indexOf(LOCATION_TEXT) != -1);
    }
  ...
  clickAddMeeting(isDescriptionUpdated, location) {
    if (!isDescriptionUpdated) {
      ...
      if (CONFERENCE_MAPPER_SCRIPT) {
        // queries a predefined location for settings
        $.getJSON(CONFERENCE_MAPPER_SCRIPT + "?conference=" + this.event.meetingId + "@conference." + BASE_DOMAIN, (jsonobj) => {
      ...
      if (location) location.addLocationText(LOCATION_TEXT + " - " + BASE_URL + this.event.meetingId);
    } else {
      this.updateButtonURL();
    }
  }
  ...
  getInviteText(dialInID) {
    ...
    if (this.event.inviteTextTemplate) {
      ...
      inviteText = "Click the following link to join the meeting " + "from your computer: " + BASE_URL + this.event.meetingId;
    }
    ...
    if (this.event.numbers && Object.keys(this.event.numbers).length > 0) {
      if (this.event.inviteNumbersTextTemplate) {
        ...
        inviteText += "\n\nSay your conference name: '" + this.event.meetingId + "' and you will be connected!";
      }
    }
...
class GEvent extends EventContainer {
   ...
   */
  get location() {
    if (!this.locationInstance) this.locationInstance = new GLocation();
  ...
  get buttonContainer() {
    ...
    let newRow = $('<tr id="' + newRowID + '">' + '<th class="ep-dp-dt-th"></th>' + '<td class="ep-dp-dt-td"></td>' + "</tr>");
   ...
   */
  get description() {
    if (!this.descriptionInstance) this.descriptionInstance = new GDescription(this);
...
class G2Event extends EventContainer {
   ...
   */
  get description() {
    if (!this.descriptionInstance) this.descriptionInstance = new G2Description(this);
    return this.descriptionInstance;
  }
}
...
class G2Location extends Location {
  ...
  addLocationText(text) {
    ...
    if (locationNode) {
      ...
      elem.attr("value", locationNode.value === "" ? text : locationNode.value + ", " + text);
      locationNode.dispatchEvent(getKeyboardEvent("input"));
...
class MSLiveEvent extends EventContainer {
   ...
   */
  get buttonContainer() {
    var container = $("span[id='MeetingCompose.LocationInputLabel']").parent().parent();
   ...
   */
  get description() {
    if (!this.descriptionInstance) this.descriptionInstance = new MSLiveDescription(this);
    return this.descriptionInstance;
  }
...
function checkAndUpdateCalendar() {
  ...
  if (c) {
    ...
    if (!c.isButtonPresent()) {
      ...
      // create page
      if (findGetParameter("autoCreateMeeting") && findGetParameter("extid") === chrome.runtime.id) {
    ...
    new MutationObserver(function () {
      ...
      if (quickAddDialog.length >= 1) {
        ...
        setTimeout(function () {
          var quickAddDialogContainer = $(".bubblecontent .event-create-container");
          // skip if our button is already added
          if (quickAddDialogContainer.length < 1 || $("#jitsi-meet_button_quick_add").length != 0) {
            return;
          }

          var buttonsRow = $(".bubblecontent .event-create-container > .action-tile");
          if (buttonsRow.length < 1) {
            return;
          }

          var numberOfButtons = buttonsRow.find(".split-tile-right").length;
          var lastButtonGroup = buttonsRow.find(".split-tile-right:last");
...
function checkAndUpdateCalendarG2() {
  ...
  if (c) {
    // anyway try to add the button, this is the case when directly going
    // to create event page
    if (document.querySelector("body").dataset.viewfamily === "EVENT_EDIT" && !c.isButtonPresent()) {
      // popup adds autoCreateMeeting param when open directly event
      // create page
      if (findGetParameter("autoCreateMeeting") && findGetParameter("extid") === chrome.runtime.id) {
        c.scheduleAutoCreateMeeting = true;
    ...
    new MutationObserver(function (mutations) {
        ...
        mutations.forEach(function (mutation) {
          ...
          if (newElement && newElement.search('role="dialog"') !== -1) {
            ...
            if (tabEvent.length > 0) {
              ...
              var clickHandler = jitsiQuickAddButton.find("#jitsi-meet_button_quick_add");
      ...
      } else if (document.querySelector("body").dataset.viewfamily === "EVENT_EDIT") {
        c.update();
      }
    }).observe(body, {
      attributes: false,
