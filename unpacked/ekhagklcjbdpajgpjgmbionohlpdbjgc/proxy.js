...
(function () {
  ...
  Zotero.Proxies = new (function () {
    ...
    this.loadPrefs = function () {
      ...
      Zotero.Proxies.autoRecognize = Zotero.isBrowserExt && Zotero.Proxies.transparent && Zotero.Prefs.get("proxies.autoRecognize");

      var disableByDomainPref = Zotero.Prefs.get("proxies.disableByDomain");
      Zotero.Proxies.disableByDomain =
        Zotero.Proxies.transparent && disableByDomainPref ? Zotero.Prefs.get("proxies.disableByDomainString") : null;
    ...
    this.updateDisabledByDomain = function () {
      ...
      if (now - this.lastIPCheck > 15 * 60 * 1000) {
        ...
        Zotero.Proxies.DNS.getHostnames().then(
          function (hosts) {
    ...
    this.loadFromClient = function () {
      ...
      return Zotero.Connector.callMethod("proxies", null).then(
        function (result) {
    ...
    this.onBeforeNavigateSafari = function (e) {
      ...
      let details = { url: e.url || "", originUrl: e.target.url, type: "main_frame", requestHeadersObject: {}, tabId: e.target };
    ...
    this._maybeAddHost = function (details) {
      ...
      if (
        proxy.autoAssociate && // if autoAssociate is on
    ...
    this.notifyNewProxy = async function (proxy, tabId) {
      let response = await _showNotification(
        "New Zotero Proxy",
        `Zotero detected that you are accessing ${
          proxy.hosts[proxy.hosts.length - 1]
        } through a proxy. Would you like to automatically redirect future requests to ${
          proxy.hosts[proxy.hosts.length - 1]
        } through ${proxy.toDisplayName()}?`,
      ...
      if (response == 2) {
        let result = await Zotero.Messaging.sendMessage("confirm", {
          ...
          message:
            "Adding other proxies allows malicious sites to masquerade as sites you trust.<br/></br>" +
    ...
    this._maybeRedirect = function (details) {
      ...
      if (
        ...
        (details.url.substr(0, 5) == "https" && proxied.substr(0, 5) != "https")
      )
        return;
  ...
  Zotero.Proxy.prototype.toProper = function (m) {
    ...
    if (protocol == "https://" || !properURL.includes(".")) {
