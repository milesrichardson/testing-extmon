  ...
  (this.callMethod = async function (options, data, tab) {
        ...
        if (req.status == 0 || req.status >= 400) {
          ...
          if (req.status === 412) {
            if (Zotero.Connector_Browser && Zotero.Connector_Browser.onIncompatibleStandaloneVersion) {
              ...
              deferred.reject(
                "Connector: Version mismatch: Connector version " +
                  Zotero.version +
                  ", Standalone version " +
                  (standaloneVersion ? standaloneVersion : "<unknown>", val)
              );
            }
          }
    ...
    (this.callMethodWithCookies = function (options, data, tab) {
      if (Zotero.isBrowserExt) {
        ...
        return browser.cookies.getAll(cookieParams).then(
          function (cookies) {
            var cookieHeader = "";
      ...
      if (method.startsWith("document/")) {
        try {
          ...
          let response = await Zotero.Connector.callMethod(
            { method: "document/respond", timeout: false },
            JSON.stringify({
          ...
          while (method != "complete") {
            ...
            if (method == "displayAlert") {
              ...
              response = await Zotero.Connector.callMethod(
                { method: "document/respond", timeout: false },
