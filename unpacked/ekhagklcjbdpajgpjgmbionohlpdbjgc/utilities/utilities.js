...
(function () {
  ...
  var Utilities = {
      ...
      return string
        ...
        .map((part) => {
          if (part.toUpperCase() === part || part.toLowerCase() === part) {
            return Utilities.XRegExp.replace(part.toLowerCase(), Utilities.XRegExp("(^|[^\\pL])\\pL", "g"), (m) => m.toUpperCase());
          } else {
            return part;
          }
        })
    ...
    cleanAuthor: function (author, type, useComma) {
      ...
      if (firstName && allCapsRe.test(firstName) && firstName.length < 4 && (firstName.length == 1 || lastName.toUpperCase() != lastName)) {
    ...
    isHTTPURL: function (url, allowNoScheme = false) {
      ...
      return /^https?:\/\//.test(url) || (allowNoScheme && !url.startsWith("mailto:") && noSchemeRE.test(url));
    },
    ...
    cleanISSN: function (/**String*/ issnStr) {
      issnStr = issnStr.toUpperCase().replace(/[\x2D\xAD\u2010-\u2015\u2043\u2212]+/g, ""); // Ignore dashes
    ...
    htmlSpecialChars: function (str) {
      if (str && typeof str != "string") {
        Zotero.debug("#htmlSpecialChars: non-string arguments are deprecated. Update your code", 1, undefined, true);
        str = str.toString();
      }
        ...
        if (words[i].length != 0 && (words[i].length != 1 || !delimiterRegexp.test(words[i]))) {
          ...
          if (isUpperCase || words[i] == lowerCaseVariant) {
            if (
              ...
              words[i] = words[i].length ? words[i].substr(0, punct).toUpperCase() + words[i].substr(punct).toLowerCase() : words[i];
            }
          }
    ...
    xpathText: function (node, xpath, namespaces, delimiter) {
      ...
      for (var i = 0, n = elements.length; i < n; i++) {
        ...
        strings[i] =
          el.nodeType === 2 /*ATTRIBUTE_NODE*/ && "value" in el
            ? el.value
            : "textContent" in el
            ? el.textContent
            : "innerText" in el
    ...
    varDump: function (obj, level, maxLevel, parentObjects, path) {
      ...
      if (Zotero.isFx && !Zotero.isBookmarklet) {
        ...
        } else if (obj instanceof Components.interfaces.nsIDOMNode || obj instanceof Components.interfaces.nsIDOMWindow) {
          level = maxLevel;
        }
      }
      ...
      var dumpKey = (key) => (typeof key == "symbol" ? key.toString() : JSON.stringify(key));
      if (isArray) {
        dumpedText = "[";
      ...
      for (var prop in obj) {
        dumpedText += "\n" + level_padding + (rawProps ? prop : dumpKey(prop)) + ": ";
