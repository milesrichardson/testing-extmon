    ...
    function createContextObject(item, version, asObj) {
      ...
      if (item.creators && item.creators.length) {
        ...
        for (var i = 0; i < item.creators.length; i++) {
          _mapTag(
            (item.creators[i].firstName ? item.creators[i].firstName + " " : "") + item.creators[i].lastName,
            item.itemType == "patent" ? "inventor" : "au"
          );
        }
      }
    ...
    function parseContextObject(co, item) {
      ...
      for (var i = 0; i < coParts.length; i++) {
        ...
        if (key == "rft_id") {
          ...
          if (complexAu.length && !lastCreator.lastName && !lastCreator.institutional) {
            ...
            complexAu.push(
              _cloneIfNecessary(
                { lastName: value, creatorType: key == "rft.aulast" ? "author" : "inventor", offset: item.creators.length },
                item
              )
            );
          }
          ...
          if (complexAu.length && !lastCreator.firstName && !lastCreator.institutional) {
            ...
            complexAu.push(
              _cloneIfNecessary(
                { firstName: value, creatorType: key == "rft.aufirst" ? "author" : "inventor", offset: item.creators.length },
                item
              )
            );
          }
          ...
          if (key == "rft.identifier") {
            if (value.length > 8) {
              // we could check length separately for
      ...
      for (var i = 0; i < complexAu.length; i++) {
        ...
        for (var j = 0; j < item.creators.length; j++) {
          ...
          if (
            item.creators[j].lastName == complexAu[i].lastName &&
