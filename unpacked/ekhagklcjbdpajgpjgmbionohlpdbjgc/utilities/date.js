...
(function () {
  var Utilities_Date = new (function () {
    ...
    this.init = function (dateFormatsJSON) {
      ...
      let months = _monthsWithEnglish.short.map((m) => m.toLowerCase()).concat(_monthsWithEnglish.long.map((m) => m.toLowerCase()));
    ...
    this.sqlToDate = function (sqldate, isUTC) {
      try {
        if (!this.isSQLDate(sqldate) && !this.isSQLDateTime(sqldate) && !this.isSQLDateTimeWithoutSeconds(sqldate)) {
    ...
    this.strToDate = function (string) {
      ...
      if (date.month === undefined) {
        ...
        months = months.short.map((m) => m.toLowerCase()).concat(months.long.map((m) => m.toLowerCase()));
        for (var i in parts) {
    ...
    this.formatDate = function (date, shortFormat) {
      if (shortFormat) {
        ...
        return string
          .replace("y", date.year !== undefined ? date.year : "00")
    ...
    this.strToMultipart = function (str) {
      ...
      var multi =
        (parts.year ? Zotero.Utilities.lpad(parts.year, "0", 4) : "0000") +
        "-" +
    ...
    function toUnixTimestamp(date) {
      if (date === null || typeof date != "object" || date.constructor.name != "Date") {
    ...
    function isToday(date) {
      var d = new Date();
      return d.getDate() == date.getDate() && d.getMonth() == d.getMonth() && d.getFullYear() == d.getFullYear();
    }
