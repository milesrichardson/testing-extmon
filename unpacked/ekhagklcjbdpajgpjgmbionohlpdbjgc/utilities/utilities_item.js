  ...
  var Utilities_Item = {
    ...
    itemToCSLJSON: function (zoteroItem) {
      ...
      if (typeof Zotero.Item !== "undefined" && zoteroItem instanceof Zotero.Item) {
        return Utilities_Item.itemToCSLJSON(Zotero.Utilities.Internal.itemToExportFormat(zoteroItem, false, true));
      }
        ...
        if (variable in cslItem) {
          ...
          if (Zotero.ItemFields.isValidForType(fieldID, itemTypeID)) {
            ...
            if (cslDate.literal || cslDate.raw) {
              ...
              if (
                cslDate["date-parts"] &&
                typeof cslDate["date-parts"] === "object" &&
    ...
    getFirstCreatorFromItemJSON: function (json) {
      var primaryCreatorType = Zotero.CreatorTypes.getName(Zotero.CreatorTypes.getPrimaryIDForType(Zotero.ItemTypes.getID(json.itemType)));
    ...
    itemToAPIJSON: function (item) {
      ...
      for (var field in item) {
        if (field === "complete" || field === "itemID" || field === "attachments" || field === "seeAlso") continue;
        ...
        if (field === "itemType") {
          ...
          for (var j = 0; j < n; j++) {
            ...
            if (creator.creatorType) {
              if (Zotero.CreatorTypes.getID(creator.creatorType)) {
                ...
                Zotero.debug(`itemToAPIJSON: Invalid creator type ${creator.creatorType}; ` + "falling back to author");
              }
            }
          ...
          if (Zotero.ItemFields.isValidForType(fieldID, typeID)) {
            ...
            Zotero.debug(`itemToAPIJSON: Discarded field ${field}: ` + `field not valid for type ${item.itemType}`, 3);
          }
        } else {
    ...
    itemToLegacyExportFormat: function (item) {
      ...
      for (let field in item) {
        try {
          var baseField = Zotero.ItemFields.getName(Zotero.ItemFields.getBaseIDFromTypeAndField(item.itemType, field));
        } catch (e) {
          continue;
        }
    ...
    compareCallNumbers: function (fieldA, fieldB) {
      ...
      if (classificationA && classificationB) {
        ...
        let classificationComp = Zotero.localeCompare(classificationA[0].replace(/[\s.]/g, ""), classificationB[0].replace(/[\s.]/g, ""));
