...
(function () {
  ...
  Zotero.GoogleDocs.ClientAppsScript.prototype = {
    ...
    call: async function (request) {
      ...
      try {
        ...
        if (e.message == "Handled Error") {
          ...
          this.displayAlert = async function () {
            return 0;
          };
        } else {
      ...
      while (count < keys.length || this.queued.documentData) {
        ...
        try {
          ...
          if (e.status == 429 || e.message.startsWith("Too many changes applied before saving document.")) {
            ...
            if (batchSize == 1) {
              throw new Error(`Document update for batch size 1 failed with error ${e.message}. Not going to retry`);
            }
    ...
    getFields: async function () {
      if (this.fields) {
        ...
        if (typeof this.insertIdx == "number" && this.queued.insert.length) {
          ...
          fields = fields.slice(0, this.insertIdx).concat(insert, fields.slice(this.insertIdx));
        }
        return fields;
      }
    ...
    setBibliographyStyle: async function (firstLineIndent, bodyIndent, lineSpacing, entrySpacing, tabStops, tabStopsCount) {
      this.queued.bibliographyStyle = { firstLineIndent, bodyIndent, lineSpacing, entrySpacing, tabStops };
    },
    ...
    convert: async function (fieldIDs, fieldType, fieldNoteTypes) {
      ...
      if (fieldMap[fieldIDs[0]].noteIndex != fieldNoteTypes[0]) {
        ...
        if (fieldNoteTypes[0] > 0) {
          ...
          return Zotero.GoogleDocs_API.run(this.documentID, "footnotesToInline", [fieldIDs]);
        }
      }
    },
