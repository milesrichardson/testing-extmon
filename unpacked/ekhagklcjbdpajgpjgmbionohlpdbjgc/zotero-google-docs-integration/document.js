  ...
  Zotero.GoogleDocs.Document = class Document {
    ...
    async exportDocument(_, importInstructions) {
      ...
      if (docData) {
        ...
        this.addBatchedUpdate("insertText", { text: docData, location: { index: this.bodyRange.endIndex - 1 } });
    ...
    getBibliographyStyle() {
      ...
      paragraphStyle.lineSpacing = bibStyle.lineSpacing / 2.4;
    ...
    getLinks() {
      ...
      var reducer = (elem, paragraphStyle, noteIndex = 0, footnoteId = null) => {
        if (elem.textRun) {
          ...
          this._reduceStructuralElements(this.footnotes[footnoteReference.footnoteId].content, (elem, paragraphStyle) =>
            reducer(elem, paragraphStyle, parseInt(footnoteReference.footnoteNumber, 10), footnoteReference.footnoteId)
          );
        }
      };
    ...
    async placeholdersToFields(placeholderIDs, noteType) {
      ...
      for (let link of links) {
        if (
          link.url.startsWith(Zotero.GoogleDocs.config.fieldURL) ||
          !link.url.startsWith(Zotero.GoogleDocs.config.noteInsertionPlaceholderURL)
        )
          continue;
      ...
      if (placeholders.length != placeholderIDs.length) {
        throw new Error(
          `convertPlaceholdersToFields: number of placeholders (${placeholders.length}) do not match the number of provided placeholder IDs (${placeholderIDs.length})`
        );
      }
      // Sort for update by reverse order of appearance to correctly update the doc
  ...
  let Field = (Zotero.GoogleDocs.Field = class {
    ...
    write(onlyText = false, ignoreBibliographyStyle = false) {
      ...
      if (field.text) {
        ...
        let paragraphStyle = {};
        var isBibl = (field.code && field.code.substr(0, 4) == "BIBL") || this.code.substr(0, 4) == "BIBL";
        if (isBibl && !ignoreBibliographyStyle) {
          paragraphStyle = this._doc.getBibliographyStyle();
        }
        newTextRange = new HTMLInserter().insert(
          this._doc,
          field.text,
          range.startIndex,
          range.segmentId,
      ...
      if (!onlyText && newCode) {
        ...
        if (this.namedRanges[0] && !this.namedRanges[0].namedRangeId) {
          this._doc._batchedUpdates = this._doc._batchedUpdates.filter((update) => {
            return !update.createNamedRange || !update.createNamedRange.name.startsWith(prefix);
          });
        } else {
          this.namedRanges.forEach((namedRange) => {
            this._doc.addBatchedUpdate("deleteNamedRange", { namedRangeId: namedRange.namedRangeId });
        ...
        this.namedRanges = this._doc.encodeRange(newTextRange || this.getRange(), newCode, prefix);
      }
    }
