...
Zotero.GoogleDocs.API = {
  ...
  onAuthComplete: async function (url, tab) {
    ...
    try {
      ...
      url = ZOTERO_CONFIG.OAUTH.GOOGLE_DOCS.ACCESS_URL + `?access_token=${params.access_token}`;
  ...
  onAuthCancel: function () {
    ...
    error.type = "Alert";
    Zotero.GoogleDocs.API.authDeferred && Zotero.GoogleDocs.API.authDeferred.reject(error);
  },
  ...
  run: async function (docID, method, args, tab) {
    ...
    try {
      var xhr = await Zotero.HTTP.request("POST", ZOTERO_CONFIG.GOOGLE_DOCS_API_URL, { headers, body, timeout: null });
    } catch (e) {
  ...
  displayLockErrorPrompt: async function (error, tab) {
    ...
    var result = await Zotero.Messaging.sendMessage(
      "confirm",
      {
  ...
  getDocument: async function (docId) {
    ...
    headers["Content-Type"] = "application/json";
    try {
      var xhr = await Zotero.HTTP.request("GET", `https://docs.googleapis.com/v1/documents/${docId}`, { headers, timeout: 60000 });
    } catch (e) {
      if (e.status == 403) {
        this.resetAuth();
