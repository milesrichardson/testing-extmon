...
Zotero.HTTP = new (function () {
  ...
  this._requestXHR = async function (method, url, options) {
    ...
    if (["GET", "HEAD"].includes(method)) {
      ...
      if (options.headers["Content-Type"] !== "multipart/form-data") {
        ...
        logBody = `: ${options.body.substr(0, options.logBodyLength)}` + options.body.length > options.logBodyLength ? "..." : "";
    ...
    return promise.then(function (xmlhttp) {
      ...
      let invalidDefaultStatus = options.successCodes === null && (xmlhttp.status < 200 || xmlhttp.status >= 300);
  ...
  this._requestFetch = async function (method, url, options) {
    ...
    let invalidDefaultStatus = options.successCodes === null && (response.status < 200 || response.status >= 300);
