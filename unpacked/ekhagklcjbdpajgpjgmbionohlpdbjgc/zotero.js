...
var Zotero = (global.Zotero = new (function () {
  ...
  this.allowRepoTranslatorTester = this.isManifestV3 = this.isFirefox = this.isSafari = this.isBrowserExt = null;
  this.allowRepoTranslatorTester = false;
  this.isManifestV3 = false;

  this.initialized = false;
  this.initDeferred = {};
  this.initDeferred.promise = new Promise(
    ...
    return this.getAll().then(
      function (prefs) {
        ...
        for (let namespace of namespaces) {
          keys.filter((key) => key.indexOf(namespace) === 0).forEach((key) => (this.syncStorage[key] = prefs[key]));
        }
      }.bind(this)
    );
  };
