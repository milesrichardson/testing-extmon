    ...
    handleImportContent: function (details) {
      (async () => {
        ...
        if (isEnabledHost) {
          ...
          Zotero.ContentTypeHandler.confirm(
            details,
            `Import items from ${URI.host} into Zotero?<br/><br/>` +
    ...
    importFile: async function (details, type) {
      ...
      if (type == "csl") {
        ...
        try {
          ...
          if (e.status == 404) {
            return Zotero.Messaging.sendMessage("progressWindow.done", [false, "upgradeClient"], tab);
          } else {
            return Zotero.Messaging.sendMessage("progressWindow.done", [false, "clientRequired"], tab);
          }
        }
      }
      // RIS/BibTeX import
