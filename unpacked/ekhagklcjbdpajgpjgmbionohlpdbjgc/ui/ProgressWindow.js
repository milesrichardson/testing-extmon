...
Zotero.UI.ProgressWindow = class ProgressWindow extends React.PureComponent {
  ...
  componentDidUpdate() {
    ...
    window.requestAnimationFrame(() => {
      if (
        this.lastHeight != this.rootNode.scrollHeight && // In Firefox this ends up being 0 when the pane closes
  ...
  renderHeadlineSelect() {
    var rowTargets = [
      this.state.target, // Show recent targets
  ...
  renderTargetSelector() {
    return this.state.targetSelectorShown
      ? React.createElement(
          "div",
          null,
          React.createElement(
  ...
  renderItem(item) {
    var itemStyle = Object.assign(
      {}, // Start at 50% opacity
      {
        opacity: (item.percentage || 0) / 200 + 0.5
      },
      item.parentItem && {
        marginTop: "4px",
        marginLeft: "12px"
  ...
  renderError(row, index) {
    ...
    if (err === "translationError") {
      ...
      contents =
        "No items could be saved because this website " +
