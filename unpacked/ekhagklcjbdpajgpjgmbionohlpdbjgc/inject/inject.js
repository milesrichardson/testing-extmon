...
Zotero.Inject = new (function () {
  ...
  this.init = async function (force) {
    ...
    if (
      document.location.href.substr(0, ZOTERO_CONFIG.OAUTH.ZOTERO.CALLBACK_URL.length + 1) ===
      ZOTERO_CONFIG.OAUTH.ZOTERO.CALLBACK_URL + "?"
    ) {
      Zotero.API.onAuthorizationComplete(document.location.href.substr(ZOTERO_CONFIG.OAUTH.ZOTERO.CALLBACK_URL.length + 1));
    } else if (
      document.location.href.substr(0, ZOTERO_CONFIG.OAUTH.ZOTERO.CALLBACK_URL.length + 1) ===
      ZOTERO_CONFIG.OAUTH.GOOGLE_DOCS.CALLBACK_URL + "#"
    ) {
    ...
    try {
      ...
      return _translate.getTranslators(true).then(
  ...
  this.firstUsePrompt = function () {
    ...
    return this.confirm({
      ...
      button2Text: "",
      message:
        Zotero.getString("firstRun_text1", [clientName, "https://www.zotero.org/support/adding_items_to_zotero"]) +
        "<br><br>" +
        Zotero.getString("firstRun_text2", [
          clientName, // TODO: Make download URL configurable (instead of just base URL + "download")
  ...
  this.firstSaveToServerPrompt = async function () {
    ...
    var result = await this.confirm({
      ...
      message:
        Zotero.getString("error_connection_save", [Zotero.getString("appConnector", clientName), clientName, ZOTERO_CONFIG.DOMAIN_NAME]) +
        "<br /><br />" +
  ...
  this.translate = async function (translatorID, options = {}) {
    ...
    if (
      this.sessionDetails.id && // Same page (no history push)
    ...
    Zotero.Messaging.sendMessage("progressWindow.show", [
      sessionID,
      null,
      false, // If we're likely to show the Select Items window, delay the opening of the
...
if (!isHiddenIFrame) {
  var doInject = async function () {
    ...
    Zotero.Messaging.addMessageListener(
      "pageModified",
      Zotero.Utilities.debounce(function () {
