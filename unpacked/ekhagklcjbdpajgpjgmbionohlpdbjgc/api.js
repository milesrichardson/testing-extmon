...
Zotero.API = new (function () {
  ...
  function _decodeFormData(postData) {
    ...
    for (var i in splitData) {
      ...
      decodedData[decodeURIComponent(variable.substr(0, splitIndex))] = decodeURIComponent(
        variable.substr(splitIndex + 1).replace(/\+/g, "%20")
      );
    }
    return decodedData;
  }
  ...
  this.authorize = async function () {
    ...
    var oauthSimple = new OAuthSimple(config.OAUTH.ZOTERO.CLIENT_KEY, config.OAUTH.ZOTERO.CLIENT_SECRET);
    ...
    try {
      ...
      this._authWindow = await Zotero.Connector_Browser.openWindow(url, {
        width: 900,
        height: 600,
        type: "normal",
  ...
  this.onAuthorizationComplete = async function (data, tab) {
    ...
    if (!access.user.library || !access.user.write) {
      ...
      return deferred.reject(
        new Error(
          "The key you have generated does not have adequate " +
  ...
  this.createItem = async function (payload, askForAuth) {
    ...
    if (!userInfo) {
      ...
      return Zotero.API.authorize().then(
        function () {
  ...
  this.uploadAttachment = function (attachment, callbackOrTab) {
    var _dispatchAttachmentCallback = function (id, status, error) {
      Zotero.Messaging.sendMessage("attachmentCallback", error ? [id, status, error.toString()] : [id, status], callbackOrTab);
    };
    ...
    for (var i = 0; i < REQUIRED_PROPERTIES.length; i++) {
      if (!attachment[REQUIRED_PROPERTIES[i]]) {
        _dispatchAttachmentCallback(attachment.id, false, 'Required property "' + REQUIRED_PROPERTIES[i] + '" not defined');
        return;
      }
    }
    ...
    Zotero.API.getUserInfo().then(function (userInfo) {
      ...
      return Zotero.HTTP.request("POST", url, options).then(
        function (xmlhttp) {
          try {
          ...
          var prefixLength = Zotero.Utilities.getStringByteLength(response.prefix),
            ...
            uploadData = new Uint8Array(attachment.data.byteLength + prefixLength + suffixLength);
          ...
          Zotero.Utilities.stringToUTF8Array(response.suffix, uploadData, attachment.data.byteLength + prefixLength);
