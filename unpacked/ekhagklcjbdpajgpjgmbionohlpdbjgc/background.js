...
Zotero.Connector_Browser = new (function () {
  ...
  this.onIncompatibleStandaloneVersion = function (zoteroVersion, standaloneVersion) {
    ...
    alert(
      "Zotero Connector for Chrome " +
        zoteroVersion +
        " is incompatible with the running " +
        "version of Zotero Standalone" +
  ...
  this.injectScripts = async function (scripts, tab, frameId = 0) {
    function* injectScripts() {
      ...
      for (let script of scripts) {
        ...
        yield Zotero.Connector_Browser.executeScript(tab.id, { file: script, frameId, runAt: "document_end" }).catch(() => undefined);
      }
  ...
  this.notify = async function (text, buttons, seenTimeout = 5000, tab = null) {
    ...
    if (!tab) {
      return browser.tabs.query({ active: true, lastFocusedWindow: true }).then((tabs) => this.notify(text, buttons, seenTimeout, tabs[0]));
  ...
  this._updateExtensionUI = function (tab) {
    if (!tab) {
      return chrome.tabs.query({ lastFocusedWindow: true, active: true }, (tabs) => tabs.length && this._updateExtensionUI(tabs[0]));
    }
  ...
  function _showWebpageIcon(tab) {
    ...
    let withSnapshot = Zotero.Connector.isOnline ? Zotero.Connector.automaticSnapshots : Zotero.Prefs.get("automaticSnapshots");
  ...
  function _browserAction(tab) {
    if (_isBetaBuildBeyondExpiration) {
      ...
      Zotero.Messaging.sendMessage("firstUse", null, tab).then(function () {
      ...
      if (_tabInfo[tab.id] && _tabInfo[tab.id].isPDF) {
        Zotero.Connector_Browser.saveAsWebpage(tab, _tabInfo[tab.id].frameId, {
          snapshot: true
        });
      } else {
        let withSnapshot = Zotero.Connector.isOnline ? Zotero.Connector.automaticSnapshots : Zotero.Prefs.get("automaticSnapshots");
  ...
  this.saveWithTranslator = function (tab, i, options = {}) {
    ...
    return Zotero.Messaging.sendMessage("translate", [_tabInfo[tab.id].instanceID, translator.translatorID, options], tab, null);
  };
  ...
  async function onNavigation(details, historyChange = false) {
    ...
    if (
      details.tabId <= 0 ||
      _isDisabledForURL(details.url, true) ||
      details.url.indexOf(browser.runtime.getURL("itemSelector/itemSelector.html")) === 0
    )
      return;
  ...
  browser.tabs.onActivated.addListener(
    waitForInit(
      logListenerErrors(async function (details) {
        var tab = await browser.tabs.get(details.tabId);
        if (!tab) return;
