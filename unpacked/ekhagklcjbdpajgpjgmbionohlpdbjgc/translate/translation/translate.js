...
Zotero.Translate.Sandbox = {
    ...
    _itemDone: function (translate, item) {
      ...
      var run = async function (async) {
        ...
        // For async import, save items immediately
          ...
          return translate._saveItems([item]).then(() => translate.decrementAsyncProcesses("Zotero.Translate#_saveItems()"));
        }
      };
    ...
    loadTranslator: function (translate, type) {
      ...
      safeTranslator.setDocument = function (arg) {
        if (Zotero.isFx && !Zotero.isBookmarklet) {
          return translation.setDocument(Zotero.Translate.DOMWrapper.wrap(arg, arg.SpecialPowers_wrapperOverrides));
        } else {
      ...
      safeTranslator.setHandler = function (arg1, arg2) {
        ...
        translation.setHandler(arg1, function (obj, item) {
          try {
      ...
      safeTranslator.getTranslators = async function () {
        if (!translation._handlers["translators"] || !translation._handlers["translators"].length) {
          throw new Error(
            'Translator must register a "translators" handler to ' + "call getTranslators() in this translation environment."
          );
        }
      ...
      safeTranslator.getTranslatorObject = function (callback) {
        ...
        translator = typeof translator === "object" ? translator : translation._translatorProvider.get(translator);
        ...
        var promise = Zotero.Promise.resolve(translator)
          ...
          .then(function () {
            ...
            if (!Zotero.Utilities.isEmpty(sandbox.exports)) {
              ...
              translate._debug(
                "COMPAT WARNING: " +
                  translation.translator[0].label +
                  " does " +
                  "not export any properties. Only detect" +
                  translation._entryFunctionSuffix +
                  " and do" +
                  translation._entryFunctionSuffix +
  ...
  Web: {
      ...
      function helper(translate, items, callback) {
        ...
        if (translate._selectedItems) {
          ...
          if (returnValue !== undefined) {
            ...
            Zotero.debug(
              "WARNING: Returning items from a select handler is deprecated. " +
    ...
    _itemDone: function (translate, item) {
      ...
      if (!translate._parentTranslator) {
        ...
        if (item.ISBN) {
          ...
          var isbnRe =
              /\b(?:97[89][\s\x2D\xAD\u2010-\u2015\u2043\u2212]*)?(?:\d[\s\x2D\xAD\u2010-\u2015\u2043\u2212]*){9}[\dx](?![\x2D\xAD\u2010-\u2015\u2043\u2212])\b/gi,
            validISBNs = [],
            isbn;
        ...
        if (item.tags) {
          for (var i = 0; i < item.tags.length; i++) {
            var tag = item.tags[i],
              tagString = typeof tag === "string" ? tag : typeof tag === "object" ? tag.tag || tag.name : null;
        ...
        for (var i = 0; i < item.attachments.length; i++) {
          ...
          if (attachment.url) {
            ...
            attachment.url = translate.resolveURL(attachment.url, attachment.proxy === false || attachment.snapshot === false);
          }
        }
      }
    ...
    monitorDOMChanges: function (translate, target, config) {
      ...
      if (!translate._registeredDOMObservers[translator.translatorID]) translate._registeredDOMObservers[translator.translatorID] = [];
...
Zotero.Translate.Base.prototype = {
  ...
  setTranslator: function (translator) {
    ...
    if (typeof translator == "object") {
      // passed an object and not an ID
    ...
    if (checkSetTranslator) {
      ...
      if (!this.translator || !this.translator[0]) {
        return Zotero.Promise.reject(
          new Error(
            "getTranslators: translator must be set via setTranslator before calling" + " getTranslators with the checkSetTranslator flag"
          )
        );
      }
      ...
      .then(
        function (result) {
          ...
          if (this._waitingForRPC) {
            ...
            Zotero.Connector.callMethod("detect", {
   ...
   */
  translate: Zotero.Promise.method(function (options = {}, ...args) {
    ...
    if (this.noWait) {
      ...
      Zotero.Promise.resolve(this.translator[0]).then(
        function (translator) {
  ...
  _translateTranslatorLoaded: Zotero.Promise.method(function () {
    ...
    function rest() {
      ...
      try {
        ...
        let maybePromise = Function.prototype.apply.call(fn, null, this._getParameters());

        if (maybePromise) {
          maybePromise.then(() => this.decrementAsyncProcesses("Zotero.Translate#translate()")).catch((e) => this.complete(false, e));
          return;
        }
      } catch (e) {
        this.complete(false, e);
  ...
  complete: function (returnValue, error) {
    ...
    if (this._currentState === null) {
      if (!returnValue) {
        ...
        var e = new Error();
        Zotero.debug(
          "Translate: WARNING: Zotero.done() called after translation completion. This should never happen. Please examine the stack below."
        );
  ...
  _saveItems: Zotero.Promise.method(function (items) {
    ...
    return this._itemSaver
      .saveItems(
        items.slice(),
        attachmentCallback.bind(this),
        function (newItems) {
      ...
      .then(
        function (newItems) {
          ...
          for (var i = 0; i < deferredProgress.length; i++) {
            this._runHandler("attachmentProgress", deferredProgress[i][0], deferredProgress[i][1], deferredProgress[i][2]);
          }
  ...
  _checkIfDone: function () {
    if (!this._savingItems && !this._savingAttachments.length && (!this._currentState || this._waitingForSave)) {
      if (this.newCollections && this._libraryID !== false && this._itemSaver.saveCollections) {
        var me = this;
        this._itemSaver
          .saveCollections(this.newCollections)
  ...
  _detectTranslatorLoaded: async function () {
    ...
    var maybePromise = Function.prototype.apply.call(fn, null, this._getParameters());
  ...
  _loadTranslator: Zotero.Promise.method(function (translator) {
    ...
    var parse = function (code) {
      Zotero.debug(
        "Translate: Parsing code for " + translator.label + " " + "(" + translator.translatorID + ", " + translator.lastUpdated + ")",
        4
      );
      try {
        this._sandboxManager.eval(
          "var exports = {}, ZOTERO_TRANSLATOR_INFO = " + code,
          ["detect" + this._entryFunctionSuffix, "do" + this._entryFunctionSuffix, "exports", "ZOTERO_TRANSLATOR_INFO"],
  ...
  _generateSandbox: function () {
    Zotero.debug(
      "Translate: Binding sandbox to " +
        (typeof this._sandboxLocation == "object" ? this._sandboxLocation.document.location : this._sandboxLocation),
      4
    );
    ...
    this._sandboxZotero.parentTranslator =
      this._parentTranslator && this._parentTranslator._currentTranslator ? this._parentTranslator._currentTranslator.translatorID : null;
...
Zotero.Translate.Web.prototype._getSandboxLocation = function () {
  if (this._parentTranslator) {
    ...
    (this.document.defaultView.toString().indexOf("Window") !== -1 || this.document.defaultView.toString().indexOf("XrayWrapper") !== -1)
  ) {
...
Zotero.Translate.Web.prototype._translateTranslatorLoaded = async function () {
  ...
  if (runMode === Zotero.Translator.RUN_MODE_IN_BROWSER || this._parentTranslator) {
    ...
    runMode === Zotero.Translator.RUN_MODE_ZOTERO_STANDALONE ||
    ...
    Zotero.Connector.callMethod(
      { method: "savePage", timeout: 60 * 1000 },
      {
        sessionID: this._sessionID,
        uri: this.location.toString(),
    ...
    var me = this;
    Zotero.API.createItem({ url: this.document.location.href.toString() }).then(
      function (response) {
...
Zotero.Translate.Web.prototype._translateRPCComplete = async function (obj, failureCode) {
  ...
  if (obj.selectItems) {
    ...
    var me = this;
    this._runHandler("select", obj.selectItems, function (selectedItems) {
      Zotero.Connector.callMethod("selectItems", { instanceID: obj.instanceID, selectedItems: selectedItems }).then((obj) =>
        me._translateRPCComplete(obj)
      );
    });
  } else {
...
Zotero.Translate.Web.prototype._translateServerComplete = function (statusCode, response) {
  if (statusCode === 300) {
    ...
    var me = this;
    this._runHandler("select", response.items, function (selectedItems) {
...
Zotero.Translate.Web.prototype.complete = async function (returnValue, error) {
  ...
  if (
    ...
    this.translator[0].inRepository &&
    ...
    var postBody =
      "id=" +
      encodeURIComponent(translator.translatorID) +
      "&lastUpdated=" +
      encodeURIComponent(translator.lastUpdated) +
      "&diagnostic=" +
      encodeURIComponent(info) +
      "&errorData=" +
      encodeURIComponent(errorString);
...
Zotero.Translate.Import.prototype._loadTranslator = function (translator) {
  return Zotero.Translate.Base.prototype._loadTranslator.call(this, translator).then(
    function () {
...
Zotero.Translate.Export.prototype._prepareTranslation = Zotero.Promise.method(function () {
  ...
  function rest() {
    ...
    if (!this.location) {
      ...
      if (this.translator.translatorID == "1412e9e2-51e1-42ec-aa35-e036a895534b" && ZOTERO_CONFIG.ID != "zotero") {
...
Zotero.Translate.Search.prototype.complete = function (returnValue, error) {
  if (
    ...
    Zotero.debug(
      "Translate: Could not find a result using " +
        this.translator[0].label +
        (this.translator.length > 1 ? " -- trying next translator" : ""),
      3
    );
...
Zotero.Translate.IO.String.prototype = {
  ...
  init: function (newMode) {
    ...
    if (newMode === "xml/e4x") {
      ...
      newMode &&
      (Zotero.Translate.IO.rdfDataModes.indexOf(newMode) !== -1 || newMode.substr(0, 3) === "xml/dom") &&
      this._xmlInvalid
    ) {
  ...
  getStatementsMatching: function (subj, pred, obj, objLiteral, justOne) {
    ...
    for (var i = 0; i < statements.length; i++) {
      returnArray.push([
        ...
        statements[i].object.termType == "literal" ? statements[i].object.toString() : statements[i].object
      ]);
    }
    return returnArray;
  }
