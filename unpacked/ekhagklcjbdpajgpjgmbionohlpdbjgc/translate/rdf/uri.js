...
$rdf.Util.uri.join = function (given, base) {
  ...
  if (lastSlash >= 0 && lastSlash < path.length - 1) path = path.slice(0, lastSlash + 1); // Chop trailing filename from base
  path = path + given;
  while (path.match(/[^\/]*\/\.\.\//))
...
$rdf.Util.uri.refTo = function (base, uri) {
  ...
  if (base.slice(0, i).match($rdf.Util.uri.commonHost)) {
    ...
    if (uri.slice(l + 1, l + 2) != "/" && base.slice(l + 1, l + 2) != "/" && uri.slice(0, l) == base.slice(0, l))
  ...
  if (base.indexOf("//", i - 2) > 0 || uri.indexOf("//", i - 2) > 0) return uri; // an unshared '//'
