...
*/
  ...
  __Serializer.prototype.statementsToN3 = function (sts) {
    ...
    function objectTree(obj, stats, force) {
      if (
        ...
        (force || stats.rootsHash[obj.toNT()] == undefined)
      )
        // and not a root
    ...
    function symbolToN3(uri) {
      // c.f. symbolString() in notation3.py
      var j = uri.indexOf("#");
      if (j < 0 && sz.flags.indexOf("/") < 0) {
        j = uri.lastIndexOf("/");
      }
      if (j >= 0 && sz.flags.indexOf("p") < 0) {
        // Can split at namespace
        var canSplit = true;
        ...
        if (canSplit) {
          ...
          if (sz.defaultNamespace && sz.defaultNamespace == namesp && sz.flags.indexOf("d") < 0) {
            // d -> suppress default
            if (sz.flags.indexOf("k") >= 0 && sz.keyords.indexOf(localid) < 0) return localid;
            return ":" + localid;
          }
          ...
          if (uri.slice(0, j) == sz.base) return "<#" + localid + ">";
          // Fall though if can't do qname
        }
      }
      if (sz.flags.indexOf("r") < 0 && sz.base) uri = $rdf.Util.uri.refTo(sz.base, uri);
      else if (sz.flags.indexOf("u") >= 0) uri = backslashUify(uri);
      else uri = hexify(uri);
      return "<" + uri + ">";
    }
    ...
    function prefixDirectives() {
      var str = "";
      if (sz.defaultNamespace) str += "@prefix : <" + sz.defaultNamespace + ">.\n";
  ...
  function backslashUify(str) {
    ...
    for (var i = 0; i < str.length; i++) {
      k = str.charCodeAt(i);
      if (k > 65535) res += "\\U" + ("00000000" + n.toString(16)).slice(-8); // convert to upper?
      else if (k > 126) res += "\\u" + ("0000" + n.toString(16)).slice(-4);
      else res += str[i];
    }
    return res;
  }
  ...
  __Serializer.prototype.statementsToXML = function (sts) {
    ...
    function subjectXMLTree(subject, stats) {
      ...
      if (subject.termType == "bnode") {
        if (!sz.incoming[subject] || sz.incoming[subject].length != 1) {
    ...
    var str = "<rdf:RDF";
    if (sz.defaultNamespace) str += ' xmlns="' + escapeForXML(sz.defaultNamespace) + '"';
    for (var ns in namespaceCounts) {
