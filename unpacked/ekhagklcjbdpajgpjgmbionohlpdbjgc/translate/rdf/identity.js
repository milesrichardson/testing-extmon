...
$rdf.IndexedFormula = (function () {
  ...
  $rdf.IndexedFormula = function (features) {
    ...
    if (features === undefined) features = ["sameAs", "InverseFunctionalProperty", "FunctionalProperty"];
    //    this.features = features
    // Callbackify?
    function handleRDFType(formula, subj, pred, obj, why) {
      if (formula.typeCallback != undefined) formula.typeCallback(formula, obj, why);
  ...
  $rdf.IndexedFormula.prototype.newPropertyAction = function newPropertyAction(pred, action) {
    ...
    if (this.propertyActions[hash] == undefined) this.propertyActions[hash] = [];
...
*/
  ...
  $rdf.IndexedFormula.prototype.replaceWith = function (big, small) {
    ...
    if (big.uri) {
      ...
      if (this.aliases[newhash] == undefined) this.aliases[newhash] = [];
  ...
  function RDFMakeTerm(formula, val, canonicalize) {
    if (typeof val != "object") {
      ...
      if (typeof val == "boolean") return new $rdf.Literal(val ? "1" : "0", undefined, $rdf.Symbol.prototype.XSDboolean);
      if (typeof val == "undefined") return undefined;
      // @@ add converting of dates and numbers
      else throw "Can't make Term from " + val + " of type " + typeof val;
    }
    return val;
  }
  ...
  $rdf.IndexedFormula.prototype.add = function (subj, pred, obj, why) {
    ...
    if (this.predicateCallback != undefined) this.predicateCallback(this, pred, why);
    ...
    var hash = [this.canon(subj).hashString(), this.canon(pred).hashString(), this.canon(obj).hashString(), this.canon(why).hashString()];
...
*/
  $rdf.IndexedFormula.prototype.copyTo = function (template, target, flags) {
    ...
    if ($rdf.Util.ArrayIndexOf(flags, "two-direction") != -1) statList.concat(this.statementsMatching(undefined, undefined, template));
  ...
  $rdf.BlankNode.prototype.copy = function (formula) {
