...
(function () {
  ...
  Object.defineProperty(Term.Collection.prototype, "toString", {
    value: function () {
      var str = "(";
      for (var i = 0; i < this.elements.length; i++) str += this.elements[i] + " ";
      return str + ")";
    }
  });
  ...
  Term.term = function (val) {
      ...
      if (val instanceof Date) {
        ...
        return new Term.Literal(
          "" +
            val.getUTCFullYear() +
            "-" +
            d2(val.getUTCMonth() + 1) +
            "-" +
            d2(val.getUTCDate()) +
            "T" +
            d2(val.getUTCHours()) +
            ":" +
            d2(val.getUTCMinutes()) +
            ":" +
            d2(val.getUTCSeconds()) +
            "Z",
          undefined,
        ...
        for (var i = 0; i < val.length; i++) x.append(Term.term(val[i]));
        return x;
      } else return val;
    if (typeof val == "string") return new Term.Literal(val);
    ...
    if (typeof val == "boolean") return new Term.Literal(val ? "1" : "0", undefined, $rdf.Symbol.prototype.XSDboolean);
  ...
  Term.Formula.prototype.collection = function () {
    // obsolete
  ...
  Term.Literal.prototype.sameTerm = function (other) {
    ...
    return (
      ...
      ((!this.datatype && !other.datatype) || (this.datatype && this.datatype.sameTerm(other.datatype)))
    );
  };
  ...
  Term.Formula.prototype.the = function (s, p, o, w) {
    ...
    if (typeof x == "undefined") $rdf.log("No value found for the(){" + s + " " + p + " " + o + "}.");
    return x;
  };
