...
Zotero.Utilities.Translate.prototype.processDocuments = async function (urls, processor, noCompleteOnError) {
  ...
  for (var i = 0; i < urls.length; i++) {
    if (translate.document && translate.document.location && translate.document.location.toString() === urls[i]) {
...
Zotero.Utilities.Translate.prototype.doGet = function (urls, processor, done, responseCharset, requestHeaders, successCodes) {
  ...
  var xmlhttp = Zotero.HTTP.doGet(
    ...
    responseCharset,
    this._translate.cookieSandbox,
    Object.assign({}, this._translate.requestHeaders, requestHeaders)
  );
};
...
Zotero.Utilities.Translate.prototype.doPost = function (url, body, onDone, headers, responseCharset, successCodes) {
  ...
  var xmlhttp = Zotero.HTTP.doPost(
    url,
    body,
    function (xmlhttp) {
      if (successCodes) {
    ...
    Object.assign({}, translate.requestHeaders, headers),
    responseCharset,
    translate.cookieSandbox ? translate.cookieSandbox : undefined
  );
};
