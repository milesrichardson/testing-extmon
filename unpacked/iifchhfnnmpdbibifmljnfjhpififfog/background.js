...
function isValidApprovedSite(site) {
  if (g_user_approved_sites[site]) return Date.now() < g_user_approved_sites[site] + 24 * 60 * 60 * 1000;
  return false;
}

function set_log_level(level) {
  current_log_level = level;
  if (current_log_level == LOG_LEVEL_DEBUG) cpcsp_console_log(LOG_LEVEL_INFO, "background.js: log_level = DEBUG");
  if (current_log_level == LOG_LEVEL_INFO) cpcsp_console_log(LOG_LEVEL_INFO, "background.js: log_level = INFO");
  if (current_log_level == LOG_LEVEL_ERROR) cpcsp_console_log(LOG_LEVEL_INFO, "background.js: log_level = ERROR");
}
...
function sendNativeMessage(msg) {
  try {
    ...
    var err_message = {
      tabid: msg.tabid,
      data: { requestid: msg.data.requestid, message: "Error sending message to Native Host" },
      type: "error"
    };
...
function connect(_content_port) {
  ...
  g_native_ports[_content_port.name].onDisconnect.addListener(function () {
    ...
    if (g_content_ports[_content_port.name]) g_content_ports[_content_port.name].disconnect();
...
browserInstance.runtime.onConnect.addListener(function (_content_port) {
  ...
  _content_port.onDisconnect.addListener(function () {
    g_content_ports[_content_port.name] = null;
    if (g_native_ports[_content_port.name]) g_native_ports[_content_port.name].disconnect();
    g_native_ports[_content_port.name] = null;
  });
  if (!g_native_ports[_content_port.name]) connect(_content_port);
