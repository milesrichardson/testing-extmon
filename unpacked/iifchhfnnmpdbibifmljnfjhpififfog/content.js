(function () {
  ...
  if (browserSpecs.name === "Opera" || browserSpecs.name === "Chrome") {
    browserInstance = chrome;
  }
    ...
    window.addEventListener(
      ...
      function (event) {
        ...
        try {
          ...
          window.postMessage(
            { tabid: g_tabid, data: { type: "error", requestid: event.data.requestid, message: "Lost connection to extension" } },
            "*"
          );
        }
        cpcsp_console_log(
          LOG_LEVEL_DEBUG,
          "content.js: Sent message to background:" + JSON.stringify({ tabid: g_tabid, data: event.data })
        );
      },
      false
    );
  ...
  function cpcsp_console_log(level, msg) {
    if (level <= current_log_level) {
      if (level === LOG_LEVEL_DEBUG) console.log("DEBUG: %s", msg);
      if (level === LOG_LEVEL_INFO) console.info("INFO: %s", msg);
      if (level === LOG_LEVEL_ERROR) console.error("ERROR: %s", msg);
    }
  }

  function set_log_level(level) {
    current_log_level = level;
    if (current_log_level === LOG_LEVEL_DEBUG) cpcsp_console_log(LOG_LEVEL_INFO, "content.js: log_level = DEBUG");
    if (current_log_level === LOG_LEVEL_INFO) cpcsp_console_log(LOG_LEVEL_INFO, "content.js: log_level = INFO");
    if (current_log_level === LOG_LEVEL_ERROR) cpcsp_console_log(LOG_LEVEL_INFO, "content.js: log_level = ERROR");
  }
  ...
  function connect() {
    if (!isSafari) {
      ...
      if (cadesplugin_not_exists()) return;
      g_bg_port = 1;
      ...
      window.addEventListener("pagehide", function (event) {
        safari.extension.dispatchMessage("messageToExtension", {
          message: JSON.stringify({ tabid: g_tabid, data: { type: "Page reloaded" } })
        });
      });
    }
  }
  ...
  window.addEventListener(
    "message",
    function (event) {
      if (event.source !== window || isSafari) return;
  ...
  window.addEventListener(
    "message",
    function (event) {
      if (event.source !== window || isYandex) return;
      if (event.data === "cadesplugin_echo_request") {
        var answer = "cadesplugin_loaded";
        if (isFireFox) {
          answer += "url:";
