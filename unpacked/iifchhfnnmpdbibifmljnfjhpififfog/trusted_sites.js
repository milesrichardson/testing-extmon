...
function addExample() {
  try {
    if (document.getElementById("example") != null) centralBlock.removeChild(example);
    ...
    exampleDiv.innerHTML =
      "<div>Введена неправильная последовательность подстановочных знаков, некорректный адрес, или добавляемый адрес уже существует.<br>\
    ...
    addErrorMessage(
      "Введена неправильная последовательность подстановочных знаков, некорректный адрес, или добавляемый адрес уже существует.",
      GetErrorMessage(e)
    );
  }
}
...
window.addEventListener(
  "message",
  function (event) {
    if (typeof event.data === "string" && event.data.match("cadesplugin_loaded")) loadNodes();
  },
  false
);
