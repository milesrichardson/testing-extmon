(function () {
  ...
  function set_log_level(level) {
    if (is_cadesplugin_defined()) {
      ...
      if (cadesplugin.current_log_level === cadesplugin.LOG_LEVEL_DEBUG) cpcsp_console_log(cadesplugin.LOG_LEVEL_INFO, "log_level = DEBUG");
      if (cadesplugin.current_log_level === cadesplugin.LOG_LEVEL_INFO) cpcsp_console_log(cadesplugin.LOG_LEVEL_INFO, "log_level = INFO");
      if (cadesplugin.current_log_level === cadesplugin.LOG_LEVEL_ERROR) cpcsp_console_log(cadesplugin.LOG_LEVEL_INFO, "log_level = ERROR");
    }
  }
  ...
  function CallMethod() {
    ...
    for (let i = 1; i < arguments.length; i++) {
      if (typeof arguments[i] === "object") {
        ...
        if (j !== callbackFuncs.length) continue;
    ...
    object_messsage = { destination: "nmcades", requestid: g_request_id, objid: this["objid"], method: arguments[0], params: args };
  ...
  function ReleasePluginObjects() {
    ...
    return requestPromise.then(function (result) {
      if (result.data.value == 0) return true;
      return false;
    });
  }
  ...
  function EnableInternalCSPListener(event) {
    ...
    if (event.data === "EnableInternalCSP_request") {
      if (is_cadesplugin_defined()) window.postMessage("EnableInternalCSP=" + cadesplugin.EnableInternalCSP, "*");
    }
  }
