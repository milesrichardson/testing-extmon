...
var NativeHostManager = {
  ...
  _testNativeHost: function (port, fnResult) {
    if (port) {
      port.sendMessage({ type: "PING" }, function (msgResponse) {
        if (msgResponse && msgResponse.type == "PONG") fnResult(true);
        else fnResult(false);
      });
    }
  },
  ...
  _tryConnectHost: function (hostId, fnResponse) {
    ...
    port.sendMessage(
      { type: "PING", extVersion: extVersion, firstSeen: firstSeen },
      function (msg) {
  ...
  _connect: function (name, fnResponse) {
    ...
    this._tryConnectHost(
      this.HOST_ID_PERUSER,
      function (port) {
        if (port) {
