  ...
  var IETabApi = {
    ...
    _sendRequest: function (request, fnResponse) {
      ...
      if (fnResponse) {
        ...
        window.setTimeout(
          function () {
            if (this.waitingCalls[requestNumber]) delete self.waitingCalls[requestNumber];
          }.bind(this),
    ...
    _init: function () {
      window.addEventListener(
        ...
        function (event) {
          ...
          if (!event.data || !event.data.src || !event.data.type) return;

          // Make sure it's from the content script
          if (event.data.src != "CS") return;
          ...
          if (event.data.type == "IETABAPI_REQUEST") {
            ...
            if (this.onRequest) this.onRequest(event.data.request, fnResponse);
            return;
          }
...
var IETabApi = {
  ...
  onApiRequest: function (event) {
    ...
    if (fnResponse) chrome.runtime.sendMessage(event.data, null, fnResponse);
    else chrome.runtime.sendMessage(event.data);
  },
