...
function Touchscreen(config, dialog, display, network, user) {
  ...
  function processEvent(e) {
    ...
    if (!setTouchPosition(e)) return false;
  ...
  function setTouchPosition(e) {
    ...
    if (
      touchX < 0 ||
      touchY < 0 ||
      touchX > display.getBrowserWidth() + scrollLeft - display.getHorizontalOffset() ||
      touchY > display.getBrowserHeight() + scrollTop - display.getVerticalOffset()
    ) {
