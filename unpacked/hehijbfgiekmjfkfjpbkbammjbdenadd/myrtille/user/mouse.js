...
function Mouse(config, dialog, display, network, user) {
  this.init = function () {
    try {
      user.addListener("mousemove", function (e) {
        mouseMove(e);
      });
      user.addListener("mousedown", function (e) {
        mouseClick(e, 1);
      });
      user.addListener("mouseup", function (e) {
        mouseClick(e, 0);
      });
      user.addListener(
        display.isFirefoxBrowser() ? "DOMMouseScroll" : "mousewheel",
        function (e) {
          mouseScroll(e);
        },
        user.getPassiveEventListeners() ? { passive: true } : false
      );
      user.addListener("selectstart", function () {
        return false;
      });
      user.addListener("contextmenu", function (e) {
        user.cancelEvent(e);
        return false;
      });
    } catch (exc) {
  ...
  function processEvent(e) {
    ...
    if (!setMousePosition(e)) return false;
  ...
  function setMousePosition(e) {
    ...
    if (
      mouseX < 0 ||
      mouseY < 0 ||
      mouseX > display.getBrowserWidth() + scrollLeft - display.getHorizontalOffset() ||
      mouseY > display.getBrowserHeight() + scrollTop - display.getVerticalOffset()
    ) {
  ...
  function mouseMove(e) {
    try {
      ...
      var send = true;
      if (
        config.getMouseMoveSamplingRate() == 5 ||
  ...
  function mouseScroll(e) {
    try {
      ...
      if (e.detail) delta = -e.detail / 3; // firefox
      else if (e.wheelDelta) delta = e.wheelDelta / 120; // others
