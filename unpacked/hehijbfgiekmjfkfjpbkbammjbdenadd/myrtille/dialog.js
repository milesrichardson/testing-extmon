...
function Dialog(config) {
  ...
  this.showStat = function (key, value) {
    ...
    try {
      ...
      statData.Bandwidth =
        ...
        statBandwidthSize +
        " (" +
        (statBandwidthSize > 0 ? Math.round((statBandwidthUsage * 100) / statBandwidthSize) : 0) +
        "%)";
      ...
      statData.NetworkMode =
        statNetworkMode.text +
        (statNetworkMode == config.getNetworkModeEnum().LONGPOLLING ? " (" + config.getLongPollingDuration() / 1000 + "s)" : "");
      ...
      statData.Audio =
        statAudioFormat.text + (statAudioFormat == config.getAudioFormatEnum().NONE ? "" : " (" + statAudioBitrate + " kbps)");
      ...
      if (config.getStatEnabled()) {
        ...
        statDiv.innerText =
          ...
          ", " +
          "MOUSE SAMPLING (%): " +
          statData.MouseMoveSampling +
          ", " +
          "BANDWIDTH (KB/s): " +
          statData.Bandwidth +
          ", " +
          "PERIODICAL FSU (s): " +
          statData.PeriodicalFSU +
          ", " +
          "ADAPTIVE FSU (s): " +
          statData.AdaptiveFSU +
          ", " +
          "NETWORK: " +
          statData.NetworkMode +
          ", " +
          "DISPLAY: " +
          statData.DisplayMode +
          ", " +
          "IMG COUNT: " +
          statData.ImageCount +
          ", " +
          ...
          ", " +
          "FORMAT: " +
          statData.ImageFormat +
          ", " +
          "QUALITY (%): " +
          statData.ImageQuality +
          ", " +
          "QUANTITY (%): " +
          statData.ImageQuantity +
          ", " +
          "MODE: " +
          statData.ImageMode +
          ", " +
  ...
  this.showDebug = function (message) {
    try {
      if (!config.getDebugEnabled() || message == "") return;
  ...
  this.showMessage = function (message, duration) {
    try {
      if (msgDisplayed || message == "") return;
      ...
      if (duration > 0) {
        ...
        msgDivTimeout = window.setTimeout(function () {
          doHideMessage();
        }, duration);
      }
    } catch (exc) {
  ...
  this.showKeyboardHelper = function (text) {
    try {
      if (!config.getKeyboardHelperEnabled() || text == "") return;
      ...
      if (kbhText.length > config.getKeyboardHelperSize()) {
        ...
        kbhTimeout = window.setTimeout(function () {
          doHideKeyboardHelper();
        }, config.getKeyboardHelperTimeout());
      }
    } catch (exc) {
  ...
  function doHideKeyboardHelper() {
    try {
      if (!config.getKeyboardHelperEnabled() || kbhDiv == null) return;
...
this.getShowDialogPopupDesc = function () {
  return showDialogPopupDesc;
};

var showDialogPopupText = null;
this.getShowDialogPopupText = function () {
  return showDialogPopupText;
};

var showDialogPopupSelectText = false;
this.getShowDialogPopupSelectText = function () {
  return showDialogPopupSelectText;
};
