...
function Display(config, dialog) {
  ...
  this.getHorizontalOffset = function () {
    return displayDiv.offsetLeft;
  };
  this.getVerticalOffset = function () {
    return displayDiv.offsetTop;
  };
  ...
  this.init = function () {
    try {
      ...
      if (config.getHostType() == config.getHostTypeEnum().RDP) {
        ...
        dialog.showStat(
          dialog.getShowStatEnum().IMAGE_COUNT_OK,
          config.getDisplayMode() == config.getDisplayModeEnum().CANVAS ? "N/A" : config.getImageCountOk()
        );

        // maximal number of images to display when using divs
        dialog.showStat(
          dialog.getShowStatEnum().IMAGE_COUNT_MAX,
          config.getDisplayMode() == config.getDisplayModeEnum().CANVAS ? "N/A" : config.getImageCountMax()
        );
      } else {
  ...
  function checkWebpSupport() {
    try {
      ...
      var img = new Image();
      img.onload = function () {
        /*dialog.showDebug('webp image loaded');*/ if (img.height <= 0 || img.width <= 0)
          config.setImageEncoding(config.getImageEncodingEnum().AUTO);
      };
      img.onerror = function () {
        /*dialog.showDebug('webp image error');*/ config.setImageEncoding(config.getImageEncodingEnum().AUTO);
      };
          ...
          window.setTimeout(
            function () {
              document.body.style.cursor =
                "url('" + config.getHttpServerUrl() + "GetCursor.aspx?imgIdx=" + idx + "&noCache=" + new Date().getTime() + "'), auto";
            },
            Math.round(config.getAdditionalLatency() / 2)
          );
        } else {
          document.body.style.cursor =
            "url('" + config.getHttpServerUrl() + "GetCursor.aspx?imgIdx=" + idx + "&noCache=" + new Date().getTime() + "'), auto";
  ...
  this.getLastImgCountPerSec = function () {
    return lastImgCountPerSec;
  };

  // last displayed image
  var imgIdx = 0;
  this.getImgIdx = function () {
    return imgIdx;
  };
  ...
  this.addImage = function (idx, posX, posY, width, height, format, quality, fullscreen, data) {
    try {
      ...
      // region or fullscreen image
        ...
        // divs
          ...
          divs.addImage(
            ...
            quality,
            fullscreen,
            config.getImageMode() != config.getImageModeEnum().BINARY ? data : this.bytesToBase64(data)
          );
        }
      ...
      dialog.showStat(
        dialog.getShowStatEnum().IMAGE_SIZE,
        config.getImageMode() != config.getImageModeEnum().BINARY
          ? config.getImageMode() != config.getImageModeEnum().BASE64 || data == ""
            ? "N/A"
            : Math.ceil(data.length / 1024)
