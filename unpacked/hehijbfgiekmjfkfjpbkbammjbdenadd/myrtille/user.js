...
function User(config, dialog, display, network) {
  ...
  this.addListener = function (eventType, listener, useCapture) {
    return eventListener(eventType, listener, useCapture);
  };

  // passive event listeners
  var passiveEventListeners = false;
  this.getPassiveEventListeners = function () {
    ...
    try {
      ...
      if (window.addEventListener) {
        ...
        eventListener = function (eventType, listener, useCapture) {
          if (eventType == "resize") return window.addEventListener(eventType, listener, useCapture);
          else display.getCanvas().getCanvasObject().addEventListener(eventType, listener, useCapture);
        };
  ...
  function browserResize() {
    if (config.getBrowserResize() == config.getBrowserResizeEnum().NONE) return;
    ...
    try {
      ...
      if (
        config.getBrowserResize() == config.getBrowserResizeEnum().SCALE &&
  ...
  this.triggerActivity = function () {
    if (config.getAdaptiveFullscreenTimeout() == 0) return;
