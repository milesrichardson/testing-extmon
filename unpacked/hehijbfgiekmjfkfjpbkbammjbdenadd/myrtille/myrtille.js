...
function processImage(idx, posX, posY, width, height, format, quality, fullscreen, base64Data) {
  try {
    ...
    if (
      config.getDisplayMode() != config.getDisplayModeEnum().CANVAS &&
      display.getImgCount() >= config.getImageCountOk() &&
      !fullscreenPending
    ) {
...
function toggleScaleDisplay() {
  try {
    ...
    network.send(
      network.getCommandEnum().SET_SCALE_DISPLAY.text +
        (config.getBrowserResize() == config.getBrowserResizeEnum().SCALE ? 0 : width + "x" + height)
    );
  } catch (exc) {
...
function toggleReconnectSession() {
  try {
    disableToolbar();
    network.send(
      network.getCommandEnum().SET_RECONNECT_SESSION.text + (config.getBrowserResize() == config.getBrowserResizeEnum().RECONNECT ? 0 : 1)
    );
  } catch (exc) {
  ...
  try {
    ...
    for (var i = 0; i < text.length; i++) {
      if (config.getHostType() == config.getHostTypeEnum().RDP) {
        ...
        keys.push(
          (charCode == 13 ? network.getCommandEnum().SEND_KEY_SCANCODE.text : network.getCommandEnum().SEND_KEY_UNICODE.text) +
            charCode +
            "-1"
        );
        keys.push(
    ...
    if (keys.length > 0) network.processUserEvent("keyboard", keys.toString());
  } catch (exc) {
...
this.sendKey = function (keyCode, release) {
  try {
    ...
    if (keyCode == null || keyCode == "") return;
    ...
    if (release) keys.push(network.getCommandEnum().SEND_KEY_SCANCODE.text + keyCode + "-0");
...
this.sendChar = function (char, release) {
  try {
    ...
    if (release) keys.push(network.getCommandEnum().SEND_KEY_UNICODE.text + charCode + "-0");
  ...
  try {
    if (!pdfLoad) {
      ...
      pdf.src =
        ...
        "PrintDocument.aspx?name=" +
        pdfName +
        "&disposition=" +
        (display.isFirefoxBrowser() ? "attachment" : "inline");
    } else {
      pdf.focus();
