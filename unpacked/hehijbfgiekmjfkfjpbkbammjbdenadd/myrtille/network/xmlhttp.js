  ...
  this.createXhr = function () {
    ...
    if (window.ActiveXObject) {
      try {
        ...
        var MSXMLXMLHTTPPROGIDS = new Array(
          "MSXML2.XMLHTTP.5.0",
          "MSXML2.XMLHTTP.4.0",
          "MSXML2.XMLHTTP.3.0",
          "MSXML2.XMLHTTP",
  ...
  this.send = function (data, startTime) {
    try {
      ...
      if (xhr != null) {
        ...
        if (
          data != null &&
          (data.indexOf(network.getCommandEnum().REQUEST_FULLSCREEN_UPDATE.text) != -1 ||
      ...
      xhr.open(
        ...
        config.getHttpServerUrl() +
          ...
          display.getImgIdx() +
          "&imgReturn=" +
          (config.getNetworkMode() == config.getNetworkModeEnum().XHR ? 1 : 0) +
          "&noCache=" +
          startTime
      );
