...
function Websocket(config, dialog, display, network) {
  ...
  this.newInit = function () {
    try {
      ...
      var wsUrl =
        "wss://" +
        config.remoteHost +
        "/Myrtille/ConnectUser.ashx?sessionId=" +
        ...
        displayHeight +
        "&type=" +
        (config.getImageMode() != config.getImageModeEnum().BINARY ? "text" : "binary");
  ...
  this.init = function () {
    ...
    try {
      ...
      var wsUrl =
        config.getHttpServerUrl().replace("http", "ws") +
        "SocketHandler.ashx?type=" +
        (config.getImageMode() != config.getImageModeEnum().BINARY ? "text" : "binary");
  ...
  function close(e) {
    if (wsOpened && !wsError) {
      ...
      dialog.showDebug(
        "websocket connection closed with error code " +
          e.code +
          " (if you have IIS 8 or greater, ensure the websocket protocol is enabled), falling back to long-polling"
      );
      //config.setNetworkMode(config.getNetworkModeEnum().LONGPOLLING);
      // network.init();
    }
    ...
    try {
      ...
      if (data != null && data != "") {
        ...
        if (network.onMessage) {
          ...
          if (network.onMessage(text)) return;
        }
        ...
        // remote clipboard
          ...
          showDialogPopup(
            "showDialogPopup",
            "ShowDialog.aspx",
            "Ctrl+C to copy to local clipboard (Cmd-C on Mac)",
            text.substr(10, text.length - 10),
            true
          );
        }
        // print job
        ...
        // new image
          ...
          if (
            ...
            config.display.getImgCount() >= config.getImageCountOk() &&
            !fullscreenPending
          ) {
