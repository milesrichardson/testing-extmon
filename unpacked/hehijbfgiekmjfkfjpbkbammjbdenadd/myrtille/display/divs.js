...
function Divs(config, dialog, display) {
  ...
  this.addImage = function (idx, posX, posY, width, height, format, quality, fullscreen, base64Data) {
    try {
      ...
      if (
        config.getImageMode() == config.getImageModeEnum().ROUNDTRIP ||
        base64Data == "" ||
        (display.isIEBrowser() && display.getIEVersion() == 8 && base64Data.length >= 32768)
      ) {
        if (config.getAdditionalLatency() > 0) {
          window.setTimeout(
            function () {
              div.style.backgroundImage =
                "url('" + config.getHttpServerUrl() + "GetUpdate.aspx?imgIdx=" + idx + "&noCache=" + new Date().getTime() + "')";
            },
