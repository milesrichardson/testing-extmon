...
function Network(config, dialog, display) {
  ...
  this.getRoundtripDurationAvg = function () {
    return roundtripDurationAvg == null ? 0 : roundtripDurationAvg;
  };
  ...
  this.getBandwidthUsage = function () {
    return bandwidthUsage;
  };
  this.setBandwidthUsage = function (value) {
  ...
  this.init = function () {
    try {
        ...
        switch (config.getImageMode()) {
          ...
          default:
            config.setImageMode(
              !binaryAvailable
                ? !base64Available
                  ? config.getImageModeEnum().ROUNDTRIP
                  : config.getImageModeEnum().BASE64
                : config.getImageModeEnum().BINARY
            );
        }
      ...
      if (
        false &&
        config.getNetworkMode() == config.getNetworkModeEnum().WEBSOCKET &&
        config.getHostType() == config.getHostTypeEnum().RDP
      ) {
        // audio
  ...
  function tweakDisplay() {
    try {
      ...
      var bandwidthUsageRatio =
        bandwidthUsage != null && bandwidthSize != null && bandwidthSize > 0 ? Math.round((bandwidthUsage * 100) / bandwidthSize) : 0;
      if (bandwidthUsageRatio >= config.getImageTweakBandwidthHigherThreshold()) {
        ...
        bandwidthUsageRatio >= config.getImageTweakBandwidthLowerThreshold() &&
        bandwidthUsageRatio < config.getImageTweakBandwidthHigherThreshold()
      ) {
      ...
      if (
        ...
        (bandwidthUsageRatio >= config.getImageTweakBandwidthLowerThreshold() &&
          bandwidthUsageRatio < config.getImageTweakBandwidthHigherThreshold()) ||
        (latency >= config.getImageTweakLatencyLowerThreshold() &&
          latency < config.getImageTweakLatencyHigherThreshold() &&
          ips >= config.getImageTweakLatencyCountPerSec())
      ) {
    ...
    try {
      ...
      if (config.getAdditionalLatency() > 0) {
        window.setTimeout(
          function () {
            if (config.getNetworkMode() != config.getNetworkModeEnum().WEBSOCKET) {
              xmlhttp.send(data, now);
            } else {
              websocket.send(data, now, excludeImg);
            }
          },
