...
chrome.i18n = (function () {
  ...
  var theI18nObject = {
    ...
    _getL10nData: function () {
      ...
      if (preferences.useCustomLocale && preferences.customLocale != null) result.locales.push(preferences.customLocale);
      ...
      if (navigator.language.length > 2) result.locales.push(navigator.language.substring(0, 2));
      // 3: Set English 'en' as default locale
      if (result.locales.indexOf("en") == -1) result.locales.push("en");
    ...
    getMessage: function (messageID, args) {
      ...
      for (var i = 0; i < l10nData.locales.length; i++) {
        ...
        if (map && messageID in map) return parseString(map[messageID], args);
      }
      return "";
    },
