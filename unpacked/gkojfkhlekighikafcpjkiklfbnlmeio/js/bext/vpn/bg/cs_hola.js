...
(function () {
  ...
  function hola_ext_present() {
    ...
    try {
      ...
      document.documentElement.setAttribute("hola_ext_version", manifest.version);
    } catch (e) {
  ...
  function embed_info(resp) {
    if (!resp) return console.error("ipc got empty response");
    if (!chrome && !web_ext) {
      if ((resp.id != "ping" && resp.id != "callback") || got_info) return;
      got_info = true;
  ...
  function message_cb(e) {
    ...
    if (e.data.id == "cs_ping") return resp_cb(Object.assign({ pong: 1 }, e.data));
    if (chrome) {
      try {
        chrome.runtime.sendMessage(e.data, resp_cb);
      } catch (err) {
        if (err.message.includes("context invalidated")) location.reload();
      }
      return;
    }
    if (web_ext) return browser.runtime.sendMessage(e.data, resp_cb);
    firefox.port.emit("req", e.data);
  }
  window.addEventListener("message", message_cb, false);
  if (chrome) return chrome.runtime.sendMessage({ id: "ping" }, embed_info);
  if (web_ext) return browser.runtime.sendMessage({ id: "ping" }, embed_info);

  function get_info_ff() {
    if (got_info) return;
    if (tries++ > 10) return console.error("failed to get info from extensions");
