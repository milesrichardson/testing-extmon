...
(self["webpackChunk"] = self["webpackChunk"] || []).push([
  [386],
  {
    6386: (module, exports, __webpack_require__) => {
      ...
      (function () {
        ...
        var is_node = typeof module == "object" && module.exports && module.children && typeof __webpack_require__ != "function";
        var is_rn =
        ...
        !((__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(8425), __webpack_require__(6545), __webpack_require__(1291)]),
        (__WEBPACK_AMD_DEFINE_RESULT__ = function (events, array, zutil) {
          var E = Etask;
          var GEN_FN = "GeneratorFunction";
          var etask = Etask;
          var env = process.env,
          ...
          function _cb_post(et, ctx) {
            ...
            if (longcb_ms && ms > longcb_ms) {
              zerr("long cb " + ms + "ms: " + et.get_name() + ", " + et.funcs[et.cur_state].toString().slice(0, 128));
            }
            if (perf_enable) {
          ...
          function Etask(opt, states) {
            if (!(this instanceof Etask)) {
              ...
              if (typeof states == "function" && states.constructor.name == GEN_FN) return E._generator(null, states, opt);
            ...
            for (var i = 0; i < states.length; ++i) {
              ...
              var type = this._get_state_type(func.name, undefined);
              if (type.label) this.states_idx[type.label] = i;
              if (type.finally) {
            ...
            if (opt.spawn_parent) this.spawn_parent(opt.spawn_parent);
            else if (opt.up) opt.up._set_down(this);
            else if (in_run) this._spawn_parent_guess(in_run);
          ...
          E.prototype._root_remove = function () {
            ...
            if (!E.root.delete(this)) assert(0, "etask not in root\n" + E.ps({ MARK: this }));
          };
          ...
          E.prototype._check_free = function () {
            if (this.down || this.child.size) return;
            this._parent_remove();
            this.free = true;
          };
          ...
          E.prototype._complete = function () {
            if (zerr.is(zerr.L.DEBUG)) zerr.debug(this.shortname() + ": close");
          ...
          E.prototype._next = function (rv) {
            ...
            var state = this.at_return ? states.length : this.next_state != -1 ? this.next_state : this.cur_state + 1;
            this.retval = rv && rv.ret;
            this.error = rv && rv.err;
            if (this.error !== undefined) {
              if (zerr.on_exception) zerr.on_exception(this.error, this);
            ...
            } else for (; state < states.length && states[state].aux; state++);
          ...
          E.prototype._handle_rv = function (rv) {
            ...
            if (ret === this.retval);
              ...
              if (typeof ret.then == "function") {
                ...
                ret.then(
                  function (_ret) {
                    _this._got_retval(wait_retval, _ret);
                  },
                  function (err) {
          ...
          E.prototype._run = function () {
            ...
            while (1) {
              ...
              E.in_run.push(this);
              if (zerr.is(zerr.L.DEBUG)) zerr.debug(this.shortname() + ":S" + this.cur_state + ": running");
              if (cb_pre) _cb_ctx = cb_pre(this);
              try {
                rv.ret =
                  this.state0_args && this.cur_state == 0
                    ? this.funcs[this.cur_state].apply(this, this.state0_args)
                    : this.funcs[this.cur_state].call(this, this.error && !this.use_retval ? this.error : this.retval);
              } catch (e) {
                rv.err = e;
                if (rv.err instanceof Error) rv.err.etask = this;
              }
              this.use_retval = false;
              if (cb_post) cb_post(this, _cb_ctx);
              this.running = false;
              E.in_run.pop();
          ...
          E.prototype._set_down = function (down) {
            if (this.down) assert(0, "caller already has a down\n" + this.ps());
            if (down.parent_guess) down._parent_guess_remove();
            assert(!down.parent, "returned etask already has a spawn parent");
            assert(!down.up, "returned etask already has a caller parent, consider using wait_ext");
            down._parent_remove();
            this.down = down;
            down.up = this;
          };
          var state_type_cache = {};
          E.prototype._get_state_type = function (name, on_fail) {
            var type = state_type_cache[name];
            if (type) return type;
            ...
            for (var j = 0; j < f.length; j++) {
              if (f[j] == "try") {
                type.try_catch = true;
                if (j + 1 < f.length && f[j + 1] == "catch") j++;
              } else if (f[j] == "catch") type["catch"] = true;
              else if (f[j] == "finally" || f[j] == "ensure") type.finally = true;
              else if (f[j] == "cancel") type.cancel = true;
              else {
                return void (on_fail || assert.bind(null, false))("unknown func name " + name);
              }
            }
            if (+(type.catch || type.try_catch) + +type.finally + +type.cancel > 1) {
              return void (on_fail || assert.bind(null, false))("invalid multiple state types");
            }
          ...
          E.prototype.spawn = function (child, replace) {
            ...
            if (!replace && child.parent) assert(0, "child already has a parent\n" + child.parent.ps());
          ...
          E.prototype.spawn_parent = function (parent) {
            if (this.up) assert(0, "child already has an up\n" + this.up.ps());
            if (this.tm_completed && !this.parent) return;
            this._parent_remove();
            if (parent && parent.free) parent = undefined;
            if (!parent) return void E.root.add(this);
          ...
          E.prototype.finally = function (cb) {
            if (this.tm_completed) process.nextTick(cb);
            else this.prependListener("finally", cb);
          };
          ...
          E.prototype._set_wait_child = function (wait_retval) {
            ...
            var cond = wait_retval.cond,
              wait_on;
            var rethrow = wait_retval.rethrow;
            assert(!cond || child == "any", 'condition supported only for "any" ' + "option, you can add support if needed");
            if (child == "any") {
              if (!this._get_child_running()) return true;
              wait_on = function () {
                _this.once("child", function (_child) {
                  if (rethrow && E.is_err(_child)) return this._got_retval(wait_retval, _child);
                  if (!cond || cond.call(_child, _child.retval)) return _this._got_retval(wait_retval, { child: _child });
                  if (!_this._get_child_running()) return _this._got_retval(wait_retval);
                  wait_on();
                });
              };
              wait_on();
              ...
              wait_on = function () {
                _this.once("child", function (_child) {
                  if (rethrow && E.is_err(_child)) return _this._got_retval(wait_retval, _child);
                  if (!_this._get_child_running()) return _this._got_retval(wait_retval);
                  wait_on();
                });
              };
              ...
              child.once("finally", function () {
                return _this._got_retval(wait_retval, { child });
              });
            }
          ...
          E.prototype._ecancel = function () {
            ...
            if (this._cancel != -1) return this._call_safe(this.funcs[this._cancel]);
            if (this.cancelable) return this.return();
          };
          E.prototype._ecancel_child = function () {
            if (!this.child.size) return;
            var children = Array.from(this.child.values());
            for (var child of children) child._ecancel();
          };
          ...
          E.prototype._operation_opt = function (opt) {
            if (opt.goto) return { ret: this.goto(opt.goto, opt.ret) };
            if (opt.throw) return { ret: this.throw(opt.throw) };
            if (opt.return !== undefined) return { ret: this.return(opt.return) };
            if (opt.continue !== undefined) return { ret: this.continue(opt.continue) };
          };
          E.prototype.alarm = function (ms, cb) {
            ...
            if (cb && typeof cb != "function") {
              ...
              cb = function () {
                var v;
                if (!(v = _this._operation_opt(opt))) assert(0, "invalid alarm cb opt");
                return v.ret;
              };
            }
            this.del_alarm();
          ...
          Etask_wait.prototype.continue = function (res) {
            ...
            if (!this.et.wait_retval) return void (this.ready = { ret: res });
          ...
          E.prototype.wait_child = function (child, timeout, cond, opt) {
            ...
            return new Etask_wait(this, "wait_child", { timeout, child, cond, rethrow: opt && opt.rethrow });
          };
          ...
          E._res2rv = function (res) {
            return E.is_err(res) ? { ret: undefined, err: res.error } : { ret: res, err: undefined };
          };
          ...
          E.prototype.then = function (on_res, on_err) {
            ...
            if (this.tm_completed)
              return etask("then_completed", [
                function () {
                  return on_done();
                }
              ]);
            var then_wait = etask("then_wait", [
              function () {
                return this.wait();
              }
            ]);
          ...
          E.prototype._ps = function (pre_first, pre_next, flags) {
            ...
            for (var first = 1; et; et = et.down, first = 0) {
              ...
              if (flags.RECURSIVE) {
                ...
                var child = et.child;
                if (flags.GUESS) child = new Set([...child, ...et.child_guess]);
                i = 0;
                for (let child_i of child.values()) {
                  task_trail = i < child.size - 1 ? "|" : stack_trail;
                  child_guess = child_i.parent_guess ? "\\? " : child_i.parent_type == "call" ? "\\> " : "\\_ ";
                  s += child_i._ps(pre_next + task_trail + child_guess, pre_next + task_trail + "   ", flags);
                  i++;
                }
              }
            }
          ...
          function ps_flags(flags) {
            var m, _m;
            if ((m = flags.MARK)) {
              if (!Array.isArray(m)) _m = { sp: [m], name: [] };
              else if (!Array.isArray(flags.MARK[0])) _m = { sp: m, name: [] };
              else {
                _m = { sp: [], name: [] };
          ...
          E.prototype.ps = function (flags) {
            flags = assign({ STACK: 1, RECURSIVE: 1, LIMIT: 1e7, TIME: 1, GUESS: 1 }, flags, { limit_n: 0 });
            ps_flags(flags);
          ...
          E.ps = function (flags) {
            ...
            if (flags.GUESS) {
              ...
              E.root.forEach((root_i) => {
                if (!root_i.parent_guess) child.push(root_i);
              });
            }
          ...
          function assert_tree_unique(a) {
            var i;
            for (i = 0; i < a.length - 1; i++) assert(!a.includes(a[i], i + 1));
          }
          E.prototype._assert_tree = function (opt) {
            ...
            if (this.child_guess.size) assert(E.in_run.includes(this));
          ...
          E.prototype._assert_parent = function () {
            if (this.up) return assert(!this.parent && !this.parent_guess);
            assert(this.parent && this.parent_guess, "parent_guess together with parent");
            if (this.parent) {
              var child = this.parent ? this.parent.child : E.root;
              assert(child.has(this), "cannot find in parent " + (this.parent ? "" : "root"));
            } else if (this.parent_guess) {
              assert(this.parent_guess.child_guess.has(this), "cannot find in parent_guess");
              assert(E.in_run.includes(this.parent_guess));
            }
          };
          E.prototype.return_child = function () {
            var child = Array.from(this.child.values());
            for (var i = 0; i < child.length; i++) child[i].return();
          };
          E.sleep = function (ms) {
            var timer;
            ms = ms || 0;
            return etask({ name: "sleep", cancel: true }, [
              function () {
                this.info.ms = ms + "ms";
          ...
          E.prototype.break = function (ret) {
            return this.throw({ ebreak: ebreak_obj, ret });
          };
          E.for = function (cond, inc, opt, states) {
            ...
            return etask({ name: "for", cancel: true, init: opt && opt.init_parent }, [
              function loop() {
                return !cond || cond.call(this);
              },
              function try_catch$(res) {
                if (!res) return this.return();
                return etask({ name: "for_iter", cancel: true, init: opt && opt.init }, states || []);
              },
              function () {
                if (this.error) {
          ...
          E.for_each = function (obj, states) {
            ...
            var iter = { obj, keys, i: 0, key: undefined, val: undefined };
            function init_iter() {
              this.iter = iter;
            }
            return E.for(
          ...
          E.all = function (a_or_o, ao2) {
            ...
            if (Array.isArray(a_or_o)) {
              ...
              i = 0;
              return etask({ name: "all_a", cancel: true }, [
                function () {
                  for (j = 0; j < a.length; j++) this.spawn(a[j]);
                },
              ...
              var keys = Object.keys(a_or_o),
                o = {};
              i = 0;
              return etask({ name: "all_o", cancel: true }, [
                function () {
                  for (j = 0; j < keys.length; j++) this.spawn(a_or_o[keys[j]]);
                },
                function try_catch$loop() {
          ...
          E.all_limit = function (limit, arr_iter, cb) {
            var at = 0;
            var iter = !Array.isArray(arr_iter)
              ? arr_iter
              : function () {
                  if (at < arr_iter.length) return cb.call(this, arr_iter[at], at++);
                };
            return etask({ name: "all_limit", cancel: true }, [
              function () {
                var next;
                if (!(next = iter.call(this))) return this.goto("done");
                if (E.is_err(next)) return this.throw(next.error);
                var _this = this;
                ...
                this.spawn(next);
                this.loop();
                if (this.child.size >= limit) return this.wait_child("any", { rethrow: true });
              },
              function done() {
                ...
                args.push(function cb(err, res) {
                  ...
                  if (opt.ret_o) {
                    ...
                    if (Array.isArray(opt.ret_o)) {
                      for (i = 0; i < opt.ret_o.length; i++) o[opt.ret_o[i]] = arguments[i + nfn];
                    } else if (typeof opt.ret_o == "string") o[opt.ret_o] = array.slice(arguments, nfn);
                    else assert(0, "invalid opt.ret_o");
                    if (typeof opt.ret_sync == "string") o[opt.ret_sync] = ret_sync;
                    res = o;
                  } else if (opt.ret_a) res = array.slice(arguments, nfn);
                  else if (!nfn) res = err;
          ...
          E.augment = function (_prototype, method, e_method) {
            ...
            if (Array.isArray(method)) {
              ...
              for (i = 0; i < method.length; i++) E.augment(_prototype, opt, method[i]);
              return;
            }
          ...
          E.to_nfn = function (promise, cb, opt) {
            return etask({ name: "to_nfn", async: true }, [
              function try_catch$() {
                return promise;
              },
              function (res) {
                var ret = [this.error];
                if (opt && opt.ret_a) ret = ret.concat(res);
                else ret.push(res);
                cb.apply(null, ret);
              }
            ]);
          ...
          function etask_fn(opt, states, push_this) {
            ...
            var is_gen = typeof states == "function" && states.constructor.name == GEN_FN;
            var arg_start = +push_this;
            var wrap = function () {
              var _opt = assign({}, opt);
              _opt.state0_args = new Array(arg_start + arguments.length);
              _opt.state0_args[0] = this;
              for (var i = 0; i < arguments.length; i++) _opt.state0_args[arg_start + i] = arguments[i];
              if (is_gen) return E._generator(null, states, _opt);
              return new Etask(_opt, typeof states == "function" ? [states] : states);
            };
            if (states.name) {
              Object.defineProperty(wrap, "name", { value: states.name, writable: false });
            }
            return wrap;
          }
          ...
          E._generator = function (gen, ctor, opt) {
            ...
            return new Etask(opt, [
              ...
              function (ret) {
                return this.goto("loop", this.error ? { ret: undefined, err: this.error } : { ret, err: undefined });
              },
              function finally$() {
          ...
          E.ef = function (err, et) {
            if (zerr.on_exception) zerr.on_exception(err, et);
            return err;
          };
          E.interval = function (opt, states) {
            if (typeof opt == "number") opt = { ms: opt };
            if (!opt.mode || opt.mode == "smart") return interval_smart(opt, states);
            if (opt.mode == "fixed") return interval_fixed(opt, states);
            if (opt.mode == "spawn") return interval_spawn(opt, states);
            throw new Error("unexpected mode " + opt.mode);
          };
          function interval_smart(opt, states) {
            var STATE_BEGIN = 0;
            var STATE_OPS = 1;
            var STATE_TIMER = 2;
            var STATE_DONE = 4;
            var state, w, timer, gap_timer;
            var init_parent = function () {
              w = this.wait();
              this.finally(() => clearTimeout(timer));
            };
            const set_state = (for_et, flag) => {
              if ((state = state | flag) != (STATE_OPS | STATE_TIMER)) return;
              state = state | STATE_DONE;
              if (flag == STATE_TIMER) return void for_et.continue();
              if (gap_timer && zutil.is_timer_refresh) gap_timer.refresh();
              else {
            ...
            return E.for(
              function () {
                state = STATE_BEGIN;
                if (timer && zutil.is_timer_refresh) timer.refresh();
                else {
                  clearTimeout(timer);
          ...
          function interval_fixed(opt, states) {
            var w, timer;
            var init_parent = function () {
              w = this.wait();
              this.finally(() => clearTimeout(timer));
          ...
          function interval_spawn(opt, states) {
            var w,
              timer,
              stopped = false;
            var init = function () {
          ...
          E._class = function (cls) {
            ...
            for (var i = 0; i < keys.length; i++) {
              ...
              var p = proto[key];
              if (typeof p == "function" && p.constructor.name == GEN_FN) proto[key] = E._fn(p);
            }
            return cls;
          };
          E.shutdown = function () {
            ...
            while (E.root.size) {
              ...
              if (e == prev) {
                ...
                zerr.zexit("etask root not removed after return - " + "fix non-cancelable child etask");
              }
              prev = e;
              e.return();
            }
          ...
          E.race = function (ets) {
            return etask(function race() {
              if (!Array.isArray(ets)) zerr.zexit("provided argument is not an array");
          ...
          E.any = function (opt, ets) {
            ...
            return etask(function race() {
              ...
              function _child_error_handler(e) {
                errors.push(e);
                if (errors.length != length) return;
                var err = new Error("aggregation error");
                err.errors = errors;
                if (opt.rethrow) race_et.throw(err);
                else race_et.continue();
              }
              var _race = etask([
                function () {
                  return E.for_each(ets, function () {
                    var et = this.iter.val;
                    var child = _this.spawn(
                      etask([
                        function () {
                          this.on("uncaught", _child_error_handler);
                          return et;
                    ...
                    if (opt.timeout) setTimeout(() => child.throw("timeout"), opt.timeout);
                  });
                },
                function () {
                  return race_et;
...
//# sourceMappingURL=https://hola.org/be_source_map/1.218.811/386.bundle.js.map?build=nopeer_v2
