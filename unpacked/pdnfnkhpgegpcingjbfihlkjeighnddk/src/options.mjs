...
function showProxyError(content) {
  $("#proxy_message").html(
    '<div class="alert alert-danger">' + '<button type="button" class="close" data-dismiss="alert">×</button>' + content + "</div>"
  );
}
...
Settings.getCustomProxy().then((serverInfo) => {
  ...
  if (typeof serverInfo !== "undefined" && typeof serverInfo.proc !== "undefined" && typeof serverInfo.addr !== "undefined") {
    customEnabled = true;
...
$("#custom_proxy_enable").click(function () {
  ...
  Settings.setCustomProxy(customProxyProtocol, customProxyAddress).then(() => {
    ...
    showProxyMessage("warning", "Set the custom proxy server, and changed mode to use proxy");
