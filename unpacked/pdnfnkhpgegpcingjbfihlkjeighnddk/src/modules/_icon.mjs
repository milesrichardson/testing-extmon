...
export function setIcon(mode) {
  ...
  if (isSpring()) {
    ...
    chrome.action.setTitle({ title: "Happy Spring Festival! (Unblock Youku " + chrome.runtime.getManifest().version + ")" });
    return;
  } else if (isXmas()) {
    chrome.action.setIcon({ path: ICON_RELATIVE_PATH + "icon19xmas.png" });
    chrome.action.setTitle({ title: "Merry Christmas! (Unblock Youku " + chrome.runtime.getManifest().version + ")" });
    return;
  }
