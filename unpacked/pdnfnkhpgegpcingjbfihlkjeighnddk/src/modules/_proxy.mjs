...
import { DEFAULT_PROXY_PROTOCOL, DEFAULT_PROXY_ADDRESS, BACKUP_PROXY_PROTOCOL, BACKUP_PROXY_ADDRESS } from "../configs/servers.mjs";
import { urls2pac } from "./_url_utils.mjs";

async function setPacScript(defaultProxyProtocol, defaultProxyAddress, backupProxyProtocol, backupProxyAddress) {
  ...
  console.log("Successfully set the PAC script with: " + defaultProxyProtocol + " " + defaultProxyAddress);
}

export function setDefaultProxy() {
  return setPacScript(DEFAULT_PROXY_PROTOCOL, DEFAULT_PROXY_ADDRESS, BACKUP_PROXY_PROTOCOL, BACKUP_PROXY_ADDRESS);
}
...
export function setCustomProxy(customProxyProtocol, customProxyAddress) {
  ...
  return setPacScript(customProxyProtocol, customProxyAddress, customProxyProtocol, customProxyAddress);
}
