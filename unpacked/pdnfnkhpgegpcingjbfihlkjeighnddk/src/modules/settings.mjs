...
function applyModeSettings(mode) {
  if (mode === Modes.OFF) {
    return Promise.all([Proxy.clearProxy(), Header.clearHeaderModifier(), Icon.setIcon(Modes.OFF)]);
  } else {
    const setProxy = async () => {
      const customProxy = await getCustomProxy();
      if (typeof customProxy === "undefined" || typeof customProxy.proc === "undefined" || typeof customProxy.addr === "undefined") {
        return Proxy.setDefaultProxy();
      } else {
        return Proxy.setCustomProxy(customProxy.proc, customProxy.addr);
      }
    };
    return Promise.all([setProxy(), Header.setHeaderModifier(), Icon.setIcon(Modes.NORMAL)]);
  }
}
...
export async function setNewMode(mode) {
  await Promise.all([applyModeSettings(mode), Storage.setItem(MODE_STORAGE_KEY, mode)]);
...
export async function setCustomProxy(customProxyProtocol, customProxyAddress) {
  ...
  console.log("Successfully set the custom proxy server to " + customProxyProtocol + " " + customProxyAddress);
}
