/******/ (() => {
  // webpackBootstrap
  /******/ "use strict";
  var __webpack_exports__ = {}; // CONCATENATED MODULE: ./src/js/common/graph-utils.ts
  ...
  class TSimpleRgn {
    ...
    containsPoint(x, y) {
      for (var i = this.rectangles.length - 1; i >= 0; i--) if (this.rectangles[i].containsPoint(x, y)) return true;
      return false;
    }
    getBoundsRect() {
  ...
  class GraphUtils {
    ...
    static drawTextSingle(ctx, text, options, position, maxWidth, draw = true) {
      ...
      const calcRealWidth = function (element) {
        element.realWidth = Math.max(element.width, element.actualBoundingBoxLeft + element.actualBoundingBoxRight);
      };
      (ctx.font = `${bold} ${italic} ${options.fontSize}px ${options.fontName}`), (ctx.fillStyle = options.fontColor);
      let words = text.trimEnd().split(" ");
      if (!words.length) return { width: 0, height: 0, nextLineY: 0, gap: 0 };
      let lines = [];
    ...
    static drawText(ctx, text, options, position, maxWidth, draw = true) {
      if (text === "") return { width: 0, height: 0, gap: 0, nextLineY: 0 };
  ...
  class HeaderFooter {
    ...
    padString(str, padding, length) {
      str = str.toString();
      while (str.length < length) str = padding + str;
      return str;
    }
    /*********************************************************************************/
    processTemplate() {
    ...
    getURLFromImage() {
      if (this.options.type === "bitmap" && this.options.bitmapLink.length) {
        ...
        if (URL.length && /(^[a-zA-Z]+:\/\/)/.test(URL)) return URL;
  ...
  class PDFPageMetrics {
    ...
    /*********************************************************************************/
    areMarginsValid() {
      let rfMargins = this.getPageMargins(),
    ...
    if (typeof window != "object" || !window.fsWorkerConnection) {
      ...
      return {
        postMessage: function (data) {
          if (isBackgroundPage) setTimeout(() => connection.host({ data: data }, 10));
          else requestAnimationFrame(() => connection.host({ data: data }));
        },
        isWorker: false
      };
    }
  ...
  (function () {
    ...
    function cleanup() {
      if (encoderScript) document.head.removeChild(encoderScript);
    }
    ...
    myImportScripts("scripts/enc/fsEncoder.js")
      .then(() =>
        Module().then((instance) => {
          ...
          function initHandlers(data) {
            let fsHeader = new HeaderFooter(
              new HFWSettings("header", data.headerSettings),
              new HFWVars(data.title, data.URL, data.created, new TSize(data.width, data.height))
            );
            let fsFooter = new HeaderFooter(
              new HFWSettings("footer", data.footerSettings),
              new HFWVars(data.title, data.URL, data.created, new TSize(data.width, data.height))
            );
            let fsWatermark = new Watermark(
              new HFWSettings("watermark", data.wmSettings),
              new HFWVars(data.title, data.URL, data.created, new TSize(data.width, data.height))
            );
