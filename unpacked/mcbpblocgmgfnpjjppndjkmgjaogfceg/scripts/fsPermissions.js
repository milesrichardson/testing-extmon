...
function addHandlers() {
  $("#btnAddPermissions").click(() => {
    addPermissions(permissionsObject, () => {
      alert(chrome.i18n.getMessage("alert_thanks_for_granting_permissions"));
      window.close();
    });
  });
...
chrome.runtime.onMessage.addListener((a, b, c) => {
  "setRequiredPermissions" === a.topic && a.data && (permissionsObject = a.data);
});
document.addEventListener("DOMContentLoaded", function () {
  $(isSafari() ? "imgPermisionsDialogMac" : "#imgPermisionsDialogWindows").show();
  addHandlers();
});
window.addEventListener("unload", function () {
  getExtension().fPermissionsPageOpened = !1;
});
