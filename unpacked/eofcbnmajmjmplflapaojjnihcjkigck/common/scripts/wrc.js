...
(function (_, PROTO) {
  ...
  _.extend(AvastWRC, {
    DEFAULTS: {
      ...
      IGNORE_TABS: {
        ...
        "^moz-extension:": ["ALL"],
        "^ms-browser-extension:": ["ALL"],
        "^chrome-devtools:\\/\\/": ["ALL"],
        "^https:\\/\\/chrome\\.google\\.com\\/webstore": ["ALL"],
        "^about:": ["ALL"],
        "^view-source:": ["ALL"],
        "^file:\\/\\/": ["ALL"],
        "^http(s)?:\\/\\/([\\w|\\d]+:[\\w|\\d]+@)?localhost": ["ALL"],
        "^data:text\\/html": ["ALL"]
      },
  ...
  _.extend(AvastWRC, {
    ...
    init: function () {
      ...
      if (!AvastWRC.CONFIG.OPT_IN.showNextTime) AvastWRC.CONFIG.OPT_IN.showNextTime = false;
    ...
    getStorage: function (storage, callback) {
      ...
      if (this.browserVersion === 0) this.browserVersion = this.getBrowserVersion();
      if (
        AvastWRC.Utils.getBrowserInfo().isSafari() ||
        (AvastWRC.Utils.getBrowserInfo().isFirefox() && this.browserVersion < 48) ||
    ...
    onSendHeaders: function (details) {
      if (details.type === "main_frame") {
        ...
        if (AvastWRC.ASDetector) {
          ...
          AvastWRC.ASDetector.onNavigationEvent(details.tabId, details.url, details.requestId, details.timeStamp);
        }
      }
    },
  ...
  AvastWRC.UrlInfo = function (url, values, multirequest) {
    ...
    if (values.webrep) {
      ...
      this.values = _.extend({}, this.defaults, { ajax: values.ajax }, values.phishing, values.webrep, values.blocker, values.typo, {
        safeShop: values.safeShop
      });
    ...
    this.values.expireTime =
      this.values.phishing > 1 /*phishing*/ ||
      this.values.block === 1 /*malware*/ ||
      (this.values.phishingDomain > 1 &&
        this.values.rating_level === 0) /*There are urls on this domain that makes phishing or have a bad reputation*/
        ? this.setExpireTime(this.values.ttl_phishing)
        : this.setExpireTime(this.values.ttl);
  ...
  AvastWRC.UrlInfo.prototype = {
    ...
    setExpireTime: function (ttl) {
      ...
      return AvastWRC.Utils.dateFormat(new Date(new Date().valueOf() + ttl * 1000), TTL_DATE_FORMAT);
    },
    /**
     * Get expiration time
  ...
  AvastWRC.Utils = {
    ...
    getInstallTime: function (dateAsString) {
      ...
      if (dateAsString !== "") {
        ...
        console.log(
          "getInstallTime -> new string from InstallDate in localstorage: ",
          dateAsString,
          "params to build timestamp: ",
          year,
          month,
          day,
          hours,
...
(function (AvastWRC) {
  ...
  var Browser = {
    ...
    searchString: function (data, isBurgerData) {
      for (var i = 0; i < data.length; i++) {
        ...
        if (dataString) {
          if (dataString.indexOf(data[i].subString) !== -1) return !isBurgerData ? data[i].identity : data[i].identityBurger;
        }
      }
    },
    /**
  ...
  AvastWRC.Utils.getBrowserInfo = function () {
    return {
      ...
      isWindows: function () {
        return (this.os !== null && this.os === "WIN") || (this.osBurger !== null && this.osBurger === "WINDOWS");
      },
      isFirefox: function () {
        return Browser.get("browser") === "FIREFOX";
      },
      isChrome: function () {
        return Browser.get("browser") === "CHROME";
      },
      isEdge: function () {
        return Browser.get("browser") === "MS_EDGE";
      },
      isOpera: function () {
        return Browser.get("browser") === "OPERA";
      },
      isSafari: function () {
        return Browser.get("browser") === "SAFARI";
      },
      isAvast: function () {
        return Browser.get("browser") === "AVAST";
      },
      isAvg: function () {
        return Browser.get("browser") === "AVG";
      },
      isCCleaner: function () {
        return Browser.get("browser") === "CCLEANER";
      },
...
(function (AvastWRC) {
  ...
  AvastWRC.Utils.getClientInfo = function () {
    ...
    clientInfo.referer = "";
    clientInfo.extension_guid = AvastWRC.CONFIG.DEBUG_MODE
      ? "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx"
      : AvastWRC.CONFIG.PLG_GUID !== null
      ? AvastWRC.CONFIG.PLG_GUID
      : "";
    ...
    var cmp = { showABTest: false, campaignId: "default" };
  ...
  AvastWRC.Utils.buildUserSettings = function (settings) {
    ...
    _userSettings.advanced.offers_configs.offer_limit = userSettings.notifications.offers.showAlways
      ? 110
      : userSettings.notifications.offers.showBetter
      ? 99
      : userSettings.notifications.offers.hide
      ? 0
      : 110;
    ...
    _userSettings.advanced.offers_configs.offers_visibility = userSettings.notifications.offers.showAlways
      ? 0
      : userSettings.notifications.offers.showBetter
      ? 1
      : userSettings.notifications.offers.hide
    ...
    _userSettings.advanced.coupons_configs.coupons_visibility = userSettings.notifications.coupons.showAlways
      ? 0
      : userSettings.notifications.coupons.showOnce
      ? 1
      : userSettings.notifications.coupons.hide
      ? 2
      : 0;
  ...
  AvastWRC.Utils.resolveImageUrl = function (primaryImage = "", secondaryImage = "", subcategory = 0) {
    ...
    return {
      barImage:
        secondaryImage !== "" && secondaryImage.indexOf("rfid") !== -1
          ? getImageUrl(imagesMap.rfid)
          : !imagesMap[subcategory.toString()]
          ? getImageUrl(imagesMap["0"])
          : getImageUrl(imagesMap[subcategory.toString()]),
      primaryImage: primaryImage === "" ? "" : primaryImage.indexOf("http") !== -1 ? primaryImage : "",
      secondaryImage:
        secondaryImage === ""
          ? getImageUrl(imagesMap.secondarydefault)
          : secondaryImage.indexOf("http") !== -1
          ? secondaryImage
          : !imagesMap[secondaryImage]
          ? getImageUrl(imagesMap.secondaryDefault)
          : getImageUrl(imagesMap[secondaryImage])
    };
