...
(function (_) {
    ...
    var urlInfo = new Promise((resolve, reject) => {
      AvastWRC.getUrlInfo(urlDetails, (result) => {
        ...
        let response = {
          ...
          isPhishing: AvastWRC.Utils.getBrowserInfo().isAvast()
            ? false
            : (data.values.phishing && data.values.phishing.phishing > 1) || false
        };
    ...
    AvastWRC.bs.tabExists(tabId, function () {
      ...
      switch (changeInfo.status) {
        case "complete":
          ...
          if (
            cachedData &&
            cachedData.transactionFinished &&
            cachedData.scrapers &&
            cachedData.scrapers.length > 0 &&
            lastUrl &&
            lastUrl.url === tab.url &&
          ...
          AvastWRC.TabReqCache.set(tab.id, "safePriceInTab", {
          ...
          console.log(
            "onTabUpdated() Clean the cache of the scraper (new onComplete event)",
            "transactionID: ",
            clientInfo.transaction_id,
            " requestID: ",
            clientInfo.request_id
          );
          AvastWRC.bs.accessContent(tab, {
  ...
  function onClicked(tab) {
    AvastWRC.bs.tabExists(tab.id, function () {
      ...
      var clientInfo =
        cachedData && cachedData.clientInfo && typeof cachedData.clientInfo === "string" ? JSON.parse(cachedData.clientInfo) : null;
      if (closedCoupon && tab.url.indexOf(closedCoupon.closedCouponUrl) !== -1) {
        ...
        var data = { panelData: JSON.parse(JSON.stringify(AvastWRC.bal.sp.panelData)), iconClicked: 1 };
  ...
  _.extend(
    AvastWRC.bs.core, // Browser specific
    {
      /**
      ...
      init: function (balInst) {
        ...
        chrome.webRequest.onSendHeaders.addListener(AvastWRC.onSendHeaders, { urls: ["http://*/*", "https://*/*"] }, ["requestHeaders"]);
      }
    }
  ); // AvastWRC.bs.aos
...
(function (AvastWRC, chrome, _) {
  ...
  function showText(tabId, text, bgcolor) {
    if (tabId < 0) return;
    if (AvastWRC.Utils.getBrowserInfo().isFirefox() && AvastWRC.getBrowserVersion() >= 63)
  ...
  _.extend(
    AvastWRC.bs.icon, // Browser specific
    {
      /**
       * Function called on BAL initialization to initialize the module.
       */
...
(function (_) {
  ...
  _.extend(
    AvastWRC.bs.SP, // Browser specific
    {
      /**
       * Function called on BAL initialization to initialize the module.
       */
